...
function Firebase({contratos,paneles,clientes,gastos,fbConnected,fbLoading,fbError}){
  ...
  // Calcular uso REAL de Firebase Storage leyendo metadata de cada foto
  useEffect(()=>{
    const fotosUrls=gastos.filter(g=>g.foto_url).map(g=>g.foto_url);
    if(fotosUrls.length===0){setStorageStats({bytes:0,count:0,loading:false,error:false});return;}
    // Importar getMetadata dinámicamente (ya está en el bundle)
    import("firebase/storage").then(({getMetadata,ref})=>{
      Promise.allSettled(
        fotosUrls.map(url=>{
          // Convertir URL de descarga a referencia de Storage
          try{
            const sRef=storageRef(storage,decodeURIComponent(url.split("/o/")[1].split("?")[0]));
            return getMetadata(sRef).then(m=>m.size||0);
          }catch{ return Promise.resolve(0); }
        })
      ).then(results=>{
        const totalBytes=results.reduce((a,r)=>a+(r.status==="fulfilled"?r.value:0),0);
        setStorageStats({bytes:totalBytes,count:fotosUrls.length,loading:false,error:false});
      });
    }).catch(()=>setStorageStats(s=>({...s,loading:false,error:true})));
  },[gastos]);
  ...
  // Firebase Storage plan Spark: 5 GB
  const STORAGE_LIMIT=5368709120; // 5 GB
  ...
  return(<div>
    ...
    <Card style={{marginBottom:16}}>
      <SecTit ch="📸 Firebase Storage — Fotos de Boletas"/>
      {storageStats.loading
        ...
        :<div>
          ...
          <div style={{marginBottom:16}}>
            <div style={{display:"flex",justifyContent:"space-between",marginBottom:6}}>
              ...
              <span style={{fontSize:13,fontWeight:800,color:storagePct>80?C.red:storagePct>50?C.amber:C.cyan}}>
                {fmtBytes(storageStats.bytes)} <span style={{fontSize:11,color:C.muted,fontWeight:400}}>de 5 GB</span>
              </span>
            </div>
            <div style={{background:C.border,borderRadius:8,height:10,overflow:"hidden"}}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:14}}>
            {[
              {label:"📁 Capacidad total",val:"5 GB",sub:"Plan Spark gratuito",color:C.cyan},
              {label:"🖼️ Fotos subidas",val:String(storageStats.count),sub:"boletas escaneadas",color:C.green},
              {label:"📦 Espacio usado",val:fmtBytes(storageStats.bytes),sub:"medido en Firebase",color:C.amber},
          ...
          <div style={{padding:"10px 14px",background:"#06B6D418",border:"1px solid #06B6D444",borderRadius:10,display:"flex",gap:10,alignItems:"flex-start"}}>
            <span style={{fontSize:16}}>ℹ️</span>
            <div style={{fontSize:11,color:C.muted,lineHeight:1.6}}>
              <strong style={{color:C.white}}>Firebase Storage (Plan Spark gratuito):</strong> 5 GB de almacenamiento · 1 GB/día descarga · Las fotos se comprimen automáticamente a ≤400px antes de subir para maximizar el espacio disponible.
            </div>
          </div>
        </div>
      }
...
function Gastos({gastos,setGastos}){
  ...
  // ══════════════════════════════════════════════════════════════
  // Google Vision API Key — leída desde variable de entorno de Vercel
  ...
  const escanear=async(file)=>{
    ...
    setOcr({loading:true,progress:20,fase:"Analizando con Google Cloud Vision...",text:"",imgUrl,previewUrl:imgUrl});

    try{
      // ── Fase 1: Enviar directamente a Google Vision (sin preprocesamiento) ──
      // Google Vision maneja imágenes crudas perfectamente — el preprocesamiento
      // local solo agrega latencia sin beneficio real para la API de Google.
      ...
      // ── Fase 2: Extracción inteligente con RegEx ──────────────
      setOcr(o=>({...o,progress:95,fase:"Extrayendo datos estructurados..."}));
      const datosExtraidos=extraerDatosInteligente(textoRaw);

      setOcr(o=>({...o,loading:false,progress:100,fase:"✓ Completado con Google Vision",text:textoRaw,imgUrl,previewUrl:imgUrl}));
...
export default function App(){
  ...
  return(<>
    ...
    <div style={{fontFamily:"'DM Sans',sans-serif",background:C.bg,height:"100dvh",overflowY:"auto",overflowX:"hidden",color:C.text,display:"flex",flexDirection:"column",WebkitOverflowScrolling:"touch"}}>
      <nav className="top-nav" style={{background:"#0A0F1E",borderBottom:"2px solid rgba(255,255,255,0.08)",paddingLeft:16,paddingRight:16,paddingTop:"env(safe-area-inset-top)",display:"flex",alignItems:"center",minHeight:"calc(56px + env(safe-area-inset-top))",position:"sticky",top:0,zIndex:200,gap:2,overflowX:"auto",scrollbarWidth:"none",flexShrink:0}}>
        ...
        <div style={{marginLeft:"auto",display:"flex",alignItems:"center",gap:8,flexShrink:0}}>
          <button onClick={()=>setShowConfig(true)} title="Información de Firebase" style={{background:"#18253D",border:`1px solid ${C.border}`,borderRadius:8,padding:"5px 9px",color:C.muted,cursor:"pointer",fontSize:14,display:"flex",alignItems:"center"}}>⚙️</button>
        </div>
      </nav>

      {/* ── BOTTOM NAV MOBILE ── */}
      <style>{`@media(max-width:640px){.top-nav{display:none!important}.bottom-nav{display:flex!important}}`}</style>
      <nav className="bottom-nav" style={{display:"none",position:"fixed",bottom:0,left:0,right:0,zIndex:300,background:"#0A0F1E",borderTop:"1px solid rgba(255,255,255,0.10)",paddingBottom:"env(safe-area-inset-bottom)",paddingLeft:4,paddingRight:4,overflowX:"auto",scrollbarWidth:"none",WebkitOverflowScrolling:"touch"}}>
        <style>{`.bottom-nav::-webkit-scrollbar{display:none}`}</style>
        {tabs.map(t=>(
          <button key={t.id} onClick={()=>setTab(t.id)} style={{flex:"0 0 auto",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:3,padding:"8px 12px",background:"none",border:"none",cursor:"pointer",color:tab===t.id?"#FFFFFF":"rgba(255,255,255,0.4)",borderTop:tab===t.id?"2px solid #3B82F6":"2px solid transparent",minWidth:56}}>
            <span style={{fontSize:18}}>{t.icon}</span>
            <span style={{fontSize:10,fontWeight:tab===t.id?700:400,whiteSpace:"nowrap"}}>{t.label}</span>
          </button>
        ))}
      </nav>

      <main style={{flex:1,padding:"20px 16px",paddingBottom:"calc(80px + env(safe-area-inset-bottom))",maxWidth:1240,width:"100%",margin:"0 auto",boxSizing:"border-box"}}>
        {error&&(
        ...
        {tab==="firebase"   &&<Firebase   contratos={contratos} paneles={paneles} clientes={clientes} gastos={gastos} fbConnected={!error&&!loading} fbLoading={loading} fbError={!!error}/>}
      </main>
    </div>
  </>);
}
