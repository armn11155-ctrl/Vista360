...
import { initializeFirestore, persistentLocalCache, persistentMultipleTabManager, persistentSingleTabManager, collection, getDocs, addDoc, updateDoc, deleteDoc, doc, orderBy, query, serverTimestamp, setDoc, getDoc, onSnapshot, Timestamp } from "firebase/firestore";
...
// ── ALIAS DE TIPOS — unions estrictos sin escape | string ─────────
// Usar estos tipos en lugar de strings sueltos asegura que TypeScript
// detecte valores inválidos en tiempo de compilación.
type PanelEstado    = "Disponible" | "Ocupado" | "Mantenimiento" | "Libre";
type ClienteEstado  = "Activo" | "Por vencer" | "Inactivo" | "En contacto" | "Propuesta enviada" | "Frío" | "Perdido";
type ClienteTipo    = "Cliente" | "Prospecto";
type Moneda         = "PEN" | "USD";
type FacturaTipo    = "FACTURA" | "BOLETA" | "NOTA_CREDITO" | "NOTA_DEBITO";
/** Coordenada — llega como string de Firestore o number desde geocoding.
 *  Usar internamente solo como `number` mediante `toNumber()`. */
type Coord          = number | string;
/** Timestamp de Firestore o string ISO al hidratarse desde snapshot.
 *  Usar internamente solo como `Date` mediante `toDate()`. */
type FsTimestamp    = Timestamp | string;

// ── HELPERS DE CONVERSIÓN — límite Firestore → dominio interno ────
// Estas funciones son el único lugar donde `Coord` y `FsTimestamp`
// se normalizan. El resto del código trabaja con `number` y `Date`.
//
// Uso:
//   const lat = toNumber(panel.lat);   // number garantizado
//   const creado = toDate(doc.createdAt); // Date garantizada
// ──────────────────────────────────────────────────────────────────

/** Convierte una coordenada Firestore (string o number) a number.
...
/** Convierte un FsTimestamp (Firestore Timestamp, string ISO o null) a Date.
 *  Retorna `new Date()` como fallback si el valor no es parseable. */
...
// ── VIEWPORT SETUP: inyecta TODAS las meta tags en <head> ────────
// Centralizar aquí evita duplicados con el JSX del return y garantiza
// que las etiquetas estén en <head> (no en <body>), que es donde los
// motores de renderizado y los service workers las leen.
function useViewportSetup() {
  useEffect(() => {
    try {
      ...
      // Google Fonts — inyectar en <head> para que el navegador lo descargue
      // en paralelo con el bundle JS, eliminando el FOUT (flash de fuente sin estilo).
      if (!document.querySelector('link[href*="fonts.googleapis.com"]')) {
        // Preconnect — reduce latencia de la conexión TLS
        ...
        // Hoja de estilos de las fuentes
...
// Lee las credenciales desde variables de entorno (RECOMENDADO).
// Define estos valores en tu archivo .env (nunca en el código):
...
//   VITE_CLOUDINARY_CLOUD_NAME=djwhrurww   ← mover aquí en producción
//   VITE_CLOUDINARY_PRESET=vista360_gastos  ← mover aquí en producción
//   VITE_ALLOWED_EMAILS=email1@...,email2@...
//   VITE_EMISOR_RAZON_SOCIAL=Tu Empresa SAC
//   VITE_EMISOR_RUC=20000000000
//   VITE_EMISOR_DIRECCION=Jr. Ejemplo 123
//   VITE_EMISOR_CIUDAD=Lima
...
// persistentMultipleTabManager requiere SharedWorker, que NO existe en
// iOS Safari ni Chrome móvil → causa pantalla blanca en mobile.
// Solución: usar singleTabManager en móviles, multipleTabManager en desktop.
// Guard defensivo: navigator puede no existir en SSR, workers o entornos de test.
const _isMobile = typeof navigator !== "undefined"
  ? /Mobi|Android|iPhone|iPad|iPod/i.test(navigator.userAgent)
  : false;
const db = initializeFirestore(firebaseApp, {
  localCache: persistentLocalCache({
    tabManager: _isMobile
...
// ── Helper de query Firestore — a nivel módulo (no captura estado React) ──
// Intenta construir una query ordenada; si orderBy falla (p.ej. índice
// ausente en entorno dev) cae al fallback sin orden.
// NOTA: definido antes de `fb` para que fb.get() y fb.subscribe() lo usen.
...
const fb = {
  ...
  async post<T extends FirebaseDoc>(
    ...
    // En Firestore, serverTimestamp() solo resuelve su valor real en el servidor.
    // Lo enviamos en el payload para que Firestore lo almacene correctamente,
    // pero en el estado optimista local usamos Timestamp.now() como aproximación.
    // El valor definitivo llega en el siguiente onSnapshot (normalmente < 1 s).
  ...
  async patch<T extends FirebaseDoc>(
    ...
    // NOTA sobre el retorno optimista:
    // body es Partial<T>, por lo que solo contiene los campos modificados.
    // Los campos no tocados quedan como undefined hasta el siguiente onSnapshot.
    // El componente que llame a patch debe actualizar su estado local via
    // setXxx(prev => prev.map(item => item.id === id ? { ...item, ...body } : item))
    // en lugar de reemplazar el item completo con este retorno.
    return [{ ...body, id } as T];
  },
  ...
  /**
   * subscribe — suscripción en tiempo real a una colección con onSnapshot.
   * Devuelve una función de cleanup (llámala en el return del useEffect).
   *
   * Uso:
   *   useEffect(() => {
   *     return fb.subscribe<Gasto>("gastos", items => setGastos(items));
   *   }, []);
   */
  subscribe<T extends FirebaseDoc>(
    ...
    let q;
    try { q = query(collection(db, col), orderBy(ord, "desc")); }
    catch { q = collection(db, col) as ReturnType<typeof collection>; }
    return onSnapshot(
      q,
      snap => onChange(snap.docs.map(d => ({ id: d.id, ...d.data() } as T))),
      err  => onError ? onError(err) : console.error(`[fb.subscribe] ${col}:`, err),
    );
  },
  async uploadImagen(file: File | Blob | string): Promise<string> {
    // Cloudinary unsigned upload — gratis 25GB, sin tarjeta
    // Define en .env: VITE_CLOUDINARY_CLOUD_NAME y VITE_CLOUDINARY_PRESET
    const CLOUD_NAME = import.meta.env.VITE_CLOUDINARY_CLOUD_NAME ?? "djwhrurww";
    const PRESET     = import.meta.env.VITE_CLOUDINARY_PRESET     ?? "vista360_gastos";
    ...
    const webpBlob = await new Promise<Blob>((res, rej) => {
      ...
      img.onload = () => {
        ...
        if (!ctx) { URL.revokeObjectURL(src); rej(new Error("Canvas 2D no disponible (navegador restringido)")); return; }
...
export const dias  = (f: string): number => Math.ceil((new Date(f).getTime() - hoy().getTime()) / 86400000);
...
// ══════════════════════════════════════════════════════════════════
// ✅ VALIDACIÓN — helpers reutilizables para formularios
// ══════════════════════════════════════════════════════════════════
// Uso en guardar():
//   const err = validate.panel(form);
//   if (err) { toast.warn(err); return; }
// ──────────────────────────────────────────────────────────────────

/** Resultado de validación: null = OK, string = mensaje de error */
type ValidationResult = string | null;

/** Tipo para el objeto de sugerencias de geocodificación */
interface GeoSugerencia { label: string; lat: number; lng: number; }

export const validate = {
  /** Validación de RUC peruano con dígito verificador oficial SUNAT (módulo 11).
   *  Referencia: https://www.sunat.gob.pe/legislacion/ruc/
   *  Pesos: [5,4,3,2,7,6,5,4,3,2] sobre los 10 primeros dígitos.
   *  Dígito verificador = 11 - (suma mod 11); casos especiales: 10→0, 11→1. */
  ruc(ruc?: string): ValidationResult {
    if (!ruc) return null; // campo opcional
    ...
    // Dígito verificador módulo 11 — algoritmo oficial SUNAT
  ...
  /** Valida que fin >= inicio para un contrato. */
  ...
  /** Valida un contrato antes de guardarlo. */
  contrato(form: Record<string, unknown>): ValidationResult {
    ...
    const fechaErr = validate.fechasContrato(
      form.inicio as string,
      form.fin    as string,
    );
...
// Contador incremental para IDs de toast — más determinista que Date.now()+Math.random()
// y útil para debugging (los IDs son predecibles: 1, 2, 3...).
let _toastSeq = 0;
const nextToastId = () => ++_toastSeq;
...
function ToastProvider({ children }: { children?: React.ReactNode }) {
  ...
  return (
    ...
    <UIContext.Provider value={ctxValue}>
      {/* Contenido de la app — cualquier componente hijo puede llamar useUI() */}
      {children}
      {/* Toasts — aria-live anuncia cada mensaje a lectores de pantalla */}
      <div
        aria-live="polite"
        aria-atomic="true"
        aria-label="Notificaciones"
        style={{ position: "fixed", bottom: "calc(90px + env(safe-area-inset-bottom))", left: "50%", transform: "translateX(-50%)", zIndex: 9999, display: "flex", flexDirection: "column", gap: 8, alignItems: "center", pointerEvents: "none", width: "min(340px, 92vw)" }}
      >
      ...
      {confirm && (
        <div
          role="presentation"
          onKeyDown={e => { if (e.key === "Escape") { confirm.resolve(false); setConfirm(null); } }}
        ...
        >
          ...
          >
            <div id="confirm-title" style={{ fontSize: 15, fontWeight: 700, color: T.text, marginBottom: 8, lineHeight: 1.4 }}>{confirm.opts?.title || "Confirmar"}</div>
            <div id="confirm-desc"  style={{ fontSize: 13, color: T.muted, marginBottom: 24, lineHeight: 1.5 }}>{confirm.msg}</div>
...
function OfflineBanner() {
  ...
  return (
    <div
      role={isBack ? "status" : "alert"}
      aria-live={isBack ? "polite" : "assertive"}
      aria-label={isBack ? "Conexión restaurada" : "Sin conexión a internet"}
      style={{
      position:   "fixed",
      top:        0,
      left:       0,
...
// ══════════════════════════════════════════════════════════════════
// 📡 HOOK useCollection — suscripción en tiempo real a Firestore
// ══════════════════════════════════════════════════════════════════
// Encapsula el patrón onSnapshot + limpieza en un hook reutilizable.
// Uso desde cualquier componente que necesite datos en tiempo real:
//
//   const { items: gastos, loading, error } = useCollection<Gasto>("gastos");
//
// VENTAJAS vs getDocs (one-time fetch):
//   • Los cambios de otro dispositivo/sesión se reflejan instantáneamente.
//   • El loading state es preciso: false solo cuando llegó el primer snapshot.
//   • El cleanup del useEffect evita memory leaks automáticamente.
// ──────────────────────────────────────────────────────────────────
...
function useCollection<T extends FirebaseDoc>(
  col:     ColName,
  enabled = true,   // false mientras el usuario no esté autenticado
  ord = "createdAt",
  ...
  useEffect(() => {
    ...
    const unsub = fb.subscribe<T>(
      col,
      data => { setItems(data); setError(null); setLoading(false); },
...
function Paneles({ paneles, setPaneles, contratos, loading, setTab, onModalChange }: PanelesProps){
  ...
  const buscarSugerencias=useCallback(async(texto: string)=>{
    ...
    try{
      const r=await fetch(`https://photon.komoot.io/api/?q=${encodeURIComponent(query)}&limit=5&lang=es&bbox=-81.5,-18.5,-68.5,-0.0`, { signal: sugAbortRef.current.signal });
      const d=await r.json();
      ...
      if ((e as {name?:string}).name === "AbortError") return;
      console.warn("[Geocoding] buscarSugerencias falló:", e); setSugerencias([]);
    }
  },[form.ciudad]);
  ...
  const geocodificar=async()=>{
    ...
    try{
      const r=await fetch(`https://photon.komoot.io/api/?q=${enc}&limit=1&lang=es&bbox=-81.5,-18.5,-68.5,-0.0`, { signal: controller.signal });
      const d=await r.json();
      if(d.features?.length>0){
    ...
    try{
      const r=await fetch(`https://nominatim.openstreetmap.org/search?q=${enc}&format=json&limit=1&countrycodes=pe`,{headers:{"User-Agent":"Vista360/1.0","Accept-Language":"es"}, signal: controller.signal});
      const d=await r.json();
      if(d.length>0){
  ...
  return(<div style={{margin:"-20px -16px",minHeight:"100%",background:"#070D1C",paddingBottom:"calc(100px + env(safe-area-inset-bottom))"}}>
    ...
    <Card style={{padding:0,overflow:"hidden",position:"relative",borderRadius:20}}>
      ...
      {sel&&(()=>{
        ...
        const contrato=info;
        const diasVence=contrato?Math.ceil((new Date(contrato.fin).getTime()-Date.now())/86400000):null;
        return(
          <div style={{
            position:"absolute",
...
const fFmt  = fmt;
const fFmtK = (n: number | null | undefined) => { const v=Number(n||0); return v>=1e6?`S/ ${(v/1e6).toFixed(1)}M`:v>=1e3?`S/ ${(v/1e3).toFixed(1)}k`:`S/ ${Math.round(v)}`; };
const fPct  = (n: number, d = 1) => `${n>=0?"+":""}${Number(n||0).toFixed(d)}%`;
const fMesL = (m: string | null | undefined) => { if(!m)return"—"; const [y,mo]=m.split("-"); return["Ene","Feb","Mar","Abr","May","Jun","Jul","Ago","Sep","Oct","Nov","Dic"][+mo-1]+"'"+y.slice(2); };
...
const fBuildMeses = (desde: string, hasta: string): string[] => {
  const ms: string[]=[],cur=new Date(desde+"-01"),end=new Date(hasta+"-01");
...
function useOOHEngine(
  ...
  return useMemo(() => {
    ...
    const ingPorContrato = (c: Contrato) =>
    ...
    const gpPanel: Record<string, number> = {};
    let gastoGen = 0;
          ...
          {([
            ...
            return (
              <button key={m.id} onClick={()=>setModoPeriodo(m.id)} style={{
                ...
                boxShadow: active ? "0 4px 16px rgba(15,23,41,0.4)" : "0 1px 4px rgba(15,23,41,0.08)",
                outline: active ? "none" : "1px solid #E2E8F0",
                transition:"all .15s",
              }}>
                <span style={{color: active ? "rgba(255,255,255,0.8)" : "#1E40AF"}}>{m.ic}</span>
                {m.l}
              </button>
            );
          })}
  ...
  return(<div>
    ...
    <div style={{ background:"linear-gradient(160deg,#080D18 0%,#0D1525 60%,#0F172A 100%)", margin:"-20px -16px 0", padding:"20px 20px 0", borderBottomLeftRadius:0, borderBottomRightRadius:0, boxShadow:"0 8px 28px rgba(0,0,0,0.35)" }}>
      ...
      <div style={{ display:"flex", alignItems:"center", gap:8, marginBottom:16, marginTop:8 }}>
        <select
          value={anioSel}
          onChange={e=>{ setAnioSel(Number(e.target.value)); setMesSel(null); }}
          onMouseDown={e=>e.stopPropagation()} onTouchStart={e=>e.stopPropagation()} onTouchEnd={e=>e.stopPropagation()} onPointerDown={e=>e.stopPropagation()} onClick={e=>e.stopPropagation()}
          style={{ padding:"8px 14px", borderRadius:12, background:"rgba(255,255,255,0.13)", border:"1px solid rgba(255,255,255,0.25)", color:"#fff", fontWeight:700, fontSize:14, cursor:"pointer", touchAction:"manipulation", fontFamily:"inherit", outline:"none", colorScheme:"dark" }}
        >
          {aniosDisp.map(a=><option key={a} value={a} style={{background:"#1E293B",color:"#fff"}}>{a}</option>)}
        </select>
        <select
          value={mesSel===null ? "" : String(mesSel)}
          onChange={e=>{ const v=e.target.value; setMesSel(v===""?null:Number(v)); }}
          onMouseDown={e=>e.stopPropagation()} onTouchStart={e=>e.stopPropagation()} onTouchEnd={e=>e.stopPropagation()} onPointerDown={e=>e.stopPropagation()} onClick={e=>e.stopPropagation()}
          style={{ flex:1, padding:"8px 14px", borderRadius:12, background:"rgba(255,255,255,0.13)", border:"1px solid rgba(255,255,255,0.25)", color:"#fff", fontWeight:600, fontSize:14, cursor:"pointer", touchAction:"manipulation", fontFamily:"inherit", outline:"none", colorScheme:"dark" }}
        >
          <option value="" style={{background:"#1E293B",color:"#fff"}}>Todos los meses</option>
          {MESES_FULL.map((m,i)=><option key={i} value={String(i)} style={{background:"#1E293B",color:"#fff"}}>{m}</option>)}
        </select>
      </div>
      ...
      {(mesSel!==null) && (
        <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", padding:"10px 0 12px" }}>
          <span style={{ fontSize:12, color:"#60A5FA", fontWeight:700 }}>
            {MESES_FULL[mesSel]} {anioEfectivo} · {listaFiltrada.length} contrato{listaFiltrada.length!==1?"s":""}
          </span>
        </div>
      )}
    </div>
                    ...
                    <div style={{ position: "relative", zIndex: 2 }}>
                      ...
                      {(isEmitible || isCobrable || f.pdf_url) && (
                        <div style={{ display: "flex", gap: 8, justifyContent: "flex-end", flexWrap: "wrap" }}>
                          ...
                          {f.pdf_url && (
                            <button onClick={(e) => { e.stopPropagation(); window.open(f.pdf_url, "_blank", "noopener,noreferrer"); }}
                              style={{
...
function ResumenNuevo({ clientes, contratos, paneles, gastos, setTab, userName = "" }: ResumenNuevoProps) {
  ...
  const fmtS = (n: number | null | undefined) => `$${Number(n || 0).toLocaleString("es-PE", { minimumFractionDigits: 0 })}`;
...
// ── Helpers de formato ──
// fmtS2: alias de fmt (mismo formato S/ es-PE 2 decimales) — evita duplicación
const fmtS2 = fmt;
const fmtK  = (n: number | null | undefined) => { const v=Number(n||0); return v>=1000?`S/ ${(v/1000).toFixed(1)}k`:`S/ ${v.toLocaleString("es-PE",{minimumFractionDigits:0})}`; };
...
function Capital({ paneles, contratos, gastos, proveedores }: CapitalProps) {
  ...
  if (totalActivos>totalDeudas*2) insights.push({icon:"◆",text:"Tus activos duplican tus deudas — solidez financiera",color:"#60A5FA"});
  if (ingMes>gastosMes*1.5) insights.push({icon:"✓",text:"Flujo positivo: ingresas más del doble de lo que gastas",color:"#60A5FA"});

  // ── UI helpers ──
  const TABS = [
    {id:"patrimonio",label:"Patrimonio",icon:<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><path d="M3 9l9-7 9 7v11a2 2 0 01-2 2H5a2 2 0 01-2-2z"/><polyline points="9 22 9 12 15 12 15 22"/></svg>},
    {id:"liquidez",  label:"Liquidez",  icon:<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><path d="M12 2C6 8 4 12 4 15a8 8 0 0016 0c0-3-2-7-8-13z"/></svg>},
    {id:"activos",   label:"Activos",   icon:<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><rect x="2" y="7" width="20" height="14" rx="2"/><path d="M16 7V5a2 2 0 00-2-2h-4a2 2 0 00-2 2v2"/></svg>},
    {id:"fondos",    label:"Fondos",    icon:<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><line x1="12" y1="1" x2="12" y2="23"/><path d="M17 5H9.5a3.5 3.5 0 000 7h5a3.5 3.5 0 010 7H6"/></svg>},
    {id:"objetivos", label:"Objetivos", icon:<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><circle cx="12" cy="12" r="10"/><circle cx="12" cy="12" r="6"/><circle cx="12" cy="12" r="2"/></svg>},
  ];
      ...
      <CapCard style={{padding:"18px 20px",marginBottom:12}}>
        ...
        {data.cuentas.length===0?(
          ...
          <div key={c.id} style={{display:"flex",alignItems:"center",gap:12,padding:"12px 0",borderBottom:"1px solid rgba(255,255,255,0.07)"}}> 
            <div style={{width:40,height:40,borderRadius:11,background:"rgba(59,130,246,0.10)",border:"1px solid rgba(16,185,129,0.22)",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#3B82F6" strokeWidth="1.8" strokeLinecap="round"><line x1="3" y1="22" x2="21" y2="22"/><line x1="6" y1="18" x2="6" y2="11"/><line x1="10" y1="18" x2="10" y2="11"/><line x1="14" y1="18" x2="14" y2="11"/><line x1="18" y1="18" x2="18" y2="11"/><polygon points="12 2 20 7 4 7"/></svg></div>
            <div style={{flex:1,minWidth:0}}>
  ...
  const catIcon = {panel:"PAN",equipo:"EQP",vehiculo:"VEH",inmueble:"INM",herramienta:"HER",otro:"OTR"};
          ...
          return (
            <div>
              ...
              <div style={{display:"flex",flexWrap:"wrap",gap:6}}>
                {Object.entries(porCat).map(([cat,val],i)=>{
                  const cs=["#3B82F6","#3B82F6","#8B5CF6",T.amber,T.red,"#EC4899"];
                  return <span key={cat} style={{fontSize:10,color:cs[i%cs.length],background:`${cs[i%cs.length]}18`,borderRadius:99,padding:"2px 8px",fontWeight:600}}>{catIcon[cat]||"OTR"} {fmtK(val)}</span>;
                })}
              </div>
            </div>
          );
...
function NotifPanel({ open, onClose, contratos, clientes, paneles, gastos }: NotifPanelProps) {
  ...
  const marcarTodas = () => { const t: Record<string, boolean> = {}; notifs.forEach(n => { t[n.id] = true; }); setLeidas(t); };
...
// ══════════════════════════════════════════════════════════════════
interface ErrorBoundaryState { hasError: boolean; message: string; retryCount: number; }
  ...
  render() {
    ...
    return (
      <div style={{
        ...
        {canRetry ? (
          <button
            onClick={() => this.setState(s => ({ hasError: false, message: "", retryCount: s.retryCount + 1 }))}
            style={{
          ...
          <div style={{ fontSize: 12, color: T.muted }}>
            Refresca la página para continuar.
          </div>
        )}
      </div>
    );
