...
function Historico({contratos,setContratos,paneles,clientes,onModalChange}){
  ...
  // Años disponibles (de mayor a menor)
  const aniosDisp=useMemo(()=>{
    const set=new Set();
    todos.forEach(c=>{ if(c.inicio) set.add(parseInt(c.inicio.slice(0,4),10)); });
    const anioActual=new Date().getFullYear();
    set.add(anioActual);
    return Array.from(set).sort((a,b)=>b-a);
  },[todos]);

  // Si el año seleccionado no está disponible, usar el primero
  const anioEfectivo=aniosDisp.includes(anioSel)?anioSel:(aniosDisp[0]||new Date().getFullYear());

  // Contratos del año seleccionado (por mes de inicio)
  const contratosAnio=todos.filter(c=>c.inicio&&parseInt(c.inicio.slice(0,4),10)===anioEfectivo);
  ...
  // Contratos a mostrar según filtro de mes
  const listaFiltrada=mesSel!==null
    ? contratosAnio.filter(c=>c.inicio&&parseInt(c.inicio.slice(5,7),10)-1===mesSel)
    : contratosAnio;
  ...
  return(<div>
    ...
    <div style={{ background:"linear-gradient(160deg,#080D18 0%,#0D1525 60%,#0F172A 100%)", margin:"-20px -16px 0", padding:"20px 20px 0", borderBottomLeftRadius:0, borderBottomRightRadius:0, boxShadow:"0 8px 28px rgba(0,0,0,0.35)" }}>
      ...
      <div style={{ display:"flex", alignItems:"center", gap:8, marginBottom:16, marginTop:8 }}>
        {aniosDisp.map(a=>(
          <button key={a} onClick={()=>{ setAnioSel(a); setMesSel(null); }} style={{
