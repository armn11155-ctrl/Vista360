...
function Resultados({ contratos, paneles = [], clientes = [], gastos = [], loading }: ResultadosProps) {
  ...
  const applyPeriodo = useCallback((p) => {
    const hs = fMesActual();
    let ds = hs;
    if(p==="3m")  ds=fNMesesAtras(3);
    if(p==="6m")  ds=fNMesesAtras(6);
    if(p==="12m") ds=fNMesesAtras(12);
    if(p==="ytd") ds=`${new Date().getFullYear()}-01`;
    setPeriodo(p); setDesde(ds); setHasta(hs);
  },[]);
  ...
  const PERIODOS = [{id:"3m",l:"3M"},{id:"6m",l:"6M"},{id:"ytd",l:"YTD"},{id:"12m",l:"12M"},{id:"custom",l:"…"}];
    ...
    <div style={{color:F.text}}>
      ...
      <div style={{marginBottom:16}}>
        ...
        <div style={{display:"flex",alignItems:"flex-start",justifyContent:"space-between",marginBottom:10}}>
          <div>
            <div style={{fontSize:21,fontWeight:900,color:F.text,letterSpacing:"-0.03em",lineHeight:1}}>Estado de Resultados</div>
            <div style={{fontSize:11,color:F.muted,marginTop:3}}>
        ...
        <div style={{display:"flex",gap:5,marginBottom:10}}>
          {PERIODOS.map(p=>(
            <button key={p.id} onClick={()=>{ if(p.id!=="custom") applyPeriodo(p.id); else setPeriodo("custom"); }} style={{
              flex:1,padding:"7px 0",borderRadius:9,
              background:periodo===p.id?"rgba(59,130,246,0.18)":"rgba(255,255,255,0.05)",
              border:`1px solid ${periodo===p.id?F.blueBd:F.border}`,
              color:periodo===p.id?F.blue:F.muted,fontSize:12,fontWeight:periodo===p.id?700:500,
              cursor:"pointer",touchAction:"manipulation",fontFamily:"inherit",
            }}>{p.l}</button>
          ))}
        </div>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:10}}>
            {[{l:"Desde",v:desde,s:setDesde},{l:"Hasta",v:hasta,s:setHasta}].map(f=>(
              <div key={f.l}>
                ...
                <input type="month" value={f.v} onChange={e=>f.s(e.target.value)} style={{width:"100%",background:"rgba(255,255,255,0.06)",border:`1px solid ${F.border2}`,borderRadius:9,padding:"9px 11px",color:F.text,fontSize:13,fontFamily:"inherit",outline:"none",boxSizing:"border-box",colorScheme:"dark"}}/>
              </div>
            ))}
          </div>
        )}
        ...
        <div>
          ...
          <div style={{display:"flex",flexDirection:"column",gap:10}}>
            {mesesAnio.map(m => {
              ...
              const ctrsDelMes = contratos.filter(c =>
                c.inicio && c.fin &&
                c.inicio.slice(0,7) <= m.mes && c.fin.slice(0,7) >= m.mes
              );
              const ctrsConDetalle = ctrsDelMes.map(c => ({
                ...c,
                panel: paneles.find(p=>p.id===c.panel_id),
                cliente: clientes.find(cl=>cl.id===c.cliente_id),
              })).filter(c=>c.panel&&c.cliente);
                  ...
                  <div style={{display:"flex",alignItems:"flex-start",gap:12}}>
                    ...
                    <div style={{flex:1,minWidth:0}}>
                      ...
                      <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:10,flexWrap:"wrap"}}>
                        <span style={{fontSize:15,fontWeight:900,color:T.white,letterSpacing:"-0.2px"}}>
                          {mesLabel(m.mes)}
                        </span>
                        <span style={{
                        ...
                        {margen > 0 && (
                          <span style={{
                            ...
                            borderRadius:8,padding:"2px 9px",fontSize:10.5,fontWeight:700,whiteSpace:"nowrap",
                          }}>{margen}% margen</span>
                        )}
                        <button onClick={()=>exportMesPDF(m)} style={{
                          ...
                          background:"linear-gradient(135deg,#1E35C8 0%,#3854EE 100%)",
                          color:T.white,fontWeight:700,fontSize:12,cursor:"pointer",touchAction:"manipulation",fontFamily:"inherit",
                      ...
                      {/* 4 columnas: COBRADO / PENDIENTE / GASTOS / UTILIDAD */}
                      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr 1fr",gap:6,marginBottom:10}}>
                        <div>
                          <div style={{fontSize:8.5,fontWeight:700,color:"#5B7FCC",letterSpacing:0.8}}>COBRADO</div>
                          <div style={{fontSize:12.5,fontWeight:900,color:T.green,fontFamily:"monospace",marginTop:2,fontVariantNumeric:"tabular-nums"}}>{fmt(m.ingPagado)}</div>
                        </div>
                        <div>
                          <div style={{fontSize:8.5,fontWeight:700,color:"#5B7FCC",letterSpacing:0.8}}>PENDIENTE</div>
                          <div style={{fontSize:12.5,fontWeight:900,color:pendiente>0?T.amber:"rgba(160,180,220,0.4)",fontFamily:"monospace",marginTop:2,fontVariantNumeric:"tabular-nums"}}>{fmt(pendiente)}</div>
                        </div>
                        <div>
                          <div style={{fontSize:8.5,fontWeight:700,color:"#5B7FCC",letterSpacing:0.8}}>GASTOS</div>
                          <div style={{fontSize:12.5,fontWeight:900,color:T.red,fontFamily:"monospace",marginTop:2,fontVariantNumeric:"tabular-nums"}}>{fmt(m.gastosMes)}</div>
                        </div>
                        <div>
                          <div style={{fontSize:8.5,fontWeight:700,color:"#5B7FCC",letterSpacing:0.8}}>UTILIDAD</div>
                          <div style={{fontSize:12.5,fontWeight:900,color:m.utilidad>=0?T.green:T.red,fontFamily:"monospace",marginTop:2,fontVariantNumeric:"tabular-nums"}}>{fmt(m.utilidad)}</div>
                        </div>
                      </div>
                      ...
                      {ctrsConDetalle.length > 0 && (
                        <button onClick={()=>setExpandido(e=>!e)} style={{
                          ...
                          borderRadius:10,padding:"7px 12px",cursor:"pointer",touchAction:"manipulation",fontFamily:"inherit",
                        }}>
                          <span style={{fontSize:11,fontWeight:600,color:"#7FAEFF"}}>
                            {expandido?"Ocultar":"Ver"} contratos del mes ({ctrsConDetalle.length})
                          </span>
                          <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#7FAEFF" strokeWidth="2.5" strokeLinecap="round" style={{transform:expandido?"rotate(180deg)":"none",transition:"transform .2s"}}>
                            <polyline points="6 9 12 15 18 9"/>
                          </svg>
                        </button>
                      )}

                      {/* Lista de contratos del mes (expandible) */}
