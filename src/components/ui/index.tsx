...
export const SkPulse = ({ w="100%", h=16, r=8, style={} }: SkPulseProps) => (
  <div style={{ width:w, height:h, borderRadius:r,
    ...
    backgroundSize:"200% 100%", animation:"skPulse 1.4s ease infinite", flexShrink:0, ...style }}/>
);
export const SkCard = () => (
  <div style={{ background:"#fff", borderRadius:20, padding:20, border:"1px solid #E5E7EB", marginBottom:14 }}>
    <div style={{ display:"flex", alignItems:"center", gap:12, marginBottom:16 }}>
      ...
      <div style={{ flex:1, display:"flex", flexDirection:"column", gap:8 }}>
        <SkPulse h={14} r={6}/><SkPulse w="60%" h={11} r={6}/>
      </div>
    </div>
    <SkPulse h={11} r={6} style={{ marginBottom:8 }}/><SkPulse w="80%" h={11} r={6}/>
  </div>
);
...
const badgeStyle = (c: string): React.CSSProperties => ({
  ...
  background:`${c}18`, color:c, fontSize:11, fontWeight:700, letterSpacing:"0.02em",
});
...
const cardBaseStyle: React.CSSProperties = {
  background:T.white, borderRadius:20, padding:20, border:`1px solid ${T.border}`,
  boxShadow:"0 1px 4px rgba(15,23,41,0.06)", marginBottom:14,
};
export const Card   = ({ children, style={} }: CardProps) => <div style={{ ...cardBaseStyle, ...style }}>{children}</div>;
...
// ── PAGE TITLE ────────────────────────────────────────────────────
interface PgTitProps { icon?: string; title: string; sub?: string; dark?: boolean; }
export const PgTit = ({ icon, title, sub, dark }: PgTitProps) => (
  <div style={{ marginBottom:20 }}>
    {icon && <span style={{ fontSize:28, display:"block", marginBottom:6 }}>{icon}</span>}
    <div style={{ fontSize:22, fontWeight:800, color: dark ? T.white : T.text, letterSpacing:"-0.5px" }}>{title}</div>
    {sub && <div style={{ fontSize:13, color: dark ? "rgba(255,255,255,0.55)" : T.muted, marginTop:4 }}>{sub}</div>}
  </div>
);
...
export function Modal({ title, onClose, onSave, saveLabel="Guardar ✓", children }: ModalProps) {
  return (
    <div role="dialog" aria-modal="true" aria-label={title}
      ...
      <div style={{ background:T.white, borderRadius:"20px 20px 0 0", padding:"24px 20px",
        ...
        <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:20 }}>
          ...
          <button onClick={onClose} style={{ background:"none", border:"none", fontSize:20, cursor:"pointer",
            color:T.muted, padding:4, touchAction:"manipulation" }}>✕</button>
        </div>
        {children}
        {onSave && (
          <button onClick={onSave}
            style={{ width:"100%", marginTop:20, padding:"15px", background:T.accent, color:"#fff",
  ...
  return (
    <div style={{ marginBottom:16 }}>
      <label style={{ display:"block", fontSize:12, fontWeight:700, color:T.muted,
        marginBottom:6, textTransform:"uppercase", letterSpacing:"0.06em" }}>{label}</label>
      {children}
    </div>
  );
}
...
interface PaginationProps {
  ...
  total: number; pageSize: number; dark?: boolean;
}
export function Pagination({ page, totalPages, setPage, total, pageSize, dark=true }: PaginationProps) {
  if (totalPages <= 1) return null;
  const from = (page-1)*pageSize+1, to = Math.min(page*pageSize, total);
  const bg = dark?T.dark:T.white, border = dark?"rgba(59,130,246,0.22)":T.border;
  ...
  const btn = (label: string|number, onClick: ()=>void, active=false, disabled=false) => (
    <button key={String(label)+String(active)} onClick={onClick} disabled={disabled}
      style={{ minWidth:32, height:32, borderRadius:8, border:"none",
        ...
        touchAction:"manipulation", display:"flex", alignItems:"center", justifyContent:"center", padding:"0 6px" }}>
      {label}
    </button>
  );
  return (
    <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", gap:8,
      marginTop:20, padding:"10px 14px", background:bg, border:`1px solid ${border}`, borderRadius:16 }}>
      <span style={{ fontSize:11, color:text, fontWeight:600, flexShrink:0 }}>{from}–{to} de {total}</span>
...
interface SwipeRowProps {
  children: React.ReactNode; onDelete: ()=>void; onEdit?: ()=>void; deleteLabel?: string;
}
  ...
  return (
    <div style={{ position:"relative", overflow:"hidden", borderRadius:16, marginBottom:10 }}>
      <div style={{ position:"absolute", right:0, top:0, bottom:0, display:"flex", alignItems:"stretch" }}>
        {onEdit && (
          <button onClick={onEdit} style={{ background:T.accent, color:"#fff", border:"none",
            padding:"0 16px", cursor:"pointer", fontSize:12, fontWeight:700, touchAction:"manipulation" }}>
            Editar
          </button>
        )}
        <button onClick={onDelete} style={{ background:T.red, color:"#fff", border:"none",
          padding:"0 16px", cursor:"pointer", fontSize:12, fontWeight:700, touchAction:"manipulation" }}>
          {deleteLabel}
        </button>
      </div>
      <div onClick={() => setOpen(o => !o)}
        style={{ transform:open?"translateX(-140px)":"translateX(0)", transition:"transform 0.2s ease",
...
export function OfflineBanner() {
  return (
    <div style={{ background:T.amber, color:"#fff", textAlign:"center", padding:"8px 16px",
      fontSize:13, fontWeight:600, position:"fixed", top:0, left:0, right:0, zIndex:9000 }}>
