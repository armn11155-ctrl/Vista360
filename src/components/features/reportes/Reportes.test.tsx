...
vi.mock("../../../config/constants", () => ({
  CIUDADES:["Lima"], CAT_GASTOS:["Mantenimiento","Personal"], CAT_PROVE:["Elect"],
  SECTORES:["Retail"], ESTADOS_CLI:["Activo"], ESTADOS_PRO:["En contacto"],
  EMOJIS:["🖥"], EMISOR:{ruc:"20123",nombre:"Test SA"}, ALLOWED_EMAILS:[], BOTTOM_TABS_LIST:[],
}));
...
const props = { contratos:[], paneles:[], clientes:[], gastos:[], initialSeccion: undefined };

beforeEach(()=>vi.clearAllMocks());

describe("Reportes", () => {
  it("renderiza sin errores", () => {
    expect(() => render(<Reportes {...props} />)).not.toThrow();
  });
  it("muestra contenido", () => {
    render(<Reportes {...props} />);
...
describe("Reportes — interacciones UI básicas", () => {
  it("responde a clicks e inputs sin lanzar error", () => {
    const { container } = render(<Reportes {...props} />);
