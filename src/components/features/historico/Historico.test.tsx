...
describe("Historico — con contratos", () => {
  const contratos = [
    ...
    },
    {
      ...
      monto: 2000,
      pagado: true,
      pagosMeses: { "2024-03": true },
    },
  ];
  const paneles = [
    { id: "p1", nombre: "Panel A", tipo: "LED", ciudad: "Lima", estado: "Disponible" as const },
  ];
  const clientes = [{ id: "cl1", empresa: "Acme SA", estado: "Activo" as const }];

  it("renderiza con datos e interactúa con botones", () => {
    const { container } = render(
      <Historico {...props} contratos={contratos} paneles={paneles} clientes={clientes} />,
    );
    container.querySelectorAll("button").forEach(btn => {
      try {
        fireEvent.click(btn);
    ...
    container.querySelectorAll("input, select").forEach(el => {
      try {
        fireEvent.change(el, { target: { value: "2024" } });
      } catch {}
    });
    expect(document.body).toBeTruthy();
  });
