...
describe("PanelCard", () => {
  it("muestra el nombre del panel", () => {
    render(<PanelCard panel={mockPanel} ocupado={false} onEdit={vi.fn()} onDelete={vi.fn()} />);
  ...
  it("muestra 'Ocupado' cuando está ocupado", () => {
    render(<PanelCard panel={mockPanel} ocupado={true} onEdit={vi.fn()} onDelete={vi.fn()} />);
    expect(screen.getByText("Ocupado")).toBeTruthy();
  });

  it("llama a onEdit cuando se hace clic en Editar", () => {
    const onEdit = vi.fn();
    render(<PanelCard panel={mockPanel} ocupado={false} onEdit={onEdit} onDelete={vi.fn()} />);
  ...
  it("llama a onDelete con el id correcto al eliminar", () => {
    ...
    render(<PanelCard panel={mockPanel} ocupado={false} onEdit={vi.fn()} onDelete={onDelete} />);
