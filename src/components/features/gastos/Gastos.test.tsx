...
vi.mock("firebase/firestore", () => ({
  ...
  addDoc: vi.fn().mockResolvedValue({ id: "x" }), updateDoc: vi.fn(),
  deleteDoc: vi.fn(), onSnapshot: vi.fn().mockReturnValue(vi.fn()),
  doc: vi.fn(), query: vi.fn((...a) => a), orderBy: vi.fn(), where: vi.fn(),
...
vi.mock("../../../config/theme", () => ({
  T: { bg:"#0F172A",card:"#1E293B",surface:"#1E293B",border:"#334155",text:"#F1F5F9",
       muted:"#94A3B8",accent:"#3B82F6",red:"#EF4444",green:"#10B981",
       white:"#fff",dark:"#0F172A",yellow:"#F59E0B" },
  tCol: vi.fn(() => "#3B82F6"), catCol: vi.fn(() => "#3B82F6"),
}));
vi.mock("../../../config/constants", () => ({
  CIUDADES: ["Lima"], CAT_GASTOS: ["Mantenimiento","Personal"],
  CAT_PROVE: ["Electricidad"], SECTORES: ["Retail"],
  ESTADOS_CLI: ["Activo"], ESTADOS_PRO: ["En contacto"],
  EMOJIS: ["🖥"], EMISOR: { ruc:"20123456789" },
  ALLOWED_EMAILS: [], BOTTOM_TABS_LIST: [],
}));
...
const props = { gastos: [], setGastos: vi.fn(), autoScan: false, setAutoScan: vi.fn(), onModalChange: vi.fn() };
...
describe("Gastos", () => {
  ...
  it("renderiza con un gasto en la lista", () => {
    const g = [{ id:"g1", descripcion:"Panel Norte", monto:500, categoria:"Mantenimiento", fecha:"2024-01-15" }];
...
describe("Gastos — con datos reales", () => {
  const gastos = [
    { id:"g1", descripcion:"Panel Norte", monto:500, categoria:"Mantenimiento", fecha:"2024-01-15" },
  ...
  it("renderiza la lista de gastos", () => {
    ...
    container.querySelectorAll("button, [role='button']").forEach(el => {
      try { fireEvent.click(el); } catch {}
    });
    container.querySelectorAll("input, select, textarea").forEach(el => {
      try { fireEvent.change(el, { target: { value: "test" } }); } catch {}
      try { fireEvent.focus(el); } catch {}
    });
