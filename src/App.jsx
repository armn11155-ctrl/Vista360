  ...
  async get(col) {
    try {
      ...
      try {
        ...
        return snap.docs.map(d => ({ id: d.id, ...d.data() }));
      } catch(e2) { alert('Firebase error en ' + col + ': ' + (e2.code || e2.message)); return []; }
    }
  },
  async post(col, body) {
...
function Splash({done, isReady}){
  ...
  useEffect(()=>{
    const ts=[
      ...
      setTimeout(()=>setAnimDone(true), 1800), // animación mínima lista
    ];
...
function LoginScreen({ onLoginSuccess, authReady }) {
  ...
  const handleGoogleLogin = async () => {
    setLoading(true);
    setError("");
    // iOS Safari bloquea popups — detectar y usar redirect directamente
    const isIOS = /iPad|iPhone|iPod/.test(navigator.userAgent) && !window.MSStream;
    const isSafari = /^((?!chrome|android).)*safari/i.test(navigator.userAgent);
    if (isIOS || isSafari) {
      try { await signInWithRedirect(auth, googleProvider); return; } catch(e) {}
    }
    try {
      ...
      if (ALLOWED_EMAILS.length > 0 && !ALLOWED_EMAILS.includes(u.email)) {
        await signOut(auth);
        setError(`Acceso denegado. El email ${u.email} no está autorizado.`);
        setLoading(false);
        return;
      }
      onLoginSuccess(u);
      ...
      const msgs = {
        "auth/popup-closed-by-user":  "Cerraste la ventana. Intenta de nuevo.",
        "auth/unauthorized-domain":   "Dominio no autorizado en Firebase Console → Authentication → Authorized domains.",
        "auth/operation-not-allowed": "Google Sign-In no está activado en Firebase Console.",
        "auth/network-request-failed":"Sin conexión. Verifica tu red.",
      };
      setError(msgs[err.code] || `Error (${err.code}): ${err.message || "intenta de nuevo"}`);
      setLoading(false);
    }
  };
  ...
  return (
    <div style={{
      ...
      {/* Card — spinner si Firebase aún está verificando sesión, botón si ya sabe que no hay usuario */}
      <div style={{
        ...
        {!authReady ? (
          /* Firebase todavía verificando sesión — no mostrar botón aún */
          <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 14, padding: "8px 0" }}>
            <div style={{ width: 28, height: 28, border: "3px solid rgba(255,255,255,0.12)", borderTopColor: "#2563EB", borderRadius: "50%", animation: "spin .75s linear infinite" }}/>
            <div style={{ fontSize: 13, color: "rgba(255,255,255,0.45)" }}>Verificando sesión...</div>
          </div>
        ) : (
          <>
...
export default function App() {
  ...
  // ── Cleanup defensivo: desregistrar SW viejos y borrar caches del origen ──
  // Esto protege contra Service Workers casero/Workbox que hayan quedado de
  // builds anteriores y que puedan estar interceptando requests o sirviendo
  // bundles cacheados. Corre una sola vez al montar la app.
  useEffect(() => {
  ...
  useEffect(() => {
    // Safety: si Firebase no responde en 8s, mostrar login de todas formas
    const timer = setTimeout(() => { setAuthReady(true); setLoading(false); }, 8000);

    // ── Capturar resultado del redirect de Google (iOS/móvil/Safari) ──
    // signInWithRedirect devuelve el usuario aquí al volver a la app
    getRedirectResult(auth).then((result) => {
      ...
      // Ignorar errores de redirect (no hubo redirect previo, es normal)
    ...
    const unsub = onAuthStateChanged(auth, (u) => {
      clearTimeout(timer);
      setAuthReady(true); // ← primero marcamos que ya sabemos el estado de auth
    ...
    contratos.forEach(c => {
      ...
      [30, 15].forEach(umbral => {
        if (d > 0 && d <= umbral) {
          ...
          try {
            new Notification(titulo, { body: cuerpo, tag: key, icon: "/favicon.ico" });
            enviadas[key] = true;
          } catch {}
        }
      });
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      console.error("❌ Error cargando datos de Firebase:", err?.code || err?.message || err);
      setError(true);
      setLoading(false);
    });
  }, [user]);
            ...
            <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.5)",zIndex:600,display:"flex",alignItems:"center",justifyContent:"center",padding:24,backdropFilter:"blur(4px)"}}>
              <div style={{background:NL.white,borderRadius:20,padding:28,width:"100%",maxWidth:320,textAlign:"center"}}>
                ...
                <div style={{display:"flex",gap:10}}>
                  ...
                  <button onClick={() => signOut(auth).finally(() => window.location.reload())} style={{flex:1,padding:"12px",background:NL.red,border:"none",borderRadius:12,fontWeight:700,fontSize:14,cursor:"pointer",color:"#fff"}}>Salir</button>
                </div>
              </div>
            </div>
          )}
      ...
      <>
        {error && (
          <div style={{ background: "#FFF8EC", border: `1px solid ${NL.amber}55`, borderRadius: 14, padding: 16, marginBottom: 20, display: "flex", gap: 12, alignItems: "center" }}>
            ...
            <div>
              <div style={{ fontWeight: 700, color: NL.amber, fontSize: 14 }}>Error al cargar datos de Firebase</div>
              <div style={{ fontSize: 12, color: NL.muted }}>Verifica las reglas de Firestore y que tu dominio esté autorizado en Firebase Console → Authentication → Authorized domains.</div>
            </div>
          </div>
        )}
  ...
  return (
    <>
      ...
      {/* Pantalla de login: se muestra si no hay usuario (con o sin authReady) */}
      {!splash && !user && (
        <LoginScreen authReady={authReady} onLoginSuccess={(u) => setUser(u)}/>
      )}

      {/* ✅ FIX: la app solo se renderiza cuando hay usuario autenticado.
          Antes siempre se renderizaba y tapaba la LoginScreen con position:fixed,
          impidiendo que apareciera el botón de Google tras cerrar sesión. */}
      <div style={{ position: "fixed", top: 0, right: 0, bottom: 0, left: 0, background: NL.bg, display: !splash && authReady && user ? "flex" : "none", flexDirection: "column", fontFamily: "'DM Sans',sans-serif", color: NL.text, overflow: "hidden", overscrollBehavior: "none" }}>
              ...
              ? <div style={{ display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", height:260, gap:16 }}>
                  ...
                  <div style={{ display:"flex", gap:10 }}>
                    <button onClick={() => window.location.reload()}
                      style={{ padding:"8px 20px", background:NL.accent, border:"none", borderRadius:10, color:"#fff", fontSize:12, cursor:"pointer", fontWeight:600 }}>
                      🔄 Reintentar
                    </button>
