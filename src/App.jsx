  ...
  return(<div>
    ...
    <div style={{display:"flex",flexDirection:"column",gap:12}}>
      {fil.map(c=>{
        ...
        return(
          <Card key={c.id} style={{padding:0,overflow:"hidden",background:C.card,border:`1px solid ${C.border}`}}>
            {/* Barra superior de color según estado */}
            <div style={{height:5,background:c.d<=0?C.muted:c.d<=15?C.red:c.d<=30?C.amber:C.green}}/>
            
            <div style={{padding:"20px 22px"}}>
              {/* Header con Panel y Foto */}
              <div style={{display:"flex",alignItems:"center",gap:14,marginBottom:16}}>
                <div style={{
                  width:56,height:56,borderRadius:14,
                  background:`linear-gradient(135deg, ${c.d<=15?C.red:c.d<=30?C.amber:C.green}22, ${c.d<=15?C.red:c.d<=30?C.amber:C.green}11)`,
                  border:`2px solid ${c.d<=15?C.red:c.d<=30?C.amber:C.green}44`,
                  display:"flex",alignItems:"center",justifyContent:"center",fontSize:30,flexShrink:0
                }}>{c.panel.foto}</div>
                <div style={{flex:1,minWidth:0}}>
                  <div style={{fontSize:16,fontWeight:800,color:C.white,letterSpacing:"-0.3px"}}>{c.panel.nombre}</div>
                  <div style={{fontSize:13,color:C.muted,marginTop:3,display:"flex",alignItems:"center",gap:6}}>
                    <span>📍</span>
                    <span>{c.panel.ciudad}</span>
                  </div>
                </div>
                <div style={{
                  background:c.d<=15?C.red+"22":c.d<=30?C.amber+"22":C.green+"22",
                  border:`2px solid ${c.d<=15?C.red:c.d<=30?C.amber:C.green}`,
                  borderRadius:12,padding:"8px 14px",textAlign:"center"
                }}>
                  <div style={{fontSize:11,color:c.d<=15?C.red:c.d<=30?C.amber:C.green,fontWeight:700,textTransform:"uppercase",letterSpacing:0.5}}>
                    {c.d<=0?"Venció":c.d<=15?"Crítico":c.d<=30?"Urgente":"Activo"}
                  </div>
                  <div style={{fontSize:18,fontWeight:900,color:c.d<=15?C.red:c.d<=30?C.amber:C.green,marginTop:2}}>
                    {c.d<=0?`-${Math.abs(c.d)}d`:`${c.d}d`}
                  </div>
                </div>
              </div>
              ...
              {/* Info del Cliente */}
              <div style={{
                background:C.surface,borderRadius:12,padding:"14px 16px",marginBottom:16,
                border:`1px solid ${C.border}`
              }}>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
                  <div>
                    <div style={{fontSize:11,color:C.muted,fontWeight:700,textTransform:"uppercase",letterSpacing:1,marginBottom:4}}>Cliente</div>
                    <div style={{fontSize:14,fontWeight:700,color:C.white}}>{c.cliente.empresa}</div>
                    <div style={{fontSize:12,color:C.muted,marginTop:2}}>{c.cliente.contacto}</div>
                  </div>
                  <a href={`https://wa.me/${c.cliente.celular?.replace(/\D/g,"")}?text=${msg}`} target="_blank" rel="noopener noreferrer"
                    style={{
                      background:"#25D366",borderRadius:10,padding:"10px 16px",
                ...
                <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
                  <div style={{
                    ...
                    borderRadius:8,padding:"6px 12px",fontSize:12,fontWeight:700,color:C.accent
                  }}>
              ...
              <div style={{
                ...
                <div style={{
                  background:C.green+"11",border:`2px solid ${C.green}33`,
                  borderRadius:12,padding:"14px",textAlign:"center"
                }}>
                  <div style={{fontSize:11,color:C.green,fontWeight:700,textTransform:"uppercase",letterSpacing:1,marginBottom:6}}>💰 Monto Mensual</div>
                  <div style={{fontSize:24,fontWeight:900,color:C.green,letterSpacing:"-1px"}}>{fmt(c.monto)}</div>
                  <div style={{fontSize:10,color:C.green+"99",marginTop:2}}>por mes</div>
                </div>
                <div style={{
                  background:pagados===meses.length&&meses.length>0?C.green+"11":pagados>0?C.amber+"11":C.red+"11",
                  border:`2px solid ${pagados===meses.length&&meses.length>0?C.green:pagados>0?C.amber:C.red}33`,
                  borderRadius:12,padding:"14px",textAlign:"center"
                }}>
                  <div style={{fontSize:11,color:pagados===meses.length&&meses.length>0?C.green:pagados>0?C.amber:C.red,fontWeight:700,textTransform:"uppercase",letterSpacing:1,marginBottom:6}}>💳 Pagos</div>
                  <div style={{fontSize:24,fontWeight:900,color:pagados===meses.length&&meses.length>0?C.green:pagados>0?C.amber:C.red,letterSpacing:"-1px"}}>{pagados}/{meses.length}</div>
                  <div style={{fontSize:10,color:(pagados===meses.length&&meses.length>0?C.green:pagados>0?C.amber:C.red)+"99",marginTop:2}}>{fmt(pagados*Number(c.monto))} cobrado</div>
                </div>
              </div>

              {/* Timeline de pagos por mes */}
              {meses.length>0&&(
                <div style={{marginBottom:16}}>
                  <div style={{fontSize:11,color:C.muted,fontWeight:700,textTransform:"uppercase",letterSpacing:1,marginBottom:10}}>📆 Historial de Pagos</div>
                      ...
                      return(
                        <button key={m.key} onClick={()=>togglePagoRapido(c,m.key)}
                          style={{
                            ...
                            border:`2px solid ${pag?C.green:C.border}`,
                            background:pag?C.green+"18":C.surface,
                            cursor:"pointer",transition:"all 0.2s",
                            display:"flex",flexDirection:"column",alignItems:"center",gap:4
                          }}>
                          <div style={{fontSize:18}}>{pag?"✅":"⭕"}</div>
                          <div style={{fontSize:11,fontWeight:700,color:pag?C.green:C.muted}}>{m.label}</div>
                          <div style={{fontSize:10,color:pag?C.green:C.red,fontWeight:600}}>
              ...
              <div style={{display:"flex",gap:8,paddingTop:16,borderTop:`1px solid ${C.border}`}}>
                ...
                <button onClick={()=>eliminar(c.id)} style={{
                  ...
                  background:C.red+"22",border:`2px solid ${C.red}44`,
