...
function Dashboard({clientes,contratos,paneles,setTab}){
  ...
  return(<div style={{paddingBottom:40,display:"flex",flexDirection:"column",gap:16}}>
    ...
    {/* ── HERO CARD ── */}
    <div style={{marginBottom:14,background:"linear-gradient(135deg,#0D1B3E 0%,#0F2255 35%,#1A2E6E 65%,#0D1B3E 100%)",borderRadius:22,position:"relative",overflow:"hidden",boxShadow:"0 12px 48px rgba(10,30,120,0.5)",minHeight:180}}>
      {/* Estrellas de fondo */}
      {[[14,18],[28,60],[48,12],[70,45],[88,22],[60,70],[20,80],[80,55]].map(([x,y],i)=>(
        <div key={i} style={{position:"absolute",left:`${x}%`,top:`${y}%`,width:i%2===0?2:1.5,height:i%2===0?2:1.5,borderRadius:"50%",background:"rgba(255,255,255,"+(0.3+i*0.06)+")"}}/>
      ))}
      {/* Glow morado fondo */}
      <div style={{position:"absolute",bottom:-40,left:-20,width:220,height:180,borderRadius:"50%",background:"radial-gradient(circle,rgba(79,70,229,0.2) 0%,transparent 70%)",pointerEvents:"none"}}/>
      <div style={{position:"relative",padding:"22px 20px 20px",display:"flex",alignItems:"flex-start",justifyContent:"space-between"}}>
        <div style={{flex:1,minWidth:0}}>
          <div style={{fontSize:14,color:"rgba(255,255,255,0.7)",fontWeight:500,marginBottom:2}}>
            {saludoReal}, <span style={{color:"#93C5FD",fontWeight:700}}>Alan</span>! 👋
          </div>
          <div style={{fontSize:26,fontWeight:900,color:"#FFFFFF",letterSpacing:"-0.5px",lineHeight:1.15,marginBottom:8}}>
            Alan Martínez
          </div>
          <div style={{fontSize:13,color:"rgba(255,255,255,0.5)",marginBottom:16}}>Aquí tienes el resumen de hoy.</div>
          <div style={{display:"inline-flex",alignItems:"center",gap:8,background:"rgba(255,255,255,0.08)",border:"1px solid rgba(255,255,255,0.12)",borderRadius:30,padding:"6px 14px"}}>
        ...
        {/* Antena SVG 3D premium */}
        <div style={{flexShrink:0,marginTop:-8,marginRight:-8}}>
          <AntenaImg/>
        </div>
      </div>
    </div>
    ...
    {/* ── INGRESOS ── */}
    <div style={{marginBottom:14,padding:"18px 18px 14px",background:"linear-gradient(145deg,#0A1A14,#071008)",borderRadius:22,border:"1px solid rgba(16,185,129,0.2)",boxShadow:"0 4px 32px rgba(16,185,129,0.1)",position:"relative",overflow:"hidden"}}>
      <div style={{position:"absolute",top:-30,right:-30,width:120,height:120,borderRadius:"50%",background:"radial-gradient(circle,rgba(16,185,129,0.12) 0%,transparent 70%)"}}/>
      <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:12}}>
        <IconDolar/>
        <div style={{fontSize:10,fontWeight:800,color:"rgba(110,231,183,0.8)",textTransform:"uppercase",letterSpacing:2}}>Ingresos este mes</div>
        <div style={{marginLeft:"auto",display:"flex",alignItems:"center",gap:4,background:"rgba(16,185,129,0.12)",border:"1px solid rgba(16,185,129,0.2)",borderRadius:20,padding:"3px 10px"}}>
          <svg width="10" height="10" viewBox="0 0 12 12" fill="none"><path d="M1 9 L4 5 L7 7 L11 2" stroke={crecPct>=0?"#10B981":C.red} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/></svg>
          <span style={{fontSize:11,fontWeight:700,color:crecPct>=0?"#10B981":C.red}}>{crecPct===0?"0":crecPct>0?"+"+crecPct:crecPct}%</span>
        </div>
      </div>
      <div style={{fontSize:40,fontWeight:900,color:"#10B981",letterSpacing:"-2px",lineHeight:1,marginBottom:6}}>{fmt(ingMes)}</div>
      <div style={{fontSize:12,color:"rgba(110,231,183,0.6)"}}>{pag} de {contratos.length} contratos cobrados</div>
    </div>

    {/* ── DONUTS ── */}
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:14}}>
      {/* Ocupación */}
      <div style={{padding:"16px 14px",background:"linear-gradient(145deg,#0C1830,#080F20)",borderRadius:20,border:"1px solid rgba(79,110,247,0.15)",display:"flex",flexDirection:"row",alignItems:"center",gap:12}}>
        <svg width="72" height="72" viewBox="0 0 72 72">
          <defs><linearGradient id="gocu3" x1="0" y1="0" x2="1" y2="1"><stop stopColor="#818CF8"/><stop offset="1" stopColor="#4F6EF7"/></linearGradient></defs>
          <circle cx="36" cy="36" r="28" fill="none" stroke="rgba(79,110,247,0.15)" strokeWidth="7"/>
          <circle cx="36" cy="36" r="28" fill="none" stroke="url(#gocu3)" strokeWidth="7"
            strokeDasharray={`${(ocu/100)*2*Math.PI*28} ${2*Math.PI*28}`} strokeLinecap="round" transform="rotate(-90 36 36)"
            style={{filter:"drop-shadow(0 0 6px rgba(79,110,247,0.8))"}}/>
          <text x="36" y="40" textAnchor="middle" fill="white" fontSize="15" fontWeight="900">{ocu}%</text>
        </svg>
        <div>
          <div style={{fontSize:10,fontWeight:700,color:"rgba(129,140,248,0.8)",textTransform:"uppercase",letterSpacing:1,marginBottom:4}}>Ocupación</div>
          <div style={{fontSize:12,color:"rgba(255,255,255,0.5)",lineHeight:1.4}}>{paneles.filter(p=>p.estado==="Ocupado").length}/{paneles.length} paneles</div>
          {paneles.length===0&&<div style={{fontSize:10,color:"#4F6EF7",marginTop:4}}>Sin datos aún</div>}
        </div>
      </div>
      {/* Cobro */}
      <div style={{padding:"16px 14px",background:"linear-gradient(145deg,#081A12,#04100A)",borderRadius:20,border:"1px solid rgba(16,185,129,0.15)",display:"flex",flexDirection:"row",alignItems:"center",gap:12}}>
        <svg width="72" height="72" viewBox="0 0 72 72">
          <defs><linearGradient id="gcob3" x1="0" y1="0" x2="1" y2="1"><stop stopColor="#6EE7B7"/><stop offset="1" stopColor="#10B981"/></linearGradient></defs>
          <circle cx="36" cy="36" r="28" fill="none" stroke="rgba(16,185,129,0.15)" strokeWidth="7"/>
          <circle cx="36" cy="36" r="28" fill="none" stroke="url(#gcob3)" strokeWidth="7"
            strokeDasharray={`${(cob/100)*2*Math.PI*28} ${2*Math.PI*28}`} strokeLinecap="round" transform="rotate(-90 36 36)"
            style={{filter:"drop-shadow(0 0 6px rgba(16,185,129,0.8))"}}/>
          <text x="36" y="40" textAnchor="middle" fill="white" fontSize="15" fontWeight="900">{cob}%</text>
        </svg>
        <div>
          <div style={{fontSize:10,fontWeight:700,color:"rgba(110,231,183,0.8)",textTransform:"uppercase",letterSpacing:1,marginBottom:4}}>Cobro</div>
          <div style={{fontSize:12,color:"rgba(255,255,255,0.5)",lineHeight:1.4}}>{pag}/{contratos.length} contratos</div>
          {contratos.length===0&&<div style={{fontSize:10,color:"#10B981",marginTop:4}}>Sin datos aún</div>}
        </div>
      </div>
    </div>
    ...
    <div style={{padding:"18px 20px",background:"linear-gradient(145deg,#0F1828,#0A1220)",borderRadius:20,border:"1px solid #1A2540"}}>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:8}}>
        {[
          ...
          <div key={m.label} style={{background:"linear-gradient(145deg,#0A1020,#060C18)",border:"1px solid rgba(255,255,255,0.06)",borderRadius:16,padding:"14px 8px",textAlign:"center"}}>
...
export default function App(){
  ...
  return(<>
    ...
    <style>{`*{box-sizing:border-box;-webkit-tap-highlight-color:transparent}html{background:#080C14;height:100%;height:-webkit-fill-available}body{margin:0;padding:0;background:#080C14;overflow:hidden;width:100%;height:100%;height:-webkit-fill-available;-webkit-text-size-adjust:100%}#root{width:100%;height:100%;height:-webkit-fill-available;display:flex;flex-direction:column;overflow:hidden;box-sizing:border-box}::-webkit-scrollbar{display:none}@keyframes spin{to{transform:rotate(360deg)}}@keyframes pulse{from{opacity:.2}to{opacity:.6}}`}</style>
    ...
    <div style={{fontFamily:"'DM Sans',sans-serif",background:"#080C14",flex:1,minHeight:0,color:C.text,display:"flex",overflow:"hidden",alignItems:"stretch",position:"relative"}}>
      ...
      <aside style={{
        ...
        {/* Nav */}
        <div style={{flex:1,width:"100%",overflowY:"auto",scrollbarWidth:"none",paddingTop:10,display:"flex",flexDirection:"column",gap:2,padding:"10px 10px 0"}}>
          {tabs.map(t=>(
        ...
        <div style={{padding:"0 10px",borderTop:"1px solid rgba(255,255,255,0.04)",paddingBottom:"calc(10px + env(safe-area-inset-bottom))"}}>
          ...
          {/* Perfil con foto real */}
          <button onClick={()=>setShowProfile(v=>!v)} style={{
            width:"100%",marginBottom:6,
            display:"flex",alignItems:"center",gap:10,padding:"10px 12px",
            background:showProfile?"rgba(79,110,247,0.12)":"none",
            border:showProfile?"1px solid rgba(79,110,247,0.25)":"1px solid transparent",
            cursor:"pointer",borderRadius:12,transition:"all 0.15s",
          }}>
            <div style={{position:"relative",flexShrink:0}}>
              <img
                src="https://ui-avatars.com/api/?name=Alan+Martinez&background=4F46E5&color=fff&size=64&bold=true"
                style={{width:36,height:36,borderRadius:"50%",border:"2px solid rgba(79,110,247,0.5)",objectFit:"cover"}}
                alt="AM"
              />
      ...
      {/* ── MAIN CONTENT ── */}
      <div style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden",marginLeft:168,background:"#080C14"}}>
        <main style={{flex:1,overflowY:"auto",overflowX:"hidden",padding:"16px",paddingTop:"calc(env(safe-area-inset-top) + 16px)",paddingBottom:"calc(env(safe-area-inset-bottom) + 16px)",WebkitOverflowScrolling:"touch",scrollBehavior:"smooth",background:"#080C14"}}>
