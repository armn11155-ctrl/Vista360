...
function CRM({clientes,setClientes,contratos,loading}){
  ...
  // Avatar initials
  const avatarColors=["#7C3AED","#10B981","#F59E0B","#3B82F6","#EF4444","#06B6D4","#8B5CF6","#EC4899"];
  const AvatarCRM=({name,size=32})=>{
    const initials=(name||"?").split(" ").map(w=>w[0]).join("").slice(0,2).toUpperCase();
    const col=avatarColors[(name||"?").charCodeAt(0)%avatarColors.length];
    return(<div style={{width:size,height:size,borderRadius:"50%",background:`linear-gradient(135deg,${col},${col}88)`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:size*0.35,fontWeight:700,color:"#fff",flexShrink:0,border:"2px solid rgba(255,255,255,0.08)"}}>{initials}</div>);
  };
          ...
          return(
            <div key={r.id} className="crm-row-hover"
              ...
              <div style={{display:"flex",alignItems:"center",gap:11}}>
                <AvatarCRM name={r.empresa} size={32}/>
                <div>
              ...
              <div style={{display:"flex",alignItems:"center",gap:7}}>
                <AvatarCRM name={r.contacto||"?"} size={26}/>
