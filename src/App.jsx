...
const PgTit  = ({icon,title,sub})=><div style={{marginBottom:22}}><div style={{fontSize:22,fontWeight:800,color:D.text,letterSpacing:"-0.5px"}}>{icon} {title}</div>{sub&&<div style={{fontSize:13,color:"#64748B",marginTop:3}}>{sub}</div>}</div>;
...
// ── PULL TO REFRESH ──────────────────────────────────────────────
function PullToRefresh({ scrollRef, onRefresh }) {
  ...
  return (
    <div style={{ position:"absolute", top: pullY - 44, left:0, right:0, display:"flex", justifyContent:"center", zIndex:50, pointerEvents:"none", transition: pullY === 0 ? "top .3s ease" : "none" }}>
      <div style={{ width:36, height:36, borderRadius:"50%", background:"#fff", boxShadow:"0 4px 16px rgba(0,0,0,0.12)", display:"flex", alignItems:"center", justifyContent:"center" }}>
        {refreshing
          ? <div style={{ width:16, height:16, border:"2px solid #E5E7EB", borderTopColor:D.accent, borderRadius:"50%", animation:"spin .7s linear infinite" }}/>
          : <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke={D.accent} strokeWidth="2.5" strokeLinecap="round" style={{ transform:`rotate(${pct*180}deg)`, transition:"transform .1s" }}><polyline points="23 4 23 10 17 10"/><path d="M20.49 15a9 9 0 1 1-2.12-9.36L23 10"/></svg>
        }
      </div>
    </div>
  );
      ...
      <div style={{ padding: "14px 16px", background: "#fff", borderBottom: "1px solid #E5E7EB" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10, background: "#F8FAFC", border: "1px solid #E5E7EB", borderRadius: 14, padding: "11px 14px" }}>
          ...
          <input value={buscar} onChange={e => { setBuscar(e.target.value); setPage(1); }}
            ...
            style={{ flex: 1, border: "none", outline: "none", fontSize: 14, color: D.text, background: "transparent", fontFamily: "inherit" }}/>
      ...
      <div style={{
        ...
        <div style={{flex:"1 1 56%",minWidth:0,display:"flex",flexDirection:"column"}}>
          ...
          <div style={{display:"flex",alignItems:"baseline",gap:8,marginTop:6,flexWrap:"wrap"}}>
            <div style={{fontSize:28,fontWeight:900,color:"#fff",letterSpacing:"-0.5px",lineHeight:1,fontFamily:"'Barlow Condensed', sans-serif"}}>{fmt(ingActual)}</div>
...
function CapModal({ open, onClose, title, subtitle, children }) {
  if (!open) return null;
  return (
    <div onClick={e=>e.target===e.currentTarget&&onClose()} style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.45)",zIndex:700,display:"flex",alignItems:"flex-end",justifyContent:"center"}}>
      <div style={{width:"100%",maxWidth:540,background:"#fff",borderRadius:"24px 24px 0 0",maxHeight:"90vh",display:"flex",flexDirection:"column",boxShadow:"0 -12px 48px rgba(0,0,0,0.25)"}}>
        <div style={{display:"flex",justifyContent:"center",padding:"12px 0 4px"}}>
          <div style={{width:40,height:4,borderRadius:2,background:"#E5E7EB"}}/>
        </div>
        <div style={{padding:"8px 20px 16px",borderBottom:"1px solid #F1F3F8"}}>
          <div style={{fontSize:18,fontWeight:800,color:"#0D1117"}}>{title}</div>
...
// ── Input oscuro reutilizable ──
function DarkInput({label,value,onChange,type="text",placeholder,prefix,suffix}) {
  return (
    <div style={{marginBottom:14}}>
      {label&&<div style={{fontSize:11,fontWeight:700,color:"#9CA3AF",letterSpacing:1.2,textTransform:"uppercase",marginBottom:6}}>{label}</div>}
      <div style={{display:"flex",alignItems:"center",background:"#F8FAFC",border:"1px solid #E5E7EB",borderRadius:12,padding:"0 14px",gap:8}}>
        {prefix&&<span style={{fontSize:14,color:"#9CA3AF",flexShrink:0}}>{prefix}</span>}
        <input type={type} inputMode={type==="number"?"decimal":"text"} value={value} onChange={onChange} placeholder={placeholder||""}
          style={{flex:1,background:"transparent",border:"none",outline:"none",color:"#0D1117",fontSize:15,fontWeight:600,padding:"13px 0",fontFamily:"inherit"}}/>
...
function DarkSelect({label,value,onChange,options}) {
  return (
    <div style={{marginBottom:14}}>
      {label&&<div style={{fontSize:11,fontWeight:700,color:"#9CA3AF",letterSpacing:1.2,textTransform:"uppercase",marginBottom:6}}>{label}</div>}
      <select value={value} onChange={onChange} style={{width:"100%",background:"#F8FAFC",border:"1px solid #E5E7EB",borderRadius:12,padding:"13px 14px",color:"#0D1117",fontSize:14,fontFamily:"inherit",outline:"none",appearance:"none"}}>
...
// ── Tarjeta premium blanca ──
function CapCard({children, accent, style={}}) {
  return (
    <div style={{
      background:"#fff",
      border:`1px solid ${accent?`${accent}22`:"#F1F3F8"}`,
      borderRadius:20,
      boxShadow:`0 2px 16px rgba(0,0,0,0.06)${accent?`, 0 0 0 1px ${accent}11`:""}`,
      overflow:"hidden",
      ...style,
    }}>{children}</div>
  );
...
// ── Barra de progreso ──
function ProgressBar({pct,color,height=8,bg="rgba(255,255,255,0.06)"}) {
  return (
...
function Capital({ paneles, contratos, gastos, proveedores }) {
  ...
  const [data, setData] = useState({
    ...
    fondos: [             // porcentajes estratégicos
      ...
      {id:"expansion",  label:"Expansión",    color:"#10B981", emoji:"📈", pct:25},
      {id:"inversion",  label:"Inversión",    color:"#8B5CF6", emoji:"💎", pct:25},
      {id:"personal",   label:"Personal",     color:"#F59E0B", emoji:"👤", pct:20},
    ],
  ...
  const insights = [];
  if (patrimonioChange>0) insights.push({icon:"📈",text:`Tu patrimonio aumentó ${patrimonioChange}% este mes`,color:"#34D399"});
  if (totalLiquidez > gastosMes*3) insights.push({icon:"💧",text:"Liquidez saludable — más de 3 meses cubiertos",color:"#10B981"});
  if (mesesProyeccion) insights.push({icon:"🎯",text:`Podrías cumplir "${objPendiente.nombre}" en ~${mesesProyeccion} mes${mesesProyeccion!==1?"es":""}`,color:"#10B981"});
  if (totalActivos>totalDeudas*2) insights.push({icon:"🏛️",text:"Tus activos duplican tus deudas — solidez financiera",color:"#34D399"});
  if (ingMes>gastosMes*1.5) insights.push({icon:"✅",text:"Flujo positivo: ingresas más del doble de lo que gastas",color:"#34D399"});
  ...
  if (!loaded) return (
    <div style={{display:"flex",alignItems:"center",justifyContent:"center",height:300,color:"#9CA3AF",fontSize:14}}>Cargando...</div>
  );
  ...
  const renderPatrimonio = () => (
    <div>
      {/* Hero patrimonio */}
      <CapCard accent="#10B981" style={{padding:"26px 22px 22px",marginBottom:12,background:"linear-gradient(145deg,#0D2B1F,#0A1F18)"}}>
        <div style={{fontSize:10,fontWeight:700,color:"#10B981",letterSpacing:2.5,textTransform:"uppercase",marginBottom:8}}>Patrimonio neto total</div>
        ...
        <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:20}}>
          <span style={{fontSize:12,fontWeight:700,color:patrimonioChange>=0?"#34D399":"#F87171",background:patrimonioChange>=0?"rgba(52,211,153,0.15)":"rgba(248,113,113,0.15)",borderRadius:99,padding:"3px 10px"}}>
            {patrimonioChange>=0?"+":""}{patrimonioChange}% este mes
          </span>
        </div>
        <Sparkline data={spark6} color="#34D399" height={44}/>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:8,marginTop:16}}>
          {[
            {label:"Liquidez",value:totalLiquidez,color:"#34D399"},
      ...
      <CapCard style={{padding:"18px 20px",marginBottom:12}}>
        <div style={{fontSize:10,fontWeight:700,color:"#9CA3AF",letterSpacing:2,textTransform:"uppercase",marginBottom:14}}>Flujo mensual</div>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
          {[
            {label:"Ingresos",   value:ingMes,    color:"#10B981", bg:"#F0FDF4", icon:"↑"},
            {label:"Gastos",     value:gastosMes, color:"#EF4444", bg:"#FEF2F2", icon:"↓"},
            {label:"Por cobrar", value:porCobrar, color:"#10B981", bg:"#F0FDF4", icon:"⏳"},
            {label:"Neto libre", value:Math.max(ingresoMensualNeto,0), color:"#0D1117", bg:"#F8FAFC", icon:"✓"},
          ].map(({label,value,color,bg,icon})=>(
            <div key={label} style={{background:bg,border:"1px solid #F1F3F8",borderRadius:14,padding:"13px 14px"}}>
              <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:6}}>
                <span style={{width:20,height:20,borderRadius:6,background:`${color}18`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:11,color}}>{icon}</span>
                <span style={{fontSize:10,color:"#9CA3AF",fontWeight:600,textTransform:"uppercase",letterSpacing:0.8}}>{label}</span>
              </div>
      ...
      {insights.length>0&&(
        <CapCard style={{padding:"18px 20px"}}>
          <div style={{fontSize:10,fontWeight:700,color:"#9CA3AF",letterSpacing:2,textTransform:"uppercase",marginBottom:12}}>Análisis inteligente</div>
          <div style={{display:"flex",flexDirection:"column",gap:8}}>
            {insights.map((ins,i)=>(
              <div key={i} style={{display:"flex",alignItems:"center",gap:12,padding:"10px 12px",borderRadius:12,background:ins.color==="#34D399"?"#F0FDF4":"#FEF2F2",border:`1px solid ${ins.color==="#34D399"?"#BBF7D0":"#FECACA"}`}}>
                <span style={{fontSize:18,flexShrink:0}}>{ins.icon}</span>
                <span style={{fontSize:13,fontWeight:600,color:ins.color==="#34D399"?"#065F46":"#991B1B",lineHeight:1.4}}>{ins.text}</span>
              </div>
            ))}
          </div>
        </CapCard>
  ...
  const renderLiquidez = () => (
    <div>
      ...
      <CapCard accent={CAP_COLORS.liquidez} style={{padding:"22px 20px",marginBottom:12}}>
        <div style={{fontSize:10,fontWeight:700,color:"rgba(16,185,129,0.7)",letterSpacing:2.5,textTransform:"uppercase",marginBottom:6}}>Dinero disponible</div>
        <div style={{fontSize:40,fontWeight:900,color:"#0D1117",letterSpacing:"-1.5px",lineHeight:1,marginBottom:4}}>{fmtS2(totalLiquidez)}</div>
        <div style={{fontSize:12,color:liquidezNeta>=0?"#34D399":"#F87171",fontWeight:600,marginBottom:16}}>
          {liquidezNeta>=0?"✓":"⚠"} {fmtS2(Math.abs(liquidezNeta))} {liquidezNeta>=0?"libre tras gastos del mes":"déficit este mes"}
        </div>
        {/* Caja vs bancos */}
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
          <div style={{background:"#EFF6FF",border:"1px solid #BFDBFE",borderRadius:12,padding:"11px 13px"}}>
            ...
            <div style={{fontSize:16,fontWeight:900,color:"#10B981"}}>{fmtS2(totalCuentas)}</div>
          </div>
          <div style={{background:"#F0FDF4",border:"1px solid #BBF7D0",borderRadius:12,padding:"11px 13px"}}>
            <div style={{fontSize:10,color:"rgba(16,185,129,0.7)",fontWeight:700,marginBottom:4,textTransform:"uppercase",letterSpacing:0.8}}>Caja / Efectivo</div>
            <div style={{fontSize:16,fontWeight:900,color:"#34D399"}}>{fmtS2(data.caja)}</div>
          </div>
        </div>
      </CapCard>
      ...
      <CapCard style={{padding:"18px 20px",marginBottom:12}}>
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:14}}>
          <div style={{fontSize:10,fontWeight:700,color:"#9CA3AF",letterSpacing:2,textTransform:"uppercase"}}>Cuentas bancarias</div>
          <button onClick={()=>openAdd("cuentas")} style={{display:"inline-flex",alignItems:"center",gap:5,background:"#F0FDF4",border:"1px solid #BBF7D0",borderRadius:99,padding:"5px 12px",color:"#10B981",fontSize:11,fontWeight:700,cursor:"pointer",touchAction:"manipulation",fontFamily:"inherit"}}>
            + Agregar
          </button>
        </div>
        {data.cuentas.length===0?(
          <div style={{textAlign:"center",padding:"24px 0",color:"#9CA3AF",fontSize:13}}>Sin cuentas registradas</div>
        ):data.cuentas.map(c=>(
          <div key={c.id} style={{display:"flex",alignItems:"center",gap:12,padding:"12px 0",borderBottom:"1px solid #F1F3F8"}}> 
            <div style={{width:40,height:40,borderRadius:11,background:"#EFF6FF",border:"1px solid #BFDBFE",display:"flex",alignItems:"center",justifyContent:"center",fontSize:18,flexShrink:0}}>🏦</div>
            <div style={{flex:1,minWidth:0}}>
              <div style={{fontSize:14,fontWeight:700,color:"#0D1117"}}>{c.nombre}</div>
              <div style={{fontSize:11,color:"#9CA3AF"}}>{c.banco||"—"} · {c.tipo||"Ahorro"}</div>
            </div>
            <div style={{textAlign:"right",flexShrink:0}}>
              <div style={{fontSize:15,fontWeight:900,color:"#10B981"}}>{fmtS2(c.saldo)}</div>
            </div>
            <button onClick={()=>openEdit("cuentas",c)} style={{width:30,height:30,borderRadius:8,background:"#F8FAFC",border:"1px solid #E5E7EB",cursor:"pointer",touchAction:"manipulation",display:"flex",alignItems:"center",justifyContent:"center",color:"#9CA3AF",flexShrink:0}}>
      ...
      {/* Caja efectivo */}
      <CapCard style={{padding:"18px 20px"}}>
        <div style={{fontSize:10,fontWeight:700,color:"#9CA3AF",letterSpacing:2,textTransform:"uppercase",marginBottom:12}}>Caja / Efectivo</div>
        <div style={{display:"flex",alignItems:"center",gap:14}}>
          <div style={{flex:1}}>
            <div style={{fontSize:28,fontWeight:900,color:"#34D399",letterSpacing:"-1px"}}>{fmtS2(data.caja)}</div>
            <div style={{fontSize:11,color:"#9CA3AF",marginTop:4}}>Efectivo disponible en caja</div>
          </div>
          <button onClick={()=>{setModal("caja");setForm({caja:String(data.caja)});}} style={{background:"#D1FAE5",border:"1px solid #6EE7B7",borderRadius:12,padding:"10px 16px",color:"#34D399",fontSize:12,fontWeight:700,cursor:"pointer",touchAction:"manipulation",fontFamily:"inherit",whiteSpace:"nowrap"}}>
            Actualizar
          </button>
        </div>
      </CapCard>
  ...
  const renderActivos = () => (
    <div>
      <CapCard accent={CAP_COLORS.activos} style={{padding:"22px 20px",marginBottom:12}}>
        <div style={{fontSize:10,fontWeight:700,color:"rgba(16,185,129,0.7)",letterSpacing:2.5,textTransform:"uppercase",marginBottom:6}}>Valor total de activos</div>
        <div style={{fontSize:40,fontWeight:900,color:"#0D1117",letterSpacing:"-1.5px",lineHeight:1,marginBottom:4}}>{fmtS2(totalActivos)}</div>
        <div style={{fontSize:12,color:"#9CA3AF",marginBottom:16}}>{data.activos.length} activo{data.activos.length!==1?"s":""} registrado{data.activos.length!==1?"s":""}</div>
            ...
            <div>
              <div style={{height:8,borderRadius:99,overflow:"hidden",display:"flex",gap:2,marginBottom:8}}>
                {Object.entries(porCat).map(([cat,val],i)=>{
                  const cs=["#10B981","#3B82F6","#8B5CF6","#F59E0B","#EF4444","#EC4899"];
      ...
      <CapCard style={{padding:"18px 20px"}}>
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:14}}>
          <div style={{fontSize:10,fontWeight:700,color:"#9CA3AF",letterSpacing:2,textTransform:"uppercase"}}>Mis activos</div>
          <button onClick={()=>openAdd("activos")} style={{display:"inline-flex",alignItems:"center",gap:5,background:"#F0FDF4",border:"1px solid #BBF7D0",borderRadius:99,padding:"5px 12px",color:"#34D399",fontSize:11,fontWeight:700,cursor:"pointer",touchAction:"manipulation",fontFamily:"inherit"}}>
            + Agregar
          </button>
        </div>
        {data.activos.length===0?(
          <div style={{textAlign:"center",padding:"24px 0",color:"#9CA3AF",fontSize:13}}>Sin activos registrados</div>
        ):data.activos.map(a=>{
          const rent=Number(a.rentabilidad||0);
          return (
            <div key={a.id} style={{display:"flex",alignItems:"center",gap:12,padding:"12px 0",borderBottom:"1px solid #F1F3F8"}}> 
              ...
              <div style={{flex:1,minWidth:0}}>
                <div style={{fontSize:14,fontWeight:700,color:"#0D1117",whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>{a.nombre}</div>
                <div style={{fontSize:11,color:"#9CA3AF"}}>{a.fecha||"—"}{rent>0?` · +${rent}% rentab.`:""}</div>
              </div>
              <div style={{textAlign:"right",flexShrink:0}}>
                <div style={{fontSize:15,fontWeight:900,color:"#10B981"}}>{fmtS2(a.valor)}</div>
              </div>
              <button onClick={()=>openEdit("activos",a)} style={{width:30,height:30,borderRadius:8,background:"#F8FAFC",border:"1px solid #E5E7EB",cursor:"pointer",touchAction:"manipulation",display:"flex",alignItems:"center",justifyContent:"center",color:"#9CA3AF",flexShrink:0}}>
                <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round"><path d="M17 3a2.828 2.828 0 114 4L7.5 20.5 2 22l1.5-5.5L17 3z"/></svg>
              </button>
            </div>
          );
    ...
    return (
      <div>
        <CapCard accent={CAP_COLORS.fondos} style={{padding:"22px 20px",marginBottom:12}}>
          <div style={{fontSize:10,fontWeight:700,color:"rgba(16,185,129,0.7)",letterSpacing:2.5,textTransform:"uppercase",marginBottom:6}}>Fondos estratégicos</div>
          <div style={{fontSize:36,fontWeight:900,color:"#0D1117",letterSpacing:"-1px",lineHeight:1,marginBottom:4}}>{fmtS2(liquidezNeta>0?liquidezNeta:0)}</div>
          <div style={{fontSize:12,color:"#9CA3AF",marginBottom:16}}>disponible para distribuir</div>
          {/* Barra coloreada */}
          ...
          <div style={{fontSize:10,color:"#9CA3AF",textAlign:"right"}}>{totalPct}% asignado</div>
        </CapCard>

        <CapCard style={{padding:"18px 20px",marginBottom:12}}>
          <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:14}}>
            <div style={{fontSize:10,fontWeight:700,color:"#9CA3AF",letterSpacing:2,textTransform:"uppercase"}}>Distribución</div>
            <button onClick={()=>{setModal("fondos");const f2={};data.fondos.forEach((f,i)=>f2[`pct_${i}`]=f.pct);setForm(f2);}} style={{display:"inline-flex",alignItems:"center",gap:5,background:"#F0FDF4",border:"1px solid #BBF7D0",borderRadius:99,padding:"5px 12px",color:"#10B981",fontSize:11,fontWeight:700,cursor:"pointer",touchAction:"manipulation",fontFamily:"inherit"}}>
              Ajustar %
            </button>
          </div>
          <div style={{display:"flex",flexDirection:"column",gap:10}}>
            {data.fondos.map(f=>{
              ...
              return (
                <div key={f.id} style={{padding:"13px 14px",borderRadius:14,background:"#F8FAFC",border:"1px solid #F1F3F8"}}>
                  <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:8}}>
                    <span style={{fontSize:20}}>{f.emoji}</span>
                    <div style={{flex:1}}>
                      <div style={{fontSize:14,fontWeight:700,color:"#0D1117"}}>{f.label}</div>
                      <div style={{fontSize:11,color:"#9CA3AF"}}>{f.pct}% del disponible</div>
                    </div>
                    <div style={{fontSize:16,fontWeight:900,color:"#10B981"}}>{fmtS2(monto)}</div>
                  </div>
  ...
  const renderObjetivos = () => (
    <div>
      <CapCard accent={CAP_COLORS.objetivos} style={{padding:"22px 20px",marginBottom:12}}>
        <div style={{fontSize:10,fontWeight:700,color:"rgba(16,185,129,0.7)",letterSpacing:2.5,textTransform:"uppercase",marginBottom:8}}>Objetivos de expansión</div>
        <div style={{fontSize:28,fontWeight:900,color:"#0D1117",letterSpacing:"-1px",lineHeight:1,marginBottom:6}}>
      ...
      <CapCard style={{padding:"18px 20px"}}>
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:14}}>
          <div style={{fontSize:10,fontWeight:700,color:"#9CA3AF",letterSpacing:2,textTransform:"uppercase"}}>Mis objetivos</div>
          <button onClick={()=>openAdd("objetivos")} style={{display:"inline-flex",alignItems:"center",gap:5,background:"#F0FDF4",border:"1px solid #BBF7D0",borderRadius:99,padding:"5px 12px",color:"#10B981",fontSize:11,fontWeight:700,cursor:"pointer",touchAction:"manipulation",fontFamily:"inherit"}}>
            + Nuevo
          </button>
        </div>
        {data.objetivos.length===0?(
          <div style={{textAlign:"center",padding:"24px 0",color:"#9CA3AF",fontSize:13}}>Sin objetivos · agrega tu primero</div>
          ...
          return (
            <div key={o.id} style={{marginBottom:14,padding:"14px 14px",borderRadius:16,background:"rgba(255,255,255,0.04)",border:`1px solid ${done?"rgba(52,211,153,0.2)":"rgba(245,158,11,0.12)"}`}}>
              <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:10}}>
                <div style={{flex:1}}>
                  <div style={{fontSize:14,fontWeight:800,color:"#0D1117"}}>{o.nombre}</div>
                ...
                <div style={{textAlign:"right",flexShrink:0}}>
                  <div style={{fontSize:18,fontWeight:900,color:done?"#34D399":"#FCD34D"}}>{pct}%</div>
                  {done&&<div style={{fontSize:10,color:"#34D399",fontWeight:700}}>✓ Completado</div>}
                </div>
                <button onClick={()=>openEdit("objetivos",o)} style={{width:30,height:30,borderRadius:8,background:"#F8FAFC",border:"1px solid #E5E7EB",cursor:"pointer",touchAction:"manipulation",display:"flex",alignItems:"center",justifyContent:"center",color:"#9CA3AF",flexShrink:0}}>
                  <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round"><path d="M17 3a2.828 2.828 0 114 4L7.5 20.5 2 22l1.5-5.5L17 3z"/></svg>
                </button>
              </div>
              <ProgressBar pct={pct} color={done?"#34D399":"#F59E0B"} height={8}/>
              {!done&&<div style={{display:"flex",justifyContent:"space-between",marginTop:6}}>
                <span style={{fontSize:10,color:"#9CA3AF"}}>Faltan {fmtS2(faltan)}</span>
                <span style={{fontSize:10,color:"#9CA3AF",fontWeight:600}}>Meta: {fmtS2(meta)}</span>
              </div>}
            </div>
          );
        })}
  ...
  const modalCuenta = (
    <CapModal open={modal==="cuentas"} onClose={closeModal} title={editItem?"Editar cuenta":"Nueva cuenta bancaria"} subtitle="Dinero en bancos">
      ...
      <div style={{display:"flex",gap:10,marginTop:8}}>
        {editItem&&<button onClick={()=>{deleteItem("cuentas",editItem.id);closeModal();}} style={{flex:1,padding:13,background:"rgba(239,68,68,0.1)",border:"1px solid rgba(239,68,68,0.3)",borderRadius:12,color:"#F87171",fontWeight:600,fontSize:13,cursor:"pointer",touchAction:"manipulation",fontFamily:"inherit"}}>Eliminar</button>}
        <button onClick={()=>saveItem("cuentas")} disabled={saving} style={{flex:2,padding:13,background:"#10B981",border:"none",borderRadius:12,color:"#fff",fontWeight:700,fontSize:14,cursor:"pointer",touchAction:"manipulation",fontFamily:"inherit",opacity:saving?0.7:1}}>{saving?"Guardando…":"Guardar"}</button>
      </div>
    </CapModal>
  );
  ...
  const modalActivo = (
    <CapModal open={modal==="activos"} onClose={closeModal} title={editItem?"Editar activo":"Nuevo activo"} subtitle="Bienes del negocio">
      ...
      <div style={{display:"flex",gap:10,marginTop:8}}>
        {editItem&&<button onClick={()=>{deleteItem("activos",editItem.id);closeModal();}} style={{flex:1,padding:13,background:"rgba(239,68,68,0.1)",border:"1px solid rgba(239,68,68,0.3)",borderRadius:12,color:"#F87171",fontWeight:600,fontSize:13,cursor:"pointer",touchAction:"manipulation",fontFamily:"inherit"}}>Eliminar</button>}
        <button onClick={()=>saveItem("activos")} disabled={saving} style={{flex:2,padding:13,background:"#10B981",border:"none",borderRadius:12,color:"#fff",fontWeight:700,fontSize:14,cursor:"pointer",touchAction:"manipulation",fontFamily:"inherit",opacity:saving?0.7:1}}>{saving?"Guardando…":"Guardar"}</button>
      </div>
    </CapModal>
  );
  ...
  const modalFondos = (
    <CapModal open={modal==="fondos"} onClose={closeModal} title="Ajustar fondos" subtitle="Define cómo distribuir tu dinero disponible">
        ...
        <div key={f.id} style={{marginBottom:16}}>
          ...
          <div style={{display:"flex",alignItems:"center",gap:10}}>
            ...
            <div style={{flex:1}}>
              ...
              <div style={{fontSize:11,color:"#9CA3AF",textAlign:"center",marginTop:4}}>{fmtS2((liquidezNeta>0?liquidezNeta:0)*((Number(form[`pct_${i}`]??f.pct))/100))}</div>
            </div>
      ...
      <button onClick={saveFondos} disabled={saving} style={{width:"100%",padding:13,background:"#10B981",border:"none",borderRadius:12,color:"#fff",fontWeight:700,fontSize:14,cursor:"pointer",touchAction:"manipulation",fontFamily:"inherit",opacity:saving?0.7:1}}>{saving?"Guardando…":"Guardar distribución"}</button>
    </CapModal>
  );
  ...
  const modalObjetivo = (
    <CapModal open={modal==="objetivos"} onClose={closeModal} title={editItem?"Editar objetivo":"Nuevo objetivo"} subtitle="Meta de expansión del negocio">
      ...
      <div style={{display:"flex",gap:10,marginTop:8}}>
        {editItem&&<button onClick={()=>{deleteItem("objetivos",editItem.id);closeModal();}} style={{flex:1,padding:13,background:"rgba(239,68,68,0.1)",border:"1px solid rgba(239,68,68,0.3)",borderRadius:12,color:"#F87171",fontWeight:600,fontSize:13,cursor:"pointer",touchAction:"manipulation",fontFamily:"inherit"}}>Eliminar</button>}
        <button onClick={()=>saveItem("objetivos")} disabled={saving} style={{flex:2,padding:13,background:"#10B981",border:"none",borderRadius:12,color:"#fff",fontWeight:700,fontSize:14,cursor:"pointer",touchAction:"manipulation",fontFamily:"inherit",opacity:saving?0.7:1}}>{saving?"Guardando…":"Guardar"}</button>
      </div>
    </CapModal>
  );
  ...
  return (
    <div style={{paddingBottom:"calc(90px + env(safe-area-inset-bottom))",minHeight:"100%"}}>
      ...
      <div style={{padding:"20px 16px 0"}}>
        <div style={{fontSize:22,fontWeight:900,color:"#0D1117",letterSpacing:"-0.03em",marginBottom:2}}>Capital e Inversiones</div>
        <div style={{fontSize:12,color:"#9CA3AF",marginBottom:16}}>Sistema financiero del negocio</div>
        ...
        <div style={{display:"flex",gap:6,overflowX:"auto",paddingBottom:4,marginBottom:16,scrollbarWidth:"none"}}>
          {TABS.map(t=>(
            <button key={t.id} onClick={()=>setSection(t.id)} style={{
              ...
              background:section===t.id?"#10B981":"#fff",
              border:`1px solid ${section===t.id?"#10B981":"#E5E7EB"}`,
              color:section===t.id?"#fff":"#6B7280",
              fontSize:12,fontWeight:section===t.id?700:500,cursor:"pointer",touchAction:"manipulation",fontFamily:"inherit",
              boxShadow:section===t.id?"0 2px 8px rgba(16,185,129,0.3)":"none",
            }}>
...
export default function App() {
  ...
  useEffect(() => {
    const navy = ((tab === "hoy" || tab === "capital" || tab === "contratos") && !showProfile);
    ...
    <>
      ...
      {!splash && !!user && <div style={{ position: "fixed", top: 0, left: 0, right: 0, bottom: 0, background: NL.bg, display: "flex", flexDirection: "column", fontFamily: "'DM Sans',sans-serif", color: NL.text, overflow: "hidden", overscrollBehavior: "none" }}>
        ...
        <div style={{
          ...
          background: ((tab === "hoy" || tab === "capital" || tab === "contratos") && !showProfile)
            ? "#0E1A3B"
            : "#F2F4F8",
          borderBottom: ((tab === "hoy" || tab === "capital" || tab === "contratos") && !showProfile)
            ? "none"
          ...
          <button onClick={() => setDrawerOpen(true)} aria-label="Menú" style={{
            ...
            background: ((tab === "hoy" || tab === "capital" || tab === "contratos") && !showProfile) ? "rgba(255,255,255,0.10)" : NL.text,
            border: ((tab === "hoy" || tab === "capital" || tab === "contratos") && !showProfile) ? "1px solid rgba(255,255,255,0.14)" : "none",
            cursor: "pointer", touchAction: "manipulation",
            display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0,
            boxShadow: ((tab === "hoy" || tab === "capital" || tab === "contratos") && !showProfile) ? "none" : "0 4px 12px rgba(15,23,41,0.18)",
          }}>
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ fontSize: 15, fontWeight: 700, color: ((tab === "hoy" || tab === "capital" || tab === "contratos") && !showProfile) ? "#fff" : NL.text, lineHeight: 1.1 }}>
              {showProfile ? "Perfil" : {
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 6, flexShrink: 0 }}>
            <button onClick={() => setGlobalSearch(true)} aria-label="Buscar" style={{ width: 40, height: 40, borderRadius: "50%", background: ((tab === "hoy" || tab === "capital" || tab === "contratos") && !showProfile) ? "rgba(255,255,255,0.10)" : NL.white, border: ((tab === "hoy" || tab === "capital" || tab === "contratos") && !showProfile) ? "1px solid rgba(255,255,255,0.14)" : `1px solid ${NL.border}`, display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", touchAction: "manipulation" }}>
              <svg width="17" height="17" fill="none" viewBox="0 0 24 24">
                <path d="M21 21L15 15M17 11C17 14.866 13.866 18 10 18C6.134 18 3 14.866 3 11C3 7.134 6.134 4 10 4C13.866 4 17 7.134 17 11Z" stroke={((tab === "hoy" || tab === "capital" || tab === "contratos") && !showProfile) ? "#fff" : NL.text} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
              </svg>
            </button>
            <button onClick={() => setNotifOpen(v => !v)} aria-label="Notificaciones" style={{ width: 40, height: 40, borderRadius: "50%", background: ((tab === "hoy" || tab === "capital" || tab === "contratos") && !showProfile) ? "rgba(255,255,255,0.10)" : NL.white, border: ((tab === "hoy" || tab === "capital" || tab === "contratos") && !showProfile) ? "1px solid rgba(255,255,255,0.14)" : `1px solid ${NL.border}`, display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", touchAction: "manipulation", position: "relative" }}>
              <svg width="17" height="17" fill="none" viewBox="0 0 24 24">
                <path d="M15 17H9M15 17C15 18.657 13.657 20 12 20C10.343 20 9 18.657 9 17M15 17H20L18.784 15.784C18.284 15.284 18 14.612 18 13.914V10C18 7.239 15.761 5 13 5H11C8.239 5 6 7.239 6 10V13.914C6 14.612 5.716 15.284 5.216 15.784L4 17H9" stroke={((tab === "hoy" || tab === "capital" || tab === "contratos") && !showProfile) ? "#fff" : NL.text} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
              </svg>
              {contratos.length > 0 && (() => {
                const hoyD = new Date();
                const count =
        ...
        <div
          ref={scrollRef}
          data-scroll
          style={{ flex: 1, minHeight: 0, overflowY: "scroll", overflowX: "hidden", overscrollBehavior: "none", touchAction: "pan-y", background: activeTab === "contratos" ? "#0E1A3B" : activeTab === "capital" ? "#F2F4F8" : undefined, position:"relative" }}
        >
          <div style={{ paddingTop: (activeTab === "capital" || activeTab === "contratos") ? 0 : 20, paddingLeft: (activeTab === "capital" || activeTab === "contratos") ? 0 : 16, paddingRight: (activeTab === "capital" || activeTab === "contratos") ? 0 : 16, paddingBottom: "calc(100px + env(safe-area-inset-bottom))" }}>
            {loading
