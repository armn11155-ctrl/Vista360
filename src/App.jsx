...
function Contratos({contratos,setContratos,paneles,clientes,loading,setTab,onModalChange}){
  ...
  const datos=contratos.filter(c=>!c.deleted).map(c=>({...c,d:dias(c.fin),panel:paneles.find(p=>p.id===c.panel_id),cliente:clientes.find(cl=>cl.id===c.cliente_id)})).filter(c=>c.panel&&c.cliente);
  const datosEliminados=contratos.filter(c=>c.deleted).map(c=>({...c,d:dias(c.fin),panel:paneles.find(p=>p.id===c.panel_id),cliente:clientes.find(cl=>cl.id===c.cliente_id)}));
...
function Capital({ paneles, contratos, gastos, proveedores }) {
  ...
  return (
    <div style={{ background: "#F2F4F8", margin: "-20px -16px", minHeight: "100%", paddingBottom: "calc(120px + env(safe-area-inset-bottom))" }}>
      ...
      <div style={{ background: "linear-gradient(160deg,#0A1020 0%,#0F1729 60%,#1E2D4A 100%)", borderRadius: "0 0 32px 32px", padding: "32px 24px 36px", boxShadow: "0 12px 40px rgba(10,16,32,0.22)", marginBottom: 20 }}>
        <div style={{ fontSize: 10, color: "rgba(180,210,255,0.5)", fontWeight: 700, letterSpacing: 2, textTransform: "uppercase", marginBottom: 24 }}>Vista360 · 8 Millas — Capital</div>
        <div style={{ fontSize: 12, color: "rgba(180,210,255,0.55)", marginBottom: 10, fontWeight: 500 }}>Tu capital disponible</div>
        {editando ? (
          <div>
            <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 14 }}>
              <span style={{ fontSize: 28, fontWeight: 900, color: "rgba(180,210,255,0.4)" }}>S/</span>
              <input ref={inputRef} type="number" value={inputVal} onChange={e => setInputVal(e.target.value)} onKeyDown={e => { if (e.key === "Enter") guardarCapital(); if (e.key === "Escape") setEditando(false); }} placeholder="0.00"
                style={{ flex: 1, background: "rgba(37,99,235,0.12)", border: "2px solid rgba(37,99,235,0.6)", borderRadius: 16, padding: "12px 18px", color: "#fff", fontSize: 32, fontWeight: 900, outline: "none", fontFamily: "inherit", letterSpacing: "-1px" }}/>
            </div>
            <div style={{ display: "flex", gap: 10 }}>
              <button onClick={() => setEditando(false)} style={{ flex: 1, padding: "12px", background: "rgba(255,255,255,0.07)", border: "1px solid rgba(255,255,255,0.12)", borderRadius: 14, color: "rgba(200,220,255,0.6)", fontWeight: 600, fontSize: 14, cursor: "pointer", touchAction: "manipulation", fontFamily: "inherit" }}>Cancelar</button>
              <button onClick={guardarCapital} style={{ flex: 2, padding: "12px", background: "linear-gradient(135deg,#2563EB,#1D4ED8)", border: "none", borderRadius: 14, color: "#fff", fontWeight: 700, fontSize: 14, cursor: "pointer", touchAction: "manipulation", fontFamily: "inherit", boxShadow: "0 4px 16px rgba(37,99,235,0.5)" }}>Guardar</button>
            </div>
          </div>
        ) : (
          <div>
            <div style={{ fontSize: 52, fontWeight: 900, color: capital > 0 ? "#fff" : "rgba(200,220,255,0.25)", letterSpacing: "-2.5px", lineHeight: 1, marginBottom: 20 }}>{fmt(capital)}</div>
            <button onClick={abrirEditor} style={{ display: "inline-flex", alignItems: "center", gap: 7, background: "rgba(37,99,235,0.2)", border: "1px solid rgba(96,165,250,0.35)", borderRadius: 50, padding: "9px 20px", color: "#93C5FD", fontWeight: 700, fontSize: 13, cursor: "pointer", touchAction: "manipulation", fontFamily: "inherit" }}>
            ...
            <div style={{ fontSize: 11, color: "rgba(180,210,255,0.28)", marginTop: 14 }}>Ingresada manualmente</div>
          </div>
        )}
      </div>

      <div style={{ margin: "0 16px 14px", background: "linear-gradient(145deg,#1E3A8A 0%,#1E40AF 100%)", borderRadius: 24, padding: "22px 20px 20px", boxShadow: "0 8px 32px rgba(30,58,138,0.3)" }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 22 }}>
          <div style={{ fontSize: 11, color: "rgba(200,220,255,0.65)", fontWeight: 700, letterSpacing: 1.4, textTransform: "uppercase" }}>Crecimiento · 6 meses</div>
          <div style={{ background: crecPct >= 0 ? "rgba(16,185,129,0.2)" : "rgba(239,68,68,0.2)", border: "1px solid " + (crecPct >= 0 ? "rgba(16,185,129,0.5)" : "rgba(239,68,68,0.5)"), borderRadius: 99, padding: "4px 12px", fontSize: 12, fontWeight: 700, color: crecPct >= 0 ? "#34D399" : "#FCA5A5" }}>
            {crecPct >= 0 ? "+" : ""}{crecPct}% vs mes ant.
          </div>
        </div>
        <div style={{ display: "flex", alignItems: "flex-end", gap: 7, height: 100, marginBottom: 10 }}>
          {flujo.map(({ mes, ing, eg }, idx) => {
            const isActual = idx === 5;
            return (
              <div key={mes} style={{ flex: 1, height: "100%", display: "flex", flexDirection: "column", justifyContent: "flex-end", gap: 2, minWidth: 0 }}>
                {ing > 0 && <div style={{ width: "100%", height: (ing/maxBar*100)+"%", minHeight: 5, borderRadius: "5px 5px 0 0", background: isActual ? "linear-gradient(180deg,#fff,rgba(255,255,255,0.7))" : "rgba(255,255,255,0.18)", boxShadow: isActual ? "0 0 16px rgba(255,255,255,0.35)" : "none", transition: "height .4s" }}/>}
                {eg > 0 && <div style={{ width: "100%", height: (eg/maxBar*100)+"%", minHeight: 5, borderRadius: "5px 5px 0 0", background: isActual ? "linear-gradient(180deg,#FCA5A5,#EF4444)" : "rgba(239,68,68,0.3)", transition: "height .4s" }}/>}
                {ing === 0 && eg === 0 && <div style={{ width: "100%", height: 4, background: "rgba(255,255,255,0.07)", borderRadius: 2 }}/>}
              </div>
            );
          })}
        </div>
        <div style={{ display: "flex", gap: 7, marginBottom: 14 }}>
          {flujo.map(({ mes }, idx) => (
            <div key={mes} style={{ flex: 1, textAlign: "center", fontSize: 9, color: idx === 5 ? "rgba(255,255,255,0.9)" : "rgba(200,220,255,0.4)", fontWeight: idx === 5 ? 700 : 400, textTransform: "uppercase" }}>{mC(mes)}</div>
          ))}
        </div>
        <div style={{ display: "flex", gap: 16, justifyContent: "center" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 5, fontSize: 11, color: "rgba(200,220,255,0.6)" }}><div style={{ width: 9, height: 9, borderRadius: 2, background: "rgba(255,255,255,0.75)" }}/> Ingresos</div>
          <div style={{ display: "flex", alignItems: "center", gap: 5, fontSize: 11, color: "rgba(200,220,255,0.6)" }}><div style={{ width: 9, height: 9, borderRadius: 2, background: "#EF4444" }}/> Gastos</div>
        </div>
      </div>

      <div style={{ margin: "0 16px", background: "#fff", borderRadius: 20, border: "1px solid #E5E7EB", boxShadow: "0 1px 4px rgba(15,23,41,0.06)", overflow: "hidden" }}>
        {[
          { label: "Ingresos 6 meses", value: fmt(ingTotal), color: "#10B981" },
          { label: "Gastos 6 meses",   value: fmt(gasTotal), color: "#EF4444" },
          { label: "Resultado neto",   value: fmt(ingTotal - gasTotal), color: (ingTotal-gasTotal) >= 0 ? "#10B981" : "#EF4444", bold: true },
        ].map(({ label, value, color, bold }, i, arr) => (
          <div key={label} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "15px 20px", borderBottom: i < arr.length-1 ? "1px solid #F1F5F9" : "none", background: bold ? "#F8FAFC" : "#fff" }}>
            <span style={{ fontSize: 13, color: bold ? "#0F1729" : "#64748B", fontWeight: bold ? 700 : 500 }}>{label}</span>
