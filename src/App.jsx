...
function Hoy({clientes,contratos,paneles,gastos,setTab}){
  const acciones = useMemo(()=>{
    ...
    return lista.sort((a,b)=>ord[a.prioridad]-ord[b.prioridad]).slice(0,15);
  ...
  return(<div>
    <PgTit icon="🌅" title="HOY" sub={`${hoyD.toLocaleDateString("es-PE",{weekday:"long",day:"numeric",month:"long",year:"numeric"})}`}/>

    {/* KPIs rápidos */}
    <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:12,marginBottom:22}}>
      {[
        {ic:"💵",label:"Ingreso del mes",val:fmt(ingMes),color:C.green},
        {ic:"📡",label:"Paneles libres",val:panelLibres,color:panelLibres>0?C.amber:C.green},
        {ic:"⏳",label:"Por cobrar",val:fmt(cobPendiente),color:cobPendiente>0?C.red:C.green},
        {ic:"🚨",label:"Vencen en 7 días",val:vence7,color:vence7>0?C.red:C.green},
      ].map(k=>(
        <Card key={k.label} style={{padding:"16px 18px",textAlign:"center"}}>
          ...
          <div style={{fontSize:10,color:C.muted,marginTop:4,textTransform:"uppercase",letterSpacing:0.8}}>{k.label}</div>
        </Card>
      ))}
    </div>
    ...
    <Card>
      <SecTit ch="⚡ Acciones recomendadas para hoy"/>
      {acciones.length===0?(
        <div style={{textAlign:"center",padding:"40px 20px",color:C.muted}}>
          <div style={{fontSize:40,marginBottom:12}}>🎉</div>
          <div style={{fontSize:15,fontWeight:700,color:C.white,marginBottom:6}}>¡Todo al día!</div>
          <div style={{fontSize:13}}>No hay acciones urgentes por el momento.</div>
        </div>
      ):(
        <div style={{display:"flex",flexDirection:"column",gap:10}}>
          {acciones.map((a,i)=>(
            ...
            >
              <div style={{fontSize:24,flexShrink:0}}>{a.icono}</div>
              <div style={{flex:1,minWidth:0}}>
                <div style={{fontSize:13,fontWeight:700,color:C.white,marginBottom:2}}>{a.titulo}</div>
                <div style={{fontSize:12,color:C.muted,whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>{a.desc}</div>
              </div>
              <div style={{display:"flex",alignItems:"center",gap:8,flexShrink:0}}>
                <span style={{
                  fontSize:10,fontWeight:700,
                  background:a.prioridad==="alta"?C.red+"22":a.prioridad==="media"?C.amber+"22":C.muted+"22",
                  color:a.prioridad==="alta"?C.red:a.prioridad==="media"?C.amber:C.muted,
                  border:`1px solid ${a.prioridad==="alta"?C.red:a.prioridad==="media"?C.amber:C.muted}44`,
                  borderRadius:20,padding:"3px 9px",textTransform:"uppercase",letterSpacing:0.5,
                }}>{a.prioridad}</span>
                <span style={{fontSize:11,color:a.color,fontWeight:600,whiteSpace:"nowrap"}}>{a.accion} →</span>
              </div>
            </div>
          ))}
        </div>
