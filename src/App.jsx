...
function Gastos({gastos,setGastos}){
  ...
  const escanear=async(file)=>{
    ...
    try{
      // ── Fase 1: Pre-procesamiento Canvas ──────────────────────
      setOcr(o=>({...o,progress:12,fase:"Pre-procesando imagen (binarización adaptativa)..."}));
      let imagenOcr, previewUrl=imgUrl;
      try{
        const resultado=await preprocesarImagen(file);
        imagenOcr=resultado.blob;
        previewUrl=resultado.previewUrl;
        setOcr(o=>({...o,previewUrl,progress:28,fase:"Imagen procesada ✓ — Cargando OCR..."}));
      }catch(_){
        imagenOcr=file;
        setOcr(o=>({...o,progress:28,fase:"Cargando motor OCR..."}));
      }

      // ── Fase 2: Cargar Tesseract ──────────────────────────────
      const Tesseract=await cargarTesseract();
      setOcr(o=>({...o,progress:35,fase:"Motor OCR listo — Reconociendo texto..."}));

      // ── Fase 3: Reconocimiento OCR ────────────────────────────
      const {data:{text:textoRaw}}=await Tesseract.recognize(
        imagenOcr,
        "spa",
        {
          logger:(m)=>{
            if(m.status==="recognizing text"){
              setOcr(o=>({...o,
                progress:Math.round(35+m.progress*50),
                fase:`Reconociendo texto... ${Math.round(m.progress*100)}%`
              }));
            }
          },
      ...
      if(!textoRaw||textoRaw.trim().length<8)
        throw new Error("No se detectó texto en la imagen. Verifica iluminación y enfoque.");

      // ── Fase 4: Extracción inteligente con RegEx ──────────────
      setOcr(o=>({...o,progress:90,fase:"Extrayendo datos estructurados..."}));
      const datosExtraidos=extraerDatosInteligente(textoRaw);

      setOcr(o=>({...o,loading:false,progress:100,fase:"✓ Completado",text:textoRaw,imgUrl,previewUrl}));

      // ── Fase 5: Rellenar formulario ───────────────────────────
      setForm(f=>({
        ...f,
