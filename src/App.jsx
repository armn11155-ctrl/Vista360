...
const fb = {
  ...
  // Igual que get() pero propaga el error si ambas queries fallan.
  // Usada en la carga inicial para detectar problemas de token / permisos en iOS.
  async getStrict(col) {
    try {
      const q    = query(collection(db, col), orderBy("createdAt","desc"));
      const snap = await getDocs(q);
      ...
      // Fallback sin orderBy (por si falta el índice compuesto)
      const snap = await getDocs(collection(db, col)); // lanza si hay error real
      return snap.docs.map(d => ({ id: d.id, ...d.data() }));
    }
  },
  async post(col, body) {
...
function LoginScreen({ onLoginSuccess }) {
  ...
  const handleGoogleLogin = async () => {
    ...
    setError("");
    try {
      // signInWithPopup funciona en iOS Safari cuando viene de un gesto del usuario.
      // NO usar signInWithRedirect en iOS PWA: el callback va a firebaseapp.com
      // (distinto origen) → iOS lo abre en Safari en vez de volver a la PWA → loop roto.
      ...
      if (err.code === "auth/popup-closed-by-user") {
        setError("Cerraste la ventana antes de terminar. Intenta de nuevo.");
      } else if (err.code === "auth/popup-blocked") {
        // En iOS PWA puede bloquearse — instruir al usuario
        setError("Tu navegador bloqueó la ventana de Google. Abre la app desde Safari (no desde la pantalla de inicio) e intenta de nuevo.");
      } else if (err.code === "auth/cancelled-popup-request") {
        setError(""); // solicitud cancelada, no mostrar error
...
export default function App() {
  ...
  useEffect(() => {
    const unsub = onAuthStateChanged(auth, (u) => {
      ...
      if (u && ALLOWED_EMAILS.length > 0 && !ALLOWED_EMAILS.includes(u.email)) {
        signOut(auth);
        setUser(null);
        // Sin usuario autorizado → no hay datos que cargar, quitar loading
        setLoading(false);
      } else {
        setUser(u);
        // FIX iOS: si no hay sesión activa, loading nunca se resolvería
        // porque el useEffect de datos retorna early con !user.
  ...
  const recargarDatos = useCallback(() => setRetryCount(n => n + 1), []);
  ...
  useEffect(() => {
    ...
    const cargar = async () => {
      try {
        // CRITICO iOS: forzar refresh del token ANTES de queries a Firestore.
        // Despues de signInWithRedirect el token puede no estar propagado
        // y Firestore devuelve permission-denied que antes se tragaba silenciosamente.
        await user.getIdToken(true);

        const [c, p, ct, g, pr] = await Promise.all([
          fb.getStrict("clientes"),
          fb.getStrict("paneles"),
          fb.getStrict("contratos"),
          fb.getStrict("gastos"),
          fb.getStrict("proveedores"),
        ]);
        ...
        clearTimeout(timeout);
        console.error("[Vista360] Error cargando datos:", err?.code, err?.message);
        setError(err?.code || "load-error");
        setLoading(false);
      }
    };
              ...
              <div style={{background:NL.white,borderRadius:20,padding:28,width:"100%",maxWidth:320,textAlign:"center"}}>
                ...
                <div style={{display:"flex",gap:10}}>
                  ...
                  <button onClick={async()=>{
                      ...
                      setUser(null);
                      setClientes([]); setPaneles([]); setContratos([]);
                      setGastos([]); setProveedores([]);
                      setError(null); setLoading(false);
                      setShowProfile(false); setConfirmLogout(false);
              ...
              ? <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", height: 280, gap: 16, padding: 24, textAlign: "center" }}>
                  ...
                  <div style={{ fontSize: 13, color: NL.muted, lineHeight: 1.5, maxWidth: 280 }}>
                    {error === "timeout"
                      ...
                      : `Error: ${error}. Verifica tu conexión a internet.`}
                  </div>
                  <button
                    onClick={recargarDatos}
                    style={{ padding: "12px 28px", background: NL.accent, color: "#fff", border: "none", borderRadius: 12, fontWeight: 700, fontSize: 15, cursor: "pointer", boxShadow: "0 4px 12px rgba(37,99,235,0.3)" }}
                  >
                    🔄 Reintentar
                  </button>
                  {error === "permission-denied" && (
