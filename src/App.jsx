...
function CRM({clientes,setClientes,contratos,loading}){
  ...
  const thSt=(col)=>({padding:"13px 16px",textAlign:"left",fontSize:10,fontWeight:700,textTransform:"uppercase",letterSpacing:1.4,cursor:"pointer",whiteSpace:"nowrap",color:orden.col===col?C.accent:C.muted,background:"transparent",transition:"color .15s"});
  const Th=({col,label})=><th style={thSt(col)} onClick={()=>setOrden(o=>({col,dir:o.col===col?-o.dir:1}))}>{label}{orden.col===col?<span style={{marginLeft:4,opacity:0.7}}>{orden.dir===1?"↑":"↓"}</span>:null}</th>;
  const td={padding:"14px 16px",verticalAlign:"middle"};

  // Micro sparkline SVG for metric cards
  const Sparkline=({color,up})=>{
    const pts=up?[0,8,4,14,9,18,12,22,16,28]:[28,22,24,16,20,12,14,8,10,4];
    const path=pts.reduce((s,y,i)=>s+(i===0?`M ${i*11} ${30-y}`:`L ${i*11} ${30-y}`),"");
    return(<svg width={100} height={32} viewBox="0 0 99 32" fill="none" style={{opacity:0.7}}>
      <path d={path} stroke={color} strokeWidth={2} strokeLinecap="round" strokeLinejoin="round"/>
      <path d={path+` L 99 32 L 0 32 Z`} fill={`url(#sg${color.replace("#","")})`} opacity={0.2}/>
      <defs><linearGradient id={`sg${color.replace("#","")}`} x1="0" y1="0" x2="0" y2="1"><stop stopColor={color}/><stop offset="1" stopColor={color} stopOpacity="0"/></linearGradient></defs>
    </svg>);
  };

  const metricCards=[
    {icon:"👥",label:"CLIENTES",value:clis.length,sub:"registrados",color:C.green,up:true},
    {icon:"🎯",label:"PROSPECTOS",value:pros.length,sub:"en pipeline",color:C.purple,up:pros.length>0},
    {icon:"🔥",label:"PROPUESTAS",value:pros.filter(p=>p.estado==="Propuesta enviada").length,sub:"por cerrar",color:C.amber,up:false},
  ];
  ...
  return(<div>
    ...
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:24}}>
      <div>
        <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:4}}>
          <div style={{width:38,height:38,borderRadius:12,background:`linear-gradient(135deg,${C.green}22,${C.green}44)`,border:`1px solid ${C.green}33`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:18}}>👥</div>
          <div style={{fontSize:22,fontWeight:800,color:C.white,letterSpacing:"-0.5px"}}>Clientes & Prospectos</div>
        </div>
        <div style={{fontSize:12,color:C.muted,marginLeft:48}}>CRM · {clientes.length} contactos</div>
      </div>
      <button onClick={openNew} style={{
        background:`linear-gradient(135deg,${C.accent},#7B5FFF)`,
    ...
    <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:14,marginBottom:20}}>
      {metricCards.map(m=>(
        <div key={m.label} style={{
          background:`linear-gradient(145deg,rgba(12,20,36,0.95),rgba(6,11,22,0.98))`,
          border:`1px solid ${m.color}22`,
          borderRadius:16,padding:"18px 20px",
          boxShadow:`0 4px 32px rgba(0,0,0,0.5),inset 0 1px 0 rgba(255,255,255,0.04),0 0 0 1px ${m.color}11`,
          position:"relative",overflow:"hidden",
          backdropFilter:"blur(10px)",WebkitBackdropFilter:"blur(10px)",
        }}>
          <div style={{position:"absolute",bottom:0,right:0,opacity:0.5}}><Sparkline color={m.color} up={m.up}/></div>
          <div style={{
            width:38,height:38,borderRadius:10,
            background:`linear-gradient(135deg,${m.color}25,${m.color}40)`,
            border:`1px solid ${m.color}33`,
            display:"flex",alignItems:"center",justifyContent:"center",
            fontSize:18,marginBottom:10,
          }}>{m.icon}</div>
          <div style={{fontSize:10,fontWeight:700,color:m.color,textTransform:"uppercase",letterSpacing:1.5,marginBottom:6}}>{m.label}</div>
          <div style={{fontSize:32,fontWeight:800,color:C.white,letterSpacing:"-2px",lineHeight:1}}>{m.value}</div>
    ...
    {/* ── Search & Filters ── */}
    <div style={{
      background:"linear-gradient(135deg,rgba(12,22,40,0.9),rgba(8,14,26,0.95))",
      ...
      <div style={{position:"relative",flex:1,minWidth:200}}>
        <svg style={{position:"absolute",left:12,top:"50%",transform:"translateY(-50%)",opacity:0.35}} width="15" height="15" viewBox="0 0 24 24" fill="none" stroke={C.text} strokeWidth="2"><circle cx="11" cy="11" r="8"/><path d="m21 21-4.35-4.35"/></svg>
        <input value={buscar} onChange={e=>setBuscar(e.target.value)} placeholder="Buscar empresa, contacto, ciudad..."
          style={{background:"rgba(255,255,255,0.04)",border:`1px solid rgba(79,124,255,0.15)`,borderRadius:10,padding:"9px 14px 9px 36px",color:C.text,fontSize:13,outline:"none",fontFamily:"inherit",width:"100%",boxSizing:"border-box",transition:"border-color .15s"}}/>
      </div>
      <div style={{display:"flex",gap:5}}>
        {["Todos","Cliente","Prospecto"].map(t=>(
          <button key={t} onClick={()=>setFiltroTipo(t)} style={{
            padding:"8px 14px",borderRadius:20,
            border:`1px solid ${filtroTipo===t?C.accent+"66":C.border}`,
            background:filtroTipo===t?`linear-gradient(135deg,${C.accent}22,${C.accent}33)`:"transparent",
            color:filtroTipo===t?C.accent:C.muted,fontWeight:filtroTipo===t?700:500,fontSize:12,cursor:"pointer",
            boxShadow:filtroTipo===t?`0 0 12px ${C.accent}22`:"none",
            transition:"all .18s",
          }}>{t}</button>
        ))}
      </div>
      <select value={filtroEst} onChange={e=>setFiltroEst(e.target.value)} style={{background:"rgba(255,255,255,0.04)",border:`1px solid ${C.border}`,borderRadius:10,padding:"8px 12px",color:C.text,fontSize:12,outline:"none",fontFamily:"inherit",cursor:"pointer"}}>
        {todosEst.map(e=><option key={e} value={e}>{e}</option>)}
      </select>
      <span style={{fontSize:12,color:C.muted,whiteSpace:"nowrap"}}>{filtrado.length} resultado{filtrado.length!==1?"s":""}</span>
    </div>

    {/* ── Table ── */}
    {loading?<Spinner/>:
    <div style={{
      background:"linear-gradient(145deg,rgba(10,18,34,0.97),rgba(6,11,20,0.99))",
      ...
      <div style={{overflowX:"auto"}}>
        <table style={{width:"100%",borderCollapse:"collapse"}}>
          <thead>
            <tr style={{borderBottom:`1px solid rgba(79,124,255,0.1)`}}>
              <Th col="empresa" label="Empresa"/>
              <Th col="tipo" label="Tipo"/>
              <Th col="ciudad" label="Ciudad"/>
              <Th col="sector" label="Sector"/>
              <Th col="estado" label="Estado"/>
              <th style={thSt("x")}>Contacto</th>
              <th style={thSt("x")}>Notas</th>
              <th style={thSt("x")}>Acciones</th>
            </tr>
          </thead>
          <tbody>
            {filtrado.length===0
              ?<tr><td colSpan={8} style={{padding:48,textAlign:"center",color:C.muted}}>
                ...
                <button onClick={openNew} style={{color:C.accent,background:`${C.accent}15`,border:`1px solid ${C.accent}33`,borderRadius:8,padding:"8px 16px",cursor:"pointer",fontWeight:700,fontSize:13}}>+ Agregar contacto</button>
              </td></tr>
              :filtrado.map((r,ri)=>{
                ...
                return(
                  <tr key={r.id}
                    onMouseEnter={e=>{e.currentTarget.style.background="rgba(79,124,255,0.05)";e.currentTarget.style.boxShadow="inset 0 0 0 1px rgba(79,124,255,0.08)";}}
                    onMouseLeave={e=>{e.currentTarget.style.background="transparent";e.currentTarget.style.boxShadow="none";}}
                    style={{transition:"background .15s,box-shadow .15s",borderBottom:`1px solid rgba(22,32,53,0.8)`}}>
                    <td style={td}>
                      <div style={{display:"flex",alignItems:"center",gap:11}}>
                        <div style={{
                          width:36,height:36,borderRadius:10,
                          background:`linear-gradient(135deg,${tc}22,${tc}40)`,
                          border:`1px solid ${tc}33`,
                          color:tc,display:"flex",alignItems:"center",justifyContent:"center",
                          fontWeight:800,fontSize:11,flexShrink:0,
                          boxShadow:`0 2px 8px ${tc}22`,
                        }}>{ini}</div>
                        <div>
                          <div style={{fontWeight:700,color:C.white,fontSize:13,letterSpacing:"-0.2px"}}>{r.empresa}</div>
                          <div style={{fontSize:11,color:C.muted,marginTop:1}}>{r.contacto||"—"}</div>
                        </div>
                      </div>
                    </td>
                    <td style={td}>
                    ...
                    <td style={td}><span style={{fontSize:12,color:C.muted}}>{r.ciudad}</span></td>
                    <td style={td}>
                      <span style={{fontSize:11,background:"rgba(255,255,255,0.06)",borderRadius:6,padding:"3px 9px",color:"rgba(255,255,255,0.55)",border:"1px solid rgba(255,255,255,0.06)"}}>{r.sector}</span>
                    </td>
                    <td style={td}>
                      <span style={{
                        background:`${col}15`,color:col,border:`1px solid ${col}30`,
                        borderRadius:20,padding:"4px 11px",fontSize:11,fontWeight:600,whiteSpace:"nowrap",
                      }}>{r.estado}</span>
                    </td>
                    <td style={td}>
                      <div style={{fontSize:12,color:"rgba(255,255,255,0.6)"}}>{r.celular||"—"}</div>
                      <div style={{fontSize:11,color:C.muted,marginTop:1}}>{r.email||""}</div>
                    </td>
                    <td style={{...td,maxWidth:130}}>
                      <div style={{fontSize:11,color:C.muted,whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis",maxWidth:120}} title={r.notas}>{r.notas||"—"}</div>
                    </td>
                    <td style={td}>
                      <div style={{display:"flex",gap:5}}>
      ...
      <div style={{padding:"12px 20px",borderTop:`1px solid rgba(22,32,53,0.9)`,display:"flex",justifyContent:"space-between",alignItems:"center",fontSize:12,color:C.muted}}>
        <span style={{display:"flex",alignItems:"center",gap:6}}>
          <span style={{width:7,height:7,borderRadius:"50%",background:C.green,boxShadow:`0 0 6px ${C.green}88`,display:"inline-block"}}/>
...
function Hoy({clientes,contratos,paneles,gastos,setTab}){
  ...
  const vence7=contratos.filter(c=>{const d=Math.ceil((new Date(c.fin)-hoyD)/86400000);return d>=0&&d<=7;}).length;
  ...
  <div style={{paddingBottom:24}}>
    ...
    <div style={{
      ...
      background:"#060818",
      boxShadow:"0 16px 60px rgba(0,0,0,0.8),0 0 0 1px rgba(109,40,217,0.25)",
    }}>
      {/* Mountain background SVG gradient */}
      <div style={{position:"absolute",inset:0,background:"linear-gradient(135deg,#060C22 0%,#0A1240 25%,#130E48 50%,#1A0A50 70%,#1E0855 100%)"}}/>
      {/* Purple bloom right */}
      <div style={{position:"absolute",top:"-20%",right:"-5%",width:"60%",height:"140%",background:"radial-gradient(ellipse at center,rgba(139,92,246,0.42) 0%,rgba(109,40,217,0.2) 35%,transparent 70%)",pointerEvents:"none"}}/>
      {/* Blue glow left */}
      <div style={{position:"absolute",top:"5%",left:"-5%",width:"50%",height:"90%",background:"radial-gradient(ellipse at center,rgba(37,99,235,0.18) 0%,transparent 65%)",pointerEvents:"none"}}/>
      {/* Stars */}
      {[[8,8,0.9,2.5],[14,5,0.6,2],[22,14,0.7,2],[36,4,0.5,1.8],[52,10,0.8,2.5],[68,6,0.4,1.8],[78,16,0.6,2],[88,8,0.7,2],[95,12,0.5,1.5],[62,3,0.9,1.5]].map(([rx,ry,op,sz],i)=>(
        <div key={i} style={{position:"absolute",top:`${ry}%`,right:`${rx}%`,width:sz,height:sz,borderRadius:"50%",background:"white",opacity:op,boxShadow:`0 0 ${sz*3}px rgba(200,180,255,0.95)`,pointerEvents:"none"}}/>
      ))}
      {/* Mountain silhouettes */}
      <svg style={{position:"absolute",bottom:0,left:0,width:"100%",pointerEvents:"none"}} viewBox="0 0 500 120" preserveAspectRatio="none">
        <defs>
          <linearGradient id="mh1" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="#1E1255" stopOpacity="0.7"/><stop offset="100%" stopColor="#0A0820" stopOpacity="1"/></linearGradient>
          <linearGradient id="mh2" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="#160E45" stopOpacity="0.88"/><stop offset="100%" stopColor="#080618" stopOpacity="1"/></linearGradient>
          <linearGradient id="mh3" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="#100A30" stopOpacity="0.97"/><stop offset="100%" stopColor="#060412" stopOpacity="1"/></linearGradient>
          <linearGradient id="snw" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="#D8C8FF" stopOpacity="0.8"/><stop offset="100%" stopColor="#8B5CF6" stopOpacity="0"/></linearGradient>
        ...
        <polygon points="0,120 0,80 28,60 60,76 90,44 125,68 158,32 195,58 228,22 265,54 298,38 335,62 368,28 405,55 438,40 470,52 500,44 500,120" fill="url(#mh1)" filter="url(#mBlur)"/>
        <polygon points="230,22 228,22 236,36 222,37" fill="url(#snw)" filter="url(#sGlow)" opacity="0.55"/>
        <polygon points="156,32 158,32 165,44 151,45" fill="url(#snw)" filter="url(#sGlow)" opacity="0.5"/>
        <polygon points="366,28 368,28 375,40 361,41" fill="url(#snw)" filter="url(#sGlow)" opacity="0.45"/>
        <polygon points="0,120 0,92 22,82 48,90 72,70 102,84 130,56 164,78 192,64 222,82 252,60 284,76 312,66 345,80 372,54 400,72 428,62 458,78 485,66 500,72 500,120" fill="url(#mh2)" filter="url(#mBlur)"/>
        <polygon points="0,120 0,102 18,97 44,104 68,92 96,100 120,82 152,96 178,88 208,100 234,80 262,94 288,84 316,96 348,76 380,90 408,82 440,96 468,86 500,90 500,120" fill="url(#mh3)"/>
        <polyline points="68,92 96,100 120,82 152,96 178,88 208,100 234,80 262,94" stroke="rgba(139,92,246,0.2)" strokeWidth="1" fill="none"/>
        {/* Horizon glow */}
        <rect x="0" y="108" width="500" height="6" fill="url(#mh3)" opacity="0.6"/>
        <ellipse cx="230" cy="110" rx="120" ry="8" fill="rgba(180,100,255,0.12)"/>
      </svg>

      {/* Gradient overlay bottom */}
      <div style={{position:"absolute",bottom:0,left:0,right:0,height:"55%",background:"linear-gradient(to top,rgba(6,8,24,0.85) 0%,transparent 100%)",pointerEvents:"none"}}/>

      {/* Content */}
      <div style={{position:"relative",zIndex:4,padding:"26px 24px 28px"}}>
        <div style={{fontSize:14,color:"rgba(255,255,255,0.75)",fontWeight:500,marginBottom:4}}>
          {saludo}, <span style={{color:"#A78BFA",fontWeight:700}}>Alan</span> 👋
        </div>
        <div style={{fontSize:34,fontWeight:900,color:"#FFFFFF",letterSpacing:"-1.2px",lineHeight:1.05,marginBottom:8,textShadow:"0 2px 32px rgba(139,92,246,0.5)"}}>
          Resumen de hoy
        </div>
        {/* Fecha */}
        <div style={{display:"inline-flex",alignItems:"center",gap:8,background:"rgba(255,255,255,0.08)",border:"1px solid rgba(255,255,255,0.14)",borderRadius:30,padding:"6px 14px",backdropFilter:"blur(8px)",marginBottom:16}}>
          ...
          <span style={{fontSize:12,color:"rgba(255,255,255,0.88)",fontWeight:500}}>{fechaCap}</span>
        </div>
        {/* Insight badge */}
        <div style={{
          ...
          boxShadow:"0 4px 24px rgba(109,40,217,0.25),inset 0 1px 0 rgba(255,255,255,0.08)",
        }}>
          <div style={{width:28,height:28,borderRadius:8,background:"linear-gradient(135deg,#7C3AED,#4F7CFF)",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0,boxShadow:"0 0 14px rgba(124,58,237,0.6)"}}>
          ...
          <div>
            <div style={{fontSize:13,fontWeight:700,color:"#E9D5FF",lineHeight:1.2}}>{insightBadge.txt}</div>
            <div style={{fontSize:11,color:"rgba(196,167,255,0.75)",marginTop:1}}>{insightBadge.sub}</div>
          </div>
        </div>
      </div>
    </div>
    ...
    <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:10,marginBottom:16}}>
      ...
      <div style={{
        background:"linear-gradient(145deg,rgba(14,24,42,0.95),rgba(8,14,26,0.98))",
        border:`1px solid ${cobPendiente>0?"rgba(245,166,35,0.2)":"rgba(22,32,53,0.8)"}`,
        borderRadius:16,padding:"16px 12px",textAlign:"center",
        boxShadow:"0 4px 20px rgba(0,0,0,0.5),inset 0 1px 0 rgba(255,255,255,0.04)",
      }}>
        <div style={{width:34,height:34,borderRadius:10,background:`rgba(245,166,35,${cobPendiente>0?0.15:0.07})`,border:`1px solid rgba(245,166,35,${cobPendiente>0?0.35:0.15})`,display:"flex",alignItems:"center",justifyContent:"center",margin:"0 auto 8px"}}>
          <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke={cobPendiente>0?"#F5A623":"#4E6080"} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg>
        </div>
        <div style={{fontSize:11,color:"rgba(255,255,255,0.45)",textTransform:"uppercase",letterSpacing:0.8,marginBottom:4,fontWeight:700}}>Por cobrar</div>
        <div style={{fontSize:20,fontWeight:900,color:cobPendiente>0?"#F5A623":"#4E6080",letterSpacing:"-0.5px",textShadow:cobPendiente>0?"0 0 20px rgba(245,166,35,0.4)":"none"}}>{fmt(cobPendiente)}</div>
        <div style={{fontSize:10,color:"rgba(255,255,255,0.3)",marginTop:4}}>{contratos.filter(c=>!c.pagado&&c.inicio?.slice(0,7)<=mesKey&&c.fin?.slice(0,7)>=mesKey).length} de {contratos.length} contratos</div>
      </div>

      {/* Vencen pronto */}
      <div style={{
        background:"linear-gradient(145deg,rgba(14,24,42,0.95),rgba(8,14,26,0.98))",
        border:`1px solid ${vence7>0?"rgba(240,71,71,0.22)":"rgba(22,32,53,0.8)"}`,
        borderRadius:16,padding:"16px 12px",textAlign:"center",
        boxShadow:"0 4px 20px rgba(0,0,0,0.5),inset 0 1px 0 rgba(255,255,255,0.04)",
      }}>
        <div style={{width:34,height:34,borderRadius:10,background:`rgba(240,71,71,${vence7>0?0.15:0.07})`,border:`1px solid rgba(240,71,71,${vence7>0?0.35:0.15})`,display:"flex",alignItems:"center",justifyContent:"center",margin:"0 auto 8px"}}>
          <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke={vence7>0?"#F04747":"#4E6080"} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="4" width="18" height="18" rx="2"/><line x1="16" y1="2" x2="16" y2="6"/><line x1="8" y1="2" x2="8" y2="6"/><line x1="3" y1="10" x2="21" y2="10"/></svg>
        </div>
        <div style={{fontSize:11,color:"rgba(255,255,255,0.45)",textTransform:"uppercase",letterSpacing:0.8,marginBottom:4,fontWeight:700}}>Vencen pronto</div>
        <div style={{fontSize:28,fontWeight:900,color:vence7>0?"#F04747":"#4E6080",letterSpacing:"-1px",textShadow:vence7>0?"0 0 20px rgba(240,71,71,0.5)":"none"}}>{vence7}</div>
        <div style={{fontSize:10,color:"rgba(255,255,255,0.3)",marginTop:4}}>Próximos 7 días</div>
      </div>
    </div>
    ...
    <div style={{
      ...
      <div style={{flex:1,minWidth:0}}>
        <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:12}}>
          ...
          <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#10B981" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" style={{marginLeft:"auto"}}><polyline points="23 6 13.5 15.5 8.5 10.5 1 18"/><polyline points="17 6 23 6 23 12"/></svg>
        </div>
        <div style={{fontSize:13,color:"rgba(255,255,255,0.62)",lineHeight:1.7,marginBottom:10}}>
          {ocupPct===0
            ?<>Hoy estás al <span style={{color:"#A78BFA",fontWeight:700}}>0%</span> de ocupación.<br/>Si asignas 1 panel, puedes generar</>
            :<>Tienes <span style={{color:"#A78BFA",fontWeight:700}}>{ocupPct}%</span> de ocupación activa.<br/>Con {panelLibres} panel{panelLibres!==1?"es":""} libre{panelLibres!==1?"s":""} puedes añadir</>
          }
        </div>
        <div style={{fontSize:28,fontWeight:900,color:"#10B981",letterSpacing:"-1px",textShadow:"0 0 24px rgba(16,185,129,0.5)",marginBottom:14}}>
          {fmt(Math.max(ingPotencial,panelLibres>0?paneles.filter(p=>p.estado==="Libre").sort((a,b)=>Number(b.precio)-Number(a.precio))[0]?.precio||0:0))}/mes
        </div>
        <button
        ...
        {(()=>{
          ...
          return(
            <svg width={W} height={H+14} viewBox={`0 0 ${W} ${H+14}`}>
              ...
              {/* Dot */}
              <circle cx={dotX} cy={dotY} r="6" fill="#9B6FFF" filter="url(#gaugeGlow)"/>
      ...
      <div style={{display:"flex",flexDirection:"column",gap:10}}>
        {ranked.map((cl,i)=>{
          ...
          return(
            ...
            >
              <div style={{width:36,height:36,borderRadius:10,background:C.accent+"22",border:`1px solid ${C.accent}33`,display:"flex",alignItems:"center",justifyContent:"center",fontWeight:800,fontSize:11,color:C.accent,flexShrink:0}}>
...
export default function App(){
  ...
  return(<>
    ...
    <div style={{fontFamily:"'DM Sans',sans-serif",background:"#020510",height:"100dvh",color:C.text,overflow:"hidden",position:"relative"}}>
      ...
      {/* ── LAYOUT: padding creates the negative-space "gap" between modules ── */}
      <div style={{display:"flex",height:"100dvh",alignItems:"stretch",padding:"10px",gap:"10px",position:"relative",zIndex:1,boxSizing:"border-box"}}>

      {/* ── SIDEBAR MODULE — floating panel with depth ── */}
      <aside style={{
        ...
        overflowY:"hidden",
        boxShadow:"0 8px 48px rgba(0,0,0,0.7),inset 0 1px 0 rgba(255,255,255,0.04)",
        backdropFilter:"blur(20px)",WebkitBackdropFilter:"blur(20px)",
      }}>
        <div style={{position:"absolute",top:0,right:0,width:1,height:"100%",background:"linear-gradient(180deg,transparent 0%,rgba(79,124,255,0.25) 40%,rgba(155,111,255,0.15) 70%,transparent 100%)",pointerEvents:"none",zIndex:1}}/>
        <div style={{position:"absolute",top:0,left:0,right:0,height:1,background:"linear-gradient(90deg,transparent,rgba(79,124,255,0.2),transparent)",pointerEvents:"none",borderRadius:"18px 18px 0 0"}}/>
        ...
        {/* Nav */}
        <div style={{flex:1,width:"100%",overflowY:"auto",scrollbarWidth:"none",paddingTop:6,display:"flex",flexDirection:"column",gap:1}}>
          {tabs.map(t=>(
        ...
        {/* Bottom: Notif + Profile */}
        <div style={{
          width:"100%",
          borderTop:"1px solid rgba(79,124,255,0.07)",
          paddingBottom:8,flexShrink:0,
        }}>
          <button style={{
            width:"calc(100% - 14px)",margin:"8px 7px 2px",
            display:"flex",flexDirection:"row",alignItems:"center",
            gap:10,padding:"9px 12px",
            background:"none",border:"none",cursor:"pointer",
            color:"rgba(255,255,255,0.35)",borderRadius:11,
          }}>
            ...
            <span style={{fontSize:12,fontWeight:500,flex:1,textAlign:"left",color:"rgba(255,255,255,0.42)"}}>Notificaciones</span>
            <span style={{background:"linear-gradient(135deg,#6B4FFF,#4F7CFF)",color:"#fff",borderRadius:20,padding:"2px 7px",fontSize:10,fontWeight:800,minWidth:18,textAlign:"center",boxShadow:"0 0 10px rgba(79,124,255,0.45)"}}>3</span>
          </button>

          <button onClick={()=>setShowProfile(v=>!v)} style={{
            width:"calc(100% - 14px)",margin:"2px 7px 4px",
            display:"flex",flexDirection:"row",alignItems:"center",
            ...
            cursor:"pointer",borderRadius:11,transition:"all 0.15s",
          }}>
            <div style={{
            ...
            <div style={{flex:1,textAlign:"left",minWidth:0}}>
              <div style={{fontSize:12,fontWeight:600,color:"rgba(255,255,255,0.82)",whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>Alan Mart.</div>
              <div style={{fontSize:10,color:"rgba(255,255,255,0.28)",marginTop:1}}>Administrador ↓</div>
            </div>
          </button>
        </div>
      </aside>

      {/* ── CONTENT MODULE — elevated window ── */}
      <div style={{
        ...
        <main style={{flex:1,overflowY:"auto",overflowX:"hidden",padding:"20px 20px",paddingBottom:"calc(60px + env(safe-area-inset-bottom))",WebkitOverflowScrolling:"touch",scrollBehavior:"smooth"}}>
