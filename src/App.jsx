    ...
    {lista.length===0?(
      ...
      <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.6)",zIndex:400,display:"flex",alignItems:"flex-end",justifyContent:"center"}}
        ...
        <div style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:"22px 22px 0 0",padding:"10px 20px 24px",width:"100%",maxHeight:"92vh",overflowY:"auto",overscrollBehavior:"none",paddingBottom:"calc(24px + env(safe-area-inset-bottom))",boxShadow:"0 -10px 40px rgba(0,0,0,0.45)"}}
              ...
              <div style={{background:C.bg,borderRadius:12,padding:14,border:`1px solid ${C.border}`}}>
                <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:8}}>
                  {mesesForm.map((m,i)=>{
                    ...
                    return(
                      <button key={m.key} onClick={()=>toggleMesPago(m.key)}
                        style={{padding:"10px 6px",borderRadius:10,border:`2px solid ${pagado?C.green:C.border}`,background:pagado?C.green+"22":C.surface,cursor:"pointer",touchAction:"manipulation",transition:"background .08s",textAlign:"center"}}>
                        <div style={{fontSize:10,color:C.muted,marginBottom:3,fontWeight:600}}>Mes {i+1}</div>
                        <div style={{fontSize:12,fontWeight:700,color:pagado?C.green:C.muted,marginBottom:4}}>{m.label}</div>
          ...
          {/* Botones principales */}
          <div style={{display:"flex",gap:10,marginTop:8}}>
            <button onClick={closeModal} style={{flex:1,padding:"12px",borderRadius:12,border:`1px solid ${C.border}`,background:"transparent",color:C.muted,fontWeight:700,fontSize:14,cursor:"pointer",touchAction:"manipulation"}}>Cancelar</button>
            <button onClick={guardar} disabled={saving} style={{flex:2,padding:"12px",borderRadius:12,border:"none",background:C.accent,color:"#fff",fontWeight:800,fontSize:14,cursor:"pointer",touchAction:"manipulation",opacity:saving?.6:1}}>
              {saving?"Guardando...":"Guardar Cambios"}
            </button>
          </div>
          ...
          <div style={{marginTop:24,paddingTop:18,borderTop:`1px dashed ${C.border}`}}>
            {!confirmDelete?(
              ...
              <div style={{background:"#FEF2F2",border:"1px solid #FECACA",borderRadius:14,padding:16}}>
                ...
                <div style={{display:"flex",gap:10}}>
                  <button onClick={()=>setConfirmDelete(false)} style={{flex:1,padding:"11px",borderRadius:10,border:`1px solid ${C.border}`,background:"transparent",color:C.muted,fontWeight:600,fontSize:13,cursor:"pointer",touchAction:"manipulation"}}>
                    Cancelar
                  </button>
                  <button onClick={eliminar} disabled={saving}
                    style={{flex:1,padding:"11px",borderRadius:10,border:"none",background:C.red,color:"#fff",fontWeight:800,fontSize:13,cursor:"pointer",touchAction:"manipulation",opacity:saving?.6:1}}>
                    {saving?"Eliminando...":"Sí, eliminar"}
                  </button>
                </div>
              </div>
...
function Capital({ paneles, contratos, gastos, proveedores }) {
  ...
  const mC = m => MESES_CORTO[parseInt(m.split("-")[1],10) - 1];

  // ── Iconos SVG ──
  const IcoBalance = (
    <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
      <line x1="12" y1="3" x2="12" y2="21"/><path d="M3 9l9-6 9 6"/><path d="M5 12l-2 5h18l-2-5"/><path d="M7 12h10"/>
    </svg>
  );
  const IcoPaneles = (
    <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
  ...
  return (
    <div>
      ...
      <div style={{
        ...
        <div style={{ position:"relative", marginBottom: 20 }}>
          <div style={{ fontSize:12, color:"rgba(200,212,240,0.55)", fontWeight:500, marginBottom:4 }}>Vista360 · 8 Millas</div>
          <div style={{ fontSize:28, fontWeight:800, color:"#fff", letterSpacing:"-0.03em", lineHeight:1.05 }}>Capital</div>
          <div style={{ fontSize:11.5, color:"rgba(200,212,240,0.5)", marginTop:5 }}>Patrimonio, depreciación y flujo de caja</div>
        </div>
        {/* Card patrimonio principal */}
        <div style={{
          position:"relative",
          background:"linear-gradient(180deg, rgba(20,32,68,0.92) 0%, rgba(14,24,56,0.92) 100%)",
          border:"1px solid rgba(255,255,255,0.07)",
          ...
          {/* Izquierda: valor patrimonio */}
          <div style={{ flex:"1 1 55%", minWidth:0, display:"flex", flexDirection:"column" }}>
            <div style={{ fontSize:12, color:"rgba(200,212,240,0.6)", fontWeight:500, marginBottom:6 }}>Patrimonio neto</div>
            <div style={{ fontSize:28, fontWeight:900, color: patrimonioNeto >= 0 ? "#5BD39A" : "#FF7A8A", letterSpacing:"-0.03em", lineHeight:1 }}>{fmt(patrimonioNeto)}</div>
            <div style={{ display:"flex", alignItems:"center", gap:6, marginTop:8 }}>
              ...
              }}>Activos {fmt(totalActivos)}</span>
            </div>
            <div style={{ flex:1 }}/>
            <div style={{ fontSize:11, color:"rgba(200,212,240,0.4)", marginTop:12 }}>Pasivos {fmt(porPagar)}</div>
          </div>
          {/* Derecha: mini barras flujo */}
          <div style={{ flex:"1 1 45%", minWidth:0, display:"flex", flexDirection:"column" }}>
            ...
            <div style={{ flex:1, display:"flex", alignItems:"flex-end", justifyContent:"space-between", gap:4, height:80 }}>
              {flujo.map((f, i) => {
                const isHi = i >= flujo.length - 3;
                const h = (f.ing / maxBar) * 100;
                return (
                  <div key={f.mes} style={{ flex:1, display:"flex", flexDirection:"column", alignItems:"center", gap:4, minWidth:0 }}>
                    <div style={{ width:"70%", height:`${Math.max(h,6)}%`, minHeight:4, borderRadius:4, background: isHi ? "linear-gradient(180deg,#4A8CFF,#1D6BFF)" : "rgba(255,255,255,0.08)", boxShadow: isHi ? "0 0 12px rgba(29,107,255,0.45)" : "none" }}/>
                    <div style={{ fontSize:8, fontWeight:600, color: isHi ? "rgba(189,210,255,0.9)" : "rgba(200,212,240,0.4)", textTransform:"uppercase", letterSpacing:0.3 }}>{mC(f.mes)}</div>
                  </div>
                );
              })}
            </div>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:10, marginBottom:20 }}>
        {[
          { icon: IcoPaneles, label:"Valor paneles",  value: fmt(valorPaneles),  sub: `Original ${fmt(costoOriginal)}`,                                        vc: NL.green, wc: NL.green  },
          { icon: IcoCobrar,  label:"Por cobrar",     value: fmt(porCobrar),     sub: `${contratos.filter(c=>!c.pagado).length} contratos pendientes`,          vc: NL.green, wc: NL.green  },
          { icon: IcoPagar,   label:"Por pagar",      value: fmt(porPagar),      sub: `${proveedores.filter(p=>!p.pagado).length} proveedores`,                 vc: NL.red,   wc: NL.red    },
          { icon: IcoDep,     label:"Dep. acumulada", value: fmt(depAcumTotal),  sub: `${pctPortfolio.toFixed(0)}% del portfolio depreciado`,                  vc: NL.red,   wc: NL.red    },
        ].map(({ icon, label, value, sub, vc, wc }) => (
          <div key={label} style={{ background: NL.white, borderRadius:18, padding:"14px 14px 10px", border:`1px solid ${NL.border}`, boxShadow:"0 1px 0 rgba(16,22,40,0.02), 0 6px 24px -16px rgba(16,22,40,0.18)", display:"flex", flexDirection:"column", minWidth:0 }}>
            <div style={{ width:38, height:38, borderRadius:11, background:"linear-gradient(180deg,#0E1A3B 0%,#15265A 100%)", display:"flex", alignItems:"center", justifyContent:"center", marginBottom:10, color:"#fff", flexShrink:0 }}>{icon}</div>
            <div style={{ fontSize:11, color:NL.text, fontWeight:600, marginBottom:3 }}>{label}</div>
            <div style={{ fontSize:22, fontWeight:800, color:vc, lineHeight:1, letterSpacing:"-0.03em" }}>{value}</div>
            <div style={{ fontSize:11, color:NL.muted, marginTop:5, lineHeight:1.35, minHeight:26 }}>{sub}</div>
            <div style={{ marginTop:4, marginLeft:-2, marginRight:-2 }}><Wave color={wc}/></div>
          </div>
        ))}
      ...
      <div style={{ background:"#0D1020", borderRadius:20, padding:"18px 18px 20px", marginBottom:20 }}>
        <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom: usandoGlobal ? 16 : 0 }}>
          <div style={{ display:"flex", alignItems:"center", gap:10 }}>
            <div style={{ width:40, height:40, borderRadius:12, background:"rgba(37,99,235,0.2)", display:"flex", alignItems:"center", justifyContent:"center", color:"#60A5FA", flexShrink:0 }}>{IcoSlider}</div>
            <div>
              <div style={{ fontSize:13, fontWeight:700, color:"#fff" }}>Tasa de depreciación</div>
              <div style={{ fontSize:11, color:"rgba(255,255,255,0.4)", marginTop:2 }}>
                {usandoGlobal ? `Global: ${tasaGlobal}% anual · ~${Math.round(100/tasaGlobal)} años de vida` : "Cada panel usa su propia tasa"}
              </div>
            </div>
          </div>
          <button
            onClick={() => setTasaGlobal(usandoGlobal ? null : 10)}
            style={{ background: usandoGlobal ? "#2563EB" : "rgba(255,255,255,0.08)", border:"1px solid rgba(255,255,255,0.12)", borderRadius:50, padding:"8px 16px", color:"#fff", fontSize:12, fontWeight:700, cursor:"pointer", touchAction:"manipulation", whiteSpace:"nowrap", flexShrink:0 }}
          >
        ...
        {usandoGlobal && (
          <div>
            ...
            <div style={{ position:"relative", marginBottom:8 }}>
              <input
                type="range" min="1" max="50" value={tasaGlobal}
                onChange={e => setTasaGlobal(Number(e.target.value))}
                style={{ width:"100%", accentColor:"#2563EB", height:6, cursor:"pointer" }}
              />
            </div>
            <div style={{ display:"flex", justifyContent:"space-between", fontSize:10, color:"rgba(255,255,255,0.35)", marginBottom:14 }}>
              <span>1%</span>
              <span style={{ fontSize:14, fontWeight:800, color:"#4A8CFF" }}>{tasaGlobal}% anual</span>
              <span>50%</span>
            </div>
            {/* Atajos */}
            <div style={{ display:"flex", gap:8, flexWrap:"wrap" }}>
              {[{l:"SUNAT 10%",v:10},{l:"5%",v:5},{l:"20%",v:20},{l:"25%",v:25}].map(({l,v}) => (
                <button key={v} onClick={() => setTasaGlobal(v)} style={{
                  padding:"6px 14px", borderRadius:99, fontSize:11, fontWeight:700, cursor:"pointer", touchAction:"manipulation",
                  border:`1px solid ${tasaGlobal===v ? "#4A8CFF" : "rgba(255,255,255,0.14)"}`,
                  background: tasaGlobal===v ? "rgba(29,107,255,0.25)" : "rgba(255,255,255,0.05)",
                  color: tasaGlobal===v ? "#7FAEFF" : "rgba(255,255,255,0.5)",
                }}>{l}</button>
              ))}
            </div>
          </div>
      ...
      {/* ── FLUJO DE CAJA ── */}
      <div style={{ background: NL.white, borderRadius:18, padding:"18px 16px", marginBottom:20, border:`1px solid ${NL.border}`, boxShadow:"0 1px 0 rgba(16,22,40,0.02),0 6px 24px -16px rgba(16,22,40,0.12)" }}>
        <div style={{ display:"flex", alignItems:"center", gap:8, marginBottom:18 }}>
          <div style={{ width:36, height:36, borderRadius:10, background:"linear-gradient(180deg,#0E1A3B,#15265A)", display:"flex", alignItems:"center", justifyContent:"center", color:"#fff", flexShrink:0 }}>
            <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
              <rect x="3" y="3" width="18" height="18" rx="2"/><path d="M3 9h18M9 21V9"/>
            </svg>
          </div>
          <div>
            <div style={{ fontSize:13, fontWeight:700, color:NL.text }}>Flujo de caja</div>
            <div style={{ fontSize:11, color:NL.muted }}>Ingresos cobrados vs gastos · 6 meses</div>
          </div>
        </div>
        {/* Barras */}
        <div style={{ display:"flex", alignItems:"flex-end", gap:6, height:120, marginBottom:6 }}>
          {flujo.map(({ mes, ing, eg }) => (
            <div key={mes} style={{ flex:1, display:"flex", flexDirection:"column", alignItems:"center", gap:2, height:"100%", minWidth:0 }}>
              <div style={{ flex:1, width:"100%", display:"flex", flexDirection:"column", justifyContent:"flex-end", gap:2 }}>
                {ing > 0 && <div style={{ width:"100%", height:`${(ing/maxBar)*100}%`, minHeight:4, background:"linear-gradient(180deg,#34D399,#10B981)", borderRadius:"5px 5px 0 0", transition:"height .4s" }}/>}
                {eg > 0 && <div style={{ width:"100%", height:`${(eg/maxBar)*100}%`, minHeight:4, background:"linear-gradient(180deg,#F87171,#EF4444CC)", borderRadius:"5px 5px 0 0", transition:"height .4s" }}/>}
              </div>
            </div>
          ))}
        </div>
        <div style={{ display:"flex", gap:6, marginBottom:12 }}>
          {flujo.map(({ mes }) => (
            <div key={mes} style={{ flex:1, textAlign:"center", fontSize:10, color:NL.muted, fontWeight:600 }}>{mC(mes)}</div>
          ))}
        </div>
        <div style={{ display:"flex", gap:14, justifyContent:"center" }}>
          <div style={{ display:"flex", alignItems:"center", gap:6, fontSize:11, color:NL.muted }}>
            <div style={{ width:10, height:10, borderRadius:2, background:"#10B981" }}/> Ingresos
          </div>
          <div style={{ display:"flex", alignItems:"center", gap:6, fontSize:11, color:NL.muted }}>
      ...
      {/* ── DETALLE MENSUAL ── */}
      <div style={{ background:"#0D1020", borderRadius:20, padding:"18px 18px 6px", marginBottom:20 }}>
        <div style={{ fontSize:11, fontWeight:700, color:"#60A5FA", letterSpacing:1.5, textTransform:"uppercase", marginBottom:14 }}>Detalle mensual</div>
        {flujo.slice().reverse().map(({ mes, ing, eg, neto }) => (
          <div key={mes} style={{ display:"flex", alignItems:"center", gap:12, paddingBottom:14, marginBottom:14, borderBottom:"1px solid rgba(255,255,255,0.05)" }}>
            <div style={{ width:44, height:44, borderRadius:13, background: neto >= 0 ? "rgba(16,185,129,0.15)" : "rgba(239,68,68,0.15)", display:"flex", alignItems:"center", justifyContent:"center", flexShrink:0, color: neto >= 0 ? "#5BD39A" : "#FF7A8A" }}>
              {neto >= 0 ? IcoUp : IcoDown}
            </div>
            <div style={{ flex:1, minWidth:0 }}>
              <div style={{ fontSize:13, fontWeight:700, color:"#fff" }}>{mesLabel(mes)}</div>
              <div style={{ fontSize:11, color:"rgba(255,255,255,0.38)", marginTop:2 }}>Ing. {fmt(ing)} · Gas. {fmt(eg)}</div>
            </div>
            <div style={{ textAlign:"right", flexShrink:0 }}>
              <div style={{ fontSize:15, fontWeight:800, color: neto >= 0 ? "#5BD39A" : "#FF7A8A", letterSpacing:"-0.02em" }}>{neto >= 0 ? "+" : ""}{fmt(neto)}</div>
              <div style={{ fontSize:10, color:"rgba(255,255,255,0.3)", marginTop:2 }}>neto</div>
            </div>
          </div>
        ))}
      </div>

      {/* ── DEPRECIACIÓN POR PANEL ── */}
      <div style={{ background: NL.white, borderRadius:18, padding:"18px 16px", marginBottom:20, border:`1px solid ${NL.border}`, boxShadow:"0 1px 0 rgba(16,22,40,0.02),0 6px 24px -16px rgba(16,22,40,0.12)" }}>
        <div style={{ display:"flex", alignItems:"center", gap:8, marginBottom:16 }}>
          <div style={{ width:36, height:36, borderRadius:10, background:"linear-gradient(180deg,#0E1A3B,#15265A)", display:"flex", alignItems:"center", justifyContent:"center", color:"#fff", flexShrink:0 }}>{IcoDep}</div>
          <div>
            <div style={{ fontSize:13, fontWeight:700, color:NL.text }}>Depreciación por panel</div>
            <div style={{ fontSize:11, color:NL.muted }}>Valor actual · desgaste acumulado</div>
          </div>
        </div>
        {panelDep.filter(p => Number(p.costoInstalacion) > 0).length === 0 ? (
          <div style={{ textAlign:"center", padding:"28px 0" }}>
            <div style={{ width:48, height:48, borderRadius:14, background:NL.bg, display:"flex", alignItems:"center", justifyContent:"center", margin:"0 auto 12px", color:NL.muted }}>
              <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.7" strokeLinecap="round" strokeLinejoin="round"><path d="M14 2H6a2 2 0 00-2 2v16a2 2 0 002 2h12a2 2 0 002-2V8z"/><polyline points="14 2 14 8 20 8"/><line x1="9" y1="13" x2="15" y2="13"/><line x1="9" y1="17" x2="13" y2="17"/></svg>
            </div>
            <div style={{ fontSize:13, fontWeight:600, color:NL.text, marginBottom:4 }}>Sin datos de capital</div>
            <div style={{ fontSize:12, color:NL.muted, lineHeight:1.5 }}>Edita cada panel y agrega el<br/><strong>costo</strong> y <strong>fecha de instalación</strong></div>
          </div>
        ) : (
          panelDep.filter(p => Number(p.costoInstalacion) > 0).sort((a,b) => b.pctDesgaste - a.pctDesgaste).map((p, idx, arr) => {
            const barColor = p.pctDesgaste > 75 ? "#EF4444" : p.pctDesgaste > 50 ? "#FFFFFF" : "#10B981";
            return (
              <div key={p.id} style={{ paddingBottom:14, marginBottom: idx < arr.length-1 ? 14 : 0, borderBottom: idx < arr.length-1 ? `1px solid ${NL.border}` : "none" }}>
                <div style={{ display:"flex", alignItems:"center", gap:10, marginBottom:10 }}>
                  <div style={{ width:42, height:42, borderRadius:12, background:"linear-gradient(135deg,#5B8DEF,#243F8C)", display:"flex", alignItems:"center", justifyContent:"center", border:"1.5px solid rgba(255,255,255,0.12)", flexShrink:0 }}>
                    <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="3" width="7" height="7" rx="1.5"/><rect x="14" y="3" width="7" height="7" rx="1.5"/><rect x="3" y="14" width="7" height="7" rx="1.5"/><rect x="14" y="14" width="7" height="7" rx="1.5"/></svg>
                  </div>
                  <div style={{ flex:1, minWidth:0 }}>
                    <div style={{ fontSize:14, fontWeight:700, color:NL.text, overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap" }}>{p.nombre}</div>
                    <div style={{ fontSize:11, color:NL.muted, marginTop:2 }}>{p.ciudad} · {(p.aniosUsados||0).toFixed(1)} años en uso</div>
                  </div>
                  <div style={{ textAlign:"right", flexShrink:0 }}>
                    <div style={{ fontSize:15, fontWeight:800, color:NL.green, letterSpacing:"-0.02em" }}>{fmt(p.valorActual)}</div>
                    <div style={{ fontSize:10, color:NL.muted, marginTop:2 }}>valor actual</div>
                  </div>
                </div>
                {/* Barra de desgaste */}
                <div style={{ background:NL.bg, borderRadius:99, height:7, overflow:"hidden", marginBottom:7 }}>
                  <div style={{ height:"100%", borderRadius:99, width:`${p.pctDesgaste}%`, background: barColor, transition:"width .5s ease" }}/>
                </div>
                <div style={{ display:"flex", justifyContent:"space-between", fontSize:11, color:NL.muted }}>
                  <span>Original {fmt(Number(p.costoInstalacion))}</span>
                  <span style={{ fontWeight:700, color: p.pctDesgaste > 75 ? NL.red : p.pctDesgaste > 50 ? NL.text : NL.green }}>{p.pctDesgaste.toFixed(0)}% depreciado</span>
                </div>
              </div>
            );
          })
