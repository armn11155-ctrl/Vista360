...
function MiniMapaPanel({lat,lng,nombre,foto,onMove}){
  ...
  const reverseGeocode=async(la,ln)=>{
    ...
    try{
      const r=await fetch(`https://nominatim.openstreetmap.org/reverse?lat=${la}&lon=${ln}&format=json&accept-language=es`,{"User-Agent":"Vista360/1.0"});
      const d=await r.json();
      const road=(d&&d.address&&(d.address.road||d.address.pedestrian||d.address.suburb))||"";
      const city=(d&&d.address&&(d.address.city||d.address.town||d.address.county||d.address.state))||"";
    ...
    <div style={{display:"flex",alignItems:"center",gap:10}}>
      {notifPerm==="granted"
        ...
        : <div style={{display:"flex",flexDirection:"column",gap:6,alignItems:"flex-end"}}>
            ...
            <span style={{fontSize:10,color:C.muted}}>iPhone: primero agrega la app a Inicio (Safari → Compartir → Agregar a inicio) · Alertas a 30 y 15 días</span>
          </div>
      }
    </div>
  );
