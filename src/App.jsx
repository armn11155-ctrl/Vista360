...
function Dashboard({clientes,contratos,paneles,setTab}){
  ...
  const mesesData = useMemo(() => {
    ...
    const keys = Object.keys(map).sort().slice(-8); // últimos 8 meses
    return keys.map(k => {
      const [y,m] = k.split("-");
  ...
  return (
    <div style={{ paddingBottom: 32 }}>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr 1fr", gap:10, marginBottom:20 }}>
        {[
          { label:"Ingresos/mes", val:`S/ ${fmt(totalMes)}`, color:C.accent, icon:"💰" },
          { label:"Ocupación", val:`${ocup_rate}%`, color:C.green, icon:"📡" },
          { label:"Clientes", val:clientes.length, color:C.purple, icon:"👥" },
        ].map((k,i) => (
          <div key={i} style={{ background:`linear-gradient(135deg,${k.color}14,${k.color}06)`, border:`1px solid ${k.color}30`, borderRadius:16, padding:"14px 12px", textAlign:"center" }}>
            <div style={{ fontSize:18, marginBottom:4 }}>{k.icon}</div>
            <div style={{ fontSize:18, fontWeight:900, color:k.color, fontFamily:"monospace", letterSpacing:"-0.5px" }}>{k.val}</div>
            <div style={{ fontSize:10, color:C.muted, fontWeight:700, textTransform:"uppercase", letterSpacing:0.8, marginTop:3 }}>{k.label}</div>
          </div>
        ))}
      </div>
      ...
      <div style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:20, padding:"20px 18px", marginBottom:20 }}>
        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:18 }}>
          ...
          {crecimiento !== null && (
            <div style={{ display:"flex", alignItems:"center", gap:5, background: Number(crecimiento)>=0 ? C.green+"18" : C.red+"18", border:`1px solid ${Number(crecimiento)>=0 ? C.green : C.red}33`, borderRadius:999, padding:"5px 10px" }}>
              <span style={{ fontSize:12 }}>{Number(crecimiento)>=0?"▲":"▼"}</span>
              <span style={{ fontSize:12, fontWeight:800, color: Number(crecimiento)>=0 ? C.green : C.red }}>{Math.abs(crecimiento)}%</span>
            </div>
          )}
        </div>
  ...
  return (
    <div
      style={{
        ...
        background: "#00000096", zIndex: 300,
        display: "flex", alignItems: "center", justifyContent: "center",
    ...
    >
      <div style={{
        ...
        borderRadius: 22, padding: 24, width: "100%", maxWidth: 540,
    ...
    <div style={{ paddingBottom: 32 }}>
      <PgTit
        ...
        sub={loading
          ...
          : `${facturas.length} comprobante${facturas.length !== 1 ? "s" : ""} ${conectado ? "del sistema externo" : "(modo demo)"}`
        }
      />
      ...
      {!conectado ? (
        <div style={{
          ...
          <div style={{ width: 36, height: 36, borderRadius: 10, background: "#2563EB22", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
            <span style={{ fontSize: 18 }}>🔌</span>
          </div>
          <div style={{ flex: 1 }}>
            <div style={{ fontSize: 13, fontWeight: 800, color: "#5A9BFF", marginBottom: 3 }}>
              Sistema externo no conectado
            </div>
            <div style={{ fontSize: 11.5, color: "rgba(160,180,220,0.72)", lineHeight: 1.55 }}>
              Cuando tu sistema de facturación esté listo, configura{" "}
              <code style={{ background: "rgba(79,124,255,0.12)", padding: "1px 6px", borderRadius: 4, fontSize: 10.5, color: "#9DBEFF" }}>FACTURACION_API</code>{" "}
              en el código y los comprobantes se cargarán automáticamente. Por ahora se muestran datos de prueba desde Firebase.
            </div>
          </div>
        </div>
      ) : (
        <div style={{
          background: "linear-gradient(135deg,#0A2D1E,#08221A)",
          border: `1px solid ${C.green}44`,
          borderRadius: 16, padding: "12px 16px", marginBottom: 18,
          display: "flex", alignItems: "center", gap: 12,
        }}>
          <div style={{ width: 32, height: 32, borderRadius: 10, background: C.green + "22", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
            <span style={{ fontSize: 16 }}>✅</span>
          </div>
          <div style={{ fontSize: 12.5, fontWeight: 700, color: C.green, flex: 1 }}>
            Conectado al sistema de facturación
          </div>
          <button onClick={cargar} disabled={loading} style={{
            background: "rgba(15,186,125,0.12)", border: `1px solid ${C.green}44`,
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(2, 1fr)", gap: 12, marginBottom: 20 }}>
        {[
          ...
          { label: "Cobrado",          val: fmt(kpis.cobradas),   color: C.green,  icon: "✅", sub: `${facturas.filter(f => f.estado === "Cobrada" || f.estado === "Pagada").length} cobradas` },
          { label: "Pendiente cobro",  val: fmt(kpis.pendientes), color: C.amber,  icon: "⏳", sub: `${facturas.filter(f => ["Emitida","Aceptada","Pendiente"].includes(f.estado)).length} pendientes` },
      ...
      <div style={{ display: "flex", gap: 10, marginBottom: 14, flexWrap: "wrap" }}>
        <div style={{ flex: 1, minWidth: 180, position: "relative" }}>
          <input
            ...
            placeholder="🔍 Buscar por cliente, panel, serie, RUC..."
            style={{ width: "100%", background: C.surface, border: `1px solid ${C.border}`, borderRadius: 10, padding: "10px 14px", color: C.text, fontSize: 13, outline: "none", fontFamily: "inherit", boxSizing: "border-box" }}
          />
        </div>
        <div style={{ display: "flex", gap: 6 }}>
          {[{ id: "lista", icon: "☰" }, { id: "resumen", icon: "📊" }].map(v => (
            <button key={v.id} onClick={() => setVista(v.id)} style={{ padding: "9px 14px", borderRadius: 10, border: `1px solid ${vista === v.id ? C.accent + "66" : C.border}`, background: vista === v.id ? C.accent + "22" : C.surface, color: vista === v.id ? C.accent : C.muted, fontSize: 14, cursor: "pointer" }}>{v.icon}</button>
          ))}
        </div>
      </div>
        ...
        <>
                  ...
                  <div key={f.id} onClick={() => setModalDetalle(f)}
                    ...
                    <div style={{ flexShrink: 0, width: 40, height: 40, borderRadius: 12, background: `linear-gradient(135deg,${C.accent}33,${C.accent}11)`, border: `1px solid ${C.accent}44`, display: "flex", alignItems: "center", justifyContent: "center" }}>
                      <span style={{ fontSize: 18 }}>
                        {f.tipo === "FACTURA" ? "🧾" : f.tipo === "BOLETA" ? "📄" : f.tipo === "NOTA_CREDITO" ? "↩️" : f.tipo === "NOTA_DEBITO" ? "➕" : "📋"}
                      </span>
                    </div>
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 4, flexWrap: "wrap" }}>
                        <span style={{ fontFamily: "monospace", fontSize: 13, fontWeight: 800, color: C.white }}>
                      ...
                      <div style={{ fontSize: 12, color: C.text, fontWeight: 600, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{cli}</div>
                    ...
                    <div style={{ flexShrink: 0, textAlign: "right" }}>
                      <div style={{ fontSize: 17, fontWeight: 900, color: isCobrada ? C.green : isVencida ? C.red : C.text, fontFamily: "monospace" }}>
                        {fmt(total)}
                      </div>
                      {f.pdf_url && (
                        <div style={{ fontSize: 10, color: C.accent, marginTop: 4, fontWeight: 700 }}>📄 PDF</div>
                      )}
                    </div>
                  </div>
                );
          ...
          {!loading && facsFiltradas.length > 0 && (
            <div style={{ marginTop: 16, background: C.surface, border: `1px solid ${C.border}`, borderRadius: 14, padding: "14px 18px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              <span style={{ fontSize: 12, color: C.muted }}>
                {facsFiltradas.length} comprobante{facsFiltradas.length > 1 ? "s" : ""} mostrados
              </span>
              <span style={{ fontSize: 16, fontWeight: 900, color: C.accent, fontFamily: "monospace" }}>
                Total: {fmt(facsFiltradas.reduce((a, f) => a + totalDe(f), 0))}
              </span>
            </div>
          )}
        </>
...
function HeroCard({ panalesLibres }) {
  ...
  return (
    <div style={{
      ...
      <HeroWaveArt/>
      <div style={{ position: "relative", zIndex: 2, padding: "22px 22px 22px", maxWidth: "68%" }}>
        <div style={{
        ...
        <button style={{
          ...
          <span style={{
            ...
            <svg width="15" height="15" viewBox="0 0 24 24" fill="#fff">
              <path d="M13 2 L4 13 h6 l-1 9 9-12 h-6 z"/>
            </svg>
          </span>
          <span style={{ fontSize: 13.5, fontWeight: 700, color: "#0D1117", letterSpacing: "-0.005em" }}>
            Acción recomendada
          </span>
          <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#0D1117" strokeWidth="2.4" strokeLinecap="round" strokeLinejoin="round" style={{ marginLeft: 2 }}>
    ...
    <>
      ...
      <div style={{
        ...
        <div style={{
          ...
          >
            <defs>
              <linearGradient id="dwArcGrad" x1="0" y1="0" x2="1" y2="0">
            ...
            <path
              d="M0 70 Q160 -5 320 70"
              stroke="url(#dwArcGrad)"
              strokeWidth="1"
              fill="none"
            />
            <circle cx="165" cy="33" r="3.5" fill="#2563EB"/>
            <circle cx="165" cy="33" r="7" fill="#2563EB" opacity="0.18"/>
          </svg>
        </div>
      </div>
    </>
    ...
    <>
      ...
      <style>{`
        *{box-sizing:border-box;-webkit-tap-highlight-color:transparent}
        ...
        input,select,textarea{scroll-margin-bottom:180px}
      `}</style>
      ...
      <div style={{ width: "100%", height: "100dvh", background: NL.bg, display: "flex", flexDirection: "column", fontFamily: "'DM Sans',sans-serif", color: NL.text, overflow: "hidden", position: "relative" }}>
        ...
        <div style={{
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 8, flexShrink: 0 }}>
            <button onClick={() => setGlobalSearch(true)} aria-label="Buscar" style={{ width: 44, height: 44, borderRadius: "50%", background: NL.white, border: `1px solid ${NL.border}`, display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer" }}>
            ...
            <button onClick={() => setNotifOpen(v => !v)} aria-label="Notificaciones" style={{ width: 44, height: 44, borderRadius: "50%", background: NL.white, border: `1px solid ${NL.border}`, display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", position: "relative" }}>
              <svg width="18" height="18" fill="none" viewBox="0 0 24 24">
        ...
        <div
          ...
          onTouchStart={e => { swRef._swX = e.touches[0].clientX; swRef._swY = e.touches[0].clientY; }}
          onTouchEnd={e => {
            if (showProfile) return;
            const dx = e.changedTouches[0].clientX - (swRef._swX || 0);
            const dy = Math.abs(e.changedTouches[0].clientY - (swRef._swY || 0));
            if (Math.abs(dx) < 60 || dy > 60) return;
            const tabs = ["hoy","paneles","contratos","crm"];
            const cur = tabs.indexOf(tab);
            if (dx < 0 && cur < tabs.length - 1) handleTabClick(tabs[cur + 1]);
            if (dx > 0 && cur > 0) handleTabClick(tabs[cur - 1]);
          }}
        >
          ...
          {BOTTOM_TABS_LIST.map(t => {
            ...
            return (
              <button key={t.id} onClick={() => handleTabClick(t.id)} style={{
                ...
                color: active ? NL.accent : NL.muted, padding: "8px 0",
                minHeight: 44,
              }}>
                {BTM_ICONS[t.id]}
