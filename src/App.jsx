...
const SUPABASE_ANON_KEY_DEFAULT = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Iml2a2hjZndnZ3p3Y3p3ZHRnaWdhIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzczNDIwNTQsImV4cCI6MjA5MjkxODA1NH0.fO0wO-T2tVV6l0xfwlyF1_w9CiJZ2QvM15r4t9H0SZk";
// La key se puede actualizar en runtime desde el modal de configuración
let SUPABASE_ANON_KEY = (() => { try { return localStorage.getItem("v360_anon_key") || SUPABASE_ANON_KEY_DEFAULT; } catch(e){ return SUPABASE_ANON_KEY_DEFAULT; } })();

// ── CLIENTE SUPABASE (sin librería extra) ────────────────────────
const sb = {
  get headers(){ return { "Content-Type": "application/json", apikey: SUPABASE_ANON_KEY, Authorization: `Bearer ${SUPABASE_ANON_KEY}` }; },
  ...
  async post(table, body)      { const r = await fetch(sb.url(table), { method: "POST",   headers: { ...sb.headers, Prefer: "return=representation" }, body: JSON.stringify(body) }); if(!r.ok){ const e=await r.text(); throw new Error(e); } const d=await r.json().catch(()=>[]); return Array.isArray(d)?d:(d&&d.id?[d]:[{...body,_ok:true}]); },
  async patch(table, id, body) { const r = await fetch(sb.url(table, `?id=eq.${id}`), { method: "PATCH",  headers: { ...sb.headers, Prefer: "return=representation" }, body: JSON.stringify(body) }); if(!r.ok){ const e=await r.text(); throw new Error(e); } const d=await r.json().catch(()=>[]); return Array.isArray(d)?d:(d&&d.id?[d]:[{...body,id,_ok:true}]); },
...
function ModalConfig({onClose}){
  ...
  const save=()=>{
    const k=key.trim();
    if(!k) return alert("La API key no puede estar vacía");
    SUPABASE_ANON_KEY=k;
    try{ localStorage.setItem("v360_anon_key",k); }catch(e){}
    setSaved(true);
    setTimeout(()=>{ setSaved(false); onClose(); window.location.reload(); },800);
  };
  const base={background:"#0D1421",border:"1px solid #18253D",borderRadius:10,padding:"10px 13px",color:"#E2E8F0",fontSize:13,outline:"none",fontFamily:"inherit",width:"100%",boxSizing:"border-box"};
  return(
    <div style={{position:"fixed",inset:0,background:"#00000092",zIndex:400,display:"flex",alignItems:"center",justifyContent:"center",padding:20,backdropFilter:"blur(6px)"}}
      ...
      <div style={{background:"#111929",border:"1px solid #18253D",borderRadius:20,padding:26,width:"100%",maxWidth:480}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:18}}>
          <div style={{fontSize:16,fontWeight:800,color:"#fff"}}>⚙️ Configuración Supabase</div>
        ...
        <div style={{fontSize:12,color:"#64748B",marginBottom:14,lineHeight:1.6}}>
          Si ves <strong style={{color:"#F59E0B"}}>"Invalid API key"</strong>, pega aquí tu <strong style={{color:"#fff"}}>anon key</strong> actualizada desde el panel de Supabase → Settings → API.
        </div>
        <div style={{display:"flex",flexDirection:"column",gap:5,marginBottom:18}}>
          <label style={{fontSize:11,fontWeight:700,color:"#64748B",textTransform:"uppercase",letterSpacing:1}}>Supabase Anon Key</label>
          <textarea value={key} onChange={e=>setKey(e.target.value)} rows={4} style={{...base,resize:"vertical",fontFamily:"monospace",fontSize:11}} placeholder="eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..."/>
        </div>
        <div style={{display:"flex",gap:10}}>
          <button onClick={onClose} style={{flex:1,padding:12,background:"transparent",border:"1px solid #18253D",borderRadius:10,color:"#64748B",fontWeight:600,cursor:"pointer",fontSize:13}}>Cancelar</button>
          <button onClick={save} style={{flex:2,padding:12,background:saved?"#10B981":"#2563EB",border:"none",borderRadius:10,color:"#fff",fontWeight:700,fontSize:13,cursor:"pointer",transition:"background .3s"}}>
...
function Dashboard({clientes,contratos,paneles,setTab}){
  ...
  const IconCal=()=>(
    <svg width="36" height="36" viewBox="0 0 36 36" fill="none">
      <rect width="36" height="36" rx="10" fill="url(#gCal)"/>
      <defs><linearGradient id="gCal" x1="0" y1="0" x2="36" y2="36"><stop stopColor="#4C1D95"/><stop offset="1" stopColor="#8B5CF6"/></linearGradient></defs>
      <rect x="7" y="10" width="22" height="18" rx="3" stroke="#C4B5FD" strokeWidth="1.5" fill="none"/>
      <line x1="7" y1="15" x2="29" y2="15" stroke="#C4B5FD" strokeWidth="1.5"/>
      <line x1="13" y1="8" x2="13" y2="13" stroke="#C4B5FD" strokeWidth="2" strokeLinecap="round"/>
      <line x1="23" y1="8" x2="23" y2="13" stroke="#C4B5FD" strokeWidth="2" strokeLinecap="round"/>
      <rect x="10" y="18" width="3" height="3" rx="1" fill="#C4B5FD"/>
      <rect x="16" y="18" width="3" height="3" rx="1" fill="#C4B5FD"/>
      <rect x="22" y="18" width="3" height="3" rx="1" fill="#C4B5FD"/>
    </svg>
  );
  ...
  const IconMetrica=({type})=>{
    if(type==="ing") return(
      <svg width="42" height="42" viewBox="0 0 42 42" fill="none">
        <rect width="42" height="42" rx="12" fill="url(#gMIng)"/>
        <defs><linearGradient id="gMIng" x1="0" y1="0" x2="42" y2="42"><stop stopColor="#065F46"/><stop offset="1" stopColor="#10B981"/></linearGradient></defs>
        <circle cx="21" cy="21" r="11" stroke="#6EE7B7" strokeWidth="2" fill="none"/>
        <text x="21" y="26" textAnchor="middle" fill="#6EE7B7" fontSize="14" fontWeight="900" fontFamily="Arial">$</text>
      </svg>
    );
    if(type==="pan") return(
      <svg width="42" height="42" viewBox="0 0 42 42" fill="none">
        <rect width="42" height="42" rx="12" fill="url(#gMPan)"/>
        <defs><linearGradient id="gMPan" x1="0" y1="0" x2="42" y2="42"><stop stopColor="#1E3A6E"/><stop offset="1" stopColor="#3B82F6"/></linearGradient></defs>
        {/* Grid 3x3 */}
        {[0,1,2].map(row=>[0,1,2].map(col=>(
          <rect key={row+"-"+col} x={13+col*6} y={13+row*6} width="4" height="4" rx="1" fill="#93C5FD" opacity={0.7+row*0.1}/>
        )))}
      </svg>
    );
    if(type==="con") return(
      <svg width="42" height="42" viewBox="0 0 42 42" fill="none">
        <rect width="42" height="42" rx="12" fill="url(#gMCon)"/>
        <defs><linearGradient id="gMCon" x1="0" y1="0" x2="42" y2="42"><stop stopColor="#4C1D95"/><stop offset="1" stopColor="#8B5CF6"/></linearGradient></defs>
        {/* Personas */}
        <circle cx="16" cy="17" r="5" fill="#C4B5FD"/>
        <path d="M8 30 Q8 24 16 24 Q24 24 24 30" fill="#C4B5FD"/>
        <circle cx="27" cy="17" r="4" fill="#A78BFA" opacity="0.7"/>
        <path d="M21 30 Q22 26 27 26 Q34 26 34 30" fill="#A78BFA" opacity="0.7"/>
      </svg>
    );
    return null;
  };
  ...
  return(<div style={{paddingBottom:20}}>
    ...
    {/* ── HERO CARD ── */}
    <div style={{marginBottom:14,padding:"22px 20px",background:"linear-gradient(135deg,#0A1535 0%,#0F1E52 40%,#152668 70%,#0D1A45 100%)",borderRadius:20,border:"1px solid #1E3370",position:"relative",overflow:"hidden",boxShadow:"0 8px 40px rgba(10,30,120,0.6)"}}>
      {/* Partículas */}
      <div style={{position:"absolute",top:14,right:145,width:4,height:4,borderRadius:"50%",background:"rgba(255,255,255,0.5)"}}/>
      <div style={{position:"absolute",top:48,right:115,width:3,height:3,borderRadius:"50%",background:"rgba(255,255,255,0.3)"}}/>
      <div style={{position:"absolute",bottom:35,right:150,width:2,height:2,borderRadius:"50%",background:"rgba(255,255,255,0.4)"}}/>
      <div style={{display:"flex",alignItems:"flex-start",justifyContent:"space-between"}}>
        <div style={{flex:1}}>
          <div style={{fontSize:14,color:"rgba(255,255,255,0.75)",fontWeight:500,marginBottom:4}}>
            {saludoReal}, <span style={{color:"#60A5FA",fontWeight:700}}>Alan</span>! 👋
          </div>
          <div style={{fontSize:30,fontWeight:900,color:"#FFFFFF",letterSpacing:"-0.5px",lineHeight:1.1,marginBottom:6}}>
            Alan Martínez
          </div>
          <div style={{fontSize:13,color:"rgba(255,255,255,0.5)",marginBottom:18}}>
            Aquí tienes el resumen de hoy.
          </div>
          <div style={{display:"inline-flex",alignItems:"center",gap:8,background:"rgba(255,255,255,0.07)",border:"1px solid rgba(255,255,255,0.13)",borderRadius:30,padding:"7px 16px"}}>
            <svg width="14" height="14" viewBox="0 0 14 14" fill="none">
              <rect x="1" y="2" width="12" height="11" rx="2" stroke="rgba(255,255,255,0.6)" strokeWidth="1.2" fill="none"/>
              <line x1="1" y1="6" x2="13" y2="6" stroke="rgba(255,255,255,0.6)" strokeWidth="1.2"/>
              <line x1="4" y1="1" x2="4" y2="4" stroke="rgba(255,255,255,0.6)" strokeWidth="1.2" strokeLinecap="round"/>
              <line x1="10" y1="1" x2="10" y2="4" stroke="rgba(255,255,255,0.6)" strokeWidth="1.2" strokeLinecap="round"/>
            </svg>
        ...
        {/* Antena emoji con fondo circular glow */}
        <div style={{flexShrink:0,marginTop:-4,marginRight:-4,display:"flex",alignItems:"center",justifyContent:"center",position:"relative"}}>
          {/* Círculos radar */}
          <div style={{position:"absolute",width:130,height:130,borderRadius:"50%",border:"1px solid rgba(59,130,246,0.2)"}}/>
          <div style={{position:"absolute",width:100,height:100,borderRadius:"50%",border:"1px solid rgba(59,130,246,0.25)"}}/>
          <div style={{position:"absolute",width:72,height:72,borderRadius:"50%",border:"1px solid rgba(59,130,246,0.3)"}}/>
          {/* Glow azul de fondo */}
          <div style={{position:"absolute",width:90,height:90,borderRadius:"50%",background:"radial-gradient(circle,rgba(37,99,235,0.35) 0%,transparent 70%)"}}/>
          {/* Emoji antena */}
          <div style={{fontSize:72,lineHeight:1,filter:"drop-shadow(0 0 18px rgba(147,197,253,0.8)) drop-shadow(0 4px 12px rgba(0,0,50,0.8))",zIndex:1,marginTop:8}}>📡</div>
        </div>
      </div>
    </div>

    {/* ── INGRESOS ── */}
    <div style={{marginBottom:14,padding:"20px 20px",background:"linear-gradient(135deg,#062818 0%,#082010 50%,#041208 100%)",borderRadius:20,border:"1px solid #174D30",boxShadow:"0 4px 24px rgba(16,185,129,0.18)"}}>
      <div style={{display:"flex",alignItems:"center",gap:0}}>
        ...
        <div style={{display:"flex",gap:14,alignItems:"center",flex:1}}>
          ...
          <div>
            ...
            <div style={{fontSize:44,fontWeight:900,color:"#10B981",letterSpacing:"-2px",lineHeight:1,textShadow:"0 0 30px rgba(16,185,129,0.4)"}}>{fmt(ingMes)}</div>
            <div style={{fontSize:12,color:"#6EE7B7",marginTop:6,opacity:0.85}}>{pag} de {contratos.length} contratos cobrados</div>
          </div>
        </div>
        {/* Línea divisora */}
        <div style={{width:1,alignSelf:"stretch",background:"rgba(16,185,129,0.25)",margin:"0 16px"}}/>
        {/* Lado derecho — sin cuadro, solo contenido */}
        <div style={{textAlign:"center",flexShrink:0,minWidth:90}}>
          <svg width="26" height="26" viewBox="0 0 26 26" fill="none" style={{marginBottom:6}}>
            <path d="M2 18 L9 10 L14 15 L24 6" stroke={crecPct>=0?"#10B981":C.red} strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"/>
            <path d="M19 6 L24 6 L24 11" stroke={crecPct>=0?"#10B981":C.red} strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"/>
          </svg>
          <div style={{fontSize:10,color:"#6EE7B7",marginBottom:4,opacity:0.9}}>vs mes anterior</div>
          <div style={{fontSize:22,fontWeight:900,color:crecPct>=0?"#10B981":C.red,lineHeight:1}}>{crecPct===0?"0":crecPct>0?"+"+crecPct:crecPct}%</div>
          <div style={{fontSize:10,color:"#6EE7B7",marginTop:4,opacity:0.75}}>{crecPct===0?"Sin cambios":crecPct>0?"Creciendo":"Bajando"}</div>
        </div>
      </div>
    </div>
    ...
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginBottom:14}}>
      {/* Ocupación */}
      <div style={{padding:"16px 12px",background:"linear-gradient(145deg,#0C1C38,#081428)",borderRadius:20,border:"1px solid #1A2F50",display:"flex",flexDirection:"column",alignItems:"center",gap:4}}>
        ...
        <svg width="120" height="120" viewBox="0 0 120 120">
          <defs><linearGradient id="gocu2" x1="0" y1="0" x2="1" y2="1"><stop stopColor="#60A5FA"/><stop offset="1" stopColor="#2563EB"/></linearGradient></defs>
          <circle cx="60" cy="60" r="46" fill="none" stroke="#1A2744" strokeWidth="12"/>
          <circle cx="60" cy="60" r="46" fill="none" stroke="url(#gocu2)" strokeWidth="12"
          ...
          <text x="60" y="55" textAnchor="middle" fill="white" fontSize="26" fontWeight="900">{ocu}%</text>
          <text x="60" y="72" textAnchor="middle" fill="#64748B" fontSize="11">ocupación</text>
        </svg>
        <div style={{fontSize:12,color:"#64748B"}}>{paneles.filter(p=>p.estado==="Ocupado").length} de {paneles.length} paneles</div>
        {paneles.length===0&&<div style={{fontSize:10,color:"#64748B",background:"#1A2744",borderRadius:20,padding:"3px 12px",marginTop:2}}>Sin datos aún  ⓘ</div>}
      </div>
      {/* Cobro */}
      <div style={{padding:"16px 12px",background:"linear-gradient(145deg,#0C2018,#081510)",borderRadius:20,border:"1px solid #1A3020",display:"flex",flexDirection:"column",alignItems:"center",gap:4}}>
        <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:2,alignSelf:"flex-start"}}>
          <IconCard/>
          <span style={{fontSize:10,fontWeight:700,color:"#94A3B8",textTransform:"uppercase",letterSpacing:1.2}}>Cobro</span>
        </div>
        <svg width="120" height="120" viewBox="0 0 120 120">
          <defs><linearGradient id="gcob2" x1="0" y1="0" x2="1" y2="1"><stop stopColor="#6EE7B7"/><stop offset="1" stopColor="#10B981"/></linearGradient></defs>
          <circle cx="60" cy="60" r="46" fill="none" stroke="#1A2744" strokeWidth="12"/>
          <circle cx="60" cy="60" r="46" fill="none" stroke="url(#gcob2)" strokeWidth="12"
          ...
          <text x="60" y="55" textAnchor="middle" fill="white" fontSize="26" fontWeight="900">{cob}%</text>
          <text x="60" y="72" textAnchor="middle" fill="#64748B" fontSize="11">cobrado</text>
        </svg>
        <div style={{fontSize:12,color:"#64748B"}}>{pag} de {contratos.length} contratos</div>
        {contratos.length===0&&<div style={{fontSize:10,color:"#64748B",background:"#1A2744",borderRadius:20,padding:"3px 12px",marginTop:2}}>Sin datos aún  ⓘ</div>}
      </div>
    </div>

    {/* ── PRÓXIMOS A VENCER ── */}
    <div style={{marginBottom:14,padding:"20px 20px",background:"linear-gradient(145deg,#13103A,#0E0C2A)",borderRadius:20,border:"1px solid #2A2060",overflow:"hidden",boxShadow:"0 4px 24px rgba(109,40,217,0.12)"}}>
      {/* Header */}
      ...
      <button onClick={()=>setTab("contratos")} style={{width:"100%",padding:"14px",background:"linear-gradient(90deg,#2563EB,#60A5FA)",border:"none",borderRadius:14,color:"#fff",fontWeight:700,fontSize:15,cursor:"pointer",boxShadow:"0 4px 20px rgba(37,99,235,0.5)",letterSpacing:0.3}}>
        Ver contratos →
      </button>
    </div>

    {/* ── MÉTRICAS DEL MES ── */}
    <div style={{padding:"18px 20px",background:"linear-gradient(145deg,#0F1828,#0A1220)",borderRadius:20,border:"1px solid #1A2540"}}>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:8}}>
        {[
          {type:"ing",label:"Ingresos",value:fmt(ingMes),sub:"Este mes",color:"#10B981"},
          {type:"pan",label:"Paneles activos",value:paneles.filter(p=>p.estado==="Ocupado").length,sub:"Total instalados",color:"#3B82F6"},
          {type:"con",label:"Contratos activos",value:contratos.filter(c=>dias(c.fin)>0).length,sub:"Este mes",color:"#8B5CF6"},
        ].map(m=>(
          <div key={m.label} style={{background:"#080F1E",border:"1px solid #141F35",borderRadius:14,padding:"12px 8px",textAlign:"center"}}>
            <div style={{display:"flex",justifyContent:"center",marginBottom:8}}><IconMetrica type={m.type}/></div>
            <div style={{fontSize:10,color:"#64748B",fontWeight:600,textTransform:"uppercase",letterSpacing:0.6,marginBottom:4,lineHeight:1.3}}>{m.label}</div>
            <div style={{fontSize:20,fontWeight:900,color:m.color,letterSpacing:"-0.5px"}}>{m.value}</div>
    ...
    {/* ── GRÁFICA CRECIMIENTO ── */}
    <div style={{marginTop:14,padding:"20px 20px",background:"linear-gradient(145deg,#0F1828,#0A1220)",borderRadius:20,border:"1px solid #1A2540"}}>
  ...
  const guardar=async()=>{
    ...
    try{
      if(modal==="nuevo"){
        ...
        // Si Supabase devolvió el objeto completo, úsalo; si no, recarga la lista
        const saved=r&&r.length>0&&r[0]&&r[0].id ? r[0] : null;
        if(saved){ setPaneles(p=>[...p,saved]); }
        else { const fresh=await sb.get("paneles","?order=created_at.desc"); setPaneles(Array.isArray(fresh)?fresh:[]); }
...
// ── CONTRATOS ────────────────────────────────────────────────────
function Contratos({contratos,setContratos,paneles,clientes,loading}){
  ...
  const guardar=async()=>{
    ...
    try{
      let r;
      if(modal==="nuevo"){
        try{ [r]=await sb.post("contratos",payloadFull); }
        catch(e){ [r]=await sb.post("contratos",payloadSimple); }
        if(r) setContratos(p=>[...p,{...r,pagosMeses}]);
      } else {
        try{ [r]=await sb.patch("contratos",modal.id,payloadFull); }
        catch(e){ [r]=await sb.patch("contratos",modal.id,payloadSimple); }
  ...
  const togglePagoRapido=async(contrato,key)=>{
    ...
    const [r]=await sb.patch("contratos",contrato.id,{pagosMeses:pm,pagado});
...
function CRM({clientes,setClientes,contratos,loading}){
  ...
  const [saving,setSaving]=useState(false);
  const emptyC={tipo:"Prospecto",empresa:"",contacto:"",celular:"",email:"",ruc:"",ciudad:"Quito",sector:"Alimentación",estado:"En contacto",notas:""};
  const [form,setForm]=useState(emptyC);
  ...
  return(<div>
    ...
    <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:14,marginBottom:20}}>
      {[["🤝 Clientes",clis.length,C.green,"registrados"],["🎯 Prospectos",pros.length,C.purple,"en pipeline"],["🔥 Propuestas",pros.filter(p=>p.estado==="Propuesta enviada").length,C.amber,"por cerrar"]].map(([l,v,c,s])=>(
    ...
    <Card style={{padding:0,overflow:"hidden"}}>
      <div style={{overflowX:"auto"}}>
        <table style={{width:"100%",borderCollapse:"collapse"}}>
          <thead style={{background:C.surface}}>
            <tr><Th col="empresa" label="Empresa"/><Th col="tipo" label="Tipo"/><Th col="ciudad" label="Ciudad"/><Th col="sector" label="Sector"/><Th col="estado" label="Estado"/><th style={thSt("x")}>Contacto</th><th style={thSt("x")}>Notas</th><th style={thSt("x")}>Acciones</th></tr>
          </thead>
          <tbody>
            {filtrado.length===0
      ...
      <div style={{padding:"11px 18px",borderTop:`1px solid ${C.border}`,display:"flex",justifyContent:"space-between",fontSize:12,color:C.muted}}>
        ...
        <span>{filtrado.length} contacto{filtrado.length!==1?"s":""}</span>
      </div>
    </Card>}
                  ...
                  <tr key={m.key}
                    ...
                    <td style={{padding:"11px 13px",borderBottom:`1px solid ${C.border}`}}>
                      <button onClick={async()=>{
                        ...
                        for(const id of ids) await sb.del("gastos",id);
...
// ── GASTOS + OCR (Tesseract.js — gratis, sin API key) ────────────
function Gastos({gastos,setGastos}){
  ...
  const [modal,setModal]=useState(null);
  const [paso,setPaso]=useState(1); // 1=foto, 2=datos
  const [form,setForm]=useState({fecha:"",proveedor:"",ruc:"",concepto:"",monto:"",igv:"",subtotal:"",categoria:"Otro",notas:"",foto_texto:""});
  const [saving,setSaving]=useState(false);
  const [ocr,setOcr]=useState({loading:false,progress:0,text:"",imgUrl:""});
  ...
  // ── PRE-PROCESAMIENTO: escala de grises + alto contraste ────────
  // Mejora el reconocimiento de montos y fechas en boletas peruanas/ecuatorianas
  const preprocesarImagen=async(file)=>{
    return new Promise((resolve,reject)=>{
      ...
      img.onload=()=>{
        ...
        // Escalar al doble si la imagen es pequeña — mejora Tesseract
        const escala=Math.max(1,Math.min(3,2400/Math.max(img.width,img.height)));
        ...
        // Escala de grises + alto contraste via manipulación de píxeles
        const imageData=ctx.getImageData(0,0,canvas.width,canvas.height);
        const data=imageData.data;
        const umbral=140; // threshold para binarización suave
        for(let i=0;i<data.length;i+=4){
          // Luminosidad perceptual (fórmula ITU-R BT.601)
          const lum=0.299*data[i]+0.587*data[i+1]+0.114*data[i+2];
          // Alto contraste: por encima del umbral → blanco, abajo → negro (con suavizado)
          const val=lum>umbral?255:Math.max(0,lum*0.4);
          data[i]=val; data[i+1]=val; data[i+2]=val;
          // Alpha intacto
        }
        ctx.putImageData(imageData,0,0);

        canvas.toBlob(blob=>resolve({blob,url}),file.type||"image/jpeg",0.92);
        URL.revokeObjectURL(url);
      };
      img.onerror=reject;
      img.src=url;
  ...
  // ── CARGAR TESSERACT.JS DESDE CDN (sin instalación) ─────────────
  const cargarTesseract=()=>new Promise((resolve,reject)=>{
  ...
  const escanear=async(file)=>{
    ...
    setOcr({loading:true,progress:5,text:"",imgUrl});
    setPaso(2);
    try{
      // ── Paso 1: Pre-procesar imagen (escala de grises + alto contraste) ──
      setOcr(o=>({...o,progress:15}));
      let imagenOcr;
      try{
        const {blob}=await preprocesarImagen(file);
        imagenOcr=blob;
      }catch(_){
        imagenOcr=file; // fallback: imagen original
      }

      // ── Paso 2: Cargar Tesseract.js ──
      setOcr(o=>({...o,progress:25}));
      const Tesseract=await cargarTesseract();

      // ── Paso 3: OCR con Tesseract (español) ──
      setOcr(o=>({...o,progress:35}));
      const {data:{text:textoCompleto}}=await Tesseract.recognize(
        imagenOcr,
        "spa", // idioma español para boletas en español
        {
          logger:(m)=>{
            if(m.status==="recognizing text"){
              // progreso de 35 a 80
              setOcr(o=>({...o,progress:Math.round(35+m.progress*45)}));
            }
          },
          // Parámetros optimizados para boletas con montos y fechas
          tessedit_char_whitelist:"",
          preserve_interword_spaces:"1",
        }
      );

      if(!textoCompleto||textoCompleto.trim().length<5)
        throw new Error("No se pudo leer texto en la imagen. Intenta con mejor iluminación.");

      setOcr(o=>({...o,progress:82,text:textoCompleto}));

      // ── Paso 4: Extraer datos estructurados (regex — sin API, gratis) ──
      setOcr(o=>({...o,progress:95}));
      extraerDatos(textoCompleto);

      setOcr(o=>({...o,loading:false,progress:100,text:textoCompleto}));

    }catch(e){
      setOcr(o=>({...o,loading:false,progress:0}));
      alert("Error al leer la boleta:\n"+e.message);
    }
  };
  ...
  const extraerDatos=(text)=>{
    ...
    // RUC — 11 dígitos
    const rucM=text.match(/(?:RUC|R\.U\.C)[\s\:\.]*(\d{11})/i)||text.match(/\b(10\d{9}|20\d{9})\b/);
    const ruc=rucM?rucM[1]:"";

    // Monto total — busca TOTAL, IMPORTE, S/, $
    const totalM=text.match(/(?:TOTAL[\s\w]*|IMPORTE[\s\w]*|GRAN\s*TOTAL)[\s\:\=]*S?\/?\s*\.?\s*([\d,]+\.?\d*)/i);
    const montosArr=[...text.matchAll(/S\/\.?\s*([\d,]+\.?\d*)/gi)].map(m=>parseFloat(m[1].replace(",",".")));
    let monto="";
    if(totalM) monto=parseFloat(totalM[1].replace(",","."));
    else if(montosArr.length>0) monto=Math.max(...montosArr.filter(n=>!isNaN(n)&&n>0));

    // IGV
    const igvM=text.match(/(?:IGV|I\.G\.V|TAX)[\s\:\=]*S?\/?\s*\.?\s*([\d,]+\.?\d*)/i);
    const igv=igvM?parseFloat(igvM[1].replace(",",".")):monto?Math.round(Number(monto)/1.18*0.18*100)/100:"";

    // Subtotal / BASE
    const subM=text.match(/(?:SUBTOTAL|BASE\s*IMPONIBLE|BI)[\s\:\=]*S?\/?\s*\.?\s*([\d,]+\.?\d*)/i);
    const subtotal=subM?parseFloat(subM[1].replace(",",".")):monto&&igv?Math.round((Number(monto)-Number(igv))*100)/100:"";

    // Proveedor — primera línea con texto significativo, descartando palabras genéricas
    const skipWords=/^(factura|boleta|ticket|nota|comprobante|ruc|fecha|total|subtotal|igv|tax|item|cantidad|precio|descripcion|detalle|cod|nro|n°)/i;
    const proveedor=lines.find(l=>l.length>3&&/[a-zA-Z]{3,}/.test(l)&&!skipWords.test(l))||"";
    ...
    if(fechaM){
      const [,d,mo,y]=fechaM;
      const yf=y.length===2?"20"+y:y;
      fecha=`${yf}-${mo.padStart(2,"0")}-${d.padStart(2,"0")}`;
    }

    // Concepto — busca líneas con descripciones de producto/servicio
    const concLines=lines.filter(l=>l.length>5&&/[a-zA-Z]{4,}/.test(l)&&!skipWords.test(l)&&l!==proveedor);
    const concepto=concLines.slice(1,3).join(" ").substring(0,120);

    // Categoría automática por palabras clave
    const txt=text.toLowerCase();
    let categoria="Otro";
    if(/restaurante|comida|almuerzo|cena|desayuno|pizza|pollo|burger|cafe|cafetería|marisco|aliment/i.test(txt)) categoria="Servicios";
    else if(/hotel|hospedaje|alojamiento|hostal|apart|airbnb/i.test(txt)) categoria="Administrativo";
    else if(/taxi|uber|bus|combustible|gasolina|petróleo|peaje|estacion/i.test(txt)) categoria="Transporte";
    else if(/ferreteria|mantenimiento|reparacion|tecnico|herramienta|electricidad/i.test(txt)) categoria="Mantenimiento";
    else if(/sueldo|salario|planilla|personal|empleado/i.test(txt)) categoria="Personal";
    else if(/publicidad|marketing|imprenta|diseño|redes/i.test(txt)) categoria="Marketing";

    setForm(f=>({
      ...f,
      proveedor:proveedor||f.proveedor,
      ruc:ruc||f.ruc,
      monto:monto||f.monto,
      igv:igv||f.igv,
      subtotal:subtotal||f.subtotal,
      fecha:fecha||f.fecha,
      concepto:concepto||f.concepto,
      categoria:categoria!=="Otro"?categoria:f.categoria,
      foto_texto:text,
    }));
  };
  ...
  const guardar=async()=>{
    ...
    setSaving(true);
    // Descripcion siempre con fallback — nunca null
    const descripcion=([form.concepto,form.proveedor].filter(Boolean).join(" — ")||"Sin descripción").trim().substring(0,255)||"Sin descripción";
    // Payload completo con TODAS las columnas de Supabase
    const payload={
      descripcion,
    ...
    try{
      if(modal==="nuevo"||modal==="editar-nuevo"){
        const r=await sb.post("gastos",payload);
        const saved=r&&r[0]&&r[0].id?r[0]:{...payload,id:Date.now()};
        setGastos(g=>[saved,...g]);
      } else {
        const r=await sb.patch("gastos",modal.id,payload);
        const saved=r&&r[0]&&r[0].id?r[0]:{...modal,...payload};
        setGastos(g=>g.map(x=>x.id===modal.id?saved:x));
      }
      setSaving(false); setModal(null); setPaso(1);
    }catch(e){
      setSaving(false);
  ...
  const eliminar=async(id)=>{
    ...
    await sb.del("gastos",id);
    setGastos(g=>g.filter(x=>x.id!==id));
  };

  const abrirNuevo=()=>{
    const h=new Date();
    setForm({fecha:`${h.getFullYear()}-${String(h.getMonth()+1).padStart(2,"0")}-${String(h.getDate()).padStart(2,"0")}`,proveedor:"",ruc:"",concepto:"",monto:"",igv:"",subtotal:"",categoria:"Otro",notas:"",foto_texto:""});
    setOcr({loading:false,progress:0,text:"",imgUrl:""});
    setPaso(1);
    setModal("nuevo");
  };

  const abrirEditar=(g)=>{
    setForm({...g,monto:g.monto||"",igv:g.igv||"",subtotal:g.subtotal||""});
    setOcr({loading:false,progress:0,text:g.foto_texto||"",imgUrl:""});
    setPaso(2);
    setModal(g);
  };

  // Input estilo Khipu — línea inferior, label flotante
  const KInp=({label,keyN,type="text",opts=[]})=>(
    <div style={{marginBottom:16}}>
      <label style={{fontSize:11,fontWeight:600,color:C.muted,textTransform:"uppercase",letterSpacing:1,display:"block",marginBottom:5}}>{label}</label>
      {type==="select"
        ?<select value={form[keyN]} onChange={e=>setForm(f=>({...f,[keyN]:e.target.value}))}
            style={{width:"100%",background:"transparent",border:"none",borderBottom:`1.5px solid ${C.border}`,padding:"8px 0",color:C.white,fontSize:14,outline:"none",WebkitAppearance:"none"}}>
        ...
        :<input
            ...
            style={{width:"100%",background:"transparent",border:"none",borderBottom:`1.5px solid ${C.border}`,padding:"8px 0",color:C.white,fontSize:16,outline:"none",boxSizing:"border-box",WebkitAppearance:"none",minHeight:36}}/>
      }
    </div>
  );

  return(<div>
    <input ref={fileRef} type="file" accept="image/*" capture="environment" style={{display:"none"}} onChange={e=>escanear(e.target.files[0])}/>

    {/* Header */}
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:20}}>
      <PgTit icon="🧾" title="Gastos" sub="Escanea boletas · Tesseract OCR gratis"/>
      <button onClick={abrirNuevo} style={{background:"linear-gradient(135deg,#2563EB,#3B82F6)",border:"none",borderRadius:12,padding:"10px 18px",color:"#fff",fontWeight:700,fontSize:13,cursor:"pointer",boxShadow:"0 4px 12px rgba(37,99,235,0.4)"}}>+ Nuevo</button>
    </div>
    ...
    {Object.keys(porCat).length>0&&(
      <div style={{display:"flex",flexWrap:"wrap",gap:8,marginBottom:18}}>
        {Object.entries(porCat).sort((a,b)=>b[1]-a[1]).map(([cat,tot])=>(
          <div key={cat} style={{background:(catColor[cat]||C.muted)+"15",border:`1px solid ${catColor[cat]||C.muted}33`,borderRadius:20,padding:"5px 12px",fontSize:12,fontWeight:700,color:catColor[cat]||C.muted,display:"flex",alignItems:"center",gap:5}}>
    ...
    {/* Lista de gastos */}
    {delMes.length===0
      ?<div style={{textAlign:"center",padding:"50px 20px",background:"linear-gradient(135deg,#131F38,#0E1829)",border:"1px solid #1E3050",borderRadius:20}}>
        <div style={{fontSize:48,marginBottom:12}}>🧾</div>
        <div style={{fontSize:15,fontWeight:700,color:C.white,marginBottom:6}}>Sin gastos este mes</div>
        <div style={{fontSize:13,color:C.muted,marginBottom:20}}>Toca "+ Nuevo" o escanea una boleta con Tesseract OCR (gratis, sin API key)</div>
        <button onClick={abrirNuevo} style={{background:"linear-gradient(135deg,#2563EB,#3B82F6)",border:"none",borderRadius:12,padding:"11px 24px",color:"#fff",fontWeight:700,fontSize:14,cursor:"pointer"}}>+ Agregar gasto</button>
      </div>
      :<div style={{display:"flex",flexDirection:"column",gap:12}}>
        {delMes.map(g=>(
          <div key={g.id} style={{background:"linear-gradient(145deg,#131F38,#0E1829)",border:"1px solid #1E3050",borderRadius:18,overflow:"hidden",boxShadow:"0 2px 16px rgba(0,0,0,0.3)"}}>
            ...
            <div style={{padding:"16px 18px"}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
                <div style={{flex:1,minWidth:0}}>
                  {/* Proveedor */}
                  <div style={{fontSize:15,fontWeight:800,color:C.white,marginBottom:3}}>{g.proveedor||"Sin proveedor"}</div>
                  {g.ruc&&<div style={{fontSize:11,color:C.muted,marginBottom:4}}>RUC {g.ruc}</div>}
                  {g.concepto&&<div style={{fontSize:12,color:"#94A3B8",marginBottom:8,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{g.concepto}</div>}
                  <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>
                    <span style={{background:(catColor[g.categoria]||C.muted)+"20",color:catColor[g.categoria]||C.muted,borderRadius:20,padding:"3px 10px",fontSize:11,fontWeight:700,display:"flex",alignItems:"center",gap:4}}>
                      {catIcon[g.categoria]||"📦"} {g.categoria||"Otro"}
                    </span>
                    <span style={{background:"#1E3050",color:C.muted,borderRadius:20,padding:"3px 10px",fontSize:11}}>📅 {g.fecha}</span>
                  </div>
                </div>
                {/* Monto + botones */}
                <div style={{display:"flex",flexDirection:"column",alignItems:"flex-end",gap:10,marginLeft:14,flexShrink:0}}>
                  <div style={{fontSize:22,fontWeight:900,color:C.green,letterSpacing:"-0.5px"}}>{fmt(g.monto)}</div>
                  <div style={{display:"flex",gap:6}}>
                    <button onClick={()=>abrirEditar(g)} style={{background:"#2563EB18",border:"1px solid #2563EB33",borderRadius:9,padding:"6px 12px",color:C.accent,cursor:"pointer",fontSize:12,fontWeight:700}}>✏️ Editar</button>
                    <button onClick={()=>eliminar(g.id)} style={{background:C.red+"18",border:`1px solid ${C.red}33`,borderRadius:9,padding:"6px 10px",color:C.red,cursor:"pointer",fontSize:13}}>🗑</button>
                  </div>
                </div>
              </div>
              {/* Desglose IGV */}
              {g.igv>0&&(
                <div style={{marginTop:12,paddingTop:10,borderTop:"1px solid #1E3050",display:"flex",gap:16,fontSize:12}}>
                  <span style={{color:C.muted}}>Subtotal: <strong style={{color:C.white}}>{fmt(g.subtotal)}</strong></span>
                  <span style={{color:C.muted}}>IGV 18%: <strong style={{color:C.white}}>{fmt(g.igv)}</strong></span>
                </div>
              )}
            </div>
          </div>
    ...
    {/* ── MODAL — estilo Khipu ── */}
    {modal&&(
      <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.85)",zIndex:400,display:"flex",alignItems:"flex-end",justifyContent:"center",backdropFilter:"blur(8px)"}}
        onClick={e=>e.target===e.currentTarget&&(setModal(null),setPaso(1))}>
        <div style={{background:"#0E1829",border:"1px solid #1E3050",borderRadius:"24px 24px 0 0",width:"100%",maxWidth:520,maxHeight:"92vh",overflowY:"auto",paddingBottom:32}}>
          ...
          {/* Indicador de pasos */}
          <div style={{display:"flex",alignItems:"center",justifyContent:"center",gap:8,padding:"8px 24px 16px"}}>
            {[{n:1,label:"Boleta"},{n:2,label:"Datos"}].map(({n,label})=>(
              <div key={n} style={{display:"flex",alignItems:"center",gap:6}}>
                <div style={{width:28,height:28,borderRadius:"50%",background:paso>=n?"linear-gradient(135deg,#2563EB,#3B82F6)":"#1E3050",border:`2px solid ${paso>=n?"#3B82F6":"#2D4060"}`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:12,fontWeight:800,color:paso>=n?"#fff":"#4B5563",transition:"all .3s"}}>
                  {paso>n?"✓":n}
                </div>
                <span style={{fontSize:12,fontWeight:600,color:paso>=n?C.white:C.muted}}>{label}</span>
                {n<2&&<div style={{width:32,height:2,background:paso>n?"#3B82F6":"#1E3050",borderRadius:1,transition:"all .3s"}}/>}
              </div>
            ))}
          </div>
          ...
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"0 24px 16px"}}>
            <div style={{fontSize:17,fontWeight:800,color:C.white}}>
              {paso===1?"Información de Documento":"Especificaciones"}
            </div>
            <button onClick={()=>{setModal(null);setPaso(1);}} style={{width:32,height:32,borderRadius:"50%",background:"#1E3050",border:"1px solid #2D4060",color:C.muted,cursor:"pointer",fontSize:16,display:"flex",alignItems:"center",justifyContent:"center"}}>✕</button>
          </div>
            ...
            <div style={{padding:"0 24px"}}>
              {ocr.imgUrl?(
                <div style={{position:"relative",marginBottom:20}}>
                  <img src={ocr.imgUrl} alt="boleta" style={{width:"100%",borderRadius:16,border:"1px solid #1E3050",maxHeight:280,objectFit:"contain",background:"#060E1A"}}/>
                  {ocr.loading&&(
                    <div style={{position:"absolute",inset:0,background:"rgba(8,12,26,0.85)",borderRadius:16,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:12}}>
                      <div style={{width:40,height:40,border:"3px solid #1E3050",borderTopColor:"#3B82F6",borderRadius:"50%",animation:"spin .7s linear infinite"}}/>
                      <div style={{fontSize:13,color:C.white,fontWeight:600}}>🔍 Tesseract OCR analizando... {ocr.progress}%</div>
                      <div style={{width:180,height:4,background:"#1E3050",borderRadius:2,overflow:"hidden"}}>
                        <div style={{height:"100%",width:`${ocr.progress}%`,background:"linear-gradient(90deg,#2563EB,#3B82F6)",borderRadius:2,transition:"width .3s"}}/>
                      </div>
                    </div>
                  )}
                  {!ocr.loading&&ocr.text&&(
                    <div style={{position:"absolute",top:10,right:10,background:"#10B981",borderRadius:20,padding:"4px 12px",fontSize:11,fontWeight:700,color:"#fff"}}>✓ Datos extraídos</div>
                  )}
                </div>
              ):(
                <div onClick={()=>fileRef.current.click()} style={{border:"2px dashed #2D4060",borderRadius:20,padding:"40px 20px",textAlign:"center",cursor:"pointer",marginBottom:20,background:"#060E1A",transition:"border-color .2s"}}>
                  <div style={{fontSize:40,marginBottom:12}}>📷</div>
                  <div style={{fontSize:15,fontWeight:700,color:C.white,marginBottom:6}}>Tomar foto o subir boleta</div>
                  <div style={{fontSize:12,color:C.muted,marginBottom:16}}>📷 Tesseract OCR — 100% gratis, funciona sin API key, preprocesa la imagen para mayor precisión</div>
                  <div style={{background:"linear-gradient(135deg,#2563EB,#3B82F6)",border:"none",borderRadius:12,padding:"11px 24px",color:"#fff",fontWeight:700,fontSize:14,display:"inline-block",boxShadow:"0 4px 12px rgba(37,99,235,0.4)"}}>
                    Seleccionar imagen
                  </div>
                </div>
              )}
              ...
              <div style={{display:"flex",gap:10}}>
                <button onClick={()=>fileRef.current.click()} style={{flex:1,padding:"13px",background:"#1E3050",border:"1px solid #2D4060",borderRadius:12,color:C.white,fontWeight:600,fontSize:13,cursor:"pointer"}}>
                  {ocr.imgUrl?"🔄 Reescanear":"📷 Subir boleta"}
                </button>
                <button onClick={()=>setPaso(2)} style={{flex:1,padding:"13px",background:"linear-gradient(135deg,#2563EB,#3B82F6)",border:"none",borderRadius:12,color:"#fff",fontWeight:700,fontSize:13,cursor:"pointer",boxShadow:"0 4px 12px rgba(37,99,235,0.3)"}}>
                  Siguiente →
                </button>
              </div>
            </div>
          ...
          {/* Paso 2 — Datos del gasto */}
          {paso===2&&(
            <div style={{padding:"0 24px"}}>
              {ocr.text&&!ocr.loading&&(
                <div style={{background:"#10B98112",border:"1px solid #10B98133",borderRadius:12,padding:"10px 14px",marginBottom:16,display:"flex",alignItems:"center",gap:10}}>
                  <span style={{fontSize:18}}>✅</span>
                  <span style={{fontSize:12,color:"#6EE7B7",fontWeight:600}}>Datos extraídos automáticamente — revisa y ajusta si es necesario</span>
                </div>
              )}

              {/* Detalle de Documento (dropdown) */}
              <div style={{background:"#060E1A",border:"1px solid #1E3050",borderRadius:12,padding:"12px 16px",marginBottom:16}}>
                <div style={{fontSize:12,fontWeight:700,color:C.muted,textTransform:"uppercase",letterSpacing:1,marginBottom:12}}>Detalle de Documento</div>
                <KInp label="Monto Total *" keyN="monto" type="number"/>
                <KInp label="Moneda" keyN="moneda" type="text"/>
                ...
                <KInp label="Fecha *" keyN="fecha" type="date"/>
                <KInp label="IGV" keyN="igv" type="number"/>
                <KInp label="Proveedor / Empresa" keyN="proveedor"/>
                <KInp label="RUC" keyN="ruc"/>
                <KInp label="Concepto" keyN="concepto"/>
                <KInp label="Notas" keyN="notas"/>
              </div>

              <div style={{display:"flex",gap:10}}>
                <button onClick={()=>setPaso(1)} style={{padding:"13px 20px",background:"transparent",border:"1px solid #1E3050",borderRadius:12,color:C.muted,fontWeight:600,fontSize:13,cursor:"pointer"}}>← Anterior</button>
                <button onClick={guardar} disabled={saving} style={{flex:1,padding:"13px",background:saving?"#1E3050":"linear-gradient(135deg,#2563EB,#3B82F6)",border:"none",borderRadius:12,color:"#fff",fontWeight:700,fontSize:14,cursor:saving?"not-allowed":"pointer",boxShadow:saving?"none":"0 4px 12px rgba(37,99,235,0.4)"}}>
                  {saving?"Guardando...":modal==="nuevo"||modal==="editar-nuevo"?"Guardar Gasto":"Actualizar Gasto"}
                </button>
              </div>
            </div>
          )}
...
export default function App(){
  ...
  const tabs=[{id:"dashboard",icon:"⚡",label:"Dashboard"},{id:"mapa",icon:"🗺️",label:"Mapa"},{id:"paneles",icon:"🖥️",label:"Paneles"},{id:"contratos",icon:"📋",label:"Contratos"},{id:"crm",icon:"👥",label:"Clientes"},{id:"resultados",icon:"📈",label:"Resultados"},{id:"gastos",icon:"🧾",label:"Gastos"},{id:"alertas",icon:"🔔",label:"Alertas"}];
  ...
  return(<>
    ...
    <style>{`*{box-sizing:border-box;-webkit-tap-highlight-color:transparent}html,body{margin:0;padding:0;background:#000;overflow:hidden;height:100%;-webkit-text-size-adjust:100%}::-webkit-scrollbar{display:none}@keyframes spin{to{transform:rotate(360deg)}}@keyframes pulse{from{opacity:.2}to{opacity:.6}}`}</style>
    ...
    <div style={{fontFamily:"'DM Sans',sans-serif",background:C.bg,height:"100dvh",overflowY:"auto",overflowX:"hidden",color:C.text,display:"flex",flexDirection:"column",WebkitOverflowScrolling:"touch"}}>
      <nav style={{background:"#0A0F1E",borderBottom:"2px solid rgba(255,255,255,0.08)",paddingLeft:16,paddingRight:16,paddingTop:"env(safe-area-inset-top)",display:"flex",alignItems:"center",minHeight:56,position:"sticky",top:0,zIndex:200,gap:2,overflowX:"auto",scrollbarWidth:"none",flexShrink:0}}>
        ...
        {tabs.map(t=>{
          ...
          return(
          <button key={t.id} onClick={()=>setTab(t.id)} style={{padding:"0 12px",height:"100%",display:"flex",alignItems:"center",gap:6,fontSize:13,fontWeight:tab===t.id?700:400,color:tab===t.id?"#FFFFFF":"rgba(255,255,255,0.45)",background:"none",border:"none",borderBottom:tab===t.id?"2px solid #3B82F6":"2px solid transparent",cursor:"pointer",whiteSpace:"nowrap",flexShrink:0,position:"relative"}}>
        ...
        <div style={{marginLeft:"auto",display:"flex",alignItems:"center",gap:8,flexShrink:0}}>
          <div style={{width:7,height:7,borderRadius:"50%",background:error?C.red:loading?C.amber:C.green}}/>
          <span style={{fontSize:11,color:C.muted}}>{error?"Error API":loading?"Conectando...":"Supabase ✓"}</span>
          <button onClick={()=>setShowConfig(true)} title="Configurar API Key" style={{background:error?C.red+"22":"#18253D",border:`1px solid ${error?C.red+"44":C.border}`,borderRadius:8,padding:"5px 9px",color:error?C.red:C.muted,cursor:"pointer",fontSize:14,display:"flex",alignItems:"center"}}>⚙️</button>
        </div>
      </nav>

      <main style={{flex:1,padding:"20px 16px",maxWidth:1240,width:"100%",margin:"0 auto",boxSizing:"border-box"}}>
        {error&&(
        ...
        {tab==="contratos"  &&<Contratos  contratos={contratos} setContratos={setContratos} paneles={paneles} clientes={clientes} loading={loading}/>}
