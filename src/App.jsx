...
// ── iOS viewport height — setter eager a nivel módulo ────────────
// Corre antes de que React monte. Mantiene --app-height = visualViewport.height
// para que html/body/Splash/contenedor principal usen la altura real visible
// (no la layout viewport, que en iOS no sigue al chrome del browser).
if (typeof window !== "undefined") {
  const __setAppHeight = () => {
  ...
  __setAppHeight();
  const __vvp = window.visualViewport;
  if (__vvp) {
    __vvp.addEventListener("resize", __setAppHeight);
    __vvp.addEventListener("scroll", __setAppHeight);
  } else {
    window.addEventListener("resize", __setAppHeight);
  }
  window.addEventListener("orientationchange", __setAppHeight);
  window.addEventListener("pageshow", __setAppHeight);
}
    ...
    <>
      ...
      <style>{`
        *{box-sizing:border-box;-webkit-tap-highlight-color:transparent;-webkit-touch-callout:none}
        html{margin:0;padding:0;background:#F2F4F8;height:var(--app-height,100dvh);width:100%;-webkit-text-size-adjust:100%}
        body{margin:0;padding:0;background:#F2F4F8;height:var(--app-height,100dvh);width:100%;position:fixed;top:0;left:0;overscroll-behavior-y:none;-webkit-text-size-adjust:100%;font-family:'DM Sans',-apple-system,BlinkMacSystemFont,'SF Pro Display','SF Pro Text','Helvetica Neue',sans-serif;-webkit-font-smoothing:antialiased;-moz-osx-font-smoothing:grayscale}
        #root{height:var(--app-height,100dvh);width:100%}
      ...
      <div style={{ width: "100vw", height: "var(--app-height, 100dvh)", position: "fixed", top: 0, left: 0, background: NL.bg, display: "flex", flexDirection: "column", fontFamily: "'DM Sans',sans-serif", color: NL.text, overflow: "hidden" }}>
