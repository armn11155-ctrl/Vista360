...
function Capital({ paneles, contratos, gastos, proveedores }) {
  ...
  const EM = {
    bg:      "#F2F4F8",
    card1:   "#0F1729",  // negro azulado — bloque capital
    card2:   "#1E3A8A",  // azul oscuro — crecimiento
    cardLt:  "#ffffff",  // blanco — tabla
  ...
  return (
    <div style={{ background: EM.bg, margin: "-20px -16px", minHeight: "100%" }}>
      ...
      {/* ══════════════════════════════════════════════
          BLOQUE 1 — TU CAPITAL (editable)  — card negro
      ══════════════════════════════════════════════ */}
      <div style={{ margin: "0 0 14px", background: EM.card1, borderRadius: "0 0 28px 28px", padding: "28px 22px 28px", boxShadow: "0 8px 32px rgba(15,23,41,0.18)" }}>
      ...
      {/* ══════════════════════════════════════════════
          BLOQUE 3 — DASHBOARD DE CRECIMIENTO — card azul oscuro
      ══════════════════════════════════════════════ */}
      <div style={{ margin: "0 16px", paddingBottom: "calc(120px + env(safe-area-inset-bottom))" }}>

        {/* Card crecimiento — azul oscuro */}
        <div style={{ background: EM.card2, borderRadius: 22, padding: "20px 20px 18px", marginBottom: 14, boxShadow: "0 6px 24px rgba(30,58,138,0.25)" }}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 18 }}>
            <div style={{ fontSize: 11, color: "rgba(200,220,255,0.65)", fontWeight: 700, letterSpacing: 1.2, textTransform: "uppercase" }}>
              Crecimiento · 6 meses
            </div>
            <div style={{
              background: crecPct >= 0 ? "rgba(16,185,129,0.2)" : "rgba(239,68,68,0.2)",
              border: `1px solid ${crecPct >= 0 ? "rgba(16,185,129,0.4)" : "rgba(239,68,68,0.4)"}`,
              ...
              return (
                <div key={mes} style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", gap: 3, height: "100%", minWidth: 0 }}>
                  <div style={{ flex: 1, width: "100%", display: "flex", flexDirection: "column", justifyContent: "flex-end", gap: 2 }}>
                    {ing > 0 && <div style={{ width: "100%", height: `${(ing/maxBar)*100}%`, minHeight: 5, background: isActual ? "linear-gradient(180deg,#60A5FA,#2563EB)" : "rgba(255,255,255,0.15)", borderRadius: "5px 5px 0 0", boxShadow: isActual ? "0 0 12px rgba(96,165,250,0.5)" : "none" }}/>}
                    {eg > 0 && <div style={{ width: "100%", height: `${(eg/maxBar)*100}%`, minHeight: 5, background: isActual ? "linear-gradient(180deg,#FCA5A5,#EF4444)" : "rgba(239,68,68,0.25)", borderRadius: "5px 5px 0 0" }}/>}
                    {ing === 0 && eg === 0 && <div style={{ width: "100%", height: 4, background: "rgba(255,255,255,0.08)", borderRadius: 2 }}/>}
                  </div>
                </div>
              );
            })}
          ...
          <div style={{ display: "flex", gap: 6, marginBottom: 14 }}>
            {flujo.map(({ mes }, idx) => (
              <div key={mes} style={{ flex: 1, textAlign: "center", fontSize: 9, color: idx === 5 ? "#fff" : "rgba(200,220,255,0.45)", fontWeight: idx === 5 ? 700 : 400, textTransform: "uppercase" }}>{mC(mes)}</div>
            ))}
          </div>

          {/* Leyenda */}
          <div style={{ display: "flex", gap: 14, justifyContent: "center" }}>
            <div style={{ display: "flex", alignItems: "center", gap: 5, fontSize: 11, color: "rgba(200,220,255,0.6)" }}>
              <div style={{ width: 9, height: 9, borderRadius: 2, background: "#60A5FA" }}/> Ingresos
            </div>
            <div style={{ display: "flex", alignItems: "center", gap: 5, fontSize: 11, color: "rgba(200,220,255,0.6)" }}>
              <div style={{ width: 9, height: 9, borderRadius: 2, background: "#EF4444" }}/> Gastos
            </div>
          </div>
        </div>

        {/* Card acumulado — negro */}
        <div style={{ background: EM.card1, borderRadius: 22, overflow: "hidden", boxShadow: "0 6px 24px rgba(15,23,41,0.2)" }}>
          {[
            { label: "Total ingresos 6m", value: fmt(ingTotal), color: EM.green },
            { label: "Total gastos 6m",   value: fmt(gasTotal), color: EM.red   },
            ...
            <div key={label} style={{
              ...
              background: bold ? "rgba(37,99,235,0.1)" : "transparent",
            }}>
              <span style={{ fontSize: 13, color: bold ? "#fff" : "rgba(200,220,255,0.65)", fontWeight: bold ? 700 : 500 }}>{label}</span>
          ...
          <div style={{ padding: "14px 20px", borderTop: "1px solid rgba(255,255,255,0.06)" }}>
            <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 8 }}>
              <span style={{ fontSize: 11, color: "rgba(200,220,255,0.5)", fontWeight: 700, textTransform: "uppercase", letterSpacing: 0.8 }}>Margen neto</span>
              <span style={{ fontSize: 13, fontWeight: 800, color: margen >= 0 ? EM.green : EM.red }}>{margen}%</span>
            </div>
            <div style={{ background: "rgba(255,255,255,0.07)", borderRadius: 99, height: 7, overflow: "hidden" }}>
              <div style={{ height: "100%", borderRadius: 99, width: `${Math.min(Math.abs(margen), 100)}%`, background: margen >= 0 ? "linear-gradient(90deg,#2563EB,#10B981)" : "#EF4444", transition: "width .6s ease" }}/>
            </div>
          </div>
        </div>
