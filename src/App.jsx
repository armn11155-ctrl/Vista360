...
export default function App() {
  ...
  // ── ANTI-BOUNCE iOS: bloquear overscroll nativo a nivel de documento ──
  useEffect(() => {
    // Forzar position:fixed en html/body para que iOS no tenga dónde rebotar
    document.documentElement.style.cssText += ";position:fixed;width:100%;height:100%;overflow:hidden;overscroll-behavior:none";
    document.body.style.cssText += ";position:fixed;width:100%;height:100%;overflow:hidden;overscroll-behavior:none;top:0;left:0";

    // Prevenir touchmove por defecto en el documento (solo iOS Safari)
    // Cada div scrollable maneja su propio touchmove internamente
    const blockDocumentScroll = (e) => {
      // Dejar pasar si el target o algún ancestro tiene data-scroll
      let el = e.target;
      while (el && el !== document.body) {
        if (el.dataset && el.dataset.scroll !== undefined) return; // dejar pasar
        el = el.parentElement;
      }
      e.preventDefault();
    };
    ...
    <>
      ...
      <style>{`
        *{box-sizing:border-box;-webkit-tap-highlight-color:transparent;-webkit-touch-callout:none}
        ...
        [data-scroll]{overscroll-behavior:none;overscroll-behavior-y:none;-webkit-overflow-scrolling:auto}
      ...
      {!splash && !!user && <div style={{ position: "fixed", top: 0, right: 0, bottom: 0, left: 0, background: NL.bg, display: "flex", flexDirection: "column", fontFamily: "'DM Sans',sans-serif", color: NL.text, overflow: "hidden", overscrollBehavior: "none" }}>
        ...
        <div
          data-scroll
          style={{ flex: 1, minHeight: 0, overflowY: "auto", overflowX: "hidden", overscrollBehavior: "none", overscrollBehaviorY: "none", scrollBehavior: "smooth" }}
        >
