...
function LoginScreen({ onLoginSuccess }) {
  ...
  const handleGoogleLogin = async () => {
    if (!navigator.onLine) { setError("Sin conexión a internet. Conéctate a WiFi o datos móviles."); return; }
    setLoading(true);
    setError("");
    try {
      ...
      const msgs = {
        ...
        "auth/popup-blocked":           "Safari bloqueó la ventana. Abre la app directamente en Safari (no desde la pantalla de inicio) e intenta de nuevo.",
        "auth/cancelled-popup-request": "",
        "auth/operation-not-allowed":   "Google Sign-In no está activado. Ve a Firebase Console → Authentication → Sign-in methods → Google → Habilitar.",
        "auth/unauthorized-domain":     "Este dominio no está autorizado en Firebase. Ve a Firebase Console → Authentication → Settings → Authorized domains y añade tu dominio.",
        "auth/network-request-failed":  "Sin conexión. Verifica tu internet e intenta de nuevo.",
        "auth/invalid-api-key":         "API key de Firebase inválida. Revisa la configuración del proyecto.",
        "auth/app-deleted":             "Proyecto Firebase no encontrado. Verifica tu configuración.",
      };
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ fontSize: 13, color: "rgba(255,255,255,0.55)", textAlign: "center", marginBottom: 20, lineHeight: 1.5 }}>
          Inicia sesión con tu cuenta de Google para acceder a Vista360
        </div>

        {/* Estado de conexión */}
        <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 6, marginBottom: 20, fontSize: 12, fontWeight: 600, color: online ? "#34D399" : "#F87171" }}>
          <div style={{ width: 7, height: 7, borderRadius: "50%", background: online ? "#34D399" : "#F87171", boxShadow: online ? "0 0 6px #34D399" : "0 0 6px #F87171" }}/>
          {online ? "Internet conectado" : "Sin conexión a internet"}
        </div>
        ...
        <button
          ...
          style={{
            width: "100%",
            padding: "14px 20px",
            background: (!online || loading) ? "rgba(255,255,255,0.35)" : "#FFFFFF",
            color: "#0F1729",
            border: "none",
            borderRadius: 12,
            fontSize: 15,
            ...
            gap: 12,
            transition: "all 0.2s",
            boxShadow: online ? "0 8px 24px rgba(0,0,0,0.3)" : "none",
            opacity: (!online || loading) ? 0.6 : 1,
          }}
        >
          ...
          {loading ? "Verificando con Google..." : !online ? "Sin internet" : "Continuar con Google"}
        </button>

        {/* Error */}
        {error && (
          <div style={{ marginTop: 16, padding: "12px 14px", background: "rgba(239,68,68,0.12)", border: "1px solid rgba(239,68,68,0.35)", borderRadius: 10, color: "#FCA5A5", fontSize: 12, lineHeight: 1.6, textAlign: "center" }}>
            ⚠️ {error}
          </div>
        )}

        {/* Footer */}
        <div style={{ marginTop: 22, paddingTop: 18, borderTop: "1px solid rgba(255,255,255,0.06)", fontSize: 11, color: "rgba(255,255,255,0.3)", textAlign: "center", lineHeight: 1.6 }}>
...
export default function App() {
  ...
  const recargarDatos = useCallback(() => {
    ...
    Promise.all([
      ...
      setError(null);
      setLoading(false);
    }).catch(() => { clearTimeout(tid); setError("load-error"); setLoading(false); });
  }, [user]);

  useEffect(() => { if (!user) return; recargarDatos(); }, [user]);

  // Auto-retry cuando vuelve el internet
  useEffect(() => {
    if (online && user && (error === "offline" || error === "timeout")) {
      recargarDatos();
    }
  }, [online]);
            ...
            {showProfile
              ...
              ? <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", height: 260, gap: 16 }}>
                  ...
                  <button
                    ...
                    style={{ marginTop: 8, padding: "8px 20px", background: "transparent", border: `1px solid ${NL.border}`, borderRadius: 10, color: NL.muted, fontSize: 12, cursor: "pointer", fontWeight: 600 }}
                  >
              ...
              ? <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", height: 280, gap: 16, padding: 24, textAlign: "center" }}>
                  ...
                  <div style={{ fontSize: 16, fontWeight: 700, color: NL.text }}>No se pudieron cargar los datos</div>
                  <div style={{ fontSize: 13, color: NL.muted, lineHeight: 1.5, maxWidth: 280 }}>
                    {error === "timeout"
                      ? "La conexión tardó demasiado. Verifica tu internet e intenta de nuevo."
                      : error === "permission-denied"
                      ? "Sin permisos para leer los datos. Cierra sesión y vuelve a entrar."
                      : `Error: ${error}. Verifica tu red e intenta de nuevo.`}
                  </div>
                  <button onClick={recargarDatos} style={{ padding: "12px 28px", background: NL.accent, color: "#fff", border: "none", borderRadius: 12, fontWeight: 700, fontSize: 15, cursor: "pointer", boxShadow: "0 4px 12px rgba(37,99,235,0.3)" }}>
                    🔄 Reintentar
                  </button>
                  <button
                    onClick={() => { signOut(auth).finally(() => { window.location.reload(); }); }}
                    style={{ padding: "10px 24px", background: "transparent", color: NL.muted, border: `1px solid ${NL.border}`, borderRadius: 12, fontWeight: 600, fontSize: 13, cursor: "pointer" }}
                  >
                    Cerrar sesión
                  </button>
                </div>
