  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      if (err.code === "auth/popup-blocked" || err.code === "auth/cancelled-popup-request") {
        // iOS Safari bloquea el popup — usar redirect como fallback
...
export default function App() {
  ...
  useEffect(() => {
    // Safety: si Firebase no responde en 6s, mostrar login de todas formas
    const timer = setTimeout(() => { setAuthReady(true); setLoading(false); }, 6000);
    ...
    const unsub = onAuthStateChanged(auth, (u) => {
      ...
      if (u && ALLOWED_EMAILS.length > 0 && !ALLOWED_EMAILS.includes(u.email)) {
        ...
        setUser(u);
        // ✅ FIX: si no hay usuario no hay nada que cargar → apagar loading.
        // Si hay usuario, el useEffect de carga de datos se encarga de apagarlo
        // cuando termina (éxito o error). Antes este path no llamaba setLoading(false)
        // y la app se quedaba en spinner infinito en Safari/redirect.
