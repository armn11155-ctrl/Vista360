...
function Contratos({contratos,setContratos,paneles,clientes,loading,setTab,onModalChange}){
  ...
  const eliminar=async(id)=>{
    if(!confirm("¿Eliminar este contrato? Solo se puede eliminar permanentemente desde Histórico.")) return;
        ...
        {gastosMes.map((g,i)=>(
          <div key={g.id} style={{padding:"12px 16px",borderBottom:i<gastosMes.length-1?`1px solid ${C.border}`:"none",display:"flex",alignItems:"center",gap:10}}>
            ...
            <div style={{display:"flex",gap:6,flexShrink:0}}>
              ...
              <button onClick={()=>eliminar(g.id)} style={{background:C.red+"22",border:"none",borderRadius:9,padding:"8px 10px",color:C.red,cursor:"pointer",touchAction:"manipulation",minWidth:36,minHeight:36}}><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><polyline points="3 6 5 6 21 6"/><path d="M19 6l-1 14a2 2 0 01-2 2H8a2 2 0 01-2-2L5 6"/></svg></button>
            </div>
          </div>
        ))}
      </div>}
...
function Capital({ paneles, contratos, gastos, proveedores }) {
  ...
  const panelDep = paneles.map(p => {
    ...
    const vida   = Number(p.vidaUtilAnios) || 10;
  ...
  return (
    <div style={{ background: EM.bg, margin: "-20px -16px", padding: "0 0 calc(120px + env(safe-area-inset-bottom))", minHeight: "calc(100vh + 300px)" }}>
      ...
      {seccion === "flujo" && (
        <div style={{ padding: "20px 18px", background: "#fff", margin: "0 -0px" }}>

          {/* Gráfico de barras */}
          <div style={{ marginBottom: 24 }}>
            <div style={{ fontSize: 11, color: "#64748B", fontWeight: 700, letterSpacing: 1, textTransform: "uppercase", marginBottom: 16 }}>Últimos 6 meses</div>
              ...
              {flujo.map(({ mes, ing, eg }) => (
                <div key={mes} style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", gap: 3, height: "100%", minWidth: 0 }}>
                  <div style={{ flex: 1, width: "100%", display: "flex", flexDirection: "column", justifyContent: "flex-end", gap: 3 }}>
                    {ing > 0 && <div style={{ width: "100%", height: `${(ing / maxBar) * 100}%`, minHeight: 5, background: "linear-gradient(180deg,#2563EB,#60A5FA)", borderRadius: "5px 5px 0 0", boxShadow: "0 0 10px rgba(37,99,235,0.3)" }}/>}
                    {eg > 0 && <div style={{ width: "100%", height: `${(eg / maxBar) * 100}%`, minHeight: 5, background: "linear-gradient(180deg,#EF4444,#FCA5A5)", borderRadius: "5px 5px 0 0" }}/>}
                  </div>
                </div>
              ))}
            </div>
            <div style={{ display: "flex", gap: 8 }}>
              {flujo.map(({ mes }) => (
                <div key={mes} style={{ flex: 1, textAlign: "center", fontSize: 9, color: "#94A3B8", fontWeight: 700, textTransform: "uppercase", letterSpacing: 0.5 }}>{mC(mes)}</div>
              ))}
            </div>
            <div style={{ display: "flex", gap: 18, justifyContent: "center", marginTop: 12 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 11, color: "#64748B" }}>
                <div style={{ width: 8, height: 8, borderRadius: 2, background: "#2563EB" }}/> Ingresos cobrados
              </div>
              <div style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 11, color: "#64748B" }}>
                <div style={{ width: 8, height: 8, borderRadius: 2, background: "#EF4444" }}/> Gastos
              </div>
            </div>
          </div>

          {/* Tabla mensual */}
          <div style={{ fontSize: 11, color: "#94A3B8", fontWeight: 700, letterSpacing: 1, textTransform: "uppercase", marginBottom: 12 }}>Detalle</div>
          {flujo.slice().reverse().map(({ mes, ing, eg, neto }) => (
            <div key={mes} style={{ display: "flex", alignItems: "center", gap: 12, padding: "13px 0", borderBottom: "1px solid #F1F5F9" }}>
              <div style={{ width: 40, height: 40, borderRadius: 12, background: neto >= 0 ? "#EFF6FF" : "#FEF2F2", border: `1px solid ${neto >= 0 ? "#BFDBFE" : "#FECACA"}`, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke={neto >= 0 ? "#2563EB" : "#EF4444"} strokeWidth="2.5" strokeLinecap="round">
                  {neto >= 0 ? <polyline points="18 15 12 9 6 15"/> : <polyline points="6 9 12 15 18 9"/>}
                </svg>
              </div>
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ fontSize: 13, fontWeight: 700, color: "#0F172A" }}>{mesLabel2(mes)}</div>
                <div style={{ fontSize: 11, color: "#94A3B8", marginTop: 2 }}>Ing. {fmt(ing)} · Gas. {fmt(eg)}</div>
              </div>
              <div style={{ textAlign: "right", flexShrink: 0 }}>
                <div style={{ fontSize: 16, fontWeight: 800, color: neto >= 0 ? "#10B981" : "#EF4444", letterSpacing: "-0.5px" }}>{neto >= 0 ? "+" : ""}{fmt(neto)}</div>
                <div style={{ fontSize: 9, color: "#94A3B8", marginTop: 2, textTransform: "uppercase", letterSpacing: 0.5 }}>neto</div>
              </div>
            </div>
          ))}
        </div>
      ...
      {seccion === "depreciacion" && (
        <div style={{ padding: "20px 18px", background: "#fff" }}>
          ...
          {/* Resumen portfolio */}
          {costoOriginal > 0 && (
            <div style={{ background: "#F8FAFC", border: "1px solid #E5E7EB", borderRadius: 16, padding: "16px 18px", marginBottom: 20 }}>
              <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 10 }}>
                <span style={{ fontSize: 11, color: "#64748B", fontWeight: 700, textTransform: "uppercase", letterSpacing: 0.8 }}>Portfolio total</span>
                <span style={{ fontSize: 11, color: "#94A3B8" }}>{pctPortfolio.toFixed(0)}% depreciado</span>
              </div>
              <div style={{ background: "#E5E7EB", borderRadius: 99, height: 8, overflow: "hidden", marginBottom: 10 }}>
                <div style={{ height: "100%", borderRadius: 99, width: `${100 - pctPortfolio}%`, background: "linear-gradient(90deg,#2563EB,#60A5FA)", transition: "width .5s ease" }}/>
              </div>
              <div style={{ display: "flex", justifyContent: "space-between", fontSize: 11 }}>
                <span style={{ color: "#10B981", fontWeight: 700 }}>{fmt(valorPaneles)} valor actual</span>
                <span style={{ color: "#EF4444", fontWeight: 700 }}>{fmt(depAcumTotal)} depreciado</span>
              </div>
            </div>
          )}

          {/* Aviso para editar panel */}
          <div style={{ display: "flex", alignItems: "center", gap: 10, background: "#EFF6FF", border: "1px solid #BFDBFE", borderRadius: 12, padding: "10px 14px", marginBottom: 20 }}>
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#2563EB" strokeWidth="2" strokeLinecap="round" style={{ flexShrink: 0 }}><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/></svg>
            <span style={{ fontSize: 12, color: "#1E40AF" }}>Para cambiar la tasa de depreciación de un panel, edítalo desde la sección <strong>Paneles</strong>.</span>
          </div>

          {/* Lista paneles */}
          <div style={{ fontSize: 11, color: "#94A3B8", fontWeight: 700, letterSpacing: 1, textTransform: "uppercase", marginBottom: 12 }}>Por panel</div>
          {panelDep.filter(p => Number(p.costoInstalacion) > 0).length === 0 ? (
            <div style={{ background: "#F8FAFC", border: "1px solid #E5E7EB", borderRadius: 16, padding: "36px 20px", textAlign: "center" }}>
              <div style={{ width: 48, height: 48, borderRadius: 14, background: "#EFF6FF", border: "1px solid #BFDBFE", display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto 12px", color: "#2563EB" }}>
              ...
              <div style={{ fontSize: 13, fontWeight: 600, color: "#0F172A", marginBottom: 4 }}>Sin datos de capital</div>
              <div style={{ fontSize: 12, color: "#94A3B8" }}>Edita cada panel y agrega costo + fecha de instalación</div>
            </div>
          ) : (
            panelDep.filter(p => Number(p.costoInstalacion) > 0).sort((a, b) => b.pctDesgaste - a.pctDesgaste).map((p, idx, arr) => (
              <div key={p.id} style={{ background: "#F8FAFC", border: "1px solid #E5E7EB", borderRadius: 16, padding: "16px 18px", marginBottom: idx < arr.length - 1 ? 10 : 0, boxShadow: "0 1px 3px rgba(0,0,0,0.04)" }}>
                <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 12 }}>
                  <div style={{ width: 42, height: 42, borderRadius: 12, background: "linear-gradient(135deg,#1E3A8A,#2563EB)", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                    <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="1.8" strokeLinecap="round"><rect x="3" y="3" width="7" height="7" rx="1.5"/><rect x="14" y="3" width="7" height="7" rx="1.5"/><rect x="3" y="14" width="7" height="7" rx="1.5"/><rect x="14" y="14" width="7" height="7" rx="1.5"/></svg>
                  </div>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ fontSize: 14, fontWeight: 700, color: "#0F172A", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{p.nombre}</div>
                    <div style={{ fontSize: 11, color: "#94A3B8", marginTop: 2 }}>{p.ciudad} · {(p.aniosUsados || 0).toFixed(1)} años · {Number(p.vidaUtilAnios)||10} años vida útil</div>
                  </div>
                  <div style={{ textAlign: "right", flexShrink: 0 }}>
                    <div style={{ fontSize: 16, fontWeight: 800, color: "#10B981", letterSpacing: "-0.5px" }}>{fmt(p.valorActual)}</div>
                    <div style={{ fontSize: 9, color: "#94A3B8", marginTop: 2, textTransform: "uppercase", letterSpacing: 0.5 }}>actual</div>
                  </div>
                </div>
                <div style={{ background: "#E5E7EB", borderRadius: 99, height: 6, overflow: "hidden", marginBottom: 7 }}>
                  <div style={{
                    height: "100%", borderRadius: 99, transition: "width .5s ease", width: `${p.pctDesgaste}%`,
                    background: p.pctDesgaste > 75 ? "linear-gradient(90deg,#EF4444,#FCA5A5)" : p.pctDesgaste > 50 ? "linear-gradient(90deg,#64748B,#94A3B8)" : "linear-gradient(90deg,#2563EB,#60A5FA)",
                  }}/>
                </div>
                <div style={{ display: "flex", justifyContent: "space-between", fontSize: 11 }}>
                  <span style={{ color: "#64748B" }}>Original {fmt(Number(p.costoInstalacion))}</span>
                  <span style={{ color: p.pctDesgaste > 75 ? "#EF4444" : p.pctDesgaste > 50 ? "#64748B" : "#2563EB", fontWeight: 700 }}>{p.pctDesgaste.toFixed(0)}% depreciado</span>
                </div>
              </div>
            ))
          )}
...
export default function App() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      // Limpiar contratos marcados con deleted:true (borrado lógico pendiente → físico)
      const ctArr = Array.isArray(ct) ? ct : [];
      const ctVivos = ctArr.filter(x => !x.deleted);
      const ctMuertos = ctArr.filter(x => x.deleted);
      if (ctMuertos.length > 0) {
        ctMuertos.forEach(c => fb.del("contratos", c.id, { hardDelete: true }).catch(() => {}));
      }
      setContratos(ctVivos);
