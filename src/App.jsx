    ...
    if (showProfile) {
      ...
      return (
        <div>
          ...
          <Firebase contratos={contratos.filter(x=>!x.deleted)} paneles={paneles} clientes={clientes} gastos={gastos} fbConnected={!error && !loading} fbLoading={loading} fbError={!!error}/>
        </div>
      );
    }
    return (
      <>
        ...
        {tab === "hoy"         && <ResumenNuevo clientes={clientes} contratos={contratos.filter(x=>!x.deleted)} paneles={paneles} gastos={gastos} setTab={setTab}/>}
    ...
    <>
      ...
      {!splash && !!user && <div style={{ position: "fixed", top: 0, left: 0, right: 0, bottom: 0, background: NL.bg, display: "flex", flexDirection: "column", fontFamily: "'DM Sans',sans-serif", color: NL.text, overflow: "hidden", overscrollBehavior: "none" }}>
        ...
        <NotifPanel
          ...
          contratos={contratos.filter(x=>!x.deleted)}
          clientes={clientes}
          paneles={paneles}
          gastos={gastos}
        />
        <BusquedaGlobal open={globalSearch} onClose={() => setGlobalSearch(false)} paneles={paneles} clientes={clientes} contratos={contratos.filter(x=>!x.deleted)} onNavigate={handleTabClick}/>
