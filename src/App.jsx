...
// ══════════════════════════════════════════════════════════════════
// Vista360 v33 — build: 2026-04-30 — botones #FFFFFF forzado
...
// ── PALETA ───────────────────────────────────────────────────────
// darkMode se actualiza via setDarkMode desde el contexto global
let darkMode = (() => { try { return localStorage.getItem("v360_dark") !== "false"; } catch(e){ return true; } })();
const DARK = { bg:"#070C18", surface:"#0D1421", card:"#111929", border:"#18253D", accent:"#2563EB", green:"#10B981", red:"#EF4444", amber:"#F59E0B", purple:"#8B5CF6", cyan:"#06B6D4", text:"#E2E8F0", muted:"#64748B", white:"#FFFFFF", money:"#10B981", btnText:"#FFFFFF" };
const LIGHT= { bg:"#E8EDF4", surface:"#F1F5F9", card:"#FFFFFF", border:"#CBD5E1", accent:"#2563EB", green:"#059669", red:"#DC2626", amber:"#D97706", purple:"#7C3AED", cyan:"#0284C7", text:"#0F172A", muted:"#64748B", white:"#1E293B", money:"#059669", btnText:"#FFFFFF" };
...
const Card   = ({children,style={}})=><div style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:16,padding:22,boxShadow:darkMode?"none":"0 2px 12px rgba(15,23,42,0.08), 0 1px 3px rgba(15,23,42,0.06)",...style}}>{children}</div>;
const SecTit = ({ch})=><div style={{fontSize:11,fontWeight:700,color:darkMode?C.muted:"#334155",textTransform:"uppercase",letterSpacing:1.2,marginBottom:14}}>{ch}</div>;
const PgTit  = ({icon,title,sub})=><div style={{marginBottom:22}}><div style={{fontSize:22,fontWeight:800,color:darkMode?C.white:"#0F172A",letterSpacing:"-0.5px"}}>{icon} {title}</div>{sub&&<div style={{fontSize:13,color:C.muted,marginTop:3}}>{sub}</div>}</div>;
...
function ModalConfig({onClose}){
  ...
  const base={background:C.surface,border:`1px solid ${C.border}`,borderRadius:10,padding:"10px 13px",color:C.text,fontSize:13,outline:"none",fontFamily:"inherit",width:"100%",boxSizing:"border-box"};
  return(
    <div style={{position:"fixed",inset:0,background:"#00000092",zIndex:400,display:"flex",alignItems:"center",justifyContent:"center",padding:20,backdropFilter:"blur(6px)"}}
      ...
      <div style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:20,padding:26,width:"100%",maxWidth:480}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:18}}>
          <div style={{fontSize:16,fontWeight:800,color:C.white}}>⚙️ Configuración</div>
        ...
        {/* TOGGLE DÍA / NOCHE */}
        <div style={{marginBottom:20,padding:"14px 16px",background:C.surface,borderRadius:12,border:`1px solid ${C.border}`,display:"flex",alignItems:"center",justifyContent:"space-between"}}>
          <div>
            <div style={{fontSize:13,fontWeight:700,color:C.white}}>{darkMode?"🌙 Modo Noche":"☀️ Modo Día"}</div>
            <div style={{fontSize:11,color:C.muted,marginTop:2}}>{darkMode?"Fondo oscuro, acentos azul":"Fondo blanco, acentos celeste"}</div>
          </div>
          <button onClick={()=>{ darkMode=!darkMode; C=darkMode?DARK:LIGHT; try{localStorage.setItem("v360_dark",darkMode?"true":"false");}catch(e){} onClose(); window.location.reload(); }}
            style={{width:52,height:28,borderRadius:14,border:"none",cursor:"pointer",position:"relative",background:darkMode?"#2563EB":"#CBD5E1",transition:"background .3s",flexShrink:0,padding:0}}>
            <div style={{position:"absolute",top:3,left:darkMode?26:3,width:22,height:22,borderRadius:"50%",background:"#fff",transition:"left .25s",boxShadow:"0 1px 4px #0003"}}/>
          </button>
        </div>
        <div style={{fontSize:12,color:C.muted,marginBottom:14,lineHeight:1.6}}>
          Si ves <strong style={{color:C.amber}}>"Invalid API key"</strong>, pega aquí tu <strong style={{color:C.white}}>anon key</strong> actualizada desde el panel de Supabase → Settings → API.
        </div>
        ...
        <div style={{display:"flex",gap:10}}>
          <button onClick={onClose} style={{flex:1,padding:12,background:"transparent",border:`1px solid ${C.border}`,borderRadius:10,color:C.muted,fontWeight:600,cursor:"pointer",fontSize:13}}>Cancelar</button>
    ...
    <div style={{position:"fixed",inset:0,background:"#00000092",zIndex:300,display:"flex",alignItems:"center",justifyContent:"center",padding:20,backdropFilter:"blur(6px)"}}
      ...
      <div style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:20,padding:26,width:"100%",maxWidth:560,maxHeight:"90vh",overflowY:"auto"}}>
        ...
        <div style={{display:"flex",gap:10,marginTop:20}}>
          ...
          <button onClick={onSave} style={{flex:2,padding:12,background:C.accent,border:"none",borderRadius:10,color:"#FFFFFF",fontWeight:700,fontSize:14,cursor:"pointer"}}>{saveLabel}</button>
        </div>
      </div>
    </div>
  );
...
function Splash({done}){
  ...
  return(
    <div style={{position:"fixed",inset:0,background:"#070C18",zIndex:999,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",opacity:f===3?0:1,transition:f===3?"opacity .55s ease":"none"}}>
      {/* Anillos decorativos */}
      <div style={{position:"absolute",inset:0,overflow:"hidden",pointerEvents:"none"}}>
        {[...Array(5)].map((_,i)=>(
          <div key={i} style={{position:"absolute",borderRadius:"50%",width:220+i*110,height:220+i*110,top:"50%",left:"50%",transform:"translate(-50%,-50%)",border:`1px solid #2563EB${["0A","0D","10","13","16"][i]}`,animation:`pulse ${2.2+i*0.3}s ease-in-out infinite alternate`}}/>
        ))}
      </div>
      {/* Logo — siempre blanco sobre fondo negro */}
      <div style={{opacity:f>=1?1:0,transform:f>=1?"scale(1) translateY(0)":"scale(.8) translateY(24px)",transition:"all .75s cubic-bezier(.34,1.4,.64,1)",filter:"drop-shadow(0 0 40px #2563EB55) brightness(0) invert(1)"}}>
        <Logo360 width={300}/>
      </div>
      <div style={{
      ...
      {/* Barra de progreso */}
      <div style={{marginTop:52,width:240,height:3,background:"#18253D",borderRadius:4,overflow:"hidden"}}>
        <div style={{height:"100%",width:f>=2?"100%":"0%",background:"linear-gradient(90deg,#2563EB,#06B6D4)",borderRadius:4,transition:f>=2?"width 1.3s cubic-bezier(.4,0,.2,1)":"none"}}/>
      </div>
      <div style={{marginTop:12,fontSize:12,color:"#64748B",opacity:f>=2?1:0,transition:"opacity .35s"}}>Conectando con Supabase...</div>
...
function Donut({pct,color,label,sub}){
  ...
  return(<div style={{display:"flex",alignItems:"center",gap:14}}>
    <svg width={80} height={80} viewBox="0 0 80 80">
      ...
      <text x={40} y={44} textAnchor="middle" fill={darkMode?C.white:"#0F172A"} fontSize={13} fontWeight={700}>{pct}%</text>
    </svg>
    <div><div style={{fontSize:14,fontWeight:700,color:darkMode?C.white:"#0F172A"}}>{label}</div><div style={{fontSize:12,color:C.muted,marginTop:2}}>{sub}</div></div>
  </div>);
}
    ...
    <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:16}}>
      <div>
        ...
        <div style={{display:"flex",alignItems:"center",gap:10,marginTop:-8}}>
          <span style={{fontSize:28,fontWeight:800,color:C.money||C.green}}>{fmt(mesActual)}</span>
          <div style={{display:"flex",alignItems:"center",gap:5,background:(crecPct>=0?C.green:C.red)+"18",border:`1px solid ${(crecPct>=0?C.green:C.red)}44`,borderRadius:8,padding:"4px 10px"}}>
            <span style={{fontSize:13,color:crecPct>=0?C.green:C.red}}>{crecPct>=0?"▲":"▼"}</span>
      ...
      <div style={{display:"flex",gap:4,background:C.surface,borderRadius:10,padding:4}}>
        {["meses","años"].map(v=>(
          <button key={v} onClick={()=>setVista(v)}
            style={{padding:"6px 14px",borderRadius:7,border:"none",background:vista===v?C.accent:"transparent",color:vista===v?"#FFFFFF":C.muted,fontWeight:700,fontSize:12,cursor:"pointer",textTransform:"uppercase",letterSpacing:0.5}}>
...
function Dashboard({clientes,contratos,paneles,setTab}){
  ...
  return(<div>
    ...
    <div style={{marginBottom:24,padding:"20px 24px",background:darkMode?"linear-gradient(135deg,#1E3A5F,#1a3353,#162d47)":"linear-gradient(135deg,#1D4ED8,#2563EB,#1E40AF)",borderRadius:18,border:`1px solid ${darkMode?"#2a4a7a":"#3B82F6"}` ,display:"flex",alignItems:"center",justifyContent:"space-between",overflow:"hidden",position:"relative"}}>
      ...
      <div>
        <div style={{fontSize:13,color:"rgba(255,255,255,0.75)",fontWeight:600,marginBottom:4,letterSpacing:0.5}}>
    ...
    <div style={{display:"grid",gridTemplateColumns:"1fr",gap:14,marginBottom:16}}>
      <Card><SecTit ch="💰 Ingresos del Mes"/>
        <div style={{fontSize:38,fontWeight:800,color:C.money||C.green,letterSpacing:"-1px"}}>{fmt(ingMes)}</div>
        <div style={{fontSize:13,color:C.muted,marginTop:4}}>{pag} de {contratos.length} contratos cobrados</div>
      </Card>
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:14}}>
        <Card style={{display:"flex",alignItems:"center",justifyContent:"center"}}><Donut pct={ocu} color={C.accent} label="Ocupación" sub={`${paneles.filter(p=>p.estado==="Ocupado").length}/${paneles.length} paneles`}/></Card>
        <Card style={{display:"flex",alignItems:"center",justifyContent:"center"}}><Donut pct={cob} color={C.green} label="Cobranza" sub={`${pag}/${contratos.length} contratos`}/></Card>
      </div>
    </div>
      ...
      <Card style={{padding:"16px 18px"}}>
        ...
        {prox.map(c=>(
          <div key={c.id} style={{display:"flex",alignItems:"center",gap:10,padding:"9px 0",borderBottom:`1px solid ${C.border}`}}>
            ...
            <div style={{flex:1,minWidth:0}}>
              <div style={{fontSize:12,fontWeight:700,color:darkMode?C.white:"#0F172A",whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>{c.panel.nombre}</div>
              <div style={{fontSize:11,color:C.muted}}>{c.cliente?.empresa}</div>
            </div>
            <Badge color={c.d<=15?C.red:C.amber} ch={`${c.d}d`}/>
          </div>
        ))}
        <button style={{width:"100%",padding:"8px",background:C.accent,border:"none",borderRadius:9,color:"#FFFFFF",fontWeight:700,fontSize:12,cursor:"pointer",marginTop:12}} onClick={()=>setTab("contratos")}>Ver contratos →</button>
      </Card>

      {/* Métricas del Mes */}
      <Card style={{padding:"16px 18px"}}>
        <SecTit ch="📊 Métricas del Mes"/>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
          {[
            {icon:"🖥️",label:"Paneles",value:paneles.length,color:C.accent},
            {icon:"✅",label:"Activos",value:paneles.filter(p=>p.estado==="Ocupado").length,color:C.green},
            {icon:"📋",label:"Contratos",value:contratos.length,color:C.cyan},
            {icon:"⚠️",label:"Por vencer",value:contratos.filter(c=>dias(c.fin)>0&&dias(c.fin)<=30).length,color:C.amber},
          ].map(m=>(
            <div key={m.label} style={{background:C.surface,border:`1px solid ${C.border}`,borderRadius:10,padding:"12px",display:"flex",flexDirection:"column",gap:6}}>
              <div style={{display:"flex",alignItems:"center",gap:6}}>
                <span style={{fontSize:18}}>{m.icon}</span>
                <span style={{fontSize:11,color:C.muted,fontWeight:700,textTransform:"uppercase"}}>{m.label}</span>
              </div>
              <div style={{fontSize:24,fontWeight:800,color:m.color,letterSpacing:"-1px"}}>{m.value}</div>
            </div>
          ))}
        </div>
      </Card>
...
function Paneles({paneles,setPaneles,loading}){
  ...
  return(<div>
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-end",marginBottom:22}}>
      ...
      <button onClick={openNew} style={{background:C.accent,border:"none",borderRadius:11,padding:"10px 20px",color:"#FFFFFF",fontWeight:700,fontSize:13,cursor:"pointer"}}>＋ Nuevo Panel</button>
    </div>
          ...
          <div style={{position:"relative"}}>
            <div style={{display:"flex",gap:8}}>
              ...
              <button onClick={geocodificar} disabled={geocoding}
                style={{padding:"10px 16px",background:geocoding?C.border:C.accent,border:"none",borderRadius:10,color:"#FFFFFF",fontWeight:700,fontSize:13,cursor:"pointer",whiteSpace:"nowrap",flexShrink:0}}>
                {geocoding?"⏳":"📍 Ubicar"}
              </button>
            </div>
  ...
  return(<div>
    ...
    <Card style={{padding:0,overflow:"hidden",position:"relative",borderRadius:20}}>
      <div style={{position:"absolute",top:16,left:16,zIndex:500,background:"rgba(13,20,33,0.9)",borderRadius:12,padding:"9px 15px",display:"flex",gap:14,backdropFilter:"blur(10px)",border:`1px solid ${C.border}`}}>
        {[[C.green,"Libre"],[C.red,"Ocupado"]].map(([c,l])=>(
          <div key={l} style={{display:"flex",alignItems:"center",gap:7}}>
            ...
            <span style={{fontSize:12,color:"#FFFFFF",fontWeight:600}}>{l}</span>
          </div>
        ))}
      </div>
      <div style={{position:"absolute",top:16,right:16,zIndex:500,background:"rgba(13,20,33,0.9)",borderRadius:12,padding:"9px 15px",backdropFilter:"blur(10px)",border:`1px solid ${C.border}`,fontSize:12,color:C.muted,fontWeight:600}}>
        <span style={{color:"#FFFFFF"}}>{paneles.length} panel{paneles.length!==1?"es":""}</span>
      </div>

      {!leafletReady&&(
        <div style={{height:560,display:"flex",alignItems:"center",justifyContent:"center",color:C.muted,gap:10,background:C.bg}}>
      ...
      {sel&&(()=>{
        ...
        return(
          <div style={{position:"absolute",bottom:0,left:0,right:0,zIndex:500,background:"linear-gradient(160deg,#0D1F3C 0%,#0A1628 60%,#071020 100%)",backdropFilter:"blur(20px)",borderTop:`2px solid ${color}`,padding:"20px 22px 24px",borderRadius:"0 0 20px 20px"}}>
            {/* Header */}
            <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:16}}>
              <div style={{display:"flex",alignItems:"center",gap:12}}>
                <div style={{width:52,height:52,borderRadius:14,background:color+"22",border:`2px solid ${color}66`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:26,flexShrink:0}}>
                  {p.foto}
                </div>
                <div>
                  <div style={{fontSize:17,fontWeight:800,color:"#FFFFFF",letterSpacing:"-0.3px"}}>{p.nombre}</div>
                  {p.direccion&&<div style={{fontSize:12,color:"#CBD5E1",marginTop:3}}>📍 {p.direccion}</div>}
                  <div style={{display:"flex",gap:6,marginTop:7}}>
                    <span style={{background:color+"28",color,border:`1px solid ${color}55`,borderRadius:6,padding:"2px 9px",fontSize:11,fontWeight:700}}>
                      {p.estado==="Ocupado"?"🔴 Ocupado":"🟢 Libre"}
                    </span>
                    <span style={{background:"#2563EB22",color:"#60A5FA",border:"1px solid #2563EB44",borderRadius:6,padding:"2px 9px",fontSize:11,fontWeight:700}}>{p.tipo}</span>
                    {p.ciudad&&<span style={{background:"#8B5CF622",color:"#A78BFA",border:"1px solid #8B5CF644",borderRadius:6,padding:"2px 9px",fontSize:11,fontWeight:700}}>{p.ciudad}</span>}
                  </div>
                </div>
              </div>
              <button onClick={()=>setSel(null)} style={{width:32,height:32,borderRadius:"50%",background:"#1E3A5F",border:"1px solid #2D4E7A",color:"#94A3B8",cursor:"pointer",fontSize:16,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}>✕</button>
            </div>
              ...
              <div>
                <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:10,marginBottom:12}}>
                  {[
                    ...
                    ["🏢 Cliente",contrato.cliente?.empresa||"—","#FFFFFF"],
                    ["📅 Vence",diasVence>0?`en ${diasVence}d`:`hace ${Math.abs(diasVence)}d`,diasVence>15?C.green:diasVence>0?C.amber:C.red],
                  ].map(([l,v,c])=>(
                    <div key={l} style={{background:"#FFFFFF",borderRadius:12,padding:"12px 14px",textAlign:"center",boxShadow:"0 2px 8px rgba(0,0,0,0.15)"}}>
                      <div style={{fontSize:10,color:"#64748B",fontWeight:700,marginBottom:5,textTransform:"uppercase",letterSpacing:0.8}}>{l}</div>
                      <div style={{fontSize:13,fontWeight:800,color:l==="🏢 Cliente"?"#1E293B":c,lineHeight:1.2}}>{v}</div>
                    </div>
                  ))}
                </div>
                <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
                  <div style={{background:"#FFFFFF",borderRadius:12,padding:"11px 14px",boxShadow:"0 2px 8px rgba(0,0,0,0.15)"}}>
                    <div style={{fontSize:10,color:"#64748B",fontWeight:700,marginBottom:3,textTransform:"uppercase",letterSpacing:0.8}}>📆 Período</div>
                    <div style={{fontSize:12,fontWeight:700,color:"#1E293B"}}>{fmtF(contrato.inicio)} → {fmtF(contrato.fin)}</div>
                  </div>
                  <div style={{background:"#FFFFFF",borderRadius:12,padding:"11px 14px",boxShadow:"0 2px 8px rgba(0,0,0,0.15)"}}>
                    <div style={{fontSize:10,color:"#64748B",fontWeight:700,marginBottom:3,textTransform:"uppercase",letterSpacing:0.8}}>💳 Pago</div>
                    <div style={{fontSize:12,fontWeight:700,color:contrato.pagado?"#10B981":"#F59E0B"}}>{contrato.pagado?"✅ Cobrado":"⏳ Pendiente"}</div>
                  </div>
                </div>
              </div>
            ):(
              <div style={{background:"#10B98112",border:"1px solid #10B98133",borderRadius:14,padding:"16px 18px",display:"flex",alignItems:"center",gap:14}}>
                <div style={{fontSize:32}}>✅</div>
                <div>
                  <div style={{fontSize:14,fontWeight:800,color:C.green}}>Panel disponible</div>
                  <div style={{fontSize:13,color:"#CBD5E1",marginTop:3}}>Precio de lista: <strong style={{color:"#FFFFFF"}}>{fmt(p.precio)}/mes</strong></div>
                  {p.precio&&<div style={{fontSize:12,color:"#94A3B8",marginTop:2}}>Anual estimado: <strong style={{color:C.cyan}}>{fmt(p.precio*12)}</strong></div>}
                </div>
              </div>
            )}
          </div>
...
// ── CONTRATOS ────────────────────────────────────────────────────
// ── Contratos form helpers — defined OUTSIDE to prevent focus loss on re-render ──
const ContratosInp={width:"100%",background:C.surface,border:`1px solid ${C.border}`,borderRadius:10,padding:"10px 13px",color:C.text,fontSize:16,outline:"none",fontFamily:"inherit",boxSizing:"border-box",minHeight:44,WebkitAppearance:"none",appearance:"none",MozAppearance:"none"};
const ContratosSel={...ContratosInp,cursor:"pointer"};
function ContratosFieldLabel({label,children}){
  return(
    <div style={{marginBottom:16}}>
...
function Contratos({contratos,setContratos,paneles,clientes,loading}){
  ...
  const mesesForm=useMemo(()=>generarMeses(form.inicio,form.fin),[form.inicio,form.fin]);
  ...
  else if(filtro==="Por vencer") fil=datos.filter(c=>c.d>0&&c.d<=30).sort((a,b)=>a.d-b.d);
  ...
  // F, inp, sel moved outside component to prevent re-mount on every keystroke

  const pagosMarcados=Object.values(form.pagosMeses||{}).filter(Boolean).length;

  return(<div>
    {modal&&(
      <div style={{position:"fixed",inset:0,background:"#00000092",zIndex:400,display:"flex",alignItems:"center",justifyContent:"center",padding:20,backdropFilter:"blur(6px)"}} onMouseDown={e=>{ if(e.target===e.currentTarget) setModal(null); }}>
        <div style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:20,padding:26,width:"100%",maxWidth:540,maxHeight:"92vh",overflowY:"auto",WebkitOverflowScrolling:"touch"}} onClick={e=>e.stopPropagation()}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:20}}>
          ...
          <ContratosFieldLabel label="Panel *">
            <select value={form.panel_id} onChange={e=>setForm(f=>({...f,panel_id:e.target.value}))} style={ContratosSel}>
              <option value="">— Selecciona un panel —</option>
              {paneles.map(p=><option key={p.id} value={p.id}>{p.foto} {p.nombre} · {p.ciudad} · {p.estado}</option>)}
            </select>
          </ContratosFieldLabel>

          <ContratosFieldLabel label="Cliente *">
            <select value={form.cliente_id} onChange={e=>setForm(f=>({...f,cliente_id:e.target.value}))} style={ContratosSel}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
            <ContratosFieldLabel label="Fecha inicio *">
              <input type="date" value={form.inicio}
                onChange={e=>setForm(f=>({...f,inicio:e.target.value,pagosMeses:{}}))}
                style={{...ContratosInp,colorScheme:"dark",cursor:"pointer",fontSize:16,minHeight:44}}/>
            </ContratosFieldLabel>
            <ContratosFieldLabel label="Fecha fin *">
          ...
          <ContratosFieldLabel label="Monto mensual ($) *">
            <input
              type="text"
              inputMode="decimal"
              pattern="[0-9]*"
              ...
              placeholder="Ej: 1500"
              autoComplete="off"
              style={{...ContratosInp,fontSize:16,minHeight:44}}
            />
          </ContratosFieldLabel>

          {/* PAGOS POR MES */}
          {mesesForm.length>0&&(
            <ContratosFieldLabel label={`Pagos por mes (${pagosMarcados}/${mesesForm.length} pagados)`}>
    ...
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-end",marginBottom:18,flexWrap:"wrap",gap:10}}>
      ...
      <button onClick={openNew} style={{background:C.accent,border:"none",borderRadius:11,padding:"10px 20px",color:"#FFFFFF",fontWeight:700,fontSize:13,cursor:"pointer"}}>＋ Nuevo Contrato</button>
    </div>
    ...
    <div style={{display:"flex",flexDirection:"column",gap:12}}>
      {fil.map(c=>{
        ...
        const pct=meses.length>0?Math.round((pagados/meses.length)*100):0;
        const statusColor=c.d<=0?C.muted:c.d<=15?C.red:c.d<=30?C.amber:C.green;
        const statusLabel=c.d<=0?`Venció hace ${Math.abs(c.d)}d`:c.d<=15?`Vence en ${c.d}d ⚠️`:c.d<=30?`Vence en ${c.d}d`:null;
        const totalCobrado=pagados*Number(c.monto);
        const totalContrato=meses.length*Number(c.monto);
        return(
          <div key={c.id} style={{
            background:C.card,
            ...
            <div style={{height:5,background:`linear-gradient(90deg,${c.d<=0?C.muted:c.d<=15?C.red:c.d<=30?C.amber:C.green},${(c.d<=0?C.muted:c.d<=15?C.red:c.d<=30?C.amber:C.green)}88)`}}/>
            ...
            <div style={{padding:"18px 18px 0 18px"}}>
              ...
              <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:14}}>
                <div style={{display:"flex",alignItems:"center",gap:12}}>
                  ...
                  <div>
                    <div style={{fontSize:15,fontWeight:800,color:C.white,lineHeight:1.2,marginBottom:3}}>{c.panel.nombre}</div>
                    <div style={{display:"flex",alignItems:"center",gap:6}}>
                      {statusLabel&&<span style={{fontSize:11,background:statusColor+"22",color:statusColor,borderRadius:6,padding:"2px 8px",fontWeight:700}}>{statusLabel}</span>}
                      <span style={{fontSize:11,color:C.muted}}>{c.panel.tipo||""}</span>
                    </div>
                  </div>
                </div>
                <div style={{textAlign:"right",flexShrink:0}}>
                  <div style={{fontSize:22,fontWeight:800,color:C.green,letterSpacing:"-0.5px",lineHeight:1}}>{fmt(c.monto)}</div>
                  <div style={{fontSize:11,color:C.muted,marginTop:2}}>por mes</div>
                </div>
              </div>
              ...
              <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:14}}>
                <div style={{
                  ...
                  display:"flex",alignItems:"center",justifyContent:"center",
                  fontWeight:900,fontSize:12,flexShrink:0,letterSpacing:"-0.5px"
                }}>
                  {c.cliente.empresa.split(" ").slice(0,2).map(w=>w[0]||"").join("").toUpperCase().slice(0,2)}
                </div>
                <div style={{flex:1,minWidth:0}}>
                  <div style={{fontSize:13,fontWeight:700,color:C.white,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{c.cliente.empresa}</div>
                  <div style={{fontSize:12,color:C.muted}}>{c.cliente.contacto}</div>
                </div>
                <div style={{textAlign:"right",flexShrink:0}}>
                  <div style={{fontSize:11,color:C.muted,marginBottom:2}}>Período</div>
                  <div style={{fontSize:11,fontWeight:700,color:C.text}}>{fmtF(c.inicio)} → {fmtF(c.fin)}</div>
                </div>
              </div>

              {/* ── FILA 3: Barra de cobro ── */}
              <div style={{marginBottom:14}}>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:6}}>
                  <span style={{fontSize:11,fontWeight:700,color:C.muted,textTransform:"uppercase",letterSpacing:1}}>Progreso de cobro</span>
                  <span style={{fontSize:12,fontWeight:800,color:pct===100?C.green:pct>0?C.amber:C.red}}>{pagados}/{meses.length} meses · {pct}%</span>
                </div>
                <div style={{height:8,background:C.surface,borderRadius:6,overflow:"hidden"}}>
                  <div style={{
                    height:"100%",
                    width:`${pct}%`,
                    background:pct===100?C.green:pct>0?C.amber:C.red,
                    borderRadius:6,
                    transition:"width .4s ease"
                  }}/>
                </div>
                <div style={{display:"flex",justifyContent:"space-between",marginTop:6}}>
                  <span style={{fontSize:11,color:C.green,fontWeight:700}}>✓ {fmt(totalCobrado)} cobrado</span>
                  <span style={{fontSize:11,color:C.red,fontWeight:700}}>{fmt(totalContrato-totalCobrado)} pendiente</span>
                </div>
              </div>

              {/* ── FILA 4: Botones de meses ── */}
              {meses.length>0&&(
                <div style={{marginBottom:14}}>
                  <div style={{fontSize:11,fontWeight:700,color:C.muted,textTransform:"uppercase",letterSpacing:1,marginBottom:8}}>Toca para marcar pago</div>
                    ...
                    {meses.map((m)=>{
                      ...
                      return(
                        <button key={m.key} onClick={()=>togglePagoRapido(c,m.key)} style={{
                          ...
                          border:`1.5px solid ${pag?C.green:C.border}`,
                          background:pag?C.green+"22":C.surface,
                          cursor:"pointer",fontSize:12,fontWeight:700,
                          color:pag?C.green:C.muted,
                          display:"flex",alignItems:"center",gap:5,
                          transition:"all .15s",minHeight:36
                        }}>
                          {pag
                            ?<span style={{fontSize:14}}>✅</span>
                            :<span style={{width:14,height:14,borderRadius:"50%",border:`1.5px solid ${C.muted}`,display:"inline-block"}}/>
                          }
                          <span>{m.label}</span>
                        </button>
                      );
            ...
            <div style={{
              ...
              <a href={`https://wa.me/${c.cliente.celular?.replace(/\D/g,"")}?text=${msg}`}
...
function CRM({clientes,setClientes,contratos,loading}){
  ...
  return(<div>
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-end",marginBottom:22}}>
      ...
      <button onClick={openNew} style={{background:C.accent,border:"none",borderRadius:11,padding:"10px 20px",color:"#FFFFFF",fontWeight:700,fontSize:13,cursor:"pointer"}}>＋ Nuevo contacto</button>
    </div>
  ...
  return(<div>
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-end",marginBottom:22}}>
      ...
      <div style={{display:"flex",gap:10,alignItems:"center",flexWrap:"wrap"}}>
        ...
        }} style={{background:C.purple,border:"none",borderRadius:11,padding:"10px 16px",color:"#FFFFFF",fontWeight:700,fontSize:13,cursor:"pointer",display:"flex",alignItems:"center",gap:6}}>
          📄 Exportar
        </button>
        {!vistaHistorica&&<button onClick={openNew} style={{background:C.accent,border:"none",borderRadius:11,padding:"10px 18px",color:"#FFFFFF",fontWeight:700,fontSize:13,cursor:"pointer"}}>＋ Gasto</button>}
      </div>
    </div>
      ...
      <div style={{background:C.surface,border:`1px solid ${C.border}`,borderRadius:16,padding:"14px 16px",marginBottom:12}}>
        ...
        <div style={{display:"flex",gap:8,alignItems:"center"}}>
          ...
          <button onPointerDown={()=>set(v=>Math.min(max,v+step))}
            style={{flex:1,height:44,borderRadius:12,border:"none",background:C.accent,color:"#FFFFFF",fontSize:22,fontWeight:700,cursor:"pointer",userSelect:"none",WebkitUserSelect:"none",display:"flex",alignItems:"center",justifyContent:"center"}}>
            +
          </button>
        </div>
      </div>
...
export default function App(){
  ...
  return(<>
    ...
    <style>{`*{box-sizing:border-box}html,body{margin:0;padding:0;background:${darkMode?C.bg:"#EEF2F7"};color:${C.text};-webkit-text-size-adjust:100%;overscroll-behavior:none;}body{padding-top:env(safe-area-inset-top);padding-left:env(safe-area-inset-left);padding-right:env(safe-area-inset-right);}::-webkit-scrollbar{width:5px;height:5px}::-webkit-scrollbar-track{background:${C.bg}}::-webkit-scrollbar-thumb{background:${C.border};border-radius:3px}@keyframes spin{to{transform:rotate(360deg)}}@keyframes pulse{from{opacity:.2}to{opacity:.6}}`}</style>
    ...
    <div style={{fontFamily:"'DM Sans',sans-serif",background:darkMode?C.bg:"linear-gradient(180deg,#E8EEF5 0%,#EEF2F7 100%)",minHeight:"100vh",color:C.text,display:"flex",flexDirection:"column",transition:"background .3s,color .3s"}}>
      {/* NAV — espacio safe-area arriba, logo + tabs en misma fila abajo */}
      <nav style={{background:"#0A0F1E",borderBottom:"1px solid #1E2D45",position:"fixed",top:0,left:0,right:0,zIndex:200}}>
        <style>{`::-webkit-scrollbar{display:none}`}</style>
        {/* Espacio para hora/batería iOS */}
        <div style={{height:"env(safe-area-inset-top)",minHeight:0,background:"#0A0F1E"}}/>
        {/* Fila única: Logo + Tabs scrollables + ⚙️ */}
        <div style={{display:"flex",alignItems:"center",height:52,paddingLeft:12,paddingRight:8}}>
          <div style={{flexShrink:0,marginRight:6,filter:"brightness(0) invert(1)"}}><Logo360 width={100}/></div>
          <div style={{display:"flex",flex:1,overflowX:"auto",WebkitOverflowScrolling:"touch",scrollbarWidth:"none",height:"100%",alignItems:"center"}}>
            {tabs.map(t=>{
              ...
              return(
              <button key={t.id} onClick={()=>setTab(t.id)} style={{padding:"0 11px",height:"100%",display:"flex",alignItems:"center",gap:5,fontSize:12,fontWeight:tab===t.id?700:400,color:tab===t.id?"#FFFFFF":"#94A3B8",background:"none",border:"none",borderBottom:tab===t.id?"2px solid #3B82F6":"2px solid transparent",cursor:"pointer",whiteSpace:"nowrap",flexShrink:0,position:"relative"}}>
                <span>{t.icon}</span><span>{t.label}</span>
                {alertCount>0&&<span style={{background:C.red,color:"#fff",borderRadius:"50%",width:15,height:15,fontSize:9,fontWeight:800,display:"flex",alignItems:"center",justifyContent:"center",position:"absolute",top:8,right:2}}>{alertCount}</span>}
              </button>
            );})}
          </div>
          <div style={{flexShrink:0,display:"flex",alignItems:"center",gap:4,marginLeft:4}}>
            <div style={{width:6,height:6,borderRadius:"50%",background:error?C.red:loading?C.amber:C.green}}/>
            <button onClick={()=>setShowConfig(true)} style={{background:"transparent",border:"none",color:"#94A3B8",cursor:"pointer",fontSize:13,padding:4}}>⚙️</button>
          </div>
        </div>
      </nav>
      {/* Contenido — padding-top = safe-area + 52px nav + 16px espacio */}
      <div style={{paddingTop:"calc(env(safe-area-inset-top) + 60px)",paddingLeft:16,paddingRight:16,paddingBottom:24,maxWidth:1240,width:"100%",margin:"0 auto",boxSizing:"border-box",flex:1}}>
        {error&&(
              ...
              <div style={{marginTop:12,display:"flex",flexDirection:"column",gap:6}}>
                {[["1","Ve a","vercel.com","y crea una cuenta gratis"],["2","Sube el archivo .jsx a un proyecto Next.js o Vite"],["3","Tu app estará en línea en menos de 5 min, conectada a Supabase"]].map(([n,a,b,c])=>(
                  <div key={n} style={{display:"flex",gap:10,alignItems:"center",fontSize:13}}>
                    <div style={{width:22,height:22,borderRadius:"50%",background:C.accent,color:"#FFFFFF",display:"flex",alignItems:"center",justifyContent:"center",fontWeight:700,fontSize:11,flexShrink:0}}>{n}</div>
