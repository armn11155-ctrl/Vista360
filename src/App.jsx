...
const sb = {
  ...
  async get(table, qs = "")    { const r = await fetch(sb.url(table, qs), { headers: { ...sb.headers, Prefer: "return=representation" } }); return r.json(); },
  ...
  async patch(table, id, body) { const r = await fetch(sb.url(table, `?id=eq.${id}`), { method: "PATCH",  headers: { ...sb.headers, Prefer: "return=representation" }, body: JSON.stringify(body) }); return r.json(); },
...
function GraficaCrecimiento({contratos}){
  const [vista,setVista]=useState("meses"); // "meses" | "años"

  // Calcular datos por mes desde contratos reales
  const datosMeses=useMemo(()=>{
    ...
    // Generar últimos 12 meses siempre, aunque sean 0
    const res=[];
    for(let i=11;i>=0;i--){
      const d=new Date();
      d.setDate(1);
      ...
      res.push({key,label,v:mapa[key]||0,mes:d.getMonth(),anio:d.getFullYear()});
    }
    return res;
  },[contratos]);
  ...
  const datosAnios=useMemo(()=>{
    ...
    for(let a=anioActual-3;a<=anioActual;a++){
      const key=String(a);
      res.push({key,label:key,v:mapa[key]||0});
    }
    return res;
  },[contratos]);
  ...
  const mejorMes=Math.max(...datosMeses.map(d=>d.v));
  const promMes=Math.round(datosMeses.filter(d=>d.v>0).reduce((a,d)=>a+d.v,0)/Math.max(datosMeses.filter(d=>d.v>0).length,1));

  // Proyección: promedio de últimos 3 meses
  const ult3=datosMeses.slice(-3).map(d=>d.v);
  const promUlt3=Math.round(ult3.reduce((a,v)=>a+v,0)/3);
  const mesActualNum=new Date().getMonth();
  const mesesRestantes=11-mesActualNum;
  const proyeccionAnio=mesActual+promUlt3*mesesRestantes;
  ...
  // Proyección (solo en vista meses) — últimos 2 puntos + 2 futuros
  const proyPts=vista==="meses"&&pts.length>=2?[
    pts[pts.length-2],
    pts[pts.length-1],
    {x:pts[pts.length-1].x+(W-PAD*2)/(data.length-1)*0.7, y:H-PAD-(Math.min(promUlt3*1.05,maxV)/maxV)*(H-PAD*2)},
  ]:[];
  const proyD=proyPts.map((p,i)=>i===0?`M${p.x},${p.y}`:`L${p.x},${p.y}`).join(" ");
  ...
  return(<div>
    ...
    <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:14}}>
      <SecTit ch="📈 Crecimiento"/>
      <div style={{display:"flex",gap:4,background:C.surface,borderRadius:8,padding:3}}>
        {["meses","años"].map(v=>(
          <button key={v} onClick={()=>setVista(v)}
            style={{padding:"5px 12px",borderRadius:6,border:"none",background:vista===v?C.accent:"transparent",color:vista===v?C.white:C.muted,fontWeight:700,fontSize:11,cursor:"pointer",textTransform:"uppercase",letterSpacing:0.5}}>
    ...
    {vista==="meses"&&(
      <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:12}}>
        <div style={{fontSize:22,fontWeight:800,color:C.white}}>{fmt(mesActual)}</div>
        <div style={{display:"flex",alignItems:"center",gap:4,background:(crecPct>=0?C.green:C.red)+"18",border:`1px solid ${(crecPct>=0?C.green:C.red)}44`,borderRadius:8,padding:"3px 10px"}}>
          <span style={{fontSize:14}}>{crecPct>=0?"▲":"▼"}</span>
          <span style={{fontSize:13,fontWeight:700,color:crecPct>=0?C.green:C.red}}>{Math.abs(crecPct)}% vs mes anterior</span>
        </div>
      </div>
    )}
    ...
    <div style={{position:"relative",marginBottom:8}}>
      <svg width="100%" viewBox={`0 0 ${W} ${H}`} style={{overflow:"visible"}}>
        ...
        {/* Línea principal */}
        {pts.length>1&&<path d={pathD} stroke={C.accent} strokeWidth="2" fill="none" strokeLinecap="round" strokeLinejoin="round"/>}
        {/* Proyección punteada */}
        {proyPts.length>1&&<path d={proyD} stroke={C.cyan} strokeWidth="1.5" fill="none" strokeDasharray="4,3" strokeLinecap="round"/>}
        {/* Puntos interactivos */}
        {pts.map((p,i)=>(
          <g key={i} onMouseEnter={()=>setTooltip(p)} onMouseLeave={()=>setTooltip(null)} style={{cursor:"pointer"}}>
            <circle cx={p.x} cy={p.y} r={8} fill="transparent"/>
            <circle cx={p.x} cy={p.y} r={p.v>0?3:2} fill={p.v>0?C.accent:C.border} stroke={C.bg} strokeWidth="1.5"/>
          </g>
        ))}
        {/* Tooltip */}
        {tooltip&&(
          <g>
            <rect x={Math.min(tooltip.x-28,W-70)} y={tooltip.y-32} width={65} height={24} rx={5} fill={C.surface} stroke={C.border} strokeWidth="1"/>
            <text x={Math.min(tooltip.x-28,W-70)+32} y={tooltip.y-16} textAnchor="middle" fill={C.white} fontSize={10} fontWeight="700">{fmt(tooltip.v)}</text>
          </g>
        )}
      </svg>
    </div>

    {/* Labels del eje X — solo algunos para no amontonar */}
    <div style={{display:"flex",justifyContent:"space-between",marginBottom:14,paddingLeft:PAD,paddingRight:PAD}}>
      {data.filter((_,i)=>data.length<=6||i%Math.ceil(data.length/6)===0||i===data.length-1).map(d=>(
    ...
    {vista==="meses"&&promUlt3>0&&(
      <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:14,padding:"8px 12px",background:C.cyan+"10",border:`1px solid ${C.cyan}30`,borderRadius:10}}>
        <div style={{width:20,height:2,background:C.cyan,borderRadius:1,flexShrink:0,borderTop:"1px dashed "+C.cyan}}/>
        <span style={{fontSize:12,color:C.cyan,fontWeight:600}}>Proyección fin de año: <strong>{fmt(proyeccionAnio)}</strong></span>
      </div>
    )}

    {/* Métricas históricas */}
    <div style={{borderTop:`1px solid ${C.border}`,paddingTop:14,display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:8}}>
      {[
        ["Total histórico",fmt(totalHist),C.white],
        ["Mejor mes",fmt(mejorMes),C.green],
        ["Promedio/mes",fmt(promMes),C.cyan],
      ].map(([l,v,c])=>(
        <div key={l} style={{textAlign:"center"}}>
          <div style={{fontSize:13,fontWeight:800,color:c}}>{v}</div>
          <div style={{fontSize:10,color:C.muted,marginTop:2,textTransform:"uppercase",letterSpacing:0.5}}>{l}</div>
        </div>
      ))}
    </div>
  </div>);
  ...
  return(<div>
    ...
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16}}>
      <Card>
        ...
        {prox.length===0&&<div style={{color:C.muted,fontSize:13,padding:"12px 0"}}>Sin contratos próximos a vencer</div>}
        {prox.map(c=>(
          <div key={c.id} style={{display:"flex",alignItems:"center",gap:12,padding:"12px 0",borderBottom:`1px solid ${C.border}`}}>
            <div style={{width:40,height:40,borderRadius:10,background:(c.d<=15?C.red:C.amber)+"22",display:"flex",alignItems:"center",justifyContent:"center",fontSize:18,flexShrink:0}}>{c.panel.foto}</div>
            <div style={{flex:1,minWidth:0}}>
              <div style={{fontSize:13,fontWeight:700,color:C.white,whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>{c.panel.nombre}</div>
              <div style={{fontSize:12,color:C.muted}}>{c.cliente?.empresa}</div>
            </div>
            <Badge color={c.d<=15?C.red:C.amber} ch={c.d<=15?`🔴 ${c.d}d`:`🟡 ${c.d}d`}/>
          </div>
        ))}
        <button style={{width:"100%",padding:10,background:C.accent,border:"none",borderRadius:10,color:C.white,fontWeight:700,fontSize:13,cursor:"pointer",marginTop:14}} onClick={()=>setTab("contratos")}>Ver todos los contratos →</button>
      </Card>
      <Card>
...
function MiniMapaPanel({lat,lng,nombre,foto,onMove}){
  ...
  const reverseGeocode=async(la,ln)=>{
    ...
    try{
      const r=await fetch(`https://nominatim.openstreetmap.org/reverse?lat=${la}&lon=${ln}&format=json&accept-language=es`,{headers:{"User-Agent":"Vista360/1.0"}});
      const d=await r.json();
      const dir=d.display_name||`${la.toFixed(5)}, ${ln.toFixed(5)}`;
      // Simplificamos la dirección: solo calle + ciudad
      const road=d.address?.road||d.address?.pedestrian||"";
      const city=d.address?.city||d.address?.town||d.address?.state||"";
      const short=road&&city?`${road}, ${city}`:dir;
      onMove(la,ln,short);
    }catch{
      onMove(la,ln,"");
    }finally{
  ...
  const activarNotif=async()=>{
    ...
    if(p==="granted"){
      ...
      new Notification("✅ Vista360 — Alertas activadas",{
        body:"Te avisaremos 15 días y 5 días antes de cada vencimiento",
        tag:"bienvenida"
      });
    ...
    <div style={{display:"flex",alignItems:"center",gap:10}}>
      {notifPerm==="granted"
        ? <div style={{display:"flex",alignItems:"center",gap:6,background:C.green+"18",border:`1px solid ${C.green}44`,borderRadius:10,padding:"8px 14px"}}>
            ...
            <span style={{fontSize:12,fontWeight:700,color:C.green}}>🔔 Notificaciones activas · 15d y 5d antes</span>
          </div>
        : notifPerm==="denied"
...
export default function App(){
  ...
  useEffect(() => {
    ...
    contratos.forEach(c => {
      const d = Math.ceil((new Date(c.fin) - hoyD) / 86400000);
      // Disparar a los 15 días y a los 5 días
      [15, 5].forEach(umbral => {
        if (d > 0 && d <= umbral) {
