...
export default function App() {
  ...
  // 100dvh en Safari iOS usa el "large viewport" (sin chrome visible),
  // pero al soltar el scroll el chrome reaparece y el layout rebota.
  // visualViewport.height siempre refleja la altura visible real.
  useEffect(() => {
    const setVH = () => {
    ...
    if (vvp) {
      vvp.addEventListener("resize", setVH);
      vvp.addEventListener("scroll", setVH);
    } else {
      window.addEventListener("resize", setVH);
    }
    return () => {
      if (vvp) {
        vvp.removeEventListener("resize", setVH);
        vvp.removeEventListener("scroll", setVH);
      } else {
