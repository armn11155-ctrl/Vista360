...
function Gastos({gastos,setGastos}){
  ...
  const extraerDatos=(text)=>{
    ...
    const rucM=text.match(/\b(\d{11})\b/);
    const ruc=rucM?rucM[1]:"";
    const montosArr=[...text.matchAll(/(?:S\/\.?|\$|TOTAL[\:\s]+)\s*([\d,\.]+)/gi)].map(m=>parseFloat(m[1].replace(",",".")));
    const monto=montosArr.length>0?Math.max(...montosArr.filter(n=>!isNaN(n))):"";
    const igvM=text.match(/(?:IGV|I\.G\.V)[\:\s]*([\d,\.]+)/i);
    const igv=igvM?parseFloat(igvM[1].replace(",",".")):monto?Math.round(Number(monto)/1.18*0.18*100)/100:"";
    const subtotal=monto&&igv?Math.round((Number(monto)-Number(igv))*100)/100:"";
    const proveedor=lines.find(l=>l.length>4&&/[a-zA-Z]/.test(l))||"";
    const fechaM=text.match(/(\d{1,2})[\-\/](\d{1,2})[\-\/](\d{2,4})/);
    let fecha="";
    if(fechaM){const [,d,mo,y]=fechaM;const yf=y.length===2?"20"+y:y;fecha=`${yf}-${mo.padStart(2,"0")}-${d.padStart(2,"0")}`;}
    const concM=lines.find(l=>/concepto|descripcion|detalle/i.test(l));
    const concepto=concM?concM.replace(/concepto[\:\s]*/i,"").trim():"";
    setForm(f=>({...f,proveedor:proveedor||f.proveedor,ruc:ruc||f.ruc,monto:monto||f.monto,igv:igv||f.igv,subtotal:subtotal||f.subtotal,fecha:fecha||f.fecha,concepto:concepto||f.concepto,foto_texto:text}));
  };

  const guardar=async()=>{
    if(!form.monto) return alert("El monto es obligatorio");
    setSaving(true);
    const payload={fecha:form.fecha||new Date().toISOString().split("T")[0],proveedor:form.proveedor,ruc:form.ruc,concepto:form.concepto,monto:Number(form.monto)||0,igv:Number(form.igv)||0,subtotal:Number(form.subtotal)||0,categoria:form.categoria,notas:form.notas,foto_texto:form.foto_texto};
    try{
