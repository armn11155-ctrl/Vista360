...
function CapModal({ open, onClose, title, subtitle, children }) {
  if (!open) return null;
  return (
    <div onClick={e=>e.target===e.currentTarget&&onClose()} style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.7)",zIndex:700,display:"flex",alignItems:"flex-end",justifyContent:"center"}}>
...
function DarkInput({label,value,onChange,type="text",placeholder,prefix,suffix}) {
  return (
    <div style={{marginBottom:14}}>
      ...
      <div style={{display:"flex",alignItems:"center",background:"rgba(255,255,255,0.05)",border:"1px solid rgba(255,255,255,0.1)",borderRadius:12,padding:"0 14px",gap:8}}>
...
function DarkSelect({label,value,onChange,options}) {
  return (
    <div style={{marginBottom:14}}>
      {label&&<div style={{fontSize:11,fontWeight:700,color:"rgba(148,175,255,0.5)",letterSpacing:1.2,textTransform:"uppercase",marginBottom:6}}>{label}</div>}
      <select value={value} onChange={onChange} style={{width:"100%",background:"rgba(255,255,255,0.05)",border:"1px solid rgba(255,255,255,0.1)",borderRadius:12,padding:"13px 14px",color:"#fff",fontSize:14,fontFamily:"inherit",outline:"none",appearance:"none"}}>
...
function CapCard({children, accent, style={}}) {
  return (
    <div style={{
      background:"linear-gradient(145deg,#111D3C 0%,#0D1629 100%)",
      border:`1px solid ${accent?`${accent}22`:"rgba(99,140,255,0.09)"}`,
      borderRadius:22,
      boxShadow:`0 4px 24px rgba(0,0,0,0.35)${accent?`, 0 0 0 1px ${accent}11`:""}`,
      overflow:"hidden",
      ...style,
    }}>{children}</div>
  );
...
function Capital({ paneles, contratos, gastos, proveedores }) {
  ...
  if (totalLiquidez > gastosMes*3) insights.push({icon:"💧",text:"Liquidez saludable — más de 3 meses cubiertos",color:"#60A5FA"});
  if (mesesProyeccion) insights.push({icon:"🎯",text:`Podrías cumplir "${objPendiente.nombre}" en ~${mesesProyeccion} mes${mesesProyeccion!==1?"es":""}`,color:"#A78BFA"});
  ...
  if (!loaded) return (
    <div style={{display:"flex",alignItems:"center",justifyContent:"center",height:300,color:"rgba(148,175,255,0.4)",fontSize:14}}>Cargando...</div>
  );
  ...
  const renderPatrimonio = () => (
    <div>
      {/* Hero patrimonio */}
      <CapCard accent="#6366F1" style={{padding:"26px 22px 22px",marginBottom:12}}>
        <div style={{fontSize:10,fontWeight:700,color:"rgba(148,175,255,0.45)",letterSpacing:2.5,textTransform:"uppercase",marginBottom:8}}>Patrimonio neto total</div>
        <div style={{fontSize:48,fontWeight:900,color:"#fff",letterSpacing:"-2px",lineHeight:1,marginBottom:8}}>
          {fmtS2(patrimonio)}
        </div>
        <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:20}}>
          <span style={{fontSize:12,fontWeight:700,color:patrimonioChange>=0?"#34D399":"#F87171",background:patrimonioChange>=0?"rgba(52,211,153,0.12)":"rgba(248,113,113,0.12)",borderRadius:99,padding:"3px 10px"}}>
            {patrimonioChange>=0?"+":""}{patrimonioChange}% este mes
          </span>
        </div>
        <Sparkline data={spark6} color="#6366F1" height={44}/>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:8,marginTop:16}}>
          {[
            {label:"Liquidez",value:totalLiquidez,color:CAP_COLORS.liquidez},
            {label:"Activos", value:totalActivos,  color:CAP_COLORS.activos},
            {label:"Deudas",  value:totalDeudas,   color:CAP_COLORS.deudas},
            ...
            <div key={label} style={{background:"rgba(255,255,255,0.05)",borderRadius:12,padding:"10px 10px"}}>
              ...
              <div style={{fontSize:11,color:"rgba(148,175,255,0.4)",marginBottom:3}}>{label}</div>
      ...
      <CapCard style={{padding:"18px 20px",marginBottom:12}}>
        <div style={{fontSize:10,fontWeight:700,color:"rgba(148,175,255,0.4)",letterSpacing:2,textTransform:"uppercase",marginBottom:14}}>Flujo mensual</div>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
          {[
            {label:"Ingresos",   value:ingMes,    color:"#34D399", icon:"↑"},
            {label:"Gastos",     value:gastosMes, color:"#F87171", icon:"↓"},
            {label:"Por cobrar", value:porCobrar, color:"#FCD34D", icon:"⏳"},
            {label:"Neto libre", value:Math.max(ingresoMensualNeto,0), color:"#60A5FA", icon:"✓"},
          ].map(({label,value,color,icon})=>(
            <div key={label} style={{background:"rgba(255,255,255,0.04)",border:"1px solid rgba(255,255,255,0.06)",borderRadius:14,padding:"13px 14px"}}>
              <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:6}}>
                ...
                <span style={{fontSize:10,color:"rgba(148,175,255,0.45)",fontWeight:600,textTransform:"uppercase",letterSpacing:0.8}}>{label}</span>
              </div>
      ...
      {insights.length>0&&(
        <CapCard style={{padding:"18px 20px"}}>
          <div style={{fontSize:10,fontWeight:700,color:"rgba(148,175,255,0.4)",letterSpacing:2,textTransform:"uppercase",marginBottom:12}}>Análisis inteligente</div>
          <div style={{display:"flex",flexDirection:"column",gap:8}}>
            {insights.map((ins,i)=>(
              <div key={i} style={{display:"flex",alignItems:"center",gap:12,padding:"10px 12px",borderRadius:12,background:"rgba(255,255,255,0.04)",border:`1px solid ${ins.color}18`}}>
                <span style={{fontSize:18,flexShrink:0}}>{ins.icon}</span>
                <span style={{fontSize:13,fontWeight:600,color:ins.color,lineHeight:1.4}}>{ins.text}</span>
              </div>
            ))}
          </div>
        </CapCard>
  ...
  const renderLiquidez = () => (
    <div>
      ...
      <CapCard accent={CAP_COLORS.liquidez} style={{padding:"22px 20px",marginBottom:12}}>
        <div style={{fontSize:10,fontWeight:700,color:"rgba(96,165,250,0.55)",letterSpacing:2.5,textTransform:"uppercase",marginBottom:6}}>Dinero disponible</div>
        ...
        {/* Caja vs bancos */}
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
          <div style={{background:"rgba(59,130,246,0.1)",border:"1px solid rgba(59,130,246,0.2)",borderRadius:12,padding:"11px 13px"}}>
            ...
            <div style={{fontSize:16,fontWeight:900,color:"#60A5FA"}}>{fmtS2(totalCuentas)}</div>
          </div>
          <div style={{background:"rgba(52,211,153,0.08)",border:"1px solid rgba(52,211,153,0.2)",borderRadius:12,padding:"11px 13px"}}>
            <div style={{fontSize:10,color:"rgba(52,211,153,0.5)",fontWeight:700,marginBottom:4,textTransform:"uppercase",letterSpacing:0.8}}>Caja / Efectivo</div>
      ...
      <CapCard style={{padding:"18px 20px",marginBottom:12}}>
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:14}}>
          <div style={{fontSize:10,fontWeight:700,color:"rgba(148,175,255,0.4)",letterSpacing:2,textTransform:"uppercase"}}>Cuentas bancarias</div>
          <button onClick={()=>openAdd("cuentas")} style={{display:"inline-flex",alignItems:"center",gap:5,background:"rgba(59,130,246,0.15)",border:"1px solid rgba(59,130,246,0.3)",borderRadius:99,padding:"5px 12px",color:"#60A5FA",fontSize:11,fontWeight:700,cursor:"pointer",touchAction:"manipulation",fontFamily:"inherit"}}>
            + Agregar
          </button>
        </div>
        {data.cuentas.length===0?(
          <div style={{textAlign:"center",padding:"24px 0",color:"rgba(148,175,255,0.3)",fontSize:13}}>Sin cuentas registradas</div>
        ):data.cuentas.map(c=>(
          <div key={c.id} style={{display:"flex",alignItems:"center",gap:12,padding:"12px 0",borderBottom:"1px solid rgba(255,255,255,0.05)"}}>
            <div style={{width:40,height:40,borderRadius:11,background:"rgba(59,130,246,0.12)",border:"1px solid rgba(59,130,246,0.2)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:18,flexShrink:0}}>🏦</div>
            <div style={{flex:1,minWidth:0}}>
              <div style={{fontSize:14,fontWeight:700,color:"#fff"}}>{c.nombre}</div>
              <div style={{fontSize:11,color:"rgba(148,175,255,0.4)"}}>{c.banco||"—"} · {c.tipo||"Ahorro"}</div>
            </div>
            <div style={{textAlign:"right",flexShrink:0}}>
              <div style={{fontSize:15,fontWeight:900,color:"#60A5FA"}}>{fmtS2(c.saldo)}</div>
            </div>
            <button onClick={()=>openEdit("cuentas",c)} style={{width:30,height:30,borderRadius:8,background:"rgba(255,255,255,0.06)",border:"1px solid rgba(255,255,255,0.1)",cursor:"pointer",touchAction:"manipulation",display:"flex",alignItems:"center",justifyContent:"center",color:"rgba(200,215,255,0.6)",flexShrink:0}}>
      ...
      {/* Caja efectivo */}
      <CapCard style={{padding:"18px 20px"}}>
        <div style={{fontSize:10,fontWeight:700,color:"rgba(148,175,255,0.4)",letterSpacing:2,textTransform:"uppercase",marginBottom:12}}>Caja / Efectivo</div>
        <div style={{display:"flex",alignItems:"center",gap:14}}>
          <div style={{flex:1}}>
            <div style={{fontSize:28,fontWeight:900,color:"#34D399",letterSpacing:"-1px"}}>{fmtS2(data.caja)}</div>
            <div style={{fontSize:11,color:"rgba(148,175,255,0.4)",marginTop:4}}>Efectivo disponible en caja</div>
          </div>
          <button onClick={()=>{setModal("caja");setForm({caja:String(data.caja)});}} style={{background:"rgba(52,211,153,0.12)",border:"1px solid rgba(52,211,153,0.25)",borderRadius:12,padding:"10px 16px",color:"#34D399",fontSize:12,fontWeight:700,cursor:"pointer",touchAction:"manipulation",fontFamily:"inherit",whiteSpace:"nowrap"}}>
            Actualizar
          </button>
        </div>
      </CapCard>
  ...
  const renderActivos = () => (
    <div>
      <CapCard accent={CAP_COLORS.activos} style={{padding:"22px 20px",marginBottom:12}}>
        <div style={{fontSize:10,fontWeight:700,color:"rgba(52,211,153,0.5)",letterSpacing:2.5,textTransform:"uppercase",marginBottom:6}}>Valor total de activos</div>
        <div style={{fontSize:40,fontWeight:900,color:"#fff",letterSpacing:"-1.5px",lineHeight:1,marginBottom:4}}>{fmtS2(totalActivos)}</div>
        <div style={{fontSize:12,color:"rgba(148,175,255,0.4)",marginBottom:16}}>{data.activos.length} activo{data.activos.length!==1?"s":""} registrado{data.activos.length!==1?"s":""}</div>
      ...
      <CapCard style={{padding:"18px 20px"}}>
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:14}}>
          <div style={{fontSize:10,fontWeight:700,color:"rgba(148,175,255,0.4)",letterSpacing:2,textTransform:"uppercase"}}>Mis activos</div>
          <button onClick={()=>openAdd("activos")} style={{display:"inline-flex",alignItems:"center",gap:5,background:"rgba(16,185,129,0.12)",border:"1px solid rgba(16,185,129,0.3)",borderRadius:99,padding:"5px 12px",color:"#34D399",fontSize:11,fontWeight:700,cursor:"pointer",touchAction:"manipulation",fontFamily:"inherit"}}>
            + Agregar
          </button>
        </div>
        {data.activos.length===0?(
          <div style={{textAlign:"center",padding:"24px 0",color:"rgba(148,175,255,0.3)",fontSize:13}}>Sin activos registrados</div>
        ):data.activos.map(a=>{
          ...
          return (
            <div key={a.id} style={{display:"flex",alignItems:"center",gap:12,padding:"12px 0",borderBottom:"1px solid rgba(255,255,255,0.05)"}}>
              ...
              <div style={{flex:1,minWidth:0}}>
                ...
                <div style={{fontSize:11,color:"rgba(148,175,255,0.4)"}}>{a.fecha||"—"}{rent>0?` · +${rent}% rentab.`:""}</div>
              </div>
              <div style={{textAlign:"right",flexShrink:0}}>
                <div style={{fontSize:15,fontWeight:900,color:"#34D399"}}>{fmtS2(a.valor)}</div>
              </div>
              <button onClick={()=>openEdit("activos",a)} style={{width:30,height:30,borderRadius:8,background:"rgba(255,255,255,0.06)",border:"1px solid rgba(255,255,255,0.1)",cursor:"pointer",touchAction:"manipulation",display:"flex",alignItems:"center",justifyContent:"center",color:"rgba(200,215,255,0.6)",flexShrink:0}}>
                <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round"><path d="M17 3a2.828 2.828 0 114 4L7.5 20.5 2 22l1.5-5.5L17 3z"/></svg>
              </button>
            </div>
          );
    ...
    return (
      <div>
        <CapCard accent={CAP_COLORS.fondos} style={{padding:"22px 20px",marginBottom:12}}>
          <div style={{fontSize:10,fontWeight:700,color:"rgba(139,92,246,0.5)",letterSpacing:2.5,textTransform:"uppercase",marginBottom:6}}>Fondos estratégicos</div>
          ...
          <div style={{fontSize:12,color:"rgba(148,175,255,0.4)",marginBottom:16}}>disponible para distribuir</div>
          {/* Barra coloreada */}
          ...
          <div style={{fontSize:10,color:"rgba(148,175,255,0.35)",textAlign:"right"}}>{totalPct}% asignado</div>
        </CapCard>

        <CapCard style={{padding:"18px 20px",marginBottom:12}}>
          <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:14}}>
            <div style={{fontSize:10,fontWeight:700,color:"rgba(148,175,255,0.4)",letterSpacing:2,textTransform:"uppercase"}}>Distribución</div>
            <button onClick={()=>{setModal("fondos");const f2={};data.fondos.forEach((f,i)=>f2[`pct_${i}`]=f.pct);setForm(f2);}} style={{display:"inline-flex",alignItems:"center",gap:5,background:"rgba(139,92,246,0.12)",border:"1px solid rgba(139,92,246,0.3)",borderRadius:99,padding:"5px 12px",color:"#A78BFA",fontSize:11,fontWeight:700,cursor:"pointer",touchAction:"manipulation",fontFamily:"inherit"}}>
              Ajustar %
            </button>
          </div>
          <div style={{display:"flex",flexDirection:"column",gap:10}}>
            {data.fondos.map(f=>{
              ...
              return (
                <div key={f.id} style={{padding:"13px 14px",borderRadius:14,background:"rgba(255,255,255,0.04)",border:"1px solid rgba(255,255,255,0.06)"}}>
                  <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:8}}>
                    <span style={{fontSize:20}}>{f.emoji}</span>
                    <div style={{flex:1}}>
                      <div style={{fontSize:14,fontWeight:700,color:"#fff"}}>{f.label}</div>
                      <div style={{fontSize:11,color:"rgba(148,175,255,0.4)"}}>{f.pct}% del disponible</div>
                    </div>
                    <div style={{fontSize:16,fontWeight:900,color:f.color}}>{fmtS2(monto)}</div>
                  </div>
  ...
  const renderObjetivos = () => (
    <div>
      <CapCard accent={CAP_COLORS.objetivos} style={{padding:"22px 20px",marginBottom:12}}>
        <div style={{fontSize:10,fontWeight:700,color:"rgba(245,158,11,0.5)",letterSpacing:2.5,textTransform:"uppercase",marginBottom:8}}>Objetivos de expansión</div>
        ...
        {ingresoMensualNeto>0&&objPendiente&&(
          <div style={{fontSize:12,color:"rgba(245,158,11,0.7)",fontWeight:600}}>
      ...
      <CapCard style={{padding:"18px 20px"}}>
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:14}}>
          <div style={{fontSize:10,fontWeight:700,color:"rgba(148,175,255,0.4)",letterSpacing:2,textTransform:"uppercase"}}>Mis objetivos</div>
          <button onClick={()=>openAdd("objetivos")} style={{display:"inline-flex",alignItems:"center",gap:5,background:"rgba(245,158,11,0.12)",border:"1px solid rgba(245,158,11,0.3)",borderRadius:99,padding:"5px 12px",color:"#FCD34D",fontSize:11,fontWeight:700,cursor:"pointer",touchAction:"manipulation",fontFamily:"inherit"}}>
            + Nuevo
          </button>
        </div>
        {data.objetivos.length===0?(
          <div style={{textAlign:"center",padding:"24px 0",color:"rgba(148,175,255,0.3)",fontSize:13}}>Sin objetivos · agrega tu primero</div>
            ...
            <div key={o.id} style={{marginBottom:14,padding:"14px 14px",borderRadius:16,background:"rgba(255,255,255,0.04)",border:`1px solid ${done?"rgba(52,211,153,0.2)":"rgba(245,158,11,0.12)"}`}}>
              <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:10}}>
                <div style={{flex:1}}>
                  ...
                  <div style={{fontSize:11,color:"rgba(148,175,255,0.4)",marginTop:2}}>
                ...
                <button onClick={()=>openEdit("objetivos",o)} style={{width:30,height:30,borderRadius:8,background:"rgba(255,255,255,0.06)",border:"1px solid rgba(255,255,255,0.1)",cursor:"pointer",touchAction:"manipulation",display:"flex",alignItems:"center",justifyContent:"center",color:"rgba(200,215,255,0.6)",flexShrink:0}}>
                  <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round"><path d="M17 3a2.828 2.828 0 114 4L7.5 20.5 2 22l1.5-5.5L17 3z"/></svg>
                </button>
              </div>
              ...
              {!done&&<div style={{display:"flex",justifyContent:"space-between",marginTop:6}}>
                <span style={{fontSize:10,color:"rgba(148,175,255,0.4)"}}>Faltan {fmtS2(faltan)}</span>
                <span style={{fontSize:10,color:"rgba(245,158,11,0.6)",fontWeight:600}}>Meta: {fmtS2(meta)}</span>
              </div>}
            </div>
          );
        })}
  ...
  const modalFondos = (
    <CapModal open={modal==="fondos"} onClose={closeModal} title="Ajustar fondos" subtitle="Define cómo distribuir tu dinero disponible">
        ...
        <div key={f.id} style={{marginBottom:16}}>
          ...
          <div style={{display:"flex",alignItems:"center",gap:10}}>
            ...
            <div style={{flex:1}}>
              ...
              <div style={{fontSize:11,color:"rgba(148,175,255,0.4)",textAlign:"center",marginTop:4}}>{fmtS2((liquidezNeta>0?liquidezNeta:0)*((Number(form[`pct_${i}`]??f.pct))/100))}</div>
            </div>
      ...
      <div style={{fontSize:12,color:"rgba(148,175,255,0.35)",textAlign:"center",marginBottom:16}}>
        Total: {data.fondos.reduce((s,f,i)=>s+Number(form[`pct_${i}`]??f.pct),0)}%
      </div>
      <button onClick={saveFondos} disabled={saving} style={{width:"100%",padding:13,background:"#8B5CF6",border:"none",borderRadius:12,color:"#fff",fontWeight:700,fontSize:14,cursor:"pointer",touchAction:"manipulation",fontFamily:"inherit",opacity:saving?0.7:1}}>{saving?"Guardando…":"Guardar distribución"}</button>
    </CapModal>
  );
  ...
  const modalObjetivo = (
    <CapModal open={modal==="objetivos"} onClose={closeModal} title={editItem?"Editar objetivo":"Nuevo objetivo"} subtitle="Meta de expansión del negocio">
      ...
      <div style={{display:"flex",gap:10,marginTop:8}}>
        ...
        <button onClick={()=>saveItem("objetivos")} disabled={saving} style={{flex:2,padding:13,background:"#F59E0B",border:"none",borderRadius:12,color:"#fff",fontWeight:700,fontSize:14,cursor:"pointer",touchAction:"manipulation",fontFamily:"inherit",opacity:saving?0.7:1}}>{saving?"Guardando…":"Guardar"}</button>
      </div>
    </CapModal>
  );
  ...
  // ━━ LAYOUT PRINCIPAL ━━
  return (
    <div style={{paddingBottom:"calc(140px + env(safe-area-inset-bottom))",minHeight:"100%"}}>
        ...
        <div style={{display:"flex",gap:6,overflowX:"auto",paddingBottom:4,marginBottom:16,scrollbarWidth:"none"}}>
          {TABS.map(t=>(
            <button key={t.id} onClick={()=>setSection(t.id)} style={{
              ...
              background:section===t.id?"rgba(99,140,255,0.22)":"rgba(255,255,255,0.05)",
              border:`1px solid ${section===t.id?"rgba(99,140,255,0.45)":"rgba(255,255,255,0.08)"}`,
              color:section===t.id?"#93C5FD":"rgba(148,175,255,0.5)",
    ...
    <>
      ...
      {!splash && !!user && <div style={{ position: "fixed", top: 0, left: 0, right: 0, bottom: 0, background: NL.bg, display: "flex", flexDirection: "column", fontFamily: "'DM Sans',sans-serif", color: NL.text, overflow: "hidden", overscrollBehavior: "none" }}>
        ...
        <div
          ref={scrollRef}
          data-scroll
          style={{ flex: 1, minHeight: 0, overflowY: "scroll", overflowX: "hidden", overscrollBehavior: "none", touchAction: "pan-y", background: (activeTab === "capital" || activeTab === "contratos") ? "#0E1A3B" : undefined, position:"relative" }}
        >
          <div style={{ paddingTop: (activeTab === "capital" || activeTab === "contratos") ? 0 : 20, paddingLeft: (activeTab === "capital" || activeTab === "contratos") ? 0 : 16, paddingRight: (activeTab === "capital" || activeTab === "contratos") ? 0 : 16, paddingBottom: "calc(280px + env(safe-area-inset-bottom))" }}>
            {loading
