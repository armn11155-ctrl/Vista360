...
function Gastos({gastos,setGastos,autoScan,setAutoScan,onModalChange}){
  ...
  const guardarSueldo = async () => {
    if (!formSueldo.nombre.trim()) return alert("Escribe el nombre");
    if (!formSueldo.monto) return alert("Escribe el monto");
...
// ══════════════════════════════════════════════════════════════════
// ══════════════════════════════════════════════════════════════════
// CAPITAL — cifra editable + dashboard de crecimiento + resumen P&L
// ══════════════════════════════════════════════════════════════════
function Capital({ paneles, contratos, gastos, proveedores }) {

  const STORAGE_KEY = "capital_cifra_v1";

  // ── Capital editable (persiste en localStorage) ──
  const [capital, setCapital]     = useState(() => {
    try { const v = localStorage.getItem(STORAGE_KEY); return v !== null ? Number(v) : 0; } catch { return 0; }
  });
  ...
  // ── Cálculos desde contratos y gastos ──
  const mesHoyK = (() => { const d = new Date(); return `${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,"0")}`; })();
  ...
  const flujo = meses6.map(mes => {
    ...
    const eg  = gastos.filter(g => g.mes === mes || (g.fecha && g.fecha.slice(0,7) === mes)).reduce((s,g) => s + Number(g.monto||0), 0);
    return { mes, ing, eg, neto: ing - eg };
  });

  // Mes actual
  const ingMes   = flujo[5]?.ing  || 0;
  const gasMes   = flujo[5]?.eg   || 0;
  const netoMes  = ingMes - gasMes;
  const mesAnt   = flujo[4]?.ing  || 0;
  const crecPct  = mesAnt > 0 ? Math.round(((ingMes - mesAnt) / mesAnt) * 100) : 0;

  // Totales acumulados
  const ingTotal = flujo.reduce((s,f) => s+f.ing, 0);
  const gasTotal = flujo.reduce((s,f) => s+f.eg, 0);
  const maxBar   = Math.max(...flujo.map(f => Math.max(f.ing, f.eg)), 1);

  // Por cobrar
  const porCobrar = contratos.filter(c => !c.pagado).reduce((s,c) => s + Number(c.monto||0), 0);

  // Margen
  const margen = ingTotal > 0 ? Math.round(((ingTotal - gasTotal) / ingTotal) * 100) : 0;
  ...
  const EM = {
    ...
    red:    "#EF4444",
    white:  "#FFFFFF",
    muted:  "rgba(180,210,255,0.6)",
  };

  return (
    <div style={{ background: EM.bg, margin: "-20px -16px", minHeight: "100%" }}>

      {/* ══════════════════════════════════════════════
          BLOQUE 1 — TU CAPITAL (editable)
      ══════════════════════════════════════════════ */}
      <div style={{ padding: "28px 22px 24px", borderBottom: `1px solid ${EM.border}` }}>
        <div style={{ fontSize: 11, color: EM.muted, fontWeight: 600, letterSpacing: 1.8, textTransform: "uppercase", marginBottom: 16 }}>
          Vista360 · 8 Millas — Capital
        </div>

        <div style={{ display: "flex", alignItems: "flex-end", justifyContent: "space-between", gap: 12 }}>
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ fontSize: 12, color: EM.muted, marginBottom: 8, fontWeight: 500 }}>Tu capital disponible</div>

            {editando ? (
              <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                <span style={{ fontSize: 28, fontWeight: 900, color: EM.muted }}>S/</span>
                <input
                  ref={inputRef}
                  type="number"
                  value={inputVal}
                  ...
                  placeholder="0.00"
                  style={{
                    flex: 1, background: "rgba(37,99,235,0.1)", border: "2px solid #2563EB",
                    borderRadius: 14, padding: "10px 16px", color: "#fff", fontSize: 28,
                    fontWeight: 900, outline: "none", fontFamily: "inherit", letterSpacing: "-0.5px",
                  }}
                />
                <button onClick={guardarCapital} style={{ background: "#2563EB", border: "none", borderRadius: 12, padding: "10px 20px", color: "#fff", fontWeight: 700, fontSize: 14, cursor: "pointer", touchAction: "manipulation", fontFamily: "inherit", flexShrink: 0 }}>
                  Guardar
                </button>
              </div>
            ) : (
              <div style={{ display: "flex", alignItems: "flex-end", gap: 12 }}>
                <div style={{ fontSize: 44, fontWeight: 900, color: capital >= 0 ? EM.white : EM.red, letterSpacing: "-2px", lineHeight: 1 }}>
                  {fmt(capital)}
                </div>
                <button onClick={abrirEditor} style={{
                  marginBottom: 4, background: "rgba(37,99,235,0.15)", border: "1px solid rgba(37,99,235,0.35)",
                  borderRadius: 10, padding: "7px 14px", color: EM.blue, fontWeight: 700, fontSize: 12,
                  cursor: "pointer", touchAction: "manipulation", fontFamily: "inherit", flexShrink: 0,
                  display: "flex", alignItems: "center", gap: 6,
                }}>
                  <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round"><path d="M17 3a2.828 2.828 0 114 4L7.5 20.5 2 22l1.5-5.5L17 3z"/></svg>
                  Editar
                </button>
              </div>
            )}
        ...
        {/* Nota */}
        <div style={{ marginTop: 12, fontSize: 11, color: "rgba(180,210,255,0.35)" }}>
          Cifra ingresada manualmente · no se calcula automáticamente
        </div>
      </div>

      {/* ══════════════════════════════════════════════
          BLOQUE 2 — RESUMEN P&L (último mes + 6m)
      ══════════════════════════════════════════════ */}
      <div style={{ padding: "20px 22px", borderBottom: `1px solid ${EM.border}` }}>
        <div style={{ fontSize: 11, color: EM.muted, fontWeight: 700, letterSpacing: 1.2, textTransform: "uppercase", marginBottom: 16 }}>
          Resumen · {mC(mesHoyK)} {new Date().getFullYear()}
        </div>

        {/* 3 KPIs en fila */}
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 1, borderRadius: 16, overflow: "hidden", border: `1px solid ${EM.border}`, marginBottom: 20 }}>
          {[
            { label: "Ingresos",  value: fmt(ingMes),  color: EM.green },
            { label: "Gastos",    value: fmt(gasMes),  color: EM.red   },
            { label: "Neto mes",  value: fmt(netoMes), color: netoMes >= 0 ? EM.green : EM.red },
          ].map(({ label, value, color }, i) => (
            <div key={label} style={{ background: EM.card, padding: "14px 10px", textAlign: "center", borderRight: i < 2 ? `1px solid ${EM.border}` : "none" }}>
              <div style={{ fontSize: 9, color: EM.muted, textTransform: "uppercase", letterSpacing: 1, fontWeight: 700, marginBottom: 6 }}>{label}</div>
              <div style={{ fontSize: 16, fontWeight: 900, color, letterSpacing: "-0.5px", lineHeight: 1 }}>{value}</div>
            </div>
          ))}
        </div>

        {/* Fila por cobrar + margen */}
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
          <div style={{ background: EM.card, border: `1px solid ${EM.border}`, borderRadius: 12, padding: "12px 14px" }}>
            <div style={{ fontSize: 9, color: EM.muted, textTransform: "uppercase", letterSpacing: 1, fontWeight: 700, marginBottom: 5 }}>Por cobrar</div>
            <div style={{ fontSize: 18, fontWeight: 800, color: porCobrar > 0 ? EM.white : EM.muted }}>{fmt(porCobrar)}</div>
          </div>
          <div style={{ background: EM.card, border: `1px solid ${EM.border}`, borderRadius: 12, padding: "12px 14px" }}>
            <div style={{ fontSize: 9, color: EM.muted, textTransform: "uppercase", letterSpacing: 1, fontWeight: 700, marginBottom: 5 }}>Margen 6m</div>
            <div style={{ fontSize: 18, fontWeight: 800, color: margen >= 0 ? EM.green : EM.red }}>{margen}%</div>
          </div>
        </div>
      </div>

      {/* ══════════════════════════════════════════════
          BLOQUE 3 — DASHBOARD DE CRECIMIENTO
      ══════════════════════════════════════════════ */}
      <div style={{ padding: "20px 22px", paddingBottom: "calc(120px + env(safe-area-inset-bottom))" }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 16 }}>
          <div style={{ fontSize: 11, color: EM.muted, fontWeight: 700, letterSpacing: 1.2, textTransform: "uppercase" }}>
            Crecimiento · 6 meses
          </div>
          <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
            <div style={{
              background: crecPct >= 0 ? "rgba(16,185,129,0.15)" : "rgba(239,68,68,0.15)",
              border: `1px solid ${crecPct >= 0 ? "rgba(16,185,129,0.3)" : "rgba(239,68,68,0.3)"}`,
              borderRadius: 99, padding: "4px 10px",
              fontSize: 12, fontWeight: 700, color: crecPct >= 0 ? EM.green : EM.red,
            }}>
              {crecPct >= 0 ? "+" : ""}{crecPct}% vs mes ant.
            </div>
          </div>
        </div>
        ...
        <div style={{ display: "flex", alignItems: "flex-end", gap: 8, height: 120, marginBottom: 8 }}>
          {flujo.map(({ mes, ing, eg }, idx) => {
            const isActual = idx === 5;
            return (
              <div key={mes} style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", gap: 3, height: "100%", minWidth: 0 }}>
                <div style={{ flex: 1, width: "100%", display: "flex", flexDirection: "column", justifyContent: "flex-end", gap: 2 }}>
                  {ing > 0 && (
                    <div style={{
                      width: "100%", height: `${(ing / maxBar) * 100}%`, minHeight: 5,
                      background: isActual ? `linear-gradient(180deg,#2563EB,#1D4ED8)` : "rgba(37,99,235,0.25)",
                      borderRadius: "5px 5px 0 0", transition: "height .4s",
                      boxShadow: isActual ? "0 0 14px rgba(37,99,235,0.5)" : "none",
                    }}/>
                  )}
                  {eg > 0 && (
                    <div style={{
                      width: "100%", height: `${(eg / maxBar) * 100}%`, minHeight: 5,
                      background: isActual ? `linear-gradient(180deg,#EF4444,#DC2626)` : "rgba(239,68,68,0.2)",
                      borderRadius: "5px 5px 0 0", transition: "height .4s",
                    }}/>
                  )}
                  {ing === 0 && eg === 0 && (
        ...
        <div style={{ display: "flex", gap: 8, marginBottom: 20 }}>
          {flujo.map(({ mes }, idx) => (
            <div key={mes} style={{ flex: 1, textAlign: "center", fontSize: 10, color: idx === 5 ? EM.white : EM.muted, fontWeight: idx === 5 ? 700 : 500, textTransform: "uppercase", letterSpacing: 0.3 }}>{mC(mes)}</div>
          ))}
        </div>
        ...
        <div style={{ display: "flex", gap: 16, justifyContent: "center", marginBottom: 24 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 11, color: EM.muted }}>
            <div style={{ width: 10, height: 10, borderRadius: 3, background: "#2563EB" }}/> Ingresos
          </div>
          <div style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 11, color: EM.muted }}>
            <div style={{ width: 10, height: 10, borderRadius: 3, background: "#EF4444" }}/> Gastos
          </div>
        </div>

        {/* Tabla acumulado 6 meses */}
        <div style={{ fontSize: 11, color: EM.muted, fontWeight: 700, letterSpacing: 1.2, textTransform: "uppercase", marginBottom: 14 }}>Acumulado 6 meses</div>
        <div style={{ background: EM.card, border: `1px solid ${EM.border}`, borderRadius: 16, overflow: "hidden" }}>
          {[
            { label: "Total ingresos", value: fmt(ingTotal), color: EM.green },
            { label: "Total gastos",   value: fmt(gasTotal), color: EM.red   },
            { label: "Resultado neto", value: fmt(ingTotal - gasTotal), color: (ingTotal - gasTotal) >= 0 ? EM.green : EM.red, bold: true },
          ].map(({ label, value, color, bold }, i, arr) => (
            <div key={label} style={{
              display: "flex", alignItems: "center", justifyContent: "space-between",
              padding: "14px 18px",
              borderBottom: i < arr.length - 1 ? `1px solid ${EM.border}` : "none",
              borderTop: bold ? `2px solid ${EM.border}` : "none",
              background: bold ? "rgba(37,99,235,0.06)" : "transparent",
            }}>
              <span style={{ fontSize: 13, color: bold ? EM.white : "rgba(200,220,255,0.7)", fontWeight: bold ? 700 : 500 }}>{label}</span>
        ...
        {/* Barra margen */}
        <div style={{ marginTop: 16, background: EM.card, border: `1px solid ${EM.border}`, borderRadius: 14, padding: "14px 18px" }}>
          <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 10 }}>
            <span style={{ fontSize: 11, color: EM.muted, fontWeight: 700, textTransform: "uppercase", letterSpacing: 0.8 }}>Margen neto</span>
            <span style={{ fontSize: 14, fontWeight: 800, color: margen >= 0 ? EM.green : EM.red }}>{margen}%</span>
          </div>
          <div style={{ background: "rgba(255,255,255,0.06)", borderRadius: 99, height: 8, overflow: "hidden" }}>
            <div style={{
              ...
              background: margen >= 0 ? "linear-gradient(90deg,#2563EB,#10B981)" : "linear-gradient(90deg,#EF4444,#DC2626)",
            }}/>
          </div>
        </div>
      </div>
  ...
  const renderContent = useCallback(() => {
    ...
    return (
      <>
        ...
        {tab === "resultados"  && <Reportes    contratos={contratos} paneles={paneles} clientes={clientes} gastos={gastos} initialSeccion="resultados"/>}
    ...
    <>
      ...
      {!splash && !!user && <div style={{ position: "fixed", top: 0, left: 0, right: 0, bottom: 0, background: NL.bg, display: "flex", flexDirection: "column", fontFamily: "'DM Sans',sans-serif", color: NL.text, overflow: "hidden", overscrollBehavior: "none" }}>
        ...
        <div
          ref={scrollRef}
          data-scroll
          style={{ flex: 1, minHeight: 0, overflowY: "scroll", overflowX: "hidden", overscrollBehavior: "none", touchAction: "pan-y", background: activeTab === "capital" ? "#04080F" : undefined }}
        >
          <div style={{ padding: activeTab === "capital" ? "0" : "20px 16px", paddingBottom: "calc(108px + env(safe-area-inset-bottom))" }}>
            {loading
