...
import { initializeFirestore, persistentLocalCache, persistentMultipleTabManager, collection, getDocs, addDoc, updateDoc, deleteDoc, doc, orderBy, query, serverTimestamp, setDoc, getDoc, onSnapshot, Timestamp } from "firebase/firestore";
...
// Los fallbacks permiten que el proyecto siga funcionando
// en desarrollo sin un .env configurado, pero en producción
// siempre usa las variables de entorno.
const firebaseConfig = {
  apiKey:            import.meta.env.VITE_FIREBASE_API_KEY             ?? "AIzaSyCklWQStUPElsRfKH3icUZht-R321zgZY8",
  authDomain:        import.meta.env.VITE_FIREBASE_AUTH_DOMAIN         ?? "base-de-datos-vista360.firebaseapp.com",
  projectId:         import.meta.env.VITE_FIREBASE_PROJECT_ID          ?? "base-de-datos-vista360",
  storageBucket:     import.meta.env.VITE_FIREBASE_STORAGE_BUCKET      ?? "base-de-datos-vista360.firebasestorage.app",
  messagingSenderId: import.meta.env.VITE_FIREBASE_MESSAGING_SENDER_ID ?? "421479773461",
  appId:             import.meta.env.VITE_FIREBASE_APP_ID              ?? "1:421479773461:web:e38de4bfda177e232be51c",
};

// Inicializar Firebase con SDK modular
const firebaseApp = initializeApp(firebaseConfig);
// ── PERSISTENCIA OFFLINE ─────────────────────────────────────────
// initializeFirestore con persistentLocalCache habilita IndexedDB:
//   • Las escrituras offline se encolan y se sincronizan al reconectar.
//   • El banner "los cambios se guardarán al reconectar" ahora es real.
//   • persistentMultipleTabManager permite usar la app en varias pestañas.
// NOTA: usar initializeFirestore en lugar de getFirestore (que no admite
// estas opciones en Firebase SDK v10+).
...
const validate = {
  /** Validación mínima de RUC peruano (11 dígitos). */
...
export default function App() {
  ...
  const getHeaderColor = (t: string, profile: boolean): string => {
  ...
  // SW completo con soporte para push remoto (eventos "push") y
  // notificationclick. Se inyecta inline como Blob para no requerir
  // un archivo public/sw.js separado.
  useEffect(() => {
    ...
    const SW = `
      ...
      // ── Push remoto (desde tu backend via web-push) ──
      self.addEventListener("push", e => {
        let data = {};
        try { data = e.data?.json() ?? {}; } catch { data = { body: e.data?.text() ?? "" }; }
        const title   = data.title ?? "Vista360";
        const options = {
          body:    data.body    ?? "",
      ...
      // ── Click en notificación → enfoca o abre la app ──
      self.addEventListener("notificationclick", e => {
        ...
        e.waitUntil(
          self.clients.matchAll({ type: "window", includeUncontrolled: true }).then(list => {
            ...
            if (win) { win.focus(); win.navigate(targetUrl); return; }
  ...
  useEffect(() => {
    ...
    const snap = (col: string, ord = "createdAt") => {
...
// ══════════════════════════════════════════════════════════════════
// 🧪 TESTS — solo se ejecutan con Vitest, nunca en producción
// ══════════════════════════════════════════════════════════════════
// Ejecutar: npx vitest run
//
// Setup (una sola vez):
//   1. npm install -D vitest
//   2. En vite.config.ts añadir dentro de defineConfig:
...
if (import.meta.env.MODE === "test") {
  ...
  describe("validate.ruc", () => {
    ...
    it("acepta RUC persona jurídica (20...)", () => expect(validate.ruc("20615629431")).toBeNull());
    it("acepta RUC persona natural (10...)",  () => expect(validate.ruc("10123456789")).toBeNull());
    it("acepta RUC con guiones",              () => expect(validate.ruc("20-615-629-431")).toBeNull());
    it("rechaza < 11 dígitos",               () => expect(validate.ruc("2061562943")).toMatch(/11 dígitos/));
    it("rechaza > 11 dígitos",               () => expect(validate.ruc("206156294311")).toMatch(/11 dígitos/));
    it("rechaza prefijo inválido (30...)",    () => expect(validate.ruc("30123456789")).toMatch(/inválido/));
  });

  // ── validate.email ──────────────────────────────────────────────
  describe("validate.email", () => {
    it("acepta campo vacío (opcional)",     () => expect(validate.email("")).toBeNull());
    it("acepta email válido",               () => expect(validate.email("a@b.com")).toBeNull());
    it("acepta email con +",               () => expect(validate.email("fact+1@emp.pe")).toBeNull());
    it("rechaza sin @",                     () => expect(validate.email("nodominio.com")).toBeTruthy());
    it("rechaza sin dominio",               () => expect(validate.email("user@")).toBeTruthy());
    it("rechaza con espacios",              () => expect(validate.email("a b@c.com")).toBeTruthy());
  });

  // ── validate.fechasContrato ─────────────────────────────────────
  describe("validate.fechasContrato", () => {
    it("acepta fin > inicio",               () => expect(validate.fechasContrato("2025-01-01","2025-12-31")).toBeNull());
    it("acepta fin === inicio",             () => expect(validate.fechasContrato("2025-06-15","2025-06-15")).toBeNull());
    it("rechaza fin < inicio",              () => expect(validate.fechasContrato("2025-06-01","2025-01-01")).toBeTruthy());
    it("rechaza inicio vacío",             () => expect(validate.fechasContrato("","2025-12-31")).toBeTruthy());
    it("rechaza fin vacío",                () => expect(validate.fechasContrato("2025-01-01","")).toBeTruthy());
  });

  // ── validate.monto ──────────────────────────────────────────────
  describe("validate.monto", () => {
    it("acepta positivo (number)",          () => expect(validate.monto(100)).toBeNull());
    it("acepta positivo (string)",          () => expect(validate.monto("1500")).toBeNull());
    it("rechaza cero",                      () => expect(validate.monto(0)).toBeTruthy());
    it("rechaza negativo",                  () => expect(validate.monto(-1)).toBeTruthy());
    it("rechaza texto",                     () => expect(validate.monto("abc")).toBeTruthy());
  });

  // ── validate.gasto ──────────────────────────────────────────────
  describe("validate.gasto", () => {
    const base = { descripcion: "Mantenimiento", monto: 350, fecha: "2025-06-01" };
    it("acepta gasto completo",             () => expect(validate.gasto(base)).toBeNull());
    it("rechaza sin descripción",           () => expect(validate.gasto({ ...base, descripcion: "" })).toBeTruthy());
    it("rechaza monto cero",                () => expect(validate.gasto({ ...base, monto: 0 })).toBeTruthy());
    it("rechaza sin fecha",                 () => expect(validate.gasto({ ...base, fecha: "" })).toBeTruthy());
  });

  // ── validate.contrato ───────────────────────────────────────────
  describe("validate.contrato", () => {
    const base = { panel_id:"p1", cliente_id:"c1", inicio:"2025-01-01", fin:"2025-12-31", monto:1500 };
    it("acepta contrato válido",            () => expect(validate.contrato(base)).toBeNull());
    it("rechaza sin panel_id",              () => expect(validate.contrato({ ...base, panel_id:"" })).toBeTruthy());
    it("rechaza sin cliente_id",            () => expect(validate.contrato({ ...base, cliente_id:"" })).toBeTruthy());
    it("rechaza fin < inicio",              () => expect(validate.contrato({ ...base, fin:"2024-01-01" })).toBeTruthy());
    it("rechaza monto 0",                   () => expect(validate.contrato({ ...base, monto:0 })).toBeTruthy());
  });
  ...
  describe("dias()", () => {
    ...
    it("positivo para fecha futura", () => {
      vi.setSystemTime(new Date("2025-06-01T12:00:00Z"));
      expect(dias("2025-06-11")).toBeGreaterThan(0);
    });
    it("negativo para fecha pasada", () => {
      vi.setSystemTime(new Date("2025-06-15T12:00:00Z"));
      expect(dias("2025-06-01")).toBeLessThan(0);
    });
    it("30 días exactos → 30", () => {
      vi.setSystemTime(new Date("2025-06-01T00:00:00Z"));
      expect(dias("2025-07-01")).toBe(30);
    });
  });
}
