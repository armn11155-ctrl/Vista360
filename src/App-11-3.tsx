...
function useViewportSetup() {
  useEffect(() => {
    try {
      ...
      // Viewport — evita zoom no deseado en iOS PWA
...
function DrawerMenu({ open, onClose, activeTab, onTabClick, onTrashOpen, trashCount = 0, userName = "", userInitials = "?" }: DrawerMenuProps) {
  // Cerrar con Escape — comportamiento esperado en cualquier panel lateral
  useEffect(() => {
    if (!open) return;
...
export default function App() {
  ...
  return (
    <>
      ...
      {/* Las meta tags se inyectan en <head> desde useViewportSetup() */}
      <style>{`
        /* ── RESET BASE ── */
