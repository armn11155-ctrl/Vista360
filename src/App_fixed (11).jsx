...
import { getFirestore, collection, getDocs, addDoc, updateDoc, deleteDoc, doc, orderBy, query, serverTimestamp, setDoc, getDoc } from "firebase/firestore";
...
function Contratos({contratos,setContratos,paneles,clientes,loading,setTab,onModalChange}){
  ...
  const eliminar=async(id)=>{
    if(!confirm("¿Mover este contrato a la papelera?")) return;
  ...
  const eliminarPermanente=async(id)=>{
    if(!confirm("¿Eliminar definitivamente este contrato? Esta acción NO se puede deshacer.")) return;
  ...
  return(<div>
    ...
    <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:8,marginBottom:18}}>
      {[
        ...
        const activeColor=isTrash?"#EF4444":"#2563EB";
        const activeBg=isTrash?"#FEF2F2":"#EFF4FF";
        const activeBorder=isTrash?"#FECACA":"#BFD3FF";
        return (
          <button key={f.label} onClick={()=>setFiltro(f.label)} style={{
            ...
            border:`1px solid ${isActive?activeBorder:"#E5E7EB"}`,
            color:isActive?activeColor:"#6B7280",
            fontWeight:isActive?700:600,fontSize:12,cursor:"pointer",touchAction:"manipulation",
          }}>
            <span style={{
            ...
            {f.count>0&&<span style={{
              ...
              background:isActive?activeColor:"#F0F1F5",
    ...
    <div style={{display:"flex",flexDirection:"column",gap:18}}>
      ...
      {filtro==="Eliminados"&&fil.length>0&&(
        <div style={{display:"flex",alignItems:"center",gap:12,padding:"12px 16px",background:"#FEF2F2",border:"1px solid #FECACA",borderRadius:16}}>
          <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#EF4444" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="9"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/></svg>
          <span style={{fontSize:13,color:"#B91C1C",fontWeight:600,flex:1}}>La 🗑️ papelera es permanente — usa el botón rojo para eliminar definitivamente o restaura el contrato.</span>
        </div>
      )}
      {fil.map(c=>{
          ...
          <div key={c.id} style={{
            ...
            <div style={{
              ...
              <div style={{position:"relative",display:"flex",alignItems:"center",gap:14}}>
                ...
                <div style={{flex:1,minWidth:0}}>
                  <div style={{fontSize:17,fontWeight:800,color:"#fff",letterSpacing:"-0.01em",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{c.panel?.nombre||"Panel eliminado"}</div>
            ...
            <div style={{background:"#fff",borderRadius:22,padding:"18px 16px 14px",position:"relative",zIndex:1}}>
              ...
              <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:14,flexWrap:"wrap"}}>
                ...
                <div style={{flex:1}}/>
                {/* Action buttons — difieren si está en papelera */}
                {filtro==="Eliminados" ? (<>
                  {/* Restaurar */}
                  <button onClick={()=>restaurar(c.id)} title="Restaurar contrato" style={{width:42,height:42,borderRadius:11,border:"1px solid #BBF7D0",background:"#F0FDF4",cursor:"pointer",touchAction:"manipulation",display:"inline-flex",alignItems:"center",justifyContent:"center",color:"#10B981"}}>
                    <svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M3 12a9 9 0 1 0 9-9 9.75 9.75 0 0 0-6.74 2.74"/><path d="M3 3v4h4"/></svg>
                  </button>
                  {/* Eliminar permanentemente */}
                  <button onClick={()=>eliminarPermanente(c.id)} title="Eliminar permanentemente" style={{width:42,height:42,borderRadius:11,border:"2px solid #EF4444",background:"#FEF2F2",cursor:"pointer",touchAction:"manipulation",display:"inline-flex",alignItems:"center",justifyContent:"center",color:"#EF4444"}}>
                    <svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round"><polyline points="3 6 5 6 21 6"/><path d="M19 6l-1 14a2 2 0 0 1-2 2H8a2 2 0 0 1-2-2L5 6"/><path d="M10 11v6M14 11v6"/><path d="M9 6V4a2 2 0 0 1 2-2h2a2 2 0 0 1 2 2v2"/><line x1="4" y1="4" x2="20" y2="20" strokeWidth="2"/></svg>
                  </button>
                </>) : (<>
                  ...
                  <button onClick={()=>eliminar(c.id)} title="Mover a papelera" style={{width:42,height:42,borderRadius:11,border:"1px solid #FECACA",background:"#FEF2F2",cursor:"pointer",touchAction:"manipulation",display:"inline-flex",alignItems:"center",justifyContent:"center",color:"#EF4444"}}>
      ...
      {fil.length===0&&(
        <div style={{textAlign:"center",padding:48,background:"#fff",borderRadius:22,border:"1px dashed #E5E7EB"}}>
          <div style={{
            ...
            background:filtro==="Eliminados"?"#FEF2F2":"#EFF4FF",
            margin:"0 auto 14px",
            display:"flex",alignItems:"center",justifyContent:"center",
            color:filtro==="Eliminados"?"#EF4444":"#2563EB",
          }}>
            {filtro==="Eliminados"
          ...
          <div style={{fontSize:16,fontWeight:700,color:"#0D1117",marginBottom:6}}>
...
function Capital({ paneles, contratos, gastos, proveedores }) {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        if (snap.exists()) setCapital(Number(snap.data().valor || 0));
      } catch {}
  ...
  const guardarCapital = async () => {
    ...
    setSaving(true);
    try {
      await setDoc(doc(db, "configuracion", "capital"), { valor: nuevo, updatedAt: serverTimestamp() });
      setCapital(nuevo);
    } catch {}
    setSaving(false);
    setEditando(false);
  ...
  const crecPct  = flujo[4]?.ing > 0 ? Math.round(((flujo[5].ing - flujo[4].ing) / flujo[4].ing) * 100) : 0;

  const neto = ingTotal - gasTotal;

  return (
    <div style={{ background: "#0A0F1E", margin: "-20px -16px", minHeight: "100%", paddingBottom: "calc(120px + env(safe-area-inset-bottom))" }}>

      {/* ── HERO CARD ── */}
      <div style={{ position: "relative", overflow: "hidden", padding: "36px 24px 40px", marginBottom: 0 }}>
        {/* Fondo con glow animado */}
        <div style={{ position: "absolute", inset: 0, background: "radial-gradient(ellipse 80% 60% at 50% 100%, rgba(37,99,235,0.22) 0%, rgba(10,15,30,0) 70%), linear-gradient(180deg,#0D1628 0%,#0A0F1E 100%)", pointerEvents: "none" }}/>
        <div style={{ position: "absolute", top: -60, right: -40, width: 220, height: 220, borderRadius: "50%", background: "radial-gradient(circle, rgba(99,140,255,0.13) 0%, transparent 70%)", pointerEvents: "none" }}/>

        {/* Breadcrumb */}
        <div style={{ position: "relative", display: "flex", alignItems: "center", gap: 6, marginBottom: 28 }}>
          <div style={{ width: 6, height: 6, borderRadius: "50%", background: "rgba(99,140,255,0.7)" }}/>
          <span style={{ fontSize: 10, color: "rgba(148,175,255,0.5)", fontWeight: 700, letterSpacing: 2.5, textTransform: "uppercase" }}>Capital · Vista360</span>
        </div>

        {/* Label */}
        <div style={{ position: "relative", fontSize: 12, color: "rgba(148,175,255,0.55)", fontWeight: 600, letterSpacing: 0.5, marginBottom: 8 }}>Capital disponible</div>

        {/* Número principal */}
        {editando ? (
          <div style={{ position: "relative" }}>
            <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 16 }}>
              <span style={{ fontSize: 30, fontWeight: 900, color: "rgba(148,175,255,0.35)", letterSpacing: "-1px" }}>S/</span>
              <input
                ref={inputRef}
                type="number"
                inputMode="decimal"
                ...
                placeholder="0.00"
                style={{ flex: 1, background: "rgba(37,99,235,0.1)", border: "1.5px solid rgba(99,140,255,0.5)", borderRadius: 18, padding: "14px 18px", color: "#fff", fontSize: 34, fontWeight: 900, outline: "none", fontFamily: "inherit", letterSpacing: "-1.5px" }}
              />
            </div>
            <div style={{ display: "flex", gap: 10 }}>
              <button onClick={() => setEditando(false)} style={{ flex: 1, padding: "13px", background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 14, color: "rgba(200,220,255,0.5)", fontWeight: 600, fontSize: 14, cursor: "pointer", touchAction: "manipulation", fontFamily: "inherit" }}>Cancelar</button>
              <button onClick={guardarCapital} disabled={saving} style={{ flex: 2, padding: "13px", background: saving ? "rgba(37,99,235,0.5)" : "linear-gradient(135deg,#3B6EFF,#2563EB)", border: "none", borderRadius: 14, color: "#fff", fontWeight: 700, fontSize: 14, cursor: "pointer", touchAction: "manipulation", fontFamily: "inherit", boxShadow: "0 6px 20px rgba(37,99,235,0.45)" }}>
          ...
          <div style={{ position: "relative" }}>
            <div style={{ fontSize: capitalLoaded ? 54 : 44, fontWeight: 900, color: capitalLoaded ? (capital > 0 ? "#FFFFFF" : "rgba(200,220,255,0.2)") : "rgba(200,220,255,0.15)", letterSpacing: "-2.5px", lineHeight: 1, marginBottom: 6, transition: "font-size .3s" }}>
              {capitalLoaded ? fmt(capital) : "—"}
            </div>
            {capitalLoaded && capital === 0 && (
              <div style={{ fontSize: 12, color: "rgba(148,175,255,0.4)", marginBottom: 16 }}>Sin capital registrado</div>
            )}
            <div style={{ display: "flex", alignItems: "center", gap: 10, marginTop: 18 }}>
              <button onClick={abrirEditor} style={{ display: "inline-flex", alignItems: "center", gap: 8, background: "rgba(59,110,255,0.15)", border: "1px solid rgba(99,140,255,0.3)", borderRadius: 50, padding: "10px 20px", color: "#93C5FD", fontWeight: 700, fontSize: 13, cursor: "pointer", touchAction: "manipulation", fontFamily: "inherit", backdropFilter: "blur(6px)" }}>
              ...
              <div style={{ fontSize: 10, color: "rgba(148,175,255,0.3)", fontWeight: 600 }}>Sincronizado en Firebase</div>
            </div>
          </div>
        )}
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, padding: "16px 16px 0" }}>
        {[
          { label: "Ingresos 6m", value: fmt(ingTotal), color: "#10B981", icon: "↑", bg: "rgba(16,185,129,0.08)", border: "rgba(16,185,129,0.18)" },
          { label: "Gastos 6m",   value: fmt(gasTotal), color: "#F87171", icon: "↓", bg: "rgba(248,113,113,0.08)", border: "rgba(248,113,113,0.18)" },
          { label: "Resultado neto", value: fmt(Math.abs(neto)), color: neto >= 0 ? "#10B981" : "#F87171", icon: neto >= 0 ? "+" : "−", bg: neto >= 0 ? "rgba(16,185,129,0.06)" : "rgba(248,113,113,0.06)", border: neto >= 0 ? "rgba(16,185,129,0.15)" : "rgba(248,113,113,0.15)" },
          { label: "Mes actual",  value: `${crecPct >= 0 ? "+" : ""}${crecPct}%`, color: crecPct >= 0 ? "#60A5FA" : "#F87171", icon: crecPct >= 0 ? "▲" : "▼", bg: "rgba(99,140,255,0.07)", border: "rgba(99,140,255,0.15)" },
        ].map(({ label, value, color, icon, bg, border }) => (
          <div key={label} style={{ background: bg, border: `1px solid ${border}`, borderRadius: 18, padding: "16px 18px" }}>
            <div style={{ fontSize: 10, color: "rgba(148,175,255,0.5)", fontWeight: 700, letterSpacing: 1.5, textTransform: "uppercase", marginBottom: 8 }}>{label}</div>
            <div style={{ display: "flex", alignItems: "baseline", gap: 5 }}>
              <span style={{ fontSize: 11, fontWeight: 800, color, opacity: 0.8 }}>{icon}</span>
              <span style={{ fontSize: 20, fontWeight: 900, color, letterSpacing: "-0.8px" }}>{value}</span>
            </div>
          </div>
        ))}
      </div>
      ...
      {/* ── GRÁFICO BARRAS ── */}
      <div style={{ margin: "16px 16px 0", background: "rgba(255,255,255,0.03)", border: "1px solid rgba(99,140,255,0.1)", borderRadius: 22, padding: "22px 20px 18px" }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 20 }}>
          <span style={{ fontSize: 11, color: "rgba(148,175,255,0.6)", fontWeight: 700, letterSpacing: 1.5, textTransform: "uppercase" }}>Flujo · 6 meses</span>
          <div style={{ display: "flex", gap: 12 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 5, fontSize: 10, color: "rgba(148,175,255,0.5)" }}><div style={{ width: 8, height: 8, borderRadius: 2, background: "rgba(99,140,255,0.7)" }}/> Ing.</div>
            <div style={{ display: "flex", alignItems: "center", gap: 5, fontSize: 10, color: "rgba(148,175,255,0.5)" }}><div style={{ width: 8, height: 8, borderRadius: 2, background: "rgba(248,113,113,0.7)" }}/> Gas.</div>
          </div>
        </div>
        <div style={{ display: "flex", alignItems: "flex-end", gap: 8, height: 90 }}>
          {flujo.map(({ mes, ing, eg }, idx) => {
            ...
            return (
              <div key={mes} style={{ flex: 1, height: "100%", display: "flex", flexDirection: "column", justifyContent: "flex-end", gap: 3, minWidth: 0 }}>
                {ing > 0 && <div style={{ width: "100%", height: (ing/maxBar*100)+"%", minHeight: 4, borderRadius: "5px 5px 0 0", background: isActual ? "linear-gradient(180deg,#7BB3FF,#3B6EFF)" : "rgba(99,140,255,0.28)", boxShadow: isActual ? "0 0 14px rgba(59,110,255,0.5)" : "none", transition: "height .5s cubic-bezier(.4,0,.2,1)" }}/>}
                {eg > 0 && <div style={{ width: "100%", height: (eg/maxBar*100)+"%", minHeight: 4, borderRadius: "5px 5px 0 0", background: isActual ? "linear-gradient(180deg,#FCA5A5,#EF4444)" : "rgba(248,113,113,0.22)", transition: "height .5s cubic-bezier(.4,0,.2,1)" }}/>}
                {ing === 0 && eg === 0 && <div style={{ width: "100%", height: 3, background: "rgba(255,255,255,0.05)", borderRadius: 2 }}/>}
              </div>
            );
          })}
        </div>
        <div style={{ display: "flex", gap: 8, marginTop: 10 }}>
          {flujo.map(({ mes }, idx) => (
            <div key={mes} style={{ flex: 1, textAlign: "center", fontSize: 9, color: idx === 5 ? "rgba(148,175,255,0.9)" : "rgba(148,175,255,0.3)", fontWeight: idx === 5 ? 800 : 400, textTransform: "uppercase", letterSpacing: 0.5 }}>{mC(mes)}</div>
          ))}
        </div>
      </div>
...
export default function App() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      // Guardar todos los contratos (vivos + eliminados) — la papelera los muestra en la pestaña Eliminados
