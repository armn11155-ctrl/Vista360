...
export default function App() {
  ...
  useEffect(() => {
    ...
    // Datos principales primero → muestra la app rápido
    Promise.all([
      fb.get("clientes"),
      fb.get("paneles"),
      fb.get("contratos"),
      ...
      setLoading(false); // ← muestra la UI ya con los datos principales

      // Datos secundarios en segundo plano (no bloquean la UI)
      fb.get("gastos").then(g  => setGastos(Array.isArray(g)  ? g : [])).catch(()=>{});
      fb.get("proveedores").then(pr => setProveedores(Array.isArray(pr) ? pr.filter(x => !x.deleted) : [])).catch(()=>{});
