...
;(function(){
  ...
  } catch(e) { /* Silencioso intencionalmente: es un guard defensivo en el arranque */ }
})();
// ══════════════════════════════════════════════════════════════════
...
const fb = {
  ...
  async uploadImagen(_path: string, file: File | Blob | string): Promise<string> {
...
// Alias de compatibilidad — apuntan al mismo objeto para no tener
// que buscar-y-reemplazar 500+ usos mientras dure la migración.
// TODO: migrar usos a T.* directamente y eliminar estos alias.
const D  = T;
const C  = T;
const tCol = (t: string) => t==="Cliente"?C.green:C.purple;
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        background: isBack ? "#3B82F6" : "#EF4444",
...
function Pagination({ page, totalPages, setPage, total, pageSize, dark = true }: {
  ...
  const bg    = dark ? "#0D1629" : "#FFFFFF";
  const border= dark ? "rgba(59,130,246,0.22)" : "#E5E7EB";
...
function CRM({clientes,setClientes,contratos,loading,onModalChange}){
  ...
  return (
    <div style={{ background: "#F2F4F8", margin: "-20px -16px", minHeight: "100%", paddingBottom: "calc(120px + env(safe-area-inset-bottom))" }}>
      ...
      {/* ── SEARCH ── */}
      <div style={{ padding: "14px 16px 10px", background: "#F2F4F8" }}>
      ...
      {/* ── FILTROS ── */}
      <div style={{ display: "flex", gap: 7, padding: "8px 16px 12px", background: "#F2F4F8", overflowX: "auto" }}>
        {filters.map(f => {
...
function FSparkline({ data, color=F.blue, height=40 }) {
...
function FBar({ pct, color=F.blue }) {
...
// ── Panel Detail Drawer ────────────────────────────────────────────
function FPanelDrawer({ pm, clientes, onClose }) {
  if(!pm) return null;
...
// ── Vista Panel ─────────────────────────────────────────────────────
function FVistaPanel({ eng, clientes }) {
...
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
function Resultados({ contratos, paneles = [], clientes = [], gastos = [], loading }) {
...
function Historico({contratos,setContratos,paneles,clientes,onModalChange}){
  ...
  const [mesSel,setMesSel]=useState(null); // null = ver todos los meses del año
  const hoyD=new Date();
      ...
      <div style={{ display:"flex", alignItems:"center", gap:8, marginBottom:16, marginTop:8 }}>
        {aniosDisp.map(a=>(
          <button key={a} onClick={()=>{ setAnioSel(a); setMesSel(null); }} onMouseDown={e=>e.stopPropagation()} onTouchStart={e=>e.stopPropagation()} onTouchEnd={e=>e.stopPropagation()} onPointerDown={e=>e.stopPropagation()} style={{
            ...
            background: a===anioEfectivo ? "#2563EB" : "rgba(255,255,255,0.08)",
...
// ══════════════════════════════════════════════════════════════════
// NL era un tercer objeto de tokens casi idéntico a D y C.
// Ahora es un alias de T (tokens unificados).
const NL = T;
...
// ══════════════════════════════════════════════════════════════════
function HeroCard({ panalesLibres, contratos = [], setTab = () => {}, userName = "" }) {
...
// ══════════════════════════════════════════════════════════════════
function ResumenNuevo({ clientes, contratos, paneles, gastos, setTab, userName = "" }) {
  ...
  return (
    <div style={{ paddingBottom: 32 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 20 }}>
        <MetricCardNL icon={iDollar} label="Cobrado este mes" value={fmtS(ingreso)} sub={`${contratos.filter(c=>{ if(!c.inicio||!c.fin||c.deleted) return false; const ini=c.inicio.slice(0,7),fin=c.fin.slice(0,7); return ini<=mesActual&&fin>=mesActual&&c.pagosMeses?.[mesActual]===true; }).length} contratos pagados`} waveColor="#2563EB"/>
        ...
        <MetricCardNL icon={iClock}  label="Por cobrar"      value={fmtS(porcobrar)} valueColor={NL.accent} sub={`${contratos.filter(c=>{ if(!c.inicio||!c.fin||c.deleted) return false; const ini=c.inicio.slice(0,7),fin=c.fin.slice(0,7); return ini<=mesActual&&fin>=mesActual&&!c.pagosMeses?.[mesActual]; }).length} contratos pendientes`} waveColor="#2563EB"/>
...
function DrawerMenu({ open, onClose, activeTab, onTabClick, onTrashOpen, trashCount = 0, userName = "", userInitials = "?" }) {
  return (
    <>
      {open && (
        <div
...
export default function App() {
  ...
  useEffect(() => {
    ...
    const color = splash ? "#0D1629" : headerColor;
  ...
  // ── Helpers de visibilidad — monta todos los tabs una sola vez y los
  //    oculta/muestra con CSS. Así el estado local de cada tab (filtros,
  //    scroll, formularios a medio rellenar) sobrevive al cambio de pestaña.
  //    Nota: los tabs que renderizan mapas pesados (mapa, capital) se siguen
  //    montando una vez y quedan en memoria; si el peso es un problema se
  //    puede añadir un flag "lazyMounted" que espere al primer foco.
  ...
  return (
    <>
      ...
      {!splash && !!user && <div className="v360-app-root" style={{ background: NL.bg, color: NL.text }}>
          ...
          {loading ? (
            ...
            <>
              ...
              <div className="v360-tab-panel v360-tab-padded" style={show("hoy")}>
                {error && <div className="v360-firebase-error"><span style={{fontSize:22}}>⚠️</span><div><div style={{fontWeight:700,color:NL.amber,fontSize:14}}>Sin conexión a Firebase</div><div style={{fontSize:12,color:NL.muted}}>Despliega en Vercel para conectar.</div></div></div>}
              ...
              <div className="v360-tab-panel v360-tab-padded" style={show("resultados")}>
                <ErrorBoundary label="Resultados"><Reportes contratos={contractsActive} paneles={paneles} clientes={clientes} gastos={gastos} initialSeccion="resultados"/></ErrorBoundary>
              </div>

              <div className="v360-tab-panel v360-tab-padded" style={show("reportes")}>
                <ErrorBoundary label="Reportes"><Reportes contratos={contractsActive} paneles={paneles} clientes={clientes} gastos={gastos}/></ErrorBoundary>
              </div>
              ...
              <div className="v360-tab-panel v360-tab-padded" style={show("facturacion")}>
                <ErrorBoundary label="Facturación"><Facturacion contratos={contractsActive} paneles={paneles} clientes={clientes}/></ErrorBoundary>
              </div>
              ...
              <div className="v360-tab-panel v360-tab-flush" style={show("capital")}>
                <ErrorBoundary label="Capital"><Capital paneles={paneles} contratos={contractsActive} gastos={gastos} proveedores={proveedores}/></ErrorBoundary>
              </div>

              <div className="v360-tab-panel v360-tab-flush" style={show("mapa")}>
                <ErrorBoundary label="Mapa"><Mapa paneles={paneles} clientes={clientes} contratos={contractsActive}/></ErrorBoundary>
              </div>
            </>
          )}
        </div>
