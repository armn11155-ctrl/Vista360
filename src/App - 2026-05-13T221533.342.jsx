...
// Race un promise contra un timeout — evita que getDocs cuelgue para siempre en iOS
const withMs = (p, ms) => Promise.race([p, new Promise((_, r) => setTimeout(r, ms))]);

const fb = {
  async get(col) {
    try {
      const q    = query(collection(db, col), orderBy("createdAt","desc"));
      const snap = await withMs(getDocs(q), 6000);  // max 6s por query
      return snap.docs.map(d => ({ id: d.id, ...d.data() }));
    } catch(e) {
      try {
        const snap = await withMs(getDocs(collection(db, col)), 6000);
        return snap.docs.map(d => ({ id: d.id, ...d.data() }));
      } catch(e2) { return []; }
    }
  },
...
function LoginScreen({ onLoginSuccess }) {
  ...
  // Detectar iOS (iPhone/iPad) y modo standalone (PWA desde pantalla de inicio)
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      if (isIOS && !isStandalone) {
        // iOS Safari (browser normal): usar redirect porque iOS bloquea
        // signInWithPopup — hace llamadas async antes de window.open()
        // y Safari lo interpreta como popup no iniciado por el usuario.
        // onAuthStateChanged en App.jsx capturará el usuario al volver.
        await signInWithRedirect(auth, googleProvider);
        // La página navega a Google — cuando vuelva, onAuthStateChanged dispara.
        return;
      }
      // Laptop / Android / iOS PWA: usar popup
      const result = await signInWithPopup(auth, googleProvider);
      const u = result.user;
      if (ALLOWED_EMAILS.length > 0 && !ALLOWED_EMAILS.includes(u.email)) {
        await signOut(auth);
        setError("Acceso denegado: " + u.email + " no está autorizado.");
        setLoading(false);
        return;
      }
      onLoginSuccess(u);
      ...
      if (err.code === "auth/popup-blocked" || err.code === "auth/cancelled-popup-request") {
        // Fallback: si el popup fue bloqueado (iOS PWA), intentar redirect
        try { await signInWithRedirect(auth, googleProvider); return; } catch {}
      }
      const msgs = {
        "auth/popup-closed-by-user":  "Cerraste la ventana. Toca de nuevo para intentarlo.",
        "auth/unauthorized-domain":   "Dominio no autorizado. Ve a Firebase Console → Authentication → Authorized domains y agrega tu dominio.",
        "auth/operation-not-allowed": "Google Sign-In no está activado en Firebase Console → Authentication → Sign-in methods.",
        "auth/network-request-failed":"Sin conexión a internet. Verifica tu red.",
        "auth/internal-error":        "Error interno. Verifica que tu dominio esté en Firebase Console → Authentication → Authorized domains.",
      };
  ...
  return (
    <div style={{
      ...
      <div style={{ position: "absolute", top: "-20%", right: "-15%", width: "60%", height: "60%", background: "radial-gradient(ellipse, rgba(37,99,235,0.18) 0%, transparent 70%)", pointerEvents: "none" }}/>
      <div style={{ position: "absolute", bottom: "-15%", left: "-10%", width: "55%", height: "50%", background: "radial-gradient(ellipse, rgba(37,99,235,0.14) 0%, transparent 70%)", pointerEvents: "none" }}/>

      <div style={{ marginBottom: 28, filter: "drop-shadow(0 0 40px rgba(37,99,235,0.35))" }}>
        <Logo360 width={220}/>
      </div>

      <div style={{ fontFamily: "'Cormorant Garamond',Georgia,serif", fontSize: 13, fontWeight: 600, color: "rgba(255,255,255,0.6)", letterSpacing: 4, textTransform: "uppercase", marginBottom: 36, textAlign: "center" }}>
        Gestión de Paneles Publicitarios
      </div>

      <div style={{ width: "100%", maxWidth: 340, background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)", backdropFilter: "blur(20px)", borderRadius: 22, padding: "26px 22px", boxShadow: "0 20px 60px rgba(0,0,0,0.5), inset 0 1px 0 rgba(255,255,255,0.05)" }}>
        <div style={{ fontSize: 21, fontWeight: 800, color: "#fff", textAlign: "center", marginBottom: 6 }}>Bienvenido</div>
        <div style={{ fontSize: 13, color: "rgba(255,255,255,0.5)", textAlign: "center", marginBottom: 20, lineHeight: 1.5 }}>
          Inicia sesión con tu cuenta de Google
        </div>

        {/* Estado de red */}
        <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 6, marginBottom: 16, fontSize: 12, fontWeight: 600, color: online ? "#34D399" : "#F87171" }}>
          <div style={{ width: 7, height: 7, borderRadius: "50%", background: online ? "#34D399" : "#F87171" }}/>
          {online ? "Internet conectado" : "Sin conexión"}
        </div>

        <button
          onClick={handleGoogleLogin}
          disabled={loading}
          style={{ width: "100%", padding: "14px 20px", background: loading ? "rgba(255,255,255,0.5)" : "#fff", color: "#0F1729", border: "none", borderRadius: 12, fontSize: 15, fontWeight: 700, cursor: loading ? "wait" : "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: 12, boxShadow: "0 8px 24px rgba(0,0,0,0.3)" }}
        >
        ...
        {error ? (
          <div style={{ marginTop: 14, padding: "12px 14px", background: "rgba(239,68,68,0.12)", border: "1px solid rgba(239,68,68,0.3)", borderRadius: 10, color: "#FCA5A5", fontSize: 12, lineHeight: 1.6, textAlign: "center" }}>
            ⚠️ {error}
          </div>
        ) : null}

        <div style={{ marginTop: 18, paddingTop: 16, borderTop: "1px solid rgba(255,255,255,0.06)", fontSize: 11, color: "rgba(255,255,255,0.3)", textAlign: "center", lineHeight: 1.6 }}>
          Acceso seguro vía Google OAuth · 8 Millas
        </div>
      </div>

      <div style={{ position: "absolute", bottom: 20, fontSize: 11, color: "rgba(255,255,255,0.18)", textAlign: "center" }}>
        Vista360 v2.0
      </div>
    </div>
  );
...
export default function App() {
  ...
  useEffect(() => {
    // Timeout de seguridad: si onAuthStateChanged nunca dispara (Firebase
    // no inicializa por dominio no autorizado, API key inválida, etc.)
    // forzar authReady=true para que el splash se cierre y aparezca algo útil.
  ...
  // ── Estado de red (solo informativo, NO dispara recargas automáticas) ──
  const [online, setOnline] = useState(navigator.onLine);
  useEffect(() => {
  ...
  // fb.get ya lleva timeout de 6s por query, así que Promise.all termina siempre.
  const recargarDatos = useCallback(() => {
    ...
    Promise.all([
      ...
      setError(null);
      setLoading(false);
    }).catch(() => { setError("load-error"); setLoading(false); });
  }, [user]);

  useEffect(() => { if (!user) return; recargarDatos(); }, [user]);
              ...
              <div style={{background:NL.white,borderRadius:20,padding:28,width:"100%",maxWidth:320,textAlign:"center"}}>
                ...
                <div style={{display:"flex",gap:10}}>
                  ...
                  <button onClick={() => {
                    ...
                    }} style={{flex:1,padding:"12px",background:NL.red,border:"none",borderRadius:12,fontWeight:700,fontSize:14,cursor:"pointer",color:"#fff"}}>Salir</button>
                </div>
              </div>
            </div>
          )}
  ...
  return (
    <>
      ...
      {splash && <Splash done={() => setSplash(false)} isReady={authReady}/>}
      ...
      <div style={{ position: "fixed", top: 0, right: 0, bottom: 0, left: 0, background: NL.bg, display: "flex", flexDirection: "column", fontFamily: "'DM Sans',sans-serif", color: NL.text, overflow: "hidden", overscrollBehavior: "none" }}>
        ...
        >
          <div style={{ padding: "20px 16px", paddingBottom: "calc(108px + env(safe-area-inset-bottom))" }}>
            {/* Perfil siempre visible aunque loading=true — para poder cerrar sesión */}
            {showProfile
              ? renderContent()
              : loading
              ? <div style={{ display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", height:280, gap:16, padding:24, textAlign:"center" }}>
                  <div style={{ display:"flex", alignItems:"center", gap:10, color:NL.muted, fontSize:14 }}>
                    <div style={{ width:22, height:22, border:`2.5px solid ${NL.border}`, borderTopColor:NL.accent, borderRadius:"50%", animation:"spin .7s linear infinite" }}/>
                    Cargando datos...
                  </div>
                  <div style={{ fontSize:12, color: online ? NL.green : NL.red, fontWeight:600, display:"flex", alignItems:"center", gap:5 }}>
                    <div style={{ width:7, height:7, borderRadius:"50%", background: online ? NL.green : NL.red }}/>
                    {online ? "Internet conectado" : "Sin internet"}
                  </div>
                  <button onClick={() => { signOut(auth).finally(() => window.location.reload()); }}
                    style={{ padding:"8px 20px", background:"transparent", border:`1px solid ${NL.border}`, borderRadius:10, color:NL.muted, fontSize:12, cursor:"pointer", fontWeight:600 }}>
                    ¿Problemas? Cerrar sesión
                  </button>
                </div>
              : error
              ? <div style={{ display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", height:300, gap:14, padding:28, textAlign:"center" }}>
                  <div style={{ fontSize:44 }}>⚠️</div>
                  <div style={{ fontSize:16, fontWeight:800, color:NL.text }}>No se pudieron cargar los datos</div>
                  <div style={{ fontSize:13, color:NL.muted, lineHeight:1.6, maxWidth:280 }}>
                    Firebase no respondió. Verifica tu conexión e intenta de nuevo.
                  </div>
                  <div style={{ fontSize:12, color: online ? NL.green : NL.red, fontWeight:600, display:"flex", alignItems:"center", gap:5 }}>
                    <div style={{ width:7, height:7, borderRadius:"50%", background: online ? NL.green : NL.red }}/>
                    {online ? "Internet conectado" : "Sin internet"}
                  </div>
                  <button onClick={recargarDatos} style={{ padding:"13px 32px", background:NL.accent, color:"#fff", border:"none", borderRadius:13, fontWeight:700, fontSize:15, cursor:"pointer", boxShadow:"0 4px 14px rgba(37,99,235,0.3)" }}>
                    🔄 Reintentar
                  </button>
                  <button onClick={() => { signOut(auth).finally(() => window.location.reload()); }}
                    style={{ padding:"8px 20px", background:"transparent", color:NL.muted, border:`1px solid ${NL.border}`, borderRadius:10, fontSize:12, cursor:"pointer", fontWeight:600 }}>
                    Cerrar sesión
                  </button>
                </div>
              : renderContent()
