  ...
  return(<div>
    ...
    <div style={{display:"flex",flexDirection:"column",gap:18,padding:"0 16px"}}>
      ...
      {fil.map(c=>{
        ...
        const dRest=c.d;
        const urgente=dRest>0&&dRest<=15;
        const porVenc=dRest>0&&dRest<=60;
        // Solo verde, rojo o azul — sin amarillo/naranja
        const accentCard=urgente?"#EF4444":porVenc?"#3B82F6":"#10B981";
        const borderCard=urgente?"rgba(239,68,68,0.25)":porVenc?"rgba(59,130,246,0.20)":"rgba(16,185,129,0.18)";
          ...
          <div style={{
            ...
            <div style={{padding:"18px 18px 14px 20px",position:"relative"}}>
              ...
              <div style={{position:"relative",display:"flex",alignItems:"center",gap:14}}>
                ...
                {/* Info */}
                <div style={{flex:1,minWidth:0}}>
                  <div style={{fontSize:17,fontWeight:800,color:"#fff",letterSpacing:"-0.02em",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",marginBottom:3}}>{c.panel?.nombre||"Panel eliminado"}</div>
                  <div style={{fontSize:12,color:"rgba(148,175,255,0.6)",fontWeight:500,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{c.cliente?.empresa||"—"} · {c.cliente?.contacto||"—"}</div>
                  {/* Badge vencimiento */}
                  <div style={{marginTop:7}}>
                    <span style={{
                      ...
                      color:accentCard,
                      background:`${accentCard}18`,
                      border:`1px solid ${accentCard}33`,
                      borderRadius:99,padding:"3px 9px",
                    }}>
                      <span style={{width:5,height:5,borderRadius:"50%",background:accentCard,display:"inline-block",flexShrink:0}}/>
                ...
                <div style={{textAlign:"right",flexShrink:0}}>
                  <div style={{fontSize:22,fontWeight:900,color:"#fff",letterSpacing:"-0.03em",lineHeight:1}}>{fmt(c.monto)}</div>
                  <div style={{fontSize:10,color:"rgba(148,175,255,0.45)",fontWeight:600,marginTop:2}}>/mes</div>
                  <div style={{fontSize:12,fontWeight:800,color:"#10B981",marginTop:5}}>{fmt(pagados*Number(c.monto))} cobrado</div>
                </div>
              </div>
            </div>
            ...
            <div style={{padding:"12px 18px 12px 20px",display:"flex",alignItems:"center",gap:10,flexWrap:"wrap"}}>
              ...
              <div style={{
                ...
                fontSize:12,fontWeight:600,color:"rgba(220,230,255,0.75)",
                flexShrink:0,
              }}>
              ...
              {filtro==="Eliminados" ? (<>
                ...
                <button onClick={()=>eliminarPermanente(c.id)} title="Eliminar definitivamente" style={{width:38,height:38,borderRadius:10,border:"1px solid rgba(239,68,68,0.35)",background:"rgba(239,68,68,0.10)",cursor:"pointer",touchAction:"manipulation",display:"inline-flex",alignItems:"center",justifyContent:"center",color:"#F87171"}}>
                ...
                {/* Editar */}
                <button onClick={()=>openEdit(c)} style={{width:38,height:38,borderRadius:10,border:"1px solid rgba(255,255,255,0.10)",background:"rgba(255,255,255,0.06)",cursor:"pointer",touchAction:"manipulation",display:"inline-flex",alignItems:"center",justifyContent:"center",color:"rgba(220,230,255,0.7)"}}>
                ...
                <a href={`https://wa.me/${c.cliente.celular?.replace(/\D/g,"")}?text=${msg}`} target="_blank" rel="noopener noreferrer"
                  style={{width:38,height:38,borderRadius:10,border:"1px solid rgba(37,211,102,0.30)",background:"rgba(37,211,102,0.09)",cursor:"pointer",touchAction:"manipulation",display:"inline-flex",alignItems:"center",justifyContent:"center",textDecoration:"none"}}>
                  ...
                  {meses.map(m=>{
                    ...
                    return(
                      <button key={m.key} onClick={()=>togglePagoRapido(c,m.key)} style={{
                        ...
                        background:pag?"rgba(16,185,129,0.08)":"rgba(255,255,255,0.04)",
                        border:`1px solid ${pag?"rgba(16,185,129,0.22)":"rgba(255,255,255,0.07)"}`,
                        ...
                        <span style={{
                          ...
                          border:pag?"none":`1.5px solid rgba(255,255,255,0.18)`,
                        }}>
                        ...
                        <span style={{flex:1,fontSize:14,fontWeight:700,color:pag?"#34D399":"rgba(220,230,255,0.8)"}}>{m.label}</span>
                        <span style={{
                          padding:"4px 11px",borderRadius:999,fontSize:11,fontWeight:700,
                          background:pag?"rgba(16,185,129,0.15)":"rgba(255,255,255,0.06)",
                          color:pag?"#34D399":"rgba(148,175,255,0.5)",
                        }}>{pag?"Pagado":"Pendiente"}</span>
                        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="rgba(148,175,255,0.3)" strokeWidth="2.4" strokeLinecap="round" strokeLinejoin="round"><polyline points="9 6 15 12 9 18"/></svg>
                      </button>
                    );
                  })}
                </div>
...
function Capital({ paneles, contratos, gastos, proveedores }) {
  ...
  const saldoNeto= ingMes - gastoMes;
  ...
  return (
    <div style={{ padding: "20px 16px", paddingBottom: "calc(120px + env(safe-area-inset-bottom))" }}>

      {/* ── HERO: Capital Principal ── */}
      <div style={{ background: "linear-gradient(145deg, #111D3C 0%, #0D1629 60%, #091120 100%)", borderRadius: 24, padding: "24px 22px 20px", marginBottom: 14, border: "1px solid rgba(99,140,255,0.10)", boxShadow: "0 12px 48px rgba(0,0,0,0.45)" }}>

        <div style={{ fontSize: 10, fontWeight: 700, color: "rgba(148,175,255,0.45)", letterSpacing: 2.5, textTransform: "uppercase", marginBottom: 6 }}>Capital disponible</div>

        {editando ? (
          <>
            <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 14 }}>
              <span style={{ fontSize: 24, fontWeight: 900, color: "rgba(255,255,255,0.2)" }}>S/</span>
              <input ref={inputRef} type="number" inputMode="decimal" value={inputVal}
                ...
                placeholder="0.00"
                style={{ flex: 1, background: "rgba(255,255,255,0.07)", border: "1.5px solid rgba(99,140,255,0.5)", borderRadius: 14, padding: "12px 16px", color: "#fff", fontSize: 28, fontWeight: 900, outline: "none", fontFamily: "inherit", letterSpacing: "-1px" }}
              />
            </div>
            <div style={{ display: "flex", gap: 10 }}>
              <button onClick={() => setEditando(false)} style={{ flex: 1, padding: 13, background: "rgba(255,255,255,0.06)", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 12, color: "rgba(255,255,255,0.4)", fontWeight: 600, fontSize: 14, cursor: "pointer", touchAction: "manipulation", fontFamily: "inherit" }}>Cancelar</button>
              <button onClick={guardarCapital} disabled={saving} style={{ flex: 2, padding: 13, background: "#2563EB", border: "none", borderRadius: 12, color: "#fff", fontWeight: 700, fontSize: 14, cursor: "pointer", touchAction: "manipulation", fontFamily: "inherit", opacity: saving ? 0.7 : 1 }}>{saving ? "Guardando…" : "Guardar"}</button>
            </div>
          </>
        ) : (
          <>
            {/* Número grande */}
            <div style={{ fontSize: 46, fontWeight: 900, color: "#fff", letterSpacing: "-2px", lineHeight: 1, marginBottom: 4 }}>
              {capitalLoaded ? fmtM(capitalReal) : "—"}
            </div>

            {/* Badge variación */}
            <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 20 }}>
              <span style={{ fontSize: 12, fontWeight: 700, color: diff >= 0 ? "#34D399" : "#F87171", background: diff >= 0 ? "rgba(52,211,153,0.12)" : "rgba(248,113,113,0.12)", borderRadius: 99, padding: "3px 10px" }}>
                {pct >= 0 ? "+" : ""}{pct}% vs mes anterior
              </span>
            </div>

            {/* Sparkline */}
            <svg viewBox={`0 0 ${W} ${H}`} width="100%" height={H} style={{ display:"block", marginBottom: 8, overflow:"visible" }}>
              <defs>
              ...
              <path d={areaPath} fill="url(#cg2)"/>
              <path d={linePath} fill="none" stroke="#3B82F6" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round"/>
              {coords.map((p,i) => <circle key={i} cx={p.x} cy={p.y} r={i===5?4.5:2.5} fill={i===5?"#60A5FA":"rgba(96,165,250,0.45)"}/>)}
            </svg>
            <div style={{ display:"flex", justifyContent:"space-between" }}>
              {meses6.map(({lbl},i) => <span key={i} style={{ fontSize:9, color: i===5?"rgba(255,255,255,0.75)":"rgba(148,175,255,0.28)", fontWeight: i===5?700:400, flex:1, textAlign:"center" }}>{lbl}</span>)}
            </div>

            {/* Fila base + ingresos */}
            <div style={{ display:"flex", gap:8, marginTop:18 }}>
              <div style={{ flex:1, background:"rgba(255,255,255,0.04)", borderRadius:12, padding:"11px 13px" }}>
                <div style={{ fontSize:9, color:"rgba(148,175,255,0.4)", fontWeight:700, letterSpacing:1.2, textTransform:"uppercase", marginBottom:4 }}>Base</div>
                <div style={{ fontSize:14, fontWeight:800, color:"rgba(255,255,255,0.65)" }}>{capitalLoaded ? fmtM(capital) : "—"}</div>
              </div>
              <div style={{ flex:1, background:"rgba(52,211,153,0.07)", borderRadius:12, padding:"11px 13px" }}>
                <div style={{ fontSize:9, color:"rgba(52,211,153,0.45)", fontWeight:700, letterSpacing:1.2, textTransform:"uppercase", marginBottom:4 }}>Ingresos mes</div>
                <div style={{ fontSize:14, fontWeight:800, color:"#34D399" }}>+{fmtM(ingMes)}</div>
              </div>
            </div>

            <button onClick={abrirEditor} style={{ marginTop:16, display:"inline-flex", alignItems:"center", gap:7, background:"rgba(37,99,235,0.18)", border:"1px solid rgba(99,140,255,0.22)", borderRadius:99, padding:"8px 16px", color:"#93C5FD", fontWeight:700, fontSize:12, cursor:"pointer", touchAction:"manipulation", fontFamily:"inherit" }}>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:10, marginBottom:14 }}>
        {[
          { label:"Por cobrar", value: fmtM(porCobrar), icon:"💰", color:"#F59E0B", bg:"rgba(245,158,11,0.10)" },
          { label:"Gastos del mes", value: fmtM(gastoMes), icon:"📊", color:"#F87171", bg:"rgba(248,113,113,0.10)" },
          { label:"Ingresos previo", value: fmtM(ingPrev), icon:"📅", color:"#60A5FA", bg:"rgba(96,165,250,0.10)" },
          { label:"Saldo neto", value: fmtM(saldoNeto), icon: saldoNeto >= 0 ? "✅" : "⚠️", color: saldoNeto >= 0 ? "#34D399" : "#F87171", bg: saldoNeto >= 0 ? "rgba(52,211,153,0.10)" : "rgba(248,113,113,0.10)" },
        ].map(({ label, value, icon, color, bg }) => (
          <div key={label} style={{ background:"linear-gradient(145deg,#111D3C,#0D1629)", border:"1px solid rgba(99,140,255,0.08)", borderRadius:18, padding:"16px 15px", boxShadow:"0 4px 20px rgba(0,0,0,0.25)" }}>
            <div style={{ display:"flex", alignItems:"center", gap:7, marginBottom:8 }}>
              <div style={{ width:30, height:30, borderRadius:9, background:bg, display:"flex", alignItems:"center", justifyContent:"center", fontSize:14 }}>{icon}</div>
              <div style={{ fontSize:10, color:"rgba(148,175,255,0.45)", fontWeight:700, letterSpacing:0.8, textTransform:"uppercase" }}>{label}</div>
            </div>
            <div style={{ fontSize:18, fontWeight:900, color, letterSpacing:"-0.5px", lineHeight:1 }}>{value}</div>
          </div>
        ))}
      </div>
      ...
      {(() => {
        ...
        return (
          <div style={{ background:"linear-gradient(145deg,#111D3C,#0D1629)", border:"1px solid rgba(99,140,255,0.08)", borderRadius:18, padding:"16px 16px", boxShadow:"0 4px 20px rgba(0,0,0,0.25)" }}>
            <div style={{ fontSize:10, color:"rgba(148,175,255,0.45)", fontWeight:700, letterSpacing:1.5, textTransform:"uppercase", marginBottom:12 }}>Contratos activos · {activos.length}</div>
            {activos.slice(0,4).map(c => {
              ...
              return (
                <div key={c.id} style={{ display:"flex", alignItems:"center", gap:10, padding:"9px 0", borderBottom:"1px solid rgba(255,255,255,0.05)" }}>
                  <div style={{ width:7, height:7, borderRadius:"50%", background: d<=30 ? "#F59E0B" : "#34D399", flexShrink:0 }}/>
                  <div style={{ flex:1, minWidth:0 }}>
                    ...
                    <div style={{ fontSize:11, color:"rgba(148,175,255,0.4)" }}>Vence en {d} días</div>
                  </div>
                  <div style={{ fontSize:13, fontWeight:800, color:"#93C5FD" }}>{fmtM(c.monto)}</div>
                </div>
              );
            })}
            {activos.length > 4 && <div style={{ fontSize:11, color:"rgba(148,175,255,0.35)", marginTop:8, textAlign:"center" }}>+{activos.length-4} más</div>}
          </div>
        );
      })()}
  ...
  return (
    <>
      ...
      {!splash && !!user && <div style={{ position: "fixed", top: 0, left: 0, right: 0, bottom: 0, background: NL.bg, display: "flex", flexDirection: "column", fontFamily: "'DM Sans',sans-serif", color: NL.text, overflow: "hidden", overscrollBehavior: "none" }}>
        ...
        >
          <div style={{ padding: (activeTab === "capital" || activeTab === "contratos") ? "0" : "20px 16px", paddingBottom: "calc(108px + env(safe-area-inset-bottom))" }}>
            {loading
