...
// ══════════════════════════════════════════════════════════════════
// 📐 TIPOS — Interfaces de los modelos de datos principales
// ══════════════════════════════════════════════════════════════════
// Migración gradual: los componentes pueden ir adoptando estos tipos
// uno a uno. Los existentes con props implícitos siguen funcionando
// (TypeScript los infiere como `any` en modo permisivo).
// Para activar chequeo estricto: añadir "strict": true en tsconfig.
// ──────────────────────────────────────────────────────────────────

/** Panel publicitario (cartelera, pantalla LED, etc.) */
interface Panel {
  id:        string;
  nombre:    string;
  tipo:      string;
...
interface Cliente {
  ...
  sector?:   string;
  ciudad?:   string;
  estado:   "Activo" | "Por vencer" | "Inactivo" | string;
  tipo?:    "Cliente" | "Prospecto" | string;
  deleted?:  boolean;
  createdAt?: Timestamp | null;
}

/** Contrato de arrendamiento de panel */
interface Contrato {
  id:          string;
  panel_id:    string;
  cliente_id:  string;
  inicio:      string;   // ISO date string YYYY-MM-DD
  fin:         string;   // ISO date string YYYY-MM-DD
  monto:       number;
  pagado:      boolean;
...
/** Factura / comprobante electrónico */
interface Factura {
  id:              string;
  numero?:         string | number;
  serie?:          string;
  tipo?:           "FACTURA" | "BOLETA" | "NOTA_CREDITO" | "NOTA_DEBITO" | string;
  estado?:         string;
  monto?:          number;
  subtotal?:       number;
  igv?:            number;
...
// ── VIEWPORT SETUP: se ejecuta en un useEffect del componente raíz ─
// Antes era un IIFE que mutaba el DOM al importar el módulo (side-effect
// en tiempo de carga). Ahora vive en un hook que se llama desde App(),
// lo que es más predecible y testeable.
function useViewportSetup() {
  useEffect(() => {
    ...
    } catch { /* Silencioso: guard defensivo en el arranque */ }
  }, []); // Solo al montar — equivalente al IIFE original
}
// ══════════════════════════════════════════════════════════════════
import { initializeApp }    from "firebase/app";
import { getFirestore, collection, getDocs, addDoc, updateDoc, deleteDoc, doc, orderBy, query, serverTimestamp, setDoc, getDoc, onSnapshot, Timestamp, limit, writeBatch } from "firebase/firestore";
...
/** Tipo base de cualquier documento Firestore: siempre tiene un id string. */
type FirebaseDoc = { id: string; [key: string]: unknown };

/** Colecciones válidas — evita typos en strings sueltos. */
type ColName = "paneles" | "clientes" | "contratos" | "gastos" | "proveedores" | "facturas" | "sueldos";

/** Opciones del método del. */
...
const fb = {
  async get<T extends FirebaseDoc>(col: ColName): Promise<T[]> {
    try {
      ...
      try {
        ...
        return snap.docs.map(d => ({ id: d.id, ...d.data() } as T));
      } catch(e2) { console.error("[Firebase] get sin orderBy también falló:", e2); return []; }
    }
  },
  async post<T extends FirebaseDoc>(
    col: ColName,
    body: Omit<T, "id" | "createdAt">,
  ): Promise<T[]> {
    const payload = { ...body, createdAt: serverTimestamp() };
    const ref     = await addDoc(collection(db, col), payload);
    return [{ ...body, id: ref.id } as T];
  },
  ...
  async uploadImagen(file: File | Blob | string): Promise<string> {
    ...
    const webpBlob = await new Promise<Blob>((res, rej) => {
      const img = new Image();
...
// ── DATOS DEL EMISOR ─────────────────────────────────────────────
// Los datos reales se leen de variables de entorno (VITE_EMISOR_*).
// Define estos valores en tu archivo .env (nunca en el código fuente).
// Ejemplo de .env:
//   VITE_EMISOR_RAZON_SOCIAL=8 MILLAS
//   VITE_EMISOR_RUC=20615629431
//   VITE_EMISOR_DIRECCION=Jr. 28 de Julio Nro. 1279, Huánuco, Huánuco
//   VITE_EMISOR_CIUDAD=Huánuco
//   VITE_EMISOR_ACTIVIDAD=Publicidad (CIIU 7310)
// El fallback vacío evita crashes en desarrollo sin .env configurado.
const EMISOR = {
...
const dias  = (f: string): number => Math.ceil((new Date(f).getTime() - hoy().getTime()) / 86400000);
const fmt   = (n: number | null | undefined): string => `S/ ${Number(n||0).toLocaleString("es-PE",{minimumFractionDigits:2,maximumFractionDigits:2})}`;
const fmtF  = (s: string | null | undefined): string => s ? new Date(s).toLocaleDateString("es-PE",{day:"2-digit",month:"short",year:"numeric"}) : "—";
const mesLabel = (m: string): string => new Date(m+"-02").toLocaleDateString("es-PE",{month:"long",year:"numeric"}).replace(/^\w/,c=>c.toUpperCase());

// ── HAPTIC FEEDBACK ─────────────────────────────────────────────
const haptic = (type: "light" | "medium" | "success" | "error" = "light") => {
  try {
    if (navigator.vibrate) {
      const patterns: Record<string, number[]> = { light: [10], medium: [20], success: [10, 50, 20], error: [30, 20, 30] };
      navigator.vibrate(patterns[type] || [10]);
    }
  } catch { /* Silencioso: navigator.vibrate no disponible en todos los browsers */ }
};
// Un solo objeto T; D, C y NL son alias hacia él para no romper
// el código existente mientras se migra gradualmente a "T.*".
const T = {
  // ── Fondos ──────────────────────────────────────────────────────
  bg:        "#F2F4F8",   // fondo app claro
  dark:      "#0D1629",   // cards oscuras / nav oscuro
  darker:    "#07101F",   // fondo pantalla Capital
  surface:   "#FFFFFF",   // superficie de card en modo claro
  card:      "#FFFFFF",
  white:     "#FFFFFF",
  sidebarBg: "#FFFFFF",
  contentBg: "#F2F4F8",
  // ── Texto ───────────────────────────────────────────────────────
  text:      "#0D1629",   // texto principal (normalizado; era #0F1729 en C y #0D1117 en NL)
  muted:     "#64748B",   // texto secundario (normalizado; era #6B7280 en NL)
  // ── Acento de marca ─────────────────────────────────────────────
  accent:    "#2563EB",
  accentLt:  "#EFF4FF",
  // ── Semánticos ──────────────────────────────────────────────────
...
// Token único — usar T.* en todo el código.
const tCol = (t: string) => t==="Cliente"?T.green:T.purple;
const catCol= {"Mantenimiento":T.amber,"Personal":T.accent,"Transporte":T.cyan,"Administrativo":T.purple,"Servicios":T.green,"Marketing":T.red,"Otro":T.muted};


// ══════════════════════════════════════════════════════════════════
// 🔔 TOAST & CONFIRM — Context API + puente imperativo
// ══════════════════════════════════════════════════════════════════
//
// ARQUITECTURA:
//   • UIContext  → hook useUI() para componentes que consuman el Context.
//   • _uiRef     → puente estable para llamar toast.* / confirmAsync()
//                  desde funciones async fuera del árbol React (callbacks
//                  de Firebase, handlers de formularios, etc.).
//                  El objeto _uiRef es const — nunca se reasigna,
//                  solo sus propiedades internas se actualizan desde
//                  el useEffect de ToastProvider.
//
// USO recomendado dentro de componentes:
//   const { toast, confirm } = useUI();
//
// USO legado (fuera de componentes / funciones async):
//   toast.success("ok")  /  confirmAsync("¿Seguro?")
// ──────────────────────────────────────────────────────────────────
...
// Contexto para componentes que prefieran el patrón de hooks
const UIContext = createContext<UIContextValue | null>(null);

/** Hook para acceder a toast y confirm desde cualquier componente */
const useUI = (): UIContextValue => {
  const ctx = useContext(UIContext);
  if (!ctx) throw new Error("useUI debe usarse dentro de <ToastProvider>");
  return ctx;
};

// ── Puente imperativo (const — el objeto nunca se reasigna) ──────
// Permite llamar toast.* y confirmAsync() desde funciones async que
// no son componentes React (ej: callbacks de Firebase, handlers de form).
const _uiRef: {
...
// confirmAsync devuelve Promise<boolean> — se puede usar con await
const confirmAsync = (msg: string, opts: ConfirmPayload["opts"] = {}) =>
...
function ToastProvider() {
  ...
  // Funciones estables con useCallback para que el useEffect
  // no se re-ejecute en cada render (dependencias estables).
  ...
  // Registra las funciones estables en el puente imperativo.
  // El cleanup las borra al desmontar (p.ej. en tests / HMR).
  useEffect(() => {
  ...
  // Valor expuesto por el Context (memoizado para evitar re-renders)
  const ctxValue = useMemo<UIContextValue>(() => ({
    toast,
  ...
  return (
    // UIContext provee toast y confirm a cualquier componente hijo
    // que llame useUI(). Los renders de toast/confirm viven aquí
    // fuera del árbol de la app para evitar problemas de z-index.
      ...
      {confirm && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.55)", zIndex: 9998, display: "flex", alignItems: "center", justifyContent: "center", padding: 24 }}>
          <div style={{ background: "#fff", borderRadius: 22, padding: "28px 24px", width: "100%", maxWidth: 320, boxShadow: "0 20px 60px rgba(0,0,0,0.3)" }}>
            <div style={{ fontSize: 15, fontWeight: 700, color: T.text, marginBottom: 8, lineHeight: 1.4 }}>{confirm.opts?.title || "Confirmar"}</div>
            <div style={{ fontSize: 13, color: T.muted, marginBottom: 24, lineHeight: 1.5 }}>{confirm.msg}</div>
            <div style={{ display: "flex", gap: 10 }}>
              <button onClick={() => { confirm.resolve(false); setConfirm(null); }}
                style={{ flex: 1, padding: "12px", background: "transparent", border: "1px solid #E2E8F0", borderRadius: 12, fontWeight: 600, fontSize: 14, cursor: "pointer", touchAction: "manipulation", color: T.muted, fontFamily: "inherit" }}>
                {confirm.opts?.cancel || "Cancelar"}
              </button>
              <button onClick={() => { confirm.resolve(true); setConfirm(null); }}
                style={{ flex: 1, padding: "12px", background: confirm.opts?.danger ? T.red : "#0F172A", border: "none", borderRadius: 12, fontWeight: 700, fontSize: 14, cursor: "pointer", touchAction: "manipulation", color: "#fff", fontFamily: "inherit" }}>
...
// ── SKELETON COMPONENTS ──────────────────────────────────────────
interface SkPulseProps { w?: string | number; h?: number; r?: number; style?: React.CSSProperties; }
const SkPulse = ({ w = "100%", h = 16, r = 8, style = {} }: SkPulseProps) => (
...
const SkDarkCard = () => (
  <div style={{ background:T.dark, borderRadius:20, padding:20, marginBottom:12 }}>
    <div style={{ display:"flex", alignItems:"center", gap:12, marginBottom:16 }}>
...
// ══════════════════════════════════════════════════════════════════
// 📡 MODO OFFLINE — hook + banner
// ══════════════════════════════════════════════════════════════════

/** Devuelve true cuando hay conexión, false cuando no la hay.
 *  Reacciona en tiempo real a los eventos "online"/"offline" del navegador. */
...
/**
 * Banner fijo en la parte superior que:
 *  • Aparece en rojo/negro cuando se pierde la conexión.
 *  • Muestra un mensaje verde/azul de "reconectado" durante 3 s.
 *  • Desaparece solo. Sin props, sin estado externo.
 */
function OfflineBanner() {
  ...
  useEffect(() => {
    if (!online) {
      ...
      setShowBack(true);
      const t = setTimeout(() => { setShowBack(false); setWasOffline(false); }, 3200);
  ...
  return (
    <div style={{
      ...
      zIndex:     9985,
      background: isBack
        ? "linear-gradient(90deg, #0D1629 0%, #1E3A8A 50%, #0D1629 100%)"
        : "linear-gradient(90deg, #0D1629 0%, #111827 100%)",
      borderBottom: isBack
      ...
      <div style={{
        ...
        background: isBack ? "rgba(37,99,235,0.2)" : "rgba(239,68,68,0.15)",
        border:     isBack ? "1px solid rgba(59,130,246,0.4)" : "1px solid rgba(239,68,68,0.3)",
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ fontSize: 13, fontWeight: 700, color: isBack ? "#93C5FD" : "#FCA5A5", lineHeight: 1.2 }}>
          {isBack ? "Conexión restaurada" : "Sin conexión a internet"}
        </div>
        <div style={{ fontSize: 11, color: isBack ? "rgba(147,197,253,0.65)" : "rgba(252,165,165,0.65)", marginTop: 2 }}>
          {isBack
            ? "Los cambios se han sincronizado con Firebase"
            : "Modo lectura — los cambios se guardarán al reconectar"}
        </div>
      </div>

      {/* Indicador de estado (dot pulsante) */}
      <div style={{
        width: 8, height: 8, borderRadius: "50%", flexShrink: 0,
        background: isBack ? "#3B82F6" : T.red,
        boxShadow:  isBack ? "0 0 8px #3B82F6" : "0 0 8px #EF4444",
...
// ══════════════════════════════════════════════════════════════════
// 📄 PAGINACIÓN — hook + componente reutilizables
// ══════════════════════════════════════════════════════════════════
 ...
 * Resetea a página 1 automáticamente cuando cambia la longitud del array
 * (p.ej. al aplicar un filtro diferente).
 *
 * @param items    Array completo ya filtrado/ordenado.
 * @param pageSize Ítems por página (default 10).
 */
function usePagination<T>(items: T[], pageSize = 10): UsePaginationResult<T> {
  const [page, setPage] = useState(1);

  // Reset a página 1 cuando cambia el total (nuevo filtro activo)
  const prevTotal = useRef(items.length);
  useEffect(() => {
    if (prevTotal.current !== items.length) {
      setPage(1);
  ...
  return { page: safePage, setPage, totalPages, paginated, total: items.length, pageSize };
}

/** Muestra la barra de paginación. Azul y negro, compacta para móvil. */
function Pagination({ page, totalPages, setPage, total, pageSize, dark = true }: {
  page:       number;
  totalPages: number;
  setPage:    (p: number) => void;
  total:      number;
  ...
  const border= dark ? "rgba(59,130,246,0.22)" : T.border;
  const text  = dark ? "rgba(255,255,255,0.55)" : T.muted;
  const btnBg = dark ? "rgba(255,255,255,0.06)" : "#F1F5F9";
  const btnAct= T.accent;

  // Generar páginas a mostrar: siempre muestra 5 botones máximo
  const pages: (number | "…")[] = [];
  if (totalPages <= 5) {
  ...
  const btn = (label: string | number, onClick: () => void, active = false, disabled = false) => (
    <button
      ...
      style={{
        ...
        background:  active   ? btnAct : disabled ? "transparent" : btnBg,
        color:       active   ? "#fff" : disabled  ? "rgba(255,255,255,0.2)" : dark ? "rgba(255,255,255,0.75)" : "#374151",
  ...
  return (
    <div style={{
      ...
      <div style={{ display: "flex", gap: 4, alignItems: "center" }}>
        ...
        {pages.map((p, i) =>
          p === "…"
            ? <span key={`dots-${i}`} style={{ color: text, fontSize: 13, padding: "0 4px" }}>…</span>
...
// ══════════════════════════════════════════════════════════════════
// 🚀 VIRTUALIZACIÓN — hook para listas muy largas (500+ items)
// ══════════════════════════════════════════════════════════════════
/**
 * useVirtualList — renderiza solo las filas visibles en pantalla.
 * Usar cuando una lista supere ~200 items y la paginación no sea
 * una opción viable (p.ej. listas infinitas con scroll).
 *
 * Uso:
 *   const { containerProps, virtualItems, totalHeight } = useVirtualList(items, { itemHeight: 120 });
 *   <div {...containerProps} style={{ height: 600, overflowY: 'auto' }}>
...
interface UseVirtualListOptions { itemHeight: number; overscan?: number; containerHeight?: number; }
interface UseVirtualListResult<T> {
  virtualItems: VirtualItem<T>[];
  totalHeight:  number;
  containerProps: { ref: React.RefObject<HTMLDivElement>; onScroll: () => void; };
}
...
// Agregadas para eliminar props implícitas (noImplicitAny).
// ══════════════════════════════════════════════════════════════════
...
// F-component interfaces (tema oscuro / Capital/Reportes)
interface FCardProps      { children: React.ReactNode; style?: React.CSSProperties; }
interface FSparklineProps { data: number[]; color?: string; height?: number; }
interface FBarProps        { pct: number; color?: string; }
interface FBadgeProps      { label: string; color: string; }
interface FEngineData      { [key: string]: unknown; }
interface FPanelDrawerProps { pm: Record<string,unknown> | null; clientes: Cliente[]; onClose: () => void; }
interface FVistaEmpresaProps { eng: FEngineData; }
interface FVistaCiudadProps  { eng: FEngineData; }
interface FVistaClienteProps { eng: FEngineData; }
interface FVistaPanelProps   { eng: FEngineData; clientes: Cliente[]; }

interface HeroCardProps {
  panalesLibres: number;
  contratos?:    Contrato[];
  setTab?:       (tab: string) => void;
  userName?:     string;
}

interface MetricCardNLProps {
...
interface LoginScreenProps {
  onLoginSuccess: (user: import("firebase/auth").User) => void;
}
...
interface BadgeProps  { color: string; ch: React.ReactNode; }
interface TagProps    { color: string; ch: React.ReactNode; }
interface CardProps   { children: React.ReactNode; style?: React.CSSProperties; }
interface SecTitProps { ch: React.ReactNode; }
interface PgTitProps  { icon?: React.ReactNode; title: string; sub?: string; dark?: boolean; }

// Estilos extraídos fuera del render para evitar re-crear objetos en cada ciclo
...
const cardBaseStyle: React.CSSProperties = {
  background: "linear-gradient(145deg,rgba(14,24,42,0.95) 0%,rgba(8,14,26,0.98) 100%)",
  border: `1px solid rgba(79,124,255,0.13)`, borderRadius: 16, padding: 22,
...
const spinnerWrapStyle: React.CSSProperties = {
  display: "flex", alignItems: "center", justifyContent: "center",
  height: 200, color: T.muted, fontSize: 14, gap: 10,
};
...
const Badge  = ({ color, ch }: BadgeProps) => <span style={badgeStyle(color)}>{ch}</span>;
const Tag    = ({ color, ch }: TagProps)   => <span style={tagStyle(color)}>{ch}</span>;
const Card   = ({ children, style = {} }: CardProps) => <div style={{ ...cardBaseStyle, ...style }}>{children}</div>;
const SecTit = ({ ch }: SecTitProps) => <div style={secTitStyle}>{ch}</div>;
const PgTit  = ({ icon, title, sub, dark }: PgTitProps) => (
  <div style={{
    ...
    {icon && (
      <div style={{
        width: 42, height: 42, borderRadius: 12, flexShrink: 0, position: "relative", zIndex: 1,
        background: "rgba(255,255,255,0.14)", border: "1px solid rgba(255,255,255,0.22)",
        display: "flex", alignItems: "center", justifyContent: "center", color: "#fff",
      }}>{icon}</div>
    )}
    <div style={{ position: "relative", zIndex: 1 }}>
      <div style={{ fontSize: 17, fontWeight: 800, color: "#fff", letterSpacing: "-0.3px", lineHeight: 1.2 }}>{title}</div>
      {sub && <div style={{ fontSize: 12, color: "rgba(180,200,255,0.7)", marginTop: 3, fontWeight: 500 }}>{sub}</div>}
    </div>
  </div>
);
const Spinner = () => (
  <div style={spinnerWrapStyle}>
    <div style={spinnerDotStyle}/>Cargando...
  </div>
);
...
// Estilo base reutilizable para todos los inputs (creado una sola vez)
const inpBaseStyle: React.CSSProperties = {
  background: T.surface, border: `1px solid ${T.border}`, borderRadius: 10,
  padding: "12px 13px", color: T.text, fontSize: 16, outline: "none",
  fontFamily: "inherit", width: "100%", boxSizing: "border-box",
};
const inpLabelStyle: React.CSSProperties = {
  fontSize: 11, fontWeight: 700, color: T.muted,
...
const inp = (
  ...
  const set = (v: unknown) => setForm(f => ({ ...f, [key]: v }));
  const onFocus = (e: React.FocusEvent<HTMLElement>) => {
    setTimeout(() => e.target.scrollIntoView({ behavior: "smooth", block: "center" }), 350);
  };
  const isNumeric = opts.numeric || opts.type === "number"
    || ["monto","precio","ruc","telefono","celular","dni"].some(k => key.toLowerCase().includes(k));
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 5 }}>
      <label style={inpLabelStyle}>{label}</label>
      {opts.type === "select"
        ? <select value={(form[key] as string) || ""} onChange={e => set(e.target.value)} onFocus={onFocus} style={inpBaseStyle}>
            {(opts.options ?? []).map(o => <option key={o} value={o}>{o}</option>)}
          </select>
        : opts.type === "textarea"
        ? <textarea value={(form[key] as string) || ""} onChange={e => set(e.target.value)} onFocus={onFocus} rows={3} placeholder={opts.ph || ""} style={{ ...inpBaseStyle, resize: "vertical" }}/>
        : <input type={opts.type || "text"} inputMode={isNumeric ? "decimal" : undefined} value={(form[key] as string) || ""} placeholder={opts.ph || ""} onChange={e => set(e.target.value)} onFocus={onFocus} style={inpBaseStyle}/>
      }
    </div>
  );
};
...
function SwipeRow({ children, onDelete, onEdit, deleteLabel = "Eliminar" }: SwipeRowProps) {
    ...
    <div ref={rowRef} style={{ position:"relative", overflow:"hidden", marginBottom:10, borderRadius:22 }}>
      ...
      <div style={{ position:"absolute", right:0, top:0, bottom:0, width:ACTION_W, display:"flex", alignItems:"stretch", borderRadius:"0 22px 22px 0", overflow:"hidden" }}>
        <button onClick={() => { close(); onDelete?.(); }}
          style={{ flex:1, background:T.red, border:"none", color:"#fff", fontSize:11, fontWeight:700, cursor:"pointer", touchAction:"manipulation", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", gap:4 }}>
...
const modalSheetStyle: React.CSSProperties = {
  ...
  borderRadius: "22px 22px 0 0", padding: "10px 20px 24px",
  width: "100%", maxHeight: "92vh", overflowY: "auto",
  overscrollBehavior: "none",
  paddingBottom: "calc(24px + env(safe-area-inset-bottom))",
  boxShadow: "0 -10px 40px rgba(0,0,0,0.45)",
};
function Modal({ title, onClose, onSave, saveLabel = "Guardar ✓", children }: ModalProps) {
  return (
    <div style={modalOverlayStyle} onClick={e => e.target === e.currentTarget && onClose()}>
      <div style={modalSheetStyle}>
        ...
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:18}}>
          ...
          <button onClick={onClose} style={{background:T.border,border:"none",borderRadius:"50%",width:30,height:30,color:T.muted,cursor:"pointer",touchAction:"manipulation",fontSize:14,display:"flex",alignItems:"center",justifyContent:"center"}}>✕</button>
        </div>
        {children}
        <div style={{display:"flex",gap:10,marginTop:20,position:"sticky",bottom:0}}>
          <button onClick={onClose} style={{flex:1,padding:14,background:"transparent",border:`1px solid ${T.border}`,borderRadius:12,color:T.muted,fontWeight:600,cursor:"pointer",touchAction:"manipulation",fontSize:14,minHeight:46}}>Cancelar</button>
          <button onClick={onSave} style={{flex:2,padding:14,background:T.accent,border:"none",borderRadius:12,color:T.white,fontWeight:700,fontSize:14,cursor:"pointer",touchAction:"manipulation",minHeight:46}}>{saveLabel}</button>
        </div>
      </div>
    </div>
  );
...
function FieldGroup({ label, children }: FieldGroupProps){
  return(
    <div style={{marginBottom:16}}>
      <label style={{display:"block",fontSize:11,fontWeight:700,color:T.muted,textTransform:"uppercase",letterSpacing:1,marginBottom:6}}>{label}</label>
      {children}
    </div>
  );
}
...
function Paneles({ paneles, setPaneles, contratos, loading, setTab, onModalChange }: PanelesProps){
  ...
  const { paginated: panelesPag, page: panelPage, setPage: setPanelPage, totalPages: panelTotalPages, total: panelTotal, pageSize: panelPageSize } = usePagination(paneles, 12);
  ...
  const buscarSugerencias=useCallback(async(texto)=>{
    ...
    }catch(e){ console.warn("[Geocoding] buscarSugerencias falló:", e); setSugerencias([]); }
  },[form.ciudad]);
  ...
  const geocodificar=async()=>{
    ...
    }catch(e){ console.warn("[Geocoding] Photon falló, intentando Nominatim:", e); }
    try{
    ...
    }catch(e){ console.warn("[Geocoding] Nominatim también falló, usando coords de ciudad:", e); }
    if(coords){
  ...
  return(<div>
    <div style={{
      ...
      <div style={{flex:1,position:"relative",zIndex:1}}>
        <div style={{fontSize:17,fontWeight:800,color:"#fff",letterSpacing:"-0.3px",lineHeight:1.2}}>Paneles</div>
        <div style={{fontSize:12,color:"rgba(180,200,255,0.7)",marginTop:3,fontWeight:500}}>{paneles.length} paneles · {libre} libres · {ocup} ocupados</div>
      </div>
      <button onClick={openNew} style={{
        display:"inline-flex",alignItems:"center",gap:7,position:"relative",zIndex:1,
        background:"rgba(255,255,255,0.15)",border:"1px solid rgba(255,255,255,0.3)",borderRadius:12,padding:"10px 16px",
    ...
    <div style={{display:"grid",gridTemplateColumns:"1fr",gap:14}}>
          ...
          <div style={{position:"relative"}}>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1.4fr",gap:14,alignItems:"stretch",marginBottom:18}}>
              ...
              <div style={{
                ...
                <div style={{fontSize:22,fontWeight:800,color:T.green,marginTop:4,letterSpacing:"-0.02em",lineHeight:1}}>{fmt(p.precio)}</div>
              </div>
            </div>
            ...
            <div style={{display:"flex",alignItems:"center",gap:10}}>
              <button onClick={()=>openEdit(p)} style={{
                ...
                color:T.accent,fontWeight:800,fontSize:15,
                fontFamily:"inherit",
      ...
      {paneles.length===0&&<div style={{gridColumn:"1/-1",textAlign:"center",padding:60,color:"#6B7280",background:"#fff",borderRadius:22,border:"1px dashed #E5E7EB"}}>Sin paneles registrados · <button onClick={openNew} style={{color:T.accent,background:"none",border:"none",cursor:"pointer",touchAction:"manipulation",fontWeight:700}}>+ Agregar el primero</button></div>}
    </div>}
    {!loading && panelTotalPages > 1 && (
      <Pagination page={panelPage} totalPages={panelTotalPages} setPage={setPanelPage} total={panelTotal} pageSize={panelPageSize} dark={false}/>
    )}
    ...
    {modal&&(
      <Modal title={modal==="nuevo"?"➕ Nuevo Panel":"Editar Panel"} onClose={()=>{setModal(null);setGeocodeMsg("");onModalChange?.(false);}} onSave={guardar} saveLabel={saving?"Guardando...":"Guardar Panel ✓"}>
        {/* Emoji selector */}
        <div style={{marginBottom:16}}>
          <label style={{fontSize:11,fontWeight:700,color:T.muted,textTransform:"uppercase",letterSpacing:1,display:"block",marginBottom:8}}>Ícono</label>
          <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
            {EMOJIS.map(e=>(
              <button key={e} onClick={()=>setForm(f=>({...f,foto:e}))}
                style={{width:40,height:40,borderRadius:8,border:`2px solid ${form.foto===e?T.accent:T.border}`,background:form.foto===e?T.accent+"22":"transparent",fontSize:20,cursor:"pointer",touchAction:"manipulation"}}>
                {e}
              </button>
            ))}
          </div>
        ...
        <div style={{marginTop:14,paddingTop:14,borderTop:`1px solid ${T.border}`}}>
          <div style={{fontSize:11,fontWeight:700,color:T.muted,textTransform:"uppercase",letterSpacing:1,marginBottom:10}}>Información del panel</div>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
          ...
          <div style={{marginTop:12}}>
            <label style={{fontSize:11,fontWeight:700,color:T.muted,textTransform:"uppercase",letterSpacing:1,display:"block",marginBottom:5}}>Notas adicionales</label>
            <textarea
              value={form.notas||""}
              onChange={e=>setForm(f=>({...f,notas:e.target.value}))}
              placeholder="Observaciones, características especiales, acceso, etc."
              rows={3}
              style={{width:"100%",background:T.surface,border:`1px solid ${T.border}`,borderRadius:10,padding:"10px 13px",color:T.text,fontSize:14,outline:"none",fontFamily:"inherit",resize:"vertical",boxSizing:"border-box"}}
            />
          </div>
        </div>
        {/* Datos para depreciación / Capital */}
        <div style={{marginTop:14,paddingTop:14,borderTop:`1px solid ${T.border}`}}>
          <div style={{fontSize:11,fontWeight:700,color:T.muted,textTransform:"uppercase",letterSpacing:1,marginBottom:10}}>💰 Capital (depreciación)</div>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
        ...
        <div style={{marginTop:12}}>
          <label style={{fontSize:11,fontWeight:700,color:T.muted,textTransform:"uppercase",letterSpacing:1,display:"block",marginBottom:5}}>Dirección</label>
          <div style={{position:"relative"}}>
            <div style={{display:"flex",gap:8}}>
              <input
                ...
                autoComplete="off"
                style={{flex:1,background:T.surface,border:`1px solid ${showSug&&sugerencias.length>0?T.accent:T.border}`,borderRadius:showSug&&sugerencias.length>0?"10px 10px 0 0":"10px",padding:"10px 13px",color:T.text,fontSize:14,outline:"none",fontFamily:"inherit",transition:"border .2s"}}
              ...
              <button onClick={geocodificar} disabled={geocoding}
                style={{padding:"10px 16px",background:geocoding?T.border:T.accent,border:"none",borderRadius:10,color:T.white,fontWeight:700,fontSize:13,cursor:"pointer",touchAction:"manipulation",whiteSpace:"nowrap",flexShrink:0}}>
                {geocoding?"⏳":"📍 Ubicar"}
              </button>
            </div>
            {/* Dropdown sugerencias */}
            {showSug&&sugerencias.length>0&&(
              <div style={{position:"absolute",top:"100%",left:0,right:56,background:T.card,border:`1px solid ${T.accent}`,borderTop:"none",borderRadius:"0 0 10px 10px",zIndex:999,overflow:"hidden",boxShadow:`0 8px 24px rgba(0,0,0,0.5)`}}>
                {sugerencias.map((s,i)=>(
                  <div key={i} onMouseDown={()=>elegirSugerencia(s)}
                    style={{padding:"10px 14px",cursor:"pointer",touchAction:"manipulation",borderBottom:i<sugerencias.length-1?`1px solid ${T.border}`:"none",fontSize:13,color:T.text,display:"flex",alignItems:"center",gap:8,transition:"background .1s"}}
          ...
          {geocodeMsg&&<div style={{marginTop:7,fontSize:12,color:geocodeMsg.startsWith("✅")?T.green:geocodeMsg.startsWith("⚠️")?T.amber:T.red,padding:"6px 10px",background:(geocodeMsg.startsWith("✅")?T.green:geocodeMsg.startsWith("⚠️")?T.amber:T.red)+"12",borderRadius:8}}>{geocodeMsg}</div>}
          {!geocodeMsg&&form.direccion&&form.direccion.length>=3&&!showSug&&<div style={{marginTop:6,fontSize:11,color:T.muted}}>💡 Escribe para ver sugerencias automáticas o presiona 📍 para ubicar</div>}
        </div>
        {/* Mini mapa interactivo — mueve el pin y la dirección se actualiza sola */}
        {(form.lat||form.lng)&&(
          <div style={{marginTop:12}}>
            <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:6}}>
              <span style={{fontSize:11,fontWeight:700,color:T.muted,textTransform:"uppercase",letterSpacing:1}}>📌 Ajusta el pin en el mapa</span>
              <button onClick={()=>setForm(f=>({...f,lat:"",lng:"",direccion:""}))} style={{background:"none",border:"none",color:T.muted,cursor:"pointer",touchAction:"manipulation",fontSize:11}}>✕ Limpiar</button>
            </div>
            <MiniMapaPanel
              lat={Number(form.lat)}
              lng={Number(form.lng)}
            ...
            />
            <div style={{marginTop:6,padding:"8px 12px",background:T.green+"12",border:`1px solid ${T.green}33`,borderRadius:8,fontSize:12,color:T.green}}>
              📍 {form.direccion||`${form.lat}, ${form.lng}`}
            </div>
            <div style={{marginTop:4,fontSize:11,color:T.muted,textAlign:"center"}}>Arrastra el marcador para ajustar — la dirección se actualiza automáticamente</div>
          </div>
        )}
        {!form.lat&&!form.lng&&(
          <div style={{marginTop:10,padding:"9px 13px",background:T.accent+"10",borderRadius:10,fontSize:12,color:T.muted}}>
            💡 Escribe la dirección y presiona <strong style={{color:T.accent}}>📍 Ubicar</strong> — luego ajusta el pin en el mapa si necesitas precisión.
          </div>
        )}
      </Modal>
    )}
...
function Mapa({ paneles, clientes, contratos }: MapaProps){
  ...
  return(<div style={{margin:"-20px -16px",minHeight:"100%",background:"#070D1C",paddingBottom:"calc(100px + env(safe-area-inset-bottom))"}}>
    <div style={{ padding: "16px 16px 12px" }}>
      ...
      <div style={{ fontSize: 13, color: "rgba(255,255,255,0.45)", marginTop: 3 }}>OpenStreetMap · Clic en un pin para ver detalles</div>
    </div>

    {sinCoords.length>0&&(
      <div style={{background:T.amber+"12",border:`1px solid ${T.amber}33`,borderRadius:12,padding:"10px 16px",marginBottom:14,fontSize:13,color:T.amber,display:"flex",gap:10,alignItems:"center"}}>
    ...
    <Card style={{padding:0,overflow:"hidden",position:"relative",borderRadius:20}}>
      <div style={{position:"absolute",top:16,left:16,zIndex:500,background:T.dark,borderRadius:22,padding:"10px 16px",display:"flex",gap:14,boxShadow:"0 6px 20px rgba(0,0,0,0.25)"}}>
        {[["#3B82F6","Libre"],[T.red,"Ocupado"]].map(([c,l])=>(
      ...
      <div style={{position:"absolute",top:16,right:16,zIndex:500,background:T.dark,borderRadius:22,padding:"10px 16px",fontSize:13,color:T.white,fontWeight:600,boxShadow:"0 6px 20px rgba(0,0,0,0.25)"}}>
        {paneles.length} panel{paneles.length!==1?"es":""}
      </div>

      {!leafletReady&&(
        <div style={{height:560,display:"flex",alignItems:"center",justifyContent:"center",color:T.muted,gap:10,background:"#070C18"}}>
          <div style={{width:20,height:20,border:`2px solid ${T.border}`,borderTopColor:T.accent,borderRadius:"50%",animation:"spin .7s linear infinite"}}/>
          Cargando mapa...
        </div>
      )}
            ...
            {isOcup && contrato ? (
              <>
                ...
                <div style={{
                  ...
                  <div style={{flex:1,minWidth:0}}>
                    <div style={{fontSize:16,fontWeight:800,color:T.white,letterSpacing:"-0.3px",marginBottom:3}}>
                      Panel ocupado
                    </div>
                    <div style={{fontSize:13,color:"rgba(255,255,255,0.55)",fontWeight:500,whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>
                      Cliente: <span style={{color:T.white,fontWeight:700}}>{contrato.cliente?.empresa||"—"}</span>
                    </div>
              ...
              <div style={{
                ...
                {/* Info de precio */}
                <div style={{flex:1,minWidth:0}}>
                  <div style={{fontSize:16,fontWeight:800,color:T.white,letterSpacing:"-0.3px",marginBottom:5}}>
                    Panel disponible
                  </div>
                  <div style={{fontSize:13,color:"rgba(255,255,255,0.55)",fontWeight:500,lineHeight:1.4}}>
                    Precio de lista: <span style={{color:T.white,fontWeight:700}}>{fmt(p.precio)}/mes</span>
                  </div>
                  {p.precio>0 && (
                    <div style={{fontSize:13,color:"rgba(255,255,255,0.55)",fontWeight:500,lineHeight:1.4}}>
                ...
                <div style={{display:"flex",flexDirection:"column",gap:6,flexShrink:0}}>
                  <button
                    ...
                    style={{
                      ...
                      color:T.white,fontSize:18,fontWeight:600,
                      cursor:"pointer",touchAction:"manipulation",
                      display:"flex",alignItems:"center",justifyContent:"center",
                      lineHeight:1,
                    }}
                  ...
                  <button
                    ...
                    style={{
                      ...
                      background:"rgba(255,255,255,0.08)",
                      border:"1px solid rgba(255,255,255,0.08)",
                      color:T.white,fontSize:20,fontWeight:600,
                      cursor:"pointer",touchAction:"manipulation",
                      display:"flex",alignItems:"center",justifyContent:"center",
                      lineHeight:1,
                    }}
...
// ── CONTRATOS ────────────────────────────────────────────────────
function Contratos({ contratos, setContratos, paneles, clientes, loading, setTab, onModalChange }: ContratosProps){
  ...
  if(filtro==="Eliminados") fil=datosEliminados.sort((a,b)=>new Date(b.deletedAt||0).getTime()-new Date(a.deletedAt||0).getTime());
  ...
  const { page: cPage, setPage: setCPage, totalPages: cTotalPages,
          paginated: filPaged, total: filTotal, pageSize: cPageSize } = usePagination(fil, 8);
  ...
  const guardar=async()=>{
    ...
    try{
      ...
      if(modal==="nuevo"){
        ...
        catch(e){ console.warn("[Contrato] payloadFull rechazado, usando payloadSimple:", e); [r]=await fb.post("contratos",payloadSimple); }
        if(r) setContratos(p=>[...p,{...r,pagosMeses}]);
      } else {
        try{ [r]=await fb.patch("contratos",modal.id,payloadFull); }
        catch(e){ console.warn("[Contrato] patch payloadFull rechazado, usando payloadSimple:", e); [r]=await fb.patch("contratos",modal.id,payloadSimple); }
  ...
  const F=FieldGroup;
  const inp={width:"100%",background:T.bg,border:`1px solid ${T.border}`,borderRadius:10,padding:"10px 13px",color:T.text,fontSize:14,outline:"none",fontFamily:"inherit",boxSizing:"border-box"};
  ...
  return(<div>
    {modal&&(
      ...
      >
        <div style={{background:T.card,border:`1px solid ${T.border}`,borderRadius:"22px 22px 0 0",padding:"10px 20px 24px",width:"100%",maxHeight:"92vh",overflowY:"auto",overscrollBehavior:"none",paddingBottom:"calc(24px + env(safe-area-inset-bottom))",boxShadow:"0 -10px 40px rgba(0,0,0,0.45)"}}
        ...
        >
          ...
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:18}}>
            <span style={{fontSize:17,fontWeight:800,color:T.text}}>{modal==="nuevo"?"➕ Nuevo Contrato":"Editar Contrato"}</span>
            <button onClick={()=>{setModal(null);onModalChange?.(false);}} style={{background:T.border,border:"none",borderRadius:"50%",width:30,height:30,color:T.muted,cursor:"pointer",touchAction:"manipulation",fontSize:14,display:"flex",alignItems:"center",justifyContent:"center"}}>✕</button>
          </div>
            ...
            <F label={`Pagos por mes (${pagosMarcados}/${mesesForm.length} pagados)`}>
              <div style={{background:T.bg,borderRadius:12,padding:14,border:`1px solid ${T.border}`}}>
                <div style={{fontSize:11,color:T.muted,marginBottom:10}}>
                  💡 Toca cada mes para marcarlo como <strong style={{color:T.green}}>Pagado</strong> o <strong style={{color:T.red}}>Pendiente</strong>
                </div>
                  ...
                  {mesesForm.map((m,i)=>{
                    ...
                    return(
                      <button key={m.key} onClick={()=>toggleMesPago(m.key)}
                        ...
                        <div style={{fontSize:10,color:T.muted,marginBottom:3,fontWeight:600}}>Mes {i+1}</div>
                        <div style={{fontSize:12,fontWeight:700,color:pagado?T.green:T.muted,marginBottom:4}}>{m.label}</div>
                        <div style={{
                        ...
                        {form.monto&&<div style={{fontSize:10,color:T.muted,marginTop:3}}>{fmt(form.monto)}</div>}
                      </button>
                    );
                  })}
                </div>
                {mesesForm.length>0&&form.monto&&(
                  <div style={{marginTop:12,paddingTop:10,borderTop:`1px solid ${T.border}`,display:"flex",justifyContent:"space-between",fontSize:12}}>
                    <span style={{color:T.muted}}>Total cobrado:</span>
                    <span style={{fontWeight:800,color:T.green}}>{fmt(pagosMarcados*Number(form.monto))}</span>
                    <span style={{color:T.muted}}>Por cobrar:</span>
                    <span style={{fontWeight:800,color:T.red}}>{fmt((mesesForm.length-pagosMarcados)*Number(form.monto))}</span>
                  </div>
                )}
              </div>
            </F>
          ...
          {!form.inicio||!form.fin?
            <div style={{textAlign:"center",fontSize:12,color:T.muted,padding:"10px 0",marginBottom:16}}>
              📅 Selecciona las fechas para ver los meses de pago
            </div>:null
          }

          <div style={{display:"flex",gap:10,marginTop:8}}>
            <button onClick={()=>{setModal(null);onModalChange?.(false);}} style={{flex:1,padding:"12px",borderRadius:12,border:`1px solid ${T.border}`,background:"transparent",color:T.muted,fontWeight:700,fontSize:14,cursor:"pointer",touchAction:"manipulation"}}>Cancelar</button>
            <button onClick={guardar} disabled={saving} style={{flex:2,padding:"12px",borderRadius:12,border:"none",background:T.accent,color:"#fff",fontWeight:800,fontSize:14,cursor:"pointer",touchAction:"manipulation",opacity:saving?.6:1}}>
    ...
    <div style={{display:"flex",alignItems:"center",gap:14,marginBottom:18,padding:"20px 16px 0"}}>
      ...
      <button onClick={openNew} style={{
        ...
        background:T.accent,border:"none",borderRadius:13,padding:"10px 16px",
        color:"#fff",fontWeight:700,fontSize:13,cursor:"pointer",touchAction:"manipulation",
    ...
    <div style={{display:"flex",flexDirection:"column",gap:18,padding:"0 16px 32px"}}>
          ...
          <div style={{
            ...
            <div style={{padding:"18px 18px 14px 20px",position:"relative"}}>
              ...
              <div style={{position:"relative",display:"flex",alignItems:"center",gap:14}}>
                ...
                {/* Info */}
                <div style={{flex:1,minWidth:0}}>
                  <div style={{fontSize:17,fontWeight:800,color:T.white,letterSpacing:"-0.02em",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",marginBottom:3}}>{c.panel?.nombre||"Panel eliminado"}</div>
                ...
                <div style={{textAlign:"right",flexShrink:0}}>
                  <div style={{fontSize:22,fontWeight:900,color:T.white,letterSpacing:"-0.03em",lineHeight:1}}>{fmt(c.monto)}</div>
      ...
      {filTotal===0&&(
        <div style={{textAlign:"center",padding:48,background:"rgba(255,255,255,0.05)",borderRadius:22,border:"1px dashed rgba(255,255,255,0.12)"}}>
          ...
          {filtro!=="Eliminados"&&<button onClick={openNew} style={{marginTop:10,background:T.accent,border:"none",borderRadius:12,padding:"11px 20px",color:"#fff",fontWeight:700,fontSize:14,cursor:"pointer",touchAction:"manipulation"}}>+ Crear primer contrato</button>}
        </div>
      )}
      {/* Paginación de contratos */}
      <Pagination
        page={cPage} setPage={setCPage}
        totalPages={cTotalPages} total={filTotal} pageSize={cPageSize}
        dark={true}
      />
    </div>}
  </div>);
  ...
  <div style={{
    ...
    <div style={{ position:"relative", zIndex:2 }}>
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:12 }}>
        ...
        <div style={{ width:38, height:38, borderRadius:10, border:`1px solid ${accent===T.green?"rgba(16,185,129,0.4)":accent===T.amber?"rgba(245,158,11,0.4)":accent===T.red?"rgba(239,68,68,0.4)":"rgba(255,255,255,0.18)"}`, display:"flex", alignItems:"center", justifyContent:"center", flexShrink:0 }}>
          {icon}
        </div>
      </div>
...
const ICN = {
  users: <svg width="22" height="22" viewBox="0 0 24 24" fill="none"><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2" stroke={T.white} strokeWidth="1.6" strokeLinecap="round"/><circle cx="9" cy="7" r="4" stroke={T.white} strokeWidth="1.6"/><path d="M23 21v-2a4 4 0 0 0-3-3.87M16 3.13a4 4 0 0 1 0 7.75" stroke={T.white} strokeWidth="1.6" strokeLinecap="round"/></svg>,
  target: <svg width="22" height="22" viewBox="0 0 24 24" fill="none"><circle cx="12" cy="12" r="9" stroke={T.white} strokeWidth="1.6"/><circle cx="12" cy="12" r="5" stroke={T.white} strokeWidth="1.6"/><circle cx="12" cy="12" r="1.5" fill={T.white}/></svg>,
  bolt: <svg width="22" height="22" viewBox="0 0 24 24" fill="none"><polygon points="13 2 3 14 12 14 11 22 21 10 12 10 13 2" stroke={T.amber} strokeWidth="1.6" strokeLinejoin="round" fill="none"/></svg>,
  cash: <svg width="22" height="22" viewBox="0 0 24 24" fill="none"><rect x="3" y="6" width="18" height="13" rx="2" stroke={T.white} strokeWidth="1.6"/><circle cx="12" cy="12.5" r="2.4" stroke={T.white} strokeWidth="1.6"/></svg>,
  hourglass: <svg width="22" height="22" viewBox="0 0 24 24" fill="none"><path d="M6 2H18M6 22H18M6 2V8C6 10 9 12 12 12C15 12 18 10 18 8V2M6 22V16C6 14 9 12 12 12C15 12 18 14 18 16V22" stroke={T.white} strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round"/></svg>,
  down: <svg width="22" height="22" viewBox="0 0 24 24" fill="none"><path d="M12 5V19M5 12L12 19L19 12" stroke={T.red} strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round"/></svg>,
  up: <svg width="22" height="22" viewBox="0 0 24 24" fill="none"><path d="M3 17L9 11L13 15L21 7M21 7H15M21 7V13" stroke={T.green} strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round" fill="none"/></svg>,
  check: <svg width="22" height="22" viewBox="0 0 24 24" fill="none"><circle cx="12" cy="12" r="9" stroke={T.green} strokeWidth="1.6"/><polyline points="8 12 11 15 16 9" stroke={T.green} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" fill="none"/></svg>,
  archive: <svg width="22" height="22" viewBox="0 0 24 24" fill="none"><rect x="3" y="4" width="18" height="4" rx="1" stroke={T.white} strokeWidth="1.6"/><path d="M5 8V19A2 2 0 0 0 7 21H17A2 2 0 0 0 19 19V8" stroke={T.white} strokeWidth="1.6"/><line x1="10" y1="12" x2="14" y2="12" stroke={T.white} strokeWidth="1.6" strokeLinecap="round"/></svg>,
  panel: <svg width="22" height="22" viewBox="0 0 24 24" fill="none"><rect x="3" y="4" width="18" height="12" rx="1.5" stroke={T.white} strokeWidth="1.6"/><line x1="12" y1="16" x2="12" y2="20" stroke={T.white} strokeWidth="1.6"/><line x1="8" y1="20" x2="16" y2="20" stroke={T.white} strokeWidth="1.6" strokeLinecap="round"/></svg>,
  clock: <svg width="22" height="22" viewBox="0 0 24 24" fill="none"><circle cx="12" cy="12" r="9" stroke={T.amber} strokeWidth="1.6"/><polyline points="12 7 12 12 15 14" stroke={T.amber} strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round"/></svg>,
};
...
function CRM({ clientes, setClientes, contratos, loading, onModalChange }: CRMProps){
  ...
  const estadoColorCRM=(e)=>({
    "Activo":T.green,"Por vencer":T.white,"Inactivo":T.muted,
    "En contacto":T.accent,"Propuesta enviada":T.accent,"Frío":T.muted,"Perdido":T.red,
  }[e]||T.white);

  return (
    <div style={{ background: T.bg, margin: "-20px -16px", minHeight: "100%", paddingBottom: "calc(120px + env(safe-area-inset-bottom))" }}>

      {/* ── TOP STRIP AZUL ── */}
      <div style={{ background: "linear-gradient(180deg,#2563EB 0%,#2563EB 60%,#3B82F6 100%)", padding: "20px 20px 28px", borderBottomLeftRadius: 28, borderBottomRightRadius: 28, boxShadow: "0 8px 28px rgba(37,99,235,0.28)" }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 18 }}>
          ...
          <button onClick={openNew} style={{ display: "flex", alignItems: "center", gap: 7, padding: "10px 18px", background: "#fff", border: "none", borderRadius: 50, color: T.accent, fontWeight: 700, fontSize: 13, cursor: "pointer", touchAction: "manipulation", fontFamily: "inherit", boxShadow: "0 4px 14px rgba(0,0,0,0.15)" }}>
      ...
      <div style={{ padding: "14px 16px 10px", background: T.bg }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10, background: "#fff", border: "1px solid #E5E7EB", borderRadius: 14, padding: "11px 14px", boxShadow: "0 1px 4px rgba(0,0,0,0.06)" }}>
          ...
          {buscar && <button onClick={() => { setBuscar(""); setPage(1); }} style={{ background: "none", border: "none", cursor: "pointer", color: "#94A3B8", display: "flex", padding: 0 }}>
      ...
      {/* ── FILTROS ── */}
      <div style={{ display: "flex", gap: 7, padding: "8px 16px 12px", background: T.bg, overflowX: "auto" }}>
        {filters.map(f => {
          ...
          return (
            <button key={f} onClick={() => { setActiveFilter(f); setPage(1); }} style={{
              ...
              {dot && <div style={{ width: 6, height: 6, borderRadius: "50%", background: active ? "#fff" : dot }}/>}
              {f}
            </button>
          );
        })}
      ...
      {/* ── LISTA DE CONTACTOS ── */}
      {loading ? <div style={{ display: "flex", alignItems: "center", justifyContent: "center", height: 200, gap: 10, color: T.muted }}><div style={{ width: 20, height: 20, border: "2px solid #E5E7EB", borderTopColor: T.accent, borderRadius: "50%", animation: "spin .7s linear infinite" }}/> Cargando...</div> :
      <div style={{ padding: "10px 16px", display: "flex", flexDirection: "column", gap: 8 }}>
        {paginated.length === 0 ? (
          <div style={{ background: "#fff", borderRadius: 20, padding: "48px 20px", textAlign: "center", border: "1px solid #E5E7EB" }}>
            <div style={{ width: 52, height: 52, borderRadius: 16, background: "#EFF4FF", display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto 14px", color: T.accent }}>
            ...
            <div style={{ fontSize: 14, fontWeight: 600, color: T.text, marginBottom: 12 }}>Sin resultados</div>
            <button onClick={openNew} style={{ background: T.accent, border: "none", borderRadius: 50, padding: "9px 20px", color: "#fff", fontWeight: 700, fontSize: 13, cursor: "pointer", touchAction: "manipulation", fontFamily: "inherit" }}>+ Agregar contacto</button>
          </div>
        ) : paginated.map(r => {
              ...
              <div style={{ display: "flex" }}>
                ...
                <div style={{ flex: 1, padding: "14px 14px 12px" }}>
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: 11, marginBottom: 10 }}>
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ fontSize: 15, fontWeight: 700, color: T.text, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{r.empresa}</div>
                  ...
                  <div style={{ display: "flex", flexWrap: "wrap", gap: 5, marginBottom: 10 }}>
                    <span style={{ display: "inline-flex", alignItems: "center", gap: 4, background: "#F8FAFC", color: T.muted, borderRadius: 6, padding: "3px 8px", fontSize: 10, fontWeight: 600, border: "1px solid #E5E7EB" }}>
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                    ...
                    <div style={{ display: "flex", gap: 5 }}>
                      ...
                      <button onClick={() => openEdit(r)} style={{ width: 34, height: 34, borderRadius: 10, background: "#EFF4FF", border: "1px solid #BFDBFE", color: T.accent, cursor: "pointer", touchAction: "manipulation", display: "flex", alignItems: "center", justifyContent: "center" }}>
                      ...
                      <button onClick={() => eliminar(r.id)} style={{ width: 34, height: 34, borderRadius: 10, background: "#FEF2F2", border: "1px solid #FECACA", color: T.red, cursor: "pointer", touchAction: "manipulation", display: "flex", alignItems: "center", justifyContent: "center" }}>
        ...
        {totalPages > 1 && (
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "10px 4px" }}>
            ...
            <div style={{ display: "flex", gap: 5 }}>
              {page > 1 && <button onClick={() => setPage(p => p-1)} style={{ width: 30, height: 30, borderRadius: 8, border: "1px solid #E5E7EB", cursor: "pointer", background: "#fff", color: T.muted, display: "flex", alignItems: "center", justifyContent: "center" }}><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round"><polyline points="15 18 9 12 15 6"/></svg></button>}
              {Array.from({length: Math.min(totalPages,5)}, (_,i) => i+1).map(n => (
                <button key={n} onClick={() => setPage(n)} style={{ width: 30, height: 30, borderRadius: 8, border: `1px solid ${page===n?T.accent:T.border}`, cursor: "pointer", background: page===n?T.accent:"#fff", color: page===n?"#fff":T.muted, fontSize: 12, fontWeight: page===n?700:400, fontFamily: "inherit" }}>{n}</button>
              ))}
              {page < totalPages && <button onClick={() => setPage(p => Math.min(p+1,totalPages))} style={{ width: 30, height: 30, borderRadius: 8, border: "1px solid #E5E7EB", cursor: "pointer", background: "#fff", color: T.muted, display: "flex", alignItems: "center", justifyContent: "center" }}><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round"><polyline points="9 18 15 12 9 6"/></svg></button>}
            </div>
          </div>
        )}
      </div>}
          ...
          <div style={{ display: "flex", gap: 10, marginBottom: 18 }}>
            {["Cliente","Prospecto"].map(t => (
              <button key={t} onClick={() => setForm(f => ({ ...f, tipo: t, estado: t==="Cliente" ? "Activo" : "En contacto" }))}
                style={{ flex: 1, padding: 12, borderRadius: 12, border: `2px solid ${form.tipo===t ? tCol(t) : T.border}`, background: form.tipo===t ? tCol(t)+"22" : "transparent", color: form.tipo===t ? tCol(t) : T.muted, fontWeight: 700, cursor: "pointer", touchAction: "manipulation", fontFamily: "inherit", fontSize: 14 }}>
                {t}
              </button>
            ))}
          </div>
      ...
      <div style={{ margin: "0 16px 8px" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 10 }}>
          <div>
            ...
            <div style={{ fontSize: 11, color: T.muted, fontWeight: 500 }}>{pros.length} prospecto{pros.length !== 1 ? "s" : ""} · {propuestas.length} con propuesta</div>
          </div>
          <button onClick={openNew} style={{ display: "inline-flex", alignItems: "center", gap: 7, background: T.dark, border: "none", borderRadius: 50, padding: "10px 18px", color: "#fff", fontSize: 13, fontWeight: 700, cursor: "pointer", touchAction: "manipulation", fontFamily: "inherit" }}>
        ...
        {[
          ...
          return (
            <div key={estado} style={{ marginBottom: 8 }}>
              ...
              {grupo.map(p => (
                <div key={p.id} onClick={() => openEdit(p)} style={{ background: T.dark, borderRadius: 16, padding: "14px 16px", marginBottom: 7, display: "flex", alignItems: "center", gap: 12, cursor: "pointer", touchAction: "manipulation" }}>
        ...
        {pros.length === 0 && (
          <div style={{ background: T.dark, borderRadius: 20, padding: "28px 20px", textAlign: "center" }}>
            <div style={{ fontSize: 13, color: "rgba(255,255,255,0.35)", marginBottom: 14 }}>Sin prospectos aún. Agrega tu primer lead.</div>
            <button onClick={openNew} style={{ background: T.accent, border: "none", borderRadius: 50, padding: "10px 22px", color: "#fff", fontSize: 13, fontWeight: 700, cursor: "pointer", touchAction: "manipulation", fontFamily: "inherit" }}>+ Agregar prospecto</button>
          </div>
        )}
      </div>
...
function Proveedores({ proveedores, setProveedores, loading, onModalChange }: ProveedoresProps){
  ...
  const { page: pPage, setPage: setPPage, totalPages: pTotalPages,
          paginated: filtradosPaged, total: pTotal, pageSize: pPageSize } = usePagination(filtrados, 10);

  // Sin morado/amarillo — todas las categorías en azul/blanco (solo verde=ingreso, rojo=gasto)
  const catColor={
    "Impresión":T.accent, "Materiales":T.accent, "Mantenimiento":T.white,
    "Servicios":T.green, "Transporte":T.cyan, "Tecnología":T.accent, "Otro":T.muted,
  };
  ...
  return(<div>
    ...
    {modal&&(
      ...
      >
        ...
        >
          ...
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:18}}>
            <span style={{fontSize:17,fontWeight:800,color:T.text}}>{modal==="nuevo"?"➕ Nuevo Proveedor":"Editar Proveedor"}</span>
            <button onClick={()=>{setModal(null);onModalChange?.(false);}} style={{background:T.border,border:"none",borderRadius:"50%",width:30,height:30,color:T.muted,cursor:"pointer",touchAction:"manipulation",fontSize:14,display:"flex",alignItems:"center",justifyContent:"center"}}>✕</button>
          </div>

          {(()=>{
            const inpS={width:"100%",background:T.bg,border:`1px solid ${T.border}`,borderRadius:10,padding:"10px 13px",color:T.text,fontSize:16,outline:"none",fontFamily:"inherit",boxSizing:"border-box"};
    ...
    {/* ── HEADER ── */}
    <div style={{display:"flex",alignItems:"center",gap:14,marginBottom:18,background:"linear-gradient(135deg,#0F1729 0%,#1E3A8A 100%)",borderRadius:18,padding:"16px 20px",boxShadow:"0 6px 24px rgba(15,23,41,0.32)",position:"relative",overflow:"hidden"}}>
      <div style={{position:"absolute",top:0,left:0,right:0,height:"50%",background:"linear-gradient(180deg,rgba(255,255,255,0.06) 0%,transparent 100%)",borderRadius:"18px 18px 0 0",pointerEvents:"none"}}/>
      <div style={{width:42,height:42,borderRadius:12,flexShrink:0,position:"relative",zIndex:1,background:"rgba(255,255,255,0.14)",border:"1px solid rgba(255,255,255,0.22)",display:"flex",alignItems:"center",justifyContent:"center",color:"#fff"}}>
        <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 0 0-3-3.87"/><path d="M16 3.13a4 4 0 0 1 0 7.75"/></svg>
      </div>
      <div style={{flex:1,minWidth:0,position:"relative",zIndex:1}}>
        <div style={{fontSize:17,fontWeight:800,color:"#fff",letterSpacing:"-0.3px",lineHeight:1.2}}>Proveedores</div>
        <div style={{fontSize:12,color:"rgba(180,200,255,0.7)",marginTop:3,fontWeight:500}}>{proveedores.length} proveedor{proveedores.length!==1?"es":""} registrado{proveedores.length!==1?"s":""}</div>
      </div>
      <button onClick={openNew} style={{display:"inline-flex",alignItems:"center",gap:7,position:"relative",zIndex:1,background:"rgba(255,255,255,0.15)",border:"1px solid rgba(255,255,255,0.3)",borderRadius:12,padding:"10px 16px",color:"#fff",fontWeight:700,fontSize:13,cursor:"pointer",touchAction:"manipulation",flexShrink:0}}>
        <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round"><path d="M12 5v14M5 12h14"/></svg>
        Nuevo
      </button>
    </div>
      ...
      filtrados.length===0?(
        <div style={{textAlign:"center",padding:60,color:"#6B7280",background:"#fff",borderRadius:22,border:"1px dashed #E5E7EB"}}>
          {proveedores.length===0
            ?<>Sin proveedores registrados · <button onClick={openNew} style={{color:T.accent,background:"none",border:"none",cursor:"pointer",touchAction:"manipulation",fontWeight:700}}>+ Agregar el primero</button></>
                ...
                <div style={{position:"relative",zIndex:2}}>
                  ...
                  <div style={{display:"flex",alignItems:"flex-start",gap:12,marginBottom:10}}>
                    <div style={{
                      ...
                      <svg width="26" height="32" viewBox="0 0 24 28" fill="none">
                        ...
                        <text x="5" y="5.6" fontSize="2.2" fontWeight="900" fill={T.accent} fontFamily="sans-serif">PROVEEDOR</text>
                        <rect x="5" y="9" width="14" height="6" rx="0.6" fill="#EFF6FF" stroke={T.accent} strokeWidth="0.5"/>
                    ...
                    <div style={{flex:1,minWidth:0}}>
                      <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:6,flexWrap:"wrap"}}>
                        <span style={{fontSize:15,fontWeight:900,color:T.white,letterSpacing:"-0.2px",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",maxWidth:"100%"}}>{p.empresa}</span>
                      ...
                      <div style={{fontSize:13,color:T.white,fontWeight:600,whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis",marginBottom:2}}>
                        {p.contacto||"—"}
                      </div>
                  ...
                  {p.producto&&(
                    <div style={{
                      ...
                      fontSize:13,color:T.white,lineHeight:1.4,
                      border:"1px solid rgba(255,255,255,0.08)",
                    }}>
                  ...
                  {(p.celular||p.email)&&(
                    <div style={{display:"flex",flexWrap:"wrap",gap:6,marginBottom:10}}>
                      {p.celular&&<span style={{fontSize:11,color:T.white,background:"rgba(255,255,255,0.05)",padding:"4px 10px",borderRadius:8,border:"1px solid rgba(255,255,255,0.08)"}}>{p.celular}</span>}
                      {p.email&&<span style={{fontSize:11,color:T.white,background:"rgba(255,255,255,0.05)",padding:"4px 10px",borderRadius:8,border:"1px solid rgba(255,255,255,0.08)",maxWidth:200,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{p.email}</span>}
                    </div>
                  )}
    ...
    {/* Paginación de proveedores */}
    <Pagination
      page={pPage} setPage={setPPage}
      totalPages={pTotalPages} total={pTotal} pageSize={pPageSize}
      dark={true}
    />
  </div>);
}
...
const fFmt  = (n) => `S/ ${Number(n||0).toLocaleString("es-PE",{minimumFractionDigits:2,maximumFractionDigits:2})}`;
const fFmtK = (n) => { const v=Number(n||0); return v>=1e6?`S/ ${(v/1e6).toFixed(1)}M`:v>=1e3?`S/ ${(v/1e3).toFixed(1)}k`:`S/ ${Math.round(v)}`; };
const fPct  = (n,d=1) => `${n>=0?"+":""}${Number(n||0).toFixed(d)}%`;
const fMesL = (m) => { if(!m)return"—"; const [y,mo]=m.split("-"); return["Ene","Feb","Mar","Abr","May","Jun","Jul","Ago","Sep","Oct","Nov","Dic"][+mo-1]+"'"+y.slice(2); };
const fMesActual = () => { const d=new Date(); return `${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,"0")}`; };
const fBuildMeses = (desde,hasta) => {
  const ms=[],cur=new Date(desde+"-01"),end=new Date(hasta+"-01");
  while(cur<=end){ ms.push(`${cur.getFullYear()}-${String(cur.getMonth()+1).padStart(2,"0")}`); cur.setMonth(cur.getMonth()+1); }
  return ms;
};
const fNMesesAtras = (n) => { const d=new Date(); d.setDate(1); d.setMonth(d.getMonth()-(n-1)); return `${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,"0")}`; };

// ── Sub-components ─────────────────────────────────────────────────
function FCard({ children, style = {} }: FCardProps) {
  return <div style={{ background:F.card, border:`1px solid ${F.border}`, borderRadius:20, overflow:"hidden", boxShadow:"0 4px 24px rgba(0,0,0,0.35)", ...style }}>{children}</div>;
}

function FSparkline({ data, color = F.blue, height = 40 }: FSparklineProps) {
  ...
  const gid=`fsp${color.replace(/[^a-z0-9]/gi,"")}${Math.floor(Math.random()*999)}`;
  return (
...
function FBar({ pct, color = F.blue }: FBarProps) {
  return <div style={{height:5,borderRadius:99,background:"rgba(255,255,255,0.07)",overflow:"hidden"}}><div style={{height:"100%",width:`${Math.min(pct,100)}%`,background:color,borderRadius:99,transition:"width .6s ease"}}/></div>;
}

function FBadge({ label, color }: FBadgeProps) {
  return <span style={{display:"inline-flex",alignItems:"center",background:`${color}18`,border:`1px solid ${color}38`,borderRadius:99,padding:"2px 8px",fontSize:10,fontWeight:700,color,letterSpacing:0.3,whiteSpace:"nowrap"}}>{label}</span>;
}

// ── Financial Engine ────────────────────────────────────────────────
function useOOHEngine(paneles,contratos,gastos,clientes,desde,hasta,ciudadF,clienteF,panelF) {
  return useMemo(() => {
    const meses = fBuildMeses(desde, hasta);
    const mesHoy = fMesActual();

    const contrActivos = contratos.filter(c =>
      !c.deleted && c.inicio && c.fin &&
      c.inicio.slice(0,7) <= hasta && c.fin.slice(0,7) >= desde
    );

    const ingPorContrato = (c) =>
      meses.filter(m => c.inicio.slice(0,7) <= m && c.fin.slice(0,7) >= m)
           .reduce((s) => s + Number(c.monto||0), 0);

    const gastosFil = gastos.filter(g =>
      !g.deleted && g.fecha &&
      g.fecha.slice(0,7) >= desde && g.fecha.slice(0,7) <= hasta
    );

    // Separar gastos directos vs generales
    const gpPanel = {};
    let gastoGen = 0;
    for (const g of gastosFil) {
      if (g.panel_id) { gpPanel[g.panel_id] = (gpPanel[g.panel_id]||0)+Number(g.monto||0); }
      else { gastoGen += Number(g.monto||0); }
    }
    const gastoDirectoTotal = gastosFil.filter(g=>g.panel_id).reduce((s,g)=>s+Number(g.monto||0),0);

    const panelesFil = paneles.filter(p => {
      if(p.deleted) return false;
      if(ciudadF!=="all" && p.ciudad!==ciudadF) return false;
      if(panelF!=="all" && p.id!==panelF)       return false;
      return true;
    });

    const panConIng = panelesFil.filter(p => contrActivos.some(c=>c.panel_id===p.id));
    const nConIng   = Math.max(panConIng.length,1);

    const panelMets = panelesFil.map(p => {
      let pCtrs = contrActivos.filter(c=>c.panel_id===p.id);
      if(clienteF!=="all") pCtrs = pCtrs.filter(c=>c.cliente_id===clienteF);

      const ingresos    = pCtrs.reduce((s,c)=>s+ingPorContrato(c),0);
      const gasDir      = gpPanel[p.id]||0;
      const gasGenProp  = panConIng.some(x=>x.id===p.id) ? gastoGen/nConIng : 0;
      const gasTotal    = gasDir+gasGenProp;
      const utilidad    = ingresos-gasTotal;
      const margen      = ingresos>0?(utilidad/ingresos)*100:0;
      const roi         = gasTotal>0?(utilidad/gasTotal)*100:(ingresos>0?999:0);

      const mOcup = meses.filter(m=>pCtrs.some(c=>c.inicio.slice(0,7)<=m&&c.fin.slice(0,7)>=m)).length;
      const ocupacion = meses.length>0?(mOcup/meses.length)*100:0;

      const cActivosN = contrActivos.filter(c=>
        c.panel_id===p.id && c.inicio.slice(0,7)<=mesHoy && c.fin.slice(0,7)>=mesHoy
      ).length;

      const historial = meses.map(m => {
        const ing = contrActivos.filter(c=>c.panel_id===p.id&&c.inicio.slice(0,7)<=m&&c.fin.slice(0,7)>=m)
                                .reduce((s,c)=>s+Number(c.monto||0),0);
        const gas = gastosFil.filter(g=>g.panel_id===p.id&&g.fecha?.slice(0,7)===m)
                             .reduce((s,g)=>s+Number(g.monto||0),0);
        return { mes:m, ingresos:ing, gastos:gas, utilidad:ing-gas };
      });

      const cliCounts={};
      for(const c of pCtrs) cliCounts[c.cliente_id]=(cliCounts[c.cliente_id]||0)+ingPorContrato(c);
      const topCliId = Object.entries(cliCounts).sort((a,b)=>b[1]-a[1])[0]?.[0];
      const clientePpal = clientes.find(c=>c.id===topCliId)?.empresa||"—";

      return { panel:p, ingresos, gastos:gasTotal, gasDir, gasGenProp, utilidad, margen,
               roi:isFinite(roi)?roi:999, ocupacion, cActivosN, historial, clientePpal, ctrs:pCtrs };
    });

    const totalIng  = panelMets.reduce((s,p)=>s+p.ingresos,0);
    const totalGas  = gastoDirectoTotal+gastoGen;
    const totalUtil = totalIng-totalGas;
    const margenEmp = totalIng>0?(totalUtil/totalIng)*100:0;
    const roiEmp    = totalGas>0?(totalUtil/totalGas)*100:0;
    const ranking   = [...panelMets].sort((a,b)=>b.utilidad-a.utilidad);
    const topPanel  = ranking[0]||null;
    const worstPanel= [...panelMets].filter(p=>p.ingresos>0).sort((a,b)=>a.utilidad-b.utilidad)[0]||null;
    const vacios    = panelMets.filter(p=>p.ingresos===0);
    const proy12m   = meses.length>0?(totalIng/meses.length)*12:0;

    const ciudades    = [...new Set(panelesFil.map(p=>p.ciudad).filter(Boolean))];
    const cityMets    = ciudades.map(ciudad => {
      const pm = panelMets.filter(p=>p.panel.ciudad===ciudad);
      const ing=pm.reduce((s,p)=>s+p.ingresos,0), gas=pm.reduce((s,p)=>s+p.gastos,0);
      return { ciudad, ingresos:ing, gastos:gas, utilidad:ing-gas, margen:ing>0?((ing-gas)/ing)*100:0, n:pm.length, pm };
    }).sort((a,b)=>b.ingresos-a.ingresos);

    const totalIngMax = Math.max(totalIng,1);
    const cliIds  = [...new Set(contrActivos.map(c=>c.cliente_id).filter(Boolean))];
    const cliMets = cliIds.map(id => {
      const cl    = clientes.find(c=>c.id===id)||{id,empresa:"Desconocido",estado:""};
      const ctrs  = contrActivos.filter(c=>c.cliente_id===id);
      const ing   = ctrs.reduce((s,c)=>s+ingPorContrato(c),0);
      return { cl, ingresos:ing, nCtrs:ctrs.length, nPaneles:new Set(ctrs.map(c=>c.panel_id)).size,
               part:(ing/totalIngMax)*100 };
    }).sort((a,b)=>b.ingresos-a.ingresos);

    const histEmp = meses.map(m => {
      const ing = contrActivos.filter(c=>c.inicio.slice(0,7)<=m&&c.fin.slice(0,7)>=m)
                              .reduce((s,c)=>s+Number(c.monto||0),0);
      const gas = gastosFil.filter(g=>g.fecha?.slice(0,7)===m).reduce((s,g)=>s+Number(g.monto||0),0);
      return { mes:m, ingresos:ing, gastos:gas, utilidad:ing-gas };
    });

    return { panelMets, ranking, totalIng, totalGas, totalUtil, margenEmp, roiEmp,
             cityMets, cliMets, topPanel, worstPanel, vacios, proy12m,
             gastoGen, gastoDirectoTotal, histEmp, meses };
  }, [paneles,contratos,gastos,clientes,desde,hasta,ciudadF,clienteF,panelF]);
}

// ── Panel Detail Drawer ────────────────────────────────────────────
function FPanelDrawer({ pm, clientes, onClose }: FPanelDrawerProps) {
  if(!pm) return null;
  const { panel, ingresos, gastos, utilidad, margen, roi, ocupacion, cActivosN,
          historial, gasDir, gasGenProp, ctrs } = pm;
  const margenColor = margen>=40?F.green:margen>=20?F.gold:F.red;
  return (
    <div onClick={e=>e.target===e.currentTarget&&onClose()}
      style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.72)",zIndex:900,
              display:"flex",alignItems:"flex-end",justifyContent:"center"}}>
      <div style={{width:"100%",maxWidth:540,background:F.card2,borderRadius:"24px 24px 0 0",
        maxHeight:"88vh",display:"flex",flexDirection:"column",
        boxShadow:"0 -16px 60px rgba(0,0,0,0.7)"}}>
        <div style={{display:"flex",justifyContent:"center",padding:"12px 0 6px"}}>
          <div style={{width:36,height:4,borderRadius:2,background:F.border2}}/>
        </div>
        {/* Header */}
        <div style={{padding:"4px 20px 16px",borderBottom:`1px solid ${F.border}`}}>
          <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:8}}>
            <span style={{fontSize:26}}>{panel.emoji||"📡"}</span>
            <div style={{flex:1}}>
              <div style={{fontSize:17,fontWeight:800,color:F.text}}>{panel.nombre}</div>
              <div style={{fontSize:12,color:F.muted}}>{panel.ciudad} · {panel.tipo}</div>
            </div>
            <button onClick={onClose} style={{background:"rgba(255,255,255,0.08)",border:"none",borderRadius:10,width:32,height:32,cursor:"pointer",color:F.muted,fontSize:16,display:"flex",alignItems:"center",justifyContent:"center",touchAction:"manipulation",fontFamily:"inherit"}}>✕</button>
          </div>
          <div style={{display:"flex",gap:5,flexWrap:"wrap"}}>
            <FBadge label={`${ocupacion.toFixed(0)}% ocupado`} color={ocupacion>=80?F.green:ocupacion>=50?F.gold:F.red}/>
            <FBadge label={`${cActivosN} contrato${cActivosN!==1?"s":""} activo${cActivosN!==1?"s":""}`} color={F.blue}/>
            <FBadge label={panel.estado||"—"} color={F.muted}/>
          </div>
        </div>
        {/* Body */}
        <div style={{overflowY:"auto",flex:1,padding:"16px 20px",paddingBottom:"calc(20px + env(safe-area-inset-bottom))"}}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:12}}>
            {[
              {l:"Ingresos",  v:fFmtK(ingresos),      c:F.blue},
              {l:"Gastos",    v:fFmtK(gastos),         c:F.red},
              {l:"Utilidad",  v:fFmtK(utilidad),       c:utilidad>=0?F.green:F.red},
              {l:"Margen",    v:`${margen.toFixed(1)}%`,c:margenColor},
            ].map(k=>(
              <div key={k.l} style={{background:F.card,border:`1px solid ${F.border}`,borderRadius:14,padding:"12px 14px"}}>
                <div style={{fontSize:9,fontWeight:700,color:F.muted,textTransform:"uppercase",letterSpacing:1.2,marginBottom:5}}>{k.l}</div>
                <div style={{fontSize:20,fontWeight:900,color:k.c,letterSpacing:"-0.5px"}}>{k.v}</div>
              </div>
            ))}
          </div>
          {/* ROI + gastos */}
          <FCard style={{padding:"12px 14px",marginBottom:10}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:6}}>
              <span style={{fontSize:10,fontWeight:700,color:F.muted,textTransform:"uppercase",letterSpacing:1.2}}>ROI del panel</span>
              <span style={{fontSize:20,fontWeight:900,color:roi>=50?F.green:roi>=0?F.gold:F.red}}>
                {roi>=999?"∞":`${roi.toFixed(1)}%`}
              </span>
            </div>
            <div style={{fontSize:11,color:F.muted}}>
              <span style={{color:F.text,fontWeight:600}}>{fFmt(gasDir)}</span> directo
              {" · "}
              <span style={{color:F.muted2,fontWeight:600}}>{fFmt(gasGenProp)}</span> overhead
            </div>
          </FCard>
          {/* Sparklines */}
          <FCard style={{padding:"12px 14px",marginBottom:10}}>
            <div style={{fontSize:10,fontWeight:700,color:F.muted,textTransform:"uppercase",letterSpacing:1.2,marginBottom:6}}>Ingresos por mes</div>
            <FSparkline data={historial.map(h=>h.ingresos)} color={F.blue} height={42}/>
            <div style={{fontSize:10,fontWeight:700,color:F.muted,textTransform:"uppercase",letterSpacing:1.2,marginBottom:6,marginTop:12}}>Utilidad por mes</div>
            <FSparkline data={historial.map(h=>h.utilidad)} color={historial.every(h=>h.utilidad>=0)?F.green:F.gold} height={34}/>
          </FCard>
          {/* Historial */}
          <FCard style={{padding:"12px 14px",marginBottom:10}}>
            <div style={{fontSize:10,fontWeight:700,color:F.muted,textTransform:"uppercase",letterSpacing:1.2,marginBottom:8}}>Historial financiero</div>
            {historial.slice().reverse().map(h=>(
              <div key={h.mes} style={{display:"flex",alignItems:"center",gap:8,padding:"7px 8px",background:"rgba(255,255,255,0.03)",borderRadius:9,marginBottom:4}}>
                <div style={{fontSize:10,fontWeight:700,color:F.muted2,width:50,flexShrink:0}}>{fMesL(h.mes)}</div>
                <div style={{flex:1}}><FBar pct={h.ingresos>0?100:0} color={h.ingresos>0?F.blue:"rgba(255,255,255,0.08)"}/></div>
                <div style={{fontSize:11,fontWeight:700,color:F.text,width:62,flexShrink:0,textAlign:"right"}}>{fFmtK(h.ingresos)}</div>
                <div style={{fontSize:10,color:h.utilidad>=0?F.green:F.red,width:42,flexShrink:0,textAlign:"right"}}>
                  {h.ingresos>0?fPct((h.utilidad/h.ingresos)*100):"—"}
                </div>
              </div>
            ))}
          </FCard>
          {/* Contratos */}
          {ctrs.length>0&&(
            <FCard style={{padding:"12px 14px"}}>
              <div style={{fontSize:10,fontWeight:700,color:F.muted,textTransform:"uppercase",letterSpacing:1.2,marginBottom:8}}>Contratos en período</div>
              {ctrs.map(c=>{
                const cli=clientes.find(x=>x.id===c.cliente_id);
                return (
                  <div key={c.id} style={{display:"flex",alignItems:"center",gap:8,padding:"7px 8px",background:"rgba(255,255,255,0.03)",borderRadius:9,marginBottom:4}}>
                    <div style={{flex:1}}>
                      <div style={{fontSize:12,fontWeight:700,color:F.text}}>{cli?.empresa||"—"}</div>
                      <div style={{fontSize:10,color:F.muted}}>{c.inicio} → {c.fin}</div>
                    </div>
                    <div style={{fontSize:13,fontWeight:800,color:F.blue}}>{fFmtK(Number(c.monto||0))}<span style={{fontSize:10,color:F.muted}}>/mes</span></div>
                  </div>
                );
              })}
            </FCard>
...
// ── Vista Empresa ───────────────────────────────────────────────────
function FVistaEmpresa({ eng }: FVistaEmpresaProps) {
  const { totalIng, totalGas, totalUtil, margenEmp, roiEmp, topPanel, worstPanel,
          vacios, proy12m, ranking, gastoGen, gastoDirectoTotal, histEmp, meses } = eng;
  const margenColor = margenEmp>=40?F.green:margenEmp>=20?F.gold:F.red;
  const sparkIng = histEmp.map(h=>h.ingresos);
  return (
    <div>
      {/* Hero utilidad */}
      <FCard style={{padding:"22px 18px 16px",marginBottom:10}}>
        <div style={{fontSize:10,fontWeight:700,color:F.muted,textTransform:"uppercase",letterSpacing:2,marginBottom:8}}>Resultado neto · {meses.length} mes{meses.length!==1?"es":""}</div>
        <div style={{fontSize:44,fontWeight:900,letterSpacing:"-2px",lineHeight:1,marginBottom:8,color:totalUtil>=0?F.green:F.red}}>
          {fFmtK(totalUtil)}
        </div>
        <div style={{display:"flex",gap:6,flexWrap:"wrap",marginBottom:14}}>
          <FBadge label={`Margen ${margenEmp.toFixed(1)}%`} color={margenColor}/>
          <FBadge label={`ROI ${roiEmp.toFixed(1)}%`} color={F.blue}/>
          {vacios.length>0&&<FBadge label={`${vacios.length} panel${vacios.length!==1?"es":""} sin ingresos`} color={F.gold}/>}
        </div>
        <FSparkline data={sparkIng} color={F.blue} height={46}/>
      </FCard>
      {/* 4 KPI cards */}
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:10}}>
        {[
          {l:"Ingresos totales",v:fFmtK(totalIng),  ic:"💰",c:F.blue},
          {l:"Gastos totales",  v:fFmtK(totalGas),  ic:"📉",c:F.red},
          {l:"Proyección 12M",  v:fFmtK(proy12m),   ic:"📈",c:F.gold},
          {l:"Paneles sin ingr.",v:String(vacios.length),ic:"📡",c:F.purple},
        ].map(k=>(
          <FCard key={k.l} style={{padding:"14px 14px"}}>
            <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:10}}>
              <div style={{width:32,height:32,borderRadius:9,flexShrink:0,background:`${k.c}18`,border:`1px solid ${k.c}28`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:14}}>{k.ic}</div>
              <div style={{fontSize:9,fontWeight:700,color:F.muted,textTransform:"uppercase",letterSpacing:1.3,lineHeight:1.2}}>{k.l}</div>
            </div>
            <div style={{fontSize:22,fontWeight:900,color:F.text,letterSpacing:"-0.8px"}}>{k.v}</div>
          </FCard>
        ))}
      </div>
      {/* Bar chart */}
      {histEmp.length>1&&(
        <FCard style={{padding:"14px 14px 8px",marginBottom:10}}>
          <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:10}}>
            <div style={{fontSize:10,fontWeight:700,color:F.muted,textTransform:"uppercase",letterSpacing:1.5}}>Ingresos vs Gastos</div>
            <div style={{display:"flex",gap:10}}>
              {[{c:F.blue,l:"Ingresos"},{c:F.red,l:"Gastos"}].map(x=>(
                <div key={x.l} style={{display:"flex",alignItems:"center",gap:4}}>
                  <div style={{width:7,height:7,borderRadius:2,background:x.c}}/>
                  <span style={{fontSize:9,color:F.muted}}>{x.l}</span>
                </div>
              ))}
            </div>
          </div>
          {/* SVG Bar chart */}
          {(() => {
            const data=histEmp, H=88, W=320;
            const maxV=Math.max(...data.flatMap(d=>[d.ingresos,d.gastos||0]),1);
            const bw=Math.floor((W-data.length*6)/(data.length*2));
            return (
              <svg viewBox={`0 0 ${W} ${H+20}`} width="100%" height={H+20} style={{display:"block"}}>
                {data.map((d,i)=>{
                  const x=i*(bw*2+7)+4;
                  const hI=Math.max((d.ingresos/maxV)*H,2);
                  const hG=Math.max((d.gastos/maxV)*H,d.gastos>0?2:0);
                  return (
                    <g key={d.mes}>
                      <rect x={x}       y={H-hI} width={bw} height={hI} rx={3} fill={F.blue} opacity={0.82}/>
                      {hG>0&&<rect x={x+bw+3} y={H-hG} width={bw} height={hG} rx={3} fill={F.red} opacity={0.65}/>}
                      <circle cx={x+bw+1.5} cy={H-hI-5} r={2.5} fill={d.utilidad>=0?F.green:F.red}/>
                      <text x={x+bw} y={H+14} textAnchor="middle" fill={F.muted} fontSize={8} fontFamily="inherit">{fMesL(d.mes)}</text>
                    </g>
                  );
                })}
              </svg>
      ...
      <FCard style={{padding:"14px",marginBottom:10}}>
        <div style={{fontSize:10,fontWeight:700,color:F.muted,textTransform:"uppercase",letterSpacing:1.5,marginBottom:10}}>Estructura de gastos</div>
        {[
          {l:"Directos a paneles",v:gastoDirectoTotal,c:F.blue,p:totalGas>0?(gastoDirectoTotal/totalGas)*100:0},
          {l:"Generales empresa", v:gastoGen,         c:F.red, p:totalGas>0?(gastoGen/totalGas)*100:0},
        ].map(r=>(
          <div key={r.l} style={{marginBottom:10}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:4}}>
              <span style={{fontSize:12,color:F.text,fontWeight:600}}>{r.l}</span>
              <div style={{display:"flex",gap:8,alignItems:"center"}}>
                <span style={{fontSize:10,color:F.muted}}>{r.p.toFixed(0)}%</span>
                <span style={{fontSize:13,fontWeight:800,color:r.c}}>{fFmtK(r.v)}</span>
              </div>
            </div>
            <FBar pct={r.p} color={r.c}/>
          </div>
        ))}
      </FCard>
      {/* Top/Worst */}
      {(topPanel||worstPanel)&&(
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:10}}>
          {topPanel&&(
            <FCard style={{padding:"12px"}}>
              <div style={{fontSize:9,fontWeight:700,color:F.green,textTransform:"uppercase",letterSpacing:1,marginBottom:5}}>🏆 Más rentable</div>
              <div style={{fontSize:13,fontWeight:800,color:F.text,marginBottom:2,lineHeight:1.2}}>{topPanel.panel.nombre}</div>
              <div style={{fontSize:10,color:F.muted,marginBottom:7}}>{topPanel.panel.ciudad}</div>
              <div style={{fontSize:18,fontWeight:900,color:F.green}}>{fFmtK(topPanel.utilidad)}</div>
              <div style={{fontSize:10,color:F.muted}}>{topPanel.margen.toFixed(1)}% margen</div>
            </FCard>
          )}
          {worstPanel&&(
            <FCard style={{padding:"12px"}}>
              <div style={{fontSize:9,fontWeight:700,color:F.gold,textTransform:"uppercase",letterSpacing:1,marginBottom:5}}>⚠️ Menor margen</div>
              <div style={{fontSize:13,fontWeight:800,color:F.text,marginBottom:2,lineHeight:1.2}}>{worstPanel.panel.nombre}</div>
              <div style={{fontSize:10,color:F.muted,marginBottom:7}}>{worstPanel.panel.ciudad}</div>
              <div style={{fontSize:18,fontWeight:900,color:worstPanel.utilidad>=0?F.gold:F.red}}>{fFmtK(worstPanel.utilidad)}</div>
              <div style={{fontSize:10,color:F.muted}}>{worstPanel.margen.toFixed(1)}% margen</div>
            </FCard>
          )}
        </div>
      )}
      ...
      {ranking.length>0&&(
        <FCard style={{padding:"14px"}}>
          <div style={{fontSize:10,fontWeight:700,color:F.muted,textTransform:"uppercase",letterSpacing:1.5,marginBottom:10}}>Ranking rentabilidad</div>
          {ranking.slice(0,5).map((pm,i)=>{
            const pct=totalIng>0?(pm.ingresos/totalIng)*100:0;
            const rankColor=i===0?F.gold:i===1?"#94A3B8":i===2?"#B45309":F.blue;
            return (
              <div key={pm.panel.id} style={{display:"flex",alignItems:"center",gap:8,marginBottom:10}}>
                <div style={{width:24,height:24,borderRadius:7,flexShrink:0,background:`${rankColor}18`,border:`1px solid ${rankColor}38`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:11,fontWeight:900,color:rankColor}}>{i+1}</div>
                <div style={{flex:1,minWidth:0}}>
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:3}}>
                    <span style={{fontSize:12,fontWeight:700,color:F.text,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{pm.panel.nombre}</span>
                    <span style={{fontSize:12,fontWeight:800,color:pm.utilidad>=0?F.green:F.red,flexShrink:0,marginLeft:6}}>{fFmtK(pm.utilidad)}</span>
                  </div>
                  <FBar pct={pct} color={pm.utilidad>=0?F.green:F.red}/>
                  <div style={{display:"flex",justifyContent:"space-between",marginTop:3}}>
                    <span style={{fontSize:10,color:F.muted}}>{pm.panel.ciudad}</span>
                    <span style={{fontSize:10,color:F.muted}}>{pm.margen.toFixed(1)}%</span>
                  </div>
                </div>
              </div>
            );
...
// ── Vista Ciudad ────────────────────────────────────────────────────
function FVistaCiudad({ eng }: FVistaCiudadProps) {
  const { cityMets, totalIng } = eng;
  const [expanded, setExpanded] = useState<string | null>(null);
  const maxIng = Math.max(...cityMets.map(c=>c.ingresos),1);
  if(cityMets.length===0) return <div style={{textAlign:"center",padding:"40px 20px",color:F.muted}}>Sin datos de ciudades en el período</div>;
  return (
    <div>
      <FCard style={{padding:"14px",marginBottom:10}}>
        <div style={{fontSize:10,fontWeight:700,color:F.muted,textTransform:"uppercase",letterSpacing:1.5,marginBottom:12}}>Ranking por ciudad</div>
        {cityMets.map((c,i)=>{
          const pct=(c.ingresos/maxIng)*100;
          const isExp=expanded===c.ciudad;
          return (
            <div key={c.ciudad}>
              <div onClick={()=>setExpanded(isExp?null:c.ciudad)} style={{display:"flex",alignItems:"center",gap:8,marginBottom:8,cursor:"pointer",padding:"8px",borderRadius:12,background:isExp?"rgba(255,255,255,0.04)":"transparent",transition:"background .15s"}}>
                <div style={{width:26,height:26,borderRadius:8,flexShrink:0,background:i===0?F.goldLt:F.blueLt,border:`1px solid ${i===0?F.goldBd:F.blueBd}`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:11,fontWeight:900,color:i===0?F.gold:F.blue}}>{i+1}</div>
                <div style={{flex:1,minWidth:0}}>
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:4}}>
                    <div><span style={{fontSize:14,fontWeight:700,color:F.text}}>{c.ciudad}</span><span style={{fontSize:10,color:F.muted,marginLeft:6}}>{c.n} panel{c.n!==1?"es":""}</span></div>
                    <div style={{textAlign:"right"}}>
                      <div style={{fontSize:14,fontWeight:800,color:F.blue}}>{fFmtK(c.ingresos)}</div>
                      <div style={{fontSize:10,color:c.utilidad>=0?F.green:F.red}}>{fFmtK(c.utilidad)} util.</div>
                    </div>
                  </div>
                  <div style={{height:5,borderRadius:99,background:"rgba(255,255,255,0.06)",overflow:"hidden"}}>
                    <div style={{height:"100%",width:`${pct}%`,borderRadius:99,background:`linear-gradient(90deg,${F.blue},${F.cyan})`,transition:"width .6s"}}/>
                  </div>
                  <div style={{fontSize:9,color:F.muted,marginTop:3,textAlign:"right"}}>{((c.ingresos/Math.max(totalIng,1))*100).toFixed(0)}% del total</div>
                </div>
              </div>
              {isExp&&(
                <div style={{marginBottom:8,marginLeft:34,display:"flex",flexDirection:"column",gap:5}}>
                  {c.pm.map(pm=>(
                    <div key={pm.panel.id} style={{display:"flex",alignItems:"center",gap:8,padding:"7px 10px",background:"rgba(255,255,255,0.03)",borderRadius:9}}>
                      <span style={{fontSize:14}}>{pm.panel.emoji||"📡"}</span>
                      <div style={{flex:1,minWidth:0}}>
                        <div style={{fontSize:11,fontWeight:700,color:F.text,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{pm.panel.nombre}</div>
                        <div style={{fontSize:9,color:F.muted}}>{pm.ocupacion.toFixed(0)}% ocupado</div>
                      </div>
                      <div style={{textAlign:"right",flexShrink:0}}>
                        <div style={{fontSize:12,fontWeight:800,color:F.blue}}>{fFmtK(pm.ingresos)}</div>
                        <div style={{fontSize:9,color:pm.margen>=0?F.green:F.red}}>{pm.margen.toFixed(1)}%</div>
                      </div>
                    </div>
                  ))}
                </div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
        {cityMets.map(c=>(
          <FCard key={c.ciudad} style={{padding:"12px"}}>
            <div style={{fontSize:12,fontWeight:800,color:F.text,marginBottom:6}}>{c.ciudad}</div>
            <div style={{fontSize:9,color:F.muted,marginBottom:2}}>Ingresos</div>
            <div style={{fontSize:17,fontWeight:900,color:F.blue,marginBottom:8}}>{fFmtK(c.ingresos)}</div>
            <div style={{display:"flex",justifyContent:"space-between"}}>
              <div><div style={{fontSize:9,color:F.muted}}>Margen</div><div style={{fontSize:13,fontWeight:700,color:c.margen>=30?F.green:c.margen>=0?F.gold:F.red}}>{c.margen.toFixed(1)}%</div></div>
              <div><div style={{fontSize:9,color:F.muted}}>Paneles</div><div style={{fontSize:13,fontWeight:700,color:F.text}}>{c.n}</div></div>
            </div>
          </FCard>
        ))}
      </div>
...
// ── Vista Cliente ───────────────────────────────────────────────────
function FVistaCliente({ eng }: FVistaClienteProps) {
  const { cliMets, totalIng } = eng;
  const maxIng = Math.max(...cliMets.map(c=>c.ingresos),1);
  if(cliMets.length===0) return <div style={{textAlign:"center",padding:"40px 20px",color:F.muted}}>Sin contratos en el período</div>;
  return (
    <div>
      {cliMets.length>=2&&(
        <FCard style={{padding:"14px 14px 12px",marginBottom:10}}>
          <div style={{fontSize:10,fontWeight:700,color:F.muted,textTransform:"uppercase",letterSpacing:1.5,marginBottom:12}}>Top clientes</div>
          <div style={{display:"flex",gap:8,alignItems:"flex-end",justifyContent:"center",marginBottom:8}}>
            {[cliMets[1],cliMets[0],cliMets[2]].filter(Boolean).map((cm,pi)=>{
              const rank=pi===0?2:pi===1?1:3;
              const h=rank===1?88:rank===2?68:52;
              const col=rank===1?F.gold:rank===2?"#94A3B8":"#B45309";
              return (
                <div key={cm.cl.id} style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",gap:4}}>
                  <div style={{fontSize:10,fontWeight:700,color:F.muted}}>{cm.part.toFixed(1)}%</div>
                  <div style={{fontSize:13,fontWeight:800,color:col}}>{fFmtK(cm.ingresos)}</div>
                  <div style={{width:"100%",height:h,borderRadius:"6px 6px 0 0",background:`${col}22`,border:`1px solid ${col}38`,display:"flex",alignItems:"flex-start",justifyContent:"center",paddingTop:7,fontSize:18}}>
                    {rank===1?"🥇":rank===2?"🥈":"🥉"}
                  </div>
                  <div style={{fontSize:10,fontWeight:700,color:F.text,textAlign:"center",lineHeight:1.2}}>{cm.cl.empresa}</div>
                </div>
              );
            })}
          </div>
      ...
      <FCard style={{padding:"14px"}}>
        <div style={{fontSize:10,fontWeight:700,color:F.muted,textTransform:"uppercase",letterSpacing:1.5,marginBottom:10}}>Todos los clientes</div>
        {cliMets.map((cm,i)=>{
          const pct=(cm.ingresos/maxIng)*100;
          return (
            <div key={cm.cl.id} style={{padding:"10px 10px",background:"rgba(255,255,255,0.03)",borderRadius:12,marginBottom:7}}>
              <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:6}}>
                <div style={{width:26,height:26,borderRadius:8,flexShrink:0,background:i<3?F.goldLt:F.blueLt,border:`1px solid ${i<3?F.goldBd:F.blueBd}`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:11,fontWeight:900,color:i<3?F.gold:F.blue}}>{i+1}</div>
                <div style={{flex:1,minWidth:0}}>
                  <div style={{fontSize:13,fontWeight:700,color:F.text,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{cm.cl.empresa}</div>
                  <div style={{fontSize:10,color:F.muted}}>{cm.nCtrs} contrato{cm.nCtrs!==1?"s":""} · {cm.nPaneles} panel{cm.nPaneles!==1?"es":""}</div>
                </div>
                <div style={{textAlign:"right",flexShrink:0}}>
                  <div style={{fontSize:15,fontWeight:900,color:F.blue}}>{fFmtK(cm.ingresos)}</div>
                  <div style={{fontSize:10,color:F.muted}}>{cm.part.toFixed(1)}% share</div>
                </div>
              </div>
              <FBar pct={pct} color={F.blue}/>
            </div>
          );
        })}
      </FCard>
...
// ── Vista Panel ─────────────────────────────────────────────────────
function FVistaPanel({ eng, clientes }: FVistaPanelProps) {
  const { panelMets, ranking, totalIng } = eng;
  ...
  const sorted = useMemo(() => {
    let arr = [...panelMets];
    if(search.trim()) arr=arr.filter(pm=>pm.panel.nombre.toLowerCase().includes(search.toLowerCase())||pm.panel.ciudad?.toLowerCase().includes(search.toLowerCase()));
    return arr.sort((a,b)=>{
      if(sortBy==="utilidad")  return b.utilidad-a.utilidad;
      if(sortBy==="ingresos")  return b.ingresos-a.ingresos;
      if(sortBy==="margen")    return b.margen-a.margen;
      if(sortBy==="ocupacion") return b.ocupacion-a.ocupacion;
      if(sortBy==="roi")       return b.roi-a.roi;
      return 0;
    });
  }, [panelMets, sortBy, search]);

  const maxIng = Math.max(...panelMets.map(p=>p.ingresos),1);

  return (
    <div>
      {/* Search */}
      <div style={{display:"flex",alignItems:"center",gap:8,background:F.card,border:`1px solid ${F.border}`,borderRadius:12,padding:"9px 12px",marginBottom:8}}>
        <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke={F.muted} strokeWidth="2.5" strokeLinecap="round"><circle cx="11" cy="11" r="8"/><path d="m21 21-4.35-4.35"/></svg>
        <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Buscar panel o ciudad…"
          style={{flex:1,background:"transparent",border:"none",outline:"none",color:F.text,fontSize:13,fontFamily:"inherit"}}/>
      </div>
      {/* Sort pills */}
      <div style={{display:"flex",gap:5,overflowX:"auto",paddingBottom:2,scrollbarWidth:"none",marginBottom:10}}>
        {[{id:"utilidad",l:"Utilidad"},{id:"ingresos",l:"Ingresos"},{id:"margen",l:"Margen"},{id:"ocupacion",l:"Ocup."},{id:"roi",l:"ROI"}].map(o=>(
          <button key={o.id} onClick={()=>setSortBy(o.id)} style={{
            padding:"6px 12px",borderRadius:99,whiteSpace:"nowrap",flexShrink:0,
            background:sortBy===o.id?F.goldLt:"rgba(255,255,255,0.05)",
            border:`1px solid ${sortBy===o.id?F.goldBd:F.border}`,
            color:sortBy===o.id?F.gold:F.muted,fontSize:11,fontWeight:sortBy===o.id?700:500,
      ...
      <div style={{display:"flex",flexDirection:"column",gap:8}}>
        {sorted.map((pm,i)=>{
          const {panel,ingresos,utilidad,margen,roi,ocupacion,cActivosN,historial,clientePpal}=pm;
          const sparkD=historial.map(h=>h.ingresos);
          const pct=(ingresos/maxIng)*100;
          const margenColor=margen>=40?F.green:margen>=20?F.gold:margen>=0?F.red:"rgba(239,68,68,0.5)";
          const ocupColor=ocupacion>=80?F.green:ocupacion>=50?F.gold:F.red;
          const gRank=ranking.findIndex(x=>x.panel.id===panel.id)+1;
          return (
            <div key={panel.id} onClick={()=>setSelected(pm)} style={{cursor:"pointer"}}>
              <FCard style={{padding:"14px"}}>
                {/* Top row */}
                <div style={{display:"flex",alignItems:"flex-start",gap:10,marginBottom:10}}>
                  <div style={{width:40,height:40,borderRadius:11,flexShrink:0,background:"rgba(255,255,255,0.06)",border:`1px solid ${F.border}`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:18}}>{panel.emoji||"📡"}</div>
                  <div style={{flex:1,minWidth:0}}>
                    <div style={{display:"flex",alignItems:"center",gap:5,marginBottom:3,flexWrap:"wrap"}}>
                      <span style={{fontSize:14,fontWeight:800,color:F.text,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{panel.nombre}</span>
                      {gRank<=3&&<span style={{fontSize:12}}>{gRank===1?"🥇":gRank===2?"🥈":"🥉"}</span>}
                    </div>
                    <div style={{display:"flex",gap:4,flexWrap:"wrap"}}>
                      <FBadge label={panel.ciudad||"?"} color={F.muted}/>
                      {cActivosN>0&&<FBadge label={`${cActivosN} activo${cActivosN!==1?"s":""}`} color={F.green}/>}
                    </div>
                  </div>
                  <div style={{textAlign:"right",flexShrink:0}}>
                    <div style={{fontSize:20,fontWeight:900,letterSpacing:"-0.5px",color:utilidad>=0?F.green:F.red}}>{fFmtK(utilidad)}</div>
                    <div style={{fontSize:10,color:F.muted}}>utilidad</div>
                  </div>
                </div>
                {/* 4 micro-KPIs */}
                <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:5,marginBottom:10}}>
                  {[
                    {l:"Ingresos", v:fFmtK(ingresos),       c:F.blue},
                    {l:"Margen",   v:`${margen.toFixed(1)}%`,c:margenColor},
                    {l:"ROI",      v:roi>=999?"∞%":`${roi.toFixed(0)}%`,c:roi>=0?F.gold:F.red},
                    {l:"Ocup.",    v:`${ocupacion.toFixed(0)}%`,c:ocupColor},
                  ].map(m=>(
                    <div key={m.l} style={{background:"rgba(255,255,255,0.04)",borderRadius:9,padding:"7px 7px"}}>
                      <div style={{fontSize:8,color:F.muted,fontWeight:700,textTransform:"uppercase",letterSpacing:0.8,marginBottom:2}}>{m.l}</div>
                      <div style={{fontSize:12,fontWeight:800,color:m.c}}>{m.v}</div>
                    </div>
                  ))}
                </div>
                {/* Barra relativa */}
                <FBar pct={pct} color={utilidad>=0?F.blue:F.red}/>
                {/* Sparkline */}
                {sparkD.some(v=>v>0)&&(
                  <div style={{marginTop:8}}>
                    <FSparkline data={sparkD} color={utilidad>=0?F.blue:F.red} height={30}/>
                  </div>
                )}
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginTop:6}}>
                  <span style={{fontSize:10,color:F.muted}}>{clientePpal!=="—"&&<>Cliente: <span style={{color:F.text}}>{clientePpal}</span></>}</span>
                  <span style={{fontSize:10,color:F.muted}}>Ver detalle →</span>
                </div>
              </FCard>
            </div>
          );
        })}
        {sorted.length===0&&<div style={{textAlign:"center",padding:"40px 20px",color:F.muted}}>{search?"Sin resultados":"No hay paneles en el período"}</div>}
      </div>
      {selected&&<FPanelDrawer pm={selected} clientes={clientes} onClose={()=>setSelected(null)}/>}
    </div>
  );
}

// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
// RESULTADOS — Sistema de Rentabilidad OOH (reemplaza versión simple)
// Recibe: contratos, paneles, clientes, gastos, loading
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
function Resultados({ contratos, paneles = [], clientes = [], gastos = [], loading }: ResultadosProps) {
  ...
  const [desde,    setDesde]    = useState(fNMesesAtras(6));
  const [hasta,    setHasta]    = useState(fMesActual());
  const [ciudadF,  setCiudadF]  = useState("all");
  const [clienteF, setClienteF] = useState("all");
  const [panelF,   setPanelF]   = useState("all");
  const [showFilt, setShowFilt] = useState(false);

  const ciudadesDisp = useMemo(()=>[...new Set(paneles.filter(p=>!p.deleted).map(p=>p.ciudad).filter(Boolean))],[paneles]);
  const clientesDisp = useMemo(()=>clientes.filter(c=>!c.deleted),[clientes]);
  const panelsDisp   = useMemo(()=>paneles.filter(p=>!p.deleted),[paneles]);

  // ── Modo de período: "mes" | "anio" | "custom" ───────────────
  const [modoPeriodo, setModoPeriodo] = useState<"mes"|"anio"|"custom">("mes");
  const [mesSel,  setMesSel]  = useState(fMesActual());
  const [anioSel, setAnioSel] = useState(new Date().getFullYear());

  // Sincronizar desde/hasta según modo
  useEffect(() => {
    if (modoPeriodo === "mes") {
      setDesde(mesSel); setHasta(mesSel); setPeriodo("mes");
    } else if (modoPeriodo === "anio") {
      setDesde(`${anioSel}-01`); setHasta(`${anioSel}-12`); setPeriodo("anio");
    }
  }, [modoPeriodo, mesSel, anioSel]);

  const eng = useOOHEngine(paneles, contratos, gastos, clientes, desde, hasta, ciudadF, clienteF, panelF);
  const hasFilters = ciudadF!=="all"||clienteF!=="all"||panelF!=="all";

  const VISTAS = [
    {id:"empresa", l:"Empresa", ic:"🏢"},
    {id:"ciudad",  l:"Ciudades",ic:"🌆"},
    {id:"cliente", l:"Clientes",ic:"👤"},
    {id:"panel",   l:"Paneles", ic:"📡"},
  ];

  // Año mín/máx de los datos disponibles
  const aniosDisp = useMemo(() => {
    const years = new Set<number>();
    contratos.forEach(c => { if(c.inicio) years.add(Number(c.inicio.slice(0,4))); });
    gastos.forEach(g => { if(g.fecha) years.add(Number(g.fecha.slice(0,4))); });
    if(years.size === 0) years.add(new Date().getFullYear());
    return [...years].sort((a,b) => b-a);
  }, [contratos, gastos]);

  if(loading) return <div style={{textAlign:"center",padding:"60px 20px",color:F.muted}}>Cargando datos…</div>;
  ...
  return (
    <div style={{color:F.text}}>
      ...
      <div style={{marginBottom:16}}>
        ...
        {/* ── Selector de modo: Mes / Año / Período ── */}
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:6,marginBottom:14}}>
          {([
            {id:"mes",    l:"Por Mes",    ic:<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round"><rect x="3" y="4" width="18" height="18" rx="2"/><line x1="16" y1="2" x2="16" y2="6"/><line x1="8" y1="2" x2="8" y2="6"/><line x1="3" y1="10" x2="21" y2="10"/></svg>},
            {id:"anio",   l:"Por Año",   ic:<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round"><polyline points="22 12 18 12 15 21 9 3 6 12 2 12"/></svg>},
            {id:"custom", l:"Período",   ic:<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round"><line x1="4" y1="6" x2="20" y2="6"/><line x1="8" y1="12" x2="16" y2="12"/><line x1="11" y1="18" x2="13" y2="18"/></svg>},
          ] as {id:"mes"|"anio"|"custom"; l:string; ic:React.ReactNode}[]).map(m => {
            const active = modoPeriodo === m.id;
            return (
              <button key={m.id} onClick={()=>setModoPeriodo(m.id)} style={{
                display:"flex",alignItems:"center",justifyContent:"center",gap:6,
                padding:"11px 6px",borderRadius:13,border:"none",cursor:"pointer",touchAction:"manipulation",fontFamily:"inherit",
                background: active ? "linear-gradient(135deg,#0F1729,#1E3A8A)" : "rgba(255,255,255,0.05)",
                color: active ? "#fff" : F.muted,
                fontWeight: active ? 700 : 500, fontSize:13,
                boxShadow: active ? "0 4px 16px rgba(15,23,41,0.4)" : "none",
                outline: active ? "none" : `1px solid ${F.border}`,
                transition:"all .15s",
              }}>
                <span style={{color: active ? "rgba(255,255,255,0.8)" : F.muted}}>{m.ic}</span>
                {m.l}
              </button>
            );
          })}
        ...
        {modoPeriodo === "mes" && (
          <div style={{marginBottom:12}}>
            {/* Navegación mes con flechas */}
            <div style={{display:"flex",alignItems:"center",gap:8,background:"rgba(255,255,255,0.04)",border:`1px solid ${F.border}`,borderRadius:13,padding:"8px 12px",marginBottom:8}}>
              <button onClick={()=>{
                const [y,m2]=mesSel.split("-").map(Number);
                const d=new Date(y,m2-2,1);
                setMesSel(`${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,"0")}`);
              }} style={{background:"rgba(255,255,255,0.08)",border:"none",borderRadius:8,width:32,height:32,color:F.text,cursor:"pointer",touchAction:"manipulation",fontSize:16,fontWeight:700,display:"flex",alignItems:"center",justifyContent:"center"}}>‹</button>
              <div style={{flex:1,textAlign:"center"}}>
                <div style={{fontSize:15,fontWeight:800,color:F.text}}>
                  {new Date(mesSel+"-02").toLocaleDateString("es-PE",{month:"long",year:"numeric"}).replace(/^\w/,c=>c.toUpperCase())}
                </div>
              </div>
              <button onClick={()=>{
                const [y,m2]=mesSel.split("-").map(Number);
            ...
            {/* Input directo mes */}
            <input type="month" value={mesSel} onChange={e=>setMesSel(e.target.value)} style={{width:"100%",background:"rgba(255,255,255,0.06)",border:`1px solid ${F.border2}`,borderRadius:9,padding:"9px 11px",color:F.text,fontSize:13,fontFamily:"inherit",outline:"none",boxSizing:"border-box" as const,colorScheme:"dark"}}/>
          </div>
        )}

        {modoPeriodo === "anio" && (
          <div style={{marginBottom:12}}>
            {/* Selector de año con flechas + chips de años disponibles */}
            <div style={{display:"flex",alignItems:"center",gap:8,background:"rgba(255,255,255,0.04)",border:`1px solid ${F.border}`,borderRadius:13,padding:"8px 12px",marginBottom:8}}>
              <button onClick={()=>setAnioSel(a=>a-1)} style={{background:"rgba(255,255,255,0.08)",border:"none",borderRadius:8,width:32,height:32,color:F.text,cursor:"pointer",touchAction:"manipulation",fontSize:16,fontWeight:700,display:"flex",alignItems:"center",justifyContent:"center"}}>‹</button>
              <div style={{flex:1,textAlign:"center",fontSize:22,fontWeight:900,color:F.text,fontFamily:"monospace",letterSpacing:"-0.5px"}}>{anioSel}</div>
              <button onClick={()=>setAnioSel(a=>a+1)} style={{background:"rgba(255,255,255,0.08)",border:"none",borderRadius:8,width:32,height:32,color:F.text,cursor:"pointer",touchAction:"manipulation",fontSize:16,fontWeight:700,display:"flex",alignItems:"center",justifyContent:"center"}}>›</button>
            </div>
            {/* Chips de años con datos */}
            <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>
              {aniosDisp.map(a=>(
                <button key={a} onClick={()=>setAnioSel(a)} style={{
                  padding:"5px 14px",borderRadius:20,cursor:"pointer",touchAction:"manipulation",fontFamily:"inherit",
                  background:a===anioSel?"linear-gradient(135deg,#0F1729,#1E3A8A)":"rgba(255,255,255,0.05)",
                  border:`1px solid ${a===anioSel?"transparent":F.border}`,
                  color:a===anioSel?"#fff":F.muted,fontWeight:700,fontSize:12,
                }}>{a}</button>
              ))}
            </div>
          </div>
        ...
        {modoPeriodo === "custom" && (
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:12}}>
            {[{l:"Desde",v:desde,s:setDesde},{l:"Hasta",v:hasta,s:setHasta}].map(f=>(
              <div key={f.l}>
                <div style={{fontSize:9,fontWeight:700,color:F.muted,textTransform:"uppercase",letterSpacing:1.2,marginBottom:4}}>{f.l}</div>
                <input type="month" value={f.v} onChange={e=>{f.s(e.target.value);setPeriodo("custom");}} style={{width:"100%",background:"rgba(255,255,255,0.06)",border:`1px solid ${F.border2}`,borderRadius:9,padding:"9px 11px",color:F.text,fontSize:13,fontFamily:"inherit",outline:"none",boxSizing:"border-box" as const,colorScheme:"dark"}}/>
              </div>
            ))}
          </div>
        )}
        ...
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:10}}>
          <div style={{fontSize:11,color:F.muted}}>
            {eng.meses.length>0?`${eng.meses[0]} → ${eng.meses[eng.meses.length-1]}`:"Sin período"}
            {" · "}{eng.panelMets.length} panel{eng.panelMets.length!==1?"es":""}
          </div>
          <button onClick={()=>setShowFilt(f=>!f)} style={{
            display:"flex",alignItems:"center",gap:5,
            background:hasFilters?F.goldLt:"rgba(255,255,255,0.06)",
            border:`1px solid ${hasFilters?F.goldBd:F.border}`,
            borderRadius:11,padding:"7px 11px",color:hasFilters?F.gold:F.muted,
            fontSize:12,fontWeight:600,cursor:"pointer",touchAction:"manipulation",fontFamily:"inherit",flexShrink:0,
          }}>
            ...
            {hasFilters?"Filtros":"Filtrar"}
          </button>
        </div>

        {/* Filtros panel */}
        {showFilt&&(
          <div style={{background:F.card,border:`1px solid ${F.border}`,borderRadius:14,padding:"12px",marginBottom:10}}>
            <div style={{fontSize:10,fontWeight:700,color:F.muted,textTransform:"uppercase",letterSpacing:1.5,marginBottom:8}}>Filtros</div>
            {[
              {l:"Ciudad",v:ciudadF,s:setCiudadF,opts:[{v:"all",o:"Todas las ciudades"},...ciudadesDisp.map(c=>({v:c,o:c}))]},
              {l:"Cliente",v:clienteF,s:setClienteF,opts:[{v:"all",o:"Todos los clientes"},...clientesDisp.map(c=>({v:c.id,o:c.empresa}))]},
              {l:"Panel",v:panelF,s:setPanelF,opts:[{v:"all",o:"Todos los paneles"},...panelsDisp.map(p=>({v:p.id,o:p.nombre}))]},
            ].map(f=>(
              <div key={f.l} style={{marginBottom:7}}>
                <div style={{fontSize:9,fontWeight:700,color:F.muted,textTransform:"uppercase",letterSpacing:1.2,marginBottom:3}}>{f.l}</div>
                <select value={f.v} onChange={e=>f.s(e.target.value)} style={{width:"100%",background:"rgba(255,255,255,0.06)",border:`1px solid ${F.border2}`,borderRadius:9,padding:"8px 11px",color:F.text,fontSize:13,fontFamily:"inherit",outline:"none",appearance:"none",colorScheme:"dark"}}>
                  {f.opts.map(o=><option key={o.v} value={o.v} style={{background:F.card2}}>{o.o}</option>)}
                </select>
              </div>
            ))}
            {hasFilters&&<button onClick={()=>{setCiudadF("all");setClienteF("all");setPanelF("all");}} style={{width:"100%",marginTop:4,padding:"7px",background:F.redLt,border:`1px solid ${F.redBd}`,borderRadius:9,color:F.red,fontSize:12,fontWeight:600,cursor:"pointer",touchAction:"manipulation",fontFamily:"inherit"}}>Limpiar filtros</button>}
          </div>
        )}
        ...
        <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:7,marginBottom:12}}>
          {[
            {l:"Ingresos",v:fFmtK(eng.totalIng),  c:F.blue},
            {l:"Utilidad", v:fFmtK(eng.totalUtil), c:eng.totalUtil>=0?F.green:F.red},
            {l:"Margen",   v:`${eng.margenEmp.toFixed(1)}%`,c:eng.margenEmp>=30?F.green:eng.margenEmp>=10?F.gold:F.red},
          ].map(k=>(
            <div key={k.l} style={{background:F.card,border:`1px solid ${F.border}`,borderRadius:12,padding:"9px 10px"}}>
              <div style={{fontSize:9,fontWeight:700,color:F.muted,textTransform:"uppercase",letterSpacing:1,marginBottom:3}}>{k.l}</div>
              <div style={{fontSize:14,fontWeight:900,color:k.c,letterSpacing:"-0.4px"}}>{k.v}</div>
            </div>
          ))}
        </div>
        ...
        <div style={{display:"flex",background:"rgba(255,255,255,0.04)",borderRadius:12,padding:3}}>
          {VISTAS.map(v=>(
            <button key={v.id} onClick={()=>setVista(v.id)} style={{
              ...
              padding:"7px 3px",borderRadius:9,
              background:vista===v.id?"rgba(59,130,246,0.16)":"transparent",
              border:`1px solid ${vista===v.id?F.blueBd:"transparent"}`,
              color:vista===v.id?F.blue:F.muted,fontSize:10,fontWeight:vista===v.id?700:500,
              cursor:"pointer",touchAction:"manipulation",fontFamily:"inherit",transition:"all .18s",
            }}>
              <span style={{fontSize:15}}>{v.ic}</span>{v.l}
            </button>
          ))}
        </div>
      </div>
      ...
      {/* ── Vista content ── */}
      {vista==="empresa" && <FVistaEmpresa eng={eng}/>}
      {vista==="ciudad"  && <FVistaCiudad  eng={eng}/>}
      {vista==="cliente" && <FVistaCliente eng={eng}/>}
      {vista==="panel"   && <FVistaPanel   eng={eng} clientes={clientes}/>}
    </div>
  );
}
...
function Firebase({ contratos, paneles, clientes, gastos, fbConnected, fbLoading, fbError }: FirebaseStatusProps){
  ...
  const AlertRow=({c,color,label})=>{
    ...
    return(
      <div style={{display:"flex",alignItems:"center",gap:12,padding:"12px 0",borderBottom:`1px solid ${T.border}`}}>
        ...
        <div style={{flex:1,minWidth:0}}>
          <div style={{fontSize:13,fontWeight:700,color:T.white,whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>{c.panel?.nombre}</div>
          <div style={{fontSize:11,color:T.muted,marginTop:1}}>{c.cliente?.empresa} · Vence {fmtF(c.fin)}</div>
        </div>
  ...
  return(<div>
    ...
    <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:16,padding:"12px 16px",background:fbError?"#EF444412":fbLoading?"#F59E0B12":"#10B98112",border:`1px solid ${fbError?T.red+"33":fbLoading?T.amber+"33":T.green+"33"}`,borderRadius:12}}>
      <div style={{width:9,height:9,borderRadius:"50%",background:fbError?T.red:fbLoading?T.amber:T.green,flexShrink:0,animation:fbLoading?"pulse 1s infinite":"none"}}/>
      <div style={{flex:1}}>
        <div style={{fontSize:13,fontWeight:700,color:fbError?T.red:fbLoading?T.amber:T.green}}>
          {fbError?"⚠️ Sin conexión a Firebase":fbLoading?"🔄 Conectando con Firebase...":"✅ Firebase conectado"}
        </div>
        {!fbError&&!fbLoading&&<div style={{fontSize:11,color:T.muted,marginTop:1}}>
          Proyecto: base-de-datos-vista360 · Firestore + Storage activos
        </div>}
        {fbError&&<div style={{fontSize:11,color:T.muted,marginTop:1}}>
    ...
    <Card style={{marginBottom:16}}>
      ...
      {dbStats.loading
        ?<div style={{color:T.muted,fontSize:13}}>Calculando...</div>
        :<div>
          {/* Barra total */}
          <div style={{marginBottom:16}}>
            <div style={{display:"flex",justifyContent:"space-between",marginBottom:6}}>
              <span style={{fontSize:13,fontWeight:700,color:T.white}}>Total usado</span>
              <span style={{fontSize:13,fontWeight:800,color:dbPct>80?T.red:dbPct>50?T.amber:T.green}}>{fmtBytes(dbStats.total)} <span style={{fontSize:11,color:T.muted,fontWeight:400}}>de 1 GB</span></span>
            </div>
            ...
            <div style={{fontSize:11,color:T.muted,marginTop:4}}>{dbPct.toFixed(3)}% del límite gratuito</div>
          </div>

          {/* Desglose por tabla */}
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
            {[
              {label:"🖥️ Paneles",size:dbStats.paneles,count:paneles.length,color:T.accent},
              {label:"📋 Contratos",size:dbStats.contratos,count:contratos.length,color:T.green},
              {label:"👥 Clientes",size:dbStats.clientes,count:clientes.length,color:T.purple},
              {label:"🧾 Gastos",size:dbStats.gastos,count:gastos.length,color:T.amber},
            ].map(({label,size,count,color})=>(
              <div key={label} style={{background:T.surface,border:`1px solid ${T.border}`,borderRadius:12,padding:"12px 13px"}}>
                ...
                <div style={{fontSize:11,color:T.muted,marginTop:2}}>{count} registros</div>
                <div style={{marginTop:6,background:T.border,borderRadius:4,height:3,overflow:"hidden"}}>
          ...
          <div style={{marginTop:14,padding:"10px 14px",background:"#0EA5E918",border:"1px solid #0EA5E944",borderRadius:10,display:"flex",gap:10,alignItems:"flex-start"}}>
            ...
            <div style={{fontSize:11,color:T.muted,lineHeight:1.6}}>
              <strong style={{color:T.white}}>Firebase Firestore (Plan Spark gratuito):</strong> 1 GiB almacenamiento · 50,000 lecturas/día · 20,000 escrituras/día · Sin límite de filas.
            </div>
          </div>
        </div>
      }
    ...
    <Card style={{marginBottom:16}}>
      ...
      {storageStats.loading
        ?<div style={{color:T.muted,fontSize:13,display:"flex",gap:8,alignItems:"center"}}><div style={{width:14,height:14,border:`2px solid ${T.border}`,borderTopColor:T.cyan,borderRadius:"50%",animation:"spin .7s linear infinite"}}/>Calculando espacio real...</div>
        :<div>
          {/* Barra storage fotos */}
          <div style={{marginBottom:16}}>
            <div style={{display:"flex",justifyContent:"space-between",marginBottom:6}}>
              <span style={{fontSize:13,fontWeight:700,color:T.white}}>Storage de imágenes</span>
              <span style={{fontSize:13,fontWeight:800,color:storagePct>80?T.red:storagePct>50?T.amber:T.cyan}}>
                {fmtBytes(storageStats.bytes)} <span style={{fontSize:11,color:T.muted,fontWeight:400}}>de 25 GB</span>
              </span>
            </div>
            <div style={{background:T.border,borderRadius:8,height:10,overflow:"hidden"}}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:14}}>
            {[
              {label:"📁 Capacidad total",val:"25 GB",sub:"Cloudinary gratuito",color:T.cyan},
              {label:"🖼️ Fotos subidas",val:String(storageStats.count),sub:"boletas escaneadas",color:T.green},
              {label:"📦 Espacio usado",val:fmtBytes(storageStats.bytes),sub:"estimado · ~25 KB/foto",color:T.amber},
              {label:"♻️ Espacio libre",val:fmtBytes(Math.max(0,STORAGE_LIMIT-storageStats.bytes)),sub:"disponible",color:T.purple},
          ...
          <div style={{padding:"10px 14px",background:"#06B6D418",border:"1px solid #06B6D444",borderRadius:10,display:"flex",gap:10,alignItems:"flex-start"}}>
            <span style={{fontSize:16}}>ℹ️</span>
            <div style={{fontSize:11,color:T.muted,lineHeight:1.6}}>
              <strong style={{color:T.white}}>Cloudinary (Plan gratuito):</strong> 25 GB de almacenamiento · Las fotos se comprimen a WebP ~25 KB antes de subir.
            </div>
          </div>
        </div>
      }
    ...
    {/* ── ALERTAS DE VENCIMIENTO ── */}
    <Card style={{marginBottom:16,borderColor:totalAlertas>0?T.amber+"44":T.border}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16}}>
        <SecTit ch={`🔔 Alertas de Vencimiento${totalAlertas>0?` · ${totalAlertas} contratos`:""}`}/>
        {totalAlertas>0&&<span style={{background:T.red,color:"#fff",borderRadius:20,padding:"2px 10px",fontSize:12,fontWeight:700}}>{totalAlertas}</span>}
      </div>

      {totalAlertas===0
        ?<div style={{textAlign:"center",padding:"20px 0"}}>
            <div style={{fontSize:32,marginBottom:8}}>✅</div>
            <div style={{fontSize:14,fontWeight:700,color:T.white,marginBottom:4}}>Todo al día</div>
            <div style={{fontSize:12,color:T.muted}}>No hay contratos por vencer en los próximos 30 días</div>
          </div>
        :<div>
          {vencidos.length>0&&<div style={{marginBottom:12}}>
            <div style={{fontSize:11,fontWeight:700,color:T.red,textTransform:"uppercase",letterSpacing:0.8,marginBottom:8}}>🚨 Vencidos — {vencidos.length}</div>
            {vencidos.map(c=><AlertRow key={c.id} c={c} color={T.red} label={`Venció hace ${Math.abs(c.d)}d`}/>)}
          </div>}
          {criticos.length>0&&<div style={{marginBottom:12}}>
            <div style={{fontSize:11,fontWeight:700,color:T.red,textTransform:"uppercase",letterSpacing:0.8,marginBottom:8}}>🔴 Críticos ≤15 días — {criticos.length}</div>
            {criticos.map(c=><AlertRow key={c.id} c={c} color={T.red} label={`⚠️ ${c.d}d`}/>)}
          </div>}
          {proximos.length>0&&<div>
            <div style={{fontSize:11,fontWeight:700,color:T.amber,textTransform:"uppercase",letterSpacing:0.8,marginBottom:8}}>🟡 Próximos 16-30 días — {proximos.length}</div>
            {proximos.map(c=><AlertRow key={c.id} c={c} color={T.amber} label={`📅 ${c.d}d`}/>)}
          </div>}
        </div>
      }
    </Card>
    ...
    <Card>
      ...
      <div style={{marginBottom:14,fontSize:13,color:T.muted,lineHeight:1.6}}>
        Recibe alertas automáticas en tu celular <strong style={{color:T.white}}>30 días y 15 días antes</strong> de que venza cada contrato.
      </div>
      {notifPerm==="granted"
        ?<div style={{display:"flex",alignItems:"center",gap:10,background:T.green+"15",border:`1px solid ${T.green}44`,borderRadius:12,padding:"13px 16px"}}>
            <div style={{width:10,height:10,borderRadius:"50%",background:T.green,animation:"pulse 1.5s infinite",flexShrink:0}}/>
            <div>
              <div style={{fontSize:13,fontWeight:700,color:T.green}}>Notificaciones activas</div>
              <div style={{fontSize:11,color:T.muted,marginTop:2}}>Recibirás alertas 30d y 15d antes de cada vencimiento</div>
            </div>
          </div>
        :notifPerm==="denied"
        ?<div style={{background:T.red+"15",border:`1px solid ${T.red}44`,borderRadius:12,padding:"13px 16px"}}>
            <div style={{fontSize:13,fontWeight:700,color:T.red,marginBottom:4}}>🚫 Notificaciones bloqueadas</div>
            <div style={{fontSize:11,color:T.muted}}>Ve a Ajustes del navegador → Notificaciones → Permitir para este sitio.</div>
          </div>
        :<div>
...
function Historico({ contratos, setContratos, paneles, clientes, onModalChange }: HistoricoProps){
  ...
  // Años disponibles — año actual + años futuros donde hay contratos activos
  const aniosDisp=useMemo(()=>{
    const set=new Set();
    ...
    todos.forEach(c=>{
      ...
      if(finAnio>=anioActual) set.add(finAnio);
    });
    return Array.from(set).sort((a,b)=>a-b);
  },[todos]);

  // Si el año seleccionado no está en la lista, usar el primero
  const anioEfectivo=aniosDisp.includes(anioSel)?anioSel:(aniosDisp[0]||new Date().getFullYear());
  ...
  const listaFiltrada=mesSel!==null
  ...
  const inpStyle={width:"100%",background:T.bg,border:`1px solid ${T.border}`,borderRadius:10,padding:"10px 13px",color:T.text,fontSize:14,outline:"none",fontFamily:"inherit",boxSizing:"border-box"};
  ...
  return(<div>
    ...
    {modal&&(
      <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.6)",zIndex:400,display:"flex",alignItems:"flex-end",justifyContent:"center"}}
        ...
        <div style={{background:T.card,border:`1px solid ${T.border}`,borderRadius:"22px 22px 0 0",padding:"10px 20px 24px",width:"100%",maxHeight:"92vh",overflowY:"auto",overscrollBehavior:"none",paddingBottom:"calc(24px + env(safe-area-inset-bottom))",boxShadow:"0 -10px 40px rgba(0,0,0,0.45)"}}
          ...
          <div style={{display:"flex",justifyContent:"center",marginBottom:8}}><div style={{width:36,height:4,borderRadius:2,background:T.border}}/></div>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:18}}>
            <span style={{fontSize:17,fontWeight:800,color:T.text}}>Editar Contrato</span>
            <button onClick={closeModal} style={{background:T.border,border:"none",borderRadius:"50%",width:30,height:30,color:T.muted,cursor:"pointer",touchAction:"manipulation",fontSize:14,display:"flex",alignItems:"center",justifyContent:"center"}}>✕</button>
          </div>
          ...
          {mesesForm.length>0&&(<F label={`Meses pagados (${pagosMarcados}/${mesesForm.length})`}><div style={{display:"flex",flexWrap:"wrap",gap:8}}>{mesesForm.map(m=>{const pagado=!!(form.pagosMeses||{})[m.key];return(<button key={m.key} onClick={()=>toggleMesPago(m.key)} onMouseDown={e=>e.stopPropagation()} onTouchStart={e=>e.stopPropagation()} onPointerDown={e=>e.stopPropagation()} style={{padding:"6px 12px",borderRadius:8,border:`1.5px solid ${pagado?T.green:T.border}`,background:pagado?T.green+"22":"transparent",color:pagado?T.green:T.muted,fontWeight:pagado?700:500,fontSize:12,cursor:"pointer",touchAction:"manipulation",fontFamily:"inherit"}}>{m.label}</button>);})}</div></F>)}
          <div style={{display:"flex",gap:10,marginTop:20,position:"sticky",bottom:0}}>
            <button onClick={closeModal} style={{flex:1,padding:14,background:"transparent",border:`1px solid ${T.border}`,borderRadius:12,color:T.muted,fontWeight:600,cursor:"pointer",touchAction:"manipulation",fontSize:14,minHeight:46}}>Cancelar</button>
            <button onClick={guardar} disabled={saving} style={{flex:2,padding:14,background:T.accent,border:"none",borderRadius:12,color:T.white,fontWeight:700,fontSize:14,cursor:"pointer",touchAction:"manipulation",minHeight:46,opacity:saving?.7:1}}>{saving?"Guardando...":"Guardar Cambios"}</button>
          </div>
          <div style={{marginTop:24,paddingTop:18,borderTop:`1px dashed ${T.border}`}}>
            {!confirmDelete?(<button onClick={()=>setConfirmDelete(true)} style={{width:"100%",padding:"12px",borderRadius:12,border:"1px solid #FECACA",background:"#FEF2F2",color:T.red,fontWeight:700,fontSize:14,cursor:"pointer",touchAction:"manipulation",display:"flex",alignItems:"center",justifyContent:"center",gap:8}}><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><polyline points="3 6 5 6 21 6"/><path d="M19 6l-1 14a2 2 0 01-2 2H8a2 2 0 01-2-2L5 6"/></svg>Eliminar contrato</button>):(<div style={{background:"#FEF2F2",border:"1px solid #FECACA",borderRadius:14,padding:16}}><div style={{fontSize:14,fontWeight:700,color:T.red,marginBottom:6,textAlign:"center"}}>¿Confirmar eliminación?</div><div style={{fontSize:12,color:T.muted,marginBottom:14,textAlign:"center"}}>Esta acción marcará el contrato como eliminado.</div><div style={{display:"flex",gap:10}}><button onClick={()=>setConfirmDelete(false)} style={{flex:1,padding:"11px",borderRadius:10,border:`1px solid ${T.border}`,background:"transparent",color:T.muted,fontWeight:600,fontSize:13,cursor:"pointer",touchAction:"manipulation"}}>Cancelar</button><button onClick={eliminar} disabled={saving} style={{flex:1,padding:"11px",borderRadius:10,border:"none",background:T.red,color:"#fff",fontWeight:800,fontSize:13,cursor:"pointer",touchAction:"manipulation",opacity:saving?.6:1}}>{saving?"Eliminando...":"Sí, eliminar"}</button></div></div>)}
          </div>
        </div>
      </div>
    )}
    ...
    {/* ── CABECERA DARK ── */}
    <div style={{ background:"linear-gradient(160deg,#080D18 0%,#0D1525 60%,#0F172A 100%)", margin:"-20px -16px 0", padding:"20px 20px 0", borderBottomLeftRadius:0, borderBottomRightRadius:0, boxShadow:"0 8px 28px rgba(0,0,0,0.35)" }}>
      ...
      <div style={{ display:"flex", alignItems:"center", gap:8, marginBottom:16, marginTop:8 }}>
        {aniosDisp.map(a=>(
          <button key={a} onClick={()=>{ setAnioSel(a); setMesSel(null); }} onMouseDown={e=>e.stopPropagation()} onTouchStart={e=>e.stopPropagation()} onTouchEnd={e=>e.stopPropagation()} onPointerDown={e=>e.stopPropagation()} style={{
            padding:"6px 16px", borderRadius:20,
            background: a===anioEfectivo ? T.accent : "rgba(255,255,255,0.08)",
            border: a===anioEfectivo ? "none" : "1px solid rgba(255,255,255,0.12)",
            color: a===anioEfectivo ? "#fff" : "rgba(255,255,255,0.45)",
            fontWeight:700, fontSize:13, cursor:"pointer", touchAction:"manipulation", fontFamily:"inherit",
            transition:"all .15s",
          }}>{a}</button>
        ))}
      ...
      <div style={{ display:"flex", gap:20, marginBottom:18 }}>
        {[
          { label:"Ingresos año", val:`S/ ${totalAnio.toLocaleString("es-PE",{minimumFractionDigits:0})}`, color:"#4ADE80" },
          { label:"Contratos", val:contratosAnio.length, color:"#fff" },
        ].map(({label,val,color})=>(
          <div key={label}>
            <div style={{ fontSize:20, fontWeight:900, color, letterSpacing:"-0.5px", lineHeight:1 }}>{val}</div>
      ...
      <div style={{ marginLeft:-20, marginRight:-20, padding:"0 20px 0", borderTop:"1px solid rgba(255,255,255,0.07)" }}>
        <div style={{ paddingTop:16, paddingBottom:4 }}>
          <div style={{ fontSize:10, color:"rgba(255,255,255,0.3)", fontWeight:600, letterSpacing:1, textTransform:"uppercase", marginBottom:10 }}>Ingresos mensuales {anioEfectivo}</div>
          {/* Barras */}
          <div style={{ display:"flex", alignItems:"flex-end", gap:3, height:60 }}>
            {ingresosPorMes.map((val,i)=>{
              ...
              const futuro = anioEfectivo===new Date().getFullYear() && i>mesActual && val===0;
              return(
                <button key={i} onClick={()=>setMesSel(mesSel===i?null:i)} style={{
                  flex:1, display:"flex", flexDirection:"column", alignItems:"center", gap:3,
                  background:"none", border:"none", cursor:"pointer", touchAction:"manipulation", padding:0,
                }}>
                  <div style={{
          ...
          <div style={{ display:"flex", gap:3, marginTop:4 }}>
            {MESES_LABEL.map((m,i)=>{
              ...
              const esMesActual=anioEfectivo===new Date().getFullYear()&&i===mesActual;
              return(
                <div key={i} style={{ flex:1, textAlign:"center", fontSize:8, color: esSel?"#60A5FA": esMesActual?"#4ADE80":"rgba(255,255,255,0.25)", fontWeight: esSel||esMesActual?700:400 }}>{m}</div>
              );
            })}
          </div>
        </div>
      ...
      {mesSel!==null && (
        <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", padding:"10px 0 12px" }}>
          <span style={{ fontSize:12, color:"#60A5FA", fontWeight:700 }}>
            📅 {MESES_FULL[mesSel]} {anioEfectivo} · {listaFiltrada.length} contratos
          </span>
          <button onClick={()=>setMesSel(null)} style={{ background:"rgba(255,255,255,0.08)", border:"1px solid rgba(255,255,255,0.12)", borderRadius:12, padding:"4px 12px", color:"rgba(255,255,255,0.5)", fontSize:11, fontWeight:600, cursor:"pointer", touchAction:"manipulation", fontFamily:"inherit" }}>
            Ver todos
          </button>
        </div>
      )}
      ...
      {listaFiltrada.length===0 ? (
        <div style={{ padding:"60px 20px", textAlign:"center" }}>
          <div style={{ width:52, height:52, borderRadius:16, background:"#EFF4FF", display:"flex", alignItems:"center", justifyContent:"center", margin:"0 auto 14px" }}>
            <svg width="26" height="26" viewBox="0 0 24 24" fill="none" stroke={T.accent} strokeWidth="1.6" strokeLinecap="round"><path d="M14 2H6a2 2 0 00-2 2v16a2 2 0 002 2h12a2 2 0 002-2V8z"/><polyline points="14 2 14 8 20 8"/></svg>
          </div>
          <div style={{ fontSize:14, fontWeight:600, color:T.text, marginBottom:4 }}>
            {mesSel!==null ? `Sin contratos en ${MESES_FULL[mesSel]}` : `Sin contratos en ${anioEfectivo}`}
          </div>
        ...
        return Object.entries(grupos).sort(([a],[b])=>b.localeCompare(a)).map(([key,items])=>(
          <div key={key} style={{ marginBottom:24 }}>
            {/* Cabecera mes */}
            <div style={{ display:"flex", alignItems:"center", gap:12, padding:"0 16px 12px" }}>
              <div style={{ width:10, height:10, borderRadius:"50%", background:T.accent, flexShrink:0, boxShadow:"0 0 0 3px #DBEAFE" }}/>
              <div style={{ height:1, flex:1, background:T.border }}/>
              <span style={{ fontSize:11, fontWeight:700, color:T.muted, textTransform:"uppercase", letterSpacing:1, flexShrink:0 }}>{labelGrupo(key)}</span>
            ...
            {/* Cards */}
            <div style={{ display:"flex", flexDirection:"column", gap:10, paddingLeft:16, paddingRight:16 }}>
              {items.map(c=>{
                const activo=c.d>0;
                ...
                return(
                  <div key={c.id} style={{ background:"#fff", borderRadius:18, border:"1px solid #E5E7EB", overflow:"hidden", boxShadow:"0 1px 3px rgba(0,0,0,0.04), 0 4px 12px -6px rgba(0,0,0,0.08)" }}>
                    <div style={{ height:3, background:activo?(c.d<=30?T.amber:T.accent):T.border }}/>
                    <div style={{ padding:"14px 16px" }}>
                      <div style={{ display:"flex", alignItems:"flex-start", justifyContent:"space-between", gap:12, marginBottom:10 }}>
                        <div style={{ flex:1, minWidth:0 }}>
                          <div style={{ display:"flex", alignItems:"center", gap:8, marginBottom:3 }}>
                            ...
                            <div style={{ fontSize:15, fontWeight:700, color:T.text, overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap" }}>{c.panel.nombre}</div>
                          </div>
                          <div style={{ fontSize:12, color:T.muted, marginLeft:40 }}>{c.cliente.empresa} · {c.panel.ciudad}</div>
                        </div>
                        <div style={{ textAlign:"right", flexShrink:0 }}>
                          <div style={{ fontSize:17, fontWeight:800, color:T.green, letterSpacing:"-0.5px" }}>{fmt(c.monto)}<span style={{ fontSize:10, color:"#94A3B8", fontWeight:500 }}>/mes</span></div>
                      ...
                      <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between" }}>
                        <div style={{ display:"flex", alignItems:"center", gap:6 }}>
                          <div style={{ width:7, height:7, borderRadius:"50%", background:c.pagado?T.green:T.border }}/>
                          <span style={{ fontSize:11, color:c.pagado?T.green:"#94A3B8", fontWeight:600 }}>{c.pagado?"Pagado":"Pendiente"}</span>
                        </div>
                        <button onClick={()=>openEdit(c)} style={{ display:"inline-flex", alignItems:"center", gap:5, padding:"7px 14px", borderRadius:50, border:"1px solid #E5E7EB", background:"#F8FAFC", color:T.muted, fontWeight:600, fontSize:12, cursor:"pointer", touchAction:"manipulation", fontFamily:"inherit" }}>
    ...
    >
      <div style={{
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 18, gap: 12 }}>
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ fontSize: 11, fontWeight: 700, color: T.muted, textTransform: "uppercase", letterSpacing: 1.5, marginBottom: 4 }}>
              {tipoLabel}
            </div>
            <div style={{ fontSize: 22, fontWeight: 900, color: T.white, fontFamily: "monospace", letterSpacing: "-0.5px" }}>
            ...
            <div style={{ display: "flex", gap: 6, marginTop: 8, flexWrap: "wrap" }}>
              ...
              {factura.sunat_estado && (
                <span style={{ background: T.cyan + "18", color: T.cyan, border: `1px solid ${T.cyan}44`, borderRadius: 8, padding: "3px 11px", fontSize: 11, fontWeight: 700 }}>
                  SUNAT: {factura.sunat_estado}
                </span>
              )}
            </div>
          </div>
          <button onClick={onClose} style={{ background: T.border, border: "none", borderRadius: 8, padding: "5px 11px", color: T.muted, cursor: "pointer", touchAction: "manipulation", fontSize: 15, flexShrink: 0 }}>✕</button>
        </div>

        {/* CLIENTE */}
        <div style={{ background: T.surface, borderRadius: 12, padding: "12px 14px", marginBottom: 10, border: `1px solid ${T.border}` }}>
          <div style={{ fontSize: 10, fontWeight: 700, color: T.muted, textTransform: "uppercase", letterSpacing: 1, marginBottom: 6 }}>
            Cliente
          </div>
          ...
          <div style={{ background: T.surface, borderRadius: 12, padding: "12px 14px", marginBottom: 10, border: `1px solid ${T.border}` }}>
            ...
            {pan && (
              <div style={{ fontSize: 12, color: T.muted }}>
                Panel: <strong style={{color:T.text}}>{pan.nombre}</strong>{pan.ciudad ? ` · ${pan.ciudad}` : ""}
              </div>
            )}
        ...
        {/* MONTOS */}
        <div style={{ background: T.surface, borderRadius: 12, padding: "14px 16px", marginBottom: 10, border: `1px solid ${T.border}` }}>
          {subtotal != null && (
            <div style={{ display: "flex", justifyContent: "space-between", fontSize: 13, color: T.muted, marginBottom: 5 }}>
              <span>Subtotal</span>
              <span style={{ fontFamily: "monospace", color: T.text }}>{monedaSimbolo} {subtotal.toLocaleString("es-PE", { minimumFractionDigits: 2 })}</span>
            </div>
          )}
          {igv != null && (
            <div style={{ display: "flex", justifyContent: "space-between", fontSize: 13, color: T.muted, marginBottom: 8 }}>
              ...
              <span style={{ fontFamily: "monospace", color: igv === 0 ? T.green : T.text }}>{monedaSimbolo} {igv.toLocaleString("es-PE", { minimumFractionDigits: 2 })}</span>
            </div>
          )}
          <div style={{
        ...
        {/* FECHAS */}
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, marginBottom: 10 }}>
          <div style={{ background: T.surface, borderRadius: 12, padding: "10px 14px", border: `1px solid ${T.border}` }}>
            <div style={{ fontSize: 10, fontWeight: 700, color: T.muted, textTransform: "uppercase", letterSpacing: 1, marginBottom: 3 }}>Emisión</div>
            <div style={{ fontSize: 12, fontWeight: 700, color: T.text }}>{fmtF(factura.fecha_emision)}</div>
          </div>
          <div style={{ background: T.surface, borderRadius: 12, padding: "10px 14px", border: `1px solid ${T.border}` }}>
            <div style={{ fontSize: 10, fontWeight: 700, color: T.muted, textTransform: "uppercase", letterSpacing: 1, marginBottom: 3 }}>Vencimiento</div>
            <div style={{ fontSize: 12, fontWeight: 700, color: T.text }}>{fmtF(factura.fecha_vencimiento)}</div>
          </div>
        </div>

        {/* PAGO (si está cobrada y vino info de pago del API) */}
        {isCobrada && (factura.metodo_pago || factura.fecha_pago || factura.nro_operacion) && (
          <div style={{ background: T.green + "10", borderRadius: 12, padding: "12px 14px", marginBottom: 10, border: `1px solid ${T.green}33` }}>
            <div style={{ fontSize: 10, fontWeight: 700, color: T.green, textTransform: "uppercase", letterSpacing: 1, marginBottom: 6 }}>
              ✅ Pago registrado
            </div>
            {factura.metodo_pago && (
        ...
        {factura.hash && (
          <div style={{ background: T.surface, borderRadius: 12, padding: "10px 14px", marginBottom: 10, border: `1px solid ${T.border}` }}>
            <div style={{ fontSize: 10, fontWeight: 700, color: T.muted, textTransform: "uppercase", letterSpacing: 1, marginBottom: 4 }}>
              Hash SUNAT
            </div>
            <div style={{ fontSize: 11, color: T.muted, fontFamily: "monospace", wordBreak: "break-all", lineHeight: 1.4 }}>
              {factura.hash}
            </div>
          </div>
        )}
        ...
        {factura.pdf_url && mostrarPDF && (
          <div style={{
            ...
            <div style={{
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                <span style={{ fontSize: 14 }}>📄</span>
                <span style={{ fontSize: 12, fontWeight: 700, color: T.white, fontFamily: "monospace" }}>
                  {nombreArchivo}
                </span>
              </div>
              <button
                onClick={() => setMostrarPDF(false)}
                style={{ background: "transparent", border: "none", color: T.muted, cursor: "pointer", touchAction: "manipulation", fontSize: 16, lineHeight: 1 }}
              >
                ✕
              </button>
            </div>
        ...
        {/* Nota informativa */}
        <div style={{ marginTop: 14, padding: "8px 12px", background: T.accent + "08", borderRadius: 8, border: `1px solid ${T.accent}22` }}>
...
function Facturacion({ paneles, clientes, contratos }: FacturacionProps) {
  ...
  return (
    <div style={{ paddingBottom: 32 }}>
      {/* ── HEADER ── */}
      <div style={{ display: "flex", alignItems: "center", gap: 14, marginBottom: 20, background:"linear-gradient(135deg,#0F1729 0%,#1E3A8A 100%)", borderRadius:18, padding:"16px 20px", boxShadow:"0 6px 24px rgba(15,23,41,0.32)", position:"relative", overflow:"hidden" }}>
        <div style={{position:"absolute",top:0,left:0,right:0,height:"50%",background:"linear-gradient(180deg,rgba(255,255,255,0.06) 0%,transparent 100%)",borderRadius:"18px 18px 0 0",pointerEvents:"none"}}/>
        <div style={{ width: 42, height: 42, borderRadius: 12, background: "rgba(255,255,255,0.14)", border:"1px solid rgba(255,255,255,0.22)", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, position:"relative", zIndex:1 }}>
        ...
        <div style={{ flex: 1, minWidth: 0, position:"relative", zIndex:1 }}>
          <div style={{ fontSize: 17, fontWeight: 800, color: "#fff", letterSpacing: "-0.3px", lineHeight: 1.2 }}>Facturación</div>
          <div style={{ fontSize: 12, color: "rgba(180,200,255,0.7)", fontWeight: 500, marginTop:3 }}>
            {loading ? "Cargando..." : `${facturas.length} comprobante${facturas.length !== 1 ? "s" : ""} registrado${facturas.length !== 1 ? "s" : ""}`}
          </div>
        </div>
      </div>

      {error && (
        <div style={{ background: T.red + "12", border: `1px solid ${T.red}44`, borderRadius: 12, padding: "10px 14px", marginBottom: 16, fontSize: 12, color: T.red }}>
          ⚠️ {error}
        </div>
      )}
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: 20 }}>
        {[
          { label: "TOTAL FACTURADO", val: fmt(kpis.total),      valueColor: T.white, labelColor: "#5B7FCC", sub: `${kpis.count} comprobante${kpis.count !== 1 ? "s" : ""}`, accent: T.accent,
            icon: <svg width="22" height="22" viewBox="0 0 24 24" fill="none"><rect x="3" y="6" width="18" height="13" rx="2" stroke={T.white} strokeWidth="1.6"/><line x1="3" y1="10" x2="21" y2="10" stroke={T.white} strokeWidth="1.6"/><line x1="6.5" y1="14.5" x2="9.5" y2="14.5" stroke={T.white} strokeWidth="1.6" strokeLinecap="round"/></svg>
          },
          { label: "COBRADO", val: fmt(kpis.cobradas), valueColor: T.green, labelColor: "#5B7FCC", sub: `${facturas.filter(f => f.estado === "Cobrada" || f.estado === "Pagada").length} cobrada${facturas.filter(f => f.estado === "Cobrada" || f.estado === "Pagada").length !== 1 ? "s" : ""}`, accent: T.green,
            icon: <svg width="22" height="22" viewBox="0 0 24 24" fill="none"><circle cx="12" cy="12" r="9" stroke={T.green} strokeWidth="1.6"/><polyline points="8 12 11 15 16 9" stroke={T.green} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" fill="none"/></svg>
          },
          { label: "PENDIENTE COBRO", val: fmt(kpis.pendientes), valueColor: T.white, labelColor: "#5B7FCC", sub: `${facturas.filter(f => ["Emitida","Aceptada","Pendiente"].includes(f.estado)).length} emitida${facturas.filter(f => ["Emitida","Aceptada","Pendiente"].includes(f.estado)).length !== 1 ? "s" : ""}`, accent: T.amber,
            icon: <svg width="22" height="22" viewBox="0 0 24 24" fill="none"><path d="M6 2H18M6 22H18M6 2V8C6 10 9 12 12 12C15 12 18 10 18 8V2M6 22V16C6 14 9 12 12 12C15 12 18 14 18 16V22" stroke={T.white} strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round"/></svg>
          },
          { label: "VENCIDAS", val: fmt(kpis.vencidas), valueColor: T.white, labelColor: "#5B7FCC", sub: `${facturas.filter(f => f.estado === "Vencida").length} sin cobrar`, accent: T.red,
            icon: <svg width="22" height="22" viewBox="0 0 24 24" fill="none"><path d="M12 4C9 4 7 6 7 9V13L5 16H19L17 13V9C17 6 15 4 12 4Z" stroke={T.white} strokeWidth="1.6" strokeLinejoin="round"/><circle cx="12" cy="9" r="0.8" fill={T.white}/><line x1="12" y1="11" x2="12" y2="13" stroke={T.white} strokeWidth="1.6" strokeLinecap="round"/><path d="M10 19C10 20 11 21 12 21C13 21 14 20 14 19" stroke={T.white} strokeWidth="1.6" strokeLinecap="round"/></svg>
          },
        ].map((k, i) => (
          <div key={i} style={{
            position: "relative", overflow: "hidden",
            ...
            <div style={{ position: "relative", zIndex: 2 }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 12 }}>
                ...
                <div style={{ width: 38, height: 38, borderRadius: 10, border: `1px solid ${k.accent === T.green ? "rgba(16,185,129,0.4)" : "rgba(255,255,255,0.18)"}`, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                  {k.icon}
                </div>
              </div>
      ...
      <div style={{ display: "flex", gap: 10, marginBottom: 14 }}>
        <button onClick={() => toast.info("Usa el botón emitir desde un contrato para generar facturas.")}
          style={{
            ...
            padding: "12px 18px", color: T.white, fontSize: 13, fontWeight: 700,
        ...
        <div style={{ flex: 1, position: "relative" }}>
          ...
          <input
            ...
            style={{ width: "100%", background: T.white, border: "1px solid #E5E7EB", borderRadius: 12, padding: "12px 14px 12px 38px", color: T.text, fontSize: 13, outline: "none", fontFamily: "inherit", boxSizing: "border-box" }}
          />
        </div>
      </div>
      ...
      <div style={{ display: "flex", gap: 8, marginBottom: 18, alignItems: "center" }}>
          ...
          {[
            ...
            <div key={i} style={{ position: "relative", flexShrink: 0 }}>
              <select value={f.value} onChange={e => f.onChange(e.target.value)}
                style={{ background: T.white, border: "1px solid #E5E7EB", borderRadius: 10, padding: "10px 32px 10px 14px", color: T.text, fontSize: 12, fontWeight: 600, outline: "none", fontFamily: "inherit", appearance: "none", cursor: "pointer", touchAction: "manipulation", boxShadow: "0 1px 3px rgba(15,23,41,0.04)" }}>
        ...
        <div style={{ display: "flex", gap: 6, flexShrink: 0 }}>
          <button onClick={() => setVista("lista")} style={{ width: 42, height: 40, borderRadius: 10, border: vista === "lista" ? "1px solid rgba(37,99,235,0.4)" : "1px solid #E5E7EB", background: vista === "lista" ? "rgba(37,99,235,0.08)" : T.white, cursor: "pointer", touchAction: "manipulation", display: "flex", alignItems: "center", justifyContent: "center" }}>
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none">
              <line x1="8" y1="6" x2="20" y2="6" stroke={vista === "lista" ? T.accent : "#0F1729"} strokeWidth="2" strokeLinecap="round"/>
              <line x1="8" y1="12" x2="20" y2="12" stroke={vista === "lista" ? T.accent : "#0F1729"} strokeWidth="2" strokeLinecap="round"/>
              <line x1="8" y1="18" x2="20" y2="18" stroke={vista === "lista" ? T.accent : "#0F1729"} strokeWidth="2" strokeLinecap="round"/>
          ...
          <button onClick={() => setVista("resumen")} style={{ width: 42, height: 40, borderRadius: 10, border: vista === "resumen" ? "1px solid rgba(37,99,235,0.4)" : "1px solid #E5E7EB", background: vista === "resumen" ? "rgba(37,99,235,0.08)" : T.white, cursor: "pointer", touchAction: "manipulation", display: "flex", alignItems: "center", justifyContent: "center" }}>
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none">
              <rect x="4" y="13" width="3" height="8" fill={vista === "resumen" ? T.accent : "#0F1729"}/>
              <rect x="10.5" y="9" width="3" height="12" fill={vista === "resumen" ? T.accent : "#0F1729"}/>
              <rect x="17" y="5" width="3" height="16" fill={vista === "resumen" ? T.accent : "#0F1729"}/>
            </svg>
          </button>
        </div>
      </div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 14, marginBottom: 20 }}>
                  ...
                  <div key={est} style={{ marginBottom: 12 }}>
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 5 }}>
                      <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                        ...
                        <span style={{ fontSize: 11, color: T.muted }}>{facs.length} comprobantes</span>
                      </div>
          ...
          <Card>
            ...
            {(() => {
              ...
              if (top.length === 0) return <div style={{ fontSize: 12, color: T.muted, textAlign: "center", padding: "12px 0" }}>Sin datos para mostrar</div>;
              return top.map((c, i) => {
                const pct = c.total > 0 ? Math.round((c.cobrado / c.total) * 100) : 0;
                return (
                  <div key={i} style={{ display: "flex", alignItems: "center", gap: 12, padding: "10px 0", borderBottom: i < top.length - 1 ? `1px solid ${T.border}` : "none" }}>
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ fontSize: 13, fontWeight: 700, color: T.white, marginBottom: 3, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{c.nombre}</div>
                      <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
                        <div style={{ background: T.border, borderRadius: 4, height: 4, flex: 1, overflow: "hidden" }}>
                          <div style={{ height: "100%", width: `${pct}%`, background: T.green, borderRadius: 4 }} />
                        </div>
                        <span style={{ fontSize: 11, color: T.muted, flexShrink: 0 }}>{pct}% cobrado</span>
                      </div>
                    </div>
                    <div style={{ textAlign: "right", flexShrink: 0 }}>
                      <div style={{ fontSize: 14, fontWeight: 800, color: T.text, fontFamily: "monospace" }}>{fmt(c.total)}</div>
                ...
                return (
                  <div key={f.id}
                    ...
                    <div style={{ position: "relative", zIndex: 2 }}>
                      ...
                      <div style={{ display: "flex", alignItems: "flex-start", gap: 12, marginBottom: 10 }}>
                        <div onClick={() => setModalDetalle(f)} style={{ flexShrink: 0, width: 44, height: 52, borderRadius: 8, background: T.white, border: "1px solid #E5E7EB", display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", touchAction: "manipulation", boxShadow: "0 2px 6px rgba(0,0,0,0.25)" }}>
                          <svg width="26" height="32" viewBox="0 0 24 28" fill="none">
                            <rect x="3" y="2" width="18" height="24" rx="1" fill={T.white} stroke="#0F1729" strokeWidth="1.2"/>
                        ...
                        <div onClick={() => setModalDetalle(f)} style={{ flex: 1, minWidth: 0, cursor: "pointer", touchAction: "manipulation" }}>
                          <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 6, flexWrap: "wrap" }}>
                            <span style={{ fontFamily: "monospace", fontSize: 16, fontWeight: 900, color: T.white, letterSpacing: "-0.2px" }}>
                          ...
                          <div style={{ fontSize: 13, color: T.white, fontWeight: 600, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis", marginBottom: 2 }}>{cli}</div>
                          <div style={{ fontSize: 11, color: "rgba(160,180,220,0.6)", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>
                        ...
                        <div onClick={() => setModalDetalle(f)} style={{ flexShrink: 0, textAlign: "right", cursor: "pointer", touchAction: "manipulation" }}>
                          <div style={{ fontSize: 18, fontWeight: 900, color: isCobrada ? T.green : isVencida ? T.red : T.white, fontFamily: "monospace", letterSpacing: "-0.3px" }}>
                            {fmt(total)}
                          </div>
                        </div>
                      </div>
                        ...
                        <div style={{ display: "flex", gap: 8, justifyContent: "flex-end", flexWrap: "wrap" }}>
                          ...
                          {isCobrable && (
                            <button onClick={(e) => { e.stopPropagation(); toast.info("Marca esta factura como cobrada en tu sistema."); }}
                              style={{ background: "rgba(16,185,129,0.12)", border: "1px solid rgba(16,185,129,0.45)", borderRadius: 10, padding: "7px 14px", color: T.green, fontSize: 12, fontWeight: 700, cursor: "pointer", touchAction: "manipulation", display: "flex", alignItems: "center", gap: 6, fontFamily: "inherit" }}>
                              <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke={T.green} strokeWidth="2.4" strokeLinecap="round" strokeLinejoin="round">
    ...
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.9)",zIndex:500,
      ...
      <div style={{background:"#0B1324",border:"1px solid #1E3050",
        ...
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"8px 22px 18px"}}>
          <div>
            <div style={{fontSize:17,fontWeight:800,color:T.white}}>🧾 Generar Comprobante PDF</div>
            <div style={{fontSize:12,color:T.muted,marginTop:2}}>RUC {EMISOR.ruc} · 8 Millas · Huánuco</div>
          </div>
          <button onClick={onClose} style={{width:30,height:30,borderRadius:"50%",
            background:"#1E3050",border:"none",color:T.muted,cursor:"pointer",touchAction:"manipulation",
        ...
        <div style={{padding:"0 22px",display:"flex",flexDirection:"column",gap:16}}>
          ...
          <div>
            <div style={{fontSize:11,fontWeight:700,color:T.muted,textTransform:"uppercase",letterSpacing:1.2,marginBottom:8}}>Tipo de Comprobante</div>
            <div style={{display:"flex",gap:8}}>
              {["FACTURA","BOLETA"].map(t=>(
                <button key={t} onClick={()=>{setTipo(t);setSerie(t==="FACTURA"?"F001":"B001");}}
                  style={{flex:1,padding:"11px",borderRadius:10,cursor:"pointer",touchAction:"manipulation",
                    ...
                    color:tipo===t?T.white:T.muted,fontWeight:700,fontSize:13,transition:"background .08s"}}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
            <div>
              <div style={{fontSize:11,fontWeight:700,color:T.muted,textTransform:"uppercase",letterSpacing:1.2,marginBottom:6}}>Serie</div>
              <input value={serie} onChange={e=>setSerie(e.target.value.toUpperCase())} style={inpStyle} placeholder="F001"/>
            </div>
            <div>
              <div style={{fontSize:11,fontWeight:700,color:T.muted,textTransform:"uppercase",letterSpacing:1.2,marginBottom:6}}>Número</div>
              <input value={numero} onChange={e=>setNumero(e.target.value.replace(/\D/g,""))} style={inpStyle} placeholder="1"/>
            </div>
          </div>

          <div style={{background:T.surface,border:`1px solid ${exonerado ? T.green : T.border}`,borderRadius:12,padding:"14px 16px"}}>
            <div style={{fontSize:11,fontWeight:700,color:T.muted,textTransform:"uppercase",letterSpacing:1.2,marginBottom:10}}>Resumen del Comprobante</div>
            {exonerado && (
              <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:10,padding:"6px 10px",background:T.green+"18",borderRadius:8,border:`1px solid ${T.green}44`}}>
                <span style={{fontSize:13}}>🌿</span>
                <span style={{fontSize:11,fontWeight:700,color:T.green}}>Exonerado de IGV — Ley Amazonía N° 27037 (Huánuco)</span>
              </div>
            )}
            <div style={{display:"flex",flexDirection:"column",gap:6}}>
              {[
                ...
                {l:tipo==="FACTURA"?"RUC":"DNI", v:cliente.ruc||cliente.dni||"Sin documento", c:cliente.ruc||cliente.dni?T.text:T.amber},
                {l:"Panel",        v:`${panel.nombre} · ${panel.ciudad||""}`, c:T.text},
                {l:"Período",      v:`${fmtF(contrato.inicio)} → ${fmtF(contrato.fin)}`, c:T.muted},
                {l:exonerado?"Op. Inafecta":"Valor s/IGV", v:`S/ ${subtotal.toLocaleString("es-PE",{minimumFractionDigits:2})}`, c:T.text},
                {l:exonerado?"IGV (0% — Exonerado)":"IGV 18%", v:`S/ ${igv.toLocaleString("es-PE",{minimumFractionDigits:2})}`, c:exonerado?T.green:T.text},
                {l:"TOTAL",        v:fmt(contrato.monto), c:T.green, bold:true},
              ].map(({l,v,c,bold})=>(
                <div key={l} style={{display:"flex",justifyContent:"space-between",alignItems:"center",paddingBottom:4,borderBottom:`1px solid ${T.border}22`}}>
          ...
          <div style={{display:"flex",gap:10,flexWrap:"wrap"}}>
            ...
            <button onClick={descargar} style={{flex:1,padding:"13px 22px",
              ...
              borderRadius:50,color:T.white,fontWeight:700,fontSize:14,cursor:"pointer",touchAction:"manipulation",
              boxShadow:"0 4px 22px rgba(30,53,200,0.45), inset 0 1px 0 rgba(255,255,255,0.22)",
              display:"flex",alignItems:"center",justifyContent:"center",gap:8,
              minHeight:46,fontFamily:"inherit",letterSpacing:"0.01em"}}>
              <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round"><path d="M14 2H6a2 2 0 00-2 2v16a2 2 0 002 2h12a2 2 0 002-2V8z"/><polyline points="14 2 14 8 20 8"/><line x1="12" y1="18" x2="12" y2="12"/><polyline points="9 15 12 18 15 15"/></svg>
...
// ══════════════════════════════════════════════════════════════════
function Wave({ color = T.accent, data = [3,4,3,5,4,6,5,7,6,8,7,9] }: { color?: string; data?: number[] }) {
...
// ══════════════════════════════════════════════════════════════════
function HeroCard({ panalesLibres, contratos = [], setTab = () => {}, userName = "" }: HeroCardProps) {
    ...
    <div style={{
      ...
      <div style={{ position:"relative", marginTop:4 }}>
        <div style={{ minWidth:0 }}>
          <div style={{ fontSize:12, color:"rgba(200,212,240,0.62)", fontWeight:500 }}>{saludo}{userName ? ", " + userName.split(" ")[0] : ""}</div>
...
function MetricCardNL({ icon, label, value, sub, waveColor, valueColor }: MetricCardNLProps) {
  return (
    <div style={{ background: T.white, borderRadius: 18, padding: "14px 12px 12px", minWidth: 0, border: `1px solid ${T.border}`, boxShadow: "0 1px 0 rgba(16,22,40,0.02), 0 6px 24px -16px rgba(16,22,40,0.18)", display: "flex", flexDirection: "column" }}>
      ...
      <div style={{ fontSize: 26, fontWeight: 800, color: valueColor || T.text, lineHeight: 1, letterSpacing: "-0.03em" }}>{value}</div>
      <div style={{ fontSize: 11, color: T.muted, marginTop: 6, lineHeight: 1.35, minHeight: 28 }}>{sub}</div>
      <div style={{ marginTop: 4, marginLeft: -2, marginRight: -2 }}><Wave color={waveColor || T.accent} /></div>
    </div>
  );
}
  ...
  return (
    <div style={{ background: "#0D1020", borderRadius: 20, padding: 18, marginBottom: 20 }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
        ...
        <button onClick={() => setTab("paneles")} style={{ background: T.accent, border: "none", borderRadius: 50, padding: "9px 16px", color: "#fff", fontSize: 12, fontWeight: 700, cursor: "pointer", touchAction: "manipulation", whiteSpace: "nowrap", flexShrink: 0 }}>
          Asignar ahora ›
        </button>
      </div>
    </div>
...
// ══════════════════════════════════════════════════════════════════
function ResumenNuevo({ clientes, contratos, paneles, gastos, setTab, userName = "" }: ResumenNuevoProps) {
  const hoyStr = new Date().toISOString().slice(0,10);
  const ocupadosHoy = new Set(
    contratos.filter(c => !c.deleted && c.inicio && c.fin && c.inicio <= hoyStr && c.fin >= hoyStr)
      .map(c => c.panel_id)
  );
  const panalesLibres = paneles.filter(p => !ocupadosHoy.has(p.id) && p.estado !== "Inactivo").length;
  ...
  return (
    <div style={{ paddingBottom: 32 }}>
      <HeroCard panalesLibres={panalesLibres} contratos={contratos} setTab={setTab} userName={userName} />
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 20 }}>
        <MetricCardNL icon={iDollar} label="Cobrado este mes" value={fmtS(ingreso)} sub={`${contratos.filter(c=>{ if(!c.inicio||!c.fin||c.deleted) return false; const ini=c.inicio.slice(0,7),fin=c.fin.slice(0,7); return ini<=mesActual&&fin>=mesActual&&c.pagosMeses?.[mesActual]===true; }).length} contratos pagados`} waveColor={T.accent}/>
        <MetricCardNL icon={iPanels} label="Paneles libres"  value={panalesLibres} valueColor={T.accent} sub="Listos para asignar" waveColor={T.accent}/>
        <MetricCardNL icon={iClock}  label="Por cobrar"      value={fmtS(porcobrar)} valueColor={T.accent} sub={`${contratos.filter(c=>{ if(!c.inicio||!c.fin||c.deleted) return false; const ini=c.inicio.slice(0,7),fin=c.fin.slice(0,7); return ini<=mesActual&&fin>=mesActual&&!c.pagosMeses?.[mesActual]; }).length} contratos pendientes`} waveColor={T.accent}/>
        <MetricCardNL icon={iCal}    label="Vencen pronto"   value={vencenProx} sub="Próximos 30 días" waveColor={T.accent}/>
      </div>
...
function CapModal({ open, onClose, title, subtitle, children }: CapModalProps) {
  ...
  return (
    <div onClick={e=>e.target===e.currentTarget&&onClose()} style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.65)",zIndex:700,display:"flex",alignItems:"flex-end",justifyContent:"center"}}>
      <div style={{width:"100%",maxWidth:540,background:T.dark,borderRadius:"24px 24px 0 0",maxHeight:"90vh",display:"flex",flexDirection:"column",boxShadow:"0 -12px 48px rgba(0,0,0,0.6)"}}>
...
function DarkInput({ label, value, onChange, type = "text", placeholder, prefix, suffix }: DarkInputProps) {
  return (
    <div style={{marginBottom:14}}>
  ...
  return (
    <div style={{marginBottom:14}}>
      ...
      <select value={value} onChange={onChange} style={{width:"100%",background:"rgba(255,255,255,0.06)",border:"1px solid rgba(255,255,255,0.12)",borderRadius:12,padding:"13px 14px",color:"#fff",fontSize:14,fontFamily:"inherit",outline:"none",appearance:"none"}}>
        {options.map(o=><option key={o.value} value={o.value} style={{background:T.dark}}>{o.label}</option>)}
      </select>
    </div>
  );
}
...
function ProgressBar({ pct, color = "#60A5FA", height = 8, bg = "rgba(255,255,255,0.08)" }: ProgressBarProps) {
  return (
...
function Capital({ paneles, contratos, gastos, proveedores }: CapitalProps) {
  ...
  useEffect(() => {
    (async () => {
      ...
      } catch(e) { console.error("[Capital] Error cargando configuracion/capitalv2:", e); }
  ...
  const saveData = async (patch) => {
    ...
    setData(next);
    try { await setDoc(doc(db,"configuracion","capitalv2"), next, {merge:true}); } catch(e) { console.error("[Capital] Error guardando configuracion:", e); }
  };
...
// ══════════════════════════════════════════════════════════════════
function TrashModal({ open, onClose, contratos, clientes, paneles, proveedores, setContratos, setClientes, setPaneles, setProveedores }: TrashModalProps) {
  ...
  useEffect(() => {
    ...
    const loadDeleted = async () => {
      ...
      } catch(e) { console.error("[Papelera] Error cargando elementos eliminados:", e); }
    };
    loadDeleted();
  }, [open]);
            ...
            <div key={item.id} style={{
              ...
              <div style={{ display: "flex", gap: 6, flexShrink: 0 }}>
                ...
                <button onClick={() => eliminarPermanente(tab, item.id)} style={{
                  ...
                  cursor: "pointer", touchAction: "manipulation",
                  display: "flex", alignItems: "center", justifyContent: "center", color: T.red
                }}>
...
// ══════════════════════════════════════════════════════════════════
// ══════════════════════════════════════════════════════════════════
// 💰 CENTRO DE COBRANZA + TIMELINE OPERATIVO — Módulos SaaS
// Arquitectura event-driven · Moonflow / Linear inspired
// ══════════════════════════════════════════════════════════════════

// ── Nuevos tipos ─────────────────────────────────────────────────
type TipoActividad =
  | "pago" | "contrato_firmado" | "panel_liberado" | "mora_detectada"
  | "whatsapp_enviado" | "email_enviado" | "factura_emitida"
  | "kpis_actualizados" | "alerta_creada";

type EtapaCobranza =
  | "sin_contactar" | "contactado" | "negociando" | "comprometido" | "accion_legal";

interface ActividadDoc {
  id: string;
  tipo: TipoActividad;
  titulo: string;
  descripcion?: string;
...
function calcRiskScore(diasMora: number, monto: number, intentosFallidos: number): number {
  const porMora      = Math.min(50, Math.round((diasMora / 90) * 50));
  const porMonto     = Math.min(25, Math.round((monto / 20000) * 25));
  const porIntentos  = intentosFallidos === 0 ? 0 : intentosFallidos === 1 ? 10 : intentosFallidos === 2 ? 20 : 25;
  return Math.min(100, porMora + porMonto + porIntentos);
}

function riskLabel(score: number): { label: string; bg: string; color: string } {
  if (score >= 80) return { label: "Crítico",   bg: "#FEE2E2", color: "#B91C1C" };
  if (score >= 60) return { label: "Alto",      bg: "#FEF3C7", color: "#92400E" };
  if (score >= 40) return { label: "Medio",     bg: "#DBEAFE", color: "#1E40AF" };
  return              { label: "Bajo",       bg: "#DCFCE7", color: "#166534" };
}

// ── Helpers event-bus ─────────────────────────────────────────────
async function logActividad(data: Omit<ActividadDoc, "id" | "createdAt">) {
  try {
    await addDoc(collection(db, "actividad"), { ...data, createdAt: serverTimestamp() });
  } catch (e) { console.error("[Vista360] logActividad:", e); }
}

async function crearAlerta(data: Omit<AlertaDoc, "id" | "createdAt" | "leida" | "archivada">) {
  try {
    await addDoc(collection(db, "alertas"), {
      ...data, leida: false, archivada: false, createdAt: serverTimestamp(),
    });
  } catch (e) { console.error("[Vista360] crearAlerta:", e); }
}

async function onContratoVence(contrato: Contrato, cliente: Cliente, panel: Panel) {
  try {
    const batch = writeBatch(db);
    // 1. Liberar panel
    batch.update(doc(db, "paneles", panel.id), { estado: "Disponible" });
    // 2. Actualizar estado del cliente
    batch.update(doc(db, "clientes", cliente.id), { estado: "Inactivo" });
    await batch.commit();
    // 3. Log en timeline
    await logActividad({
      tipo: "panel_liberado",
      titulo: "Panel liberado por vencimiento",
      descripcion: `${panel.nombre} — ${cliente.empresa}`,
    ...
    await crearAlerta({
      tipo: "mora_detectada",
      titulo: `Contrato vencido — ${cliente.empresa}`,
      descripcion: `Panel ${panel.nombre} · S/ ${Number(contrato.monto || 0).toLocaleString("es-PE")}`,
      clienteId: cliente.id,
      contratoId: contrato.id,
      severidad: "critical",
    });
  } catch (e) { console.error("[Vista360] onContratoVence:", e); }
}

// ── Hook: useCobranza ─────────────────────────────────────────────
function useCobranza(contratos: Contrato[], clientes: Cliente[], paneles: Panel[]) {
  const today = new Date();
  today.setHours(0, 0, 0, 0);

  const morosos = useMemo(() => {
    return contratos
      .filter(c => !c.deleted && !c.pagado && new Date(c.fin) < today)
      .map(c => {
        const cliente = clientes.find(cl => cl.id === c.cliente_id);
        const panel   = paneles.find(p => p.id === c.panel_id);
        const diasMora = Math.max(0, Math.ceil((today.getTime() - new Date(c.fin).getTime()) / 86400000));
        const monto    = Number(c.monto) || 0;
        const score    = calcRiskScore(diasMora, monto, 0);
        return { contrato: c, cliente, panel, diasMora, monto, score, ...riskLabel(score) };
      })
      .sort((a, b) => b.score - a.score);
  }, [contratos, clientes, paneles]);

  const kpis = useMemo(() => {
    const totalMora = morosos.reduce((s, m) => s + m.monto, 0);
    const activosMes = contratos.filter(c => !c.deleted && c.pagado).length;
    const ingMes    = contratos
      .filter(c => !c.deleted && c.pagado)
      .reduce((s, c) => s + (Number(c.monto) || 0), 0);
    return { totalMora, activosMes, ingMes, morososCount: morosos.length };
  }, [morosos, contratos]);

  const pipeline = useMemo(() => {
    const etapas: EtapaCobranza[] = ["sin_contactar","contactado","negociando","comprometido","accion_legal"];
    return etapas.map(e => ({ etapa: e, count: e === "sin_contactar" ? morosos.length : 0 }));
  }, [morosos]);

  const sendWhatsApp = useCallback((cliente: Cliente, monto: number) => {
    const tel    = (cliente.celular || "").replace(/\D/g, "");
    const msg    = encodeURIComponent(`Hola ${cliente.contacto || cliente.empresa}, le recordamos que tiene un pago pendiente de S/ ${monto.toLocaleString("es-PE")}. Por favor contáctenos para regularizar. Gracias.`);
    window.open(`https://wa.me/${tel}?text=${msg}`, "_blank");
    logActividad({ tipo: "whatsapp_enviado", titulo: "WhatsApp enviado", clienteNombre: cliente.empresa, clienteId: cliente.id, monto });
  }, []);

  const sendEmail = useCallback((cliente: Cliente) => {
    if (cliente.email) window.open(`mailto:${cliente.email}?subject=Recordatorio de pago — Vista360&body=Estimado ${cliente.contacto || cliente.empresa},%0A%0ALe recordamos que tiene un pago pendiente.%0A%0AAtentamente.`, "_blank");
    logActividad({ tipo: "email_enviado", titulo: "Email enviado", clienteNombre: cliente.empresa, clienteId: cliente.id });
  }, []);

  return { morosos, kpis, pipeline, sendWhatsApp, sendEmail };
}

// ── Hook: useTimeline ─────────────────────────────────────────────
function useTimeline(maxEventos = 30) {
  const [eventos, setEventos] = useState<ActividadDoc[]>([]);
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(q, snap => {
      setEventos(snap.docs.map(d => ({ id: d.id, ...d.data() }) as ActividadDoc));
      setLoading(false);
    }, () => setLoading(false));
    return unsub;
  }, [maxEventos]);

  const FILTROS: Record<string, TipoActividad[]> = {
    Pagos:     ["pago", "factura_emitida"],
    Contratos: ["contrato_firmado"],
    Paneles:   ["panel_liberado"],
    Alertas:   ["mora_detectada", "alerta_creada"],
    Mensajes:  ["whatsapp_enviado", "email_enviado"],
  };

  const eventosFiltrados = useMemo(() =>
    filtro === "Todos" ? eventos : eventos.filter(e => FILTROS[filtro]?.includes(e.tipo)),
  [eventos, filtro]);

  return { eventos: eventosFiltrados, filtro, setFiltro, loading };
}
...
function useAlertas() {
  ...
  useEffect(() => {
    const q = query(collection(db, "alertas"), orderBy("createdAt", "desc"), limit(50));
    return onSnapshot(q, snap => {
      setAlertas(snap.docs.map(d => ({ id: d.id, ...d.data() }) as AlertaDoc).filter(a => !a.archivada));
    }, () => {});
  }, []);

  const marcarLeida = useCallback(async (id: string) => {
    await updateDoc(doc(db, "alertas", id), { leida: true });
  }, []);

  const archivar = useCallback(async (id: string) => {
    await updateDoc(doc(db, "alertas", id), { archivada: true });
  }, []);

  return { alertas, noLeidas: alertas.filter(a => !a.leida).length, marcarLeida, archivar };
}

// ── Componente: CentroCobranza ────────────────────────────────────
function CentroCobranza({ contratos, clientes, paneles, gastos }: {
  contratos: Contrato[]; clientes: Cliente[]; paneles: Panel[]; gastos: Gasto[];
}) {
  const { morosos, kpis, pipeline, sendWhatsApp, sendEmail } = useCobranza(contratos, clientes, paneles);
  const { alertas, noLeidas, archivar } = useAlertas();
  const [seccion, setSeccion] = useState<"mora" | "pipeline" | "alertas">("mora");

  const totalGastos = gastos.filter(g => !g.deleted).reduce((s,g) => s + (Number(g.monto)||0), 0);
  const flujo30 = kpis.ingMes - totalGastos;

  const PIPELINE_LABELS: Record<string,string> = {
    sin_contactar: "Sin contactar", contactado: "Contactado",
    negociando: "Negociando", comprometido: "Comprometido", accion_legal: "Acción legal",
  };
  const PIPELINE_COLORS: Record<string,{bg:string,text:string}> = {
    sin_contactar: {bg:"#F1F5F9",text:"#475569"},
    contactado:    {bg:"#DBEAFE",text:"#1E40AF"},
    negociando:    {bg:"#FEF3C7",text:"#92400E"},
    comprometido:  {bg:"#DCFCE7",text:"#166534"},
    accion_legal:  {bg:"#FEE2E2",text:"#B91C1C"},
  };

  const TIPO_ICONS: Record<string,string> = {
    mora_detectada:"⚠️", vencimiento_3d:"🔔", panel_disponible:"📡",
    riesgo_critico:"🚨", info:"ℹ️",
  };
  ...
  return (
    <div style={{ paddingBottom: 100 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 16 }}>
        {[
          { label: "Total en mora",     valor: `S/ ${kpis.totalMora.toLocaleString("es-PE",{minimumFractionDigits:0})}`, color: T.red,    bg: "#FEF2F2" },
          { label: "Contratos activos", valor: String(kpis.activosMes),     color: T.green,  bg: "#F0FDF4" },
          { label: "Clientes morosos",  valor: String(kpis.morososCount),   color: T.amber,  bg: "#FFFBEB" },
          { label: "Flujo neto 30d",    valor: `S/ ${Math.max(0,flujo30).toLocaleString("es-PE",{minimumFractionDigits:0})}`, color: T.accent, bg: "#EFF4FF" },
        ].map(k => (
          <div key={k.label} style={{ background: k.bg, borderRadius: 16, padding: "14px 16px", border: `1px solid ${k.color}18` }}>
            <div style={{ fontSize: 11, color: T.muted, marginBottom: 4, fontWeight: 600 }}>{k.label}</div>
            <div style={{ fontSize: 22, fontWeight: 800, color: k.color, letterSpacing: "-0.5px" }}>{k.valor}</div>
          </div>
        ))}
      </div>
      ...
      <div style={{ display: "flex", gap: 6, marginBottom: 16, background: "#F1F5F9", borderRadius: 12, padding: 4 }}>
        {(["mora","pipeline","alertas"] as const).map(s => (
          <button key={s} onClick={() => setSeccion(s)} style={{
            flex: 1, padding: "8px 4px", borderRadius: 10, border: "none",
            background: seccion === s ? T.white : "transparent",
            color: seccion === s ? T.text : T.muted,
            fontSize: 12, fontWeight: seccion === s ? 700 : 500,
            cursor: "pointer", touchAction: "manipulation",
            boxShadow: seccion === s ? "0 1px 4px rgba(0,0,0,0.08)" : "none",
            transition: "all .18s",
          }}>
            {s === "mora" ? "Morosos" : s === "pipeline" ? "Pipeline" : `Alertas${noLeidas > 0 ? ` (${noLeidas})` : ""}`}
          </button>
        ))}
      </div>
      ...
      {seccion === "mora" && (
        morosos.length === 0
          ? <div style={{ textAlign:"center", padding: "40px 20px", color: T.muted, fontSize: 14 }}>No hay clientes en mora 🎉</div>
          : morosos.map(m => (
            <div key={m.contrato.id} style={{ background: T.white, borderRadius: 18, padding: 16, marginBottom: 10, border: `1px solid ${m.color}33`, boxShadow: "0 2px 12px rgba(15,23,41,0.04)" }}>
              <div style={{ display: "flex", alignItems: "flex-start", gap: 12, marginBottom: 12 }}>
                {/* Score ring */}
                <div style={{ width: 42, height: 42, borderRadius: "50%", background: m.bg, border: `2px solid ${m.color}44`, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                  <span style={{ fontSize: 13, fontWeight: 800, color: m.color }}>{m.score}</span>
                </div>
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 2 }}>
                    <span style={{ fontSize: 14, fontWeight: 700, color: T.text, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                      {m.cliente?.empresa || "Cliente"}
                    </span>
                    <span style={{ background: m.bg, color: m.color, borderRadius: 6, padding: "2px 8px", fontSize: 11, fontWeight: 700, flexShrink: 0 }}>{m.label}</span>
                  </div>
                  <div style={{ fontSize: 12, color: T.muted }}>{m.panel?.nombre || "Panel"} · {m.diasMora} días mora</div>
                  <div style={{ fontSize: 13, fontWeight: 700, color: T.red, marginTop: 2 }}>S/ {m.monto.toLocaleString("es-PE", { minimumFractionDigits: 2 })}</div>
                </div>
              </div>
              <div style={{ display: "flex", gap: 8 }}>
                <button
                  onClick={() => m.cliente && sendWhatsApp(m.cliente, m.monto)}
                  style={{ flex: 1, padding: "9px 12px", background: "#DCFCE7", border: "1px solid #86EFAC", borderRadius: 12, color: "#166534", fontWeight: 700, fontSize: 12, cursor: "pointer", touchAction: "manipulation", display: "flex", alignItems: "center", justifyContent: "center", gap: 6 }}>
                  <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"/></svg>
                  WhatsApp
                </button>
                <button
                  onClick={() => m.cliente && sendEmail(m.cliente)}
                  disabled={!m.cliente?.email}
                  style={{ flex: 1, padding: "9px 12px", background: "#EFF4FF", border: "1px solid #BFDBFE", borderRadius: 12, color: "#1D4ED8", fontWeight: 700, fontSize: 12, cursor: m.cliente?.email ? "pointer" : "default", touchAction: "manipulation", opacity: m.cliente?.email ? 1 : 0.5, display: "flex", alignItems: "center", justifyContent: "center", gap: 6 }}>
      ...
      {/* Pipeline */}
      {seccion === "pipeline" && (
        <div style={{ background: T.white, borderRadius: 18, padding: 16, border: `1px solid ${T.border}` }}>
          {pipeline.map((p, i) => {
            const c = PIPELINE_COLORS[p.etapa] || {bg:"#F1F5F9",text:"#475569"};
            return (
              <div key={p.etapa} style={{ display: "flex", alignItems: "center", gap: 12, padding: "11px 0", borderBottom: i < pipeline.length - 1 ? `1px solid ${T.border}` : "none" }}>
                <div style={{ width: 10, height: 10, borderRadius: "50%", background: c.bg, border: `2px solid ${c.text}55`, flexShrink: 0 }}/>
                <span style={{ flex: 1, fontSize: 13, color: T.muted, fontWeight: 500 }}>{PIPELINE_LABELS[p.etapa]}</span>
                <span style={{ background: c.bg, color: c.text, borderRadius: 8, padding: "3px 10px", fontSize: 12, fontWeight: 700 }}>
      ...
      {seccion === "alertas" && (
        alertas.length === 0
          ? <div style={{ textAlign:"center", padding: "40px 20px", color: T.muted, fontSize: 14 }}>Sin alertas activas ✓</div>
          : alertas.slice(0, 15).map(a => (
            <div key={a.id} style={{ background: T.white, borderRadius: 16, padding: "12px 14px", marginBottom: 8, border: `1px solid ${a.severidad === "critical" ? "#FCA5A5" : a.severidad === "warning" ? "#FCD34D" : "#93C5FD"}55`, display: "flex", alignItems: "flex-start", gap: 10 }}>
              <span style={{ fontSize: 18, flexShrink: 0 }}>{TIPO_ICONS[a.tipo] || "🔔"}</span>
              <div style={{ flex: 1, minWidth: 0 }}>
              ...
              <button onClick={() => archivar(a.id)} style={{ background: "none", border: "none", color: T.muted, cursor: "pointer", fontSize: 18, padding: 2, lineHeight: 1, touchAction: "manipulation" }}>×</button>
            </div>
          ))
      )}
    </div>
...
// ── Componente: TimelineOperativo ─────────────────────────────────
function TimelineOperativo() {
  const { eventos, filtro, setFiltro, loading } = useTimeline(40);

  const FILTROS = ["Todos","Pagos","Contratos","Paneles","Alertas","Mensajes"];

  const TIPO_CONFIG: Record<string,{icon:string;bg:string;ic:string}> = {
    pago:              {icon:"💰",bg:"#DCFCE7",ic:"#166534"},
    factura_emitida:   {icon:"🧾",bg:"#DCFCE7",ic:"#166534"},
    contrato_firmado:  {icon:"📄",bg:"#DBEAFE",ic:"#1E40AF"},
    panel_liberado:    {icon:"📡",bg:"#F3E8FF",ic:"#7E22CE"},
    mora_detectada:    {icon:"⚠️",bg:"#FEE2E2",ic:"#B91C1C"},
    alerta_creada:     {icon:"🔔",bg:"#FEF3C7",ic:"#92400E"},
    whatsapp_enviado:  {icon:"💬",bg:"#D1FAE5",ic:"#065F46"},
    email_enviado:     {icon:"✉️",bg:"#E0F2FE",ic:"#0C4A6E"},
    kpis_actualizados: {icon:"📊",bg:"#EDE9FE",ic:"#5B21B6"},
  };
  ...
  return (
    <div style={{ paddingBottom: 100 }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 14 }}>
        <div style={{ width: 7, height: 7, borderRadius: "50%", background: T.green, animation: "pulse 1.4s ease-in-out infinite" }}/>
        <span style={{ fontSize: 12, color: T.muted, fontWeight: 600 }}>Actividad en tiempo real · Firebase onSnapshot</span>
      </div>
      ...
      <div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginBottom: 16 }}>
        {FILTROS.map(f => (
          <button key={f} onClick={() => setFiltro(f)} style={{
            ...
            border: filtro === f ? `1px solid ${T.accent}` : `1px solid ${T.border}`,
            background: filtro === f ? T.accentLt : T.white,
            color: filtro === f ? T.accent : T.muted,
            fontSize: 12, fontWeight: filtro === f ? 700 : 500,
      ...
      {/* Feed */}
      {loading ? (
        <div style={{ textAlign: "center", padding: "40px 0", color: T.muted, fontSize: 14 }}>Cargando actividad…</div>
      ) : eventos.length === 0 ? (
        <div style={{ textAlign: "center", padding: "40px 0", color: T.muted, fontSize: 14 }}>
          Sin actividad registrada aún.<br/>
          <span style={{ fontSize: 12 }}>Los eventos aparecen aquí al registrar pagos, contratos y paneles.</span>
        </div>
      ) : (
        <div style={{ background: T.white, borderRadius: 18, padding: "0 16px", border: `1px solid ${T.border}` }}>
          {eventos.map((ev, i) => {
            const cfg = TIPO_CONFIG[ev.tipo] || {icon:"📌",bg:"#F1F5F9",ic:"#475569"};
            return (
              <div key={ev.id} style={{ display: "flex", gap: 12, padding: "13px 0", borderBottom: i < eventos.length-1 ? `1px solid ${T.border}` : "none", alignItems: "flex-start" }}>
                <div style={{ width: 36, height: 36, borderRadius: 10, background: cfg.bg, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 17, flexShrink: 0 }}>{cfg.icon}</div>
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ fontSize: 13, fontWeight: 700, color: T.text }}>{ev.titulo}</div>
                  {ev.descripcion && <div style={{ fontSize: 12, color: T.muted, marginTop: 1, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{ev.descripcion}</div>}
                  {ev.monto && <div style={{ fontSize: 12, fontWeight: 700, color: T.green, marginTop: 2 }}>S/ {Number(ev.monto).toLocaleString("es-PE", { minimumFractionDigits: 2 })}</div>}
                </div>
                <span style={{ fontSize: 11, color: T.muted, flexShrink: 0, marginTop: 2 }}>{fmtTS(ev.createdAt)}</span>
              </div>
            );
          })}
        </div>
...
const DRAWER_ICONS = {
  cobranza: (
    <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.7" strokeLinecap="round" strokeLinejoin="round">
      ...
      <path d="M9 3.5C9 3.5 10.5 2 12 2s3 1.5 3 1.5"/>
    </svg>
  ),
  timeline: (
    <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.7" strokeLinecap="round" strokeLinejoin="round">
...
// 🔍 BÚSQUEDA GLOBAL
// ══════════════════════════════════════════════════════════════════
function BusquedaGlobal({ open, onClose, paneles, clientes, contratos, onNavigate }: BusquedaGlobalProps) {
  ...
  const resultados = useMemo(() => {
    ...
    paneles.forEach(p => {
      ...
      if (haystack.includes(texto)) {
        ...
        res.push({ tipo: "panel", id: p.id, titulo: p.nombre, sub: `${p.ciudad || ""}${p.ubicacion ? " · " + p.ubicacion : ""}`, tag: activo ? "Activo" : "Libre", tagColor: activo ? T.green : T.muted, tab: "paneles" });
      }
    });
    ...
    clientes.forEach(c => {
      ...
      if (haystack.includes(texto)) {
        res.push({ tipo: "cliente", id: c.id, titulo: c.empresa, sub: `${c.contacto || ""}${c.ciudad ? " · " + c.ciudad : ""}`, tag: c.tipo, tagColor: c.tipo === "Cliente" ? T.green : T.accent, tab: "crm" });
      }
    });
    ...
    contratos.forEach(ct => {
      ...
      if (haystack.includes(texto)) {
        ...
        const tagColor = ct.estado === "Activo" ? T.green : ct.estado === "Por vencer" ? T.white : T.red;
    ...
    >
      <div style={{ width: "100%", maxWidth: 560, display: "flex", flexDirection: "column", gap: 0, maxHeight: "80vh" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 10, background: "#fff", borderRadius: 18, padding: "12px 16px", boxShadow: "0 20px 60px rgba(0,0,0,0.35)" }}>
          <svg width="20" height="20" fill="none" viewBox="0 0 24 24" style={{ flexShrink: 0, color: T.muted }}>
            <path d="M21 21L15 15M17 11C17 14.866 13.866 18 10 18C6.134 18 3 14.866 3 11C3 7.134 6.134 4 10 4C13.866 4 17 7.134 17 11Z" stroke={T.muted} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
          </svg>
          <input
            ref={inputRef}
            value={q}
            ...
            style={{ flex: 1, border: "none", outline: "none", fontSize: 16, color: T.text, background: "transparent", fontFamily: "'DM Sans',sans-serif" }}
          />
          {q && (
            <button onClick={() => setQ("")} style={{ background: "#F1F5F9", border: "none", borderRadius: "50%", width: 26, height: 26, display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", touchAction: "manipulation", flexShrink: 0 }}>
              <svg width="14" height="14" fill="none" viewBox="0 0 24 24"><path d="M18 6L6 18M6 6L18 18" stroke={T.muted} strokeWidth="2.5" strokeLinecap="round"/></svg>
            </button>
          )}
          <button onClick={onClose} style={{ background: "#F1F5F9", border: "none", borderRadius: 8, padding: "4px 10px", color: T.muted, fontSize: 12, cursor: "pointer", touchAction: "manipulation", flexShrink: 0, fontFamily: "'DM Sans',sans-serif" }}>Esc</button>
        </div>
          ...
          <div style={{ background: "#fff", borderRadius: 16, marginTop: 8, overflow: "hidden", boxShadow: "0 20px 60px rgba(0,0,0,0.25)", overflowY: "auto" }}>
            {resultados.length === 0 ? (
              ...
              Object.entries(grupos).map(([tipo, items]) => (
                <div key={tipo}>
                  <div style={{ padding: "10px 16px 4px", fontSize: 11, fontWeight: 700, color: T.muted, textTransform: "uppercase", letterSpacing: 1.2, borderTop: "1px solid #F1F5F9" }}>
                  ...
                  {items.map(r => (
                    ...
                    >
                      ...
                      <div style={{ flex: 1, minWidth: 0 }}>
                        ...
                        <div style={{ fontSize: 12, color: T.muted, marginTop: 1, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{r.sub}</div>
                      </div>
        ...
        {q.length < 2 && (
          <div style={{ background: "#fff", borderRadius: 16, marginTop: 8, padding: "16px", boxShadow: "0 20px 60px rgba(0,0,0,0.25)" }}>
            <div style={{ fontSize: 11, fontWeight: 700, color: T.muted, textTransform: "uppercase", letterSpacing: 1.2, marginBottom: 12 }}>Accesos rápidos</div>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
              {[
                { label: "Ver paneles",   svgPath: <svg width="18" height="18" fill="none" viewBox="0 0 24 24"><rect x="3" y="3" width="7" height="7" rx="1.5" stroke={T.accent} strokeWidth="2"/><rect x="14" y="3" width="7" height="7" rx="1.5" stroke={T.accent} strokeWidth="2"/><rect x="3" y="14" width="7" height="7" rx="1.5" stroke={T.accent} strokeWidth="2"/><rect x="14" y="14" width="7" height="7" rx="1.5" stroke={T.accent} strokeWidth="2"/></svg>, tab: "paneles" },
                { label: "Ver contratos", svgPath: <svg width="18" height="18" fill="none" viewBox="0 0 24 24"><path d="M14 2H6C5.448 2 5 2.448 5 3V21C5 21.552 5.448 22 6 22H18C18.552 22 19 21.552 19 21V7L14 2Z" stroke={T.accent} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/><polyline points="14 2 14 8 19 8" stroke={T.accent} strokeWidth="2" strokeLinecap="round"/></svg>, tab: "contratos" },
                { label: "Ver clientes",  svgPath: <svg width="18" height="18" fill="none" viewBox="0 0 24 24"><path d="M17 21V19C17 17.343 15.657 16 14 16H10C8.343 16 7 17.343 7 19V21" stroke={T.accent} strokeWidth="2" strokeLinecap="round"/><circle cx="12" cy="8" r="4" stroke={T.accent} strokeWidth="2"/></svg>, tab: "crm" },
                { label: "Facturación",   svgPath: <svg width="18" height="18" fill="none" viewBox="0 0 24 24"><rect x="3" y="3" width="18" height="18" rx="2" stroke={T.accent} strokeWidth="2"/><path d="M7 8H17M7 12H17M7 16H13" stroke={T.accent} strokeWidth="2" strokeLinecap="round"/></svg>, tab: "facturacion" },
              ].map(a => (
                <button key={a.tab} onClick={() => { onNavigate(a.tab); onClose(); }} style={{ display: "flex", alignItems: "center", gap: 10, padding: "10px 12px", background: "#F8FAFC", border: "1px solid #E5E7EB", borderRadius: 12, cursor: "pointer", touchAction: "manipulation", fontSize: 13, fontWeight: 600, color: T.text, fontFamily: "'DM Sans',sans-serif" }}>
...
function DrawerMenu({ open, onClose, activeTab, onTabClick, onTrashOpen, trashCount = 0, userName = "", userInitials = "?" }: DrawerMenuProps) {
  return (
    <>
      {open && (
        <div
      ...
      <div style={{
        ...
        >
          <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
            <div style={{
              ...
              display: "flex", alignItems: "center", justifyContent: "center",
              color: T.white, fontWeight: 800, fontSize: 17,
              flexShrink: 0,
              letterSpacing: "0.5px",
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ fontSize: 17, fontWeight: 800, color: T.text, letterSpacing: "-0.3px" }}>
                {userName || "—"}
              </div>
        ...
        <div style={{
          ...
          >
            ...
            {trashCount > 0 && (
              <span style={{ background: T.accent, color: "#fff", borderRadius: 99, padding: "2px 8px", fontSize: 11, fontWeight: 700 }}>{trashCount}</span>
            )}
          </button>
        </div>
      </div>
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ fontSize: 22, fontWeight: 800, color: T.white, textAlign: "center", marginBottom: 8, letterSpacing: "-0.5px" }}>
          Bienvenido
        </div>
        ...
        <button
          ...
          style={{
            width: "100%",
            padding: "14px 20px",
            background: loading ? "rgba(255,255,255,0.5)" : T.white,
            color: T.text,
            border: "none",
            borderRadius: 12,
            fontSize: 15,
      ...
      <div style={{
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "14px 16px 10px", borderBottom: `1px solid ${T.border}`, flexShrink: 0 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            ...
            {noLeidas > 0 && (
              <span style={{ background: T.red, color: "#fff", fontSize: 10, fontWeight: 700, borderRadius: 99, padding: "1px 7px" }}>
                {noLeidas}
              </span>
            )}
          </div>
          <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
            {noLeidas > 0 && (
              <button onClick={marcarTodas} style={{ fontSize: 11, color: T.muted, background: "none", border: "none", cursor: "pointer", touchAction: "manipulation", padding: "3px 6px" }}>
                Marcar leídas
              </button>
            )}
            <button onClick={onClose} style={{ background: T.border, border: "none", borderRadius: 8, padding: "4px 10px", color: T.muted, cursor: "pointer", touchAction: "manipulation", fontSize: 14 }}>✕</button>
          </div>
        </div>

        {/* Filtros */}
        <div style={{ display: "flex", gap: 6, padding: "10px 12px", overflowX: "auto", borderBottom: `1px solid ${T.border}`, flexShrink: 0 }}>
          ...
          {visibles.length === 0 ? (
            ...
            return (
              <div key={n.id}
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ fontSize: 12, fontWeight: 700, color: T.text, marginBottom: 2, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{n.titulo}</div>
...
> {
  ...
  render() {
    ...
    return (
      <div style={{
        margin: "24px 16px", padding: "24px 20px",
        ...
        <div style={{ fontSize: 12, color: T.muted, marginBottom: 18, fontFamily: "monospace", wordBreak: "break-word" }}>
          {this.state.message}
        </div>
        <button
...
export default function App() {
  ...
  const [user, setUser] = useState<import("firebase/auth").User | null>(null);   // Firebase Auth user
  ...
  const contractsActive = contratos.filter(x => !x.deleted);

  // ── Auto-detección de contratos vencidos → event bus ────────────
  // Corre solo cuando carga contratos por primera vez (loading → false)
  const vencimientoProcesado = useRef<Set<string>>(new Set());
  useEffect(() => {
    if (loading || contractsActive.length === 0) return;
    const hoyMs = new Date().setHours(0,0,0,0);
    contractsActive.forEach(c => {
      if (c.pagado || vencimientoProcesado.current.has(c.id)) return;
      const finMs = new Date(c.fin).getTime();
      if (finMs < hoyMs) {
        vencimientoProcesado.current.add(c.id);
        const cliente = clientes.find(cl => cl.id === c.cliente_id);
        const panel   = paneles.find(p => p.id === c.panel_id);
        if (cliente && panel) onContratoVence(c, cliente, panel);
      }
    });
  }, [loading, contractsActive.length]);
      ...
      {!splash && !!user && <div className="v360-app-root" style={{ background: T.bg, color: T.text }}>
        ...
        >
          ...
          {loading ? (
            ...
            <>
              {/* ── PERFIL ── */}
              <div className="v360-tab-panel v360-tab-padded" style={show("perfil")}>
                {(() => {
              ...
              <div className="v360-tab-panel v360-tab-padded" style={show("hoy")}>
                ...
                <ErrorBoundary label="Hoy"><ResumenNuevo clientes={clientes} contratos={contratos} paneles={paneles} gastos={gastos} setTab={setTab} userName={userName}/></ErrorBoundary>
              </div>

              <div className="v360-tab-panel v360-tab-padded" style={show("paneles")}>
              ...
              <div className="v360-tab-panel v360-tab-padded" style={show("historico")}>
                <ErrorBoundary label="Histórico"><Historico contratos={contractsActive} setContratos={setContratos} paneles={paneles} clientes={clientes} onModalChange={setAnyModalOpen}/></ErrorBoundary>
              </div>

              <div className="v360-tab-panel v360-tab-padded" style={show("crm")}>
                <ErrorBoundary label="CRM"><CRM clientes={clientes} setClientes={setClientes} contratos={contractsActive} loading={loading} onModalChange={setAnyModalOpen}/></ErrorBoundary>
              </div>

              <div className="v360-tab-panel v360-tab-padded" style={show("resultados")}>
                {/* lazyTab: solo se monta al visitar por primera vez */}
                {lazyTab("resultados") && <ErrorBoundary label="Resultados"><Reportes contratos={contractsActive} paneles={paneles} clientes={clientes} gastos={gastos} initialSeccion="resultados"/></ErrorBoundary>}
              </div>

              <div className="v360-tab-panel v360-tab-padded" style={show("reportes")}>
                {lazyTab("reportes") && <ErrorBoundary label="Reportes"><Reportes contratos={contractsActive} paneles={paneles} clientes={clientes} gastos={gastos}/></ErrorBoundary>}
              </div>

              <div className="v360-tab-panel v360-tab-padded" style={show("gastos")}>
              ...
              <div className="v360-tab-panel v360-tab-padded" style={show("proveedores")}>
                <ErrorBoundary label="Proveedores"><Proveedores proveedores={proveedores} setProveedores={setProveedores} loading={loading} onModalChange={setAnyModalOpen}/></ErrorBoundary>
              </div>

              <div className="v360-tab-panel v360-tab-padded" style={show("facturacion")}>
                {lazyTab("facturacion") && <ErrorBoundary label="Facturación"><Facturacion contratos={contractsActive} paneles={paneles} clientes={clientes}/></ErrorBoundary>}
              </div>

              {/* ── TABS SIN PADDING (tienen cabecera de ancho completo) ── */}
              <div className="v360-tab-panel v360-tab-flush" style={show("contratos")}>
                <ErrorBoundary label="Contratos"><Contratos contratos={contratos} setContratos={setContratos} paneles={paneles} clientes={clientes} loading={loading} setTab={setTab} onModalChange={setAnyModalOpen}/></ErrorBoundary>
              </div>

              <div className="v360-tab-panel v360-tab-flush" style={show("capital")}>
                {lazyTab("capital") && <ErrorBoundary label="Capital"><Capital paneles={paneles} contratos={contractsActive} gastos={gastos} proveedores={proveedores}/></ErrorBoundary>}
              </div>

              <div className="v360-tab-panel v360-tab-flush" style={show("mapa")}>
                {lazyTab("mapa") && <ErrorBoundary label="Mapa"><Mapa paneles={paneles} clientes={clientes} contratos={contractsActive}/></ErrorBoundary>}
              </div>
              <div className="v360-tab-panel v360-tab-padded" style={show("cobranza")}>
                {lazyTab("cobranza") && <ErrorBoundary label="Cobranza"><CentroCobranza contratos={contractsActive} clientes={clientes} paneles={paneles} gastos={gastos}/></ErrorBoundary>}
              </div>

              <div className="v360-tab-panel v360-tab-padded" style={show("timeline")}>
                {lazyTab("timeline") && <ErrorBoundary label="Timeline"><TimelineOperativo/></ErrorBoundary>}
              </div>
            </>
          )}
        </div>
        ...
        <DrawerMenu open={drawerOpen} onClose={() => setDrawerOpen(false)} activeTab={activeTab} onTabClick={handleTabClick} onTrashOpen={() => setTrashOpen(true)} trashCount={contratos.filter(c => c.deleted).length} userName={userName} userInitials={userInitials}/>
        <TrashModal
          open={trashOpen}
