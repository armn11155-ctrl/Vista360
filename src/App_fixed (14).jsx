...
function SwipeRow({ children, onDelete, onEdit, deleteLabel = "Eliminar" }) {
  ...
  return (
    <div ref={rowRef} style={{ position:"relative", overflow:"hidden", marginBottom:10, borderRadius:18 }}>
      {/* Action buttons revealed on swipe */}
      <div style={{ position:"absolute", right:0, top:0, bottom:0, width:ACTION_W, display:"flex", alignItems:"stretch", borderRadius:"0 18px 18px 0", overflow:"hidden" }}>
...
function Capital({ paneles, contratos, gastos, proveedores }) {
  ...
  // Ingreso cobrado del mes actual (contratos pagados)
  const hoy = new Date();
  ...
  const ingMesActual = contratos.filter(c => c.pagado && c.inicio && c.inicio.slice(0,7) === mesActual).reduce((s,c) => s + Number(c.monto||0), 0);
  const ingMesPasado = contratos.filter(c => c.pagado && c.inicio && c.inicio.slice(0,7) === mesPasado).reduce((s,c) => s + Number(c.monto||0), 0);
  // Capital real = capital base + ingresos cobrados este mes
  const capitalReal = capital + ingMesActual;
  const capitalMesPasado = capital + ingMesPasado;
  const difCapital = capitalReal - capitalMesPasado;
  const pctCapital = capitalMesPasado > 0 ? Math.round((difCapital / capitalMesPasado) * 100) : ingMesActual > 0 ? 100 : 0;

  const CARD = { background: D.dark, borderRadius: 20, padding: "22px 20px" };

  return (
    <div style={{ background: "#F8F9FB", margin: "-20px -16px", minHeight: "100%", paddingBottom: "calc(120px + env(safe-area-inset-bottom))" }}>

      {/* ── CARD CAPITAL PRINCIPAL ── */}
      <div style={{ ...CARD, margin: "20px 16px 12px" }}>
        <div style={{ fontSize: 11, color: "rgba(148,175,255,0.5)", fontWeight: 700, letterSpacing: 2, textTransform: "uppercase", marginBottom: 16 }}>Capital disponible</div>

        {editando ? (
          <div>
            <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 14 }}>
              <span style={{ fontSize: 26, fontWeight: 900, color: "rgba(255,255,255,0.2)" }}>S/</span>
              <input ref={inputRef} type="number" inputMode="decimal" value={inputVal}
                ...
                placeholder="0.00"
                style={{ flex: 1, background: "rgba(255,255,255,0.07)", border: "1.5px solid rgba(99,140,255,0.5)", borderRadius: 14, padding: "12px 16px", color: "#fff", fontSize: 32, fontWeight: 900, outline: "none", fontFamily: "inherit", letterSpacing: "-1px" }}
              />
            </div>
            <div style={{ display: "flex", gap: 10 }}>
              <button onClick={() => setEditando(false)} style={{ flex: 1, padding: "13px", background: "rgba(255,255,255,0.06)", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 12, color: "rgba(255,255,255,0.4)", fontWeight: 600, fontSize: 14, cursor: "pointer", touchAction: "manipulation", fontFamily: "inherit" }}>Cancelar</button>
              <button onClick={guardarCapital} disabled={saving} style={{ flex: 2, padding: "13px", background: "#2563EB", border: "none", borderRadius: 12, color: "#fff", fontWeight: 700, fontSize: 14, cursor: "pointer", touchAction: "manipulation", fontFamily: "inherit", opacity: saving ? 0.6 : 1 }}>{saving ? "Guardando…" : "Guardar"}</button>
            </div>
          </div>
        ) : (
          <div>
            {/* Capital real = base + ingresos cobrados del mes */}
            <div style={{ fontSize: 54, fontWeight: 900, color: "#fff", letterSpacing: "-1px", lineHeight: 1, marginBottom: 10, fontFamily: "'Barlow Condensed', sans-serif" }}>
              {capitalLoaded ? fmt(capitalReal) : "—"}
            </div>

            {/* Variación vs mes anterior */}
            <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 22 }}>
              <span style={{ fontSize: 13, fontWeight: 800, color: difCapital >= 0 ? "#34D399" : "#F87171" }}>
                {difCapital >= 0 ? "+" : ""}{fmt(Math.abs(difCapital))}
              </span>
              <span style={{ fontSize: 12, fontWeight: 700, color: difCapital >= 0 ? "#34D399" : "#F87171", background: difCapital >= 0 ? "rgba(52,211,153,0.15)" : "rgba(248,113,113,0.15)", borderRadius: 99, padding: "2px 10px" }}>
                {pctCapital >= 0 ? "+" : ""}{pctCapital}%
              </span>
              <span style={{ fontSize: 11, color: "rgba(148,175,255,0.4)", fontWeight: 500 }}>vs mes anterior</span>
            </div>

            {/* Capital base + ingreso del mes */}
            <div style={{ display: "flex", gap: 8, marginBottom: 20 }}>
              <div style={{ flex: 1, background: "rgba(255,255,255,0.04)", borderRadius: 12, padding: "12px 14px" }}>
                <div style={{ fontSize: 10, color: "rgba(148,175,255,0.4)", fontWeight: 700, letterSpacing: 1, textTransform: "uppercase", marginBottom: 5 }}>Base</div>
                <div style={{ fontSize: 16, fontWeight: 800, color: "rgba(255,255,255,0.75)", letterSpacing: "-0.5px" }}>{capitalLoaded ? fmt(capital) : "—"}</div>
              </div>
              <div style={{ flex: 1, background: "rgba(52,211,153,0.08)", borderRadius: 12, padding: "12px 14px" }}>
                <div style={{ fontSize: 10, color: "rgba(52,211,153,0.5)", fontWeight: 700, letterSpacing: 1, textTransform: "uppercase", marginBottom: 5 }}>Ingresos mes</div>
                <div style={{ fontSize: 16, fontWeight: 800, color: "#34D399", letterSpacing: "-0.5px" }}>+{fmt(ingMesActual)}</div>
              </div>
            </div>

            <button onClick={abrirEditor} style={{ display: "inline-flex", alignItems: "center", gap: 8, background: "rgba(37,99,235,0.2)", border: "1px solid rgba(99,140,255,0.25)", borderRadius: 50, padding: "9px 18px", color: "#93C5FD", fontWeight: 700, fontSize: 13, cursor: "pointer", touchAction: "manipulation", fontFamily: "inherit" }}>
              <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round"><path d="M17 3a2.828 2.828 0 114 4L7.5 20.5 2 22l1.5-5.5L17 3z"/></svg>
              Editar capital base
            </button>
          </div>
        )}
      ...
      {(() => {
        const meses6 = Array.from({ length: 6 }, (_, i) => {
          ...
          return { key: `${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,"0")}`, lbl: ["Ene","Feb","Mar","Abr","May","Jun","Jul","Ago","Sep","Oct","Nov","Dic"][d.getMonth()] };
        });
        const MESES_C2 = ["Ene","Feb","Mar","Abr","May","Jun","Jul","Ago","Sep","Oct","Nov","Dic"];
        const puntos = meses6.map(({ key }) => {
          const ing = contratos.filter(c => c.pagado && c.inicio && c.inicio.slice(0,7) === key).reduce((s,c) => s + Number(c.monto||0), 0);
          return ing;
        });
        ...
        return (
          <div style={{ background: D.dark, borderRadius: 20, margin: "12px 16px 0", padding: "20px 18px" }}>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 16 }}>
              <div style={{ fontSize: 11, color: "rgba(148,175,255,0.5)", fontWeight: 700, letterSpacing: 1.8, textTransform: "uppercase" }}>Crecimiento mensual</div>
              <div style={{ fontSize: 11, fontWeight: 700, color: pctCapital >= 0 ? "#34D399" : "#F87171" }}>{pctCapital >= 0 ? "+" : ""}{pctCapital}% este mes</div>
            </div>
            <svg viewBox={`0 0 ${W} ${H}`} width="100%" height={H} style={{ overflow: "visible" }}>
              <defs>
                <linearGradient id="capGrad" x1="0" y1="0" x2="0" y2="1">
                  <stop offset="0%" stopColor="#3B6EFF" stopOpacity="0.3"/>
                  <stop offset="100%" stopColor="#3B6EFF" stopOpacity="0"/>
                </linearGradient>
              </defs>
              <path d={areaPath} fill="url(#capGrad)"/>
              <path d={linePath} fill="none" stroke="#3B6EFF" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"/>
              {coords.map((p, i) => (
                <circle key={i} cx={p.x} cy={p.y} r={i === coords.length - 1 ? 5 : 3}
                  fill={i === coords.length - 1 ? "#3B6EFF" : "rgba(59,110,255,0.5)"}
                  style={i === coords.length - 1 ? { filter: "drop-shadow(0 0 6px #3B6EFF)" } : {}}
                />
              ))}
            </svg>
            <div style={{ display: "flex", justifyContent: "space-between", marginTop: 8 }}>
              {meses6.map(({ key, lbl }, i) => (
                <div key={key} style={{ fontSize: 9, color: i === 5 ? "rgba(255,255,255,0.8)" : "rgba(148,175,255,0.3)", fontWeight: i === 5 ? 700 : 400, textTransform: "uppercase", textAlign: "center", flex: 1 }}>{lbl}</div>
              ))}
            </div>
          </div>
        );
...
export default function App() {
  ...
  useEffect(() => {
    ...
    const color = navy ? D.dark : "#F2F4F8";
    ...
    <>
      ...
      {!splash && !!user && <div style={{ position: "fixed", top: 0, left: 0, right: 0, bottom: 0, background: NL.bg, display: "flex", flexDirection: "column", fontFamily: "'DM Sans',sans-serif", color: NL.text, overflow: "hidden", overscrollBehavior: "none" }}>
        ...
        <div
          ref={scrollRef}
          data-scroll
          style={{ flex: 1, minHeight: 0, overflowY: "scroll", overflowX: "hidden", overscrollBehavior: "none", touchAction: "pan-y", background: (activeTab === "capital" || activeTab === "contratos") ? D.darker : undefined, position:"relative" }}
        >
          <PullToRefresh scrollRef={scrollRef} onRefresh={async () => { haptic("medium"); toast.info("Sincronizando..."); }}/>
