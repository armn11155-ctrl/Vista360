...
interface ToastEntry { id: number; type: "success"|"error"|"info"|"warn"; msg: string; }
interface ConfirmPayload {
  msg: string; opts: { title?: string; ok?: string; cancel?: string; danger?: boolean };
  resolve: (value: boolean) => void;
}
interface UIContextValue { toast: typeof toast; confirm: typeof confirmAsync; }
...
export function ToastProvider({ children }: { children?: React.ReactNode }) {
  ...
  useEffect(() => {
    _uiRef.addToast = addToast; _uiRef.showConfirm = showConfirm;
  ...
  return (
    <UIContext.Provider value={ctxValue}>
      {children}
      <div aria-live="polite" aria-atomic="true" aria-label="Notificaciones"
        style={{ position:"fixed", bottom:"calc(90px + env(safe-area-inset-bottom))", left:"50%",
          transform:"translateX(-50%)", zIndex:9999, display:"flex", flexDirection:"column",
          gap:8, alignItems:"center", pointerEvents:"none", width:"min(340px,92vw)" }}>
        {toasts.map(({ id, type, msg }) => {
          const c = COLORS[type];
          return (
            <div key={id} style={{ background:c.bg, border:`1px solid ${c.border}44`,
              borderLeft:`3px solid ${c.border}`, borderRadius:14, padding:"12px 16px",
              display:"flex", alignItems:"center", gap:10, boxShadow:"0 8px 32px rgba(0,0,0,0.4)",
              animation:"fadeUp .2s ease", pointerEvents:"auto", width:"100%" }}>
      ...
      {confirm && (
        <div role="presentation"
          ...
          <div role="dialog" aria-modal="true" aria-labelledby="confirm-title" aria-describedby="confirm-desc"
            style={{ background:"#fff", borderRadius:22, padding:"28px 24px", width:"100%",
            ...
            <div style={{ display:"flex", gap:10 }}>
              <button autoFocus onClick={() => { confirm.resolve(false); setConfirm(null); }}
                style={{ flex:1, padding:"12px", background:"transparent", border:"1px solid #E2E8F0",
                  borderRadius:12, fontWeight:600, fontSize:14, cursor:"pointer", color:T.muted, fontFamily:"inherit" }}>
                {confirm.opts?.cancel||"Cancelar"}
              </button>
              <button onClick={() => { confirm.resolve(true); setConfirm(null); }}
                style={{ flex:1, padding:"12px", background:confirm.opts?.danger?T.red:"#0F172A",
