...
function CRM({clientes,setClientes,contratos,loading,onModalChange}){
  ...
  return(<div>
    ...
    <div style={{display:"flex",alignItems:"flex-start",justifyContent:"space-between",marginBottom:26,animation:"fadeUp 0.5s ease both"}}>
      <div>
        <h1 style={{fontSize:26,fontWeight:900,color:C.text,letterSpacing:"-0.8px",marginBottom:4}}>Clientes & Prospectos</h1>
      ...
      <button className="crm-btn" onClick={openNew} style={{
        ...
        background:"#2563EB",
        border:"none",borderRadius:12,padding:"10px 18px",cursor:"pointer",touchAction:"manipulation",
    ...
    <div style={{background:"#0E1228",border:`1px solid rgba(255,255,255,0.07)`,borderRadius:16,marginBottom:18,overflow:"hidden",animation:"fadeUp 0.5s ease both",animationDelay:"0.26s",boxShadow:"0 8px 40px rgba(0,0,0,0.4),inset 0 1px 0 rgba(255,255,255,0.04)"}}>
      ...
      {paginated.length===0
        ?<div style={{padding:48,textAlign:"center",color:C.muted}}>
          ...
          <button onClick={openNew} style={{color:C.accent,background:`${C.accent}15`,border:`1px solid ${C.accent}33`,borderRadius:8,padding:"8px 16px",cursor:"pointer",touchAction:"manipulation",fontWeight:700,fontSize:13,fontFamily:"inherit"}}>+ Agregar contacto</button>
        </div>
        :paginated.map((r,i)=>{
            ...
            <div key={r.id} className="crm-row-hover"
              ...
              <div style={{display:"flex",alignItems:"center",gap:8}}>
                ...
                <div style={{display:"flex",gap:6,flexShrink:0}}>
                  <button onClick={()=>wa(r)} title="WhatsApp" style={{background:"#25D36618",border:"1px solid #25D36630",borderRadius:10,padding:"8px 10px",color:"#25D366",cursor:"pointer",touchAction:"manipulation",display:"flex",alignItems:"center",justifyContent:"center",minWidth:36,minHeight:36}}>
                  ...
                  <button onClick={()=>openEdit(r)} title="Editar" style={{background:`${C.accent}18`,border:`1px solid ${C.accent}30`,borderRadius:10,padding:"8px 10px",color:C.accent,cursor:"pointer",touchAction:"manipulation",display:"flex",alignItems:"center",justifyContent:"center",minWidth:36,minHeight:36}}>
                    <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/></svg>
                  </button>
                  <button onClick={()=>eliminar(r.id)} title="Eliminar" style={{background:`${C.red}18`,border:`1px solid ${C.red}30`,borderRadius:10,padding:"8px 10px",color:C.red,cursor:"pointer",touchAction:"manipulation",display:"flex",alignItems:"center",justifyContent:"center",minWidth:36,minHeight:36}}>
          ...
          {Array.from({length:Math.min(totalPages,5)},(_,i)=>{
            ...
            return(
              <button key={i} onClick={()=>typeof n==="number"&&setPage(n)}
                style={{width:28,height:28,borderRadius:8,border:"none",cursor:"pointer",touchAction:"manipulation",
                  ...
                  boxShadow:page===n?"0 2px 12px rgba(37,99,235,0.4)":"none",transition:"background .08s"}}>
                {n}
              </button>
            );
          })}
          {totalPages>1&&<button onClick={()=>setPage(p=>Math.min(p+1,totalPages))}
            style={{width:28,height:28,borderRadius:8,border:"none",cursor:"pointer",touchAction:"manipulation",background:"rgba(255,255,255,0.05)",color:C.muted,display:"flex",alignItems:"center",justifyContent:"center"}}>
        ...
        <div style={{display:"flex",gap:10,marginBottom:18}}>
          {["Cliente","Prospecto"].map(t=>(
            <button key={t} onClick={()=>{setForm(f=>({...f,tipo:t,estado:t==="Cliente"?"Activo":"En contacto"}));}}
              style={{flex:1,padding:10,borderRadius:10,border:`2px solid ${form.tipo===t?tCol(t):C.border}`,background:form.tipo===t?tCol(t)+"22":"transparent",color:form.tipo===t?tCol(t):C.muted,fontWeight:700,cursor:"pointer",touchAction:"manipulation",fontFamily:"inherit"}}>
      ...
      {[
        ...
        <button key={f.id} onClick={()=>setFiltro(f.id)}
          style={{padding:"8px 14px",borderRadius:9,border:"none",
            ...
            fontWeight:700,fontSize:12,cursor:"pointer",touchAction:"manipulation",transition:"background .08s",
          }}>
          {f.label}
        </button>
      ))}
                ...
                <div style={{display:"flex",alignItems:"flex-start",justifyContent:"space-between",gap:12,marginBottom:10}}>
                  ...
                  <div style={{display:"flex",flexDirection:"column",alignItems:"flex-end",gap:8,flexShrink:0}}>
                    ...
                    <button onClick={()=>openEdit(c)}
                      style={{display:"inline-flex",alignItems:"center",gap:6,padding:"8px 14px",borderRadius:10,border:`1px solid ${C.border}`,background:C.surface,color:C.white,fontWeight:700,fontSize:12,cursor:"pointer",touchAction:"manipulation"}}>
...
function Capital({ paneles, contratos, gastos, proveedores }) {
  ...
  // ── Depreciación lineal de paneles ──
  const hoyMs = Date.now();
  const panelDepreciacion = paneles.map(p => {
    ...
    if (!costo || !inicio) return { ...p, valorActual: costo, depAcum: 0, depAnual: 0, pctDesgaste: 0 };
    const aniosUsados = Math.max(0, (hoyMs - inicio) / (365.25 * 24 * 3600 * 1000));
    const depAnual    = costo / vida;
    const depAcum     = Math.min(costo, depAnual * aniosUsados);
    const valorActual = Math.max(0, costo - depAcum);
    const pctDesgaste = Math.min(100, (depAcum / costo) * 100);
    return { ...p, valorActual, depAcum, depAnual, pctDesgaste, aniosUsados };
  });

  // ── Activos ──
  const valorPaneles = panelDepreciacion.reduce((s, p) => s + (p.valorActual || 0), 0);
  ...
  const flujoPorMes = meses6.map(mes => {
    const ingresos = contratos
      .filter(c => c.pagado && c.inicio && c.inicio.slice(0, 7) <= mes && (!c.fin || c.fin.slice(0, 7) >= mes))
      .reduce((s, c) => s + Number(c.monto || 0), 0);
    const egresos = gastos
      .filter(g => g.fecha && g.fecha.slice(0, 7) === mes)
      .reduce((s, g) => s + Number(g.monto || 0), 0);
    const neto = ingresos - egresos;
    return { mes, ingresos, egresos, neto };
  });

  const maxValor = Math.max(...flujoPorMes.flatMap(m => [m.ingresos, m.egresos]), 1);

  const mesLabel2 = (m) => {
    const meses = ["Ene","Feb","Mar","Abr","May","Jun","Jul","Ago","Sep","Oct","Nov","Dic"];
    const [, mm] = m.split("-");
    return meses[parseInt(mm, 10) - 1];
  };

  // ── Depreciación acumulada del portfolio ──
  const costoTotalOriginal = panelDepreciacion.reduce((s, p) => s + (Number(p.costoInstalacion) || 0), 0);
  const depAcumTotal       = panelDepreciacion.reduce((s, p) => s + (p.depAcum || 0), 0);
  const pctPortfolio       = costoTotalOriginal > 0 ? (depAcumTotal / costoTotalOriginal) * 100 : 0;
  ...
  return (
    <div>
      <PgTit icon="💰" title="Capital" sub="Patrimonio, depreciación y flujo de caja"/>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 20 }}>
        <div style={{
          ...
          <div style={{ fontSize: 12, fontWeight: 700, color: "rgba(255,255,255,0.5)", textTransform: "uppercase", letterSpacing: 1.2, marginBottom: 6 }}>Patrimonio Neto</div>
          <div style={{ fontSize: 32, fontWeight: 900, color: patrimonioNeto >= 0 ? "#4ADE80" : "#F87171", letterSpacing: "-1px" }}>{fmt(patrimonioNeto)}</div>
          <div style={{ fontSize: 12, color: "rgba(255,255,255,0.4)", marginTop: 6 }}>Activos {fmt(totalActivos)} — Pasivos {fmt(porPagar)}</div>
        </div>
        <KPI icon="🏗️" label="Valor paneles" value={fmt(valorPaneles)} sub={`Costo original ${fmt(costoTotalOriginal)}`} color={C.accent}/>
        <KPI icon="📥" label="Por cobrar" value={fmt(porCobrar)} sub={`${contratos.filter(c => !c.pagado).length} contratos`} color={C.amber}/>
        <KPI icon="📤" label="Por pagar" value={fmt(porPagar)} sub={`${proveedores.filter(p => !p.pagado).length} proveedores`} color={C.red}/>
        <KPI icon="📉" label="Dep. acumulada" value={fmt(depAcumTotal)} sub={`${pctPortfolio.toFixed(0)}% del portfolio`} color={C.purple}/>
      </div>

      {/* ── Flujo de caja ── */}
      <div style={{ background: C.card, borderRadius: 18, padding: "18px 16px", marginBottom: 20, border: `1px solid ${C.border}`, boxShadow: "0 2px 8px rgba(0,0,0,0.04)" }}>
        <div style={{ fontSize: 13, fontWeight: 700, color: C.text, marginBottom: 16 }}>📊 Flujo de Caja — últimos 6 meses</div>
        <div style={{ display: "flex", gap: 8, alignItems: "flex-end", height: 140, marginBottom: 8 }}>
          {flujoPorMes.map(({ mes, ingresos, egresos }) => (
            <div key={mes} style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", gap: 3, height: "100%" }}>
              <div style={{ flex: 1, width: "100%", display: "flex", flexDirection: "column", justifyContent: "flex-end", gap: 2 }}>
                <div title={`Ingresos: ${fmt(ingresos)}`} style={{ width: "100%", height: `${(ingresos / maxValor) * 100}%`, minHeight: ingresos > 0 ? 4 : 0, background: C.green, borderRadius: "4px 4px 0 0", transition: "height .3s" }}/>
                <div title={`Egresos: ${fmt(egresos)}`} style={{ width: "100%", height: `${(egresos / maxValor) * 100}%`, minHeight: egresos > 0 ? 4 : 0, background: C.red + "CC", borderRadius: "4px 4px 0 0", transition: "height .3s" }}/>
              </div>
            </div>
          ))}
        </div>
        <div style={{ display: "flex", gap: 8 }}>
          {flujoPorMes.map(({ mes }) => (
            <div key={mes} style={{ flex: 1, textAlign: "center", fontSize: 10, color: C.muted, fontWeight: 600 }}>{mesLabel2(mes)}</div>
          ))}
        </div>
        <div style={{ display: "flex", gap: 16, marginTop: 12, justifyContent: "center" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 5, fontSize: 11, color: C.muted }}>
            <div style={{ width: 10, height: 10, borderRadius: 2, background: C.green }}/> Ingresos cobrados
          </div>
          <div style={{ display: "flex", alignItems: "center", gap: 5, fontSize: 11, color: C.muted }}>
            <div style={{ width: 10, height: 10, borderRadius: 2, background: C.red + "CC" }}/> Gastos
          </div>
        </div>
      </div>
      ...
      <div style={{ background: C.card, borderRadius: 18, padding: "18px 16px", marginBottom: 20, border: `1px solid ${C.border}`, boxShadow: "0 2px 8px rgba(0,0,0,0.04)" }}>
        <div style={{ fontSize: 13, fontWeight: 700, color: C.text, marginBottom: 14 }}>🏗️ Depreciación por panel</div>
        {panelDepreciacion.filter(p => Number(p.costoInstalacion) > 0).length === 0 ? (
          <div style={{ textAlign: "center", padding: "24px 0", color: C.muted, fontSize: 13 }}>
            <div style={{ fontSize: 28, marginBottom: 8 }}>📋</div>
            Agrega el <strong>costo de instalación</strong> y <strong>fecha de instalación</strong> en cada panel para ver la depreciación.
          </div>
        ) : (
          panelDepreciacion
            .filter(p => Number(p.costoInstalacion) > 0)
            .sort((a, b) => b.pctDesgaste - a.pctDesgaste)
            .map(p => (
              <div key={p.id} style={{ marginBottom: 14, paddingBottom: 14, borderBottom: `1px solid ${C.border}` }}>
                <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 8 }}>
                  <span style={{ fontSize: 22 }}>{p.foto || "📡"}</span>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ fontSize: 13, fontWeight: 700, color: C.text, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{p.nombre}</div>
                    <div style={{ fontSize: 11, color: C.muted }}>{p.ciudad}</div>
                  </div>
                  <div style={{ textAlign: "right", flexShrink: 0 }}>
                    <div style={{ fontSize: 14, fontWeight: 800, color: C.accent }}>{fmt(p.valorActual)}</div>
                    <div style={{ fontSize: 10, color: C.muted }}>valor actual</div>
                  </div>
                </div>
                {/* Barra de desgaste */}
                <div style={{ background: C.bg, borderRadius: 6, height: 8, overflow: "hidden", marginBottom: 6 }}>
                  <div style={{
                    ...
                    width: `${p.pctDesgaste}%`,
                    background: p.pctDesgaste > 75 ? C.red : p.pctDesgaste > 50 ? C.amber : C.green,
                  }}/>
                </div>
                <div style={{ display: "flex", justifyContent: "space-between", fontSize: 11, color: C.muted }}>
                  <span>Costo original: {fmt(Number(p.costoInstalacion))}</span>
                  <span>Depreciado {p.pctDesgaste.toFixed(0)}% · {(p.aniosUsados || 0).toFixed(1)} años</span>
                </div>
              </div>
            ))
        )}
      ...
      {/* ── Resumen flujo neto ── */}
      <div style={{ background: C.card, borderRadius: 18, padding: "18px 16px", border: `1px solid ${C.border}`, boxShadow: "0 2px 8px rgba(0,0,0,0.04)", marginBottom: 20 }}>
        <div style={{ fontSize: 13, fontWeight: 700, color: C.text, marginBottom: 12 }}>📅 Detalle mensual</div>
        {flujoPorMes.slice().reverse().map(({ mes, ingresos, egresos, neto }) => (
          <div key={mes} style={{ display: "flex", alignItems: "center", gap: 12, padding: "10px 0", borderBottom: `1px solid ${C.bg}` }}>
            <div style={{ width: 42, height: 42, borderRadius: 12, background: neto >= 0 ? C.green + "18" : C.red + "18", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
              <span style={{ fontSize: 18 }}>{neto >= 0 ? "📈" : "📉"}</span>
            </div>
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ fontSize: 13, fontWeight: 700, color: C.text }}>{mesLabel(mes)}</div>
              <div style={{ fontSize: 11, color: C.muted }}>Ing. {fmt(ingresos)} · Gas. {fmt(egresos)}</div>
            </div>
            <div style={{ textAlign: "right", flexShrink: 0 }}>
              <div style={{ fontSize: 14, fontWeight: 800, color: neto >= 0 ? C.green : C.red }}>{neto >= 0 ? "+" : ""}{fmt(neto)}</div>
              <div style={{ fontSize: 10, color: C.muted }}>neto</div>
            </div>
          </div>
        ))}
      </div>
