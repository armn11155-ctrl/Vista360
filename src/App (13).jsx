...
function RankingClientes({clientes,contratos}){
  ...
  const maxIngreso = Math.max(1, ...stats.map(s=>s.ingreso));
  const tabs=[
    {id:"ingresos",label:"Por Ingresos",icon:(<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M12 2v20"/><path d="M17 5H9.5a3.5 3.5 0 0 0 0 7h5a3.5 3.5 0 0 1 0 7H6"/></svg>)},
    {id:"estables",label:"Más Estables",icon:(<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M3 12a9 9 0 0 1 15-6.7L21 8"/><path d="M21 3v5h-5"/><path d="M21 12a9 9 0 0 1-15 6.7L3 16"/><path d="M3 21v-5h5"/></svg>)},
    {id:"riesgo",label:"En Riesgo",icon:(<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M10.3 3.86 1.82 18a2 2 0 0 0 1.71 3h16.94a2 2 0 0 0 1.71-3L13.71 3.86a2 2 0 0 0-3.42 0z"/><path d="M12 9v4"/><path d="M12 17h.01"/></svg>)},
  ];
  const RANK_TONE = [
    {ring:"linear-gradient(135deg,#FFE7A3,#F5B642)",chip:"#F5B642",label:"#3B2A07"},   // gold
    {ring:"linear-gradient(135deg,#E5E7EB,#9CA3AF)",chip:"#C4CAD4",label:"#1F2937"},   // silver
    {ring:"linear-gradient(135deg,#E2A37A,#A05A2C)",chip:"#C57645",label:"#3B1F0A"},   // bronze
  ];
  ...
  return(<div>
    ...
    <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:14,padding:"0 2px"}}>
      <div>
        <div style={{fontSize:10,fontWeight:700,color:C.muted,letterSpacing:1.4,textTransform:"uppercase"}}>Clientes</div>
        <div style={{fontSize:16,fontWeight:800,color:C.white,letterSpacing:"-0.01em",marginTop:2}}>Ranking</div>
      </div>
      <div style={{fontSize:11,color:C.muted}}>{stats.length} {stats.length===1?"cliente":"clientes"}</div>
    </div>

    {/* Segmented selector — premium */}
    <div style={{
      ...
      {tabs.map(v=>{
        ...
        return (
          <button key={v.id} onClick={()=>setVista(v.id)} style={{
            ...
            background:active?"linear-gradient(180deg,rgba(124,58,237,0.22),rgba(124,58,237,0.08))":"transparent",
            boxShadow:active?"inset 0 0 0 1px rgba(168,135,255,0.35), 0 4px 14px -8px rgba(124,58,237,0.5)":"none",
            color:active?C.white:C.muted,
            fontWeight:active?700:600,fontSize:11.5,letterSpacing:0.2,
            transition:"all .18s",
          }}>
            <span style={{display:"inline-flex",opacity:active?1:0.7}}>{v.icon}</span>
            {v.label}
          </button>
        );
      })}
    ...
    {ranked.length===0?(
      <div style={{textAlign:"center",padding:"36px 20px",color:C.muted,fontSize:13,
        background:"rgba(255,255,255,0.015)",border:`1px dashed ${C.border}`,borderRadius:14}}>
        {vista==="riesgo"?"Sin clientes en riesgo. Todo en orden.":"Sin clientes registrados"}
      </div>
    ):(
      <div style={{display:"flex",flexDirection:"column",gap:8}}>
        {ranked.map((cl,i)=>{
          const ini=cl.empresa.split(" ").slice(0,2).map(w=>w[0]).join("").toUpperCase();
          const isPodium = vista!=="riesgo" && i<3;
          const tone = isPodium ? RANK_TONE[i] : null;
          const pct = Math.max(2, Math.round((cl.ingreso/maxIngreso)*100));
          const riskCol = riskColor[cl.riesgo];
          return(
            <div key={cl.id} style={{
              position:"relative",
              display:"grid",gridTemplateColumns:"32px 44px 1fr auto",alignItems:"center",gap:12,
              padding:"12px 14px",
              background:isPodium?"linear-gradient(180deg,rgba(255,255,255,0.035),rgba(255,255,255,0.012))":"rgba(255,255,255,0.02)",
              border:`1px solid ${isPodium?"rgba(255,255,255,0.08)":C.border}`,
              borderRadius:14,
              transition:"all .18s ease",
              overflow:"hidden",
            }}
              onMouseEnter={e=>{e.currentTarget.style.borderColor="rgba(168,135,255,0.35)";e.currentTarget.style.transform="translateY(-1px)";}}
              onMouseLeave={e=>{e.currentTarget.style.borderColor=isPodium?"rgba(255,255,255,0.08)":C.border;e.currentTarget.style.transform="translateY(0)";}}
            >
              {/* Rank number */}
              <div style={{
                width:28,height:28,borderRadius:8,display:"flex",alignItems:"center",justifyContent:"center",
                background:isPodium?tone.ring:"rgba(255,255,255,0.04)",
                color:isPodium?tone.label:C.muted,
                fontSize:13,fontWeight:800,letterSpacing:"-0.02em",
                boxShadow:isPodium?`0 0 0 1px rgba(255,255,255,0.18) inset`:"none",
              }}>{String(i+1).padStart(2,"0")}</div>

              {/* Avatar */}
              <div style={{
                width:40,height:40,borderRadius:"50%",
                background:isPodium?tone.ring:"linear-gradient(145deg,#3a2c66,#1f1842)",
                color:isPodium?tone.label:"#E9E4FF",
                display:"flex",alignItems:"center",justifyContent:"center",
                fontWeight:800,fontSize:13,letterSpacing:0.4,
                boxShadow:"inset 0 1px 0 rgba(255,255,255,0.25), 0 6px 14px -8px rgba(0,0,0,0.6)",
              }}>{ini}</div>

              {/* Body */}
              <div style={{minWidth:0}}>
                <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:6,flexWrap:"wrap"}}>
                  <span style={{fontSize:13.5,fontWeight:700,color:C.white,letterSpacing:"-0.01em",
                    overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",maxWidth:180}}>{cl.empresa}</span>
                  {cl.sector&&<span style={{fontSize:10,color:C.muted,fontWeight:600,letterSpacing:0.3,textTransform:"uppercase"}}>· {cl.sector}</span>}
                </div>

                {/* Inline metrics row */}
                <div style={{display:"flex",alignItems:"center",gap:14,fontSize:11.5,color:C.muted,marginBottom:8}}>
                  <span><span style={{color:C.white,fontWeight:700,letterSpacing:"-0.01em"}}>{fmt(cl.ingreso)}</span> ingresos</span>
                  <span style={{width:3,height:3,borderRadius:"50%",background:C.border}}/>
                  <span><span style={{color:C.white,fontWeight:700}}>{cl.totalCtrs}</span> contratos</span>
                  <span style={{width:3,height:3,borderRadius:"50%",background:C.border}}/>
                  <span><span style={{color:C.white,fontWeight:700}}>{cl.renovaciones}</span> renov.</span>
                  <span style={{width:3,height:3,borderRadius:"50%",background:C.border}}/>
                  <span style={{color:cl.tasaPago>=80?C.green:cl.tasaPago>=50?C.amber:C.red,fontWeight:700}}>{cl.tasaPago}% pago</span>
                </div>

                {/* Progress — share of revenue vs top */}
              ...
              <div style={{
                ...
                color:riskCol,fontSize:10.5,fontWeight:700,letterSpacing:0.4,textTransform:"uppercase",
                whiteSpace:"nowrap",
              }}>
                <span style={{width:6,height:6,borderRadius:"50%",background:riskCol,boxShadow:`0 0 8px ${riskCol}`}}/>
                {cl.riesgo}
              </div>
            </div>
          );
          ...
          {BOTTOM_TABS_LIST.map(t => {
            if (t.id === "__add__") return (
              <div key="add" style={{ flex: 1, display: "flex", justifyContent: "center" }}>
                <button style={{ width: 58, height: 58, borderRadius: "50%", background: NL.accent, border: `4px solid ${NL.bg}`, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", boxShadow: "0 10px 26px rgba(37,99,235,0.45)", marginTop: -32 }}>
