      ...
      {[
        ...
        return (
          <button key={f.label} onClick={()=>setFiltro(f.label)} style={{
            ...
            fontWeight:isActive?700:600,fontSize:14,cursor:"pointer",touchAction:"manipulation",
          }}>
            <span style={{
    ...
    <div style={{display:"flex",flexDirection:"column",gap:18}}>
          ...
          <div key={c.id} style={{
            ...
            <div style={{background:"#fff",borderRadius:22,padding:"18px 16px 14px",position:"relative",zIndex:1}}>
              ...
              <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:14,flexWrap:"wrap"}}>
                ...
                <div style={{flex:1}}/>
                {/* Action buttons */}
                <button onClick={()=>openEdit(c)} style={{width:42,height:42,borderRadius:11,border:"1px solid #E5E7EB",background:"#fff",cursor:"pointer",touchAction:"manipulation",display:"inline-flex",alignItems:"center",justifyContent:"center",color:"#6B7280"}}>
                ...
                <button onClick={()=>eliminar(c.id)} style={{width:42,height:42,borderRadius:11,border:"1px solid #FECACA",background:"#FEF2F2",cursor:"pointer",touchAction:"manipulation",display:"inline-flex",alignItems:"center",justifyContent:"center",color:"#EF4444"}}>
                ...
                <a href={`https://wa.me/${c.cliente.celular?.replace(/\D/g,"")}?text=${msg}`} target="_blank" rel="noopener noreferrer"
                  style={{width:42,height:42,borderRadius:11,border:"1px solid #BBF7D0",background:"#F0FDF4",cursor:"pointer",touchAction:"manipulation",display:"inline-flex",alignItems:"center",justifyContent:"center",color:"#10B981",textDecoration:"none"}}>
      ...
      {fil.length===0&&(
        <div style={{textAlign:"center",padding:48,background:"#fff",borderRadius:22,border:"1px dashed #E5E7EB"}}>
          ...
          <button onClick={openNew} style={{marginTop:10,background:"#2563EB",border:"none",borderRadius:12,padding:"11px 20px",color:"#fff",fontWeight:700,fontSize:14,cursor:"pointer",touchAction:"manipulation"}}>+ Crear primer contrato</button>
        </div>
      )}
    </div>}
  </div>);
...
function CRM({clientes,setClientes,contratos,loading,onModalChange}){
  ...
  return(<div>
    <style>{`
      ...
      .crm-row-hover:active{background:rgba(37,99,235,0.06)!important;}
      .crm-chip-hover:hover{background:rgba(255,255,255,0.06)!important;}
    `}</style>

    {/* ── HERO DARK ── */}
    <div style={{
      position:"relative", margin:"-20px -16px 22px",
      ...
      <div aria-hidden style={{position:"absolute",inset:0,pointerEvents:"none",overflow:"hidden",borderBottomLeftRadius:28,borderBottomRightRadius:28}}>
        <div style={{position:"absolute",bottom:-120,right:-80,width:320,height:320,borderRadius:"50%",background:"radial-gradient(closest-side,rgba(60,130,255,0.25),rgba(60,130,255,0))",filter:"blur(6px)"}}/>
        <div style={{position:"absolute",bottom:-80,left:-100,width:260,height:260,borderRadius:"50%",background:"radial-gradient(closest-side,rgba(80,140,255,0.14),rgba(0,0,0,0))"}}/>
      </div>
      <div style={{position:"relative",display:"flex",alignItems:"flex-start",justifyContent:"space-between",gap:16}}>
        <div>
          <div style={{fontSize:12,color:"rgba(200,212,240,0.55)",fontWeight:500,marginBottom:4}}>Vista360 · 8 Millas</div>
          <div style={{fontSize:28,fontWeight:800,color:"#fff",letterSpacing:"-0.03em",lineHeight:1.05}}>Clientes</div>
          <div style={{fontSize:11.5,color:"rgba(200,212,240,0.5)",marginTop:5}}>{clientes.length} contactos · {clis.filter(c=>c.estado==="Activo").length} activos</div>
        </div>
        <button onClick={openNew} style={{
          display:"flex",alignItems:"center",gap:7,
          background:"#2563EB",border:"none",borderRadius:50,padding:"10px 18px",
          color:"#fff",fontWeight:700,fontSize:13,cursor:"pointer",touchAction:"manipulation",fontFamily:"inherit",
          boxShadow:"0 8px 24px rgba(37,99,235,0.42),inset 0 1px 0 rgba(255,255,255,0.18)",
          flexShrink:0,
        }}>
      ...
      {/* Mini sparkline + totales */}
      <div style={{
        position:"relative",marginTop:20,
        background:"linear-gradient(180deg,rgba(20,32,68,0.92),rgba(14,24,56,0.92))",
        ...
        <div style={{flex:"1 1 55%",minWidth:0,display:"flex",flexDirection:"column",gap:14}}>
          <div style={{display:"flex",gap:10}}>
            {[
              {label:"Clientes",  val:clis.length,  color:"#5BD39A"},
              {label:"Prospectos",val:pros.length,   color:"#7FAEFF"},
              {label:"Propuestas",val:propuestas.length,color:"rgba(200,212,240,0.6)"},
            ].map(({label,val,color})=>(
              <div key={label} style={{flex:1,minWidth:0}}>
                <div style={{fontSize:20,fontWeight:900,color,letterSpacing:"-0.03em",lineHeight:1}}>{val}</div>
                <div style={{fontSize:10,color:"rgba(200,212,240,0.5)",marginTop:3,fontWeight:500}}>{label}</div>
              </div>
            ))}
          </div>
        </div>
        <div style={{flex:"1 1 45%",minWidth:0,display:"flex",flexDirection:"column"}}>
          <div style={{display:"flex",justifyContent:"flex-end",marginBottom:8}}>
            <div style={{display:"inline-flex",alignItems:"center",gap:5,background:"rgba(29,107,255,0.18)",border:"1px solid rgba(127,174,255,0.35)",color:"#BDD2FF",borderRadius:999,padding:"4px 10px",fontSize:11,fontWeight:600}}>
              12 meses
            </div>
          </div>
          <div style={{flex:1,display:"flex",alignItems:"flex-end",justifyContent:"space-between",gap:4,height:60}}>
            {SPARK_CLI.slice(-8).map((v,i)=>{
              const max=Math.max(...SPARK_CLI.slice(-8),1);
              const h=(v/max)*100;
              const isHi=i>=5;
              return(<div key={i} style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",gap:3,minWidth:0}}>
                <div style={{width:"70%",height:`${Math.max(h,6)}%`,minHeight:4,borderRadius:4,background:isHi?"linear-gradient(180deg,#4A8CFF,#1D6BFF)":"rgba(255,255,255,0.08)",boxShadow:isHi?"0 0 12px rgba(29,107,255,0.45)":"none"}}/>
              </div>);
            })}
          </div>
        </div>
    ...
    {/* ── 4 METRIC CARDS (white + wave) ── */}
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:20}}>
      {[
        {icon:ICN.users,  label:"Clientes",      value:clis.length,       sub:`${clis.filter(c=>c.estado==="Activo").length} activos`,         wc:NL.green,  sp:SPARK_CLI},
        {icon:ICN.target, label:"Prospectos",     value:pros.length,       sub:"En pipeline",                                                   wc:NL.accent, sp:SPARK_PRO},
        {icon:ICN.bolt,   label:"Propuestas",     value:propuestas.length, sub:"Abiertas este ciclo",                                           wc:NL.accent, sp:SPARK_PROP},
        {icon:ICN.check,  label:"Total contactos",value:clientes.length,   sub:`${clientes.filter(c=>c.estado==="Activo").length} activos`,     wc:NL.green,  sp:calcSparkline(clientes)},
      ].map(({icon,label,value,sub,wc,sp})=>(
        <div key={label} style={{background:NL.white,borderRadius:18,padding:"14px 14px 10px",border:`1px solid ${NL.border}`,boxShadow:"0 1px 0 rgba(16,22,40,0.02),0 6px 24px -16px rgba(16,22,40,0.18)",display:"flex",flexDirection:"column",minWidth:0}}>
          <div style={{width:38,height:38,borderRadius:11,background:"linear-gradient(180deg,#0E1A3B,#15265A)",display:"flex",alignItems:"center",justifyContent:"center",marginBottom:10,flexShrink:0}}>{icon}</div>
          <div style={{fontSize:11,color:NL.text,fontWeight:600,marginBottom:3}}>{label}</div>
          <div style={{fontSize:24,fontWeight:800,color:NL.text,lineHeight:1,letterSpacing:"-0.03em"}}>{value}</div>
          <div style={{fontSize:11,color:NL.muted,marginTop:5,lineHeight:1.35,minHeight:26}}>{sub}</div>
          <div style={{marginTop:4,marginLeft:-2,marginRight:-2}}><Wave color={wc} data={sp}/></div>
        </div>
      ))}
    </div>

    {/* ── SEARCH ── */}
    <div style={{background:NL.white,borderRadius:16,padding:"12px 14px",marginBottom:12,border:`1px solid ${NL.border}`,boxShadow:"0 1px 0 rgba(16,22,40,0.02),0 4px 12px -8px rgba(16,22,40,0.1)",display:"flex",alignItems:"center",gap:10}}>
      <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke={NL.muted} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" style={{flexShrink:0}}>
        <circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/>
      </svg>
      <input value={buscar} onChange={e=>{setBuscar(e.target.value);setPage(1);}}
        placeholder="Buscar empresa, contacto, ciudad..."
        style={{flex:1,border:"none",outline:"none",fontSize:14,color:NL.text,background:"transparent",fontFamily:"inherit"}}/>
      {buscar && (
        <button onClick={()=>{setBuscar("");setPage(1);}} style={{background:"none",border:"none",cursor:"pointer",touchAction:"manipulation",color:NL.muted,display:"flex",alignItems:"center",padding:0}}>
    ...
    {/* ── FILTER CHIPS ── */}
    <div style={{display:"flex",gap:7,marginBottom:18,alignItems:"center",overflowX:"auto",paddingBottom:2}}>
      {filters.map(f=>{
        const active=activeFilter===f;
        const dot=f==="Activos"?NL.green:f==="En riesgo"?NL.red:null;
        return(
          <button key={f} onClick={()=>{setActiveFilter(f);setPage(1);}} className="crm-chip-hover"
            style={{display:"flex",alignItems:"center",gap:5,padding:"7px 15px",borderRadius:99,flexShrink:0,
              border:`1px solid ${active?"#2563EB":NL.border}`,
              background:active?"linear-gradient(135deg,#2563EB,#1D4ED8)":"transparent",
              color:active?"#fff":NL.muted,fontWeight:active?700:500,
              fontSize:12,cursor:"pointer",touchAction:"manipulation",fontFamily:"inherit",
      ...
      <span style={{marginLeft:"auto",fontSize:11,color:NL.muted,flexShrink:0,fontWeight:600}}>{filtrado.length} resultado{filtrado.length!==1?"s":""}</span>
    </div>

    {/* ── LISTA DE CLIENTES ── */}
    {loading ? <Spinner/> :
    <div style={{background:"linear-gradient(160deg,#0F1B36 0%,#0A1430 55%,#070E22 100%)",border:"1px solid rgba(255,255,255,0.06)",borderRadius:22,overflow:"hidden",boxShadow:"0 14px 40px -18px rgba(8,12,30,0.55)",marginBottom:18}}>

      {/* Header de tabla */}
      <div style={{padding:"14px 20px 12px",borderBottom:"1px solid rgba(255,255,255,0.06)",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
        <span style={{fontSize:12,fontWeight:700,color:"rgba(200,212,240,0.55)",textTransform:"uppercase",letterSpacing:1.2}}>Contactos ({filtrado.length})</span>
        <span style={{fontSize:11,color:"rgba(200,212,240,0.35)"}}>Pág. {page}/{totalPages}</span>
      </div>

      {paginated.length===0
        ? <div style={{padding:"48px 20px",textAlign:"center"}}>
            <div style={{width:52,height:52,borderRadius:16,background:"rgba(255,255,255,0.05)",display:"flex",alignItems:"center",justifyContent:"center",margin:"0 auto 14px"}}>
              <svg width="26" height="26" viewBox="0 0 24 24" fill="none" stroke="rgba(255,255,255,0.3)" strokeWidth="1.6" strokeLinecap="round"><path d="M17 21v-2a4 4 0 00-4-4H5a4 4 0 00-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 00-3-3.87M16 3.13a4 4 0 010 7.75"/></svg>
            </div>
            <div style={{fontSize:14,color:"rgba(255,255,255,0.4)",marginBottom:12}}>Sin resultados</div>
            <button onClick={openNew} style={{background:"#2563EB",border:"none",borderRadius:50,padding:"9px 18px",color:"#fff",fontWeight:700,fontSize:13,cursor:"pointer",touchAction:"manipulation",fontFamily:"inherit"}}>+ Agregar contacto</button>
          </div>
        : paginated.map((r,i)=>{
          ...
          return(
            <div key={r.id} className="crm-row-hover"
              style={{padding:"16px 18px",borderBottom:i<paginated.length-1?"1px solid rgba(255,255,255,0.04)":"none",transition:"background 0.15s"}}>

              {/* Fila superior: avatar + nombre + badge tipo */}
              <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:10}}>
                <AvatarEmpresa name={r.empresa} size={44}/>
                <div style={{flex:1,minWidth:0}}>
                  <div style={{fontSize:15,fontWeight:700,color:"#fff",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",letterSpacing:"-0.01em"}}>{r.empresa}</div>
                  <div style={{fontSize:11,color:"rgba(200,212,240,0.45)",marginTop:2,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{r.ruc?"RUC "+r.ruc:r.email||"—"}</div>
                </div>
                <span style={{flexShrink:0,background:`${tc}18`,border:`1px solid ${tc}40`,color:tc,borderRadius:8,padding:"4px 10px",fontSize:10,fontWeight:700,whiteSpace:"nowrap"}}>
                  {r.tipo}
                </span>
              </div>

              {/* Chips: ciudad / sector / estado — SVG en vez de emojis */}
              <div style={{display:"flex",flexWrap:"wrap",gap:6,marginBottom:12}}>
                <span style={{display:"inline-flex",alignItems:"center",gap:5,background:"rgba(255,255,255,0.05)",color:"rgba(200,212,240,0.55)",borderRadius:7,padding:"4px 9px",fontSize:10,fontWeight:600,border:"1px solid rgba(255,255,255,0.07)"}}>
                  <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round"><path d="M21 10c0 7-9 13-9 13S3 17 3 10a9 9 0 0118 0z"/><circle cx="12" cy="10" r="3"/></svg>
                  {r.ciudad||"—"}
                </span>
                <span style={{display:"inline-flex",alignItems:"center",gap:5,background:"rgba(255,255,255,0.05)",color:"rgba(200,212,240,0.55)",borderRadius:7,padding:"4px 9px",fontSize:10,fontWeight:600,border:"1px solid rgba(255,255,255,0.07)"}}>
                  <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round"><rect x="2" y="7" width="20" height="14" rx="2"/><path d="M16 7V5a2 2 0 00-2-2h-4a2 2 0 00-2 2v2"/></svg>
                  {r.sector||"—"}
                </span>
                <span style={{display:"inline-flex",alignItems:"center",gap:5,background:`${ec}14`,border:`1px solid ${ec}35`,color:ec,borderRadius:7,padding:"4px 9px",fontSize:10,fontWeight:700,whiteSpace:"nowrap"}}>
                  <div style={{width:5,height:5,borderRadius:"50%",background:ec,flexShrink:0}}/>
                  {r.estado}
                </span>
              </div>

              {/* Contacto + botones acción */}
              <div style={{display:"flex",alignItems:"center",gap:8}}>
                <div style={{flex:1,display:"flex",alignItems:"center",gap:8,minWidth:0}}>
                  <AvatarContacto name={r.contacto||"?"} size={28}/>
                  <span style={{fontSize:12,color:"rgba(200,212,240,0.5)",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{r.contacto||"—"}</span>
                </div>
                <div style={{display:"flex",gap:6,flexShrink:0}}>
                  {/* WhatsApp */}
                  <button onClick={()=>wa(r)} style={{width:36,height:36,borderRadius:11,background:"rgba(37,211,102,0.12)",border:"1px solid rgba(37,211,102,0.25)",color:"#25D366",cursor:"pointer",touchAction:"manipulation",display:"flex",alignItems:"center",justifyContent:"center"}}>
                  ...
                  {/* Editar */}
                  <button onClick={()=>openEdit(r)} style={{width:36,height:36,borderRadius:11,background:"rgba(37,99,235,0.12)",border:"1px solid rgba(37,99,235,0.25)",color:"#60A5FA",cursor:"pointer",touchAction:"manipulation",display:"flex",alignItems:"center",justifyContent:"center"}}>
                    <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M11 4H4a2 2 0 00-2 2v14a2 2 0 002 2h14a2 2 0 002-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 013 3L12 15l-4 1 1-4z"/></svg>
                  </button>
                  {/* Eliminar */}
                  <button onClick={()=>eliminar(r.id)} style={{width:36,height:36,borderRadius:11,background:"rgba(239,68,68,0.1)",border:"1px solid rgba(239,68,68,0.22)",color:"#F87171",cursor:"pointer",touchAction:"manipulation",display:"flex",alignItems:"center",justifyContent:"center"}}>
      ...
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"12px 18px",borderTop:"1px solid rgba(255,255,255,0.06)"}}>
        <span style={{fontSize:11,color:"rgba(200,212,240,0.35)"}}>
          {Math.min((page-1)*perPage+1,filtrado.length)}–{Math.min(page*perPage,filtrado.length)} de {filtrado.length}
        </span>
        <div style={{display:"flex",gap:5}}>
          {page>1&&<button onClick={()=>setPage(p=>p-1)} style={{width:28,height:28,borderRadius:8,border:"none",cursor:"pointer",touchAction:"manipulation",background:"rgba(255,255,255,0.06)",color:"rgba(200,212,240,0.6)",display:"flex",alignItems:"center",justifyContent:"center"}}>
            <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round"><polyline points="15 18 9 12 15 6"/></svg>
          </button>}
          {Array.from({length:Math.min(totalPages,5)},(_,i)=>i+1).map(n=>(
            <button key={n} onClick={()=>setPage(n)} style={{width:28,height:28,borderRadius:8,border:"none",cursor:"pointer",touchAction:"manipulation",background:page===n?"#2563EB":"rgba(255,255,255,0.05)",color:page===n?"#fff":"rgba(200,212,240,0.5)",fontSize:12,fontWeight:page===n?700:400,fontFamily:"inherit",boxShadow:page===n?"0 2px 12px rgba(37,99,235,0.4)":"none"}}>
              {n}
            </button>
          ))}
          {page<totalPages&&<button onClick={()=>setPage(p=>Math.min(p+1,totalPages))} style={{width:28,height:28,borderRadius:8,border:"none",cursor:"pointer",touchAction:"manipulation",background:"rgba(255,255,255,0.06)",color:"rgba(200,212,240,0.6)",display:"flex",alignItems:"center",justifyContent:"center"}}>
    ...
    {/* ── CTA ACCIÓN ── */}
    <div style={{background:"#0D1020",borderRadius:20,padding:"18px 18px",display:"flex",alignItems:"center",gap:14,marginBottom:8}}>
      <div style={{width:46,height:46,borderRadius:14,background:"rgba(37,99,235,0.2)",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}>
        <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="#60A5FA" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><path d="M17 21v-2a4 4 0 00-4-4H5a4 4 0 00-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 00-3-3.87M16 3.13a4 4 0 010 7.75"/></svg>
      </div>
      <div style={{flex:1,minWidth:0}}>
        <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:3}}>
          <span style={{fontSize:14,fontWeight:700,color:"#fff"}}>Agregar prospectos</span>
          <span style={{background:"rgba(29,107,255,0.18)",color:"#7FAEFF",border:"1px solid rgba(127,174,255,0.35)",fontSize:10,fontWeight:700,letterSpacing:0.6,padding:"2px 7px",borderRadius:6}}>SUGERIDO</span>
        </div>
        <div style={{fontSize:12,color:"rgba(255,255,255,0.4)"}}>10 nuevos prospectos puede aumentar tus ingresos un 30%.</div>
      </div>
      <button onClick={openNew} style={{background:"#2563EB",border:"none",borderRadius:50,padding:"9px 16px",color:"#fff",fontSize:12,fontWeight:700,cursor:"pointer",touchAction:"manipulation",whiteSpace:"nowrap",flexShrink:0,fontFamily:"inherit",boxShadow:"0 4px 16px rgba(37,99,235,0.35)"}}>
        Agregar
      </button>
    </div>
  ...
  return(<div>
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-end",marginBottom:22}}>
      ...
      <div style={{display:"flex",gap:10,alignItems:"center",flexWrap:"wrap"}}>
        <div style={{display:"flex",gap:4}}>
          {["Mensual","Histórico"].map(v=>(
            <button key={v} onClick={()=>setVistaHistorica(v==="Histórico")}
              style={{padding:"8px 14px",borderRadius:9,border:`1px solid ${vistaHistorica===(v==="Histórico")?C.accent:C.border}`,background:vistaHistorica===(v==="Histórico")?C.accent+"22":"transparent",color:vistaHistorica===(v==="Histórico")?C.accent:C.muted,fontWeight:600,fontSize:13,cursor:"pointer",touchAction:"manipulation"}}>
              {v}
            </button>
          ))}
        </div>
        {!vistaHistorica&&<input type="month" value={mes} onChange={e=>setMes(e.target.value)} style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:10,padding:"8px 13px",color:C.text,fontSize:14,outline:"none",fontFamily:"inherit",cursor:"pointer",touchAction:"manipulation"}}/>}
        <button onClick={()=>{
        ...
        }} style={{background:"linear-gradient(135deg,#1E35C8 0%,#3854EE 100%)",border:"none",borderRadius:50,padding:"12px 22px",color:"#FFFFFF",fontWeight:700,fontSize:14,cursor:"pointer",touchAction:"manipulation",display:"flex",alignItems:"center",gap:8,boxShadow:"0 4px 22px rgba(30,53,200,0.45), inset 0 1px 0 rgba(255,255,255,0.22)",letterSpacing:"0.01em",minHeight:46,fontFamily:"inherit"}}>
        ...
        {!vistaHistorica&&<button onClick={openNew} style={{background:C.accent,border:"none",borderRadius:11,padding:"10px 18px",color:C.white,fontWeight:700,fontSize:13,cursor:"pointer",touchAction:"manipulation"}}>＋ Gasto</button>}
      </div>
    </div>
      ...
      <div>
        ...
        {gastosMes.map((g,i)=>(
          <div key={g.id} style={{padding:"12px 16px",borderBottom:i<gastosMes.length-1?`1px solid ${C.border}`:"none",display:"flex",alignItems:"center",gap:10}}>
            ...
            <div style={{display:"flex",gap:6,flexShrink:0}}>
              <button onClick={()=>openEdit(g)} style={{background:C.accent+"22",border:"none",borderRadius:9,padding:"8px 10px",color:C.accent,cursor:"pointer",touchAction:"manipulation",minWidth:36,minHeight:36}}>✏️</button>
              <button onClick={()=>eliminar(g.id)} style={{background:C.red+"22",border:"none",borderRadius:9,padding:"8px 10px",color:C.red,cursor:"pointer",touchAction:"manipulation",minWidth:36,minHeight:36}}>🗑</button>
            </div>
          </div>
        ))}
      </div>}
...
function Gastos({gastos,setGastos,autoScan,setAutoScan,onModalChange}){
  ...
  return(<div>
    ...
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:20}}>
      ...
      <button onClick={abrirNuevo} style={{background:"linear-gradient(135deg,#2563EB,#3B82F6)",border:"none",borderRadius:12,padding:"10px 20px",color:"#fff",fontWeight:700,fontSize:13,cursor:"pointer",touchAction:"manipulation",boxShadow:"0 4px 14px rgba(37,99,235,0.4)",whiteSpace:"nowrap"}}>+ Nuevo</button>
    </div>
  ...
  return(<div>
      ...
      <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.6)",zIndex:400,display:"flex",alignItems:"flex-end",justifyContent:"center"}}
        ...
        <div style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:"22px 22px 0 0",padding:"10px 20px 24px",width:"100%",maxHeight:"92vh",overflowY:"auto",overscrollBehavior:"none",paddingBottom:"calc(24px + env(safe-area-inset-bottom))",boxShadow:"0 -10px 40px rgba(0,0,0,0.45)"}}
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
            <F label="Fecha inicio *">
              <input type="date" value={form.inicio} onChange={e=>setForm(f=>({...f,inicio:e.target.value,pagosMeses:{}}))}
                onMouseDown={e=>e.stopPropagation()} onTouchStart={e=>e.stopPropagation()} onTouchEnd={e=>e.stopPropagation()} onPointerDown={e=>e.stopPropagation()} onClick={e=>e.stopPropagation()}
                style={{...inpStyle,colorScheme:"light",cursor:"pointer",touchAction:"manipulation",fontSize:16}}/>
            </F>
            <F label="Fecha fin *">
            ...
            <F label={`Meses pagados (${pagosMarcados}/${mesesForm.length})`}>
              <div style={{display:"flex",flexWrap:"wrap",gap:8}}>
                {mesesForm.map(m=>{
                  ...
                  return(<button key={m.key} onClick={()=>toggleMesPago(m.key)}
                    onMouseDown={e=>e.stopPropagation()} onTouchStart={e=>e.stopPropagation()} onPointerDown={e=>e.stopPropagation()}
                    style={{padding:"6px 12px",borderRadius:8,border:`1.5px solid ${pagado?C.green:C.border}`,background:pagado?C.green+"22":"transparent",color:pagado?C.green:C.muted,fontWeight:pagado?700:500,fontSize:12,cursor:"pointer",touchAction:"manipulation",fontFamily:"inherit"}}>
                    {m.label}
                  </button>);
                })}
              </div>
          ...
          <div style={{display:"flex",gap:10,marginTop:20,position:"sticky",bottom:0}}>
            ...
            <button onClick={guardar} disabled={saving} style={{flex:2,padding:14,background:C.accent,border:"none",borderRadius:12,color:C.white,fontWeight:700,fontSize:14,cursor:"pointer",touchAction:"manipulation",minHeight:46,opacity:saving?.7:1}}>
          ...
          <div style={{marginTop:24,paddingTop:18,borderTop:`1px dashed ${C.border}`}}>
            {!confirmDelete?(
              <button onClick={()=>setConfirmDelete(true)}
                style={{width:"100%",padding:"12px",borderRadius:12,border:"1px solid #FECACA",background:"#FEF2F2",color:C.red,fontWeight:700,fontSize:14,cursor:"pointer",touchAction:"manipulation",display:"flex",alignItems:"center",justifyContent:"center",gap:8}}>
              ...
              <div style={{background:"#FEF2F2",border:"1px solid #FECACA",borderRadius:14,padding:16}}>
                <div style={{fontSize:14,fontWeight:700,color:C.red,marginBottom:6,textAlign:"center"}}>¿Confirmar eliminación?</div>
                <div style={{fontSize:12,color:C.muted,marginBottom:14,textAlign:"center"}}>Esta acción marcará el contrato como eliminado.</div>
                <div style={{display:"flex",gap:10}}>
                  <button onClick={()=>setConfirmDelete(false)} style={{flex:1,padding:"11px",borderRadius:10,border:`1px solid ${C.border}`,background:"transparent",color:C.muted,fontWeight:600,fontSize:13,cursor:"pointer",touchAction:"manipulation"}}>Cancelar</button>
                  <button onClick={eliminar} disabled={saving} style={{flex:1,padding:"11px",borderRadius:10,border:"none",background:C.red,color:"#fff",fontWeight:800,fontSize:13,cursor:"pointer",touchAction:"manipulation",opacity:saving?.6:1}}>
    ...
    <div style={{
      ...
      <div style={{position:"relative",marginBottom:20}}>
        <div style={{fontSize:12,color:"rgba(200,212,240,0.55)",fontWeight:500,marginBottom:4}}>Vista360 · 8 Millas</div>
        <div style={{fontSize:28,fontWeight:800,color:"#fff",letterSpacing:"-0.03em",lineHeight:1.05}}>Histórico</div>
        <div style={{fontSize:11.5,color:"rgba(200,212,240,0.5)",marginTop:5}}>{todos.length} registros · {activos.length} activos · {historicos.length} finalizados</div>
      </div>
      {/* Card resumen */}
      <div style={{
        position:"relative",
        ...
        <div style={{flex:"1 1 55%",display:"flex",flexDirection:"column",gap:14}}>
          <div style={{display:"flex",gap:12}}>
            {[
              {label:"Total",    val:todos.length,    color:"#fff"},
              {label:"Activos",  val:activos.length,  color:"#5BD39A"},
              {label:"Cerrados", val:historicos.length,color:"rgba(200,212,240,0.55)"},
            ].map(({label,val,color})=>(
              <div key={label} style={{flex:1,minWidth:0}}>
                <div style={{fontSize:22,fontWeight:900,color,letterSpacing:"-0.03em",lineHeight:1}}>{val}</div>
                <div style={{fontSize:10,color:"rgba(200,212,240,0.45)",marginTop:3,fontWeight:500}}>{label}</div>
              </div>
            ))}
          </div>
          {todos.length>0&&(
            <div style={{fontSize:11,color:"rgba(200,212,240,0.4)"}}>
              Total facturado: <span style={{color:"#5BD39A",fontWeight:700}}>{fmt(todos.reduce((s,c)=>s+Number(c.monto||0),0))}/mes</span>
            </div>
          )}
        </div>
        <div style={{flex:"1 1 45%",display:"flex",flexDirection:"column",justifyContent:"center",alignItems:"flex-end",gap:6}}>
          <div style={{display:"inline-flex",alignItems:"center",gap:5,background:"rgba(29,107,255,0.18)",border:"1px solid rgba(127,174,255,0.35)",color:"#BDD2FF",borderRadius:999,padding:"4px 10px",fontSize:11,fontWeight:600}}>
            <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round"><rect x="3" y="4" width="18" height="18" rx="2"/><path d="M3 9h18M8 2v4M16 2v4"/></svg>
            Todos los períodos
          </div>
          <div style={{fontSize:11,color:"rgba(200,212,240,0.35)",textAlign:"right",lineHeight:1.5}}>
            {activos.length>0&&`${activos.length} contrato${activos.length!==1?"s":""} vigente${activos.length!==1?"s":""}`}
          </div>
        </div>
      </div>
    </div>
    ...
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:20}}>
      {[
        {icon:ICN.archive, label:"Total",      value:todos.length,     sub:`${todos.length} registro${todos.length!==1?"s":""}`,    wc:NL.accent},
        {icon:ICN.check,   label:"Activos",    value:activos.length,   sub:"En curso",                                              wc:NL.green},
        {icon:ICN.archive, label:"Finalizados",value:historicos.length,sub:"Cerrados",                                              wc:NL.muted},
      ].map(({icon,label,value,sub,wc})=>(
        <div key={label} style={{background:NL.white,borderRadius:18,padding:"14px 14px 10px",border:`1px solid ${NL.border}`,boxShadow:"0 1px 0 rgba(16,22,40,0.02),0 6px 24px -16px rgba(16,22,40,0.18)",display:"flex",flexDirection:"column",minWidth:0}}>
          <div style={{width:38,height:38,borderRadius:11,background:"linear-gradient(180deg,#0E1A3B,#15265A)",display:"flex",alignItems:"center",justifyContent:"center",marginBottom:10,flexShrink:0}}>{icon}</div>
          <div style={{fontSize:11,color:NL.text,fontWeight:600,marginBottom:3}}>{label}</div>
          <div style={{fontSize:24,fontWeight:800,color:label==="Activos"?NL.green:NL.text,lineHeight:1,letterSpacing:"-0.03em"}}>{value}</div>
          <div style={{fontSize:11,color:NL.muted,marginTop:5,lineHeight:1.35,minHeight:26}}>{sub}</div>
          <div style={{marginTop:4,marginLeft:-2,marginRight:-2}}><Wave color={wc}/></div>
        </div>
      ))}
    </div>
    ...
    <div style={{display:"flex",gap:7,marginBottom:18,alignItems:"center"}}>
      {[
        ...
        {id:"historicos",label:"Finalizados",count:historicos.length},
      ].map(f=>{
        const active=filtro===f.id;
        return(
          <button key={f.id} onClick={()=>setFiltro(f.id)} style={{
            display:"flex",alignItems:"center",gap:6,padding:"8px 16px",borderRadius:99,flexShrink:0,
            border:`1px solid ${active?"#2563EB":NL.border}`,
            background:active?"linear-gradient(135deg,#2563EB,#1D4ED8)":"transparent",
            color:active?"#fff":NL.muted,fontWeight:active?700:500,
            fontSize:12,cursor:"pointer",touchAction:"manipulation",fontFamily:"inherit",
            ...
            {f.label}
            <span style={{padding:"1px 7px",borderRadius:99,background:active?"rgba(255,255,255,0.2)":"rgba(100,116,139,0.12)",color:active?"#fff":NL.muted,fontSize:11,fontWeight:700}}>
              {f.count}
            </span>
          </button>
        );
    ...
    {/* ── LISTA ── */}
    {lista.length===0?(
      <div style={{background:"linear-gradient(160deg,#0F1B36,#0A1430)",border:"1px solid rgba(255,255,255,0.06)",borderRadius:22,padding:"48px 20px",textAlign:"center",boxShadow:"0 14px 40px -18px rgba(8,12,30,0.55)"}}>
        <div style={{width:52,height:52,borderRadius:16,background:"rgba(255,255,255,0.05)",display:"flex",alignItems:"center",justifyContent:"center",margin:"0 auto 14px"}}>
          <svg width="26" height="26" viewBox="0 0 24 24" fill="none" stroke="rgba(255,255,255,0.3)" strokeWidth="1.6" strokeLinecap="round"><path d="M14 2H6a2 2 0 00-2 2v16a2 2 0 002 2h12a2 2 0 002-2V8z"/><polyline points="14 2 14 8 20 8"/></svg>
        </div>
        <div style={{fontSize:14,color:"rgba(255,255,255,0.35)"}}>No hay registros en esta categoría</div>
      </div>
    ):(
      <div style={{display:"flex",flexDirection:"column",gap:12}}>
        {lista.map(c=>{
          const activo=c.d>0;
          const statusColor=activo?(c.d<=30?"#F59E0B":"#10B981"):"rgba(200,212,240,0.4)";
          ...
          return(
            <div key={c.id} style={{
              ...
              background:"linear-gradient(160deg,#0F1B36 0%,#0A1430 55%,#070E22 100%)",
              border:"1px solid rgba(255,255,255,0.06)",
              boxShadow:"0 14px 40px -18px rgba(8,12,30,0.55),inset 0 1px 0 rgba(255,255,255,0.04)",
              padding:20,
            }}>
              {/* Línea de estado */}
              <div style={{position:"absolute",top:0,left:0,right:0,height:3,background:statusColor,borderRadius:"22px 22px 0 0"}}/>

              {/* Fila superior */}
              <div style={{display:"flex",alignItems:"flex-start",gap:14,marginBottom:14}}>
                <div style={{width:52,height:52,borderRadius:14,flexShrink:0,background:"linear-gradient(135deg,#5B8DEF,#243F8C)",display:"flex",alignItems:"center",justifyContent:"center",border:"1.5px solid rgba(255,255,255,0.18)",boxShadow:"0 6px 18px rgba(0,0,0,0.4)"}}>
                  <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="3" width="7" height="7" rx="1.5"/><rect x="14" y="3" width="7" height="7" rx="1.5"/><rect x="3" y="14" width="7" height="7" rx="1.5"/><rect x="14" y="14" width="7" height="7" rx="1.5"/></svg>
                </div>
                <div style={{flex:1,minWidth:0,paddingTop:2}}>
                  <div style={{fontSize:16,fontWeight:800,color:"#fff",letterSpacing:"-0.01em",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{c.panel.nombre}</div>
                  <div style={{fontSize:12,color:"rgba(200,212,240,0.55)",marginTop:3}}>{c.cliente.empresa} · {c.panel.ciudad}</div>
                </div>
                <div style={{textAlign:"right",flexShrink:0}}>
                  <div style={{fontSize:18,fontWeight:900,color:"#10B981",letterSpacing:"-0.02em",lineHeight:1}}>{fmt(c.monto)}<span style={{fontSize:11,color:"rgba(200,212,240,0.45)",fontWeight:600}}>/mes</span></div>
                  <span style={{display:"inline-block",marginTop:6,padding:"3px 10px",borderRadius:99,border:`1px solid ${statusColor}55`,color:statusColor,fontSize:10,fontWeight:700}}>{statusLabel}</span>
                </div>
              </div>
              ...
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:14}}>
                {[{l:"Inicio",v:fmtF(c.inicio)},{l:"Fin",v:fmtF(c.fin)}].map(({l,v})=>(
                  <div key={l} style={{background:"rgba(255,255,255,0.04)",borderRadius:10,padding:"8px 11px",border:"1px solid rgba(255,255,255,0.06)"}}>
                    <div style={{fontSize:9,color:"rgba(200,212,240,0.4)",textTransform:"uppercase",letterSpacing:1,marginBottom:3,fontWeight:700}}>{l}</div>
                    <div style={{fontSize:12,fontWeight:700,color:"rgba(200,212,240,0.85)"}}>{v}</div>
                  </div>
                ))}
              </div>
              ...
              <div style={{display:"flex",alignItems:"center",gap:10}}>
                <div style={{flex:1,display:"flex",alignItems:"center",gap:7}}>
                  <div style={{width:7,height:7,borderRadius:"50%",background:c.pagado?"#10B981":"rgba(200,212,240,0.3)",flexShrink:0}}/>
                  <span style={{fontSize:11,color:c.pagado?"#5BD39A":"rgba(200,212,240,0.4)",fontWeight:600}}>{c.pagado?"Pagado":"Pendiente"}</span>
                </div>
                <button onClick={()=>openEdit(c)} style={{display:"inline-flex",alignItems:"center",gap:6,padding:"8px 16px",borderRadius:50,border:"1px solid rgba(255,255,255,0.12)",background:"rgba(255,255,255,0.06)",color:"rgba(200,212,240,0.8)",fontWeight:700,fontSize:12,cursor:"pointer",touchAction:"manipulation",fontFamily:"inherit"}}>
                  <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M17 3a2.828 2.828 0 114 4L7.5 20.5 2 22l1.5-5.5L17 3z"/></svg>
                  Editar
                </button>
              </div>
            </div>
          ...
          {[
            ...
            <div key={i} style={{ position: "relative", flexShrink: 0 }}>
              <select value={f.value} onChange={e => f.onChange(e.target.value)}
                style={{ background: "#FFFFFF", border: "1px solid #E5E7EB", borderRadius: 10, padding: "10px 32px 10px 14px", color: "#0F1729", fontSize: 12, fontWeight: 600, outline: "none", fontFamily: "inherit", appearance: "none", cursor: "pointer", touchAction: "manipulation", boxShadow: "0 1px 3px rgba(15,23,41,0.04)" }}>
  ...
  <div style={{paddingBottom:24}}>
      ...
      <div style={{position:"relative",display:"flex",alignItems:"flex-start",justifyContent:"space-between",marginTop:4}}>
        ...
        <div style={{display:"flex",alignItems:"center",gap:10}}>
          <button onClick={()=>setTab("resultados")} aria-label="Notificaciones" style={{
            ...
            color:"#fff",position:"relative",cursor:"pointer",touchAction:"manipulation",
            display:"flex",alignItems:"center",justifyContent:"center",
          }}>
    ...
    <div style={{
      ...
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:14}}>
        ...
        <button onClick={()=>setTab("resultados")} style={{background:"none",border:"none",color:"rgba(167,139,250,0.7)",fontSize:12,cursor:"pointer",touchAction:"manipulation",fontWeight:600,padding:0}}>
          Ver todas →
        </button>
      </div>
          ...
          {acciones.slice(0,4).map((a,i)=>{
            ...
            return(
              <div key={i} style={{
                ...
                borderRadius:14,cursor:"pointer",touchAction:"manipulation",transition:"background .08s",
              }}
          ...
          <div style={{flex:1,minWidth:0}}>
            ...
            <button
              ...
              style={{
                ...
                cursor:"pointer",touchAction:"manipulation",transition:"background .08s",
              }}
        ...
        {secciones.map(s=>{
          ...
          return (
            <button key={s.id} onClick={()=>setSeccion(s.id)} style={{
              ...
              gap:5,padding:"12px 8px",borderRadius:14,border:"none",cursor:"pointer",touchAction:"manipulation",
              background:active
                ?"linear-gradient(135deg,#1E3A8A 0%,#2563EB 100%)"
                :"#FFFFFF",
              color:active?"#FFFFFF":"#1E3A8A",
              fontWeight:active?800:600,fontSize:10.5,transition:"background .08s",
              boxShadow:active
          ...
          <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:18,flexWrap:"wrap",gap:10}}>
            <div style={{display:"flex",alignItems:"center",gap:10,background:"#FFFFFF",borderRadius:14,padding:"6px 10px",boxShadow:"0 1px 4px rgba(15,23,41,0.10)",border:"1px solid #E2E8F0"}}>
              <button onClick={()=>setAnio(a=>a-1)} style={{
                ...
                background:"#EFF6FF",color:"#1E40AF",cursor:"pointer",touchAction:"manipulation",fontSize:16,fontWeight:700,
            ...
            <button onClick={exportResumenPDF} style={{
              ...
              background:"linear-gradient(135deg,#1E35C8 0%,#3854EE 100%)",
              color:"#FFFFFF",fontWeight:700,fontSize:14,cursor:"pointer",touchAction:"manipulation",fontFamily:"inherit",
              boxShadow:"0 4px 22px rgba(30,53,200,0.45), inset 0 1px 0 rgba(255,255,255,0.22)",
              letterSpacing:"0.01em",minHeight:46,
            }}>
              <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round"><path d="M14 2H6a2 2 0 00-2 2v16a2 2 0 002 2h12a2 2 0 002-2V8z"/><polyline points="14 2 14 8 20 8"/><line x1="12" y1="18" x2="12" y2="12"/><polyline points="9 15 12 18 15 15"/></svg>
              Exportar PDF {anio}
            </button>
                  ...
                  <div style={{display:"flex",alignItems:"flex-start",gap:12}}>
                    ...
                    <button
                      ...
                      style={{flexShrink:0,padding:"8px 14px",
                        background:"linear-gradient(135deg,#1E35C8 0%,#3854EE 100%)",
                        border:"none",borderRadius:50,
                        color:"#FFFFFF",fontWeight:700,fontSize:12,cursor:"pointer",touchAction:"manipulation",
                        boxShadow:"0 3px 14px rgba(30,53,200,0.45), inset 0 1px 0 rgba(255,255,255,0.22)",
                        display:"flex",alignItems:"center",gap:5,minHeight:34,fontFamily:"inherit"}}>
...
function ActividadReciente({ contratos, clientes, paneles }) {
  ...
  [...contratos]
    ...
    .forEach(c => {
      ...
      items.push({ icon: "contrato", bg: "#EFF4FF", title: "Contrato creado", sub: cl?.empresa || "—", time });
    });
  [...clientes]
    .sort((a, b) => (b.createdAt?.seconds || 0) - (a.createdAt?.seconds || 0))
    .slice(0, 2)
    .forEach(cl => {
      ...
      const diff = ts ? Math.floor((Date.now() - ts) / 3600000) : null;
      const time = diff === null ? "" : diff < 24 ? `Hace ${diff}h` : `Hace ${Math.floor(diff/24)}d`;
      items.push({ icon: "cliente", bg: "#EFF4FF", title: "Nuevo cliente agregado", sub: cl.empresa || cl.nombre || "—", time });
    });
  ...
  return (
    <div style={{ background: NL.white, borderRadius: 20, padding: "18px 16px", marginBottom: 20, boxShadow: "0 2px 12px rgba(0,0,0,0.05)" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14 }}>
        ...
        <button style={{ background: "none", border: "none", color: NL.accent, fontSize: 12, fontWeight: 600, cursor: "pointer", touchAction: "manipulation" }}>Ver todas ›</button>
      </div>
      ...
      {sorted.map((a, i) => (
        <div key={i} style={{ display: "flex", alignItems: "center", gap: 12, padding: "11px 0", borderBottom: i < sorted.length - 1 ? `1px solid ${NL.border}` : "none" }}>
          <div style={{ width: 42, height: 42, borderRadius: 13, background: a.bg, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
            {a.icon === "contrato"
              ? <svg width="20" height="20" fill="none" viewBox="0 0 24 24"><path d="M14 2H6C5.448 2 5 2.448 5 3V21C5 21.552 5.448 22 6 22H18C18.552 22 19 21.552 19 21V7L14 2Z" stroke="#2563EB" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/><polyline points="14 2 14 8 19 8" stroke="#2563EB" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/><line x1="9" y1="13" x2="15" y2="13" stroke="#2563EB" strokeWidth="2" strokeLinecap="round"/><line x1="9" y1="17" x2="15" y2="17" stroke="#2563EB" strokeWidth="2" strokeLinecap="round"/></svg>
              : <svg width="20" height="20" fill="none" viewBox="0 0 24 24"><path d="M20 21V19C20 17.343 18.657 16 17 16H7C5.343 16 4 17.343 4 19V21" stroke="#2563EB" strokeWidth="2" strokeLinecap="round"/><circle cx="12" cy="8" r="4" stroke="#2563EB" strokeWidth="2"/></svg>
            }
          </div>
          <div style={{ flex: 1 }}>
...
// ══════════════════════════════════════════════════════════════════
// CAPITAL — Patrimonio, depreciación y flujo de caja
// ══════════════════════════════════════════════════════════════════
// ══════════════════════════════════════════════════════════════════
// CAPITAL — diseño financiero único (dark emerald)
// ══════════════════════════════════════════════════════════════════
function Capital({ paneles, contratos, gastos, proveedores }) {

  const [tasaGlobal, setTasaGlobal] = useState(null);
  const [seccion, setSeccion] = useState("flujo"); // flujo | depreciacion
  const usandoGlobal = tasaGlobal !== null;

  // ── Cálculo depreciación ──
  const hoyMs = Date.now();
  const panelDep = paneles.map(p => {
    const costo  = Number(p.costoInstalacion) || 0;
    const vida   = usandoGlobal ? (100 / tasaGlobal) : (Number(p.vidaUtilAnios) || 10);
    const inicio = p.fechaInstalacion ? new Date(p.fechaInstalacion).getTime() : null;
    if (!costo || !inicio) return { ...p, valorActual: costo, depAcum: 0, pctDesgaste: 0, aniosUsados: 0 };
    const aniosUsados = Math.max(0, (hoyMs - inicio) / (365.25 * 24 * 3600 * 1000));
    const depAcum     = Math.min(costo, (costo / vida) * aniosUsados);
    const valorActual = Math.max(0, costo - depAcum);
    const pctDesgaste = Math.min(100, (depAcum / costo) * 100);
    return { ...p, valorActual, depAcum, pctDesgaste, aniosUsados };
  });

  const costoOriginal  = panelDep.reduce((s, p) => s + (Number(p.costoInstalacion) || 0), 0);
  const valorPaneles   = panelDep.reduce((s, p) => s + (p.valorActual || 0), 0);
  const depAcumTotal   = panelDep.reduce((s, p) => s + (p.depAcum || 0), 0);
  const pctPortfolio   = costoOriginal > 0 ? (depAcumTotal / costoOriginal) * 100 : 0;
  const porCobrar      = contratos.filter(c => !c.pagado).reduce((s, c) => s + Number(c.monto || 0), 0);
  const porPagar       = proveedores.filter(p => !p.pagado).reduce((s, p) => s + Number(p.monto || 0), 0);
  const totalActivos   = valorPaneles + porCobrar;
  const patrimonioNeto = totalActivos - porPagar;
  ...
  const flujo = meses6.map(mes => {
    const ing = contratos.filter(c => c.pagado && c.inicio && c.inicio.slice(0,7) <= mes && (!c.fin || c.fin.slice(0,7) >= mes)).reduce((s,c) => s + Number(c.monto||0), 0);
    const eg  = gastos.filter(g => g.fecha && g.fecha.slice(0,7) === mes).reduce((s,g) => s + Number(g.monto||0), 0);
    return { mes, ing, eg, neto: ing - eg };
  });
  const maxBar = Math.max(...flujo.flatMap(f => [f.ing, f.eg]), 1);
  const MESES_C = ["Ene","Feb","Mar","Abr","May","Jun","Jul","Ago","Sep","Oct","Nov","Dic"];
  const mC = m => MESES_C[parseInt(m.split("-")[1],10) - 1];
  const mesLabel2 = m => {
    const [y, mm] = m.split("-");
    return `${MESES_C[parseInt(mm,10)-1]} ${y}`;
  };

  // ── Paleta azul/blanco/negro exclusiva para Capital ──
  const EM = {
    bg:      "#04080F",
    card:    "#080F1C",
    card2:   "#0D1528",
    border:  "rgba(37,99,235,0.14)",
    border2: "rgba(74,140,255,0.28)",
    green:   "#4A8CFF",
    greenDim:"rgba(74,140,255,0.55)",
    red:     "#FF4D4D",
    white:   "#FFFFFF",
    muted:   "rgba(160,185,240,0.45)",
    text:    "rgba(210,225,255,0.9)",
  };

  return (
    <div style={{ background: EM.bg, margin: "-20px -16px", padding: "0 0 calc(120px + env(safe-area-inset-bottom))", minHeight: "100%" }}>

      {/* ── TOP STRIP ── */}
      <div style={{ padding: "24px 20px 20px", borderBottom: `1px solid ${EM.border}` }}>
        <div style={{ fontSize: 11, color: EM.muted, fontWeight: 600, letterSpacing: 1.5, textTransform: "uppercase", marginBottom: 6 }}>Vista360 · Capital</div>
        <div style={{ display: "flex", alignItems: "flex-end", justifyContent: "space-between", gap: 12 }}>
          <div>
            <div style={{ fontSize: 12, color: EM.muted, marginBottom: 4 }}>Patrimonio neto</div>
            <div style={{ fontSize: 38, fontWeight: 900, color: patrimonioNeto >= 0 ? EM.green : EM.red, letterSpacing: "-1.5px", lineHeight: 1 }}>
              {fmt(patrimonioNeto)}
            </div>
            <div style={{ display: "flex", gap: 14, marginTop: 8 }}>
              <span style={{ fontSize: 11, color: EM.muted }}>Activos <span style={{ color: EM.green, fontWeight: 700 }}>{fmt(totalActivos)}</span></span>
              <span style={{ fontSize: 11, color: EM.muted }}>Pasivos <span style={{ color: EM.red, fontWeight: 700 }}>{fmt(porPagar)}</span></span>
            </div>
          </div>
          {/* Mini gauge */}
          <div style={{ flexShrink: 0, display: "flex", flexDirection: "column", alignItems: "center", gap: 4 }}>
            <svg width="64" height="36" viewBox="0 0 64 36">
              <path d="M4 34 A28 28 0 0 1 60 34" fill="none" stroke="rgba(37,99,235,0.12)" strokeWidth="6" strokeLinecap="round"/>
              <path d="M4 34 A28 28 0 0 1 60 34" fill="none" stroke={EM.green} strokeWidth="6" strokeLinecap="round"
                strokeDasharray="88" strokeDashoffset={88 - Math.min(88, (totalActivos > 0 ? (patrimonioNeto / totalActivos) : 0) * 88)}/>
              <text x="32" y="30" textAnchor="middle" fontSize="9" fontWeight="800" fill={EM.green} fontFamily="DM Sans,sans-serif">
                {totalActivos > 0 ? Math.round((patrimonioNeto / totalActivos) * 100) : 0}%
              </text>
            </svg>
            <div style={{ fontSize: 9, color: EM.muted, letterSpacing: 0.5 }}>SALUD</div>
          </div>
        </div>
      </div>

      {/* ── 4 KPIS EN FILA ── */}
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 1, borderBottom: `1px solid ${EM.border}` }}>
        {[
          { label: "Valor paneles",  value: fmt(valorPaneles),  delta: costoOriginal > 0 ? `-${pctPortfolio.toFixed(0)}%` : null,  deltaColor: EM.red,   icon: <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><rect x="3" y="3" width="7" height="7" rx="1.5"/><rect x="14" y="3" width="7" height="7" rx="1.5"/><rect x="3" y="14" width="7" height="7" rx="1.5"/><rect x="14" y="14" width="7" height="7" rx="1.5"/></svg> },
          { label: "Por cobrar",     value: fmt(porCobrar),     delta: contratos.filter(c=>!c.pagado).length > 0 ? `${contratos.filter(c=>!c.pagado).length} cttos` : null, deltaColor: EM.green, icon: <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><path d="M12 2v20M17 7H9.5C8.12 7 7 8.12 7 9.5S8.12 12 9.5 12h5C15.88 12 17 13.12 17 14.5S15.88 17 14.5 17H7"/></svg> },
          { label: "Por pagar",      value: fmt(porPagar),      delta: proveedores.filter(p=>!p.pagado).length > 0 ? `${proveedores.filter(p=>!p.pagado).length} proveed.` : null, deltaColor: EM.red,   icon: <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><rect x="2" y="5" width="20" height="14" rx="2"/><line x1="2" y1="10" x2="22" y2="10"/></svg> },
          { label: "Dep. acumulada", value: fmt(depAcumTotal),  delta: pctPortfolio > 0 ? `${pctPortfolio.toFixed(0)}% portfolio` : "Sin datos",            deltaColor: EM.red,   icon: <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><polyline points="22 17 13.5 8.5 8.5 13.5 2 7"/><polyline points="16 17 22 17 22 11"/></svg> },
        ].map(({ label, value, delta, deltaColor, icon }, i) => (
          <div key={label} style={{
            padding: "16px 18px",
            background: EM.card,
            borderRight: i % 2 === 0 ? `1px solid ${EM.border}` : "none",
            borderBottom: i < 2 ? `1px solid ${EM.border}` : "none",
          }}>
            <div style={{ display: "flex", alignItems: "center", gap: 7, marginBottom: 10, color: EM.muted }}>
              {icon}
              <span style={{ fontSize: 10, fontWeight: 700, letterSpacing: 0.8, textTransform: "uppercase" }}>{label}</span>
            </div>
            <div style={{ fontSize: 20, fontWeight: 800, color: label === "Por cobrar" ? EM.green : label === "Por pagar" || label === "Dep. acumulada" ? EM.red : EM.white, letterSpacing: "-0.5px", lineHeight: 1, marginBottom: 6 }}>{value}</div>
            {delta && <div style={{ fontSize: 10, color: deltaColor, fontWeight: 600 }}>{delta}</div>}
          </div>
        ))}
      </div>

      {/* ── SELECTOR FLUJO / DEPRECIACIÓN ── */}
      <div style={{ display: "flex", borderBottom: `1px solid ${EM.border}` }}>
        {[{ id: "flujo", label: "Flujo de caja" }, { id: "depreciacion", label: "Depreciación" }].map(s => (
          <button key={s.id} onClick={() => setSeccion(s.id)} style={{
            flex: 1, padding: "14px 0", background: "none", border: "none",
            borderBottom: seccion === s.id ? `2px solid ${EM.green}` : "2px solid transparent",
            color: seccion === s.id ? EM.green : EM.muted,
            fontWeight: seccion === s.id ? 700 : 500, fontSize: 13,
      ...
      {seccion === "flujo" && (
        <div style={{ padding: "20px 18px" }}>
          ...
          <div style={{ marginBottom: 24 }}>
            <div style={{ fontSize: 11, color: EM.muted, fontWeight: 700, letterSpacing: 1, textTransform: "uppercase", marginBottom: 16 }}>Últimos 6 meses</div>
            <div style={{ display: "flex", alignItems: "flex-end", gap: 8, height: 110, marginBottom: 10 }}>
              {flujo.map(({ mes, ing, eg }) => (
                <div key={mes} style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", gap: 3, height: "100%", minWidth: 0 }}>
                  <div style={{ flex: 1, width: "100%", display: "flex", flexDirection: "column", justifyContent: "flex-end", gap: 3 }}>
                    {ing > 0 && <div style={{ width: "100%", height: `${(ing / maxBar) * 100}%`, minHeight: 5, background: `linear-gradient(180deg, ${EM.green}, rgba(74,140,255,0.6))`, borderRadius: "5px 5px 0 0", boxShadow: `0 0 12px rgba(37,99,235,0.35)` }}/>}
                    {eg > 0 && <div style={{ width: "100%", height: `${(eg / maxBar) * 100}%`, minHeight: 5, background: `linear-gradient(180deg, ${EM.red}, rgba(255,77,77,0.5))`, borderRadius: "5px 5px 0 0" }}/>}
                  </div>
                </div>
              ))}
            </div>
            <div style={{ display: "flex", gap: 8 }}>
              {flujo.map(({ mes }) => (
                <div key={mes} style={{ flex: 1, textAlign: "center", fontSize: 9, color: EM.muted, fontWeight: 700, textTransform: "uppercase", letterSpacing: 0.5 }}>{mC(mes)}</div>
              ))}
            </div>
            <div style={{ display: "flex", gap: 18, justifyContent: "center", marginTop: 12 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 11, color: EM.muted }}>
                <div style={{ width: 8, height: 8, borderRadius: 2, background: EM.green, boxShadow: `0 0 6px ${EM.green}` }}/>
                Ingresos cobrados
              </div>
              <div style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 11, color: EM.muted }}>
                <div style={{ width: 8, height: 8, borderRadius: 2, background: EM.red }}/>
                Gastos
              </div>
            </div>
          </div>
          ...
          {/* Tabla mensual */}
          <div style={{ fontSize: 11, color: EM.muted, fontWeight: 700, letterSpacing: 1, textTransform: "uppercase", marginBottom: 12 }}>Detalle</div>
          {flujo.slice().reverse().map(({ mes, ing, eg, neto }) => (
            <div key={mes} style={{ display: "flex", alignItems: "center", gap: 12, padding: "13px 0", borderBottom: `1px solid ${EM.border}` }}>
              <div style={{ width: 40, height: 40, borderRadius: 12, background: neto >= 0 ? "rgba(37,99,235,0.12)" : "rgba(255,77,77,0.1)", border: `1px solid ${neto >= 0 ? EM.border2 : "rgba(255,77,77,0.2)"}`, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke={neto >= 0 ? EM.green : EM.red} strokeWidth="2.5" strokeLinecap="round">
                  {neto >= 0 ? <polyline points="18 15 12 9 6 15"/> : <polyline points="6 9 12 15 18 9"/>}
                </svg>
              </div>
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ fontSize: 13, fontWeight: 700, color: EM.text }}>{mesLabel2(mes)}</div>
                <div style={{ fontSize: 11, color: EM.muted, marginTop: 2 }}>Ing. {fmt(ing)} · Gas. {fmt(eg)}</div>
              </div>
              <div style={{ textAlign: "right", flexShrink: 0 }}>
                <div style={{ fontSize: 16, fontWeight: 800, color: neto >= 0 ? EM.green : EM.red, letterSpacing: "-0.5px" }}>{neto >= 0 ? "+" : ""}{fmt(neto)}</div>
                <div style={{ fontSize: 9, color: EM.muted, marginTop: 2, textTransform: "uppercase", letterSpacing: 0.5 }}>neto</div>
              </div>
            </div>
          ))}
        </div>
      ...
      {seccion === "depreciacion" && (
        <div style={{ padding: "20px 18px" }}>
          ...
          {/* Control tasa */}
          <div style={{ background: EM.card, border: `1px solid ${EM.border}`, borderRadius: 16, padding: "16px 18px", marginBottom: 20 }}>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: usandoGlobal ? 16 : 0 }}>
              <div>
                <div style={{ fontSize: 12, fontWeight: 700, color: EM.text, marginBottom: 3 }}>Tasa de depreciación</div>
                <div style={{ fontSize: 11, color: EM.muted }}>
                  {usandoGlobal ? `Global ${tasaGlobal}% anual · ~${Math.round(100 / tasaGlobal)} años` : "Tasa individual por panel"}
                </div>
              </div>
              <button onClick={() => setTasaGlobal(usandoGlobal ? null : 10)} style={{
                padding: "7px 16px", borderRadius: 99, border: `1px solid ${usandoGlobal ? EM.green : EM.border2}`,
                background: usandoGlobal ? "rgba(37,99,235,0.15)" : "transparent",
                color: usandoGlobal ? EM.green : EM.muted,
                fontSize: 12, fontWeight: 700, cursor: "pointer", touchAction: "manipulation", fontFamily: "inherit",
              }}>
                {usandoGlobal ? "Activo" : "Ajustar"}
              </button>
            </div>
            {usandoGlobal && (
              <div>
                <input type="range" min="1" max="50" value={tasaGlobal} onChange={e => setTasaGlobal(Number(e.target.value))}
                  style={{ width: "100%", accentColor: EM.green, cursor: "pointer", marginBottom: 8 }}/>
                <div style={{ display: "flex", justifyContent: "space-between", fontSize: 10, color: EM.muted, marginBottom: 12 }}>
                  <span>1%</span>
                  <span style={{ fontSize: 14, fontWeight: 800, color: EM.green }}>{tasaGlobal}% anual</span>
                  <span>50%</span>
                </div>
                <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                  {[{ l: "SUNAT 10%", v: 10 }, { l: "5%", v: 5 }, { l: "20%", v: 20 }, { l: "25%", v: 25 }].map(({ l, v }) => (
                    <button key={v} onClick={() => setTasaGlobal(v)} style={{
                      padding: "5px 12px", borderRadius: 99, fontSize: 11, fontWeight: 700, cursor: "pointer", touchAction: "manipulation",
                      border: `1px solid ${tasaGlobal === v ? EM.green : EM.border2}`,
                      background: tasaGlobal === v ? "rgba(37,99,235,0.15)" : "transparent",
                      color: tasaGlobal === v ? EM.green : EM.muted,
                      fontFamily: "inherit",
                    }}>{l}</button>
                  ))}
                </div>
          ...
          {costoOriginal > 0 && (
            <div style={{ background: EM.card, border: `1px solid ${EM.border}`, borderRadius: 16, padding: "16px 18px", marginBottom: 20 }}>
              <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 10 }}>
                <span style={{ fontSize: 11, color: EM.muted, fontWeight: 700, textTransform: "uppercase", letterSpacing: 0.8 }}>Portfolio total</span>
                <span style={{ fontSize: 11, color: EM.muted }}>{pctPortfolio.toFixed(0)}% depreciado</span>
              </div>
              <div style={{ background: "rgba(37,99,235,0.08)", borderRadius: 99, height: 8, overflow: "hidden", marginBottom: 10 }}>
                <div style={{ height: "100%", borderRadius: 99, width: `${100 - pctPortfolio}%`, background: `linear-gradient(90deg, ${EM.green}, rgba(74,140,255,0.6))`, boxShadow: `0 0 10px rgba(37,99,235,0.4)`, transition: "width .5s ease" }}/>
              </div>
              <div style={{ display: "flex", justifyContent: "space-between", fontSize: 11 }}>
                <span style={{ color: EM.green, fontWeight: 700 }}>{fmt(valorPaneles)} valor actual</span>
                <span style={{ color: EM.red, fontWeight: 700 }}>{fmt(depAcumTotal)} depreciado</span>
              </div>
            </div>
          )}

          {/* Lista paneles */}
          <div style={{ fontSize: 11, color: EM.muted, fontWeight: 700, letterSpacing: 1, textTransform: "uppercase", marginBottom: 12 }}>Por panel</div>
          {panelDep.filter(p => Number(p.costoInstalacion) > 0).length === 0 ? (
            <div style={{ background: EM.card, border: `1px solid ${EM.border}`, borderRadius: 16, padding: "36px 20px", textAlign: "center" }}>
              <div style={{ width: 48, height: 48, borderRadius: 14, background: "rgba(37,99,235,0.08)", border: `1px solid ${EM.border}`, display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto 12px", color: EM.muted }}>
                <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round"><path d="M14 2H6a2 2 0 00-2 2v16a2 2 0 002 2h12a2 2 0 002-2V8z"/><polyline points="14 2 14 8 20 8"/></svg>
              </div>
              <div style={{ fontSize: 13, fontWeight: 600, color: EM.text, marginBottom: 4 }}>Sin datos de capital</div>
              <div style={{ fontSize: 12, color: EM.muted }}>Edita cada panel y agrega costo + fecha de instalación</div>
            </div>
          ) : (
            panelDep.filter(p => Number(p.costoInstalacion) > 0).sort((a, b) => b.pctDesgaste - a.pctDesgaste).map((p, idx, arr) => (
              <div key={p.id} style={{ background: EM.card, border: `1px solid ${EM.border}`, borderRadius: 16, padding: "16px 18px", marginBottom: idx < arr.length - 1 ? 10 : 0 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 12 }}>
                  <div style={{ width: 42, height: 42, borderRadius: 12, background: "rgba(37,99,235,0.08)", border: `1px solid ${EM.border2}`, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, color: EM.greenDim }}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ fontSize: 14, fontWeight: 700, color: EM.text, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{p.nombre}</div>
                    <div style={{ fontSize: 11, color: EM.muted, marginTop: 2 }}>{p.ciudad} · {(p.aniosUsados || 0).toFixed(1)} años</div>
                  </div>
                  <div style={{ textAlign: "right", flexShrink: 0 }}>
                    <div style={{ fontSize: 16, fontWeight: 800, color: EM.green, letterSpacing: "-0.5px" }}>{fmt(p.valorActual)}</div>
                    <div style={{ fontSize: 9, color: EM.muted, marginTop: 2, textTransform: "uppercase", letterSpacing: 0.5 }}>actual</div>
                  </div>
                </div>
                <div style={{ background: "rgba(37,99,235,0.06)", borderRadius: 99, height: 6, overflow: "hidden", marginBottom: 7 }}>
                  <div style={{
                    height: "100%", borderRadius: 99, transition: "width .5s ease",
                    width: `${p.pctDesgaste}%`,
                    background: p.pctDesgaste > 75 ? `linear-gradient(90deg,${EM.red},rgba(255,77,77,0.6))` : p.pctDesgaste > 50 ? "linear-gradient(90deg,#FFF,rgba(255,255,255,0.5))" : `linear-gradient(90deg,${EM.green},rgba(74,140,255,0.5))`,
                    boxShadow: p.pctDesgaste <= 50 ? `0 0 8px rgba(37,99,235,0.4)` : "none",
                  }}/>
                </div>
                <div style={{ display: "flex", justifyContent: "space-between", fontSize: 11 }}>
                  <span style={{ color: EM.muted }}>Original {fmt(Number(p.costoInstalacion))}</span>
                  <span style={{ color: p.pctDesgaste > 75 ? EM.red : p.pctDesgaste > 50 ? "#FFFFFF" : EM.green, fontWeight: 700 }}>{p.pctDesgaste.toFixed(0)}% depreciado</span>
                </div>
              </div>
            ))
          )}
          ...
          <div style={{ background: "#fff", borderRadius: 16, marginTop: 8, padding: "16px", boxShadow: "0 20px 60px rgba(0,0,0,0.25)" }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
              {[
                ...
                <button key={a.tab} onClick={() => { onNavigate(a.tab); onClose(); }} style={{ display: "flex", alignItems: "center", gap: 10, padding: "10px 12px", background: "#F8FAFC", border: "1px solid #E5E7EB", borderRadius: 12, cursor: "pointer", touchAction: "manipulation", fontSize: 13, fontWeight: 600, color: "#0F1729", fontFamily: "'DM Sans',sans-serif" }}>
...
export default function App() {
  ...
  useEffect(() => {
    ...
    // Sincronizar fondo del documento con el contenido — elimina el gap en PWA
  ...
  const renderContent = useCallback(() => {
    ...
    return (
      <>
        ...
        {tab === "capital"     && <Capital      paneles={paneles} contratos={contratos} gastos={gastos} proveedores={proveedores}/>}
