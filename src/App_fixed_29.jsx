...
function Mapa({paneles,clientes,contratos}){
  ...
  useEffect(()=>{
    ...
    paneles.forEach(p=>{
      ...
      const baseColor = panelsConContratoHoy.has(p.id)?"#EF4444":"#10B981";
...
function Hoy({clientes,contratos,paneles,gastos,setTab}){
  ...
  // ── KPIs — CORRECTO: derivar todo del estado real ──────────────
  const hoyStr = hoyD.toISOString().slice(0,10);

  // Panel ocupado = tiene contrato activo HOY (no el campo estado que es manual)
  const panelsOcupadosReal = new Set(
    contratos
      .filter(c => !c.deleted && c.inicio <= hoyStr && c.fin >= hoyStr)
      .map(c => c.panel_id)
  );
  const panelLibres  = paneles.filter(p => !panelsOcupadosReal.has(p.id)).length;
  const ocupados     = paneles.filter(p => panelsOcupadosReal.has(p.id)).length;
  const totalPaneles = paneles.length;
  const ocupPct      = totalPaneles > 0 ? Math.round((ocupados / totalPaneles) * 100) : 0;

  // Ingresos cobrados este mes = pagosMeses[mesKey] === true
  const ingMes = contratos.filter(c => {
    if (c.deleted) return false;
    const ini = c.inicio?.slice(0,7); const fin = c.fin?.slice(0,7);
    return ini && fin && ini <= mesKey && fin >= mesKey && c.pagosMeses?.[mesKey] === true;
  }).reduce((a,c) => a + Number(c.monto), 0);

  // Por cobrar este mes = activos este mes pero NO cobrados en pagosMeses[mesKey]
  const cobPendiente = contratos.filter(c => {
    if (c.deleted) return false;
    const ini = c.inicio?.slice(0,7); const fin = c.fin?.slice(0,7);
    return ini && fin && ini <= mesKey && fin >= mesKey && !c.pagosMeses?.[mesKey];
  }).reduce((a,c) => a + Number(c.monto), 0);

  const vence7       = contratos.filter(c => { const d = Math.ceil((new Date(c.fin) - hoyD)/86400000); return !c.deleted && d >= 0 && d <= 30; }).length;
  const ingPotencial = paneles.filter(p => !panelsOcupadosReal.has(p.id)).reduce((a,p) => a + Number(p.precio||0), 0);
  ...
  const acciones = useMemo(()=>{
    ...
    // Paneles libres sin contrato activo hoy (derivado, no campo estado)
    const hoyStrA = new Date().toISOString().slice(0,10);
    const panelsOcupHoy = new Set(
      contratos.filter(c => !c.deleted && c.inicio <= hoyStrA && c.fin >= hoyStrA).map(c => c.panel_id)
    );
    paneles.filter(p => !panelsOcupHoy.has(p.id)).forEach(p => {
      const diasLibre = (() => {
    ...
    <>
      ...
      {!splash && !!user && <div style={{ position: "fixed", top: 0, left: 0, right: 0, bottom: 0, background: NL.bg, display: "flex", flexDirection: "column", fontFamily: "'DM Sans',sans-serif", color: NL.text, overflow: "hidden", overscrollBehavior: "none" }}>
        ...
        <div
          ref={scrollRef}
          data-scroll
          style={{ flex: 1, minHeight: 0, overflowY: "scroll", overflowX: "hidden", overscrollBehavior: "none", touchAction: "pan-y", background: headerDark ? headerColor : undefined, position:"relative" }}
        >
