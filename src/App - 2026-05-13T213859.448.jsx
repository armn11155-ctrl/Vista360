...
function LoginScreen({ onLoginSuccess }) {
  ...
  // Detectar iOS/Safari (iPhone, iPad, iPod) — incluye iPad con "desktop mode"
  const isIOS =
    /iPad|iPhone|iPod/.test(navigator.userAgent) ||
    (navigator.platform === "MacIntel" && navigator.maxTouchPoints > 1);

  // Al montar: si venimos de un redirect de Google (iOS), procesar el resultado
  useEffect(() => {
    getRedirectResult(auth)
      .then((result) => {
        if (!result) return; // No era un redirect, no hacer nada
        const u = result.user;
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      if (isIOS) {
        // iOS Safari bloquea popups — usar redirect en su lugar
        await signInWithRedirect(auth, googleProvider);
        // La página se redirige a Google; cuando vuelva, el useEffect
        // de arriba llamará a getRedirectResult y procesará el usuario.
      } else {
      ...
      if (err.code === "auth/popup-closed-by-user") {
        ...
        setError("Google Sign-In no está habilitado en Firebase. Actívalo en Firebase Console → Authentication.");
      } else {
