...
// ══════════════════════════════════════════════════════════════════
// 🧾 MÓDULO DE FACTURACIÓN — 8 Millas Publicidad
// ══════════════════════════════════════════════════════════════════
//
// ESTADO: Estructura lista · Pendiente conexión a sistema externo
//
// CUANDO TENGAS TU SISTEMA DE FACTURACIÓN listo, reemplaza las
// llamadas a fb.get/post/patch/del de "facturas" por tu API:
//
//   📌 LEER     → fb.get("facturas")           → GET  /api/facturas
//   📌 CREAR    → fb.post("facturas", data)     → POST /api/facturas
//   📌 EDITAR   → fb.patch("facturas", id, d)   → PUT  /api/facturas/:id
//   📌 ELIMINAR → fb.del("facturas", id)        → DEL  /api/facturas/:id
//
// Campos esperados por factura:
//   id, tipo, serie, numero, estado, cliente_id, panel_id,
//   contrato_id, concepto, monto, fecha_emision, fecha_vencimiento,
//   metodo_pago, nro_operacion, fecha_pago, email_enviado
// ══════════════════════════════════════════════════════════════════
function Facturacion({ contratos, paneles, clientes }) {
  ...
  // ── FUENTE DE DATOS ──────────────────────────────────────────────
  // Actualmente: Firebase (colección "facturas")
  // TODO: Reemplazar con tu API de facturación externa cuando esté lista
  // ────────────────────────────────────────────────────────────────
  useEffect(() => {
  ...
  return (
    <div style={{ paddingBottom: 32 }}>
      ...
      {/* ── BANNER: Conexión a sistema de facturación externo pendiente ── */}
      <div style={{ background: "linear-gradient(135deg,#0D1F3C,#0A1628)", border: "1px solid #2563EB44", borderRadius: 16, padding: "14px 18px", marginBottom: 18, display: "flex", alignItems: "center", gap: 12 }}>
        ...
        <div>
          <div style={{ fontSize: 13, fontWeight: 800, color: "#5A9BFF", marginBottom: 2 }}>Conexión pendiente</div>
          <div style={{ fontSize: 11, color: "rgba(160,180,220,0.70)", lineHeight: 1.5 }}>
            Cuando tu sistema de facturación esté listo, los datos aparecerán aquí automáticamente. La estructura ya está preparada.
          </div>
        </div>
      </div>
...
function HeroCard({ panalesLibres }) {
  const msg = panalesLibres > 0
    ? `${panalesLibres} panel${panalesLibres > 1 ? "es" : ""} libre${panalesLibres > 1 ? "s" : ""} para asignar.`
    : "Todos los paneles están asignados.";

  return (
    <div style={{ borderRadius: 24, overflow: "hidden", background: "#0A0B11", position: "relative", marginBottom: 20, boxShadow: "0 8px 32px rgba(0,0,0,0.18)" }}>
      <HeroWaveArt/>
      <div style={{ position: "relative", zIndex: 2, padding: "24px 24px 28px", maxWidth: "76%" }}>
        <div style={{ fontSize: 12, color: "rgba(255,255,255,0.75)", fontWeight: 500, marginBottom: 10, letterSpacing: 0.2 }}>
          {new Date().toLocaleDateString("es-PE", { weekday: "long", day: "numeric", month: "long", year: "numeric" }).replace(/^\w/, c => c.toUpperCase())}
        </div>
        <div style={{ fontSize: 22, fontWeight: 800, color: "#FFFFFF", marginBottom: 6, letterSpacing: "-0.01em" }}>
          Hola, <span style={{ color: "#7FAEFF" }}>Alan Martinez</span>
        </div>
        <div style={{ fontSize: 28, fontWeight: 800, color: "#FFFFFF", lineHeight: 1.05, letterSpacing: "-0.025em", marginBottom: 10 }}>Todo bajo<br />control.</div>
        <div style={{ fontSize: 13, color: "rgba(235,238,250,0.60)", lineHeight: 1.5 }}>{msg}</div>
      </div>
    </div>
  );
}
  ...
  return (
    <>
      ...
      <div style={{
        ...
        <button onClick={() => { onTabClick("perfil"); onClose(); }} style={{ width: "100%", padding: "0 20px 20px", borderBottom: `1px solid ${NL.border}`, background: "none", border: "none", borderBottom: `1px solid ${NL.border}`, cursor: "pointer", textAlign: "left" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
            <div style={{ width: 44, height: 44, borderRadius: "50%", background: "linear-gradient(135deg,#2563EB,#7C3AED)", display: "flex", alignItems: "center", justifyContent: "center", color: "#fff", fontWeight: 800, fontSize: 14, flexShrink: 0 }}>AM</div>
            <div style={{ flex: 1, minWidth: 0 }}>
            ...
            <svg width="14" height="14" fill="none" viewBox="0 0 24 24" style={{ opacity: 0.35, flexShrink: 0 }}><path d="M9 18l6-6-6-6" stroke={NL.text} strokeWidth="2" strokeLinecap="round"/></svg>
          </div>
        </button>
