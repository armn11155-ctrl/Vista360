...
export default function App(){
  ...
  return(<>
    ...
    <style>{`*{box-sizing:border-box;-webkit-tap-highlight-color:transparent}html,body{margin:0;padding:0;background:#0A0F1E;overflow:hidden;height:100%;overscroll-behavior-y:none;-webkit-text-size-adjust:100%}::-webkit-scrollbar{display:none}@keyframes spin{to{transform:rotate(360deg)}}@keyframes pulse{from{opacity:.2}to{opacity:.6}}@keyframes starTwinkle{0%,100%{opacity:0.5;transform:scale(1)}50%{opacity:1;transform:scale(1.3)}}@keyframes dishPulse{0%,100%{opacity:0.35}50%{opacity:0.55}}`}</style>

    {splash&&<Splash done={()=>setSplash(false)}/>}
    {showConfig&&<ModalConfig onClose={()=>setShowConfig(false)}/>}

    <div style={{position:"fixed",top:0,left:0,right:0,bottom:0,width:"100vw",height:"100dvh",backgroundColor:"#0A0F1E",display:"flex",flexDirection:"row",margin:0,padding:0,overflow:"hidden",fontFamily:"'DM Sans',sans-serif",color:C.text}}>
      ...
      <div style={{display:"flex",flexDirection:"column",height:"100%",paddingTop:"max(10px, calc(env(safe-area-inset-top) + 6px))",paddingBottom:"env(safe-area-inset-bottom)",paddingLeft:"10px",paddingRight:"10px",gap:"8px",position:"relative",zIndex:1,boxSizing:"border-box"}}>
      ...
      <div style={{
        ...
        <div style={{display:"flex",flexDirection:"row",alignItems:"center",gap:2,flex:1,overflowX:"auto",scrollbarWidth:"none"}}>
          {tabs.map(t=>{
            ...
            return(
              <button key={t.id} onClick={()=>handleTabClick(t.id)} title={t.label} style={{
                flexShrink:0,
                display:"flex",flexDirection:"row",alignItems:"center",
                ...
                {t.id==="perfil" ? (
                  <div style={{
                    ...
                    boxShadow:active?"0 2px 8px rgba(79,70,229,0.5)":"none",
                    border:active?"2px solid rgba(79,124,255,0.5)":"2px solid rgba(255,255,255,0.15)",
      ...
      <div style={{
        ...
        <main style={{flex:1,overflowY:"auto",overflowX:"hidden",padding:"20px 20px",paddingBottom:"max(32px, calc(env(safe-area-inset-bottom) + 20px))",WebkitOverflowScrolling:"touch",scrollBehavior:"smooth"}}>
