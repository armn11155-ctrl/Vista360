...
function Gastos({gastos,setGastos}){
  ...
  const escanear=async(file)=>{
    ...
    try{
      ...
      const {data:{text}}=await window.Tesseract.recognize(file,"spa+eng",{
        logger:(m)=>{ if(m.status==="recognizing text") setOcr(o=>({...o,progress:Math.round(m.progress*100)})); }
      });
      setOcr(o=>({...o,loading:false,progress:100,text}));
      extraerDatos(text);
    }catch(e){
  ...
  const guardar=async()=>{
    ...
    setSaving(true);
    // La tabla gastos usa 'descripcion' — mapeamos concepto → descripcion
    const descripcion=(form.concepto||form.proveedor||"Sin descripción").trim()||"Sin descripción";
    const payload={
      descripcion,                                          // campo requerido en Supabase
      categoria:form.categoria||"Otro",
      monto:Number(form.monto)||0,
      mes:form.fecha?form.fecha.substring(0,7):mes,        // formato YYYY-MM
      // Campos extra (si existen en la tabla)
      fecha:form.fecha||new Date().toISOString().split("T")[0],
      proveedor:form.proveedor||"",
      ruc:form.ruc||"",
      concepto:form.concepto||"",
      igv:Number(form.igv)||0,
    ...
    try{
      if(modal==="nuevo"||modal==="editar-nuevo"){
        ...
        await sb.patch("gastos",modal.id,payload);
        setGastos(g=>g.map(x=>x.id===modal.id?{...x,...payload}:x));
      }
  ...
  const KInp=({label,keyN,type="text",opts=[]})=>(
    <div style={{marginBottom:16}}>
      ...
      {type==="select"
        ...
        :<input type={type} value={form[keyN]}
            onChange={e=>setForm(f=>({...f,[keyN]:e.target.value}))}
            inputMode={type==="number"?"decimal":undefined}
            style={{width:"100%",background:"transparent",border:"none",borderBottom:`1.5px solid ${C.border}`,padding:"8px 0",color:C.white,fontSize:14,outline:"none",boxSizing:"border-box"}}/>
      }
    </div>
  );
  ...
  return(<div>
    ...
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:20}}>
      <PgTit icon="🧾" title="Gastos" sub="Escanea boletas · OCR gratis"/>
    ...
    {delMes.length===0
      ?<div style={{textAlign:"center",padding:"50px 20px",background:"linear-gradient(135deg,#131F38,#0E1829)",border:"1px solid #1E3050",borderRadius:20}}>
        ...
        <div style={{fontSize:13,color:C.muted,marginBottom:20}}>Toca "+ Nuevo" o escanea una boleta con OCR gratis</div>
            ...
            <div style={{padding:"0 24px"}}>
              {ocr.imgUrl?(
                <div style={{position:"relative",marginBottom:20}}>
                  ...
                  {ocr.loading&&(
                    <div style={{position:"absolute",inset:0,background:"rgba(8,12,26,0.85)",borderRadius:16,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:12}}>
                      ...
                      <div style={{fontSize:13,color:C.white,fontWeight:600}}>Leyendo boleta... {ocr.progress}%</div>
                ...
                <div onClick={()=>fileRef.current.click()} style={{border:"2px dashed #2D4060",borderRadius:20,padding:"40px 20px",textAlign:"center",cursor:"pointer",marginBottom:20,background:"#060E1A",transition:"border-color .2s"}}>
                  ...
                  <div style={{fontSize:12,color:C.muted,marginBottom:16}}>OCR gratis — extrae datos automáticamente</div>
