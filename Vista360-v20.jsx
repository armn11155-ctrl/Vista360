...
function Contratos({contratos,setContratos,paneles,clientes,loading}){
  ...
  const guardar=async()=>{
    ...
    // pagado=true si el primer mes está marcado (adelanto)
    const pagado=pagosMeses[meses[0]?.key]||false;
    const payload={panel_id:form.panel_id,cliente_id:form.cliente_id,inicio:form.inicio,fin:form.fin,monto:Number(form.monto),pagado,pagosMeses};
    if(modal==="nuevo"){
      const [r]=await sb.post("contratos",payload);
      if(r) setContratos(p=>[...p,r]);
    } else {
      const [r]=await sb.patch("contratos",modal.id,payload);
      if(r) setContratos(p=>p.map(x=>x.id===modal.id?r:x));
    }
    setSaving(false); setModal(null);
  };
        ...
        <div style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:20,padding:26,width:"100%",maxWidth:540,maxHeight:"92vh",overflowY:"auto"}}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
            <F label="Fecha inicio *">
              <input type="date" value={form.inicio} onChange={e=>setForm(f=>({...f,inicio:e.target.value,pagosMeses:{}}))} style={inp}/>
            </F>
            <F label="Fecha fin *">
              <input type="date" value={form.fin} onChange={e=>setForm(f=>({...f,fin:e.target.value,pagosMeses:{}}))} style={inp}/>
            </F>
          </div>
