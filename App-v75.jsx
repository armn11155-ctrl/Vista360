...
function Dashboard({clientes,contratos,paneles,setTab}){
  ...
  return(<div style={{paddingBottom:40,display:"flex",flexDirection:"column",gap:16}}>
    ...
    {/* ── HERO CARD ── */}
    <div style={{marginBottom:16,padding:"22px 20px",background:"linear-gradient(135deg,#0A1535 0%,#0F1E52 40%,#152668 70%,#0D1A45 100%)",borderRadius:20,border:"1px solid #1E3370",position:"relative",overflow:"hidden",boxShadow:"0 8px 40px rgba(10,30,120,0.6)"}}>
      {/* Partículas */}
    ...
    {/* ── INGRESOS ── */}
    <div style={{marginBottom:16,padding:"20px 20px",background:"linear-gradient(135deg,#062818 0%,#082010 50%,#041208 100%)",borderRadius:20,border:"1px solid #174D30",boxShadow:"0 4px 24px rgba(16,185,129,0.18)"}}>
...
export default function App(){
  ...
  return(<>
    ...
    <style>{`*{box-sizing:border-box;-webkit-tap-highlight-color:transparent}html{background:#070C18;height:100%;height:-webkit-fill-available}body{margin:0;padding:0;background:#070C18;overflow:hidden;width:100%;height:100%;min-height:-webkit-fill-available;-webkit-text-size-adjust:100%}#root{width:100%;height:100%;min-height:-webkit-fill-available;display:flex;flex-direction:column;overflow:hidden;box-sizing:border-box}::-webkit-scrollbar{display:none}@keyframes spin{to{transform:rotate(360deg)}}@keyframes pulse{from{opacity:.2}to{opacity:.6}}`}</style>

    {splash&&<Splash done={()=>setSplash(false)}/>}
    {showConfig&&<ModalConfig onClose={()=>setShowConfig(false)}/>}

    <div style={{fontFamily:"'DM Sans',sans-serif",background:C.bg,flex:1,color:C.text,display:"flex",overflow:"hidden",alignItems:"stretch",position:"relative"}}>

      {/* ── SIDEBAR ── */}
      <aside style={{
        width:176,flexShrink:0,
        background:"linear-gradient(180deg,#0F1218 0%,#0A0D12 60%,#070C18 100%)",
      ...
      <div style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden"}}>
        <main style={{flex:1,overflowY:"auto",overflowX:"hidden",padding:"16px 16px 0",paddingBottom:"calc(16px + env(safe-area-inset-bottom))",WebkitOverflowScrolling:"touch",scrollBehavior:"smooth"}}>
