...
function Hoy({clientes,contratos,paneles,gastos,setTab}){
  ...
  <div style={{paddingBottom:24}}>
    ...
    {(()=>{
      // Lógica dinámica real según el estado actual
      ...
      const ingActualMes = contratos.filter(c=>{
        const ini=c.inicio?.slice(0,7); const fin=c.fin?.slice(0,7);
        return ini&&fin&&ini<=mesKey&&fin>=mesKey;
      ...
      let titulo="", mensaje="", colorVal=C.green, valor="";
      if(totalPaneles===0){
        titulo="Sin paneles registrados";
        mensaje="Agrega tu primer panel para comenzar a generar ingresos y ver proyecciones.";
        colorVal=C.muted; valor="$0/mes";
      } else if(ocupados===0){
        titulo="Ningún panel alquilado aún";
        mensaje=`Tienes ${totalPaneles} panel${totalPaneles>1?"es":""} disponible${totalPaneles>1?"s":""}. Asigna contratos para generar ingresos.`;
        colorVal=C.amber; valor=`${fmt(ingPotencialTotal)}/mes posible`;
      } else if(panelLibresMes.length>0){
        titulo=`${ocupPct}% de ocupación — podés crecer`;
        mensaje=`Estás generando ${fmt(ingActualMes)}/mes. Con los ${panelLibresMes.length} panel${panelLibresMes.length>1?"es":"" } libre${panelLibresMes.length>1?"s":""} podrías alcanzar`;
        colorVal=C.green; valor=`${fmt(ingConPaneles)}/mes`;
      } else if(contratosVencen30>0){
        titulo="100% ocupado — contratos por renovar";
        mensaje=`Excelente ocupación. Hay ${contratosVencen30} contrato${contratosVencen30>1?"s":""} que vence${contratosVencen30>1?"n":""} en 30 días. Contacta los clientes ahora.`;
        colorVal=C.amber; valor=`${fmt(ingActualMes)}/mes actual`;
      } else if(contratosPendientes>0){
        titulo=`${ocupPct}% ocupado — cobros pendientes`;
        mensaje=`Tienes ${contratosPendientes} pago${contratosPendientes>1?"s":""} pendiente${contratosPendientes>1?"s":""} este mes. El ingreso potencial del mes es:`;
        colorVal=C.amber; valor=`${fmt(ingActualMes)}/mes`;
      } else {
        titulo=`${ocupPct}% ocupado — todo en orden`;
        mensaje="Todos tus paneles están ocupados y los cobros están al día. ¡Excelente gestión!";
        colorVal=C.green; valor=`${fmt(ingActualMes)}/mes`;
      }
      ...
      return(
        <div style={{
          ...
          <div style={{flex:1,minWidth:0}}>
            ...
            <div style={{fontSize:26,fontWeight:900,color:colorVal,letterSpacing:"-1px",textShadow:`0 0 24px ${colorVal}55`,marginBottom:14}}>
              {valor}
            </div>
            <button
