  ...
  async uploadImagen(path, file) {
    const webpBlob = await new Promise((res, rej) => {
      ...
      img.onload = () => {
        ...
        // WebP 0.72 → ~20–35 KB, legible en app y en PDF descargado
    ...
    const sRef = storageRef(storage, webpPath);
    await uploadBytes(sRef, webpBlob, { contentType: "image/webp" });
...
function Gastos({gastos,setGastos}){
  ...
  const guardar=async()=>{
    ...
    if(ocr._file && !form.fotoUrl){
      try{
        const path = `gastos/${Date.now()}_${Math.random().toString(36).slice(2)}.jpg`;
        fotoUrl = await fb.uploadImagen(path, ocr._file);
      }catch(e){ console.warn("Storage upload failed:", e.message); }
    } else if(ocr.previewUrl && ocr.previewUrl.startsWith("data:") && !form.fotoUrl){
      try{
        const res  = await fetch(ocr.previewUrl);
        const blob = await res.blob();
        const path = `gastos/${Date.now()}_${Math.random().toString(36).slice(2)}.jpg`;
        fotoUrl = await fb.uploadImagen(path, blob);
      }catch(e){ console.warn("Storage upload failed:", e.message); }
    }
  ...
  return(<>
    ...
    <div style={{fontFamily:"'DM Sans',sans-serif",background:C.bg,height:"100dvh",overflowY:"auto",overflowX:"hidden",color:C.text,display:"flex",flexDirection:"column",WebkitOverflowScrolling:"touch"}}>
      <nav style={{background:"#0A0F1E",borderBottom:"2px solid rgba(255,255,255,0.08)",paddingLeft:16,paddingRight:16,paddingTop:"env(safe-area-inset-top)",display:"flex",alignItems:"center",minHeight:"calc(56px + env(safe-area-inset-top))",position:"sticky",top:0,zIndex:200,gap:2,overflowX:"auto",scrollbarWidth:"none",flexShrink:0}}>
        <style>{`::-webkit-scrollbar{display:none}@media(max-width:640px){.top-nav-inner{display:none!important}}`}</style>
        <div style={{marginRight:12,display:"flex",alignItems:"center",flexShrink:0}}>
          <Logo360 width={110}/>
        </div>
        <div className="top-nav-inner" style={{display:"flex",flex:1,alignItems:"center",gap:2,height:"100%",overflowX:"auto"}}>
        {tabs.map(t=>(
          <button key={t.id} onClick={()=>setTab(t.id)} style={{padding:"0 12px",height:"100%",display:"flex",alignItems:"center",gap:6,fontSize:13,fontWeight:tab===t.id?700:400,color:tab===t.id?"#FFFFFF":"rgba(255,255,255,0.45)",background:"none",border:"none",borderBottom:tab===t.id?"2px solid #3B82F6":"2px solid transparent",cursor:"pointer",whiteSpace:"nowrap",flexShrink:0}}>
      ...
      <style>{`@media(min-width:641px){.bottom-nav-mobile{display:none!important}}`}</style>
      <nav className="bottom-nav-mobile" style={{position:"fixed",bottom:0,left:0,right:0,zIndex:300,background:"#0A0F1E",borderTop:"1px solid rgba(255,255,255,0.12)",display:"flex",overflowX:"auto",scrollbarWidth:"none",WebkitOverflowScrolling:"touch",paddingBottom:"env(safe-area-inset-bottom)"}}>
        <style>{`.bottom-nav-mobile::-webkit-scrollbar{display:none}`}</style>
        {tabs.map(t=>(
          <button key={t.id} onClick={()=>setTab(t.id)}
            style={{flex:"0 0 auto",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:2,padding:"8px 14px",minWidth:60,background:"none",border:"none",cursor:"pointer",
              color:tab===t.id?"#FFFFFF":"rgba(255,255,255,0.4)",
              borderTop:tab===t.id?"2px solid #3B82F6":"2px solid transparent"}}>
            <span style={{fontSize:20}}>{t.icon}</span>
            <span style={{fontSize:10,fontWeight:tab===t.id?700:500,whiteSpace:"nowrap"}}>{t.label}</span>
          </button>
        ))}
      </nav>

      <main style={{flex:1,padding:"20px 16px",paddingBottom:"calc(72px + env(safe-area-inset-bottom))",maxWidth:1240,width:"100%",margin:"0 auto",boxSizing:"border-box"}}>
        {error&&(
