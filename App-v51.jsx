...
// ══════════════════════════════════════════════════════════════════
// 🔥 FIREBASE — Firestore + Storage (migrado desde Supabase)
// ══════════════════════════════════════════════════════════════════
...
// ── CLIENTE FIREBASE — misma interfaz que tenía Supabase ─────────
const fb = {
  // GET — lista todos los docs de una colección ordenados por fecha
  async get(col) {
    try {
      ...
      // Si no hay índice aún, traer sin orden
      try {
  ...
  // UPLOAD — sube imagen a Storage y retorna URL pública
  // Genera miniatura 400px ~15-20KB para preview
  async uploadImagen(path, file, calidad = 0.4) {
    // Comprimir a miniatura 400px ancho, calidad 40%
    const mini = await new Promise((res, rej) => {
      const img = new Image();
      ...
      img.onload = () => {
        ...
        c.toBlob(b => { URL.revokeObjectURL(url); b ? res(b) : rej(new Error("Blob vacío")); },
          "image/jpeg", calidad);
      };
    ...
    await uploadBytes(sRef, mini, { contentType: "image/jpeg" });
    ...
    <div style={{position:"fixed",inset:0,background:"#00000092",zIndex:400,display:"flex",alignItems:"center",justifyContent:"center",padding:20,backdropFilter:"blur(6px)"}}
      ...
      <div style={{background:"#111929",border:"1px solid #18253D",borderRadius:20,padding:26,width:"100%",maxWidth:480}}>
        ...
        <div style={{fontSize:13,color:"#64748B",lineHeight:1.8,marginBottom:16}}>
          La app usa <strong style={{color:"#F59E0B"}}>Firebase Firestore</strong> como base de datos y <strong style={{color:"#F59E0B"}}>Firebase Storage</strong> para fotos de boletas.<br/>
...
// ── FIREBASE — Almacenamiento y estado de la base de datos ───────
function Firebase({contratos,paneles,gastos,clientes}){
  const [fbStats,setFbStats]=useState({loading:true,error:null,docs:0,reads:0,storage:0,dbSize:0});
  const [notifPerm,setNotifPerm]=useState(typeof Notification!=="undefined"?Notification.permission:"default");
  const [notifLoading,setNotifLoading]=useState(false);

  useEffect(()=>{
    // Calcular estadísticas reales desde los datos cargados
    const totalDocs=contratos.length+paneles.length+clientes.length+gastos.length;
    // Estimado de tamaño: cada doc ~0.5KB promedio
    const estimadoDB=(totalDocs*0.5)/1024; // en MB
    // Fotos: cada panel con foto base64 ~50KB promedio
    const fotosPanel=paneles.filter(p=>p.foto&&p.foto.length>10).length;
    const estimadoStorage=(fotosPanel*50)/1024; // en MB
    setFbStats({loading:false,error:null,docs:totalDocs,reads:totalDocs,storage:estimadoStorage,dbSize:estimadoDB});
  },[contratos,paneles,clientes,gastos]);
  ...
  const activarNotif=async()=>{
    ...
    if(p==="granted"){
      ...
      try{ localStorage.removeItem("v360_notif"); }catch(e){}
    } else if(p==="denied"){
      alert("Bloqueaste las notificaciones.\nPara activarlas: Ajustes del navegador → Notificaciones → Permitir para este sitio.");
    }
  };

  // Plan gratuito de Firebase Firestore
  const LIMITES={
    dbMB:1024,      // 1 GB gratis Firestore
    storageMB:5120, // 5 GB gratis Storage
    reads:50000,    // lecturas/día
  };

  const pctDB=Math.min(Math.round((fbStats.dbSize/LIMITES.dbMB)*100),100);
  const pctStorage=Math.min(Math.round((fbStats.storage/LIMITES.storageMB)*100),100);
  const pctReads=Math.min(Math.round((fbStats.reads/LIMITES.reads)*100),100);

  const BarraUso=({pct,color,label,usado,total,unidad})=>(
    <div style={{marginBottom:20}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}>
        <span style={{fontSize:13,fontWeight:700,color:C.white}}>{label}</span>
        <span style={{fontSize:12,color:C.muted}}>{usado} / {total} {unidad}</span>
      </div>
      <div style={{height:10,background:C.border,borderRadius:6,overflow:"hidden"}}>
        <div style={{height:"100%",width:pct+"%",background:"linear-gradient(90deg,"+color+","+color+"BB)",borderRadius:6,transition:"width 1s ease",boxShadow:"0 0 8px "+color+"66"}}/>
      </div>
      <div style={{display:"flex",justifyContent:"space-between",marginTop:5}}>
        <span style={{fontSize:11,color:pct>80?C.red:pct>60?C.amber:C.green,fontWeight:600}}>{pct}% usado</span>
        <span style={{fontSize:11,color:C.muted}}>{Math.round((1-pct/100)*parseFloat(total)*10)/10} {unidad} libres</span>
      </div>
    </div>
  );

  return(<div>
    <PgTit icon="🔥" title="Firebase" sub="Estado del almacenamiento y base de datos en la nube"/>

    {/* ── ESTADO CONEXIÓN ── */}
    <div style={{marginBottom:16,padding:"14px 18px",background:"linear-gradient(135deg,#0A1628,#0D1E38)",borderRadius:16,border:"1px solid #1A3A5C",display:"flex",alignItems:"center",justifyContent:"space-between"}}>
      <div style={{display:"flex",alignItems:"center",gap:12}}>
        <div style={{width:42,height:42,borderRadius:12,background:"linear-gradient(135deg,#FF6B00,#FF9500)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:20,boxShadow:"0 4px 14px rgba(255,107,0,0.4)"}}>🔥</div>
        <div>
          <div style={{fontSize:14,fontWeight:800,color:C.white}}>Firebase Firestore</div>
          <div style={{fontSize:12,color:C.muted,marginTop:2}}>Plan Spark (Gratuito)</div>
        </div>
      </div>
      <div style={{display:"flex",alignItems:"center",gap:8}}>
        <div style={{width:8,height:8,borderRadius:"50%",background:C.green,boxShadow:"0 0 6px "+C.green,animation:"pulse 1.5s infinite"}}/>
        <span style={{fontSize:12,fontWeight:700,color:C.green}}>Conectado</span>
      </div>
    </div>
    ...
    <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:10,marginBottom:16}}>
      {[
        {label:"Documentos",value:fbStats.docs,icon:"📄",color:C.accent},
        {label:"Paneles",value:paneles.length,icon:"🖥️",color:C.cyan},
        {label:"Contratos",value:contratos.length,icon:"📋",color:C.green},
        {label:"Clientes",value:clientes.length,icon:"👥",color:C.purple},
      ].map((k,i)=>(
        <div key={i} style={{background:C.card,border:"1px solid "+C.border,borderRadius:14,padding:"12px 10px",textAlign:"center"}}>
          <div style={{fontSize:20,marginBottom:4}}>{k.icon}</div>
          <div style={{fontSize:22,fontWeight:900,color:k.color}}>{k.value}</div>
          <div style={{fontSize:10,color:C.muted,marginTop:2,fontWeight:600}}>{k.label}</div>
        </div>
      ))}
    </div>
    ...
    <Card style={{marginBottom:14}}>
      <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:18}}>
        <div style={{width:28,height:28,borderRadius:8,background:"linear-gradient(135deg,#1E3A70,#2563EB)",display:"flex",alignItems:"center",justifyContent:"center"}}>
          <svg width={14} height={14} viewBox="0 0 16 16" fill="none">
            <rect x={1} y={4} width={14} height={10} rx={2} stroke="white" strokeWidth={1.3} fill="none"/>
            <path d="M4,4 L4,2 L12,2 L12,4" stroke="white" strokeWidth={1.3} strokeLinecap="round"/>
            <rect x={5} y={8} width={6} height={2} rx={1} fill="white"/>
          </svg>
        </div>
        <span style={{fontSize:12,fontWeight:700,color:C.muted,textTransform:"uppercase",letterSpacing:1.2}}>Uso de Almacenamiento</span>
      </div>

      <BarraUso
        label="📦 Base de Datos (Firestore)"
        pct={pctDB}
        color="#2563EB"
        usado={(fbStats.dbSize<1?Math.round(fbStats.dbSize*1024)+"KB":fbStats.dbSize.toFixed(2)+"MB")}
        total="1"
        unidad="GB"
      />
      <BarraUso
        label="🖼️ Fotos y Archivos (Storage)"
        pct={pctStorage}
        color="#8B5CF6"
        usado={(fbStats.storage<1?Math.round(fbStats.storage*1024)+"KB":fbStats.storage.toFixed(2)+"MB")}
        total="5"
        unidad="GB"
      />
      <BarraUso
        ...
        pct={pctReads}
        color="#10B981"
        usado={fbStats.reads.toLocaleString()}
        total="50,000"
        unidad="/día"
      />

      {/* Resumen espacio libre */}
      <div style={{marginTop:8,padding:"12px 16px",background:"linear-gradient(135deg,#065F46,#10B981)",borderRadius:12,display:"flex",alignItems:"center",justifyContent:"space-between"}}>
        <div>
          <div style={{fontSize:11,color:"rgba(255,255,255,0.75)",fontWeight:600,marginBottom:2}}>ESPACIO LIBRE TOTAL</div>
          <div style={{fontSize:20,fontWeight:900,color:"#fff"}}>{(6-(fbStats.dbSize/1024+fbStats.storage/1024)).toFixed(2)} GB disponibles</div>
        </div>
        <div style={{fontSize:32}}>💾</div>
      </div>
    </Card>
    ...
    {/* ── INFO PLAN GRATUITO ── */}
    <Card style={{marginBottom:14}}>
      <div style={{fontSize:12,fontWeight:700,color:C.muted,textTransform:"uppercase",letterSpacing:1.2,marginBottom:14}}>Plan Spark — Incluye gratis</div>
      {[
        ["🗄️","Firestore","1 GB almacenamiento + 50K lecturas/día"],
        ["🖼️","Storage","5 GB para fotos e imágenes"],
    ...
    {/* ── ACTIVAR ALERTAS ── */}
    <Card style={{background:"linear-gradient(145deg,#14123A,#0F0E2C)",border:"1px solid #242060"}}>
      <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:16}}>
        <div style={{width:36,height:36,borderRadius:10,background:"linear-gradient(135deg,#4C1D95,#7C6FD4)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:18}}>🔔</div>
        <div>
          <div style={{fontSize:14,fontWeight:800,color:C.white}}>Notificaciones de Alerta</div>
          <div style={{fontSize:12,color:C.muted,marginTop:1}}>Recibe avisos de contratos por vencer</div>
        </div>
      </div>

      {notifPerm==="granted"?(
        <div style={{display:"flex",alignItems:"center",gap:10,padding:"12px 16px",background:C.green+"15",border:"1px solid "+C.green+"44",borderRadius:12}}>
          <div style={{width:10,height:10,borderRadius:"50%",background:C.green,animation:"pulse 1.5s infinite"}}/>
          <span style={{fontSize:13,fontWeight:700,color:C.green}}>Notificaciones activas · 30d y 15d antes del vencimiento</span>
        </div>
      ):notifPerm==="denied"?(
        <div style={{padding:"12px 16px",background:C.red+"15",border:"1px solid "+C.red+"44",borderRadius:12}}>
          <div style={{fontSize:13,fontWeight:700,color:C.red,marginBottom:4}}>🚫 Notificaciones bloqueadas</div>
          <div style={{fontSize:12,color:C.muted}}>Ve a Ajustes del navegador → Notificaciones → Permitir para este sitio</div>
        </div>
      ):(
        <div>
          <div style={{fontSize:13,color:C.muted,marginBottom:14,lineHeight:1.6}}>
            Activa las alertas para recibir notificaciones automáticas cuando un contrato esté a <strong style={{color:C.amber}}>30 días</strong> o <strong style={{color:C.red}}>15 días</strong> de vencer.
          </div>
          <div style={{padding:"10px 14px",background:C.amber+"12",border:"1px solid "+C.amber+"33",borderRadius:10,fontSize:12,color:C.amber,marginBottom:14}}>
            💡 En iPhone: primero agrega la app a tu pantalla de inicio<br/>
            Safari → Compartir → Agregar a pantalla de inicio → luego activa
          </div>
          <button
            onClick={activarNotif}
            disabled={notifLoading}
            style={{width:"100%",padding:"14px",background:"linear-gradient(135deg,#7C6FD4,#4C1D95)",border:"none",borderRadius:13,color:"#fff",fontWeight:800,fontSize:15,cursor:"pointer",boxShadow:"0 4px 18px rgba(124,111,212,0.45)",display:"flex",alignItems:"center",justifyContent:"center",gap:10,opacity:notifLoading?0.7:1}}>
            {notifLoading?<><div style={{width:18,height:18,border:"2px solid rgba(255,255,255,0.3)",borderTopColor:"#fff",borderRadius:"50%",animation:"spin .7s linear infinite"}}/> Activando...</>:<>🔔 Activar alertas en este dispositivo</>}
          </button>
        </div>
      )}
    </Card>
...
function Gastos({gastos,setGastos}){
  ...
  const VISION_KEY = (typeof import.meta !== "undefined" && import.meta.env?.VITE_GOOGLE_VISION_KEY) || "";
  ...
  const escanear=async(file)=>{
    ...
    setOcr({loading:true,progress:5,fase:"Preparando imagen...",text:"",imgUrl,previewUrl:""});

    try{
      // ── Fase 1: Guardar URL original para mostrar ─────────────
      setOcr(o=>({...o,progress:12,fase:"Preparando imagen..."}));
      // Mantener imagen original para mostrar — solo preprocesar internamente para OCR
      const originalUrl=URL.createObjectURL(file);
      let imagenParaOCR=file;
      try{
        const resultado=await preprocesarImagen(file);
        imagenParaOCR=resultado.blob; // solo usamos el blob procesado para OCR
        // NO usamos resultado.previewUrl — mostramos la original
        setOcr(o=>({...o,previewUrl:originalUrl,progress:28,fase:"Imagen lista ✓ — Enviando a Google Vision..."}));
      }catch(_){
        setOcr(o=>({...o,previewUrl:originalUrl,progress:28,fase:"Enviando a Google Vision..."}));
      }

      // ── Fase 2: Google Vision OCR ─────────────────────────────
      setOcr(o=>({...o,progress:40,fase:"Analizando con Google Cloud Vision..."}));
      const textoRaw = await ocr_con_vision(file);
      setOcr(o=>({...o,progress:85,fase:"Texto extraído ✓ — Procesando datos..."}));

      if(!textoRaw||textoRaw.trim().length<8)
        throw new Error("No se detectó texto. Verifica iluminación y enfoque.");

      // ── Fase 3: Extracción inteligente con RegEx ──────────────
      setOcr(o=>({...o,progress:90,fase:"Extrayendo datos estructurados..."}));
      const datosExtraidos=extraerDatosInteligente(textoRaw);

      setOcr(o=>({...o,loading:false,progress:100,fase:"✓ Completado con Google Vision",text:textoRaw,imgUrl,previewUrl}));
  ...
  const guardar=async()=>{
    ...
    // ── Subir miniatura a Firebase Storage si hay imagen nueva ──
    let fotoUrl = form.fotoUrl || "";
    if(ocr.previewUrl && ocr.previewUrl.startsWith("data:") && !form.fotoUrl){
      try{
        // Convertir dataURL a Blob
        const res   = await fetch(ocr.previewUrl);
        const blob  = await res.blob();
        const path  = `gastos/${Date.now()}_${Math.random().toString(36).slice(2)}.jpg`;
        fotoUrl     = await fb.uploadImagen(path, blob, 0.4);
...
export default function App(){
  ...
  const tabs=[{id:"dashboard",icon:"⚡",label:"Dashboard"},{id:"mapa",icon:"🗺️",label:"Mapa"},{id:"paneles",icon:"🖥️",label:"Paneles"},{id:"contratos",icon:"📋",label:"Contratos"},{id:"crm",icon:"👥",label:"Clientes"},{id:"resultados",icon:"📈",label:"Resultados"},{id:"gastos",icon:"🧾",label:"Gastos"},{id:"firebase",icon:"🔥",label:"Firebase"}];
  ...
  return(<>
    ...
    <style>{`*{box-sizing:border-box;-webkit-tap-highlight-color:transparent}html,body{margin:0;padding:0;background:#070C18;overflow:hidden;height:100%;-webkit-text-size-adjust:100%}::-webkit-scrollbar{display:none}@keyframes spin{to{transform:rotate(360deg)}}@keyframes pulse{from{opacity:.2}to{opacity:.6}}`}</style>
    ...
    <div style={{fontFamily:"'DM Sans',sans-serif",background:C.bg,height:"100dvh",overflowY:"auto",overflowX:"hidden",color:C.text,display:"flex",flexDirection:"column",WebkitOverflowScrolling:"touch"}}>
      <nav style={{background:"#070C18",borderBottom:"1px solid rgba(255,255,255,0.06)",paddingLeft:16,paddingRight:16,paddingTop:"env(safe-area-inset-top)",display:"flex",alignItems:"center",minHeight:"calc(56px + env(safe-area-inset-top))",position:"sticky",top:0,zIndex:200,gap:2,overflowX:"auto",scrollbarWidth:"none",flexShrink:0,isolation:"isolate",backdropFilter:"none",WebkitBackdropFilter:"none"}}>
        ...
        <div style={{marginLeft:"auto",display:"flex",alignItems:"center",gap:8,flexShrink:0}}>
          ...
          <span style={{fontSize:11,color:C.muted}}>{error?"Error Firebase":loading?"Conectando...":"Firebase ✓"}</span>
      ...
      <main style={{flex:1,padding:"20px 16px",maxWidth:1240,width:"100%",margin:"0 auto",boxSizing:"border-box"}}>
        {error&&(
          <div style={{background:"#1E293B",border:`1px solid ${C.amber}55`,borderRadius:14,padding:20,marginBottom:24,display:"flex",gap:16,alignItems:"flex-start"}}>
            ...
            <div>
              <div style={{fontWeight:700,color:C.amber,fontSize:15,marginBottom:8}}>La app no puede conectarse a Firebase desde aquí</div>
              <div style={{fontSize:13,color:C.muted,lineHeight:1.8}}>
                El preview de Claude bloquea conexiones a servidores externos por seguridad.<br/>
                <strong style={{color:C.white}}>Esto es normal</strong> — el código está 100% listo con Firebase. Para usarlo de verdad:
              </div>
              <div style={{marginTop:12,display:"flex",flexDirection:"column",gap:6}}>
                {[["1","Ve a","vercel.com","y crea una cuenta gratis"],["2","Sube el archivo .jsx a un proyecto Next.js o Vite"],["3","Tu app estará en línea en menos de 5 min, conectada a Firebase"]].map(([n,a,b,c])=>(
        ...
        {tab==="firebase"   &&<Firebase   contratos={contratos} paneles={paneles} gastos={gastos} clientes={clientes}/>}
      </main>
    </div>
  </>);
}
