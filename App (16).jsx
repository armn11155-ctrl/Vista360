...
function Dashboard({clientes,contratos,paneles,setTab}){
  ...
  return(<div style={{paddingBottom:32,display:"flex",flexDirection:"column",gap:0}}>
    ...
    {(()=>{
      ...
      const ingresosMes=contratos.filter(c=>c.pagado&&c.inicio&&c.inicio.startsWith(mesActualKey)).reduce((a,c)=>a+Number(c.monto||0),0);
      // gastos los obtenemos por prop si existieran, pero Dashboard no los recibe — usamos estimación basada en contratos activos
      const gastosEstimados=contratos.filter(c=>dias(c.fin)>0).length*150; // placeholder simbólico cuando no hay datos
      const margen=ingresosMes-gastosEstimados;
      const margenPct=ingresosMes>0?Math.round((margen/ingresosMes)*100):0;
      const margenColor=margenPct>=50?"#10B981":margenPct>=20?"#F5A623":"#F04747";
      const margenStatus=margenPct>=50?"Excelente":margenPct>=20?"Saludable":margenPct>=0?"Ajustado":"En pérdida";

      return(
        <div style={{padding:"20px 22px",background:"linear-gradient(145deg,#0C1F1A 0%,#08171A 50%,#040E12 100%)",borderRadius:22,border:"1px solid rgba(16,185,129,0.22)",boxShadow:"0 4px 32px rgba(0,0,0,0.55),inset 0 1px 0 rgba(110,231,183,0.05)",position:"relative",overflow:"hidden"}}>
          <div style={{position:"absolute",top:"-30%",right:"-10%",width:"60%",height:"100%",background:"radial-gradient(ellipse,rgba(16,185,129,0.15) 0%,transparent 65%)",pointerEvents:"none"}}/>
          <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:18,position:"relative"}}>
            <div style={{display:"flex",alignItems:"center",gap:10}}>
              <div style={{width:34,height:34,borderRadius:10,background:"linear-gradient(135deg,#065F46,#10B981)",display:"flex",alignItems:"center",justifyContent:"center",boxShadow:"0 0 14px rgba(16,185,129,0.35)"}}>
                <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#6EE7B7" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round"><path d="M22 12h-4l-3 9L9 3l-3 9H2"/></svg>
              </div>
              <div>
                <div style={{fontSize:11,fontWeight:800,color:"#6EE7B7",textTransform:"uppercase",letterSpacing:1.4}}>Salud financiera</div>
                <div style={{fontSize:11,color:"#94A3B8",marginTop:2}}>Margen del mes en curso</div>
              </div>
            </div>
            <div style={{padding:"5px 11px",borderRadius:99,background:`${margenColor}20`,border:`1px solid ${margenColor}55`,fontSize:10,fontWeight:800,color:margenColor,textTransform:"uppercase",letterSpacing:0.8}}>
              {margenStatus}
            </div>
          </div>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:14,position:"relative"}}>
            {/* Ingresos */}
            <div style={{background:"rgba(16,185,129,0.06)",border:"1px solid rgba(16,185,129,0.2)",borderRadius:14,padding:"14px 12px"}}>
              <div style={{fontSize:9,fontWeight:700,color:"#6EE7B7",textTransform:"uppercase",letterSpacing:1.2,marginBottom:6,opacity:0.9}}>Ingresos</div>
              <div style={{fontSize:18,fontWeight:900,color:"#10B981",letterSpacing:"-0.5px",lineHeight:1.1,fontFamily:"monospace"}}>{fmt(ingresosMes)}</div>
              <div style={{fontSize:10,color:"#94A3B8",marginTop:4}}>cobrados</div>
            </div>

            {/* Gastos estimados */}
            <div style={{background:"rgba(245,166,35,0.06)",border:"1px solid rgba(245,166,35,0.22)",borderRadius:14,padding:"14px 12px"}}>
              <div style={{fontSize:9,fontWeight:700,color:"#FCD34D",textTransform:"uppercase",letterSpacing:1.2,marginBottom:6,opacity:0.9}}>Gastos</div>
              <div style={{fontSize:18,fontWeight:900,color:"#F5A623",letterSpacing:"-0.5px",lineHeight:1.1,fontFamily:"monospace"}}>{fmt(gastosEstimados)}</div>
              <div style={{fontSize:10,color:"#94A3B8",marginTop:4}}>operativos</div>
            </div>

            {/* Margen */}
            <div style={{background:`${margenColor}10`,border:`1px solid ${margenColor}40`,borderRadius:14,padding:"14px 12px",boxShadow:`inset 0 0 22px ${margenColor}11`}}>
              <div style={{fontSize:9,fontWeight:700,color:margenColor,textTransform:"uppercase",letterSpacing:1.2,marginBottom:6,opacity:0.95}}>Margen neto</div>
              <div style={{fontSize:18,fontWeight:900,color:margenColor,letterSpacing:"-0.5px",lineHeight:1.1,fontFamily:"monospace",textShadow:`0 0 12px ${margenColor}66`}}>{fmt(margen)}</div>
              <div style={{fontSize:10,color:"#94A3B8",marginTop:4}}>{margenPct}% rentabilidad</div>
            </div>
          </div>
        </div>
      );
    ...
    {/* ── 🎯 TOP PERFORMANCE ── */}
    {(()=>{
      // Mejor cliente del año
      const ingresosPorCliente={};
      contratos.filter(c=>c.pagado).forEach(c=>{
        const cli=clientes.find(cl=>cl.id===c.cliente_id);
        if(!cli) return;
        const k=cli.id;
        if(!ingresosPorCliente[k]) ingresosPorCliente[k]={empresa:cli.empresa,total:0,count:0};
        ingresosPorCliente[k].total+=Number(c.monto||0);
        ingresosPorCliente[k].count++;
      });
      const topCliente=Object.values(ingresosPorCliente).sort((a,b)=>b.total-a.total)[0];

      // Panel estrella (más contratos / más generados)
      const ingresosPorPanel={};
      contratos.filter(c=>c.pagado).forEach(c=>{
        const pan=paneles.find(p=>p.id===c.panel_id);
        if(!pan) return;
        const k=pan.id;
        if(!ingresosPorPanel[k]) ingresosPorPanel[k]={nombre:pan.nombre,total:0,count:0};
        ingresosPorPanel[k].total+=Number(c.monto||0);
        ingresosPorPanel[k].count++;
      });
      const topPanel=Object.values(ingresosPorPanel).sort((a,b)=>b.total-a.total)[0];

      return(
        <div style={{marginTop:14,padding:"20px 22px",background:"linear-gradient(145deg,#1A0F35 0%,#150A2E 50%,#0C0620 100%)",borderRadius:22,border:"1px solid rgba(167,139,250,0.22)",boxShadow:"0 4px 32px rgba(0,0,0,0.55),inset 0 1px 0 rgba(196,181,253,0.06)",position:"relative",overflow:"hidden"}}>
          <div style={{position:"absolute",top:"-25%",left:"-15%",width:"55%",height:"100%",background:"radial-gradient(ellipse,rgba(139,92,246,0.18) 0%,transparent 65%)",pointerEvents:"none"}}/>
          <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:18,position:"relative"}}>
            <div style={{width:34,height:34,borderRadius:10,background:"linear-gradient(135deg,#5B21B6,#A78BFA)",display:"flex",alignItems:"center",justifyContent:"center",boxShadow:"0 0 14px rgba(167,139,250,0.4)"}}>
              <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#DDD6FE" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round"><polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2"/></svg>
            </div>
            <div>
              <div style={{fontSize:11,fontWeight:800,color:"#C4B5FD",textTransform:"uppercase",letterSpacing:1.4}}>Top performance</div>
              <div style={{fontSize:11,color:"#94A3B8",marginTop:2}}>Tu cliente y panel estrella</div>
            </div>
          </div>

          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,position:"relative"}}>
            {/* Top Cliente */}
            <div style={{background:"rgba(139,92,246,0.07)",border:"1px solid rgba(139,92,246,0.25)",borderRadius:14,padding:"14px"}}>
              <div style={{display:"flex",alignItems:"center",gap:7,marginBottom:8}}>
                <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="#A78BFA" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg>
                <span style={{fontSize:9,fontWeight:700,color:"#C4B5FD",textTransform:"uppercase",letterSpacing:1.2,opacity:0.9}}>Mejor cliente</span>
              </div>
              {topCliente?(
                <>
                  <div style={{fontSize:14,fontWeight:800,color:"#F5F3FF",letterSpacing:"-0.3px",marginBottom:4,lineHeight:1.2}}>{topCliente.empresa}</div>
                  <div style={{fontSize:16,fontWeight:900,color:"#A78BFA",fontFamily:"monospace",textShadow:"0 0 10px rgba(167,139,250,0.5)"}}>{fmt(topCliente.total)}</div>
                  <div style={{fontSize:10,color:"#94A3B8",marginTop:3}}>{topCliente.count} contrato{topCliente.count!==1?"s":""} cobrado{topCliente.count!==1?"s":""}</div>
                </>
              ):(
                <div style={{fontSize:11,color:"#64748B",fontStyle:"italic",padding:"6px 0"}}>Sin contratos cobrados aún</div>
              )}
            </div>

            {/* Panel estrella */}
            <div style={{background:"rgba(59,130,246,0.07)",border:"1px solid rgba(59,130,246,0.25)",borderRadius:14,padding:"14px"}}>
              <div style={{display:"flex",alignItems:"center",gap:7,marginBottom:8}}>
                <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="#60A5FA" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round"><rect x="2" y="3" width="20" height="14" rx="2"/><line x1="8" y1="21" x2="16" y2="21"/><line x1="12" y1="17" x2="12" y2="21"/></svg>
                <span style={{fontSize:9,fontWeight:700,color:"#93C5FD",textTransform:"uppercase",letterSpacing:1.2,opacity:0.9}}>Panel estrella</span>
              </div>
              {topPanel?(
                <>
                  <div style={{fontSize:14,fontWeight:800,color:"#EFF6FF",letterSpacing:"-0.3px",marginBottom:4,lineHeight:1.2}}>{topPanel.nombre}</div>
                  <div style={{fontSize:16,fontWeight:900,color:"#60A5FA",fontFamily:"monospace",textShadow:"0 0 10px rgba(96,165,250,0.5)"}}>{fmt(topPanel.total)}</div>
                  <div style={{fontSize:10,color:"#94A3B8",marginTop:3}}>{topPanel.count} contrato{topPanel.count!==1?"s":""} generado{topPanel.count!==1?"s":""}</div>
                </>
              ):(
                <div style={{fontSize:11,color:"#64748B",fontStyle:"italic",padding:"6px 0"}}>Sin datos de paneles aún</div>
              )}
            </div>
          </div>
        </div>
  ...
  return (
    <div style={{ borderRadius: 24, overflow: "hidden", background: "#0A0B11", position: "relative", minHeight: 230, marginBottom: 20, boxShadow: "0 8px 32px rgba(0,0,0,0.18)" }}>
      ...
      <div style={{ position: "relative", zIndex: 2, padding: "26px 24px 26px", maxWidth: "72%" }}>
        <div style={{ fontSize: 11, fontWeight: 700, color: "#5A8BFF", letterSpacing: 2, marginBottom: 14, textTransform: "uppercase" }}>Resumen de hoy</div>
        <div style={{ fontSize: 34, fontWeight: 800, color: "#FFFFFF", lineHeight: 1.05, letterSpacing: "-0.025em", marginBottom: 14 }}>Todo bajo<br />control.</div>
