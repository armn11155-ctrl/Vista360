...
// ══════════════════════════════════════════════════════════════════
// 🔥 FIREBASE — cargado desde CDN (compatible con Vercel JSX)
// ══════════════════════════════════════════════════════════════════
...
// Carga Firebase compat SDK (scripts normales, no modules)
let _fbCache = null;
async function getFirebase() {
  if (_fbCache) return _fbCache;
  const load = (url) => new Promise((res, rej) => {
    if (document.querySelector(`script[data-fb="${url}"]`)) {
      // ya está en DOM, esperar a que cargue o ya cargó
      const existing = document.querySelector(`script[data-fb="${url}"]`);
      if (existing._loaded) return res();
      existing.addEventListener("load", res);
      existing.addEventListener("error", rej);
      return;
    }
    const sc = document.createElement("script");
    sc.src = url; sc.setAttribute("data-fb", url);
    sc.onload = () => { sc._loaded = true; res(); };
    sc.onerror = rej;
  ...
  await load("https://www.gstatic.com/firebasejs/10.12.0/firebase-storage-compat.js");
...
// ── CLIENTE FIREBASE — misma interfaz, usa compat SDK desde CDN ──
const fb = {
  async get(col) {
    try {
  ...
  async post(col, body) {
    const { db } = await getFirebase();
    const payload = { ...body, createdAt: window.firebase.firestore.FieldValue.serverTimestamp() };
    const ref = await db.collection(col).add(payload);
    return [{ ...body, id: ref.id }];
  },
  async patch(col, id, body) {
    const { db } = await getFirebase();
    await db.collection(col).doc(id).update(body);
    return [{ ...body, id }];
  },
  async del(col, id) {
    const { db } = await getFirebase();
  ...
  async uploadImagen(path, file, calidad = 0.4) {
    ...
    await sRef.put(mini, { contentType: "image/jpeg" });
...
function Firebase({contratos,paneles,clientes,gastos}){
  ...
  const fmtBytes=(b)=>{ if(b<1024) return b+"B"; if(b<1048576) return (b/1024).toFixed(1)+"KB"; return (b/1048576).toFixed(2)+"MB"; };
  const pct=(b)=>Math.min(100,Math.round(b/5368709120*100*1000)/1000); // vs 5GB limit

  // Supabase free tier: 500MB DB, 1GB storage
  const DB_LIMIT=524288000; // 500MB
  const dbPct=Math.min(100,Math.round(dbStats.total/DB_LIMIT*100*100)/100);
  ...
  return(<div>
    <PgTit icon="🔥" title="Firebase & Alertas" sub="Almacenamiento, base de datos y notificaciones"/>

    {/* ── TARJETA USO SUPABASE ── */}
    <Card style={{marginBottom:16}}>
      <SecTit ch="📦 Uso de Base de Datos — Supabase Free"/>
      {dbStats.loading
        ...
        :<div>
          ...
          <div style={{marginBottom:16}}>
            <div style={{display:"flex",justifyContent:"space-between",marginBottom:6}}>
              ...
              <span style={{fontSize:13,fontWeight:800,color:dbPct>80?C.red:dbPct>50?C.amber:C.green}}>{fmtBytes(dbStats.total)} <span style={{fontSize:11,color:C.muted,fontWeight:400}}>de 500 MB</span></span>
            </div>
          ...
          <div style={{marginTop:14,padding:"10px 14px",background:"#0EA5E918",border:"1px solid #0EA5E944",borderRadius:10,display:"flex",gap:10,alignItems:"flex-start"}}>
            ...
            <div style={{fontSize:11,color:C.muted,lineHeight:1.6}}>
              <strong style={{color:C.white}}>Plan gratuito Supabase:</strong> 500 MB base de datos · 1 GB storage de archivos · 50,000 filas máx. Tu uso actual es mínimo y no requiere upgrade.
            </div>
          </div>
        </div>
      }
...
function Gastos({gastos,setGastos}){
  ...
  const VISION_KEY = ""; // API key configurada si se necesita
...
export default function App(){
  ...
  return(<>
    ...
    <div style={{fontFamily:"'DM Sans',sans-serif",background:C.bg,height:"100dvh",overflowY:"auto",overflowX:"hidden",color:C.text,display:"flex",flexDirection:"column",WebkitOverflowScrolling:"touch"}}>
      <div style={{background:"#0A0F1E",height:"env(safe-area-inset-top)",position:"sticky",top:0,zIndex:201,flexShrink:0}}/>
      <nav style={{background:"#0A0F1E",borderBottom:"2px solid rgba(255,255,255,0.08)",paddingLeft:16,paddingRight:16,paddingTop:0,display:"flex",alignItems:"center",minHeight:"56px",position:"sticky",top:0,zIndex:200,gap:2,overflowX:"auto",scrollbarWidth:"none",flexShrink:0,isolation:"isolate",backdropFilter:"none",WebkitBackdropFilter:"none"}}>
        <style>{`::-webkit-scrollbar{display:none}`}</style>
        <div style={{marginRight:20,display:"flex",alignItems:"center",flexShrink:0}}>
          <Logo360 width={130}/>
        </div>
        {tabs.map(t=>{
          const alertCount=t.id==="firebase"?contratos.map(c=>({d:Math.ceil((new Date(c.fin)-new Date())/86400000)})).filter(c=>c.d>0&&c.d<=30).length:0;
          return(
      ...
      <main style={{flex:1,padding:"20px 16px",maxWidth:1240,width:"100%",margin:"0 auto",boxSizing:"border-box"}}>
        ...
        {tab==="firebase"   &&<Firebase   contratos={contratos} paneles={paneles} clientes={clientes} gastos={gastos}/>}
      </main>
    </div>
  </>);
}
