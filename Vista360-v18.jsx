...
const SUPABASE_ANON_KEY_DEFAULT = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Iml2a2hjZndnZ3p3Y3p3ZHRnaWdhIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzczNDIwNTQsImV4cCI6MjA5MjkxODA1NH0.fO0wO-T2tVV6lOxfwIyF1_w9CiJZ2QvM15r4t9H0SZk";
      ...
      <Card style={{padding:"16px 18px"}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:10}}>
          {[
            ...
            <div key={m.label} style={{background:C.surface,border:`1px solid ${C.border}`,borderRadius:12,padding:"12px 10px",display:"flex",flexDirection:"column",alignItems:"center",gap:4,textAlign:"center"}}>
              <span style={{fontSize:22}}>{m.icon}</span>
              <div style={{fontSize:26,fontWeight:800,color:m.color,letterSpacing:"-1px",lineHeight:1}}>{m.value}</div>
              <div style={{fontSize:10,color:C.muted,fontWeight:700,textTransform:"uppercase",letterSpacing:0.5}}>{m.label}</div>
            </div>
          ))}
        </div>
      </Card>
        ...
        <Card key={p.id} style={{padding:0,overflow:"hidden"}}>
          ...
          <div style={{padding:20}}>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:14}}>
              ...
              <div style={{background:`linear-gradient(135deg,${C.green}18,${C.green}08)`,border:`1px solid ${C.green}44`,borderRadius:12,padding:"10px 14px",display:"flex",flexDirection:"column",alignItems:"flex-start",gap:2}}>
                <div style={{fontSize:10,color:C.green,marginBottom:2,textTransform:"uppercase",letterSpacing:1,fontWeight:700}}>💰 Precio/mes</div>
  ...
  return(<div>
    ...
    <Card style={{padding:0,overflow:"hidden"}}>
      ...
      <table style={{width:"100%",minWidth:480,borderCollapse:"collapse"}}>
        <thead style={{background:C.surface}}>
          <tr>{["Categoría","Descripción","Monto","Acciones"].map(h=><th key={h} style={{padding:"11px 16px",textAlign:"left",fontSize:11,fontWeight:700,color:C.muted,textTransform:"uppercase",letterSpacing:1,borderBottom:`1px solid ${C.border}`,whiteSpace:"nowrap"}}>{h}</th>)}</tr>
        </thead>
        <tbody>
          ...
          {gastosMes.map(g=>(
            <tr key={g.id} onMouseEnter={e=>e.currentTarget.style.background=C.surface} onMouseLeave={e=>e.currentTarget.style.background="transparent"} style={{transition:"background .15s"}}>
              <td style={{padding:"12px 16px",borderBottom:`1px solid ${C.border}`,whiteSpace:"nowrap"}}><Tag color={catCol[g.categoria]||C.muted} ch={g.categoria}/></td>
              <td style={{padding:"12px 16px",borderBottom:`1px solid ${C.border}`,color:C.text,fontSize:13}}>{g.descripcion}</td>
              <td style={{padding:"12px 16px",borderBottom:`1px solid ${C.border}`,fontSize:14,fontWeight:700,color:C.red,whiteSpace:"nowrap"}}>{fmt(g.monto)}</td>
...
export default function App(){
  ...
  return(<>
    ...
    <div style={{fontFamily:"'DM Sans',sans-serif",background:C.bg,minHeight:"100vh",color:C.text,display:"flex",flexDirection:"column"}}>
      <nav style={{background:C.surface,borderBottom:`1px solid ${C.border}`,padding:"0 16px",paddingTop:"env(safe-area-inset-top)",display:"flex",alignItems:"center",minHeight:56,position:"sticky",top:0,zIndex:100,gap:2,overflowX:"auto",scrollbarWidth:"none"}}>
