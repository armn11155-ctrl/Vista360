...
const DARK = { bg:"#070C18", surface:"#0D1421", card:"#111929", border:"#18253D", accent:"#2563EB", green:"#10B981", red:"#EF4444", amber:"#F59E0B", purple:"#8B5CF6", cyan:"#06B6D4", text:"#E2E8F0", muted:"#64748B", white:"#FFFFFF" };
const LIGHT= { bg:"#F0F4F8", surface:"#FFFFFF", card:"#FFFFFF", border:"#D1DDE8", accent:"#1D4ED8", green:"#059669", red:"#DC2626", amber:"#D97706", purple:"#7C3AED", cyan:"#0284C7", text:"#0F172A", muted:"#64748B", white:"#0F172A" };
...
export default function App(){
  ...
  return(<>
    ...
    <style>{`*{box-sizing:border-box}html,body{margin:0;padding:0;background:${C.bg};color:${C.text};-webkit-text-size-adjust:100%;overscroll-behavior:none;}body{padding-top:env(safe-area-inset-top);padding-left:env(safe-area-inset-left);padding-right:env(safe-area-inset-right);}::-webkit-scrollbar{width:5px;height:5px}::-webkit-scrollbar-track{background:${C.bg}}::-webkit-scrollbar-thumb{background:${C.border};border-radius:3px}@keyframes spin{to{transform:rotate(360deg)}}@keyframes pulse{from{opacity:.2}to{opacity:.6}}`}</style>
    ...
    <div style={{fontFamily:"'DM Sans',sans-serif",background:C.bg,minHeight:"100vh",color:C.text,display:"flex",flexDirection:"column",transition:"background .3s,color .3s"}}>
      {/* NAV — espacio safe-area arriba, logo + tabs en misma fila abajo */}
      <nav style={{background:C.surface,borderBottom:`1px solid ${C.border}`,position:"fixed",top:0,left:0,right:0,zIndex:200}}>
        <style>{`::-webkit-scrollbar{display:none}`}</style>
        {/* Espacio para hora/batería iOS */}
        <div style={{height:"env(safe-area-inset-top)",minHeight:0,background:C.surface}}/>
        {/* Fila única: Logo + Tabs scrollables + ⚙️ */}
        <div style={{display:"flex",alignItems:"center",height:52,paddingLeft:12,paddingRight:8}}>
          <div style={{flexShrink:0,marginRight:6}}><Logo360 width={100}/></div>
          ...
          <div style={{flexShrink:0,display:"flex",alignItems:"center",gap:4,marginLeft:4}}>
            ...
            <button onClick={()=>setShowConfig(true)} style={{background:"transparent",border:"none",color:C.muted,cursor:"pointer",fontSize:13,padding:4}}>⚙️</button>
          </div>
        </div>
      </nav>
