...
const IGV_RATE = 0.18;
const calcIGV = (total) => {
  const sub = Math.round((total / (1 + IGV_RATE)) * 100) / 100;
...
function ModalFactura({ factura, contratos, paneles, clientes, onClose, onSave }) {
  ...
  const { subtotal, igv, total } = calcIGV(Number(form.monto) || 0);
      ...
      <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 22, padding: 26, width: "100%", maxWidth: 580, maxHeight: "92vh", overflowY: "auto", boxShadow: "0 24px 80px rgba(0,0,0,0.7)" }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 10 }}>
            ...
            <div>
              <label style={{ fontSize: 11, fontWeight: 700, color: C.muted, textTransform: "uppercase", letterSpacing: 1, display: "block", marginBottom: 5 }}>Número</label>
              <input value={form.numero} onChange={e => s({ numero: e.target.value })} style={inpS} />
            </div>
            <div>
          ...
          {Number(form.monto) > 0 && (
            <div style={{ background: C.surface, borderRadius: 12, padding: "12px 16px", border: `1px solid ${C.border}` }}>
              <div style={{ display: "flex", justifyContent: "space-between", fontSize: 13, color: C.muted, marginBottom: 6 }}>
                <span>Op. Gravada (sin IGV)</span><span style={{ fontFamily: "monospace", color: C.text }}>S/ {subtotal.toLocaleString("es-PE", { minimumFractionDigits: 2 })}</span>
              </div>
              <div style={{ display: "flex", justifyContent: "space-between", fontSize: 13, color: C.muted, marginBottom: 8 }}>
                <span>IGV (18%)</span><span style={{ fontFamily: "monospace", color: C.text }}>S/ {igv.toLocaleString("es-PE", { minimumFractionDigits: 2 })}</span>
              </div>
...
function ModalDetalleFactura({ factura, contratos, paneles, clientes, onClose, onEdit, onDelete, onCambiarEstado }) {
  ...
  const { subtotal, igv } = calcIGV(Number(factura.monto) || 0);
...
<head>
<meta charset="utf-8"/>
<title>${factura.tipo} ${s} — Vista360</title>
<style>
  *{box-sizing:border-box;margin:0;padding:0}
  body{font-family:'Segoe UI',Arial,sans-serif;background:#f5f5f5;padding:30px;color:#111}
  .doc{background:#fff;max-width:750px;margin:0 auto;border-radius:12px;overflow:hidden;box-shadow:0 4px 32px rgba(0,0,0,0.15)}
  .header{background:linear-gradient(135deg,#1E3A8A,#2563EB);color:#fff;padding:28px 32px;display:flex;justify-content:space-between;align-items:flex-start}
  .logo{font-size:28px;font-weight:900;letter-spacing:-1px}.logo span{color:#93C5FD}
  .tipo-doc .tipo{font-size:22px;font-weight:900;text-align:right}
  .tipo-doc .serie{font-size:16px;opacity:.85;margin-top:4px;font-family:monospace;text-align:right}
  .body{padding:28px 32px}
  .section{margin-bottom:22px}
  .section-title{font-size:11px;font-weight:800;color:#6B7280;text-transform:uppercase;letter-spacing:1.2px;margin-bottom:12px;padding-bottom:6px;border-bottom:1px solid #F3F4F6}
  .grid2{display:grid;grid-template-columns:1fr 1fr;gap:16px}
  .field label{font-size:10px;font-weight:700;color:#9CA3AF;text-transform:uppercase;letter-spacing:.8px;display:block;margin-bottom:3px}
  .field .value{font-size:14px;font-weight:600;color:#111827}
  table{width:100%;border-collapse:collapse}
  thead th{background:#F9FAFB;padding:10px 12px;font-size:11px;font-weight:700;color:#6B7280;text-transform:uppercase;text-align:left;border-bottom:1px solid #E5E7EB}
  tbody td{padding:12px;font-size:13px;color:#374151;border-bottom:1px solid #F9FAFB}
  .totales{display:flex;flex-direction:column;align-items:flex-end;gap:6px;margin-top:16px}
  .tot-row{display:flex;gap:40px;font-size:13px;color:#6B7280}
  .tot-row .lbl{min-width:80px;text-align:right}
  .tot-row .val{font-weight:700;color:#374151;min-width:90px;text-align:right;font-family:monospace}
  .total-final{display:flex;gap:20px;background:linear-gradient(135deg,#1E3A8A,#2563EB);color:#fff;border-radius:10px;padding:14px 20px;margin-top:8px;align-items:center}
  .total-final .lbl{font-size:14px;font-weight:700;flex:1}
  .total-final .val{font-size:26px;font-weight:900;font-family:monospace;letter-spacing:-.5px}
  .footer{background:#F9FAFB;border-top:1px solid #E5E7EB;padding:16px 32px;display:flex;justify-content:space-between;align-items:center;font-size:12px;color:#9CA3AF}
...
<div class="doc">
  <div class="header">
    <div>
      ...
      <div style="font-size:13px;opacity:.75;margin-top:4px">Gestión de Paneles Publicitarios</div>
      <div style="margin-top:14px;font-size:12px;opacity:.7">RUC: ________________</div>
      <div style="font-size:12px;opacity:.7;margin-top:2px">Dirección: ________________</div>
    </div>
    <div class="tipo-doc">
      <div class="tipo">${factura.tipo} ELECTRÓNICA</div>
      <div class="serie">${s}</div>
      <div style="font-size:12px;opacity:.75;margin-top:8px;text-align:right">Emisión: ${factura.fecha_emision ? new Date(factura.fecha_emision).toLocaleDateString("es-PE") : hoyStr}</div>
      ${factura.fecha_vencimiento ? `<div style="font-size:12px;opacity:.75;text-align:right">Vence: ${new Date(factura.fecha_vencimiento).toLocaleDateString("es-PE")}</div>` : ""}
      <div style="margin-top:6px;background:rgba(255,255,255,0.15);border-radius:6px;padding:4px 10px;font-size:11px;text-align:center">Estado: ${factura.estado}</div>
    </div>
  </div>
  <div class="body">
    <div class="section">
      ...
      <div class="grid2">
        <div class="field"><label>${factura.tipo === "FACTURA" ? "Razón Social" : "Apellidos y Nombres"}</label><div class="value">${cli?.empresa || "—"}</div></div>
        <div class="field"><label>${factura.tipo === "FACTURA" ? "RUC" : "DNI / CE"}</label><div class="value">${cli?.ruc || cli?.dni || "________________"}</div></div>
        <div class="field"><label>Contacto</label><div class="value">${cli?.contacto || "—"}</div></div>
        <div class="field"><label>Teléfono</label><div class="value">${cli?.celular || "—"}</div></div>
        <div class="field" style="grid-column:1/-1"><label>Dirección</label><div class="value">${cli?.direccion || "—"}</div></div>
      </div>
    </div>
    <div class="section">
      <div class="section-title">Detalle del Servicio</div>
      <table>
        <thead><tr><th>Cant.</th><th>Descripción</th><th>Período</th><th style="text-align:right">P. Unit.</th><th style="text-align:right">Total</th></tr></thead>
        <tbody>
          <tr>
            <td style="text-align:center;font-weight:700">1</td>
            <td><strong>${factura.concepto || "Arrendamiento de Panel Publicitario"}</strong>${pan ? `<br/><span style="font-size:12px;color:#6B7280">${pan.nombre} · ${pan.ciudad || ""}</span>` : ""}</td>
            <td style="font-size:12px;color:#6B7280">${factura.periodo_inicio ? new Date(factura.periodo_inicio).toLocaleDateString("es-PE") : "—"}<br/>al ${factura.periodo_fin ? new Date(factura.periodo_fin).toLocaleDateString("es-PE") : "—"}</td>
      ...
      <div class="totales">
        <div class="tot-row"><span class="lbl">Op. Gravada</span><span class="val">S/ ${subtotal.toLocaleString("es-PE", { minimumFractionDigits: 2 })}</span></div>
        <div class="tot-row"><span class="lbl">IGV (18%)</span><span class="val">S/ ${igv.toLocaleString("es-PE", { minimumFractionDigits: 2 })}</span></div>
        <div class="total-final"><span class="lbl">IMPORTE TOTAL</span><span class="val">S/ ${Number(factura.monto).toLocaleString("es-PE", { minimumFractionDigits: 2 })}</span></div>
      </div>
    </div>
    ${factura.notas ? `<div class="section"><div class="section-title">Notas</div><p style="font-size:13px;color:#6B7280">${factura.notas}</p></div>` : ""}
  </div>
  <div class="footer">
    <div>Generado por <strong>Vista360</strong> · Sistema de Gestión de Paneles Publicitarios</div>
    ...
    const a = document.createElement("a");
    a.href = url;
    a.download = `${factura.tipo}-${s}-${cli?.empresa || "cliente"}.html`;
    document.body.appendChild(a); a.click(); document.body.removeChild(a); URL.revokeObjectURL(url);
  };
    ...
    <div style={{ position: "fixed", inset: 0, background: "#00000096", zIndex: 300, display: "flex", alignItems: "center", justifyContent: "center", padding: 16, backdropFilter: "blur(8px)" }}
      ...
      <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 22, padding: 26, width: "100%", maxWidth: 560, maxHeight: "92vh", overflowY: "auto", boxShadow: "0 24px 80px rgba(0,0,0,0.7)" }}>
        ...
        <div style={{ background: `linear-gradient(135deg,${C.green}18,${C.green}08)`, border: `1px solid ${C.green}33`, borderRadius: 16, padding: "20px 24px", marginBottom: 20, textAlign: "center" }}>
          ...
          <div style={{ fontSize: 12, color: C.muted, marginTop: 6 }}>Sub: S/ {subtotal.toLocaleString("es-PE", { minimumFractionDigits: 2 })} + IGV S/ {igv.toLocaleString("es-PE", { minimumFractionDigits: 2 })}</div>
        </div>
        ...
        <div style={{ marginBottom: 16 }}>
          ...
          <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
            {EST_FAC.filter(e => e !== factura.estado).map(e => (
              <button key={e} onClick={() => onCambiarEstado(e)} style={{ padding: "6px 14px", borderRadius: 8, border: `1px solid ${EST_FAC_COL[e]}55`, background: EST_FAC_COL[e] + "18", color: EST_FAC_COL[e], fontSize: 12, fontWeight: 700, cursor: "pointer" }}>
                → {e}
              </button>
            ))}
          </div>
        ...
        <div style={{ display: "flex", gap: 8 }}>
          ...
          <button onClick={onEdit} style={{ flex: 1, padding: 11, background: C.surface, border: `1px solid ${C.border}`, borderRadius: 10, color: C.text, fontWeight: 600, fontSize: 13, cursor: "pointer" }}>
            ✏️ Editar
          </button>
...
// ══════════════════════════════════════════════════════════════════
// 🧾 GENERADOR DE PRE-FACTURA PDF
// Genera un HTML imprimible listo para copiar datos a SUNAT SOL
// ══════════════════════════════════════════════════════════════════
function ModalPreFactura({ contrato, panel, cliente, onClose }) {
  const [tipo, setTipo] = useState("FACTURA");
  const [serie, setSerie] = useState(tipo === "FACTURA" ? "F001" : "B001");
...
<head>
<meta charset="utf-8"/>
<title>Pre-Factura — ${s}-${numero.padStart(8,"0")}</title>
<style>
  *{box-sizing:border-box;margin:0;padding:0}
  body{font-family:'Segoe UI',Arial,sans-serif;background:#f5f5f5;padding:30px;color:#111}
  .doc{background:#fff;max-width:750px;margin:0 auto;border-radius:12px;overflow:hidden;box-shadow:0 4px 32px rgba(0,0,0,0.15)}
  ...
  .tipo-doc .tipo{font-size:22px;font-weight:900;letter-spacing:0.5px}
  .tipo-doc .serie{font-size:16px;opacity:0.85;margin-top:4px;font-family:monospace}
  .aviso{background:#FEF3C7;border-left:4px solid #F59E0B;padding:12px 20px;display:flex;gap:10;align-items:center;font-size:13px;color:#92400E}
  .aviso strong{color:#78350F}
  .body{padding:28px 32px}
  .section{margin-bottom:22px}
  .section-title{font-size:11px;font-weight:800;color:#6B7280;text-transform:uppercase;letter-spacing:1.2px;margin-bottom:12px;padding-bottom:6px;border-bottom:1px solid #F3F4F6}
  .grid2{display:grid;grid-template-columns:1fr 1fr;gap:16px}
  .field label{font-size:10px;font-weight:700;color:#9CA3AF;text-transform:uppercase;letter-spacing:0.8px;display:block;margin-bottom:3px}
  .field .value{font-size:14px;font-weight:600;color:#111827}
  .field .value.mono{font-family:monospace;font-size:13px}
  .concepto-box{background:#F9FAFB;border:1px solid #E5E7EB;border-radius:10px;padding:14px 16px}
  .concepto-box .desc{font-size:14px;color:#374151;line-height:1.5}
  table{width:100%;border-collapse:collapse}
  thead th{background:#F9FAFB;padding:10px 12px;font-size:11px;font-weight:700;color:#6B7280;text-transform:uppercase;letter-spacing:0.8px;text-align:left;border-bottom:1px solid #E5E7EB}
  tbody td{padding:12px;font-size:13px;color:#374151;border-bottom:1px solid #F9FAFB}
  .totales{display:flex;flex-direction:column;align-items:flex-end;gap:6px;margin-top:16px}
  .tot-row{display:flex;gap:40px;font-size:13px;color:#6B7280}
  .tot-row .lbl{min-width:80px;text-align:right}
  ...
  .total-final{display:flex;gap:20px;background:linear-gradient(135deg,#1E3A8A,#2563EB);color:#fff;border-radius:10px;padding:14px 20px;margin-top:8px;align-items:center}
  .total-final .lbl{font-size:14px;font-weight:700;flex:1}
  .total-final .val{font-size:26px;font-weight:900;font-family:monospace;letter-spacing:-0.5px}
  .footer{background:#F9FAFB;border-top:1px solid #E5E7EB;padding:16px 32px;display:flex;justify-content:space-between;align-items:center}
  .footer .brand{font-size:12px;color:#9CA3AF}
  .footer .brand strong{color:#6B7280}
  .instrucciones{margin-top:20px;background:#EFF6FF;border:1px solid #BFDBFE;border-radius:10px;padding:16px 20px}
  .instrucciones h3{font-size:13px;font-weight:700;color:#1D4ED8;margin-bottom:10px}
  .instrucciones ol{padding-left:18px}
  .instrucciones li{font-size:12px;color:#374151;margin-bottom:6px;line-height:1.5}
  .instrucciones li strong{color:#1D4ED8}
  @media print{
...
<div class="doc">
  ...
  <div class="header">
    <div>
      <div class="logo">Vista<span>360</span></div>
      <div style="font-size:13px;opacity:0.75;margin-top:4px">Gestión de Paneles Publicitarios</div>
      <div style="margin-top:14px;font-size:12px;opacity:0.7">RUC: ________________</div>
      <div style="font-size:12px;opacity:0.7;margin-top:2px">Dirección: ________________</div>
    </div>
    <div class="tipo-doc">
      <div class="tipo">${tipo} ELECTRÓNICA</div>
      <div class="serie">${s}-${numero.padStart(8,"0")}</div>
      <div style="font-size:12px;opacity:0.75;margin-top:8px">Fecha: ${hoy}</div>
      <div style="margin-top:6px;background:rgba(255,255,255,0.15);border-radius:6px;padding:4px 10px;font-size:11px;text-align:center">⚠️ PRE-FACTURA — Solo para referencia</div>
    </div>
  </div>
  ...
  <div class="aviso">
    ...
    <div>
      <strong>Documento de pre-factura — NO tiene validez tributaria.</strong>
      Úsalo para ingresar los datos en <strong>SUNAT SOL</strong> y emitir el comprobante oficial.
    </div>
  </div>
  ...
  <div class="body">
    ...
    <div class="section">
      ...
      <div class="grid2">
        ...
        <div class="field">
          ...
          <div class="value mono">${cliente.ruc || cliente.dni || "________________"}</div>
        </div>
        <div class="field">
          <label>Contacto</label>
          <div class="value">${cliente.contacto || "—"}</div>
        </div>
        <div class="field">
          <label>Teléfono</label>
          <div class="value mono">${cliente.celular || cliente.telefono || "—"}</div>
        </div>
    ...
    <div class="section">
        ...
        <tbody>
          <tr>
            ...
            <td>
              ...
              <span style="font-size:12px;color:#6B7280">${panel.nombre} · ${panel.tipo || ""} · ${panel.ciudad || ""}</span>
              ${panel.direccion ? `<br/><span style="font-size:11px;color:#9CA3AF">📍 ${panel.direccion}</span>` : ""}
            </td>
            <td style="font-size:12px;color:#6B7280">
              ${fmtF(contrato.inicio)}<br/>al ${fmtF(contrato.fin)}
            </td>
            <td style="text-align:right;font-family:monospace;font-weight:600">S/ ${subtotal.toLocaleString("es-PE",{minimumFractionDigits:2})}</td>
            <td style="text-align:right;font-family:monospace;font-weight:700">S/ ${subtotal.toLocaleString("es-PE",{minimumFractionDigits:2})}</td>
          </tr>
      ...
      <div class="totales">
        <div class="tot-row">
          <span class="lbl">Op. Gravada</span>
          <span class="val">S/ ${subtotal.toLocaleString("es-PE",{minimumFractionDigits:2})}</span>
        </div>
        <div class="tot-row">
          <span class="lbl">IGV (18%)</span>
          <span class="val">S/ ${igv.toLocaleString("es-PE",{minimumFractionDigits:2})}</span>
        </div>
        <div class="total-final">
          <span class="lbl">IMPORTE TOTAL</span>
          <span class="val">S/ ${monto.toLocaleString("es-PE",{minimumFractionDigits:2})}</span>
        </div>
      </div>
    </div>
    ...
    <!-- ESTADO DEL CONTRATO -->
    <div class="section">
      <div class="section-title">Estado del Contrato</div>
      <div class="grid2">
        <div class="field">
          <label>Estado de pago</label>
          <div class="value" style="color:${contrato.pagado?"#059669":"#D97706"}">${contrato.pagado ? "✅ PAGADO" : "⏳ PENDIENTE DE PAGO"}</div>
        </div>
        <div class="field">
          <label>Estado del panel</label>
          <div class="value">${panel.estado || "—"}</div>
        </div>
      </div>
    </div>

    <!-- INSTRUCCIONES SUNAT SOL -->
    <div class="instrucciones no-print">
      <h3>📋 Cómo emitir en SUNAT SOL con estos datos</h3>
      <ol>
        <li>Ingresa a <strong>sol.sunat.gob.pe</strong> con tu clave SOL</li>
        <li>Ve a <strong>Empresas → Comprobantes de pago → Emitir ${tipo}</strong></li>
        <li>Ingresa el <strong>${tipo==="FACTURA"?"RUC":"DNI"} del cliente</strong>: <strong>${cliente.ruc || cliente.dni || "—"}</strong></li>
        <li>En descripción escribe: <strong>Arrendamiento Panel Publicitario — ${panel.nombre}</strong></li>
        <li>Valor de venta (sin IGV): <strong>S/ ${subtotal.toLocaleString("es-PE",{minimumFractionDigits:2})}</strong></li>
        <li>El sistema calculará el IGV (18%) automáticamente → Total: <strong>S/ ${monto.toLocaleString("es-PE",{minimumFractionDigits:2})}</strong></li>
        <li>Confirma y descarga el <strong>XML + PDF</strong> del comprobante oficial</li>
      </ol>
    </div>
  ...
  <div class="footer">
    <div class="brand">
      Generado por <strong>Vista360</strong> · Sistema de Gestión de Paneles Publicitarios
    </div>
    <div style="font-size:11px;color:#9CA3AF">${new Date().toLocaleString("es-PE")}</div>
  </div>
  ...
  const descargar = () => {
    ...
    const a    = document.createElement("a");
    a.href = url;
    a.download = `PreFactura-${cliente.empresa||"cliente"}-${s}-${numero.padStart(8,"0")}.html`;
  ...
  return (
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.9)",zIndex:500,
      ...
      backdropFilter:"blur(8px)",padding:"0 0 0 0"}}
      onClick={e=>e.target===e.currentTarget&&onClose()}>
      <div style={{background:"#0B1324",border:"1px solid #1E3050",
        ...
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"8px 22px 18px"}}>
          <div>
            <div style={{fontSize:17,fontWeight:800,color:C.white}}>🧾 Generar Pre-Factura</div>
            <div style={{fontSize:12,color:C.muted,marginTop:2}}>Para ingresar datos en SUNAT SOL</div>
          </div>
        ...
        <div style={{padding:"0 22px",display:"flex",flexDirection:"column",gap:16}}>
          ...
          <div style={{background:C.amber+"12",border:`1px solid ${C.amber}33`,
            ...
            <span style={{fontSize:12,color:C.amber,fontWeight:600}}>
              Este PDF es solo para referencia. El comprobante oficial debes emitirlo en SUNAT SOL (gratis).
            </span>
          </div>
              ...
              {["FACTURA","BOLETA"].map(t=>(
                <button key={t} onClick={()=>{setTipo(t);setSerie(t==="FACTURA"?"F001":"B001");}}
                  ...
                  <div style={{fontSize:10,fontWeight:400,marginTop:2,opacity:0.7}}>
                    {t==="FACTURA"?"Con RUC del cliente":"Con DNI / sin RUC"}
                  </div>
                </button>
              ))}
            </div>
          ...
          <div style={{background:C.surface,border:`1px solid ${C.border}`,borderRadius:12,padding:"14px 16px"}}>
            <div style={{fontSize:11,fontWeight:700,color:C.muted,textTransform:"uppercase",letterSpacing:1.2,marginBottom:10}}>Resumen</div>
            <div style={{display:"flex",flexDirection:"column",gap:6}}>
              {[
                {l:"Cliente",v:cliente.empresa||"—",c:C.white},
                {l:tipo==="FACTURA"?"RUC":"DNI",v:cliente.ruc||cliente.dni||"⚠️ Sin documento",c:cliente.ruc||cliente.dni?C.text:C.amber},
                {l:"Panel",v:`${panel.nombre} · ${panel.ciudad||""}`,c:C.text},
                {l:"Período",v:`${fmtF(contrato.inicio)} → ${fmtF(contrato.fin)}`,c:C.muted},
                {l:"Subtotal s/IGV",v:`S/ ${(Number(contrato.monto||0)/1.18).toFixed(2)}`,c:C.text},
                {l:"IGV 18%",v:`S/ ${(Number(contrato.monto||0)-(Number(contrato.monto||0)/1.18)).toFixed(2)}`,c:C.text},
                {l:"TOTAL",v:fmt(contrato.monto),c:C.green,bold:true},
              ].map(({l,v,c,bold})=>(
                <div key={l} style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                  <span style={{fontSize:12,color:C.muted}}>{l}</span>
                  <span style={{fontSize:bold?15:12,fontWeight:bold?800:600,color:c}}>{v}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          <div style={{display:"flex",gap:10}}>
            ...
            <button onClick={descargar} style={{flex:1,padding:"13px",
              ...
              📄 Descargar Pre-Factura HTML
            </button>
          </div>

          {/* Info adicional */}
          <div style={{background:C.accent+"0D",border:`1px solid ${C.accent}22`,borderRadius:10,padding:"12px 14px"}}>
            <div style={{fontSize:12,color:C.accent,fontWeight:700,marginBottom:4}}>💡 El archivo incluye:</div>
            <div style={{fontSize:11,color:C.muted,lineHeight:1.7}}>
              ✔ Datos del cliente y RUC<br/>
              ✔ Detalle del servicio con período<br/>
              ✔ Subtotal + IGV 18% + Total<br/>
              ✔ Paso a paso para emitir en SUNAT SOL<br/>
...
function Reportes({ contratos, paneles, clientes, gastos }) {
  ...
  const secciones = [
    ...
    {id:"prefactura",label:"🧾 Pre-Factura"},
  ];
    ...
    <div>
      ...
      <div style={{marginBottom:22}}>
        ...
        <div style={{fontSize:13,color:C.muted,marginTop:3}}>
          Análisis financiero y generador de pre-facturas para SUNAT SOL
        </div>
      </div>
      ...
      {seccion==="prefactura" && (
        <div>
            ...
            <div style={{display:"flex",gap:12,alignItems:"flex-start"}}>
              ...
              <div>
                <div style={{fontSize:14,fontWeight:800,color:C.white,marginBottom:4}}>
                  ¿Cómo funciona la Pre-Factura?
                </div>
                <div style={{fontSize:12,color:C.muted,lineHeight:1.7}}>
                  Selecciona un contrato → descarga el HTML con todos los datos listos → 
                  abre SUNAT SOL (sol.sunat.gob.pe) → copia los datos → emite tu factura o boleta oficial gratis.
                </div>
                <div style={{display:"flex",gap:8,marginTop:10,flexWrap:"wrap"}}>
                  {["Gratis con SUNAT SOL","100% válido tributariamente","Sin instalar nada"].map(t=>(
          ...
          <div style={{marginBottom:14,fontSize:11,fontWeight:700,color:C.muted,textTransform:"uppercase",letterSpacing:1.2}}>
            Selecciona un contrato para generar la pre-factura
          </div>
...
export default function App(){
  ...
  return(<>
    ...
    <style>{`*{box-sizing:border-box;-webkit-tap-highlight-color:transparent}html,body{margin:0;padding:0;background:#0A0F1E;height:100%;overscroll-behavior-y:none;-webkit-text-size-adjust:100%}::-webkit-scrollbar{display:none}@keyframes spin{to{transform:rotate(360deg)}}@keyframes pulse{from{opacity:.2}to{opacity:.6}}@keyframes starTwinkle{0%,100%{opacity:0.5;transform:scale(1)}50%{opacity:1;transform:scale(1.3)}}@keyframes dishPulse{0%,100%{opacity:0.35}50%{opacity:0.55}}`}</style>
    ...
    <div style={{width:"100%",height:"100dvh",backgroundColor:C.bg,display:"flex",flexDirection:"column",margin:0,padding:0,fontFamily:"'DM Sans',sans-serif",color:C.text,overflow:"hidden",position:"relative"}}>
      ...
      <div style={{
        ...
        <div style={{position:"absolute",top:0,left:0,right:0,height:1,background:"linear-gradient(90deg,transparent,rgba(79,124,255,0.5),rgba(155,111,255,0.3),transparent)",pointerEvents:"none"}}/>

        {/* Barra única: Logo · Tabs scrollables · Avatar */}
        <div style={{
          display:"flex",alignItems:"center",gap:8,
          height:44,
        }}>
          ...
          {/* Tabs con scroll horizontal — ocupa todo el espacio del medio */}
          <div style={{
            flex:1,minWidth:0,
            ...
            {tabs.filter(t=>t.id!=="perfil").map(t=>{
              ...
              return(
                <button key={t.id} onClick={()=>handleTabClick(t.id)} style={{
                  ...
                  background:active
                    ?"linear-gradient(135deg,rgba(79,124,255,0.26) 0%,rgba(123,95,255,0.16) 100%)"
                    :"transparent",
                  border:active?"1px solid rgba(79,124,255,0.35)":"1px solid transparent",
                  borderRadius:11,cursor:"pointer",
                  color:active?"#FFFFFF":"rgba(255,255,255,0.38)",
                  transition:"all 0.16s ease",
                  boxShadow:active?"0 2px 14px rgba(79,124,255,0.2),inset 0 1px 0 rgba(255,255,255,0.08)":"none",
                  whiteSpace:"nowrap",position:"relative",
                }}>
                  {active&&<div style={{position:"absolute",bottom:0,left:"25%",right:"25%",height:"2px",background:"linear-gradient(90deg,transparent,#4F7CFF 50%,transparent)",borderRadius:2,pointerEvents:"none"}}/>}
                  <span style={{flexShrink:0,display:"flex",alignItems:"center",opacity:active?1:0.45,transform:"scale(0.76)",transformOrigin:"center"}}>
                    {ICONS[t.id]||ICONS.dashboard}
                  </span>
                  <span style={{fontSize:12,fontWeight:active?700:500,letterSpacing:active?0.1:0,color:active?"#fff":"rgba(255,255,255,0.4)"}}>
                    {t.label}
                  </span>
                </button>
              );
          ...
          {/* Avatar perfil — fijo derecha */}
          <button onClick={()=>handleTabClick("perfil")} style={{
            flexShrink:0,width:34,height:34,borderRadius:"50%",
            background:"linear-gradient(135deg,#4F46E5 0%,#7B5FFF 100%)",
            display:"flex",alignItems:"center",justifyContent:"center",
            fontSize:11,fontWeight:800,color:"#fff",
            border:activeTab==="perfil"?"2px solid #4F7CFF":"2px solid rgba(255,255,255,0.12)",
            boxShadow:activeTab==="perfil"?"0 0 14px rgba(79,124,255,0.55)":"0 2px 8px rgba(0,0,0,0.4)",
            cursor:"pointer",transition:"all 0.18s",
          }}>AM</button>
        </div>
      </div>
      ...
      <div style={{
        ...
        <main style={{height:"100%",overflowY:"auto",overflowX:"hidden",padding:"20px",paddingBottom:"calc(env(safe-area-inset-bottom) + 6px)",WebkitOverflowScrolling:"touch",scrollBehavior:"smooth",backgroundColor:C.bg}}>
