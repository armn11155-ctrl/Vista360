...
function Gastos({gastos,setGastos}){
  ...
  // ══════════════════════════════════════════════════════════════
  // 2. CARGA DE TESSERACT.JS DESDE CDN (lazy, una sola vez)
  // ══════════════════════════════════════════════════════════════
  ...
  const escanear=async(file)=>{
    ...
    try{
      ...
      setOcr(o=>({...o,progress:15,fase:"Leyendo imagen..."}));
      ...
      // ── Paso 2: Llamar al endpoint OCR de Vercel ────────────
      // El endpoint /api/ocr usa Python + Tesseract + OpenCV en el servidor
      // Completamente GRATIS, sin límites, sin API keys externas
      setOcr(o=>({...o,progress:35,fase:"Extrayendo texto con Tesseract..."}));
      ...
      setOcr(o=>({...o,progress:70,fase:"Analizando datos de la boleta..."}));

      if(!resp.ok){
        const err=await resp.text();
        throw new Error("Error servidor OCR: "+err);
      }

      const datos=await resp.json();
      setOcr(o=>({...o,progress:95,fase:"Completando formulario..."}));

      if(datos.error){
        // Mostrar sugerencia al usuario si hay error
        setOcr(o=>({...o,loading:false,progress:0,
          error:datos.error,
          sugerencia:datos.sugerencia||"Intenta con la boleta bien plana y buena iluminación"
        }));
        return;
      }

      // ── Paso 3: Rellenar formulario automáticamente ─────────
      setForm(f=>({
        ...f,
        // Montos
        monto:      datos.total    ? String(datos.total)    : f.monto,
        igv:        datos.igv      ? String(datos.igv)      : f.igv,
        subtotal:   datos.subtotal ? String(datos.subtotal) : f.subtotal,
        moneda:     datos.moneda   || f.moneda || "PEN",
        // Datos del gasto
        fecha:      datos.fecha    || f.fecha,
        categoria:  datos.categoria&& datos.categoria!=="Otro" ? datos.categoria : f.categoria,
        concepto:   datos.concepto || f.concepto,
        // Proveedor
        proveedor:  datos.comercio || f.proveedor,
        ruc:        datos.ruc      || f.ruc,
        // Metadata
        foto_texto: datos.texto_extraido||"",
        // Marcar campos inciertos para que el usuario los revise
        _inciertos: datos.campos_inciertos||[],
        _confidence:datos.confidence||0,
      }));
      ...
      setOcr(o=>({
        ...
        loading:false,
        progress:100,
        fase:"¡Datos extraídos!",
        text:datos.texto_extraido||"",
        confidence:datos.confidence||0,
        inciertos:datos.campos_inciertos||[],
      }));

    }catch(e){
      setOcr(o=>({...o,loading:false,progress:0,fase:"",error:e.message}));
    }
  };
  ...
  const extraerDatosInteligente=(text)=>{
    ...
    // Descarta líneas que son solo números, etiquetas genéricas, o basura OCR
    const skipProv=/^(factura|boleta|ticket|nota|comprobante|ruc|r\.u\.c|fecha|total|subtotal|igv|tax|item|cantidad|precio|descripcion|detalle|cod|nro|n°|serie|copia|gracias|direccion|telefono|email|www|http)/i;
    const skipCorta=/^[\d\s\.\-\/\*#]{0,5}$/; // solo números o símbolos
    const proveedor=lineas.find(l=>
      l.length>=4&&
      /[a-zA-ZÀ-ÿ]{3,}/.test(l)&&  // al menos 3 letras consecutivas
      !skipProv.test(l)&&
      !skipCorta.test(l)&&
  ...
  return(<div>
    ...
    {/* ── HEADER ── */}
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:20}}>
      <PgTit icon="🧾" title="Gastos" sub="OCR binarización adaptativa · $0 costo"/>
        ...
        <div style={{background:"#FFFFFF",borderRadius:20,width:"100%",maxWidth:860,maxHeight:"90vh",overflowY:"auto",display:"flex",flexDirection:"column",boxShadow:"0 24px 80px rgba(0,0,0,0.6)"}}>
          ...
          <div style={{display:"flex",gap:0,flex:1,minHeight:0}}>
            ...
            <div style={{width:"38%",minWidth:180,background:"#F9FAFB",borderRight:"1px solid #E5E7EB",padding:"20px",display:"flex",flexDirection:"column",alignItems:"center",gap:12,borderRadius:"0 0 0 20px"}}>
              {(ocr.previewUrl||vistaDetalle.foto_url)?
                <img src={ocr.previewUrl||vistaDetalle.foto_url} alt="boleta" style={{width:"100%",borderRadius:12,border:"1px solid #E5E7EB",objectFit:"contain",maxHeight:340,background:"#fff"}}/>
      ...
      <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.88)",zIndex:450,display:"flex",alignItems:"flex-end",justifyContent:"center",backdropFilter:"blur(8px)"}}
        ...
        <div style={{background:"#0B1324",border:"1px solid #1E3050",borderRadius:"22px 22px 0 0",width:"100%",maxWidth:540,maxHeight:"93vh",overflowY:"auto",paddingBottom:36}}>
          ...
          <div style={{padding:"0 22px"}}>
            ...
            <div style={{background:"#060E1A",border:"1px solid #1E3050",borderRadius:12,padding:"14px 16px",marginBottom:14}}>
              <KInp label="Fecha de Emisión *" keyN="fecha" type="date"/>
