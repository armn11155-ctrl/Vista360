...
// ── PALETA ───────────────────────────────────────────────────────
const C = {
  bg:"#080C14", surface:"#0D1220", card:"#0F1724", border:"#1A2744",
  accent:"#4F6EF7", green:"#10B981", red:"#EF4444", amber:"#F59E0B",
  purple:"#7C5CFC", cyan:"#06B6D4", text:"#E2E8F0", muted:"#4A5A7A", white:"#FFFFFF",
};
...
function Dashboard({clientes,contratos,paneles,setTab}){
  ...
  return(<div style={{paddingBottom:40,display:"flex",flexDirection:"column",gap:16}}>
    ...
    {/* ── HERO CARD ── */}
    <div style={{marginBottom:14,borderRadius:22,position:"relative",overflow:"hidden",boxShadow:"0 12px 48px rgba(10,30,120,0.6)",minHeight:200}}>
      {/* Fondo: cielo nocturno con gradiente púrpura/azul profundo */}
      <div style={{position:"absolute",inset:0,background:"linear-gradient(160deg,#0B0B2E 0%,#15114A 25%,#2D1B69 55%,#1A0E3D 80%,#0E0B28 100%)"}}/>
      {/* Montañas / silueta de horizonte */}
      <svg style={{position:"absolute",bottom:0,left:0,right:0,width:"100%"}} viewBox="0 0 400 100" preserveAspectRatio="none" fill="none">
        <path d="M0 100 L0 65 Q40 30 80 55 Q110 70 130 40 Q160 10 190 45 Q220 70 250 35 Q275 5 310 50 Q340 80 370 45 L400 60 L400 100 Z" fill="rgba(20,10,50,0.85)"/>
        <path d="M0 100 L0 80 Q50 55 90 72 Q130 85 160 65 Q190 48 220 70 Q255 88 290 68 Q330 45 370 72 L400 80 L400 100 Z" fill="rgba(15,8,40,0.9)"/>
      </svg>
      {/* Estrellas */}
      {[[8,12],[18,8],[35,20],[55,6],[72,15],[85,25],[92,10],[45,30],[28,18],[65,8],[78,30]].map(([x,y],i)=>(
        <div key={i} style={{position:"absolute",left:`${x}%`,top:`${y}%`,width:i%3===0?2.5:1.5,height:i%3===0?2.5:1.5,borderRadius:"50%",background:"rgba(255,255,255,"+(0.4+i*0.05)+")",boxShadow:i%3===0?"0 0 4px rgba(200,200,255,0.8)":"none"}}/>
      ))}
      {/* Nebulosa glow */}
      <div style={{position:"absolute",top:-20,right:-30,width:260,height:200,borderRadius:"50%",background:"radial-gradient(ellipse,rgba(139,92,246,0.25) 0%,rgba(79,70,229,0.12) 40%,transparent 70%)",pointerEvents:"none"}}/>
      <div style={{position:"absolute",bottom:-10,left:-20,width:180,height:140,borderRadius:"50%",background:"radial-gradient(ellipse,rgba(59,130,246,0.15) 0%,transparent 70%)",pointerEvents:"none"}}/>

      <div style={{position:"relative",padding:"22px 20px 24px",display:"flex",alignItems:"flex-start",justifyContent:"space-between"}}>
        <div style={{flex:1,minWidth:0}}>
          ...
          <div style={{fontSize:28,fontWeight:900,color:"#FFFFFF",letterSpacing:"-0.5px",lineHeight:1.1,marginBottom:6}}>
            Alan Martínez
          </div>
          <div style={{fontSize:13,color:"rgba(255,255,255,0.5)",marginBottom:18}}>Aquí tienes el resumen de hoy.</div>
          <div style={{display:"inline-flex",alignItems:"center",gap:8,background:"rgba(255,255,255,0.08)",border:"1px solid rgba(255,255,255,0.15)",borderRadius:30,padding:"7px 15px"}}>
            <svg width="13" height="13" viewBox="0 0 14 14" fill="none">
              <rect x="1" y="2" width="12" height="11" rx="2" stroke="rgba(255,255,255,0.6)" strokeWidth="1.3" fill="none"/>
              <line x1="1" y1="6" x2="13" y2="6" stroke="rgba(255,255,255,0.6)" strokeWidth="1.3"/>
              <line x1="4" y1="1" x2="4" y2="4" stroke="rgba(255,255,255,0.6)" strokeWidth="1.3" strokeLinecap="round"/>
              <line x1="10" y1="1" x2="10" y2="4" stroke="rgba(255,255,255,0.6)" strokeWidth="1.3" strokeLinecap="round"/>
            </svg>
        ...
        {/* Antena con brillo */}
        <div style={{flexShrink:0,marginTop:-4,marginRight:-4,filter:"drop-shadow(0 0 20px rgba(139,92,246,0.5))"}}>
          <AntenaImg/>
        </div>
      </div>
    </div>
    ...
    {/* ── INGRESOS ── */}
    <div style={{marginBottom:14,padding:"18px 18px 16px",background:"linear-gradient(145deg,#071510,#050E09)",borderRadius:22,border:"1px solid rgba(16,185,129,0.2)",boxShadow:"0 4px 32px rgba(16,185,129,0.08)",position:"relative",overflow:"hidden"}}>
      <div style={{position:"absolute",top:-30,right:-30,width:120,height:120,borderRadius:"50%",background:"radial-gradient(circle,rgba(16,185,129,0.10) 0%,transparent 70%)"}}/>
      {/* Fila superior: ícono + título + tres puntos */}
      <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:14}}>
        <IconDolar/>
        <div style={{fontSize:10,fontWeight:800,color:"rgba(110,231,183,0.75)",textTransform:"uppercase",letterSpacing:2}}>Ingresos este mes</div>
        <div style={{marginLeft:"auto",color:"rgba(255,255,255,0.3)",fontSize:18,letterSpacing:1}}>···</div>
      </div>
      {/* Fila principal: valor + gráfica */}
      <div style={{display:"flex",alignItems:"flex-end",justifyContent:"space-between",gap:12}}>
        <div>
          <div style={{fontSize:42,fontWeight:900,color:"#10B981",letterSpacing:"-2px",lineHeight:1,marginBottom:4}}>{fmt(ingMes)}</div>
          <div style={{fontSize:12,color:"rgba(110,231,183,0.55)"}}>{pag} de {contratos.length} contratos cobrados</div>
        </div>
        {/* Mini gráfica de línea verde — igual a la foto */}
        <div style={{flexShrink:0,display:"flex",flexDirection:"column",alignItems:"flex-end",gap:6}}>
          <svg width="90" height="48" viewBox="0 0 90 48" fill="none">
            ...
            {/* Area relleno */}
            <path d="M0 38 Q15 35 22 30 Q30 25 40 28 Q50 31 58 18 Q68 8 78 6 L90 4 L90 48 L0 48 Z" fill="url(#lineGrad)"/>
            {/* Línea */}
            <path d="M0 38 Q15 35 22 30 Q30 25 40 28 Q50 31 58 18 Q68 8 78 6 L90 4" stroke="#10B981" strokeWidth="2.5" fill="none" strokeLinecap="round" strokeLinejoin="round"/>
            {/* Punto final brillante */}
            <circle cx="90" cy="4" r="4" fill="#10B981" opacity="0.9"/>
            <circle cx="90" cy="4" r="2" fill="white"/>
          </svg>
          {/* vs mes anterior */}
          <div style={{textAlign:"right"}}>
            <div style={{fontSize:10,color:"rgba(255,255,255,0.3)",marginBottom:2}}>vs mes anterior</div>
            <div style={{fontSize:20,fontWeight:900,color:crecPct>=0?"#10B981":C.red,lineHeight:1}}>{crecPct===0?"0%":crecPct>0?"+"+crecPct+"%":crecPct+"%"}</div>
            <div style={{fontSize:10,color:"rgba(255,255,255,0.3)",marginTop:2}}>{crecPct===0?"Sin cambios":crecPct>0?"Crecimiento":"Disminución"}</div>
          </div>
        </div>
      </div>
    </div>
    ...
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:14}}>
      {/* Ocupación */}
      <div style={{padding:"16px 12px 18px",background:"linear-gradient(145deg,#0C1830,#080F20)",borderRadius:20,border:"1px solid rgba(79,110,247,0.15)",display:"flex",flexDirection:"column",alignItems:"center",gap:8}}>
        <div style={{display:"flex",alignItems:"center",gap:6,alignSelf:"flex-start"}}>
          <svg width="18" height="18" viewBox="0 0 18 18" fill="none">
            <rect width="18" height="18" rx="5" fill="url(#gBarOcu)"/>
            <defs><linearGradient id="gBarOcu" x1="0" y1="0" x2="18" y2="18"><stop stopColor="#1E3A6E"/><stop offset="1" stopColor="#4F6EF7"/></linearGradient></defs>
            <rect x="3" y="10" width="3" height="5" rx="1" fill="#93C5FD"/>
            <rect x="7.5" y="7" width="3" height="8" rx="1" fill="#60A5FA"/>
            <rect x="12" y="4" width="3" height="11" rx="1" fill="#3B82F6"/>
          </svg>
          <span style={{fontSize:10,fontWeight:700,color:"rgba(129,140,248,0.9)",textTransform:"uppercase",letterSpacing:1}}>Ocupación</span>
        </div>
        <svg width="90" height="90" viewBox="0 0 90 90">
          <defs><linearGradient id="gocu4" x1="0" y1="0" x2="1" y2="1"><stop stopColor="#818CF8"/><stop offset="1" stopColor="#4F6EF7"/></linearGradient></defs>
          <circle cx="45" cy="45" r="36" fill="none" stroke="rgba(79,110,247,0.12)" strokeWidth="8"/>
          <circle cx="45" cy="45" r="36" fill="none" stroke="url(#gocu4)" strokeWidth="8"
            strokeDasharray={`${(ocu/100)*2*Math.PI*36} ${2*Math.PI*36}`} strokeLinecap="round" transform="rotate(-90 45 45)"
            style={{filter:"drop-shadow(0 0 8px rgba(79,110,247,0.9))"}}/> 
          <text x="45" y="48" textAnchor="middle" fill="white" fontSize="18" fontWeight="900">{ocu}%</text>
          <text x="45" y="62" textAnchor="middle" fill="rgba(129,140,248,0.7)" fontSize="9">ocupación</text>
        </svg>
        <div style={{textAlign:"center"}}>
          <div style={{fontSize:12,color:"rgba(255,255,255,0.5)"}}>{paneles.filter(p=>p.estado==="Ocupado").length} de {paneles.length} paneles</div>
          {paneles.length===0&&<div style={{fontSize:10,color:"#4F6EF7",marginTop:3,display:"flex",alignItems:"center",gap:3,justifyContent:"center"}}>Sin datos aún <span style={{fontSize:11}}>ⓘ</span></div>}
        </div>
      </div>
      {/* Cobro */}
      <div style={{padding:"16px 12px 18px",background:"linear-gradient(145deg,#081A12,#04100A)",borderRadius:20,border:"1px solid rgba(16,185,129,0.15)",display:"flex",flexDirection:"column",alignItems:"center",gap:8}}>
        <div style={{display:"flex",alignItems:"center",gap:6,alignSelf:"flex-start"}}>
          <svg width="18" height="18" viewBox="0 0 18 18" fill="none">
            <rect width="18" height="18" rx="5" fill="url(#gCardCob)"/>
            <defs><linearGradient id="gCardCob" x1="0" y1="0" x2="18" y2="18"><stop stopColor="#065F46"/><stop offset="1" stopColor="#10B981"/></linearGradient></defs>
            <rect x="2" y="5" width="14" height="9" rx="2.5" stroke="#6EE7B7" strokeWidth="1.2" fill="none"/>
            <rect x="2" y="8" width="14" height="2.5" fill="#6EE7B7" opacity="0.5"/>
            <rect x="4" y="11" width="4" height="1.5" rx="0.7" fill="#6EE7B7"/>
          </svg>
          <span style={{fontSize:10,fontWeight:700,color:"rgba(110,231,183,0.9)",textTransform:"uppercase",letterSpacing:1}}>Cobro</span>
        </div>
        <svg width="90" height="90" viewBox="0 0 90 90">
          <defs><linearGradient id="gcob4" x1="0" y1="0" x2="1" y2="1"><stop stopColor="#6EE7B7"/><stop offset="1" stopColor="#10B981"/></linearGradient></defs>
          <circle cx="45" cy="45" r="36" fill="none" stroke="rgba(16,185,129,0.12)" strokeWidth="8"/>
          <circle cx="45" cy="45" r="36" fill="none" stroke="url(#gcob4)" strokeWidth="8"
            strokeDasharray={`${(cob/100)*2*Math.PI*36} ${2*Math.PI*36}`} strokeLinecap="round" transform="rotate(-90 45 45)"
            style={{filter:"drop-shadow(0 0 8px rgba(16,185,129,0.9))"}}/> 
          <text x="45" y="48" textAnchor="middle" fill="white" fontSize="18" fontWeight="900">{cob}%</text>
          <text x="45" y="62" textAnchor="middle" fill="rgba(110,231,183,0.7)" fontSize="9">cobrado</text>
        </svg>
        <div style={{textAlign:"center"}}>
          <div style={{fontSize:12,color:"rgba(255,255,255,0.5)"}}>{pag} de {contratos.length} contratos</div>
          {contratos.length===0&&<div style={{fontSize:10,color:"#10B981",marginTop:3,display:"flex",alignItems:"center",gap:3,justifyContent:"center"}}>Sin datos aún <span style={{fontSize:11}}>ⓘ</span></div>}
        </div>
      </div>
    </div>

    {/* ── PRÓXIMOS A VENCER ── */}
    <div style={{marginBottom:16,padding:"20px 20px",background:"linear-gradient(145deg,#13103A,#0E0C2A)",borderRadius:20,border:"1px solid #2A2060",overflow:"hidden",boxShadow:"0 4px 24px rgba(109,40,217,0.12)"}}>
      {/* Header */}
      <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:14}}>
        <IconCal/>
      ...
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:16,gap:8}}>
        ...
        {/* Calendario 3D colorido — igual a la foto */}
        <div style={{flexShrink:0,marginRight:-10,marginBottom:-10}}>
          <svg width="100" height="100" viewBox="0 0 100 100" fill="none">
            ...
            {/* Sombra */}
            <ellipse cx="52" cy="92" rx="32" ry="6" fill="#4C1D95" opacity="0.4"/>
            {/* Cuerpo calendario */}
            <rect x="10" y="20" width="68" height="68" rx="12" fill="url(#calBody)"/>
            {/* Header morado */}
            <rect x="10" y="20" width="68" height="22" rx="12" fill="url(#calHeader)"/>
            <rect x="10" y="30" width="68" height="12" fill="url(#calHeader)"/>
            {/* Espirales */}
            <rect x="28" y="14" width="6" height="16" rx="3" fill="#A78BFA"/>
            <rect x="54" y="14" width="6" height="16" rx="3" fill="#A78BFA"/>
            {/* Grid de días */}
            {[0,1,2,3,4,5,6].map(c=>(
              <rect key={"h"+c} x={14+c*9} y={47} width="7" height="7" rx="2" fill="rgba(196,181,253,0.5)" key={c}/>
            ))}
            {[0,1,2,3,4].map(row=>[0,1,2,3,4,5,6].map(col=>(
              <rect key={row+"-"+col} x={14+col*9} y={57+row*9} width="7" height="7" rx="2" fill={row===1&&col===3?"#A78BFA":"rgba(255,255,255,0.15)"}/>
            )))}
            {/* Check circle */}
            <circle cx="76" cy="76" r="18" fill="#10B981"/>
            <circle cx="76" cy="76" r="16" fill="#059669"/>
            <path d="M68 76 L73 81 L84 70" stroke="white" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round"/>
          </svg>
        </div>
      </div>
      <button onClick={()=>setTab("contratos")} style={{width:"100%",padding:"13px",background:"linear-gradient(135deg,#4F6EF7,#7C5CFC)",border:"none",borderRadius:14,color:"#fff",fontWeight:700,fontSize:14,cursor:"pointer",boxShadow:"0 4px 24px rgba(79,110,247,0.4)",letterSpacing:0.3}}>
        Ver contratos →
      </button>
    </div>
    ...
    <div style={{padding:"18px 20px",background:"linear-gradient(145deg,#0F1828,#0A1220)",borderRadius:20,border:"1px solid #1A2540"}}>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:8}}>
        {[
          {type:"ing",label:"Ingresos",value:fmt(ingMes),sub:"Este mes",color:"#10B981",wave:"M0 20 Q10 14 20 18 Q30 22 40 12 Q50 4 60 8 Q70 12 80 6 L80 30 L0 30Z",waveStroke:"M0 20 Q10 14 20 18 Q30 22 40 12 Q50 4 60 8 Q70 12 80 6",waveColor:"#10B981"},
          {type:"pan",label:"Paneles activos",value:paneles.filter(p=>p.estado==="Ocupado").length,sub:"Total instalados",color:"#3B82F6",wave:"M0 18 Q10 22 20 16 Q30 10 40 14 Q50 18 60 10 Q70 6 80 12 L80 30 L0 30Z",waveStroke:"M0 18 Q10 22 20 16 Q30 10 40 14 Q50 18 60 10 Q70 6 80 12",waveColor:"#3B82F6"},
          {type:"con",label:"Contratos activos",value:contratos.filter(c=>dias(c.fin)>0).length,sub:"Este mes",color:"#8B5CF6",wave:"M0 22 Q10 16 20 20 Q30 24 40 14 Q50 8 60 16 Q70 20 80 10 L80 30 L0 30Z",waveStroke:"M0 22 Q10 16 20 20 Q30 24 40 14 Q50 8 60 16 Q70 20 80 10",waveColor:"#EF4444"},
        ].map(m=>(
          <div key={m.label} style={{background:"linear-gradient(145deg,#0A1020,#060C18)",border:"1px solid rgba(255,255,255,0.06)",borderRadius:16,padding:"14px 8px 0",textAlign:"center",overflow:"hidden"}}>
            ...
            <div style={{fontSize:10,color:"#64748B",marginBottom:8}}>{m.sub}</div>
            {/* Mini gráfica ondulada */}
            <svg width="100%" height="28" viewBox="0 0 80 28" preserveAspectRatio="none">
              <defs>
                <linearGradient id={"wg"+m.type} x1="0" y1="0" x2="0" y2="1">
                  <stop offset="0%" stopColor={m.waveColor} stopOpacity="0.3"/>
                  <stop offset="100%" stopColor={m.waveColor} stopOpacity="0"/>
                </linearGradient>
              </defs>
              <path d={m.wave} fill={`url(#wg${m.type})`}/>
              <path d={m.waveStroke} stroke={m.waveColor} strokeWidth="1.5" fill="none" strokeLinecap="round"/>
            </svg>
          </div>
        ))}
      </div>
...
function SideIcon({id,label,active,onClick}){
  return(
    <button onClick={onClick} title={label} style={{
      ...
      boxShadow:active?"0 4px 20px rgba(109,40,217,0.45), inset 0 1px 0 rgba(255,255,255,0.15)":"none",
      position:"relative",
      marginBottom:2,
    }}>
      <span style={{flexShrink:0,display:"flex",alignItems:"center",color:active?"#FFFFFF":"rgba(255,255,255,0.4)"}}>
        {ICONS[id]||ICONS.dashboard}
      </span>
      <span style={{fontSize:14,fontWeight:active?700:500,letterSpacing:0.1,whiteSpace:"nowrap",color:active?"#fff":"rgba(255,255,255,0.55)"}}>
        {label}
      </span>
    </button>
  );
  ...
  return(<>
    ...
    <style>{`
      *{box-sizing:border-box;-webkit-tap-highlight-color:transparent}
      html{background:#080C14;height:100%;height:-webkit-fill-available}
      body{margin:0;padding:0;background:#080C14;overflow:hidden;width:100%;height:100%;height:-webkit-fill-available;-webkit-text-size-adjust:100%}
      #root{width:100%;height:100%;height:-webkit-fill-available;display:flex;flex-direction:column;overflow:hidden;box-sizing:border-box}
      ...
      @media(min-width:769px){
        ...
        .main-with-sidebar{margin-left:200px!important}
        .bottom-nav{display:none!important}
        .main-content{
    ...
    <div style={{fontFamily:"'DM Sans',sans-serif",background:"#080C14",flex:1,minHeight:0,color:C.text,display:"flex",overflow:"hidden",alignItems:"stretch",position:"relative"}}>
      ...
      <aside className="sidebar-desktop" style={{
        ...
        {/* Logo */}
        <div style={{padding:"22px 18px 18px",display:"flex",justifyContent:"flex-start",alignItems:"center"}}>
          <Logo360 width={110}/>
        </div>

        {/* Nav items */}
        <div style={{flex:1,width:"100%",overflowY:"auto",scrollbarWidth:"none",display:"flex",flexDirection:"column",gap:4,padding:"8px 12px 0"}}>
          {tabs.map(t=>(
        ...
        {/* Bottom — notifs + perfil */}
        <div style={{padding:"12px 12px",borderTop:"1px solid rgba(255,255,255,0.05)",paddingBottom:"calc(12px + env(safe-area-inset-bottom))"}}>
          <button style={{width:"100%",marginBottom:4,display:"flex",alignItems:"center",gap:10,padding:"10px 14px",background:"none",border:"none",cursor:"pointer",color:"rgba(255,255,255,0.45)",borderRadius:14}}>
            ...
            <span style={{fontSize:13,fontWeight:500,flex:1,textAlign:"left",color:"rgba(255,255,255,0.5)"}}>Notificaciones</span>
            <span style={{background:"#7C3AED",color:"#fff",borderRadius:20,padding:"2px 8px",fontSize:11,fontWeight:800}}>3</span>
          </button>

          <button onClick={()=>setShowProfile(v=>!v)} style={{width:"100%",display:"flex",alignItems:"center",gap:10,padding:"10px 14px",background:showProfile?"rgba(79,110,247,0.1)":"none",border:showProfile?"1px solid rgba(79,110,247,0.2)":"1px solid transparent",cursor:"pointer",borderRadius:14,transition:"all 0.15s"}}>
            <div style={{position:"relative",flexShrink:0}}>
              <img src="https://ui-avatars.com/api/?name=Alan+Martinez&background=4F46E5&color=fff&size=64&bold=true" style={{width:36,height:36,borderRadius:"50%",border:"2px solid rgba(79,110,247,0.5)",objectFit:"cover"}} alt="AM"/>
              <div style={{position:"absolute",bottom:0,right:0,width:10,height:10,borderRadius:"50%",background:"#10B981",border:"2px solid #0D1117"}}/>
            </div>
            <div style={{flex:1,textAlign:"left",minWidth:0}}>
              <div style={{fontSize:13,fontWeight:700,color:"rgba(255,255,255,0.9)",whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>Alan Martínez</div>
              <div style={{fontSize:11,color:"rgba(255,255,255,0.35)",display:"flex",alignItems:"center",gap:4}}>Administrador <span style={{fontSize:10}}>∨</span></div>
            </div>
          </button>
        </div>
      </aside>

      {/* ── HEADER MÓVIL — UNA SOLA FILA: logo | tabs scroll | avatar+campana ── */}
      <div className="mobile-header" style={{
        ...
        {/* Tabs — scroll horizontal, ocupa espacio central */}
        <div style={{flex:1,display:"flex",alignItems:"center",overflowX:"auto",WebkitOverflowScrolling:"touch",scrollbarWidth:"none",gap:4,paddingBottom:0}}>
          {tabs.map(t=>{
            const isActive=activeTab===t.id;
            return(
              <button key={t.id} onClick={()=>{setTab(t.id);setShowProfile(false);}} style={{
                display:"flex",flexDirection:"column",alignItems:"center",gap:1,
                padding:"6px 10px",
                background:isActive?"linear-gradient(135deg,#6D28D9 0%,#7C3AED 100%)":"none",
                border:"none",
                borderRadius:12,cursor:"pointer",flexShrink:0,
                transition:"all 0.18s",
                boxShadow:isActive?"0 3px 12px rgba(109,40,217,0.4)":"none",
              }}>
                <span style={{display:"flex",alignItems:"center",justifyContent:"center",width:20,height:20,color:isActive?"#FFFFFF":"rgba(255,255,255,0.45)"}}>
                  {ICONS[t.id]||ICONS.dashboard}
                </span>
                <span style={{fontSize:9,fontWeight:isActive?700:500,whiteSpace:"nowrap",color:isActive?"#FFFFFF":"rgba(255,255,255,0.4)",letterSpacing:0.2}}>
                  {t.label}
                </span>
              </button>
            );
        ...
        {/* Avatar + Campana — fijo derecha */}
        <div style={{flexShrink:0,display:"flex",alignItems:"center",gap:8}}>
          <button onClick={()=>setShowProfile(v=>!v)} style={{background:"none",border:"none",cursor:"pointer",padding:0}}>
            <div style={{position:"relative"}}>
              <div style={{width:32,height:32,borderRadius:"50%",background:"linear-gradient(135deg,#4F46E5,#7C3AED)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:11,fontWeight:800,color:"#fff",border:showProfile?"2px solid #818CF8":"2px solid rgba(79,110,247,0.5)"}}>AM</div>
              <div style={{position:"absolute",bottom:0,right:0,width:8,height:8,borderRadius:"50%",background:"#10B981",border:"1.5px solid #080C14"}}/>
            </div>
          </button>
          <div style={{position:"relative"}}>
            ...
            <span style={{position:"absolute",top:-4,right:-4,background:"#7C3AED",color:"#fff",borderRadius:10,padding:"1px 5px",fontSize:9,fontWeight:800,lineHeight:1.4}}>3</span>
          </div>
        </div>
      </div>
      ...
      {/* ── MAIN CONTENT ── */}
      <div className="main-with-sidebar" style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden",background:"#080C14"}}>
        <main className="main-content" style={{flex:1,overflowY:"auto",overflowX:"hidden",padding:"16px",paddingTop:"calc(env(safe-area-inset-top) + 16px)",paddingBottom:"env(safe-area-inset-bottom)",WebkitOverflowScrolling:"touch",scrollBehavior:"smooth",background:"#080C14"}}>

          {/* ── PERFIL / SISTEMA PAGE ── */}
          {showProfile&&(
            <div>
              <div style={{display:"flex",alignItems:"center",gap:16,marginBottom:24,padding:"20px",background:"linear-gradient(135deg,#1E293B,#0F172A)",borderRadius:16,border:"1px solid rgba(255,255,255,0.08)"}}>
      ...
      <nav className="bottom-nav" style={{
        display:"none", /* CSS media query lo activa en móvil */
        ...
        {/* Barra de blur */}
        <div style={{backdropFilter:"blur(24px)",WebkitBackdropFilter:"blur(24px)",background:"rgba(8,12,20,0.94)",borderTop:"1px solid rgba(255,255,255,0.08)",padding:"6px 0 4px"}}>
          {/* Fila de tabs — scrollable horizontal */}
          <div className="bottom-nav-inner" style={{display:"flex",alignItems:"center",justifyContent:"space-around",paddingLeft:4,paddingRight:4}}>
            {tabs.map(t=>{
              const isActive=activeTab===t.id;
              return(
                <button key={t.id} onClick={()=>{setTab(t.id);setShowProfile(false);}} style={{
                  ...
                  minWidth:46,flex:"0 0 auto",position:"relative",
                  transition:"color 0.15s",
                }}>
                  {isActive&&<div style={{position:"absolute",top:0,left:"50%",transform:"translateX(-50%)",width:24,height:2.5,background:"linear-gradient(90deg,#4F6EF7,#7C5CFC)",borderRadius:2}}/>}
                  <span style={{display:"flex",alignItems:"center",justifyContent:"center",width:22,height:22,color:isActive?"#818CF8":"rgba(255,255,255,0.35)"}}>
                    {ICONS[t.id]||ICONS.dashboard}
                  </span>
                  <span style={{fontSize:9,fontWeight:isActive?700:500,letterSpacing:0.2,whiteSpace:"nowrap",color:isActive?"#818CF8":"rgba(255,255,255,0.35)"}}>
                    {t.label}
                  </span>
                </button>
              );
            })}
            {/* Perfil en bottom nav */}
            <button onClick={()=>setShowProfile(v=>!v)} style={{
              display:"flex",flexDirection:"column",alignItems:"center",gap:2,
              padding:"5px 8px",background:"none",border:"none",cursor:"pointer",
              minWidth:46,flex:"0 0 auto",position:"relative",
            }}>
              {showProfile&&<div style={{position:"absolute",top:0,left:"50%",transform:"translateX(-50%)",width:24,height:2.5,background:"linear-gradient(90deg,#4F6EF7,#7C5CFC)",borderRadius:2}}/>}
              <div style={{position:"relative"}}>
                <div style={{width:24,height:24,borderRadius:"50%",background:"linear-gradient(135deg,#4F46E5,#7C3AED)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:10,fontWeight:800,color:"#fff",border:showProfile?"2px solid #818CF8":"2px solid transparent"}}>AM</div>
                <div style={{position:"absolute",bottom:-1,right:-1,width:7,height:7,borderRadius:"50%",background:"#10B981",border:"1.5px solid #080C14"}}/>
              </div>
              <span style={{fontSize:9,fontWeight:showProfile?700:500,color:showProfile?"#818CF8":"rgba(255,255,255,0.35)"}}>Perfil</span>
            </button>
          </div>
        </div>
      </nav>
