...
// ── Sidebar icon button ──
function SideIcon({t,active,onClick}){
  return(
    <button onClick={onClick} title={t.label} style={{
      ...
      <span style={{fontSize:10,fontWeight:active?700:400,letterSpacing:0.2}}>{t.label}</span>
    </button>
  );
}
...
export default function App(){
  ...
  const tabs=[
    ...
    {id:"paneles",icon:"🖥️",label:"Paneles"},
    {id:"contratos",icon:"📋",label:"Contratos"},
    {id:"crm",icon:"👥",label:"Clientes"},
  ...
  return(<>
    ...
    {showProfile&&(
      <div style={{position:"fixed",inset:0,zIndex:500,display:"flex"}} onClick={()=>setShowProfile(false)}>
        <div style={{position:"absolute",left:72,bottom:0,width:300,background:"#111827",border:"1px solid rgba(255,255,255,0.12)",borderRadius:"0 16px 0 0",padding:24,boxShadow:"4px -4px 24px #000A"}} onClick={e=>e.stopPropagation()}>
          {/* User info */}
          <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:20,paddingBottom:16,borderBottom:"1px solid rgba(255,255,255,0.08)"}}>
            <img src="https://ui-avatars.com/api/?name=Alan+Martinez&background=3B82F6&color=fff&size=48" style={{width:48,height:48,borderRadius:"50%"}} alt="avatar"/>
            <div>
              <div style={{fontWeight:700,fontSize:15,color:"#fff"}}>Alan Martínez</div>
              <div style={{fontSize:12,color:"rgba(255,255,255,0.45)"}}>Administrador</div>
            </div>
          </div>

          {/* Firebase info */}
          <div style={{marginBottom:12}}>
            <div style={{fontSize:11,fontWeight:700,color:"rgba(255,255,255,0.4)",letterSpacing:1,marginBottom:8}}>BASE DE DATOS</div>
            <div style={{background:"#1E293B",borderRadius:10,padding:12,fontSize:12,color:"rgba(255,255,255,0.7)",lineHeight:1.8}}>
              🔥 <strong style={{color:"#F59E0B"}}>Firebase Firestore</strong><br/>
              <span style={{color:"rgba(255,255,255,0.4)"}}>Plan Spark · 1 GiB · 50k lecturas/día</span>
            </div>
          </div>

          {/* Cloudinary info */}
          <div style={{marginBottom:20}}>
            <div style={{fontSize:11,fontWeight:700,color:"rgba(255,255,255,0.4)",letterSpacing:1,marginBottom:8}}>FOTOS DE BOLETAS</div>
            <div style={{background:"#1E293B",borderRadius:10,padding:12,fontSize:12,color:"rgba(255,255,255,0.7)",lineHeight:1.8}}>
              📸 <strong style={{color:"#3B82F6"}}>Cloudinary</strong><br/>
              <span style={{color:"rgba(255,255,255,0.4)"}}>Plan gratuito · 25 GB · WebP ~25 KB/foto</span><br/>
              <span style={{color:"rgba(255,255,255,0.4)"}}>Cloud: djwhrurww</span>
            </div>
          </div>

          <button onClick={()=>setShowProfile(false)} style={{width:"100%",background:"#1E293B",border:"1px solid rgba(255,255,255,0.1)",borderRadius:10,padding:"10px",color:"rgba(255,255,255,0.6)",cursor:"pointer",fontSize:13}}>Cerrar</button>
        </div>
      </div>
    )}

    <div style={{fontFamily:"'DM Sans',sans-serif",background:C.bg,height:"100dvh",color:C.text,display:"flex",overflow:"hidden"}}>

      {/* ── SIDEBAR ── */}
      <aside style={{
        width:72,flexShrink:0,background:"#0A0F1E",
        ...
        {/* Logo */}
        <div style={{padding:"16px 0 12px",display:"flex",alignItems:"center",justifyContent:"center",borderBottom:"1px solid rgba(255,255,255,0.06)",width:"100%",marginBottom:8}}>
          <Logo360 width={42}/>
        </div>

        {/* Nav tabs */}
        <div style={{flex:1,width:"100%",overflowY:"auto",scrollbarWidth:"none"}}>
          {tabs.map(t=><SideIcon key={t.id} t={t} active={tab===t.id} onClick={()=>setTab(t.id)}/>)}
        </div>

        {/* Profile button at bottom */}
        <div style={{width:"100%",borderTop:"1px solid rgba(255,255,255,0.06)",paddingBottom:"env(safe-area-inset-bottom)"}}>
          <button onClick={()=>setShowProfile(v=>!v)} style={{
            width:"100%",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",
            gap:4,padding:"12px 0",background:"none",border:"none",cursor:"pointer",
          }}>
            <img src="https://ui-avatars.com/api/?name=Alan+Martinez&background=3B82F6&color=fff&size=36" style={{width:36,height:36,borderRadius:"50%",border:"2px solid rgba(59,130,246,0.5)"}} alt="perfil"/>
            <span style={{fontSize:9,color:"rgba(255,255,255,0.4)",fontWeight:500}}>Alan</span>
          </button>
        </div>
      </aside>
      ...
      <div style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden"}}>
        <main style={{flex:1,overflowY:"auto",overflowX:"hidden",padding:"20px 16px",paddingBottom:"calc(20px + env(safe-area-inset-bottom))",WebkitOverflowScrolling:"touch"}}>
          {error&&(
            <div style={{background:"#1E293B",border:`1px solid ${C.amber}55`,borderRadius:14,padding:20,marginBottom:24,display:"flex",gap:16,alignItems:"flex-start"}}>
              ...
              <div>
                ...
                <div style={{fontSize:13,color:C.muted,lineHeight:1.8}}>
                  ...
                  <strong style={{color:C.white}}>Esto es normal</strong> — el código está 100% listo con Firebase.
                </div>
              </div>
            </div>
          )}
