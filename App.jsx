...
const DESTINOS_DEFAULT = [
  { id:"reinversion", label:"Reinversión", emoji:"🔄", color:"#3B82F6" },
  { id:"ahorro",      label:"Ahorro",      emoji:"🏦", color:"#10B981" },
  { id:"operacion",   label:"Operación",   emoji:"⚙️",  color:"#8B5CF6" },
  { id:"reserva",     label:"Reserva",     emoji:"🛡️",  color:"#F59E0B" },
];
...
function Capital({ paneles, contratos, gastos, proveedores }) {
  ...
  // Destinos de inversión guardados en Firebase
  const [destinos, setDestinos] = useState(DESTINOS_DEFAULT.map(d => ({ ...d, pct: 0, monto: 0 })));
  const [editDestinos, setEditDestinos] = useState(false);
  ...
  useEffect(() => {
    (async () => {
      try {
        const snap = await getDoc(doc(db, "configuracion", "capital"));
        if (snap.exists()) {
          setCapital(Number(snap.data().valor || 0));
          if (snap.data().destinos) setDestinos(snap.data().destinos);
        }
      } catch {}
  ...
  const abrirEditor = () => { setInputVal(capital === 0 ? "" : String(capital)); setEditando(true); setTimeout(() => inputRef.current?.focus(), 50); };
  const guardarCapital = async () => {
    const n = Number(String(inputVal).replace(/[^0-9.]/g, ""));
    const nuevo = isNaN(n) ? capital : n;
    setSaving(true);
    try {
      await setDoc(doc(db, "configuracion", "capital"), { valor: nuevo, destinos, updatedAt: serverTimestamp() }, { merge: true });
      setCapital(nuevo);
    } catch {}
    setSaving(false);
    setEditando(false);
  ...
  const abrirDestinos = () => {
    setDestTemp(destinos.map(d => ({ ...d })));
    setEditDestinos(true);
  };
  const guardarDestinos = async () => {
    // Normalizar para que la suma de pct = 100
    const total = destTemp.reduce((s, d) => s + Number(d.pct || 0), 0);
    const norm = destTemp.map(d => ({
      ...d,
      pct: total > 0 ? Math.round((Number(d.pct||0)/total)*100) : 0,
    }));
    setSavingDest(true);
    try {
      await setDoc(doc(db, "configuracion", "capital"), { destinos: norm }, { merge: true });
      setDestinos(norm);
    } catch {}
    setSavingDest(false);
  ...
  const gastoMes  = gastos.filter(g => !g.deleted && (g.fecha||"").slice(0,7) === mesActual).reduce((s,g) => s + Number(g.monto||0), 0);
  const capitalReal = capital + ingMes;
  const disponible  = capitalReal - gastoMes;
  const diff = ingMes - ingPrev;
  const pct  = ingPrev > 0 ? Math.round((diff/ingPrev)*100) : ingMes > 0 ? 100 : 0;

  // Distribuir disponible según porcentajes
  const destinosConMonto = destinos.map(d => ({
    ...d,
    montoCalc: disponible > 0 ? (disponible * (Number(d.pct||0)/100)) : 0,
  }));
  const totalPct = destinos.reduce((s,d) => s + Number(d.pct||0), 0);

  // Sparkline
  const meses6 = Array.from({ length: 6 }, (_, i) => {
    const d = new Date(); d.setDate(1); d.setMonth(d.getMonth() - (5 - i));
    return { key: `${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,"0")}`, lbl: ["E","F","M","A","M","J","J","A","S","O","N","D"][d.getMonth()] };
  });
  const puntos = meses6.map(({ key }) =>
    contratos.filter(c => !c.deleted && c.pagado && c.inicio?.slice(0,7) === key).reduce((s,c) => s + Number(c.monto||0), 0)
  );
  const maxP = Math.max(...puntos, 1);
  const W = 260, H = 54;
  const coords = puntos.map((v, i) => ({ x: (i/(puntos.length-1))*W, y: H - (v/maxP)*(H-8) - 4 }));
  const linePath = coords.map((p,i) => `${i===0?"M":"L"}${p.x.toFixed(1)},${p.y.toFixed(1)}`).join(" ");
  const areaPath = `${linePath} L${W},${H} L0,${H} Z`;

  const fmtM = (n) => `S/ ${Number(n||0).toLocaleString("es-PE",{minimumFractionDigits:2,maximumFractionDigits:2})}`;
  const CARD = { background:"linear-gradient(145deg,#111D3C,#0D1629)", border:"1px solid rgba(99,140,255,0.09)", borderRadius:20, boxShadow:"0 4px 20px rgba(0,0,0,0.3)" };

  return (
    <div style={{ padding:"20px 16px", paddingBottom:"calc(140px + env(safe-area-inset-bottom))" }}>

      {/* ── HEADER ── */}
      <div style={{ marginBottom:18 }}>
        <div style={{ fontSize:22, fontWeight:900, color:"#fff", letterSpacing:"-0.03em" }}>Capital e Inversiones</div>
        <div style={{ fontSize:12, color:"rgba(148,175,255,0.5)", marginTop:2 }}>Resumen financiero del negocio</div>
      </div>

      {/* ── HERO CAPITAL TOTAL ── */}
      <div style={{ ...CARD, padding:"22px 20px 18px", marginBottom:12 }}>
        <div style={{ fontSize:9, fontWeight:700, color:"rgba(148,175,255,0.4)", letterSpacing:2.5, textTransform:"uppercase", marginBottom:6 }}>Capital total disponible</div>

        {editando ? (
          <>
            <div style={{ display:"flex", alignItems:"center", gap:8, marginBottom:14 }}>
              <span style={{ fontSize:22, fontWeight:900, color:"rgba(255,255,255,0.2)" }}>S/</span>
              <input ref={inputRef} type="number" inputMode="decimal" value={inputVal}
                onChange={e => setInputVal(e.target.value)}
                onKeyDown={e => { if(e.key==="Enter") guardarCapital(); if(e.key==="Escape") setEditando(false); }}
                placeholder="0.00"
                style={{ flex:1, background:"rgba(255,255,255,0.07)", border:"1.5px solid rgba(99,140,255,0.5)", borderRadius:12, padding:"11px 14px", color:"#fff", fontSize:26, fontWeight:900, outline:"none", fontFamily:"inherit", letterSpacing:"-1px" }}
              />
            </div>
            <div style={{ display:"flex", gap:8 }}>
              <button onClick={()=>setEditando(false)} style={{ flex:1, padding:12, background:"rgba(255,255,255,0.05)", border:"1px solid rgba(255,255,255,0.1)", borderRadius:11, color:"rgba(255,255,255,0.4)", fontWeight:600, fontSize:13, cursor:"pointer", touchAction:"manipulation", fontFamily:"inherit" }}>Cancelar</button>
              <button onClick={guardarCapital} disabled={saving} style={{ flex:2, padding:12, background:"#2563EB", border:"none", borderRadius:11, color:"#fff", fontWeight:700, fontSize:13, cursor:"pointer", touchAction:"manipulation", fontFamily:"inherit", opacity:saving?0.7:1 }}>{saving?"Guardando…":"Guardar"}</button>
            </div>
          </>
        ) : (
          <>
            <div style={{ fontSize:44, fontWeight:900, color:"#fff", letterSpacing:"-2px", lineHeight:1, marginBottom:6 }}>
              {capitalLoaded ? fmtM(capitalReal) : "—"}
            </div>
            <div style={{ display:"flex", alignItems:"center", gap:8, marginBottom:16 }}>
              <span style={{ fontSize:11, fontWeight:700, color:diff>=0?"#34D399":"#F87171", background:diff>=0?"rgba(52,211,153,0.12)":"rgba(248,113,113,0.12)", borderRadius:99, padding:"3px 9px" }}>
                {pct>=0?"+":""}{pct}% vs mes ant.
              </span>
              <span style={{ fontSize:11, color:"rgba(148,175,255,0.4)" }}>Base {fmtM(capital)}</span>
            </div>

            {/* Sparkline */}
            <svg viewBox={`0 0 ${W} ${H}`} width="100%" height={H} style={{ display:"block", marginBottom:6, overflow:"visible" }}>
              <defs><linearGradient id="cg3" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="#3B82F6" stopOpacity="0.25"/><stop offset="100%" stopColor="#3B82F6" stopOpacity="0"/></linearGradient></defs>
              <path d={areaPath} fill="url(#cg3)"/>
              <path d={linePath} fill="none" stroke="#3B82F6" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
              {coords.map((p,i) => <circle key={i} cx={p.x} cy={p.y} r={i===5?4:2} fill={i===5?"#60A5FA":"rgba(96,165,250,0.4)"}/>)}
            </svg>
            <div style={{ display:"flex" }}>
              {meses6.map(({lbl},i) => <span key={i} style={{ fontSize:9, color:i===5?"rgba(255,255,255,0.7)":"rgba(148,175,255,0.25)", fontWeight:i===5?700:400, flex:1, textAlign:"center" }}>{lbl}</span>)}
            </div>

            <button onClick={abrirEditor} style={{ marginTop:14, display:"inline-flex", alignItems:"center", gap:6, background:"rgba(37,99,235,0.15)", border:"1px solid rgba(99,140,255,0.2)", borderRadius:99, padding:"7px 14px", color:"#93C5FD", fontWeight:700, fontSize:11, cursor:"pointer", touchAction:"manipulation", fontFamily:"inherit" }}>
              <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round"><path d="M17 3a2.828 2.828 0 114 4L7.5 20.5 2 22l1.5-5.5L17 3z"/></svg>
              Editar capital base
            </button>
          </>
        )}
      ...
      {/* ── PANEL GENERADO ESTE MES ── */}
      <div style={{ ...CARD, padding:"18px 20px", marginBottom:12 }}>
        <div style={{ fontSize:9, fontWeight:700, color:"rgba(52,211,153,0.5)", letterSpacing:2, textTransform:"uppercase", marginBottom:10 }}>Generado este mes</div>
        <div style={{ display:"flex", alignItems:"flex-end", gap:14, marginBottom:14 }}>
          <div>
            <div style={{ fontSize:32, fontWeight:900, color:"#34D399", letterSpacing:"-1.5px", lineHeight:1 }}>{fmtM(ingMes)}</div>
            <div style={{ fontSize:11, color:"rgba(148,175,255,0.4)", marginTop:4 }}>de contratos cobrados</div>
          </div>
          <div style={{ flex:1 }}/>
          <div style={{ textAlign:"right" }}>
            <div style={{ fontSize:13, fontWeight:800, color:"#F87171" }}>-{fmtM(gastoMes)}</div>
            <div style={{ fontSize:10, color:"rgba(148,175,255,0.4)" }}>gastos</div>
          </div>
        </div>
        ...
        {ingMes > 0 && (
          <div style={{ marginBottom:14 }}>
            <div style={{ height:8, borderRadius:99, background:"rgba(255,255,255,0.06)", overflow:"hidden" }}>
              <div style={{ height:"100%", width:`${Math.min((gastoMes/ingMes)*100,100)}%`, background:"linear-gradient(90deg,#EF4444,#F87171)", borderRadius:99, transition:"width .5s" }}/>
            </div>
            <div style={{ display:"flex", justifyContent:"space-between", marginTop:5 }}>
              <span style={{ fontSize:10, color:"rgba(148,175,255,0.4)" }}>Gastos {ingMes>0?Math.round((gastoMes/ingMes)*100):0}%</span>
              <span style={{ fontSize:10, color:"rgba(52,211,153,0.6)", fontWeight:700 }}>Libre {fmtM(Math.max(ingMes-gastoMes,0))}</span>
            </div>
          </div>
        )}

        {/* Dos chips: por cobrar / disponible */}
        <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:8 }}>
          <div style={{ background:"rgba(245,158,11,0.08)", border:"1px solid rgba(245,158,11,0.18)", borderRadius:12, padding:"10px 12px" }}>
            <div style={{ fontSize:9, color:"rgba(245,158,11,0.55)", fontWeight:700, letterSpacing:1.2, textTransform:"uppercase", marginBottom:4 }}>Por cobrar</div>
            <div style={{ fontSize:15, fontWeight:900, color:"#FCD34D" }}>{fmtM(porCobrar)}</div>
          </div>
          <div style={{ background:"rgba(59,130,246,0.08)", border:"1px solid rgba(59,130,246,0.18)", borderRadius:12, padding:"10px 12px" }}>
            <div style={{ fontSize:9, color:"rgba(96,165,250,0.55)", fontWeight:700, letterSpacing:1.2, textTransform:"uppercase", marginBottom:4 }}>Disponible</div>
            <div style={{ fontSize:15, fontWeight:900, color:"#60A5FA" }}>{fmtM(Math.max(disponible,0))}</div>
          </div>
        </div>
      </div>
      ...
      <div style={{ ...CARD, padding:"18px 20px", marginBottom:12 }}>
        <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:14 }}>
          <div>
            <div style={{ fontSize:9, fontWeight:700, color:"rgba(148,175,255,0.4)", letterSpacing:2, textTransform:"uppercase" }}>Destino del dinero</div>
            <div style={{ fontSize:11, color:"rgba(148,175,255,0.35)", marginTop:2 }}>Total asignado: {totalPct}%</div>
          </div>
          <button onClick={abrirDestinos} style={{ display:"inline-flex", alignItems:"center", gap:5, background:"rgba(37,99,235,0.15)", border:"1px solid rgba(99,140,255,0.22)", borderRadius:99, padding:"6px 12px", color:"#93C5FD", fontWeight:700, fontSize:11, cursor:"pointer", touchAction:"manipulation", fontFamily:"inherit" }}>
            <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round"><path d="M17 3a2.828 2.828 0 114 4L7.5 20.5 2 22l1.5-5.5L17 3z"/></svg>
            Ajustar
          </button>
        </div>
        ...
        {totalPct > 0 && (
          <div style={{ height:10, borderRadius:99, overflow:"hidden", display:"flex", marginBottom:14, gap:2 }}>
            {destinosConMonto.filter(d=>d.pct>0).map(d => (
              <div key={d.id} style={{ flex:d.pct, background:d.color, minWidth:4, borderRadius:99 }}/>
            ))}
          </div>
        )}
        ...
        <div style={{ display:"flex", flexDirection:"column", gap:8 }}>
          {destinosConMonto.map(d => (
            <div key={d.id} style={{ display:"flex", alignItems:"center", gap:12, padding:"10px 12px", borderRadius:12, background:"rgba(255,255,255,0.04)", border:"1px solid rgba(255,255,255,0.06)" }}>
              <div style={{ width:34, height:34, borderRadius:10, background:`${d.color}18`, border:`1px solid ${d.color}33`, display:"flex", alignItems:"center", justifyContent:"center", fontSize:16, flexShrink:0 }}>{d.emoji}</div>
              <div style={{ flex:1, minWidth:0 }}>
                <div style={{ fontSize:13, fontWeight:700, color:"rgba(255,255,255,0.85)" }}>{d.label}</div>
                <div style={{ fontSize:11, color:"rgba(148,175,255,0.4)" }}>{d.pct}% del disponible</div>
              </div>
              <div style={{ textAlign:"right", flexShrink:0 }}>
                <div style={{ fontSize:14, fontWeight:900, color:d.color }}>{fmtM(d.montoCalc)}</div>
              </div>
            </div>
          ))}
        </div>
        ...
        {totalPct === 0 && (
          <div style={{ textAlign:"center", padding:"16px 0", color:"rgba(148,175,255,0.35)", fontSize:12 }}>
            Toca <strong style={{ color:"#93C5FD" }}>Ajustar</strong> para definir cómo distribuir tu dinero
          </div>
        )}
      </div>
      ...
      {(() => {
        const activos = contratos.filter(c => !c.deleted && Math.ceil((new Date(c.fin) - hoy)/86400000) > 0);
        if (!activos.length) return null;
        return (
          <div style={{ ...CARD, padding:"16px 18px" }}>
            <div style={{ fontSize:9, color:"rgba(148,175,255,0.4)", fontWeight:700, letterSpacing:1.5, textTransform:"uppercase", marginBottom:12 }}>Contratos activos · {activos.length}</div>
            {activos.map(c => {
              ...
              return (
                <div key={c.id} style={{ display:"flex", alignItems:"center", gap:10, padding:"9px 0", borderBottom:"1px solid rgba(255,255,255,0.05)" }}>
                  <div style={{ width:7, height:7, borderRadius:"50%", background:d<=30?"#3B82F6":"#34D399", flexShrink:0 }}/>
                  <div style={{ flex:1, minWidth:0 }}>
                    <div style={{ fontSize:13, fontWeight:700, color:"rgba(255,255,255,0.85)", whiteSpace:"nowrap", overflow:"hidden", textOverflow:"ellipsis" }}>
                      {paneles.find(p=>p.id===c.panel_id)?.nombre || "Panel"}
                    </div>
                    <div style={{ fontSize:11, color:"rgba(148,175,255,0.4)" }}>Vence en {d} día{d!==1?"s":""}</div>
                  </div>
                  <div style={{ fontSize:13, fontWeight:800, color:"#93C5FD" }}>{fmtM(c.monto)}</div>
                </div>
              );
            })}
          </div>
      ...
      {/* ── MODAL: AJUSTAR DESTINOS ── */}
      {editDestinos && (
        <div onClick={e => e.target===e.currentTarget && setEditDestinos(false)}
          style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.6)", zIndex:600, display:"flex", alignItems:"flex-end", justifyContent:"center" }}>
          <div style={{ width:"100%", maxWidth:540, background:"#0D1629", borderRadius:"22px 22px 0 0", padding:"20px 20px calc(24px + env(safe-area-inset-bottom))", boxShadow:"0 -8px 40px rgba(0,0,0,0.5)" }}>
            <div style={{ display:"flex", justifyContent:"center", marginBottom:14 }}>
              <div style={{ width:36, height:4, borderRadius:2, background:"rgba(255,255,255,0.15)" }}/>
            </div>
            <div style={{ fontSize:17, fontWeight:800, color:"#fff", marginBottom:4 }}>Destino del dinero</div>
            <div style={{ fontSize:12, color:"rgba(148,175,255,0.45)", marginBottom:20 }}>Elige qué % de tu dinero disponible ({fmtM(Math.max(disponible,0))}) va a cada categoría</div>

            <div style={{ display:"flex", flexDirection:"column", gap:10, marginBottom:20 }}>
              {destTemp.map((d, i) => (
                <div key={d.id} style={{ display:"flex", alignItems:"center", gap:12, padding:"12px 14px", borderRadius:14, background:"rgba(255,255,255,0.05)", border:"1px solid rgba(255,255,255,0.08)" }}>
                  <div style={{ width:36, height:36, borderRadius:10, background:`${d.color}18`, display:"flex", alignItems:"center", justifyContent:"center", fontSize:18, flexShrink:0 }}>{d.emoji}</div>
                  <div style={{ flex:1 }}>
                    <div style={{ fontSize:13, fontWeight:700, color:"#fff", marginBottom:4 }}>{d.label}</div>
                    <div style={{ fontSize:11, color:d.color, fontWeight:600 }}>{fmtM(disponible > 0 ? (disponible*(Number(d.pct||0)/100)) : 0)}</div>
                  </div>
                  <div style={{ display:"flex", alignItems:"center", gap:6 }}>
                    <button onClick={() => { const nd=[...destTemp]; nd[i]={...nd[i],pct:Math.max(0,Number(nd[i].pct||0)-5)}; setDestTemp(nd); }}
                      style={{ width:30,height:30,borderRadius:8,background:"rgba(255,255,255,0.07)",border:"1px solid rgba(255,255,255,0.1)",color:"#fff",fontSize:16,cursor:"pointer",touchAction:"manipulation",display:"flex",alignItems:"center",justifyContent:"center",fontFamily:"inherit" }}>−</button>
                    <span style={{ width:36,textAlign:"center",fontSize:15,fontWeight:800,color:"#fff" }}>{d.pct||0}%</span>
                    <button onClick={() => { const nd=[...destTemp]; nd[i]={...nd[i],pct:Math.min(100,Number(nd[i].pct||0)+5)}; setDestTemp(nd); }}
                      style={{ width:30,height:30,borderRadius:8,background:"rgba(255,255,255,0.07)",border:"1px solid rgba(255,255,255,0.1)",color:"#fff",fontSize:16,cursor:"pointer",touchAction:"manipulation",display:"flex",alignItems:"center",justifyContent:"center",fontFamily:"inherit" }}>+</button>
                  </div>
                </div>
              ))}
            ...
            <div style={{ height:8, borderRadius:99, overflow:"hidden", display:"flex", gap:2, marginBottom:6 }}>
              {destTemp.filter(d=>Number(d.pct)>0).map(d=>(
                <div key={d.id} style={{ flex:Number(d.pct), background:d.color, borderRadius:99 }}/>
              ))}
              {destTemp.reduce((s,d)=>s+Number(d.pct||0),0) < 100 && (
                <div style={{ flex:100-destTemp.reduce((s,d)=>s+Number(d.pct||0),0), background:"rgba(255,255,255,0.08)", borderRadius:99 }}/>
              )}
            </div>
            <div style={{ fontSize:11, color:"rgba(148,175,255,0.4)", marginBottom:20, textAlign:"center" }}>
              Asignado: {destTemp.reduce((s,d)=>s+Number(d.pct||0),0)}% · Sin asignar: {Math.max(0,100-destTemp.reduce((s,d)=>s+Number(d.pct||0),0))}%
            </div>

            <div style={{ display:"flex", gap:10 }}>
              <button onClick={()=>setEditDestinos(false)} style={{ flex:1,padding:13,background:"rgba(255,255,255,0.06)",border:"1px solid rgba(255,255,255,0.1)",borderRadius:12,color:"rgba(255,255,255,0.4)",fontWeight:600,fontSize:14,cursor:"pointer",touchAction:"manipulation",fontFamily:"inherit" }}>Cancelar</button>
              <button onClick={guardarDestinos} disabled={savingDest} style={{ flex:2,padding:13,background:"#2563EB",border:"none",borderRadius:12,color:"#fff",fontWeight:700,fontSize:14,cursor:"pointer",touchAction:"manipulation",fontFamily:"inherit",opacity:savingDest?0.7:1 }}>{savingDest?"Guardando…":"Guardar"}</button>
            </div>
          </div>
        </div>
      )}
  ...
  return (
    <>
      ...
      {!splash && !!user && <div style={{ position: "fixed", top: 0, left: 0, right: 0, bottom: 0, background: NL.bg, display: "flex", flexDirection: "column", fontFamily: "'DM Sans',sans-serif", color: NL.text, overflow: "hidden", overscrollBehavior: "none" }}>
        ...
        >
          <div style={{ padding: (activeTab === "capital" || activeTab === "contratos") ? "0" : "20px 16px", paddingBottom: "calc(240px + env(safe-area-inset-bottom))" }}>
            {loading
