...
function Dashboard({clientes,contratos,paneles,setTab}){
  ...
  return(<div>
    ...
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16}}>
      ...
      <Card>
        <SecTit ch="📊 Ingresos Últimos 6 Meses"/>
        <BarChart data={INGRESOS_HIST}/>
        <div style={{marginTop:14,paddingTop:14,borderTop:`1px solid ${C.border}`,display:"flex",justifyContent:"space-between",fontSize:13}}>
          <span style={{color:C.muted}}>Total acumulado</span>
          <span style={{fontWeight:700,color:C.white}}>{fmt(INGRESOS_HIST.reduce((a,d)=>a+d.v,0))}</span>
        </div>
      </Card>
    </div>
  </div>);
...
function Paneles({paneles,setPaneles,loading}){
  ...
  const guardar=async()=>{
    ...
    setSaving(true);
    const payload={nombre:form.nombre,tipo:form.tipo,precio:Number(form.precio)||0,estado:form.estado,foto:form.foto||"📡",ciudad:form.ciudad,direccion:form.direccion,lat:form.lat?Number(form.lat):null,lng:form.lng?Number(form.lng):null};
    if(modal==="nuevo"){
