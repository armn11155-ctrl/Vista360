...
export default function App(){
  ...
  return(<>
    ...
    <div style={{fontFamily:"'DM Sans',sans-serif",background:C.bg,height:"100dvh",overflowY:"auto",overflowX:"hidden",color:C.text,display:"flex",flexDirection:"column",WebkitOverflowScrolling:"touch"}}>
      <nav style={{background:"#0A0F1E",borderBottom:"2px solid rgba(255,255,255,0.08)",paddingLeft:8,paddingRight:8,paddingTop:"env(safe-area-inset-top)",display:"flex",alignItems:"center",minHeight:"calc(56px + env(safe-area-inset-top))",position:"sticky",top:0,zIndex:200,overflowX:"auto",scrollbarWidth:"none",flexShrink:0,gap:0}}>
        <style>{`::-webkit-scrollbar{display:none}`}</style>
        <div style={{marginRight:10,display:"flex",alignItems:"center",flexShrink:0,paddingLeft:8}}>
          <Logo360 width={100}/>
        </div>
        {tabs.map(t=>(
          <button key={t.id} onClick={()=>setTab(t.id)} style={{padding:"0 11px",height:56,display:"flex",alignItems:"center",gap:5,fontSize:13,fontWeight:tab===t.id?700:400,color:tab===t.id?"#FFFFFF":"rgba(255,255,255,0.45)",background:"none",border:"none",borderBottom:tab===t.id?"2px solid #3B82F6":"2px solid transparent",cursor:"pointer",whiteSpace:"nowrap",flexShrink:0}}>
            <span style={{fontSize:16}}>{t.icon}</span><span>{t.label}</span>
          </button>
        ))}
        <div style={{marginLeft:"auto",display:"flex",alignItems:"center",gap:8,flexShrink:0,paddingRight:8}}>
          <button onClick={()=>setShowConfig(true)} style={{background:"#18253D",border:`1px solid ${C.border}`,borderRadius:8,padding:"5px 9px",color:C.muted,cursor:"pointer",fontSize:14,display:"flex",alignItems:"center"}}>⚙️</button>
        </div>
      </nav>
